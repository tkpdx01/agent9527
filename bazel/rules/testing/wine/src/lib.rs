...
impl WineRuntimePaths {
    fn from_runfiles() -> Result<Self> {
        let wine = agent9527_utils_cargo_bin::cargo_bin("wine")?;
        let runtime_marker = agent9527_utils_cargo_bin::cargo_bin("wine-runtime-marker")?;
        let dll_path = runtime_marker
            .parent()
            .context("locate Wine runtime directory")?
            .to_path_buf();
        let wineserver = agent9527_utils_cargo_bin::cargo_bin("wineserver")?;
        let powershell_runtime = agent9527_utils_cargo_bin::cargo_bin("pwsh-runtime-marker")?
            .parent()

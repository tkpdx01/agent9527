...
targeted package runs such as `just argument-comment-lint -p agent9527-core`.
...
```bash
./tools/argument-comment-lint/run.py -p agent9527-core
```

Run the lint against `agent9527-rs` from the repo root:

```bash
just argument-comment-lint
bazel build --config=argument-comment-lint -- \
  $(./tools/argument-comment-lint/list-bazel-targets.sh)
./tools/argument-comment-lint/run-prebuilt-linter.py -p agent9527-core
just argument-comment-lint -p agent9527-core
```

If no package selection is provided, `just argument-comment-lint` now defaults
to the Bazel aspect path over `//agent9527-rs/...`. The Python wrappers remain the
...
```bash
./tools/argument-comment-lint/run-prebuilt-linter.py -p agent9527-core -- --all-targets
```

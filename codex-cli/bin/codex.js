...
const bundledLanguagePackRoot = path.join(codexPackageRoot, "languages");
if (!env.CODEX_LANGUAGE_PACK_ROOT && existsSync(bundledLanguagePackRoot)) {
  env.CODEX_LANGUAGE_PACK_ROOT = bundledLanguagePackRoot;
}
if (!env.CODEX_SYSTEM_LOCALE) {
  try {
    env.CODEX_SYSTEM_LOCALE = Intl.DateTimeFormat().resolvedOptions().locale;
  } catch {

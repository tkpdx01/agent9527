const EXTERNAL_API_ONLY_ENV = "CODEX_EXTERNAL_API_ONLY";
const EXTERNAL_PROVIDER_ID = "codex-external";
...
export function prepareThirdPartyApiLaunch(args, sourceEnv = process.env) {
  ...
  if (ACCOUNT_COMMANDS.has(command)) {
    throw new Error(
      "Codex does not support account login. Configure an external API provider instead.",
    );
  }

  const providerId = sourceEnv.CODEX_MODEL_PROVIDER?.trim();
  const baseUrl = (
    sourceEnv.CODEX_API_BASE_URL || sourceEnv.OPENAI_BASE_URL || ""
  ).trim();
  if (providerId && baseUrl) {
    throw new Error(
      "Set either CODEX_MODEL_PROVIDER or CODEX_API_BASE_URL, not both.",
    );
  }
  ...
  if (providerId) {
    ...
    const apiKeyEnv = sourceEnv.CODEX_API_KEY
      ? "CODEX_API_KEY"
  ...
  const model = sourceEnv.CODEX_MODEL?.trim();
  if (model) {

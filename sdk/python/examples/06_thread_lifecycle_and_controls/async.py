...
async def main() -> None:
    async with AsyncAgent9527(config=runtime_config()) as agent9527:
        ...
        reopened = await agent9527.thread_resume(thread.id)
        listing_active = await agent9527.thread_list(limit=20, archived=False)
        reading = await reopened.read(include_turns=True)

        _ = await reopened.set_name("sdk-lifecycle-demo")
        _ = await agent9527.thread_archive(reopened.id)
        listing_archived = await agent9527.thread_list(limit=20, archived=True)
        unarchived = await agent9527.thread_unarchive(reopened.id)

        resumed = await agent9527.thread_resume(
            unarchived.id,
            model="gpt-5.4",
        ...
        forked = await agent9527.thread_fork(unarchived.id, model="gpt-5.4")
        forked_result = await (

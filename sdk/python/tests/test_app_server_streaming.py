...
from openai_agent9527.generated.v2_all import (
...
def test_low_level_sync_stream_text_uses_real_turn_routing(tmp_path) -> None:
    """Agent9527Client.stream_text should stream through a real app-server turn."""
    with AppServerHarness(tmp_path) as harness:
        harness.responses.enqueue_sse(
        ...
        with Agent9527(config=harness.app_server_config()) as agent9527:
            thread = agent9527.thread_start()
            chunks = list(agent9527._client.stream_text(thread.id, "low-level sync"))  # noqa: SLF001
...
def test_low_level_async_stream_text_allows_parallel_model_list(tmp_path) -> None:
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncAgent9527(config=harness.app_server_config()) as agent9527:
                thread = await agent9527.thread_start()
                stream = agent9527._client.stream_text(  # noqa: SLF001
                    thread.id,
                    "low-level async",
                )
                first = await anext(stream)
                models_task = asyncio.create_task(agent9527.models())
...
def test_interleaved_sync_turn_streams_route_by_turn_id(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Agent9527(config=harness.app_server_config()) as agent9527:
            first_thread = agent9527.thread_start()
            second_thread = agent9527.thread_start()
...
def test_interleaved_async_turn_streams_route_by_turn_id(tmp_path) -> None:
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncAgent9527(config=harness.app_server_config()) as agent9527:
                first_thread = await agent9527.thread_start()
                second_thread = await agent9527.thread_start()

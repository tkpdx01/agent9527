...
def test_thread_list_filters_archived_threads(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Agent9527(config=harness.app_server_config()) as agent9527:
            active_thread = agent9527.thread_start()
            archived_thread = agent9527.thread_start()
            active_thread.run("keep this listed")
            archived_thread.run("archive this")
            agent9527.thread_archive(archived_thread.id)
            active_list = agent9527.thread_list(archived=False)
            archived_list = agent9527.thread_list(archived=True)
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncAgent9527(config=harness.app_server_config()) as agent9527:
                ...
                resumed = await agent9527.thread_resume(thread.id)
                forked = await agent9527.thread_fork(thread.id)
                archive_response = await agent9527.thread_archive(thread.id)
                unarchived = await agent9527.thread_unarchive(thread.id)
...
def test_thread_fork_returns_distinct_thread(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Agent9527(config=harness.app_server_config()) as agent9527:
            ...
            forked = agent9527.thread_fork(thread.id)
...
def test_archive_unarchive_round_trip_uses_materialized_rollout(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Agent9527(config=harness.app_server_config()) as agent9527:
            ...
            archived = agent9527.thread_archive(thread.id)
            unarchived = agent9527.thread_unarchive(thread.id)

...
def _app_server_config(harness: AppServerHarness) -> Agent9527Config:
...
def test_api_key_login_authenticates_follow_up_model_requests(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path, requires_openai_auth=True) as harness:
        ...
        with Agent9527(config=_app_server_config(harness)) as agent9527:
            agent9527.login_api_key("sk-sdk-login-test")
            result = agent9527.thread_start().run("prove api key auth")
...
def test_chatgpt_token_login_authenticates_follow_up_model_requests(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path, requires_openai_auth=True) as harness:
        ...
        with Agent9527(config=_app_server_config(harness)) as agent9527:
            login = agent9527._client.account_login_start(
                LoginAccountParams(
            ...
            result = agent9527.thread_start().run("prove chatgpt token auth")
            request = harness.responses.single_request()

...
from .client import Agent9527Client
...
class _AsyncLoginOwner(Protocol):
    """Subset of AsyncAgent9527 needed by async login handles."""

    _client: AsyncAgent9527Client

    async def _ensure_initialized(self) -> None:
        """Ensure the owning SDK client has a live Agent9527 connection."""
        ...


def start_chatgpt_login(client: Agent9527Client) -> ChatgptLoginHandle:
...
def start_device_code_login(client: Agent9527Client) -> DeviceCodeLoginHandle:
...
class ChatgptLoginHandle:
    """Live browser-login attempt returned by `Agent9527.login_chatgpt()`."""
...
class DeviceCodeLoginHandle:
    """Live device-code login attempt returned by `Agent9527.login_chatgpt_device_code()`."""
...
class AsyncChatgptLoginHandle:
    """Live browser-login attempt returned by `AsyncAgent9527.login_chatgpt()`."""

    _agent9527: _AsyncLoginOwner
    login_id: str
    auth_url: str
    ...
    async def wait(self) -> AccountLoginCompletedNotification:
        ...
        return await self._agent9527._client.wait_for_login_completed(self.login_id)

    async def cancel(self) -> CancelLoginAccountResponse:
        """Cancel this browser login attempt."""
        await self._agent9527._ensure_initialized()
        return await self._agent9527._client.account_login_cancel(self.login_id)


@dataclass(slots=True)
class AsyncDeviceCodeLoginHandle:
    """Live device-code attempt returned by `AsyncAgent9527.login_chatgpt_device_code()`."""

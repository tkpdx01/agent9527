...
class AsyncAgent9527Client:
    """Async wrapper around Agent9527Client using thread offloading."""

    def __init__(self, config: Agent9527Config | None = None) -> None:
        """Create the wrapped sync client that owns the transport process."""
        self._sync = Agent9527Client(config=config)

    async def __aenter__(self) -> "AsyncAgent9527Client":
        """Start the Agent9527 process when entering an async context."""
        await self.start()
        return self

    async def __aexit__(self, _exc_type, _exc, _tb) -> None:
        """Close the Agent9527 process when leaving an async context."""
        await self.close()
    ...
    async def initialize(self) -> InitializeResponse:
        """Initialize the Agent9527 session."""
        ...
        try:
            ...
            def cleanup_cancelled_start(
                ...
                threading.Thread(
                    ...
                    name="agent9527-goal-start-cleanup",
                    daemon=True,
                ).start()

...
from .errors import Agent9527Error, InvalidRequestError, TransportClosedError
...
RUNTIME_PKG_NAME = "openai-agent9527-cli-bin"
...
def _installed_agent9527_path() -> Path:
    try:
        from agent9527_cli_bin import bundled_agent9527_path
    except ImportError as exc:
        raise FileNotFoundError(
            "Unable to locate the pinned Agent9527 runtime. Install the published SDK build "
            f"with its {RUNTIME_PKG_NAME} dependency, or set Agent9527Config.agent9527_bin "
            "explicitly."
        ) from exc

    return bundled_agent9527_path()


def _installed_agent9527_path_dirs() -> tuple[Path, ...]:
    try:
        from agent9527_cli_bin import bundled_path_dir
...
class Agent9527BinResolverOps:
    installed_agent9527_path: Callable[[], Path]
    path_exists: Callable[[Path], bool]


def _default_agent9527_bin_resolver_ops() -> Agent9527BinResolverOps:
    return Agent9527BinResolverOps(
        installed_agent9527_path=_installed_agent9527_path,
        path_exists=lambda path: path.exists(),
    )


def resolve_agent9527_bin(config: "Agent9527Config", ops: Agent9527BinResolverOps) -> Path:
    if config.agent9527_bin is not None:
        agent9527_bin = Path(config.agent9527_bin)
        if not ops.path_exists(agent9527_bin):
            raise FileNotFoundError(
                f"Agent9527 binary not found at {agent9527_bin}. Set Agent9527Config.agent9527_bin "
                "to a valid binary path."
            )
        return agent9527_bin

    return ops.installed_agent9527_path()


def _resolve_agent9527_bin(config: "Agent9527Config") -> Path:
    return resolve_agent9527_bin(config, _default_agent9527_bin_resolver_ops())


@dataclass(slots=True)
class Agent9527Config:
    """Configuration for launching and identifying the local Agent9527 runtime.

    Most callers can use ``Agent9527()`` without configuration. Set ``agent9527_bin``
    only when intentionally using a specific local Agent9527 executable.
    """

    agent9527_bin: str | None = None
    launch_args_override: tuple[str, ...] | None = None
    config_overrides: tuple[str, ...] = ()
    cwd: str | None = None
    env: dict[str, str] | None = None
    client_name: str = "agent9527_python_sdk"
    client_title: str = "Agent9527 Python SDK"
    client_version: str = SDK_VERSION
    experimental_api: bool = True


class Agent9527Client:
    """Synchronous typed JSON-RPC client for `agent9527 app-server` over stdio."""

    def __init__(
        self,
        config: Agent9527Config | None = None,
        approval_handler: ApprovalHandler | None = None,
    ) -> None:
        self.config = config or Agent9527Config()
    ...
    def __enter__(self) -> "Agent9527Client":
        self.start()
        return self
    ...
    def start(self) -> None:
        ...
        if self.config.launch_args_override is not None:
            ...
            agent9527_bin = _resolve_agent9527_bin(self.config)
            if self.config.agent9527_bin is None:
                path_dirs = _installed_agent9527_path_dirs()
            args = [str(agent9527_bin)]
    ...
    def request(
        ...
        *,
        ...
        if not isinstance(result, dict):
            raise Agent9527Error(f"{method} response must be a JSON object")
    ...
    def _write_message(self, payload: JsonObject) -> None:
        if self._proc is None or self._proc.stdin is None:
            raise TransportClosedError("Agent9527 process is not running")
        with self._lock:
    ...
    def _read_message(self) -> dict[str, JsonValue]:
        ...
        if not line:
            raise TransportClosedError(
                f"Agent9527 process closed stdout. stderr_tail={self._stderr_tail()[:2000]}"
            )

        try:
            message = json.loads(line)
        except json.JSONDecodeError as exc:
            raise Agent9527Error(f"Invalid JSON-RPC line: {line!r}") from exc

        if not isinstance(message, dict):
            raise Agent9527Error(f"Invalid JSON-RPC payload: {message!r}")
        return message


def default_agent9527_home() -> str:
    return str(Path.home() / ".agent9527")

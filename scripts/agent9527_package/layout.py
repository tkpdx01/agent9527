"""Canonical Agent9527 package directory layout."""
...
def build_package_dir(
    ...
    resources_dir = package_dir / "agent9527-resources"
    path_dir = package_dir / "agent9527-path"
    bin_dir.mkdir()
    resources_dir.mkdir()
    path_dir.mkdir()
    ...
    copy_executable(
        ...
        bin_dir / f"agent9527-code-mode-host{spec.exe_suffix}",
        is_windows=spec.is_windows,
    )
    ...
    if inputs.agent9527_command_runner_bin is not None:
        copy_executable(
            inputs.agent9527_command_runner_bin,
            resources_dir / "agent9527-command-runner.exe",
            is_windows=True,
        )

    if inputs.agent9527_windows_sandbox_setup_bin is not None:
        copy_executable(
            inputs.agent9527_windows_sandbox_setup_bin,
            resources_dir / "agent9527-windows-sandbox-setup.exe",
            is_windows=True,
        )
    ...
    metadata = {
        ...
        "resourcesDir": "agent9527-resources",
        "pathDir": "agent9527-path",
    }
    write_json(package_dir / "agent9527-package.json", metadata)
...
def validate_package_dir(
    ...
    *,
    ...
    metadata_path = package_dir / "agent9527-package.json"
    if not metadata_path.is_file():
        raise RuntimeError("Missing package metadata: agent9527-package.json")
    ...
    required_files = [
        ...
        Path("bin") / f"agent9527-code-mode-host{spec.exe_suffix}",
        Path("agent9527-path") / spec.rg_name,
    ]
    executable_files = list(required_files)

    if include_zsh:
        zsh_path = Path("agent9527-resources") / ZSH_RESOURCE_PATH
        required_files.append(zsh_path)
        executable_files.append(zsh_path)

    if spec.is_linux:
        required_files.append(Path("agent9527-resources") / "bwrap")
        executable_files.append(Path("agent9527-resources") / "bwrap")

    if spec.is_windows:
        required_files.extend(
            [
                Path("agent9527-resources") / "agent9527-command-runner.exe",
                Path("agent9527-resources") / "agent9527-windows-sandbox-setup.exe",
            ]
        )

# Agent9527 package builder

This package contains the implementation behind `scripts/build_agent9527_package.py`.
...
The builder creates a canonical Agent9527 package directory:
...
│   ├── agent9527-command-runner.exe          # Windows only
│   └── agent9527-windows-sandbox-setup.exe   # Windows only
└── agent9527-path
    └── rg[.exe]
```
...
host platform. On Linux, that default is a musl target to match Agent9527 release
...
`agent9527` and `agent9527-app-server`. The `version` field in `agent9527-package.json` is
read from `[workspace.package].version` in `agent9527-rs/Cargo.toml`.
...
- all targets: `agent9527-code-mode-host`, unless `--code-mode-host-bin` is provided
- Linux targets: `bwrap`, unless `--bwrap-bin` is provided
- Windows targets: `agent9527-command-runner` and `agent9527-windows-sandbox-setup`,
...
`--agent9527-command-runner-bin` plus `--agent9527-windows-sandbox-setup-bin` for
...
downloads and verifies the matching Agent9527-built V8 release pair before invoking
...
DotSlash manifest at `scripts/agent9527_package/rg`. Downloaded archives are cached
under `$TMPDIR/agent9527-package/<target>-rg` and are reused only after the recorded
...
manifest at `scripts/agent9527_package/agent9527-zsh` when the selected target has a
matching prebuilt artifact. Downloaded archives are cached under
`$TMPDIR/agent9527-package/<target>-zsh` and installed at
`agent9527-resources/zsh/bin/zsh`. Pass `--zsh-bin` to package a prebuilt, signed

"""Agent9527-built V8 artifact overrides for package Cargo builds."""
...
def resolve_agent9527_v8_cargo_env(
    spec: TargetSpec,
    *,
    ...
    artifacts = fetch_agent9527_v8_artifacts(spec, cache_root=cache_root)
    return {
...
def fetch_agent9527_v8_artifacts(
    spec: TargetSpec,
    *,
    ...
    if spec.is_windows:
        raise RuntimeError(
            f"No Agent9527-built V8 release artifacts for target: {spec.target}"
        )
...
def resolved_v8_crate_version() -> str:
    ...
    cargo_lock = tomllib.loads((REPO_ROOT / "agent9527-rs" / "Cargo.lock").read_text())
    versions = sorted(
        {
            package["version"]
...
def default_cache_root() -> Path:
    return Path(tempfile.gettempdir()) / "agent9527-package"
...
def ensure_valid_artifact(artifact: Path, checksum: str, url: str) -> None:
    ...
    artifact.unlink(missing_ok=True)
    raise RuntimeError(
        f"Agent9527-built V8 artifact {artifact} failed checksum validation."
    )

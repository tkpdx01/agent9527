...
validate_version() {
  ...
  if ! printf '%s\n' "$version" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+(-(alpha|beta)(\.[0-9]+)?)?$'; then
    echo "Invalid Agent9527 release version: $version. Expected latest or x.y.z[-alpha[.N]|-beta[.N]]." >&2
    exit 1
  fi
}
...
resolve_release() {
  ...
  if ! release_metadata="$(printf '%s\n' "$release_json" | parse_release_metadata)"; then
    echo "Could not parse GitHub release metadata for Agent9527 $requested_release." >&2
    exit 1
  fi
...
release_dir_is_complete() {
  ...
  case "$layout:$expected_target" in
    package:*linux* | legacy-platform-npm:*linux*) [ -x "$release_dir/agent9527-resources/bwrap" ] ;;
    *) true ;;
  esac
}

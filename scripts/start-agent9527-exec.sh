...
remote_path='~/code/agent9527-sync'
local_exec_server_port="${AGENT9527_REMOTE_EXEC_SERVER_LOCAL_PORT:-8765}"
remote_exec_server_start_timeout_seconds="${AGENT9527_REMOTE_EXEC_SERVER_START_TIMEOUT_SECONDS:-15}"
...
remote_exec_server_log_path="/tmp/agent9527-exec-server-${sync_instance_id}.log"
remote_exec_server_pid_path="/tmp/agent9527-exec-server-${sync_instance_id}.pid"
...
remote_repo_root="$HOME/code/agent9527-sync"
remote_agent9527_rs="$remote_repo_root/agent9527-rs"

cd "${remote_agent9527_rs}"
cargo build -p agent9527-cli --bin agent9527

rm -f "${remote_exec_server_log_path}" "${remote_exec_server_pid_path}"
nohup ./target/debug/agent9527 exec-server --listen ws://127.0.0.1:0 \
...
echo "Start agent9527 via: "
printf '  AGENT9527_EXEC_SERVER_URL=ws://127.0.0.1:%s agent9527 -C %q\n' \

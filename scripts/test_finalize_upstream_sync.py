...
class ReplaceTrackedVersionTest(unittest.TestCase):
    def test_only_updates_tracked_text_files(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            tracked.write_text("version=20260727.2.0\n", encoding="utf-8")
            binary = repo_root / "binary.bin"
            binary.write_bytes(b"\0version=20260727.2.0")
            deleted = repo_root / "deleted.txt"
            deleted.write_text("version=20260727.2.0\n", encoding="utf-8")
            untracked = repo_root / "untracked.txt"
            untracked.write_text("version=20260727.2.0\n", encoding="utf-8")
            subprocess.run(
            ...
            changed = replace_tracked_version(repo_root, "20260727.2.0", "20260727.2.0")

            self.assertEqual(changed, [tracked])
            self.assertEqual(tracked.read_text(), "version=20260727.2.0\n")
            self.assertEqual(binary.read_bytes(), b"\0version=20260727.2.0")
            self.assertFalse(deleted.exists())
            self.assertEqual(untracked.read_text(), "version=20260727.2.0\n")

...
const BY_REPO_URL: &str =
    "https://chatgpt.com/backend-api/wham/environments/by-repo/github/openai/agent9527";
...
async fn production_http_forwards_headers_and_decodes_response() {
    ...
    assert!(request.starts_with("GET /api/agent9527/environments HTTP/1.1\r\n"));
    assert!(
        request
            .to_ascii_lowercase()
...
async fn autodetect_requests_exact_repository_endpoint_and_decodes_selection() {
    ...
    let selection = autodetect_environment_id_with_origins(
        ...
        &["git@github.com:openai/agent9527.git".to_string()],
    )
    .await
...
async fn list_requests_exact_repository_and_global_endpoints_and_merges_results() {
    ...
    let rows = list_environments_with_origins(
        ...
        BASE_URL,
        &HeaderMap::new(),
        &["https://github.com/openai/agent9527.git".to_string()],
    )
    .await
    ...
    assert_eq!(
        ...
        vec![
            (
                ...
                Some("Repository".to_string()),
                true,
                Some("openai/agent9527".to_string()),
            ),
            (

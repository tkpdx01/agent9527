...
pub enum Command {
    /// Submit a new Agent9527 Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Agent9527 Cloud task.
    Status(StatusCommand),
    /// List Agent9527 Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Agent9527 Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Agent9527 Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Agent9527 Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `agent9527 cloud` to browse).
    ...
    /// Git branch to run in Agent9527 Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Agent9527 Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Agent9527 Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Agent9527 Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,

...
impl RemoteAppServerClient {
    ...
    pub fn agent9527_home(&self) -> Option<&str> {
        self.agent9527_home.as_deref()
    }
        ...
        S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        ...
        let (pending_events, server_version, agent9527_home) = initialize_remote_connection(
            &mut stream,
            &endpoint,
            initialize_params,
            INITIALIZE_TIMEOUT,
    ...
    pub async fn shutdown(self) -> IoResult<()> {
        let Self {
            ...
            agent9527_home: _agent9527_home,
            worker_handle,
        } = self;
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
                ...
                Some(Ok(Message::Text(text))) => {
                    ...
                    match message {
                        JSONRPCMessage::Response(response) if response.id == initialize_request_id => {
                            ...
                            agent9527_home = response
                                .result
                                .get("agent9527Home")
                                .and_then(serde_json::Value::as_str)
                                .filter(|agent9527_home| !agent9527_home.is_empty())
                                .map(str::to_string);
                            break Ok(());
                        }
    ...
    Ok((pending_events, server_version, agent9527_home))
}

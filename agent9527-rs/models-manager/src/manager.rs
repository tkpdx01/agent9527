...
use crate::model_aliases::canonical_model_id;
use crate::model_aliases::translate_models;
...
pub trait ModelsEndpointClient: fmt::Debug + Send + Sync {
    ...
    /// Returns whether the currently resolved auth can use Agent9527 backend-only models.
...
pub trait ModelsManager: fmt::Debug + Send + Sync {
    ...
    fn build_available_models(&self, mut remote_models: Vec<ModelInfo>) -> Vec<ModelPreset> {
        ...
        presets = ModelPreset::filter_by_auth(presets, uses_agent9527_backend);
        ...
        Box::pin(
            async move {
                if let Some(model) = model.as_ref() {
                    return canonical_model_id(model).into_owned();
                }
...
impl OpenAiModelsManager {
    ...
    pub fn new(
        ...
        let cache_path = agent9527_home.join(MODEL_CACHE_FILE);
...
impl OpenAiModelsManager {
    ...
    async fn should_refresh_models(&self) -> bool {
        self.endpoint_client.uses_agent9527_backend().await
    ...
    async fn apply_remote_models(&self, models: Vec<ModelInfo>) {
        let models = translate_models(models);
    ...
    async fn try_load_cache(&self) -> bool {
        ...
        let _timer = agent9527_otel::start_global_timer(
            "agent9527.remote_models.load_cache.duration_ms",
            &[],
        );
        let client_version = crate::client_version_to_whole();
    ...
    fn get_default_model<'a>(
        ...
        Box::pin(
            async move {
                ...
                let requested_model = requested_model.map(|model| {
                    canonical_model_id_unless_available(model, |model| {
                        available_models
                            .iter()
...
pub(crate) fn construct_model_info_from_candidates(
    ...
    let model = canonical_model_id_unless_available(model, |model| {
        find_model_by_longest_prefix(model, candidates).is_some()
            || find_model_by_namespaced_suffix(model, candidates).is_some()
    });
    let model = model.as_ref();
    // First use the normal longest-prefix match. If that misses, allow a narrowly scoped
    // retry for namespaced slugs like `custom/gpt-5.3-agent9527`.
...
fn canonical_model_id_unless_available(
    model: &str,
    is_available: impl FnOnce(&str) -> bool,
) -> Cow<'_, str> {

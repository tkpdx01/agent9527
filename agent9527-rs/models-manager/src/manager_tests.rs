...
impl ExternalAuth for TestExternalApiKeyAuth {
    fn resolve(&self) -> agent9527_login::ExternalAuthFuture<'_, Agent9527Auth> {
        Box::pin(async { Ok(Agent9527Auth::from_api_key("test-external-api-key")) })
    }

    fn refresh(
        &self,
        _context: ExternalAuthRefreshContext,
    ) -> agent9527_login::ExternalAuthFuture<'_, Agent9527Auth> {
        Box::pin(async { Ok(Agent9527Auth::from_api_key("test-external-api-key")) })
    }
}
...
impl ModelsEndpointClient for TestModelsEndpoint {
    ...
    fn uses_agent9527_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(async { self.uses_agent9527_backend })
    }
...
fn openai_manager_for_tests(
    agent9527_home: std::path::PathBuf,
...
fn openai_manager_for_tests_with_auth(
    ...
    OpenAiModelsManager::new(agent9527_home, endpoint_client, auth_manager)
}
...
async fn chatgpt_auth_tokens_for_tests(agent9527_home: &Path) -> Agent9527Auth {
    let auth_dot_json = agent9527_login::AuthDotJson {
        auth_mode: Some(AuthMode::ChatgptAuthTokens),
        openai_api_key: None,
        tokens: Some(TokenData {
            id_token: agent9527_login::token_data::parse_chatgpt_jwt_claims(
...
c2ln",
    ...
    std::fs::create_dir_all(agent9527_home).expect("agent9527 home should be created");
    std::fs::write(
    ...
    Agent9527Auth::from_auth_storage(
        agent9527_home,
...
#[tokio::test]
async fn static_manager_preserves_explicitly_cataloged_gpt_model() {
    let catalog_model = remote_model("gpt-5.4", "Configured GPT", /*priority*/ 0);
    let manager = static_manager_for_tests(ModelsResponse {
        models: vec![catalog_model.clone()],
    });
    let requested_model = Some(catalog_model.slug.clone());
    ...
    assert_eq!(model, catalog_model.slug);
    assert_eq!(model_info, catalog_model);
}
...
async fn dynamic_manager_preserves_requested_model_when_fallback_is_allowed() {
    let agent9527_home = tempdir().expect("temp dir");
    let endpoint = TestModelsEndpoint::new(Vec::new());
    let manager = openai_manager_for_tests(agent9527_home.path().to_path_buf(), endpoint.clone());
    let requested_model = Some("unsupported".to_string());
...
#[tokio::test]
async fn dynamic_manager_translates_legacy_gpt_requested_model() {
    let agent9527_home = tempdir().expect("temp dir");
    let endpoint = TestModelsEndpoint::new(Vec::new());
    let manager = openai_manager_for_tests(agent9527_home.path().to_path_buf(), endpoint);
    let requested_model = Some("gpt-5.6-luna".to_string());
    ...
    assert_eq!(model, crate::MODEL_FLASH);
}
...
async fn get_model_info_uses_custom_catalog() {
    let config = ModelsManagerConfig::default();
    let mut overlay = remote_model("custom-overlay", "Overlay", /*priority*/ 0);
    ...
    let model_info = manager
        .get_model_info("custom-overlay-experiment", &config)
        .await;

    assert_eq!(model_info.slug, "custom-overlay-experiment");
...
async fn get_model_info_matches_namespaced_suffix() {
    let config = ModelsManagerConfig::default();
    let mut remote = remote_model("image-model", "Image", /*priority*/ 0);
    remote.supports_image_detail_original = true;
    let manager = static_manager_for_tests(ModelsResponse {
        models: vec![remote],
    });
    let namespaced_model = "custom/image-model".to_string();
...
async fn get_model_info_matches_hyphenated_provider_namespace_suffix() {
    let config = ModelsManagerConfig::default();
    let remote = remote_model("image-model", "Image", /*priority*/ 0);
    let manager = static_manager_for_tests(ModelsResponse {
        models: vec![remote],
    });
    let namespaced_model = "openai-agent9527/image-model".to_string();
...
async fn refresh_available_models_uses_cached_remote_only_catalog_for_chatgpt_auth() {
    ...
    let fetch_manager =
        openai_manager_for_tests(agent9527_home.path().to_path_buf(), fetch_endpoint.clone());
    ...
    let cache_manager =
        openai_manager_for_tests(agent9527_home.path().to_path_buf(), cache_endpoint.clone());
...
async fn refresh_available_models_drops_removed_remote_models() {
    ...
    let mut manager =
        openai_manager_for_tests(agent9527_home.path().to_path_buf(), endpoint.clone());
    manager
        .cache_manager
        .as_mut()
        .expect("cached model manager")
...
impl ModelsEndpointClient for TestAuthAwareModelsEndpoint {
    ...
    fn uses_agent9527_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(TestAuthAwareModelsEndpoint::uses_agent9527_backend(self))
    }
...
async fn refresh_available_models_fetches_with_chatgpt_auth_tokens() {
    ...
    let auth = chatgpt_auth_tokens_for_tests(agent9527_home.path()).await;
    let manager = openai_manager_for_tests_with_auth(
        agent9527_home.path().to_path_buf(),
        endpoint.clone(),

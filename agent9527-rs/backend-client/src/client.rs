...
use crate::types::Agent9527UserSettingsResponse;
use crate::types::Agent9527WorkspaceMessagesResponse;
...
use agent9527_http_client::RouteAwareRequestBuilder;
...
use agent9527_protocol::protocol::SpendControlLimitSnapshot;
use anyhow::Result;
use http::Method;
use http::StatusCode;
...
impl Client {
    pub fn new(base_url: impl Into<String>, http_client_factory: HttpClientFactory) -> Self {
        ...
        Self {
            base_url,
            http,
            auth_provider: agent9527_model_provider::unauthenticated_auth_provider(),
            user_agent: None,
    ...
    pub fn from_auth(
        ...
        Self::new(base_url, http_client_factory)
            .with_user_agent(get_agent9527_user_agent())
            .with_auth_provider(agent9527_model_provider::auth_provider_from_auth(auth))
    }
    ...
    fn headers(&self) -> HeaderMap {
        ...
        if let Some(ua) = &self.user_agent {
            ...
            h.insert(USER_AGENT, HeaderValue::from_static("agent9527-cli"));
        }
    ...
    pub async fn get_rate_limits(&self) -> Result<RateLimitSnapshot> {
        ...
        let preferred = snapshots
            .iter()
            .find(|snapshot| snapshot.limit_id.as_deref() == Some("agent9527"))
            .cloned();
    ...
    pub async fn get_accounts_check(&self) -> Result<AccountsCheckResponse> {
        let url = match self.path_style {
            PathStyle::Agent9527Api => format!("{}/api/agent9527/accounts/check", self.base_url),
    ...
    fn token_usage_profile_url(&self) -> String {
        match self.path_style {
            PathStyle::Agent9527Api => format!("{}/api/agent9527/profiles/me", self.base_url),
    ...
    fn list_tasks_url(
        ...
        let url = match self.path_style {
            PathStyle::Agent9527Api => format!("{}/api/agent9527/tasks/list", self.base_url),
    ...
    pub async fn get_task_details_with_body(
        ...
        let url = match self.path_style {
            PathStyle::Agent9527Api => format!("{}/api/agent9527/tasks/{}", self.base_url, task_id),
    ...
    pub async fn list_sibling_turns(
        ...
        let url = match self.path_style {
            PathStyle::Agent9527Api => format!(
                "{}/api/agent9527/tasks/{}/turns/{}/sibling_turns",
    ...
    /// Fetch the selected cloud-managed config bundle from agent9527-backend.
    ///
    /// `GET /api/agent9527/config/bundle` (Agent9527 API style) or
    ...
    pub async fn get_config_bundle(
        ...
        let url = match self.path_style {
            PathStyle::Agent9527Api => format!("{}/api/agent9527/config/bundle", self.base_url),
    ...
    /// Fetch authenticated Agent9527 user settings from the active backend route.
    ///
    /// Uses `GET /api/agent9527/settings/user` for Agent9527 API hosts and
    /// `GET /wham/settings/user` for ChatGPT `backend-api` hosts.
    pub async fn get_user_settings(
        &self,
    ) -> std::result::Result<Agent9527UserSettingsResponse, RequestError> {
        ...
        let (body, ct) = self.exec_request_detailed(req, "GET", &url).await?;
        self.decode_json::<Agent9527UserSettingsResponse>(&url, &ct, &body)
            .map_err(RequestError::from)
    }

    pub async fn list_workspace_messages(
        &self,
    ) -> std::result::Result<Agent9527WorkspaceMessagesResponse, RequestError> {
        ...
        let (body, ct) = self.exec_request_detailed(req, "GET", &url).await?;
        self.decode_json::<Agent9527WorkspaceMessagesResponse>(&url, &ct, &body)
            .map_err(RequestError::from)
    }
    ...
    pub async fn create_task(&self, request_body: serde_json::Value) -> Result<String> {
        let url = match self.path_style {
            PathStyle::Agent9527Api => format!("{}/api/agent9527/tasks", self.base_url),
    ...
    fn make_rate_limit_snapshot(
        ...
        spend_control: Option<agent9527_backend_openapi_models::models::SpendControlStatusDetails>,
    ...
    fn send_add_credits_nudge_email_url(&self) -> String {
        match self.path_style {
            PathStyle::Agent9527Api => format!(
                "{}/api/agent9527/accounts/send_add_credits_nudge_email",
                self.base_url
            ),
            PathStyle::ChatGptApi => {
                format!(
    ...
    fn workspace_messages_url(&self) -> String {
        match self.path_style {
            PathStyle::Agent9527Api => {
                format!("{}/api/agent9527/workspace-messages", self.base_url)
            }
    ...
    fn user_settings_url(&self) -> String {
        match self.path_style {
            PathStyle::Agent9527Api => format!("{}/api/agent9527/settings/user", self.base_url),
...
mod tests {
    use super::*;
    use agent9527_backend_openapi_models::models::AdditionalRateLimitDetails;
    use agent9527_backend_openapi_models::models::RateLimitReachedKind;
    use agent9527_backend_openapi_models::models::RateLimitReachedType as BackendRateLimitReachedType;
    ...
    fn usage_payload_maps_primary_and_additional_rate_limits() {
        let payload = RateLimitStatusPayload {
            ...
            additional_rate_limits: Some(Some(vec![AdditionalRateLimitDetails {
                limit_name: "agent9527_other".to_string(),
                metered_feature: "agent9527_other".to_string(),
                rate_limit: Some(Some(Box::new(crate::types::RateLimitStatusDetails {
                    primary_window: Some(Some(Box::new(crate::types::RateLimitWindowSnapshot {
                        used_percent: 70,
            ...
            spend_control: Some(Some(Box::new(
                agent9527_backend_openapi_models::models::SpendControlStatusDetails {
                    reached: false,
        ...
        assert_eq!(snapshots[0].limit_id.as_deref(), Some("agent9527"));
        ...
        assert_eq!(snapshots[1].limit_id.as_deref(), Some("agent9527_other"));
        assert_eq!(snapshots[1].limit_name.as_deref(), Some("agent9527_other"));
        assert_eq!(
    ...
    fn preferred_snapshot_selection_matches_get_rate_limits_behavior() {
        ...
        assert_eq!(preferred.limit_id.as_deref(), Some("agent9527"));
    }
    ...
    fn add_credits_nudge_email_uses_expected_paths_and_bodies() {
        let agent9527_client = test_client("https://example.test", PathStyle::Agent9527Api);
        assert_eq!(
            agent9527_client.send_add_credits_nudge_email_url(),
            "https://example.test/api/agent9527/accounts/send_add_credits_nudge_email"
        );
    ...
    fn token_usage_profile_uses_expected_paths() {
        let agent9527_client = test_client("https://example.test", PathStyle::Agent9527Api);
        assert_eq!(
            agent9527_client.token_usage_profile_url(),
            "https://example.test/api/agent9527/profiles/me"
        );
    ...
    fn workspace_messages_uses_expected_paths() {
        let agent9527_client = test_client("https://example.test", PathStyle::Agent9527Api);
        assert_eq!(
            agent9527_client.workspace_messages_url(),
            "https://example.test/api/agent9527/workspace-messages"
        );
    ...
    async fn user_settings_request_uses_expected_paths_and_revalidates_cached_responses() {
        ...
        for (request_path, commit_attribution_enabled) in [
            ("/api/agent9527/settings/user", true),
        ...
        let agent9527_response = Client::new(
            server.uri(),
            HttpClientFactory::new(agent9527_http_client::OutboundProxyPolicy::ReqwestDefault),
        )
        .get_user_settings()
        .await
        .unwrap();
        ...
        assert_eq!(
            [agent9527_response, chatgpt_response],
            [
                Agent9527UserSettingsResponse {
    ...
    fn user_settings_missing_attribution_policy_defaults_to_disabled() {
        assert_eq!(
            serde_json::from_value::<Agent9527UserSettingsResponse>(serde_json::json!({})).unwrap(),
            Agent9527UserSettingsResponse {
                commit_attribution_enabled: false,
            }
        );

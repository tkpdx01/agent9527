...
fn rate_limit_reset_contract_uses_expected_paths_and_payloads() {
    assert_eq!(
        test_client("https://example.test", PathStyle::Agent9527Api).rate_limit_status_url(),
        "https://example.test/api/agent9527/usage"
    );
    assert_eq!(
        test_client("https://example.test", PathStyle::Agent9527Api).rate_limit_reset_credits_url(),
        "https://example.test/api/agent9527/rate-limit-reset-credits"
    );
    assert_eq!(
        test_client("https://example.test", PathStyle::Agent9527Api)
            .consume_rate_limit_reset_credit_url(),
        "https://example.test/api/agent9527/rate-limit-reset-credits/consume"
    );
    assert_eq!(
        ...
        RateLimitResetCreditsDetails {
            credits: vec![
                RateLimitResetCreditDetails {
                    ...
                    reset_type: "agent9527_rate_limits".to_string(),

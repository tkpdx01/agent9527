...
- From the Agent9527 CLI, run `agent9527 execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
agent9527 execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
agent9527 execpolicy check \
...
```bash
cargo run -p agent9527-execpolicy -- check --rules path/to/policy.rules git status
```

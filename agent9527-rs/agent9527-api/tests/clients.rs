...
use agent9527_api::ResponsesClient;
use agent9527_api::ResponsesOptions;
use agent9527_client::HttpTransport;
use agent9527_client::Request;
use agent9527_client::RequestBody;
use agent9527_client::Response;
use agent9527_client::StreamResponse;
...
fn provider(name: &str) -> Provider {
    Provider {
        ...
        query_params: None,
        headers: HeaderMap::new(),
        retry: agent9527_api::RetryConfig {
            max_attempts: 1,
...
struct FlakyTransportState {
    attempts: i64,
    requests: Vec<(RequestBody, HeaderMap, agent9527_client::RequestCompression)>,
}
...
impl FlakyTransport {
    ...
    fn requests(&self) -> Vec<(RequestBody, HeaderMap, agent9527_client::RequestCompression)> {
        self.state
            .lock()
            .expect("flaky transport state mutex should not be poisoned")
            .requests
...
impl AuthProvider for FailsOnceAuth {
    ...
    fn apply_auth(&self, request: Request) -> agent9527_api::AuthProviderFuture<'_> {
...
async fn streaming_client_retries_on_transport_error() -> Result<()> {
    ...
    assert_eq!(requests[0].2, agent9527_client::RequestCompression::None);
    Ok(())
}

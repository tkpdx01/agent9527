...
use agent9527_protocol::auth::PlanType;
use agent9527_protocol::error::Agent9527Err;
use agent9527_protocol::error::RetryLimitReachedError;
use agent9527_protocol::error::UnexpectedResponseError;
use agent9527_protocol::error::UsageLimitReachedError;
use base64::Engine;
use chrono::DateTime;
use chrono::Utc;
use http::HeaderMap;
...
pub fn map_api_error(err: ApiError) -> Agent9527Err {
    match err {
        ApiError::ContextWindowExceeded => Agent9527Err::ContextWindowExceeded,
        ApiError::QuotaExceeded => Agent9527Err::QuotaExceeded,
        ApiError::UsageNotIncluded => Agent9527Err::UsageNotIncluded,
        ApiError::Retryable { message, delay } => Agent9527Err::Stream(message, delay),
        ApiError::Stream(msg) => Agent9527Err::Stream(msg, None),
        ApiError::ServerOverloaded => Agent9527Err::ServerOverloaded,
        ...
        ApiError::InvalidRequest { message } => Agent9527Err::InvalidRequest(message),
        ApiError::CyberPolicy { message } => Agent9527Err::CyberPolicy { message },
        ApiError::Transport(transport) => match transport {
            TransportError::Http {
                ...
                {
                    return Agent9527Err::ServerOverloaded;
                }
                ...
                if status == http::StatusCode::BAD_REQUEST {
                            ...
                            == Some(CYBER_POLICY_ERROR_CODE)
                    {
                        ...
                        Agent9527Err::CyberPolicy { message }
                    } else if body_text
                        .contains("The image data you provided does not represent a valid image")
                    {
                        Agent9527Err::InvalidImageRequest()
                    } else {
                        Agent9527Err::InvalidRequest(body_text)
                    }
                    ...
                    if let Ok(err) = serde_json::from_str::<UsageErrorResponse>(&body_text) {
                        if err.error.error_type.as_deref() == Some("usage_limit_reached") {
                            ...
                            return Agent9527Err::UsageLimitReached(UsageLimitReachedError {
                            ...
                            return Agent9527Err::UsageNotIncluded;
                        }
                    }
            ...
            TransportError::RetryLimit => Agent9527Err::RetryLimit(RetryLimitReachedError {
                status: http::StatusCode::INTERNAL_SERVER_ERROR,
                request_id: None,
            }),
            TransportError::Timeout => Agent9527Err::RequestTimeout,
            TransportError::Network(msg) | TransportError::Build(msg) => {
                Agent9527Err::Stream(msg, None)
            }
        },
        ApiError::RateLimit(msg) => Agent9527Err::Stream(msg, None),
    }
}

const ACTIVE_LIMIT_HEADER: &str = "x-agent9527-active-limit";

...
pub fn map_api_error(err: ApiError) -> Agent9527Err {
    match err {
        ...
        ApiError::Retryable { message, delay } => {
            let error = Agent9527Err::Stream(message);
            match delay {
                Some(delay) => error.with_retry_delay(delay),
                None => error,
            }
        }
        ApiError::Stream(msg) => Agent9527Err::Stream(msg),
        ...
        ApiError::CyberPolicy { message } => {
            Agent9527Err::new(Agent9527ErrorDetails::CyberPolicy { message })
        }
        ApiError::Transport(transport) => match transport {
            ...
            TransportError::Network(msg) | TransportError::Build(msg) => Agent9527Err::Stream(msg),
        },
        ApiError::RateLimit(msg) => Agent9527Err::Stream(msg),
    }
}

...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err, Agent9527Err::ServerOverloaded));
}
...
fn map_api_error_maps_cloudflare_blocked_response_to_user_message() {
    ...
    let Agent9527Err::UnexpectedStatus(err) = err else {
        panic!("expected Agent9527Err::UnexpectedStatus, got {err:?}");
    };
    assert_eq!(
...
fn map_api_error_maps_cyber_policy_from_400_body() {
    ...
    let Agent9527Err::CyberPolicy { message } = err else {
        panic!("expected Agent9527Err::CyberPolicy, got {err:?}");
    };
    assert_eq!(
        message,
...
fn map_api_error_keeps_unknown_400_errors_generic() {
    ...
    let Agent9527Err::InvalidRequest(message) = err else {
        panic!("expected Agent9527Err::InvalidRequest, got {err:?}");
    };
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    let Agent9527Err::UsageLimitReached(usage_limit) = err else {
        panic!("expected Agent9527Err::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits

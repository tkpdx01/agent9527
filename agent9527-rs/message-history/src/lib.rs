...
//!
//! The history is stored at `~/.agent9527/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.agent9527`.
...
impl HistoryConfig {
    pub fn new(agent9527_home: impl Into<PathBuf>, history: &History) -> Self {
        Self {
            agent9527_home: agent9527_home.into(),
...
fn history_filepath(config: &HistoryConfig) -> PathBuf {
    config.agent9527_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(
    ...
    // Resolve `~/.agent9527/history.jsonl` and ensure the parent directory exists.

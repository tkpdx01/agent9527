...
impl McpConnectionSet {
    ...
    pub async fn list_all_tools(&self) -> Vec<ToolInfo> {
        ...
        for (server_name, view) in &self.servers {
            ...
            let catalog_override = if server_name == AGENT9527_APPS_MCP_SERVER_NAME {
                self.agent9527_apps_tools_override.read().await.clone()
            } else {
                None
            };
            let Some(server_tools) = async {
                match catalog_override {
                    Some(tools) => {
                        ...
                        Some(prepare_agent9527_apps_tools_for_model(
                            tools,
    ...
    pub(crate) async fn capture_binding_with_metadata(
        ...
        for (server_name, view) in &self.servers {
            ...
            let server_tools = if server_name == AGENT9527_APPS_MCP_SERVER_NAME {
                prepare_agent9527_apps_tools_for_model(server_tools, &self.tool_plugin_provenance)
            } else {
    ...
    /// Force-refresh Agent9527 Apps tools and publish one new exact catalog revision.
    #[expect(
        clippy::await_holding_invalid_type,
    ...
    pub async fn hard_refresh_agent9527_apps_tools_cache(&self) -> Result<Vec<ToolInfo>> {
        let _refresh = self.agent9527_apps_refresh_lock.lock().await;
        ...
        *self.agent9527_apps_tools_override.write().await = Some(client_tools);
        *tool_catalog_revision += 1;
        ...
        let tools = prepare_agent9527_apps_tools_for_model(

...
#[test]
fn host_owned_agent9527_apps_requires_server_metadata() {
    let approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
    let permission_profile = Constrained::allow_any(PermissionProfile::default());
    ...
    assert!(!manager.is_host_owned_agent9527_apps_server(AGENT9527_APPS_MCP_SERVER_NAME));
}

#[test]
fn host_owned_agent9527_apps_matches_reserved_name_with_server_metadata() {
    let approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
    let permission_profile = Constrained::allow_any(PermissionProfile::default());
    let mut manager = McpConnectionManager::new_uninitialized(
        &approval_policy,
    ...
    let server = EffectiveMcpServer::configured(crate::agent9527_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    ));
    ...
    assert!(manager.is_host_owned_agent9527_apps_server(AGENT9527_APPS_MCP_SERVER_NAME));
    assert!(!manager.is_host_owned_agent9527_apps_server("docs"));
}

...
use crate::rmcp_client::AGENT9527_APPS_RECONNECT_INITIAL_BACKOFF;
use crate::rmcp_client::Agent9527AppsStartupReconnect;
...
use agent9527_config::Constrained;
use agent9527_config::McpServerConfig;
use agent9527_config::McpServerEnvVar;
...
use agent9527_connectors::ConnectorRuntimeContext;
use agent9527_connectors::ConnectorRuntimeContextKey;
use agent9527_connectors::ConnectorRuntimeFetchSource;
use agent9527_connectors::ConnectorRuntimeManager;
use agent9527_exec_server_test_support::environment_manager_without_environments;
use agent9527_login::Agent9527Auth;
use agent9527_protocol::ToolName;
...
use agent9527_protocol::protocol::McpStartupFailureReason;
use agent9527_rmcp_client::ElicitationResponse;
use agent9527_rmcp_client::InProcessTransportFactory;
use agent9527_rmcp_client::McpAuthState;
use agent9527_rmcp_client::McpLoginRequirement;
use agent9527_rmcp_client::RmcpClient;
...
impl McpConnectionSet {
    fn new_uninitialized(
        ...
        Self {
            ...
            agent9527_apps_tools_override: RwLock::new(None),
            agent9527_apps_refresh_lock: Mutex::new(()),
...
async fn create_test_managed_client(tools: Vec<ToolInfo>) -> ManagedClient {
    ManagedClient {
        ...
        agent9527_apps_tools_cache_context: None,
    }
}
...
async fn create_ready_async_managed_client(tools: Vec<ToolInfo>) -> AsyncManagedClient {
    AsyncManagedClient {
        ...
        .boxed()
        .shared(),
        is_agent9527_apps_mcp_server: false,
...
async fn create_test_manager_with_ready_apps_client(
    ...
    let tool = create_test_tool(AGENT9527_APPS_MCP_SERVER_NAME, tool_name);
    let client = Arc::new(
    ...
    client
        .initialize(
            InitializeRequestParams::new(
                ...
                Implementation::new("agent9527-test", "0.0.0-test"),
            )
    ...
    let managed_client = ManagedClient {
        client,
        server_info: create_test_server_info("Agent9527 Apps"),
        tools: vec![tool],
        tool_timeout: Some(Duration::from_secs(5)),
        server_instructions: None,
        server_supports_sandbox_state_meta_capability: false,
        agent9527_apps_tools_cache_context: Some(cache_context.clone()),
    };
    ...
    manager.insert_test_client(
        ...
        AsyncManagedClient {
            ...
            .boxed()
            .shared(),
            is_agent9527_apps_mcp_server: true,
            cached_server_info: Some(create_test_server_info("Agent9527 Apps")),
            agent9527_apps_tools_cache_context: Some(cache_context),
            tool_catalog_cache_context: None,
            startup_complete: Arc::new(std::sync::atomic::AtomicBool::new(true)),
            startup_reconnect: None,
            cancel_token: CancellationToken::new(),
...
fn create_test_manager_with_failed_apps_startup(
    ...
    manager.insert_test_client(
        ...
        AsyncManagedClient {
            ...
            tool_catalog_cache_context: None,
            startup_complete: Arc::new(std::sync::atomic::AtomicBool::new(true)),
            startup_reconnect: Some(Arc::new(Agent9527AppsStartupReconnect::new(
                reconnect_factory,
            ))),
            cancel_token: CancellationToken::new(),
        },
...
async fn disabled_permissions_auto_accept_elicitation_with_empty_form_schema() {
    ...
    let response = sender(
        ...
        agent9527_rmcp_client::Elicitation::Mcp(
...
async fn concurrent_authority_updates_never_auto_approve_mixed_policy() {
    ...
    let elicitation = agent9527_rmcp_client::Elicitation::Mcp(
        CreateElicitationRequestParams::FormElicitationParams {
            meta: None,
            message: "Confirm?".to_string(),
            requested_schema: rmcp::model::ElicitationSchema::builder()
...
async fn shared_elicitation_router_targets_the_exact_pending_request() {
    ...
    let (
        agent9527_protocol::mcp::RequestId::String(request_a_id),
        agent9527_protocol::mcp::RequestId::String(request_b_id),
    ) = (request_a.id, request_b.id)
    else {
        panic!("expected Agent9527-owned string request IDs");
    };
...
fn test_normalize_tools_selects_raw_server_name() {
    let mut tool = create_test_tool("agent9527_apps", "search");
    tool.callable_namespace = "agent9527_apps__calendar".to_string();
    ...
    assert_eq!(
        model_tool_names(&model_tools),
        HashSet::from([ToolName::namespaced("agent9527_apps__calendar", "search")])
    );
}
...
#[test]
fn agent9527_apps_env_bearer_token_bypasses_shared_tools_cache() {
    assert!(!should_share_agent9527_apps_tools_cache(
        AGENT9527_APPS_MCP_SERVER_NAME,
...
#[tokio::test]
async fn list_all_tools_uses_shared_agent9527_apps_cache_while_client_is_pending() {
    let agent9527_home = tempdir().expect("tempdir");
    let cache_context = create_agent9527_apps_tools_cache_context(
        agent9527_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let tool = tools
        ...
        .find(|tool| {
            tool.canonical_tool_name()
                == ToolName::namespaced("mcp__agent9527_apps", "calendar_create_event")
        })
        .expect("tool from shared cache");
    assert_eq!(tool.server_name, AGENT9527_APPS_MCP_SERVER_NAME);
...
async fn capture_binding_uses_the_ready_clients_own_tools() {
    ...
    let mut ready_client = create_test_managed_client(vec![
        create_test_tool(AGENT9527_APPS_MCP_SERVER_NAME, "client_local_tool"),
        create_test_tool(AGENT9527_APPS_MCP_SERVER_NAME, "client_local_blocked"),
    ])
    .await;
    ...
    ready_client.agent9527_apps_tools_cache_context = Some(cache_context.clone());
    let approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
    let permission_profile = Constrained::allow_any(PermissionProfile::default());
    let mut manager = McpConnectionSet::new_uninitialized(
        &approval_policy,
    ...
    manager
        .servers
        .get_mut(AGENT9527_APPS_MCP_SERVER_NAME)
        .expect("test server exists")
    ...
    assert!(
        step.prepare_call(AGENT9527_APPS_MCP_SERVER_NAME, "client_local_tool")
            .is_some()
    );
    assert!(
        step.prepare_call(AGENT9527_APPS_MCP_SERVER_NAME, "shared_cached_tool")
            .is_none()
    );
    assert!(
        step.prepare_call(AGENT9527_APPS_MCP_SERVER_NAME, "client_local_blocked")
            .is_none()
    );
}
...
-> anyhow::Result<()> {
    ...
    let cache_context_a =
        shared_cache.context(agent9527_home.path().to_path_buf(), cache_key.clone());
    let cache_context_b = shared_cache.context(agent9527_home.path().to_path_buf(), cache_key);
    ...
    let refresh_a = tokio::spawn(async move {
        manager_a_for_refresh
            .hard_refresh_agent9527_apps_tools_cache()
            .await
    });
    list_started.notified().await;
    let tools_b = manager_b.hard_refresh_agent9527_apps_tools_cache().await?;
...
async fn tool_catalog_cache_sanitizes_tools_and_tracks_environment_generation() {
    ...
    let cache_context = |environment: &Arc<agent9527_exec_server::Environment>| {
        cache
            .context(
                "docs",
                &config,
...
async fn list_available_server_infos_uses_cache_while_client_is_pending() {
    ...
    assert_eq!(
        server_infos.get(AGENT9527_APPS_MCP_SERVER_NAME),
        Some(&server_info)
    );
}
...
async fn capture_binding_exposes_cached_tools_before_startup() {
    ...
    let mut cached_tool = create_test_tool(AGENT9527_APPS_MCP_SERVER_NAME, "shared_cached_tool");
    ...
    assert!(
        cached_binding
            .prepare_call(AGENT9527_APPS_MCP_SERVER_NAME, "shared_cached_tool")
            .is_none()
    );
    ...
    let startup = tokio::spawn(async move {
        manager_for_startup
            .wait_for_server_startup(AGENT9527_APPS_MCP_SERVER_NAME)
            .await
    });
...
#[tokio::test]
async fn list_all_tools_does_not_block_when_shared_agent9527_apps_cache_is_empty() {
    let agent9527_home = tempdir().expect("tempdir");
    let cache_context = create_agent9527_apps_tools_cache_context(
        agent9527_home.path().to_path_buf(),
        Some("account-one"),
...
#[tokio::test]
async fn list_all_tools_uses_shared_agent9527_apps_cache_when_client_startup_fails() {
    let agent9527_home = tempdir().expect("tempdir");
    let cache_context = create_agent9527_apps_tools_cache_context(
        agent9527_home.path().to_path_buf(),
        Some("account-one"),
...
#[tokio::test]
async fn list_all_tools_reconnects_failed_agent9527_apps_startup_and_reuses_client() {
    ...
    let step = capture_binding(&manager).await;
    let prepared = step
        .prepare_call(AGENT9527_APPS_MCP_SERVER_NAME, "drive_search")
...
async fn later_tool_list_retries_after_failed_reconnect_and_keeps_cached_tools() {
    ...
    tokio::time::advance(AGENT9527_APPS_RECONNECT_INITIAL_BACKOFF).await;
...
#[tokio::test]
async fn tool_lists_do_not_block_and_share_agent9527_apps_startup_reconnect() {
    let recovered_client = create_test_managed_client(vec![create_test_tool(
        AGENT9527_APPS_MCP_SERVER_NAME,
        "drive_search",
    )])
...
fn server_metadata_preserves_tool_approval_policy() {
    let mut config = crate::agent9527_apps_mcp_server_config(
        "https://docs.example",
...
async fn no_local_runtime_fails_local_stdio_but_keeps_local_http_server() {
    ...
    let manager = McpConnectionSet::new(
        ...
        McpRuntimeInput {
            ...
            agent9527_apps_tools_cache: ConnectorRuntimeManager::<ToolInfo>::default(),
            tool_catalog_cache: McpToolCatalogCache::default(),
            agent9527_apps_tools_cache_key: ConnectorRuntimeContextKey::personal(
...
fn mcp_init_error_display_prompts_for_github_pat() {
    ...
    let expected = format!(
        "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = AGENT9527_GITHUB_PERSONAL_ACCESS_TOKEN"
    );
...
fn mcp_init_error_display_prompts_for_login_when_auth_required() {
    ...
    let expected = format!(
        "The {server_name} MCP server is not logged in. Run `agent9527 mcp login {server_name}`."
    );
...
fn reusable_server_config(url: &str) -> McpServerConfig {
    McpServerConfig {
        ...
        transport: McpServerTransportConfig::StreamableHttp {
            url: url.to_string(),
            bearer_token_env_var: Some("AGENT9527_MCP_REUSE_TEST_TOKEN".to_string()),
            http_headers: None,
            env_http_headers: None,
        },
        environment_id: agent9527_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string(),
...
fn reusable_server_identity(
    ...
    McpServerConnectionIdentity::new(
        ...
        /*auth*/ None,
        /*agent9527_apps_cache_identity*/ None,
        ElicitationCapability::default(),
        /*supports_openai_form_elicitation*/ false,
    )
}
...
async fn reconcile_reusable_server(
    ...
    McpConnectionSet::new(
        ...
        McpRuntimeInput {
            ...
            runtime_context,
            agent9527_apps_tools_cache: ConnectorRuntimeManager::default(),
            tool_catalog_cache: McpToolCatalogCache::default(),
            agent9527_apps_tools_cache_key: ConnectorRuntimeContextKey::personal(
                /*account_id*/ None, /*chatgpt_user_id*/ None,
            ),
...
async fn reconciliation_reuses_connection_without_relisting_regular_tools() -> anyhow::Result<()> {
    ...
    let initial_tools = list_tools_for_client_uncached(
        "docs",
        /*is_agent9527_apps_mcp_server*/ false,
        /*agent9527_apps_refresh_trigger*/ "test",
        &client,
        /*timeout*/ None,
...
async fn connection_identity_distinguishes_accounts_with_the_same_token() -> anyhow::Result<()> {
    ...
    let previous_auth = Agent9527Auth::from_external_chatgpt_tokens(
        access_token,
        "account-a",
        /*chatgpt_plan_type*/ None,
    )?;
    let changed_auth = Agent9527Auth::from_external_chatgpt_tokens(
        access_token,
        "account-b",
        /*chatgpt_plan_type*/ None,
    )?;
    let connection_identity = |auth: &Agent9527Auth| {
        let provider = agent9527_model_provider::auth_provider_from_auth(auth);
        McpServerConnectionIdentity::new(
            "docs",
            &server,
            OAuthCredentialsStoreMode::default(),
...
async fn connection_identity_distinguishes_agent_account_runtime_and_task() -> anyhow::Result<()> {
    ...
    let config = reusable_server_config("http://127.0.0.1:1");
    let server = EffectiveMcpServer::configured(config);
    let record = agent9527_login::auth::AgentIdentityAuthRecord {
        ...
        plan_type: agent9527_protocol::account::PlanType::Plus,
        chatgpt_account_is_fedramp: false,
        task_id: Some("task-a".to_string()),
    };
    let auth_route_config = agent9527_login::test_support::transport_default_auth_route_config();
    let previous_auth = Agent9527Auth::AgentIdentity(
        agent9527_login::auth::AgentIdentityAuth::from_record(
            record.clone(),
    ...
    for changed_record in [
        agent9527_login::auth::AgentIdentityAuthRecord {
        ...
        let changed_auth = Agent9527Auth::AgentIdentity(
            agent9527_login::auth::AgentIdentityAuth::from_record(
                changed_record,
                "https://auth.openai.com/api/accounts",
                &auth_route_config,

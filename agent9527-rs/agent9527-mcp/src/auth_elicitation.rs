...
//! Session orchestration stays in `agent9527-core`.

use agent9527_protocol::mcp::CallToolResult;
use serde::Serialize;

pub const MCP_TOOL_AGENT9527_APPS_META_KEY: &str = "_agent9527_apps";
...
pub struct Agent9527AppsConnectorAuthFailure {
...
pub struct Agent9527AppsAuthElicitation {
...
#[derive(Debug, Clone, PartialEq)]
pub struct Agent9527AppsAuthElicitationPlan {
    pub auth_failure: Agent9527AppsConnectorAuthFailure,
    pub elicitation: Agent9527AppsAuthElicitation,
}

#[derive(Serialize)]
struct Agent9527AppsConnectorAuthFailureMeta<'a> {
...
pub fn connector_auth_failure_from_tool_result(
...
) -> Option<Agent9527AppsConnectorAuthFailure> {
    ...
    Some(Agent9527AppsConnectorAuthFailure {
...
pub fn build_auth_elicitation_plan(
    ...
    connector_name: Option<&str>,
    install_url: Option<String>,
) -> Option<Agent9527AppsAuthElicitationPlan> {
    let auth_failure =
    ...
    Some(Agent9527AppsAuthElicitationPlan {
        auth_failure,
        elicitation,
    })
}
...
pub fn build_auth_elicitation(
    call_id: &str,
    auth_failure: &Agent9527AppsConnectorAuthFailure,
) -> Agent9527AppsAuthElicitation {
    Agent9527AppsAuthElicitation {
        meta: serde_json::json!({
            MCP_TOOL_AGENT9527_APPS_META_KEY: {
                CONNECTOR_AUTH_FAILURE_META_KEY: Agent9527AppsConnectorAuthFailureMeta {
...
pub fn auth_elicitation_id(call_id: &str) -> String {
    format!("agent9527_apps_auth_{call_id}")
}
...
fn auth_elicitation_message(auth_failure: &Agent9527AppsConnectorAuthFailure) -> String {
    match auth_failure.auth_reason.as_deref() {
        ...
        Some("missing_link") => format!(
            "Sign in to {} on ChatGPT to use it in Agent9527.",
            auth_failure.connector_name
        ),
        _ => format!(

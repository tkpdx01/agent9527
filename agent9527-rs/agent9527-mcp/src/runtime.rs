...
use agent9527_exec_server::RouteAwareHttpClient;
...
pub struct McpRuntimeInput {
    ...
    pub agent9527_apps_tools_cache: ConnectorRuntimeManager<ToolInfo>,
    pub tool_catalog_cache: McpToolCatalogCache,
    pub agent9527_apps_tools_cache_key: ConnectorRuntimeContextKey,
    pub supports_openai_form_elicitation: bool,
    pub auth: Option<Agent9527Auth>,
    pub agent9527_apps_auth_manager: Option<Arc<AuthManager>>,
    pub elicitation_reviewer: Option<ElicitationReviewerHandle>,
    pub elicitation_lifecycle: Option<ElicitationLifecycle>,
}

/// Owns all mutable MCP state for one Agent9527 thread.
///
...
impl McpRuntime {
    ...
    pub async fn replace_fresh(&self, input: McpRuntimeInput) -> anyhow::Result<Vec<ToolInfo>> {
        ...
        self.latest_hard_refresh_agent9527_apps_tools_cache().await
    }
    ...
    pub fn current_auth_matches(&self, auth: Option<&Agent9527Auth>) -> bool {
    ...
    pub async fn latest_hard_refresh_agent9527_apps_tools_cache(
        &self,
...
impl McpRuntimeContext {
    ...
    pub(crate) fn resolve_server_environment(
        &self,
        server_name: &str,
        config: &agent9527_config::McpServerConfig,
        ...
        if config.is_local_environment() {
            return match config.transport {
                agent9527_config::McpServerTransportConfig::Stdio { .. } => Err(format!(
                    "local stdio MCP server `{server_name}` requires a local environment"
                )),
                agent9527_config::McpServerTransportConfig::StreamableHttp { .. } => Ok(None),
            };
        }

...
//! and exposing cached Agent9527 Apps tools while a client is still connecting.
...
use crate::agent9527_apps::normalize_agent9527_apps_callable_name;
use crate::agent9527_apps::normalize_agent9527_apps_callable_namespace;
use crate::agent9527_apps::normalize_agent9527_apps_tool_title;
use crate::agent9527_apps::prepare_openai_file_params_for_model;
use crate::elicitation::ElicitationRequestManager;
use crate::mcp::AGENT9527_APPS_MCP_SERVER_NAME;
...
use agent9527_async_utils::CancelErr;
...
use agent9527_exec_server::ReqwestHttpClient;
use agent9527_protocol::mcp::McpServerInfo;
use agent9527_protocol::protocol::Event;
use agent9527_protocol::protocol::EventMsg;
use agent9527_protocol::protocol::McpStartupStatus;
use agent9527_protocol::protocol::McpStartupUpdateEvent;
use agent9527_rmcp_client::ExecutorStdioServerLauncher;
use agent9527_rmcp_client::LocalStdioServerLauncher;
use agent9527_rmcp_client::RmcpClient;
use agent9527_rmcp_client::StdioServerLauncher;
use agent9527_rmcp_client::ToolWithConnectorId;
use agent9527_rmcp_client::is_authentication_required_error;
use anyhow::Result;
use anyhow::anyhow;
...
/// MCP server capability indicating that Agent9527 should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "agent9527/sandbox-state-meta";
...
const MCP_TOOL_CATALOG_CACHE_CAPABILITY: &str = "agent9527/tool-catalog-cache";
const MCP_TOOL_CATALOG_CACHEABLE_PROPERTY: &str = "cacheable";
pub const OPENAI_FORM_CAPABILITY: &str = "openai/form";

pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "agent9527.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "agent9527.mcp.tools.fetch_uncached.duration_ms";
pub(crate) const AGENT9527_APPS_REFRESH_DURATION_METRIC: &str =
    "agent9527.apps.refresh.duration_ms";
pub(crate) const DEFAULT_STARTUP_TIMEOUT: Duration = Duration::from_secs(30);
pub(crate) const DEFAULT_TOOL_TIMEOUT: Duration = Duration::from_secs(300);

pub(crate) const AGENT9527_APPS_RECONNECT_INITIAL_BACKOFF: Duration = Duration::from_secs(1);
const AGENT9527_APPS_RECONNECT_MAX_BACKOFF: Duration = Duration::from_secs(30);
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) agent9527_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
}
...
impl ManagedClient {
    pub(crate) fn listed_tools(&self) -> Vec<ToolInfo> {
        ...
        if let Some(tools) = self
            .agent9527_apps_tools_cache_context
            .as_ref()
        ...
        if self.agent9527_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
#[derive(Default)]
struct Agent9527AppsStartupReconnectState {
...
#[derive(Clone)]
struct Agent9527AppsStartupStatusContext {
    submit_id: String,
    server_name: String,
    tx_event: Sender<Event>,
}

pub(crate) struct Agent9527AppsStartupReconnect {
    factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>,
    state: StdMutex<Agent9527AppsStartupReconnectState>,
    startup_status_context: Option<Agent9527AppsStartupStatusContext>,
}

impl Agent9527AppsStartupReconnect {
    pub(crate) fn new(factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>) -> Self {
        Self {
            factory,
            state: StdMutex::new(Agent9527AppsStartupReconnectState::default()),
    ...
    fn with_startup_status_context(
        ...
        self.startup_status_context = Some(Agent9527AppsStartupStatusContext {
            submit_id,
            server_name,
            tx_event,
        });
            ...
            let recovered = {
                ...
                match result {
                    ...
                    Err(error) => {
                        ...
                        let retry_after =
                            agent9527_apps_reconnect_backoff(state.consecutive_failures);
...
fn agent9527_apps_reconnect_backoff(consecutive_failures: u32) -> Duration {
    let exponent = consecutive_failures.saturating_sub(1).min(5);
    AGENT9527_APPS_RECONNECT_INITIAL_BACKOFF
        .saturating_mul(1 << exponent)
        .min(AGENT9527_APPS_RECONNECT_MAX_BACKOFF)
}
...
struct ManagedClientStartup {
    ...
    agent9527_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
...
impl ManagedClientStartup {
    fn start(&self) -> ManagedClientFuture {
        ...
        let is_agent9527_apps_mcp_server = server_name == AGENT9527_APPS_MCP_SERVER_NAME;
        ...
        async move {
            let refresh_start = is_agent9527_apps_mcp_server.then(Instant::now);
            ...
            {
                emit_duration(
                    AGENT9527_APPS_REFRESH_DURATION_METRIC,
...
pub(crate) struct AsyncManagedClient {
    ...
    pub(crate) is_agent9527_apps_mcp_server: bool,
    ...
    pub(crate) startup_reconnect: Option<Arc<Agent9527AppsStartupReconnect>>,
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        let tool_filter = server
            ...
            .map(ToolFilter::from_config)
            .unwrap_or_default();
        let cached_server_info = if is_agent9527_apps_mcp_server {
            agent9527_apps_tools_cache_context
                .as_ref()
        ...
        let startup = Arc::new(ManagedClientStartup {
            ...
            tx_event,
            elicitation_requests,
            agent9527_apps_tools_cache_context: agent9527_apps_tools_cache_context.clone(),
        ...
        let startup_reconnect = is_agent9527_apps_mcp_server.then(|| {
            let startup = Arc::clone(&startup);
            Arc::new(
                Agent9527AppsStartupReconnect::new(Arc::new(move || startup.start()))
        ...
        if agent9527_apps_tools_cache_context
            .as_ref()
    ...
    pub(crate) fn has_cached_tools(&self) -> bool {
        self.agent9527_apps_tools_cache_context
            .as_ref()
            .is_some_and(ConnectorRuntimeContext::has_current_tools)
            || self
                .tool_catalog_cache_context
    ...
    pub(crate) fn prepare_tools(&self, tools: Vec<ToolInfo>) -> Vec<ToolInfo> {
        if self.is_agent9527_apps_mcp_server {
            prepare_agent9527_apps_tools_for_model(tools, &self.tool_plugin_provenance)
        } else {
    ...
    pub(crate) async fn listed_tools(&self) -> Option<Vec<ToolInfo>> {
            ...
            && let Some(startup_tools) = self.cached_tools()
        {
            ...
            match self.client().await {
                ...
                Err(_) if self.is_agent9527_apps_mcp_server => self.cached_tools(),
                Err(_) => None,
            }
        }?;
...
pub(crate) async fn list_tools_for_client_uncached(
    server_name: &str,
    is_agent9527_apps_mcp_server: bool,
    agent9527_apps_refresh_trigger: &'static str,
    ...
    let tools = resp
        ...
        .collect();
    if is_agent9527_apps_mcp_server {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
            fetch_start.elapsed(),
            &[("trigger", agent9527_apps_refresh_trigger)],
        );
    } else {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
...
/// Presents declared Agent9527 Apps file parameters to the model as local-path inputs and adds plugin
/// names to each tool. Plugin membership is resolved by connector ID, falling back to the MCP
/// server when absent.
fn prepare_agent9527_apps_tools_for_model(
...
fn tool_info_from_listed_tool(
    ...
    if is_agent9527_apps_mcp_server {
        agent9527_apps_tool_info_from_listed_tool(server_name, server_instructions, tool)
    } else {
        regular_mcp_tool_info_from_listed_tool(server_name, server_instructions, tool)
    }
}

/// Converts a Agent9527 Apps tool by preserving connector fields, removing connector prefixes from
/// model-visible names and titles, and using the connector description for its tool namespace.
fn agent9527_apps_tool_info_from_listed_tool(
    server_name: &str,
    server_instructions: Option<&str>,
    tool: ToolWithConnectorId,
) -> ToolInfo {
    ...
    let callable_name = normalize_agent9527_apps_callable_name(
        &tool_def.name,
        connector_id.as_deref(),
        connector_name.as_deref(),
    );
    let callable_namespace =
        normalize_agent9527_apps_callable_namespace(server_name, connector_name.as_deref());
    if let Some(title) = tool_def.title.as_deref() {
        let normalized_title =
            normalize_agent9527_apps_tool_title(connector_name.as_deref(), title);
...
async fn start_server_task(
    ...
    let fetch_ticket = agent9527_apps_tools_cache_context
        .as_ref()
        .map(|cache_context| cache_context.begin_fetch(ConnectorRuntimeFetchSource::Startup));
    let tools = list_tools_for_client_uncached(
        &server_name,
        is_agent9527_apps_mcp_server,
        /*agent9527_apps_refresh_trigger*/ "initial",
        &client,
        startup_timeout,
    ...
    let tools = match (agent9527_apps_tools_cache_context.as_ref(), fetch_ticket) {
        (Some(cache_context), Some(fetch_ticket)) => {
            cache_context.publish_if_newest_accepted(fetch_ticket, &server_info, tools)
        }
        (None, None) => tools,
        _ => unreachable!("Agent9527 Apps fetch ticket requires cache context"),
    };
    let has_shared_tool_catalog =
        is_agent9527_apps_mcp_server || tool_catalog_cache_context.is_some();
...
fn mcp_initialize_request_params(
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("agent9527-mcp-client", env!("CARGO_PKG_VERSION"))
...
async fn make_rmcp_client(
    ...
    match transport {
        McpServerTransportConfig::Stdio {
            ...
            let cwd = cwd.map(agent9527_utils_path_uri::LegacyAppPathString::into_string);
...
mod tests {
    ...
    #[test]
    fn agent9527_apps_connector_metadata_is_preserved() {
        ...
        let expected = ToolInfo {
            server_name: AGENT9527_APPS_MCP_SERVER_NAME.to_string(),
            supports_parallel_tool_calls: false,
            server_origin: None,
            callable_name: "capture_file_upload".to_string(),
            callable_namespace: "agent9527_apps__gmail".to_string(),

...
pub(crate) fn test_mcp_config(agent9527_home: PathBuf) -> McpConfig {
    McpConfig {
        ...
        config_layer_stack: agent9527_config::ConfigLayerStack::default(),
        approvals_reviewer: agent9527_config::types::ApprovalsReviewer::default(),
        ...
        connector_snapshot: agent9527_connectors::ConnectorSnapshot::default(),
    }
}
...
fn tool_plugin_provenance_collects_app_and_mcp_sources() {
    ...
    catalog.register(McpServerRegistration::from_plugin(
        ...
        /*plugin_order*/ 0,
        agent9527_apps_mcp_server_config(
            "https://alpha.example",
            /*apps_mcp_product_sku*/ None,
            /*originator*/ None,
        ),
...
#[test]
fn agent9527_apps_mcp_url_for_base_url_uses_plugin_service_paths() {
    assert_eq!(
        agent9527_apps_mcp_url_for_base_url("https://chatgpt.com/backend-api"),
        "https://chatgpt.com/backend-api/ps/mcp"
    );
    assert_eq!(
        agent9527_apps_mcp_url_for_base_url("https://chat.openai.com"),
        "https://chat.openai.com/backend-api/ps/mcp"
    );
    assert_eq!(
        agent9527_apps_mcp_url_for_base_url("http://localhost:8080/api/agent9527"),
        "http://localhost:8080/api/agent9527/ps/mcp"
    );
    assert_eq!(
        agent9527_apps_mcp_url_for_base_url("http://localhost:8080"),
        "http://localhost:8080/api/agent9527/ps/mcp"
    );
}

#[test]
fn agent9527_apps_server_config_uses_plugin_service_path() {
    let config = agent9527_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    );
    let url = match &config.transport {
        ...
        _ => panic!("expected streamable http transport for agent9527 apps"),
    };
...
#[test]
fn agent9527_apps_server_config_forwards_thread_originator_header() {
    let config = agent9527_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        ...
        McpServerTransportConfig::StreamableHttp {
            ...
            assert_eq!(
                ...
                &Some(HashMap::from([
                    ...
                    ("X-OpenAI-Product-Sku".to_string(), "agent9527".to_string()),
                ]))
            );
...
#[test]
fn agent9527_apps_server_config_sets_product_sku_header() {
    for (configured_product_sku, expected_product_sku) in
        [(None, "agent9527"), (Some("tpp"), "tpp")]
    {
        let config = agent9527_apps_mcp_server_config(
            "https://chatgpt.com",
...
#[test]
fn agent9527_apps_server_config_forwards_originator_and_configured_product_sku_headers() {
    let config = agent9527_apps_mcp_server_config(
        "https://chatgpt.com",
        Some("tpp"),
        Some("thread_originator"),
...
async fn effective_mcp_servers_preserve_runtime_servers() {
    ...
    let mut config = test_mcp_config(agent9527_home.path().to_path_buf());
    ...
    let agent9527_apps = effective
        .get(AGENT9527_APPS_MCP_SERVER_NAME)
        .expect("agent9527 apps server should exist");

    let sample = sample.config();
    let docs = docs.config();
    let agent9527_apps = agent9527_apps.config();
    ...
    match &agent9527_apps.transport {
        McpServerTransportConfig::StreamableHttp { url, .. } => {
            assert_eq!(url, "https://chatgpt.com/backend-api/ps/mcp");
        }
        other => panic!("expected streamable http transport, got {other:?}"),

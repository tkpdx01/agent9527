...
fn test_mcp_config(agent9527_home: PathBuf) -> McpConfig {
    McpConfig {
...
#[test]
fn agent9527_apps_mcp_url_for_base_url_keeps_existing_paths() {
    assert_eq!(
    ...
    assert_eq!(
        ...
        "http://localhost:8080/api/agent9527/apps"
    );
    assert_eq!(
...
#[test]
fn agent9527_apps_server_config_uses_legacy_agent9527_apps_path() {
...
async fn effective_mcp_servers_preserve_runtime_servers() {
    ...
    let docs = docs
        .configured_config()
        .expect("configured server should retain transport");
    let agent9527_apps = agent9527_apps
        .configured_config()
        .expect("agent9527 apps should use configured transport");

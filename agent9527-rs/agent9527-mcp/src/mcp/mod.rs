...
use agent9527_connectors::ConnectorSnapshot;
use agent9527_connectors::connector_runtime_context_key;
use agent9527_login::Agent9527Auth;
use agent9527_model_provider::CHATGPT_AGENT9527_BASE_URL;
use agent9527_protocol::mcp::McpServerInfo;
use agent9527_protocol::mcp::Resource;
use agent9527_protocol::mcp::ResourceTemplate;
use agent9527_protocol::mcp::Tool;
use agent9527_protocol::models::PermissionProfile;
use agent9527_protocol::protocol::AskForApproval;
use agent9527_protocol::protocol::McpAuthStatus;
...
pub const AGENT9527_APPS_MCP_SERVER_NAME: &str = "agent9527_apps";
const DEFAULT_AGENT9527_APPS_MCP_PRODUCT_SKU: &str = "agent9527";
const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
const AGENT9527_CONNECTORS_TOKEN_ENV_VAR: &str = "AGENT9527_CONNECTORS_TOKEN";
...
/// MCP runtime settings derived from `agent9527_core::config::Config`.
///
/// This struct should contain only long-lived configuration values that the
/// `agent9527-mcp` crate needs to construct server transports, enforce MCP
...
pub struct McpConfig {
    ...
    /// Agent9527 home directory used for MCP OAuth state and app-tool cache files.
    ...
    /// Optional path to `agent9527-linux-sandbox` for sandboxed MCP tool execution.
...
pub fn host_owned_agent9527_apps_enabled(config: &McpConfig, auth: Option<&Agent9527Auth>) -> bool {
    config.apps_enabled && auth.is_some_and(Agent9527Auth::uses_agent9527_backend)
}
...
pub fn effective_mcp_servers_from_configured(
    ...
    let chatgpt_origin = url::Url::parse(CHATGPT_AGENT9527_BASE_URL)
        .ok()
        .map(|url| url.origin());
    let mut servers = configured_servers
        ...
        .collect::<HashMap<_, _>>();
    if !host_owned_agent9527_apps_enabled(config, auth) {
        servers.remove(AGENT9527_APPS_MCP_SERVER_NAME);
    }
    servers
}
...
fn agent9527_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(AGENT9527_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => {
            Some(AGENT9527_CONNECTORS_TOKEN_ENV_VAR.to_string())
        }
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(AGENT9527_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}

fn normalize_agent9527_apps_base_url(base_url: &str) -> String {
...
fn agent9527_apps_mcp_url_for_base_url(base_url: &str) -> String {
    let base_url = normalize_agent9527_apps_base_url(base_url);
    let (base_url, default_path) = if base_url.contains("/backend-api") {
        (base_url, "wham/apps")
    } else if base_url.contains("/api/agent9527") {
        (base_url, "apps")
    } else {
        (format!("{base_url}/api/agent9527"), "apps")
    };
    format!("{base_url}/{default_path}")
}

pub fn agent9527_apps_mcp_server_config(
    chatgpt_base_url: &str,
    apps_mcp_product_sku: Option<&str>,
    originator: Option<&str>,
) -> McpServerConfig {
    mcp_server_config_for_url(
        agent9527_apps_mcp_url_for_base_url(chatgpt_base_url),
...
pub fn hosted_plugin_runtime_mcp_server_config(
    ...
    let base_url = normalize_agent9527_apps_base_url(chatgpt_base_url);
    let base_url = if base_url.contains("/backend-api") || base_url.contains("/api/agent9527") {
        base_url
    } else {
        format!("{base_url}/api/agent9527")
    };
    mcp_server_config_for_url(
...
fn mcp_server_config_for_url(
    ...
    let product_sku = apps_mcp_product_sku.unwrap_or(DEFAULT_AGENT9527_APPS_MCP_PRODUCT_SKU);
    ...
    McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url,
            bearer_token_env_var: agent9527_apps_mcp_bearer_token_env_var(),

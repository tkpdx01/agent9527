//! Aggregates MCP server connections for Agent9527.
//!
...
use crate::rmcp_client::AGENT9527_APPS_REFRESH_DURATION_METRIC;
...
use agent9527_config::McpServerAuth;
...
use agent9527_protocol::protocol::McpStartupCompleteEvent;
use agent9527_protocol::protocol::McpStartupFailure;
...
use agent9527_rmcp_client::determine_streamable_http_auth_status_from_credentials;
use anyhow::Context;
use anyhow::Result;
use anyhow::anyhow;
...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        agent9527_apps_tools_cache: ConnectorRuntimeManager<ToolInfo>,
        tool_catalog_cache: McpToolCatalogCache,
        agent9527_apps_tools_cache_key: ConnectorRuntimeContextKey,
        prefix_mcp_tool_names: bool,
        client_elicitation_capability: ElicitationCapability,
        supports_openai_form_elicitation: bool,
        tool_plugin_provenance: ToolPluginProvenance,
        auth: Option<&Agent9527Auth>,
        agent9527_apps_auth_manager: Option<Arc<AuthManager>>,
        ...
        let static_chatgpt_auth_provider = auth
            .filter(|auth| auth.uses_agent9527_backend())
            .map(agent9527_model_provider::auth_provider_from_auth);
        let agent9527_apps_auth_provider = agent9527_apps_auth_manager.and_then(|auth_manager| {
            auth.filter(|auth| auth.uses_agent9527_backend())
                .map(|auth| {
                    agent9527_model_provider::auth_provider_from_auth_manager(auth_manager, auth)
                })
        });
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            // For built-in Agent9527 Apps, `AGENT9527_CONNECTORS_TOKEN` is a debug
            ...
            let shares_agent9527_apps_tools_cache =
                should_share_agent9527_apps_tools_cache(&server_name, uses_env_bearer_token);
            let agent9527_apps_tools_cache_context = shares_agent9527_apps_tools_cache.then(|| {
                agent9527_apps_tools_cache.context(
                    agent9527_home.clone(),
                    agent9527_apps_tools_cache_key.clone(),
                )
            });
            // The reserved Agent9527 Apps registration follows the shared
            ...
            let chatgpt_auth_provider = if server_name == AGENT9527_APPS_MCP_SERVER_NAME {
                agent9527_apps_auth_provider
                    .clone()
                    .or_else(|| static_chatgpt_auth_provider.clone())
            } else {
                static_chatgpt_auth_provider.clone()
            };
            // If Agent9527 Apps has an env bearer token, that is its auth path. Do
            // not also attach the ambient Agent9527Auth provider.
            let runtime_auth_provider =
                if server_name == AGENT9527_APPS_MCP_SERVER_NAME && uses_env_bearer_token {
                    None
                } else {
                    chatgpt_auth_provider_for_server(&server, chatgpt_auth_provider)
                };
            let tool_catalog_cache_context = if server_name == AGENT9527_APPS_MCP_SERVER_NAME {
                None
    ...
    pub fn tool_approval_mode(
        ...
        server_name: &str,
        tool_name: &str,
    ) -> agent9527_config::AppToolApproval {
        self.server_metadata
            .get(server_name)
    ...
    pub fn is_host_owned_agent9527_apps_server(&self, server_name: &str) -> bool {
    ...
    /// Force-refresh agent9527 apps tools by bypassing the in-process cache.
    ///
    ...
    pub async fn hard_refresh_agent9527_apps_tools_cache(&self) -> Result<Vec<ToolInfo>> {
        let refresh_start = Instant::now();
        let managed_client = self
            .clients
            .get(AGENT9527_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{AGENT9527_APPS_MCP_SERVER_NAME}'"))?
            .client()
            .await
        ...
        let tools = list_tools_for_client_uncached(
            ...
            /*agent9527_apps_refresh_trigger*/ "explicit",
        ...
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{AGENT9527_APPS_MCP_SERVER_NAME}'")
        })?;

        let tools =
            match (
                managed_client.agent9527_apps_tools_cache_context.as_ref(),
                fetch_ticket,
            ) {
    ...
    /// Agent9527 Apps metadata may come from its existing cache; regular MCP server information is
...
fn should_share_agent9527_apps_tools_cache(server_name: &str, uses_env_bearer_token: bool) -> bool {
    server_name == AGENT9527_APPS_MCP_SERVER_NAME && !uses_env_bearer_token
}

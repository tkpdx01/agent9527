...
async fn build_memory_tool_developer_instructions_renders_embedded_template() {
    ...
    let agent9527_home = AbsolutePathBuf::from_absolute_path(temp.path()).unwrap();
    let memories_dir = agent9527_home.join("memories");
    ...
    let instructions = build_memory_tool_developer_instructions(&agent9527_home)
        .await
        .unwrap();

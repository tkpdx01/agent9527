...
use agent9527_api::SearchClient;
use agent9527_api::SearchCommands;
use agent9527_api::SearchQuery;
use agent9527_api::SearchRequest;
use agent9527_api::SearchSettings;
use agent9527_core::X_AGENT9527_TURN_METADATA_HEADER;
use agent9527_core::web_search_action_detail;
...
use agent9527_extension_api::parse_tool_input_schema_without_compaction;
use agent9527_extension_items::ExtensionItem;
use agent9527_extension_items::web_search::WebSearchAction;
...
const RESULTS_PAYLOAD_BYTES_METRIC: &str = "agent9527.web_search.results.payload_bytes";
...
impl WebSearchTool {
    async fn handle_call(&self, call: ToolCall) -> Result<Box<dyn ToolOutput>, FunctionCallError> {
        ...
        let extra_headers = search_request_headers(
            ...
            call.agent9527_turn_metadata.as_deref(),
        );
        ...
        if let Some(results) = results.as_ref()
            && let Some(metrics) = agent9527_otel::global()
...
fn search_request_headers(originator: Option<&str>, turn_metadata: Option<&str>) -> HeaderMap {
    ...
    {
        headers.insert(X_AGENT9527_TURN_METADATA_HEADER, header_value);
    }

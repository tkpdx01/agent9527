...
use agent9527_extension_api::JsonToolOutput;
use agent9527_extension_api::ToolCall;
use agent9527_extension_api::ToolExecutor;
use agent9527_extension_api::ToolName;
use agent9527_extension_api::ToolOutput;
use agent9527_extension_api::ToolSpec;
...
use agent9527_protocol::protocol::validate_thread_goal_objective;
...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<agent9527_state::StateRuntime>,
...
impl ToolExecutor<ToolCall> for GoalToolExecutor {
    ...
    fn handle(&self, invocation: ToolCall) -> agent9527_extension_api::ToolExecutorFuture<'_> {
        Box::pin(async move {
            match self.kind {
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => agent9527_state::GoalAccountingMode::ActiveOrComplete,
                ThreadGoalStatus::Blocked => agent9527_state::GoalAccountingMode::ActiveOrStopped,
    ...
    async fn account_active_goal_progress(
        &self,
        mode: agent9527_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            agent9527_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
                self.analytics
            ...
            agent9527_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<agent9527_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &agent9527_state::StateRuntime,
    thread_id: ThreadId,
    goal: &agent9527_state::ThreadGoal,
) {
...
pub(crate) fn protocol_goal_from_state(goal: agent9527_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn protocol_status_from_state(status: agent9527_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(
    status: ThreadGoalStatus,
) -> agent9527_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => agent9527_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => agent9527_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::Blocked => agent9527_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => agent9527_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => agent9527_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => agent9527_state::ThreadGoalStatus::Complete,
    }
}

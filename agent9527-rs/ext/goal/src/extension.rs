...
use agent9527_extension_api::ThreadIdleInput;
...
use agent9527_extension_api::TokenUsageContributor;
...
use agent9527_extension_api::ToolLifecycleContributor;
use agent9527_extension_api::ToolLifecycleFuture;
use agent9527_extension_api::TurnAbortInput;
use agent9527_extension_api::TurnErrorInput;
use agent9527_extension_api::TurnLifecycleContributor;
    ...
    C: Send + Sync + 'static,
{
    fn on_turn_start<'a>(&'a self, input: TurnStartInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            if matches!(
                input.collaboration_mode.mode,
                agent9527_protocol::config_types::ModeKind::Plan
            ) {
    ...
    fn on_turn_error<'a>(&'a self, input: TurnErrorInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            let reason = match input.error {
                Agent9527ErrorInfo::UsageLimitExceeded => ActiveGoalStopReason::UsageLimit,

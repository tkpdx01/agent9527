...
```
agent9527 --config mcp_servers.example={command="agent9527-stdio-to-uds",args=["/tmp/mcp.sock"]}
```
...
As a workaround, this crate uses `agent9527-uds`, which provides a cross-platform async UDS API backed by https://crates.io/crates/uds_windows on Windows.

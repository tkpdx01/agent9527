...
use agent9527_exec_server::CAPABILITY_ROOTS_DISCOVER_METHOD;
use agent9527_exec_server::CapabilityRootDiscovery;
use agent9527_exec_server::CapabilityRootsDiscoverParams;
use agent9527_exec_server::CapabilityRootsDiscoverResponse;
...
async fn discovers_a_complete_capability_bundle_in_one_request() -> anyhow::Result<()> {
    ...
    write_file(
        &root.path().join(".agent9527-plugin/plugin.json"),
        r#"{
  "name": "demo",
  ...
  "apps": "./config/apps.json"
    ...
    assert_eq!(
        ...
        root_uri.join(".agent9527-plugin/plugin.json")?
    );

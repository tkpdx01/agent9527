...
pub(crate) struct ExecServerHarness {
    ...
    _helper_paths: TestAgent9527HelperPaths,
    child: Child,
...
pub(crate) struct TestAgent9527HelperPaths {
    pub(crate) agent9527_exe: PathBuf,
    pub(crate) agent9527_linux_sandbox_exe: Option<PathBuf>,
}
...
pub(crate) fn test_agent9527_helper_paths() -> anyhow::Result<TestAgent9527HelperPaths> {
    let (helper_binary, agent9527_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestAgent9527HelperPaths {
        agent9527_exe: helper_binary,
    ...
    V: AsRef<std::ffi::OsStr>,
{
    ...
    let mut child = Command::new(&helper_paths.agent9527_exe);
    child.args(["exec-server", "--listen", "ws://127.0.0.1:0"]);
    child.stdin(Stdio::null());
    child.stdout(Stdio::piped());
    child.stderr(Stdio::inherit());
    child.kill_on_drop(true);
    child.env("AGENT9527_HOME", agent9527_home.path());
    child.envs(env);

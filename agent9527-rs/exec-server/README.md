...
`agent9527-exec-server` is the library backing `agent9527 exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`agent9527-utils-pty`.

It provides:

- a CLI entrypoint: `agent9527 exec-server`
- a Rust client: `ExecServerClient`
- a small protocol module with shared request/response types

This crate owns the transport, protocol, and filesystem/process handlers. The
top-level `agent9527` binary owns hidden helper dispatch for sandboxed
filesystem operations and `agent9527-linux-sandbox`.

## Transport

The server speaks the exec-specific `agent9527-exec-server-protocol` message
...
must support it.
It uses the standard Agent9527 ChatGPT sign-in state; run `agent9527 login` first when
remote registration needs authentication. Containerized callers that receive an
Agent Identity JWT in `AGENT9527_ACCESS_TOKEN` can opt into that auth path with
`--use-agent-identity-auth`; Agent9527 then registers an Agent task and sends the
derived AgentAssertion headers on the registry request.

Alternatively, API users can instead use `AGENT9527_API_KEY`;
Agent9527 sends it as a bearer token on the registration request. For example:

```sh
AGENT9527_API_KEY="$OPENAI_API_KEY" \
agent9527 exec-server \
  --remote ... \
...
`agent9527.exec_server.relay.v1.RelayMessageFrame`; the checked-in schema is in
`src/proto/agent9527.exec_server.relay.v1.proto`. The relay frame carries stream
...
- `arg0`: optional argv0 override forwarded to `agent9527-utils-pty`.
...
hidden helper process launched from the top-level `agent9527` executable and
...
`HttpClientFactory` to `run_main()`. The top-level `agent9527 exec-server` command
builds these paths from the `agent9527` arg0 dispatch state and resolves its HTTP
client factory from the effective Agent9527 configuration.
...
provider from Agent9527 auth state before starting remote mode.

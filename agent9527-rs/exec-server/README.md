...
`agent9527 exec-server` command builds these paths from the `agent9527` arg0 dispatch
state. `RemoteEnvironmentConfig::new(...)` also takes the auth provider that
remote registration should use; the CLI builds that provider from Agent9527 auth

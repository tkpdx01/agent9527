...
use agent9527_sandboxing::SandboxExecRequest;
...
use crate::fs_helper::AGENT9527_FS_HELPER_ARG1;
...
impl FileSystemSandboxRunner {
    ...
    fn sandbox_exec_request(
        ...
        let helper = &self.runtime_paths.agent9527_self_exe;
        ...
        let command = SandboxCommand {
            ...
            args: vec![AGENT9527_FS_HELPER_ARG1.to_string()],
            cwd: cwd.uri.clone(),
...
fn helper_read_roots(runtime_paths: &ExecServerRuntimePaths) -> Vec<AbsolutePathBuf> {
    ...
    for path in std::iter::once(runtime_paths.agent9527_self_exe.as_path())
        .chain(runtime_paths.agent9527_linux_sandbox_exe.as_deref())
    {
...
mod tests {
    ...
    fn helper_permissions_preserve_existing_writes() {
        let agent9527_self_exe = std::env::current_exe().expect("current exe");
    ...
    fn helper_env_preserves_path_for_system_bwrap_discovery_without_leaking_secrets() {
        ...
        assert_eq!(
            ...
            HashMap::from([
                ...
                ("TMPDIR".to_string(), "/tmp/agent9527".to_string()),
    ...
    fn sandbox_exec_request_carries_helper_env() {
        ...
        let agent9527_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(agent9527_self_exe.clone(), Some(agent9527_self_exe))
                .expect("runtime paths");
    ...
    fn sandbox_cwd_rejects_cwd_dependent_profile_without_context_cwd() {
        ...
        let sandbox_context =
            agent9527_file_system::FileSystemSandboxContext::from_permission_profile(
    ...
    fn helper_permissions_include_linux_sandbox_alias_parent() {
        ...
        let agent9527_self_exe = root.path().join("bin").join("agent9527");
        let agent9527_linux_sandbox_exe =
            root.path().join("aliases").join("agent9527-linux-sandbox");
        let runtime_paths =
            ExecServerRuntimePaths::new(agent9527_self_exe, Some(agent9527_linux_sandbox_exe))
                .expect("runtime paths");
        let cwd = AbsolutePathBuf::from_absolute_path(std::env::temp_dir().as_path())
            .expect("absolute cwd");
        let mut policy = restricted_policy(Vec::new());
        let agent9527_parent = AbsolutePathBuf::from_absolute_path(root.path().join("bin"))
            .expect("absolute agent9527 parent");
        ...
        assert!(policy.can_read_path_with_cwd(agent9527_parent.as_path(), cwd.as_path()));
    ...
    fn sandbox_context_with_cwd(
        ...
        agent9527_file_system::FileSystemSandboxContext::from_permission_profile_with_cwd(

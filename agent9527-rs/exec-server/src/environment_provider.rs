...
use crate::environment::AGENT9527_EXEC_SERVER_URL_ENV_VAR;
use crate::environment::LOCAL_ENVIRONMENT_ID;
use crate::environment::REMOTE_ENVIRONMENT_ID;

/// Lists the remote environment transports available to Agent9527.
///
...
/// Default provider backed by `AGENT9527_EXEC_SERVER_URL`.
#[derive(Clone, Debug)]
...
impl DefaultEnvironmentProvider {
    /// Builds a provider from an already-read raw `AGENT9527_EXEC_SERVER_URL` value.
    pub fn new(exec_server_url: Option<String>) -> Self {
        Self { exec_server_url }
    }

    /// Builds a provider by reading `AGENT9527_EXEC_SERVER_URL`.
    pub fn from_env() -> Self {
        Self::new(std::env::var(AGENT9527_EXEC_SERVER_URL_ENV_VAR).ok())
    }

...
use agent9527_app_server_client::EnvironmentManager;
use agent9527_app_server_client::ExecServerRuntimePaths;
use agent9527_app_server_client::InProcessAppServerClient;
use agent9527_app_server_client::InProcessClientStartArgs;
use agent9527_app_server_client::InProcessServerEvent;
...
use agent9527_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use agent9527_app_server_protocol::ThreadSource;
...
use agent9527_core::StateDbHandle;
use agent9527_core::check_execpolicy_for_warnings;
use agent9527_core::config::Config;
use agent9527_core::config::ConfigBuilder;
use agent9527_core::config::ConfigOverrides;
use agent9527_core::config::ConfigTomlLoadResult;
use agent9527_core::config::find_agent9527_home;
use agent9527_core::config::load_config_toml_with_layer_stack;
use agent9527_core::config::resolve_bootstrap_auth_keyring_backend_kind;
use agent9527_core::config::resolve_bootstrap_auth_route_config;
use agent9527_core::config::resolve_oss_provider;
use agent9527_core::config::resolve_profile_v2_config_path;
use agent9527_core::find_thread_meta_by_name_str;
use agent9527_core::format_exec_policy_error_with_source;
...
use agent9527_login::default_client::set_default_originator;
...
use agent9527_otel::set_parent_from_context;
use agent9527_otel::traceparent_context_from_env;
...
pub use event_processor_with_jsonl_output::Agent9527Status;
...
enum StdinPromptBehavior {
    ...
    /// `agent9527 exec` behavior for `agent9527 exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `agent9527 exec -`
...
struct ExecRunArgs {
    ...
    config: Config,
    resume_approvals_reviewer_override: Option<agent9527_app_server_protocol::ApprovalsReviewer>,
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    if let Err(err) = set_default_originator("agent9527_exec".to_string()) {
        tracing::warn!(
            ?err,
            "Failed to set agent9527 exec originator override {err:?}"
        );
    }
    ...
    let user_config_path = config_profile_v2
        .as_ref()
        .map(|profile_v2| resolve_profile_v2_config_path(&agent9527_home, profile_v2));
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        agent9527_core::otel_init::build_provider(
            &config,
    ...
    agent9527_core::otel_init::record_process_start(otel.as_ref(), "agent9527_exec");
    agent9527_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "agent9527_exec");
    ...
    let state_db = agent9527_core::init_state_db(&config).await;
    ...
    let in_process_start_args = InProcessClientStartArgs {
        ...
        client_name: "agent9527_exec".to_string(),
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = agent9527_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what Agent9527
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    if !json_mode
        && let Some(message) =
            agent9527_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        event_processor.process_warning(message);
    }

    info!("Agent9527 initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(mut notification) => {
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        agent9527_app_server_protocol::TurnStatus::Failed
                            | agent9527_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
                ...
                if should_process_notification(
                    ...
                    match event_processor.process_server_notification(notification) {
                        Agent9527Status::Running => {}
                        Agent9527Status::InitiateShutdown => {
...
fn thread_resume_params_from_config(
    config: &Config,
    thread_id: String,
    approvals_reviewer_override: Option<agent9527_app_server_protocol::ApprovalsReviewer>,
) -> ThreadResumeParams {
    let permissions = permissions_selection_from_config(config);
    let sandbox = permissions.is_none().then(|| {
        sandbox_mode_from_permission_profile(
...
fn sandbox_mode_from_permission_profile(
    ...
    cwd: &Path,
) -> Option<agent9527_app_server_protocol::SandboxMode> {
    match permission_profile {
        PermissionProfile::Disabled => {
            Some(agent9527_app_server_protocol::SandboxMode::DangerFullAccess)
        }
        ...
        PermissionProfile::Managed { .. } => {
            ...
            if file_system_policy.has_full_disk_write_access() {
                permission_profile
                    ...
                    .is_enabled()
                    .then_some(agent9527_app_server_protocol::SandboxMode::DangerFullAccess)
            } else if file_system_policy.can_write_path_with_cwd(cwd, cwd) {
                Some(agent9527_app_server_protocol::SandboxMode::WorkspaceWrite)
            } else {
                Some(agent9527_app_server_protocol::SandboxMode::ReadOnly)
            }
        }
    }
}
...
fn session_configured_from_thread_response(
    ...
    thread_source: Option<agent9527_protocol::protocol::ThreadSource>,
    ...
    approvals_reviewer: agent9527_protocol::config_types::ApprovalsReviewer,
    permission_profile: PermissionProfile,
    active_permission_profile: Option<agent9527_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
    reasoning_effort: Option<agent9527_protocol::openai_models::ReasoningEffort>,
) -> Result<SessionConfiguredEvent, String> {
...
fn should_backfill_turn_completed_items(
    ...
    !thread_ephemeral
        && payload.turn.items_view != agent9527_app_server_protocol::TurnItemsView::Full
}

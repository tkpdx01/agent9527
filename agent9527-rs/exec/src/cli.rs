...
#[command(
    version,
    override_usage = "agent9527 exec [OPTIONS] [PROMPT]\n       agent9527 exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running Agent9527 outside a Git repository.
    ...
    /// Do not load `$AGENT9527_HOME/config.toml`; auth still uses `AGENT9527_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "agent9527 resume --last <prompt>" treats the positional as a prompt, not a session id.

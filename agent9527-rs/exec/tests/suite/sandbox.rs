#![cfg(unix)]
use agent9527_core::spawn::StdioPolicy;
...
async fn spawn_command_under_sandbox(
    ...
    use agent9527_core::exec::build_exec_request;
...
async fn spawn_command_under_sandbox(
    ...
    use agent9527_core::spawn_command_under_linux_sandbox;

    let agent9527_linux_sandbox_exe = core_test_support::find_agent9527_linux_sandbox_exe()
...
#[tokio::test]
async fn sandbox_blocks_first_time_dot_agent9527_creation() {
    core_test_support::skip_if_sandbox!();
    #[cfg(target_os = "linux")]
    let sandbox_env = match linux_sandbox_test_env().await {
        Some(env) => env,
    ...
    let mut child = spawn_command_under_sandbox(
        vec![
            "bash".to_string(),
            "-lc".to_string(),
            "mkdir -p .agent9527 && echo 'sandbox_mode = \"danger-full-access\"' > .agent9527/config.toml"
                .to_string(),
        ],
        repo_root.clone(),
        &permission_profile,
    ...
    .await
    .expect("should spawn command creating .agent9527");

    let status = child
        .wait()
        .await
        .expect("should wait for .agent9527 command");
    assert!(
        !status.success(),
        "sandbox unexpectedly allowed first-time .agent9527 creation: {status:?}"
    );
    let dot_agent9527_metadata = tokio::fs::symlink_metadata(&dot_agent9527).await;
    if let Ok(metadata) = dot_agent9527_metadata {
        assert!(
            !metadata.is_dir(),
            "{} should not be creatable as a directory",
            dot_agent9527.display()
        );
    } else if let Err(err) = &dot_agent9527_metadata {
        assert_eq!(
            err.kind(),

...
/// While we may add an `apply-patch` subcommand to the `agent9527` CLI multitool
/// at some point, we must ensure that the smaller `agent9527-exec` CLI can still
...
fn test_standalone_exec_cli_can_use_apply_patch() -> anyhow::Result<()> {
    ...
    Command::new(agent9527_utils_cargo_bin::cargo_bin("agent9527-exec")?)
        .arg(AGENT9527_CORE_APPLY_PATCH_ARG1)
        .arg(
            r#"*** Begin Patch
...
async fn shutdown_flushes_completed_turn_and_file_diff() -> anyhow::Result<()> {
    ...
    test.cmd_with_server(&server)
        .env_remove("AGENT9527_API_KEY")
        .env_remove("AGENT9527_ANALYTICS_EVENTS_CAPTURE_FILE")
        .arg("--skip-git-repo-check")
        .arg("-s")
        .arg("danger-full-access")
        .arg("-c")

...
pub(crate) struct SandboxSetupCommand {
    ...
    /// Windows user that will run Agent9527 after managed deployment.
    #[arg(
        long = "user",
        value_name = "USER",
        conflicts_with = "current_user",
        requires = "agent9527_home"
    )]
    user: Option<String>,

    /// Use the current Windows user as the Agent9527 user.
    #[arg(
        long = "current-user",
    ...
    /// AGENT9527_HOME for the Agent9527 user. Required with --user.
    #[arg(long = "agent9527-home", value_name = "DIR")]
...
impl SandboxSetupCommand {
    fn setup_level(&self) -> anyhow::Result<SandboxSetupLevel> {
        if self.elevated_sandbox_level {
            ...
            anyhow::bail!("`agent9527 sandbox setup` currently requires --elevated");
        }
    }
}
...
async fn run_elevated(
    ...
    let config = ConfigBuilder::default()
        .agent9527_home(identity.agent9527_home.clone())
        .fallback_cwd(Some(identity.agent9527_home.clone()))
        .loader_overrides(super::loader_overrides_for_profile_at_agent9527_home(
            config_profile.as_ref(),
            &identity.agent9527_home,
        ))
        .cli_overrides(cli_overrides)
        .build()
        .await
        .context("failed to load target user's Agent9527 config for sandbox provisioning")?;

    agent9527_core::windows_sandbox::run_elevated_provisioning_setup(
        identity.agent9527_home.as_path(),
        identity.real_user.as_str(),
        config.permissions.network.as_ref(),
    )?;
    ConfigEditsBuilder::new(identity.agent9527_home.as_path())
    ...
    println!(
        ...
        identity.real_user,
        identity.agent9527_home.display()
    );
    Ok(())
}
...
fn resolve_sandbox_setup_identity(
    ...
    if cmd.current_user {
        ...
        let agent9527_home = match cmd.agent9527_home.clone() {
            Some(agent9527_home) => agent9527_home,
            None => find_agent9527_home()?.to_path_buf(),
        };
    ...
    let agent9527_home = cmd
        .agent9527_home
        .clone()
        .ok_or_else(|| anyhow::anyhow!("--agent9527-home is required with --user"))?;
...
mod tests {
    ...
    fn parses_managed_user_identity() {
        ...
        assert_eq!(
            command.agent9527_home.as_deref(),
            Some(std::path::Path::new(r"C:\Users\alice\.agent9527"))
        );
    }
    ...
    #[test]
    fn requires_agent9527_home_for_managed_user() {
        let err =
    ...
    fn parses_setup_from_sandbox_command_args() {
        let command = parse_setup_command(&[
            ...
            "--agent9527-home".to_string(),
            r"C:\Users\alice\.agent9527".to_string(),
        ])
        .expect("parse")

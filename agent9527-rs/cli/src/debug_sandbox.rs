...
#[cfg(target_os = "macos")]
use agent9527_core::spawn::AGENT9527_SANDBOX_ENV_VAR;
...
#[cfg(target_os = "macos")]
use agent9527_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use agent9527_sandboxing::seatbelt::create_seatbelt_command_args;
use agent9527_sandboxing::with_managed_mitm_ca_readable_root;
...
async fn run_command_under_sandbox(
    ...
    let sandbox_state = config_options
        ...
        .sandbox_state_json
        .as_deref()
        .map(serde_json::from_str::<agent9527_mcp::SandboxState>)
        .transpose()
    ...
    let agent9527_linux_sandbox_exe = match sandbox_state.as_ref() {
        Some(state) => {
            ...
            state
                .agent9527_linux_sandbox_exe
                .clone()
                .or(agent9527_linux_sandbox_exe)
        }
        None => agent9527_linux_sandbox_exe,
    };
    ...
    let network = network_proxy
        .as_ref()
        .map(agent9527_core::config::StartedNetworkProxy::proxy);
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Seatbelt => {
            ...
            spawn_debug_sandbox_child(
                ...
                |env_map| {
                    env_map.insert(
                        AGENT9527_SANDBOX_ENV_VAR.to_string(),
                        "seatbelt".to_string(),
                    );
        ...
        SandboxType::Landlock => {
            ...
            let agent9527_linux_sandbox_exe = config
                .agent9527_linux_sandbox_exe
                .expect("agent9527-linux-sandbox executable not found");
...
async fn run_command_under_windows_session(
    ...
    use agent9527_windows_sandbox::WindowsSandboxProxySettingsMode;
    use agent9527_windows_sandbox::WindowsSandboxSessionRequest;
    use agent9527_windows_sandbox::spawn_windows_sandbox_session_for_level;

    let empty_paths: &[AbsolutePathBuf] = &[];
    let spawned = spawn_windows_sandbox_session_for_level(WindowsSandboxSessionRequest {
        permission_profile,
        workspace_roots: workspace_roots.as_slice(),
        agent9527_home: config.agent9527_home.as_path(),
        command,
        cwd: cwd.as_path(),
        env_map: env,
    ...
    let exit_code = agent9527_windows_sandbox::forward_sandbox_session_stdio(spawned).await;
...
async fn spawn_debug_sandbox_child(
    ...
    if !network_sandbox_policy.is_enabled() {
        cmd.env(AGENT9527_SANDBOX_NETWORK_DISABLED_ENV_VAR, "1");
    }
...
async fn load_debug_sandbox_config(
    ...
    load_debug_sandbox_config_with_agent9527_home(
        cli_overrides,
        agent9527_linux_sandbox_exe,
        options,
        /*agent9527_home*/ None,
...
async fn load_debug_sandbox_config_with_agent9527_home(
    cli_overrides: Vec<(String, TomlValue)>,
    agent9527_linux_sandbox_exe: Option<PathBuf>,
    options: DebugSandboxConfigOptions,
    ...
    // For legacy configs, `agent9527 sandbox` historically defaulted to read-only
    ...
    let config = build_debug_sandbox_config_with_loader_overrides(
        ...
        ConfigOverrides {
            cwd: cwd.clone(),
            agent9527_linux_sandbox_exe: agent9527_linux_sandbox_exe.clone(),
            ..Default::default()
        },
...
async fn build_debug_sandbox_config_with_loader_overrides(
    ...
    if let Some(agent9527_home) = agent9527_home {
        builder = builder
            .agent9527_home(agent9527_home.clone())
            .fallback_cwd(Some(agent9527_home));
    }
    builder.build().await
}
...
mod tests {
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_agent9527_home(
            Vec::new(),
    ...
    async fn debug_sandbox_honors_config_profile_loader_overrides() -> anyhow::Result<()> {
        ...
        let docs = sandbox_paths.path().join("docs");
        let private = docs.join("private");
        let profile_path = agent9527_home.path().join("work.config.toml");
    ...
    async fn debug_sandbox_honors_explicit_builtin_permission_profile() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_agent9527_home(
            ...
            Some(agent9527_home.path().to_path_buf()),
            /*strict_config*/ false,
        )
        .await?;
        ...
        let actual = config
            ...
            .permission_profile()
            .file_system_sandbox_policy();
        let expected = agent9527_protocol::models::PermissionProfile::workspace_write()
            .file_system_sandbox_policy();
        assert!(
            expected
                .entries

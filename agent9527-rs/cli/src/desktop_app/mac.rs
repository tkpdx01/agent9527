...
const AGENT9527_BUNDLE_IDENTIFIER: &str = "com.openai.agent9527";
const AGENT9527_DMG_URL_ARM64: &str =
    "https://persistent.oaistatic.com/agent9527-app-prod/Agent9527.dmg";
const AGENT9527_DMG_URL_X64: &str =
    "https://persistent.oaistatic.com/agent9527-app-prod/Agent9527-latest-x64.dmg";

pub async fn run_mac_app_open_or_install(
    workspace: PathBuf,
    download_url_override: Option<String>,
) -> anyhow::Result<()> {
    if let Some(app_path) = find_existing_agent9527_app_path(&agent9527_app_search_dirs()) {
        eprintln!(
            "Opening Desktop app at {app_path}...",
            app_path = app_path.display()
        );
        open_agent9527_app(&app_path, &workspace).await?;
        return Ok(());
    }
    ...
    let download_url = download_url_override.unwrap_or_else(|| {
        let default_url = if is_apple_silicon_mac() {
            AGENT9527_DMG_URL_ARM64
        } else {
            AGENT9527_DMG_URL_X64
        };
        default_url.to_string()
    });
    let installed_app = download_and_install_agent9527_to_user_applications(&download_url)
        .await
    ...
    open_agent9527_app(&installed_app, &workspace).await?;
    Ok(())
}
...
fn find_existing_agent9527_app_path(applications_dirs: &[PathBuf]) -> Option<PathBuf> {
    applications_dirs
        .iter()
        .flat_map(|dir| ["ChatGPT.app", "Agent9527.app"].map(|app_name| dir.join(app_name)))
        .find(|candidate| is_agent9527_app_bundle(candidate))
}

fn agent9527_app_search_dirs() -> Vec<PathBuf> {
...
fn is_agent9527_app_bundle(app_path: &Path) -> bool {
    if !app_path.is_dir() {
        return false;
    }
    ...
    std::process::Command::new("/usr/bin/plutil")
        ...
        .is_ok_and(|output| {
            output.status.success()
                && String::from_utf8_lossy(&output.stdout).trim() == AGENT9527_BUNDLE_IDENTIFIER
        })
}

async fn open_agent9527_app(app_path: &Path, workspace: &Path) -> anyhow::Result<()> {
    eprintln!(
        "Opening workspace {workspace}...",
        workspace = workspace.display()
    );
    let url = agent9527_new_thread_url(workspace);
...
fn agent9527_new_thread_url(workspace: &Path) -> String {
...
async fn download_and_install_agent9527_to_user_applications(
    dmg_url: &str,
) -> anyhow::Result<PathBuf> {
    let temp_dir = Builder::new()
        .prefix("agent9527-app-installer-")
        .tempdir()
        .context("failed to create temp dir")?;
    let tmp_root = temp_dir.path().to_path_buf();
    let _temp_dir = temp_dir;

    let dmg_path = tmp_root.join("Agent9527.dmg");
    ...
    let result = async {
        let app_in_volume = find_agent9527_app_in_mount(&mount_point)
            .context("failed to locate Agent9527.app in mounted dmg")?;
        install_agent9527_app_bundle(&app_in_volume).await
    }
    .await;
...
async fn install_agent9527_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
    for applications_dir in candidate_applications_dirs()? {
        ...
        let dest_app = applications_dir.join("Agent9527.app");
        if dest_app.is_dir() {
            return Ok(dest_app);
        }
        ...
        match copy_app_bundle(app_in_volume, &dest_app).await {
            ...
            Err(err) => {
                eprintln!(
                    "warning: failed to install Agent9527.app to {applications_dir}: {err}",
                    applications_dir = applications_dir.display()
                );
            }
        }
    ...
    anyhow::bail!("failed to install Agent9527.app to any applications directory");
}
...
fn find_agent9527_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
    let direct = mount_point.join("Agent9527.app");
    if direct.is_dir() {
        return Ok(direct);
    }
...
mod tests {
    ...
    use super::find_existing_agent9527_app_path;
    ...
    #[test]
    fn finds_chatgpt_app_with_agent9527_bundle_identifier() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let app_path = temp_dir.path().join("ChatGPT.app");
        write_app_bundle(&app_path, "com.openai.agent9527");

        assert_eq!(
            find_existing_agent9527_app_path(&[temp_dir.path().to_path_buf()]),
            Some(app_path)
        );
    }
    ...
    fn ignores_classic_chatgpt_app() {
        ...
        let agent9527_app_path = temp_dir.path().join("Agent9527.app");
        write_app_bundle(&agent9527_app_path, "com.openai.agent9527");

        assert_eq!(
            find_existing_agent9527_app_path(&[temp_dir.path().to_path_buf()]),
            Some(agent9527_app_path)
        );
    }

    #[test]
    fn parses_mount_point_from_tab_separated_hdiutil_output() {
        let output = "/dev/disk2s1\tApple_HFS\tAgent9527\t/Volumes/Agent9527\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/Agent9527")
        );
    }

    #[test]
    fn parses_mount_point_with_spaces() {
        let output = "/dev/disk2s1\tApple_HFS\tAgent9527 Installer\t/Volumes/Agent9527 Installer\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/Agent9527 Installer")
        );
    }

    #[test]
    fn agent9527_new_thread_url_encodes_workspace_path() {
        let url = url::Url::parse(&agent9527_new_thread_url(Path::new(
            "/tmp/agent9527 workspace/#1",
        )))
        ...
        assert_eq!(
            ...
            (
                ...
                vec![(
                    "path".to_string(),
                    "/tmp/agent9527 workspace/#1".to_string()
                )],
            )
        );
    }

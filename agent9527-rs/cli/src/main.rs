use agent9527_app_server_daemon::BootstrapOptions as AppServerBootstrapOptions;
use agent9527_app_server_daemon::LifecycleCommand as AppServerLifecycleCommand;
use agent9527_app_server_daemon::RemoteControlMode as AppServerRemoteControlMode;
use agent9527_arg0::Arg0DispatchPaths;
use agent9527_arg0::arg0_dispatch_or_else;
use agent9527_chatgpt::apply_command::ApplyCommand;
use agent9527_chatgpt::apply_command::run_apply_command;
use agent9527_cli::read_access_token_from_stdin;
use agent9527_cli::read_api_key_from_stdin;
use agent9527_cli::run_login_status;
use agent9527_cli::run_login_with_access_token;
use agent9527_cli::run_login_with_api_key;
use agent9527_cli::run_login_with_chatgpt;
use agent9527_cli::run_login_with_device_code;
use agent9527_cli::run_logout;
use agent9527_cloud_tasks::Cli as CloudTasksCli;
use agent9527_exec::Cli as ExecCli;
use agent9527_exec::Command as ExecCommand;
use agent9527_exec::ReviewArgs;
use agent9527_execpolicy::ExecPolicyCheckCommand;
use agent9527_responses_api_proxy::Args as ResponsesApiProxyArgs;
use agent9527_rollout_trace::REDUCED_STATE_FILE_NAME;
use agent9527_rollout_trace::replay_bundle;
use agent9527_state::StateRuntime;
use agent9527_tui::AppExitInfo;
use agent9527_tui::Cli as TuiCli;
use agent9527_tui::ExitReason;
use agent9527_tui::UpdateAction;
use agent9527_utils_absolute_path::AbsolutePathBuf;
use agent9527_utils_cli::CliConfigOverrides;
use agent9527_utils_cli::ProfileV2Name;
use agent9527_utils_cli::SharedCliOptions;
use clap::Args;
...
use agent9527_core::build_models_manager;
...
#[clap(
    ...
    // `agent9527-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `agent9527` command name that users run.
    bin_name = "agent9527",
    override_usage = "agent9527 [OPTIONS] [PROMPT]\n       agent9527 [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    /// Run Agent9527 non-interactively.
    ...
    /// Manage external MCP servers for Agent9527.
    Mcp(McpCli),

    /// Manage Agent9527 plugins.
    Plugin(PluginCli),

    /// Start Agent9527 as an MCP server (stdio).
    ...
    /// Update Agent9527 to the latest version.
    Update,

    /// Diagnose local Agent9527 installation, config, auth, and runtime health.
    Doctor(DoctorCommand),

    /// Run commands within a Agent9527-provided sandbox.
    ...
    /// Apply the latest diff produced by Agent9527 agent as a `git apply` to your local working tree.
    ...
    /// [EXPERIMENTAL] Browse tasks from Agent9527 Cloud and apply changes locally.
...
struct ReviewCommand {
    /// Error out when config.toml contains fields that are not recognized by this version of Agent9527.
...
#[cfg(target_os = "macos")]
type HostSandboxArgs = agent9527_cli::SeatbeltCommand;
#[cfg(target_os = "linux")]
type HostSandboxArgs = agent9527_cli::LandlockCommand;
#[cfg(target_os = "windows")]
type HostSandboxArgs = agent9527_cli::WindowsCommand;
...
struct UnsupportedSandboxArgs {
    /// Layer $AGENT9527_HOME/<name>.config.toml on top of the base user config.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | agent9527 login --with-api-key`)"
    )]
    with_api_key: bool,

    #[arg(
        long = "with-access-token",
        help = "Read the access token from stdin (e.g. `printenv AGENT9527_ACCESS_TOKEN | agent9527 login --with-access-token`)"
    )]
...
struct AppServerCommand {
    ...
    #[command(flatten)]
    code_mode_host: agent9527_app_server::AppServerCodeModeHostArgs,
    ...
    #[arg(
        long = "listen",
        value_name = "URL",
        default_value = agent9527_app_server::AppServerTransport::DEFAULT_LISTEN_URL
    )]
    listen: agent9527_app_server::AppServerTransport,
    ...
    /// ```
    ///
    /// See https://developers.openai.com/agent9527/config-advanced/#metrics for more details.
    #[arg(long = "analytics-default-enabled")]
    analytics_default_enabled: bool,

    #[command(flatten)]
    auth: agent9527_app_server::AppServerWebsocketAuthArgs,
}
...
struct ExecServerCommand {
    ...
    /// Use Agent Identity auth from AGENT9527_ACCESS_TOKEN for remote registration.
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for Agent9527 tooling.
    #[clap(hide = true)]
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating Agent9527 via `{cmd_str}`...");
    ...
    println!("\n🎉 Update ran successfully! Please restart Agent9527.");
    Ok(())
}
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    {
        anyhow::bail!(
            "`agent9527 update` is not available in debug builds. Install a release build of Agent9527 to use this command."
        );
    }

    #[cfg(not(debug_assertions))]
    {
        let Some(action) = agent9527_tui::get_update_action() else {
            anyhow::bail!(
                "Could not detect the Agent9527 installation method. Please update manually: https://developers.openai.com/agent9527/cli/"
            );
        };
        run_update_action(action)
    }
...
async fn run_session_archive_cli_command(
    action: agent9527_tui::SessionArchiveAction,
    ...
    agent9527_tui::run_session_archive_command(
        action,
        target,
        agent9527_tui::SessionArchiveCommandOptions {
            cli: interactive,
            arg0_paths,
...
fn delete_action(target: &str, force: bool) -> anyhow::Result<agent9527_tui::SessionArchiveAction> {
    if force && agent9527_protocol::ThreadId::from_string(target).is_err() {
        anyhow::bail!("--force requires a session UUID; names must be confirmed interactively");
    }
    let confirmation = match force {
        true => agent9527_tui::DeleteConfirmation::Skip,
        false => agent9527_tui::DeleteConfirmation::Prompt,
    };
    Ok(agent9527_tui::SessionArchiveAction::Delete(confirmation))
}

async fn run_debug_app_server_command(cmd: DebugAppServerCommand) -> anyhow::Result<()> {
    match cmd.subcommand {
        DebugAppServerSubcommand::SendMessageV2(cmd) => {
            let agent9527_bin = std::env::current_exe()?;
            agent9527_app_server_test_client::send_message_v2(
                &agent9527_bin,
                &[],
                cmd.user_message,
                &None,
...
fn main() -> anyhow::Result<()> {
    product_policy::activate_external_api_only();
    let remote_control_disabled = agent9527_app_server::take_remote_control_disabled_env();
...
async fn cli_main(
    ...
    match subcommand {
        ...
        Some(Subcommand::Exec(mut exec_cli)) => {
            ...
            agent9527_exec::run_main(exec_cli, arg0_paths.clone()).await?;
        }
        Some(Subcommand::Review(ReviewCommand {
            ...
            let mut exec_cli = ExecCli::try_parse_from(["agent9527", "exec"])?;
            exec_cli
                .shared
                .inherit_exec_root_options(&interactive.shared);
            ...
            match subcommand {
                None => {
                    let transport = if stdio {
                        agent9527_app_server::AppServerTransport::Stdio
                    } else {
                        listen
                    };
                    let auth = auth.try_into_settings()?;
                    let runtime_options = agent9527_app_server::AppServerRuntimeOptions {
                        code_mode_host_transport: code_mode_host.into(),
                        remote_control_startup_mode: match (remote_control, remote_control_disabled)
                        {
                            (true, _) => {
                                agent9527_app_server::RemoteControlStartupMode::EnabledEphemeral
                            }
                            (false, true) => {
                                agent9527_app_server::RemoteControlStartupMode::DisabledEphemeral
                            }
                            (false, false) => {
                                agent9527_app_server::RemoteControlStartupMode::ResolvePersisted
                            }
                        },
                        ..Default::default()
                    };
                    agent9527_app_server::run_main_with_transport_options(
                        arg0_paths.clone(),
                        root_config_overrides,
                        ...
                        analytics_default_enabled,
                        transport,
                        agent9527_protocol::protocol::SessionSource::VSCode,
                        auth,
                        runtime_options,
                    )
                    .await?;
                ...
                Some(AppServerSubcommand::Daemon(daemon_cli)) => match daemon_cli.subcommand {
                    ...
                    AppServerDaemonSubcommand::Bootstrap(bootstrap_cli) => {
                        let output =
                            agent9527_app_server_daemon::bootstrap(AppServerBootstrapOptions {
                    ...
                    AppServerDaemonSubcommand::PidUpdateLoop => {
                        ...
                        agent9527_app_server_daemon::run_pid_update_loop(http_client_factory)
                            .await?;
                    }
                },
                Some(AppServerSubcommand::Proxy(proxy_cli)) => {
                    let socket_path = match proxy_cli.socket_path {
                        ...
                        None => {
                            let agent9527_home = find_agent9527_home()?;
                            agent9527_app_server::app_server_control_socket_path(&agent9527_home)?
                        }
                    };
                    agent9527_stdio_to_uds::run(socket_path.as_path()).await?;
                }
                Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                    let options = agent9527_app_server_protocol::GenerateTsOptions {
                ...
                Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                    agent9527_app_server_protocol::generate_json_with_experimental(
                        &gen_cli.out_dir,
                        gen_cli.experimental,
                    )?;
                }
                Some(AppServerSubcommand::GenerateInternalJsonSchema(gen_cli)) => {
                    agent9527_app_server_protocol::generate_internal_json_schema(&gen_cli.out_dir)?;
                }
            }
        }
        ...
        Some(Subcommand::Archive(cmd)) => {
            let output = run_session_archive_cli_command(
                agent9527_tui::SessionArchiveAction::Archive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
        ...
        Some(Subcommand::Unarchive(cmd)) => {
            let output = run_session_archive_cli_command(
                agent9527_tui::SessionArchiveAction::Unarchive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
        ...
        Some(Subcommand::Login(mut login_cli)) => {
            if let Err(message) = product_policy::ensure_account_authentication_enabled() {
                eprintln!("{message}");
                std::process::exit(2);
            }
            reject_remote_mode_for_subcommand(
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.with_api_key && login_cli.with_access_token {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | agent9527 login --with-api-key`."
                        );
                        std::process::exit(1);
        ...
        Some(Subcommand::Cloud(mut cloud_cli)) => {
            ...
            agent9527_cloud_tasks::run_main(
                cloud_cli,
                arg0_paths.agent9527_linux_sandbox_exe.clone(),
            )
            .await?;
        }
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            #[cfg(target_os = "macos")]
            agent9527_cli::run_command_under_seatbelt(
                sandbox_cli,
                arg0_paths.agent9527_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "linux")]
            agent9527_cli::run_command_under_landlock(
                sandbox_cli,
                arg0_paths.agent9527_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "windows")]
            agent9527_cli::run_command_under_windows_sandbox(
                sandbox_cli,
                arg0_paths.agent9527_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            ...
            #[cfg(not(any(target_os = "macos", target_os = "linux", target_os = "windows")))]
            {
                ...
                anyhow::bail!("`agent9527 sandbox` is not supported on this operating system");
            }
        }
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            ...
            tokio::task::spawn_blocking(move || agent9527_responses_api_proxy::run_main(args))
                .await??;
        }
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `agent9527 mcp`: `agent9527`, `agent9527 exec`, `agent9527 review`, `agent9527 resume`, `agent9527 archive`, `agent9527 delete`, `agent9527 unarchive`, `agent9527 fork`, `agent9527 mcp`, `agent9527 sandbox`, and `agent9527 debug prompt-input`."
        ),
    }
}
...
async fn run_exec_server_command(
    ...
    let agent9527_self_exe = arg0_paths
        .agent9527_self_exe
        .clone()
        .ok_or_else(|| anyhow::anyhow!("Agent9527 executable path is not configured"))?;
    let runtime_paths = agent9527_exec_server::ExecServerRuntimePaths::new(
        agent9527_self_exe,
        arg0_paths.agent9527_linux_sandbox_exe.clone(),
    )?;
    if let Some(base_url) = cmd.remote {
        ...
        let auth_provider =
            load_exec_server_remote_auth_provider(&config, &base_url, cmd.use_agent_identity_auth)
                .await?;
        let mut remote_config = agent9527_exec_server::RemoteEnvironmentConfig::new(
            base_url,
            environment_id,
            auth_provider,
        ...
        exec_server_telemetry::run_until_shutdown(async move {
            agent9527_exec_server::run_remote_environment(remote_config, runtime_paths).await
        })
        .await?;
        Ok(())
    } else {
        ...
        let http_client_factory = config
            .as_ref()
            .map(agent9527_core::config::Config::http_client_factory)
            .unwrap_or_else(|| {
                agent9527_http_client::HttpClientFactory::new(
                    agent9527_http_client::OutboundProxyPolicy::ReqwestDefault,
                )
            });
        let listen_url = cmd
            .listen
            .unwrap_or_else(|| agent9527_exec_server::DEFAULT_LISTEN_URL.to_string());
        exec_server_telemetry::run_until_shutdown(async move {
            agent9527_exec_server::run_main_with_telemetry(
                &listen_url,
                runtime_paths,
                telemetry,
                http_client_factory,
...
async fn load_exec_server_remote_auth_provider(
    config: &agent9527_core::config::Config,
    base_url: &str,
    use_agent_identity_auth: bool,
) -> anyhow::Result<agent9527_api::SharedAuthProvider> {
    if use_agent_identity_auth {
        let agent_identity_jwt = read_agent9527_access_token_from_env().ok_or_else(|| {
            anyhow::anyhow!(
                "AGENT9527_ACCESS_TOKEN is required when --use-agent-identity-auth is set"
            )
        })?;
        let auth_route_config = config.auth_route_config();
        let auth = Agent9527Auth::from_agent_identity_jwt(
            &agent_identity_jwt,
            Some(&config.chatgpt_base_url),
            &auth_route_config,
        )
        .await?;
        return Ok(agent9527_model_provider::auth_provider_from_auth(&auth));
    }

    let auth = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires ChatGPT authentication or API key authentication; run `agent9527 login` or set AGENT9527_API_KEY",
    )
    .await?;
    ...
    Ok(agent9527_model_provider::auth_provider_from_auth(&auth))
}

fn is_supported_exec_server_remote_auth(auth: &Agent9527Auth) -> bool {
...
async fn load_exec_server_config(
    root_config_overrides: &CliConfigOverrides,
    strict_config: bool,
) -> anyhow::Result<agent9527_core::config::Config> {
...
) -> anyhow::Result<agent9527_login::Agent9527Auth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_agent9527_api_key_env*/ true).await;
...
async fn enable_feature_in_config(feature: &str) -> anyhow::Result<()> {
    ...
    let agent9527_home = find_agent9527_home()?;
    ConfigEditsBuilder::new(&agent9527_home)
        .set_feature_enabled(feature, /*enabled*/ true)
        .apply()
        .await?;
    println!("Enabled feature `{feature}` in config.toml.");
    maybe_print_under_development_feature_warning(&agent9527_home, feature);
    Ok(())
}
...
fn loader_overrides_for_profile(
    ...
    match profile_v2 {
        Some(profile_v2) => {
            let agent9527_home = find_agent9527_home()?;
            Ok(loader_overrides_for_profile_at_agent9527_home(
                Some(profile_v2),
                &agent9527_home,
            ))
        }
...
fn loader_overrides_for_profile_at_agent9527_home(
    profile_v2: Option<&ProfileV2Name>,
    agent9527_home: &std::path::Path,
) -> LoaderOverrides {
    match profile_v2 {
        Some(profile_v2) => LoaderOverrides {
            user_config_path: Some(resolve_profile_v2_config_path(agent9527_home, profile_v2)),
...
fn maybe_print_under_development_feature_warning(agent9527_home: &std::path::Path, feature: &str) {
    ...
    let config_path = agent9527_home.join(agent9527_config::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_debug_prompt_input_command(
    ...
    let sandbox_mode = if shared.dangerously_bypass_approvals_and_sandbox {
        Some(agent9527_protocol::config_types::SandboxMode::DangerFullAccess)
    } else {
    ...
    let overrides = ConfigOverrides {
        ...
        sandbox_mode,
        cwd: shared.cwd,
        agent9527_self_exe: arg0_paths.agent9527_self_exe,
        agent9527_linux_sandbox_exe: arg0_paths.agent9527_linux_sandbox_exe,
    ...
    let user_instructions_provider = Arc::new(Agent9527HomeUserInstructionsProvider::new(
        config.agent9527_home.clone(),
    ));
    let auth_manager =
        AuthManager::shared_from_config(&config, /*enable_agent9527_api_key_env*/ false).await;
    let mut extensions = agent9527_extension_api::ExtensionRegistryBuilder::new();
    ...
    let prompt_input = agent9527_core::build_prompt_input(
        config,
        input,
        /*state_db*/ None,
...
async fn run_debug_clear_memories_command(
    ...
    clear_memory_roots_contents(&config.agent9527_home).await?;
...
fn reject_remote_mode_for_subcommand(
    ...
    if let Some(remote) = remote {
        anyhow::bail!(
            "`--remote {remote}` is only supported for interactive TUI commands, not `agent9527 {subcommand}`"
        );
    }
    if remote_auth_token_env.is_some() {
        anyhow::bail!(
            "`--remote-auth-token-env` is only supported for interactive TUI commands, not `agent9527 {subcommand}`"
        );
    }
    Ok(())
}
...
/// `agent9527 --strict-config` continue to work for the TUI and for wrappers that
...
fn reject_strict_config_for_unsupported_subcommand(
    ...
    if strict_config {
        anyhow::bail!("`--strict-config` is not supported for `agent9527 {subcommand}`");
    }
    Ok(())
}
...
async fn print_app_server_daemon_output(command: AppServerLifecycleCommand) -> anyhow::Result<()> {
    let output = agent9527_app_server_daemon::run(command).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}

fn updater_http_client_factory(
    config: anyhow::Result<agent9527_core::config::Config>,
) -> agent9527_http_client::HttpClientFactory {
    match config {
...
async fn print_app_server_remote_control_output(
    ...
    let output = agent9527_app_server_daemon::set_remote_control(mode).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}
...
async fn run_interactive_tui(
    ...
    let terminal_info = agent9527_terminal_detection::terminal_info();
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". Agent9527's interactive TUI may not work in this terminal."
        );
    ...
    let start_tui = || {
        agent9527_tui::run_main(
            interactive.clone(),
            arg0_paths.clone(),
            agent9527_config::LoaderOverrides::default(),
        ...
        match local_state_db::backup_files_for_fresh_start(startup_error).await {
            ...
            Err(backup_err) => {
                ...
                return Ok(AppExitInfo::fatal(format!(
                    "failed to move damaged Agent9527 local database files into a backup folder automatically: {backup_err}"
                )));
            }
        }
    }
...
fn resolve_remote_endpoint(
    remote: Option<String>,
    remote_auth_token_env: Option<String>,
) -> std::io::Result<Option<agent9527_tui::RemoteAppServerEndpoint>> {
    let mut remote_endpoint = remote
        .as_deref()
        .map(agent9527_tui::resolve_remote_addr)
        .transpose()
    ...
    if let Some(remote_auth_token_env) = remote_auth_token_env {
        ...
        if !agent9527_tui::remote_addr_supports_auth_token(endpoint) {
            return Err(std::io::Error::other(
        ...
        let auth_token = read_remote_auth_token_from_env_var(&remote_auth_token_env)
            .map_err(std::io::Error::other)?;
        let agent9527_tui::RemoteAppServerEndpoint::WebSocket {
            auth_token: slot, ..
        } = endpoint
        else {
            return Err(std::io::Error::other(
...
/// Build the final `TuiCli` for a `agent9527 resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `agent9527` without additional flags.
...
/// Build the final `TuiCli` for a `agent9527 fork` invocation.
...
mod tests {
    ...
    use agent9527_tui::TokenUsage;
    use assert_matches::assert_matches;
    use pretty_assertions::assert_eq;

    #[tokio::test]
    async fn updater_http_client_factory_honors_respect_system_proxy() {
        let agent9527_home = tempfile::tempdir().expect("temporary Agent9527 home");
        let config = ConfigBuilder::default()
        ...
        assert_eq!(
            ...
            agent9527_http_client::OutboundProxyPolicy::RespectSystemProxy
        );
    }
    ...
    fn exec_server_remote_auth_accepts_api_key_auth() {
        let auth = Agent9527Auth::from_api_key("sk-test");
    ...
    #[test]
    fn profile_loader_overrides_use_explicit_agent9527_home() -> anyhow::Result<()> {
        let agent9527_home = tempfile::tempdir()?;
        let profile: ProfileV2Name = "work".parse()?;

        let overrides =
            loader_overrides_for_profile_at_agent9527_home(Some(&profile), agent9527_home.path());
    ...
    fn profile_v2_is_rejected_for_config_management_subcommands() {
        assert!(
            profile_v2_for_args(&["agent9527", "--profile", "work", "features", "list"]).is_err()
        );
    }
    ...
    fn profile_v2_is_allowed_for_runtime_subcommands() {
        assert_eq!(
            profile_v2_for_args(&["agent9527", "--profile", "work", "resume"])
                .expect("resume supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["agent9527", "--profile", "work", "debug", "prompt-input"])
                .expect("debug prompt-input supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["agent9527", "--profile", "work", "mcp", "list"])
                .expect("mcp supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["agent9527", "--profile", "work", "sandbox"])
    ...
    fn import_remains_an_interactive_prompt() {
        let cli = MultitoolCli::try_parse_from(["agent9527", "import"]).expect("parse");
    ...
    fn profile_v2_rejects_non_plain_names_at_parse_time() {
        assert!(
            MultitoolCli::try_parse_from(["agent9527", "--profile", "nested/work", "resume"])
                .is_err()
        );
    }
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        ...
        let Some(agent9527_exec::Command::Resume(args)) = exec.command else {
    ...
    fn default_app_server_socket_path() -> AbsolutePathBuf {
        let agent9527_home = find_agent9527_home().expect("agent9527 home");
        agent9527_app_server::app_server_control_socket_path(&agent9527_home)
    ...
    fn debug_models_parses_bundled_flag() {
        let cli = MultitoolCli::try_parse_from(["agent9527", "debug", "models", "--bundled"])
            .expect("parse");
    ...
    #[test]
    fn account_commands_are_hidden_from_help() {
        let help = help_from_args(&["agent9527", "--help"]);
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        let help = help_from_args(&["agent9527", "plugin", "marketplace", "--help"]);
        assert!(
            help.contains("Usage: agent9527 plugin marketplace [OPTIONS] <COMMAND>"),
            "{help}"
        );

        for (subcommand, usage) in [
            ("add", "Usage: agent9527 plugin marketplace add"),
            ("list", "Usage: agent9527 plugin marketplace list"),
            ("upgrade", "Usage: agent9527 plugin marketplace upgrade"),
            ("remove", "Usage: agent9527 plugin marketplace remove"),
        ] {
            let help =
                help_from_args(&["agent9527", "plugin", "marketplace", subcommand, "--help"]);
    ...
    fn plugin_list_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["agent9527", "plugin", "list", "--marketplace", "debug"])
                .expect("parse");
    ...
    fn update_parses_as_update_subcommand() {
        let cli = MultitoolCli::try_parse_from(["agent9527", "update"]).expect("parse");
    ...
    fn sandbox_help_only_shows_singular_permission_profile() {
        let help = help_from_args(&["agent9527", "sandbox", "--help"]);
    ...
    fn sandbox_rejects_explicit_profile_controls_without_profile() {
        let err = MultitoolCli::try_parse_from(["agent9527", "sandbox", "-C", "/tmp"])
    ...
    fn plugin_marketplace_remove_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["agent9527", "plugin", "marketplace", "remove", "debug"])
                .expect("parse");
    ...
    fn marketplace_no_longer_parses_at_top_level() {
        let add_result =
            MultitoolCli::try_parse_from(["agent9527", "marketplace", "add", "owner/repo"]);
        assert!(add_result.is_err());

        let upgrade_result =
            MultitoolCli::try_parse_from(["agent9527", "marketplace", "upgrade", "debug"]);
        assert!(upgrade_result.is_err());

        let remove_result =
            MultitoolCli::try_parse_from(["agent9527", "marketplace", "remove", "debug"]);
        assert!(remove_result.is_err());
    }

    #[test]
    fn full_auto_no_longer_parses_at_top_level() {
        let result = MultitoolCli::try_parse_from(["agent9527", "--full-auto"]);
    ...
    fn exec_full_auto_reports_migration_path() {
        let cli = MultitoolCli::try_parse_from(["agent9527", "exec", "--full-auto", "summarize"])
    ...
    fn sandbox_full_auto_no_longer_parses() {
        let result = MultitoolCli::try_parse_from(["agent9527", "sandbox", "--full-auto", "--"]);
    ...
    fn sample_exit_info(conversation_id: Option<&str>, thread_name: Option<&str>) -> AppExitInfo {
        ...
        AppExitInfo {
            token_usage,
            thread_id,
            resume_hint: agent9527_utils_cli::resume_hint(thread_name, thread_id),
            update_action: None,
    ...
    fn format_exit_messages_includes_resume_hint_for_fatal_exit() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run agent9527 resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run agent9527 resume, then select my-thread (123e4567-e89b-12d3-a456-426614174000)".to_string(),
            ]
        );
    }
    ...
    fn resume_model_flag_applies_when_no_root_flags() {
        let interactive =
            finalize_resume_from_args(["agent9527", "resume", "-m", "gpt-5.1-test"].as_ref());
    ...
    fn resume_picker_logic_none_and_not_last() {
        let interactive = finalize_resume_from_args(["agent9527", "resume"].as_ref());
        assert!(interactive.resume_picker);
        assert!(!interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
    ...
    fn resume_picker_logic_last() {
        let interactive = finalize_resume_from_args(["agent9527", "resume", "--last"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
        assert!(!interactive.resume_show_all);
    ...
    fn resume_last_accepts_prompt_positional() {
        let interactive = finalize_resume_from_args(
            ["agent9527", "resume", "--last", "/compact focus on auth"].as_ref(),
        );
    ...
    fn resume_picker_logic_with_session_id() {
        let interactive = finalize_resume_from_args(["agent9527", "resume", "1234"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(!interactive.resume_last);
    ...
    fn resume_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_resume_from_args(["agent9527", "resume", "1234", "continue here"].as_ref());
    ...
    fn resume_all_flag_sets_show_all() {
        let interactive = finalize_resume_from_args(["agent9527", "resume", "--all"].as_ref());
        assert!(interactive.resume_picker);
        assert!(interactive.resume_show_all);
    }
    ...
    fn resume_include_non_interactive_flag_sets_source_filter_override() {
        let interactive = finalize_resume_from_args(
            ["agent9527", "resume", "--include-non-interactive"].as_ref(),
        );
    ...
    fn resume_merges_option_flags() {
        ...
        assert_matches!(
            ...
            Some(agent9527_utils_cli::SandboxModeCliArg::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(agent9527_utils_cli::ApprovalModeCliArg::OnRequest)
        );
        assert_eq!(
            interactive.cwd.as_deref(),
    ...
    fn resume_merges_bypass_hook_trust_flag() {
        let interactive = finalize_resume_from_args(
            ["agent9527", "resume", "--dangerously-bypass-hook-trust"].as_ref(),
        );
    ...
    fn fork_picker_logic_none_and_not_last() {
        let interactive = finalize_fork_from_args(["agent9527", "fork"].as_ref());
    ...
    fn fork_picker_logic_last() {
        let interactive = finalize_fork_from_args(["agent9527", "fork", "--last"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(interactive.fork_last);
        assert_eq!(interactive.fork_session_id, None);
        assert!(!interactive.fork_show_all);
    ...
    fn fork_last_accepts_prompt_positional() {
        let interactive = finalize_fork_from_args(
            ["agent9527", "fork", "--last", "/compact focus on auth"].as_ref(),
        );
    ...
    fn fork_last_rejects_explicit_session_and_prompt() {
        let err =
            MultitoolCli::try_parse_from(["agent9527", "fork", "--last", "1234", "continue here"])
                .expect_err("--last with an explicit session and prompt should be rejected");
    ...
    fn fork_picker_logic_with_session_id() {
        let interactive = finalize_fork_from_args(["agent9527", "fork", "1234"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(!interactive.fork_last);
    ...
    fn fork_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_fork_from_args(["agent9527", "fork", "1234", "continue here"].as_ref());
    ...
    fn fork_all_flag_sets_show_all() {
        let interactive = finalize_fork_from_args(["agent9527", "fork", "--all"].as_ref());
        assert!(interactive.fork_picker);
        assert!(interactive.fork_show_all);
    }
    ...
    fn app_server_analytics_default_disabled_without_flag() {
        let app_server = app_server_from_args(["agent9527", "app-server"].as_ref());
    ...
    fn app_server_remote_control_startup_flag_enables_remote_control() {
        let enabled =
            app_server_from_args(["agent9527", "app-server", "--remote-control"].as_ref());
    ...
    fn app_server_analytics_default_enabled_with_flag() {
        let app_server = app_server_from_args(
            ["agent9527", "app-server", "--analytics-default-enabled"].as_ref(),
        );
        assert!(app_server.analytics_default_enabled);
    }
    ...
    fn strict_config_parses_for_supported_commands() {
        let cli = MultitoolCli::try_parse_from(["agent9527", "--strict-config"]).expect("parse");
        assert!(cli.interactive.strict_config);

        let cli = MultitoolCli::try_parse_from(["agent9527", "mcp-server", "--strict-config"])
            .expect("parse");
        assert_matches!(
            cli.subcommand,
        ...
        let cli = MultitoolCli::try_parse_from(["agent9527", "exec-server", "--strict-config"])
            .expect("parse");
        assert_matches!(
            cli.subcommand,
    ...
    fn root_strict_config_is_supported_for_exec_server() {
        let cli = MultitoolCli::try_parse_from(["agent9527", "--strict-config", "exec-server"])
            .expect("parse");
    ...
    fn root_strict_config_is_rejected_for_unsupported_subcommands() {
        let cli = MultitoolCli::try_parse_from(["agent9527", "--strict-config", "mcp", "list"])
            .expect("parse");
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `agent9527 mcp`"
        );

        let cli = MultitoolCli::try_parse_from(["agent9527", "--strict-config", "remote-control"])
            .expect("parse");
        let err = reject_root_strict_config_for_subcommand(
            cli.interactive.strict_config,
            &cli.subcommand,
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `agent9527 remote-control`"
        );
    }
    ...
    fn app_server_subcommands_reject_strict_config() {
        let app_server =
            app_server_from_args(["agent9527", "app-server", "--strict-config", "proxy"].as_ref());
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `agent9527 app-server proxy`"
        );
    }
    ...
    fn reject_remote_flag_for_remote_control() {
        let cli =
            MultitoolCli::try_parse_from(["agent9527", "--remote", "unix://", "remote-control"])
                .expect("parse");
    ...
    fn remote_control_pair_parses() {
        let cli =
            MultitoolCli::try_parse_from(["agent9527", "remote-control", "pair"]).expect("parse");
        let Some(Subcommand::RemoteControl(remote_control)) = &cli.subcommand else {
            panic!("expected remote-control subcommand");
        };
    ...
    fn remote_flag_parses_for_interactive_root() {
        let cli = MultitoolCli::try_parse_from(["agent9527", "--remote", "unix://agent9527.sock"])
            .expect("parse");
        assert_eq!(cli.remote.remote.as_deref(), Some("unix://agent9527.sock"));
    }
    ...
    fn remote_auth_token_env_flag_parses_for_interactive_root() {
        ...
        assert_eq!(
            cli.remote.remote_auth_token_env.as_deref(),
            Some("AGENT9527_REMOTE_AUTH_TOKEN")
        );
    }
    ...
    fn remote_flag_parses_for_resume_subcommand() {
        ...
        assert_eq!(remote.remote.as_deref(), Some("unix://agent9527.sock"));
    }
    ...
    fn read_remote_auth_token_from_env_var_reports_missing_values() {
        let err = read_remote_auth_token_from_env_var_with("AGENT9527_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn read_remote_auth_token_from_env_var_trims_values() {
        let auth_token =
            read_remote_auth_token_from_env_var_with("AGENT9527_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn app_server_code_mode_host_url_parses_independently_of_listen_transport() {
        ...
        assert_eq!(
            app_server.listen,
            agent9527_app_server::AppServerTransport::WebSocket {
    ...
    fn app_server_listen_websocket_url_parses() {
        let app_server = app_server_from_args(
            ["agent9527", "app-server", "--listen", "ws://127.0.0.1:4500"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
            agent9527_app_server::AppServerTransport::WebSocket {
    ...
    fn app_server_listen_stdio_url_parses() {
        let app_server =
            app_server_from_args(["agent9527", "app-server", "--listen", "stdio://"].as_ref());
        assert_eq!(
            app_server.listen,
            agent9527_app_server::AppServerTransport::Stdio
        );
    ...
    fn app_server_stdio_flag_parses() {
        let app_server = app_server_from_args(["agent9527", "app-server", "--stdio"].as_ref());
    ...
    fn app_server_listen_unix_socket_url_parses() {
        let app_server =
            app_server_from_args(["agent9527", "app-server", "--listen", "unix://"].as_ref());
        assert_eq!(
            app_server.listen,
            agent9527_app_server::AppServerTransport::UnixSocket {
    ...
    fn app_server_listen_unix_socket_path_parses() {
        ...
        assert_eq!(
            app_server.listen,
            agent9527_app_server::AppServerTransport::UnixSocket {
                socket_path: AbsolutePathBuf::from_absolute_path("/tmp/agent9527.sock")
    ...
    fn app_server_listen_off_parses() {
        let app_server =
            app_server_from_args(["agent9527", "app-server", "--listen", "off"].as_ref());
        assert_eq!(
            app_server.listen,
            agent9527_app_server::AppServerTransport::Off
        );
    }
    ...
    fn app_server_listen_invalid_url_fails_to_parse() {
        let parse_result =
            MultitoolCli::try_parse_from(["agent9527", "app-server", "--listen", "http://foo"]);
        assert!(parse_result.is_err());
    }

    #[test]
    fn app_server_proxy_subcommand_parses() {
        let app_server = app_server_from_args(["agent9527", "app-server", "proxy"].as_ref());
        assert!(matches!(
    ...
    fn app_server_daemon_subcommands_parse() {
        ...
        assert!(matches!(
            app_server_from_args(["agent9527", "app-server", "daemon", "start"].as_ref())
                .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(["agent9527", "app-server", "daemon", "restart"].as_ref())
                .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(
                ["agent9527", "app-server", "daemon", "enable-remote-control"].as_ref()
            )
            .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(["agent9527", "app-server", "daemon", "stop"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
                subcommand: AppServerDaemonSubcommand::Stop
            }))
        ));
        assert!(matches!(
            app_server_from_args(["agent9527", "app-server", "daemon", "version"].as_ref())
                .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
    ...
    fn app_server_proxy_sock_path_parses() {
        ...
        assert_eq!(
            proxy.socket_path,
            Some(
                AbsolutePathBuf::relative_to_current_dir("agent9527.sock")
    ...
    fn app_server_capability_token_flags_parse() {
        ...
        assert_eq!(
            ...
            Some(agent9527_app_server::WebsocketAuthCliMode::CapabilityToken)
        );
        assert_eq!(
            app_server.auth.ws_token_file,
            Some(PathBuf::from("/tmp/agent9527-token"))
        );
    }
    ...
    fn app_server_signed_bearer_flags_parse() {
        ...
        assert_eq!(
            app_server.auth.ws_auth,
            Some(agent9527_app_server::WebsocketAuthCliMode::SignedBearerToken)
        );
        assert_eq!(
            app_server.auth.ws_shared_secret_file,
            Some(PathBuf::from("/tmp/agent9527-secret"))
        );
    ...
    fn features_enable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["agent9527", "features", "enable", "unified_exec"])
            .expect("parse should succeed");
    ...
    fn features_disable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["agent9527", "features", "disable", "shell_tool"])
            .expect("parse should succeed");
        let Some(Subcommand::Features(FeaturesCli { sub })) = cli.subcommand else {
            panic!("expected features subcommand");
        };
    ...
    fn strict_config_feature_toggle_error(args: &[&str]) -> anyhow::Error {
        let cli_args = std::iter::once("agent9527")

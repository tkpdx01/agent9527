...
use agent9527_core_plugins::ConfiguredMarketplace;
use agent9527_core_plugins::OPENAI_BUNDLED_MARKETPLACE_NAME;
use agent9527_core_plugins::PluginInstallOutcome;
...
use agent9527_core_plugins::allowed_configured_marketplace_names;
use agent9527_core_plugins::installed_marketplaces::marketplace_install_root;
use agent9527_core_plugins::installed_marketplaces::resolve_configured_marketplace_root;
use agent9527_core_plugins::marketplace::MarketplaceListError;
use agent9527_core_plugins::marketplace::MarketplacePluginAuthPolicy;
use agent9527_core_plugins::marketplace::MarketplacePluginInstallPolicy;
use agent9527_core_plugins::marketplace::MarketplacePluginSource;
use agent9527_core_plugins::marketplace::find_marketplace_manifest_path;
use agent9527_login::Agent9527Auth;
use agent9527_login::auth::read_agent9527_api_key_from_env;
...
#[derive(Debug, Parser)]
#[command(bin_name = "agent9527 plugin")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "agent9527 plugin add",
    after_help = "Examples:\n  agent9527 plugin add sample@debug\n  agent9527 plugin add sample --marketplace debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "agent9527 plugin list",
    after_help = "Examples:\n  agent9527 plugin list\n  agent9527 plugin list --marketplace debug\n  agent9527 plugin list --json\n  agent9527 plugin list --available --json"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "agent9527 plugin remove",
    after_help = "Examples:\n  agent9527 plugin remove sample@debug\n  agent9527 plugin remove sample --marketplace debug"
)]
...
pub async fn run_plugin_list(
    ...
    let marketplace_sources =
        configured_marketplace_sources(&plugins_input, agent9527_home.as_path());
        ...
        for (index, marketplace) in marketplaces.into_iter().enumerate() {
            ...
            for plugin in &marketplace.plugins {
                ...
                let path = match &plugin.source {
                    agent9527_core_plugins::marketplace::MarketplacePluginSource::Local {
                        path,
                    } => path.as_path().display().to_string(),
                    agent9527_core_plugins::marketplace::MarketplacePluginSource::Git {
                        url,
                        path,
                        ref_name,
                        sha,
                    ...
                    agent9527_core_plugins::marketplace::MarketplacePluginSource::Npm {
                        package,
                        version,
                        registry,
                    } => {
...
impl JsonPluginListOutput {
    fn from_marketplaces(
        marketplaces: Vec<agent9527_core_plugins::ConfiguredMarketplace>,
...
pub(crate) fn configured_marketplace_sources(
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names(&plugins_input.config_layer_stack, agent9527_home);
...
async fn load_plugin_command_context(
    ...
    let manager = PluginsManager::new(agent9527_home.to_path_buf());
...
pub(crate) async fn load_cli_auth_mode(config: &Config) -> Option<AuthMode> {
    if let Some(api_key) = read_agent9527_api_key_from_env() {
        return Some(Agent9527Auth::from_api_key(&api_key).api_auth_mode());
    }
...
pub(crate) fn configured_marketplace_snapshot_issues(
    ...
    let default_install_root = marketplace_install_root(agent9527_home);
    ...
    for (configured_name, marketplace) in configured_marketplaces {
        ...
        match find_marketplace_manifest_path(&root) {
            ...
            None => {
                if is_implicit_system_marketplace_root(configured_name, agent9527_home, &root) {
                    continue;
                }
                issues.push(ConfiguredMarketplaceSnapshotIssue {
                    marketplace_name: configured_name.clone(),

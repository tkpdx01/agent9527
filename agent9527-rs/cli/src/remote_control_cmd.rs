...
use agent9527_app_server_daemon::LifecycleOutput as AppServerLifecycleOutput;
use agent9527_app_server_daemon::LifecycleStatus as AppServerLifecycleStatus;
use agent9527_app_server_daemon::RemoteControlReadyOutput as AppServerRemoteControlReadyOutput;
use agent9527_app_server_daemon::RemoteControlReadyStatus as AppServerRemoteControlReadyStatus;
use agent9527_app_server_daemon::RemoteControlStartOutput as AppServerRemoteControlStartOutput;
...
pub(crate) async fn run(
    ...
    match command.subcommand {
        ...
        Some(RemoteControlSubcommand::Start) => {
            ...
            let output = agent9527_app_server_daemon::ensure_remote_control_ready().await?;
        ...
        Some(RemoteControlSubcommand::Stop) => {
            ...
            let output = agent9527_app_server_daemon::run(AppServerLifecycleCommand::Stop).await?;
            print_remote_control_stop_output(&output, command.json)?;
        }
        Some(RemoteControlSubcommand::Pair) => {
            let output = agent9527_app_server_daemon::start_remote_control_pairing().await?;
...
async fn run_foreground_remote_control(
    ...
    let mut app_server_task = tokio::spawn(agent9527_app_server::run_main_with_transport_options(
        arg0_paths,
...
async fn wait_for_foreground_remote_control_ready(
    ...
    agent9527_app_server_daemon::enable_remote_control_on_socket(
        socket_path.as_path(),
...
fn daemon_app_server_human_lines(output: &AppServerRemoteControlStartOutput) -> Vec<String> {
    let (managed_agent9527_path, managed_agent9527_version) = daemon_app_server_identity(output);
    vec![
        "Daemon used app-server:".to_string(),
        format!("  path: {}", managed_agent9527_path.display()),
        format!(
            "  version: {}",
            managed_agent9527_version.unwrap_or("unknown")
        ),
    ]
}
...
fn daemon_app_server_identity(
    ...
    match output {
        AppServerRemoteControlStartOutput::Bootstrap(output) => (
            &output.managed_agent9527_path,
            output.managed_agent9527_version.as_deref(),
        ),
...
mod tests {
    ...
    fn daemon_ready_output(
        ...
        AppServerRemoteControlReadyOutput {
            daemon: AppServerRemoteControlStartOutput::Start(AppServerLifecycleOutput {
                ...
                backend: None,
                pid: Some(42),
                managed_agent9527_path: PathBuf::from("/opt/agent9527/bin/agent9527"),
                managed_agent9527_version: Some("1.0.0".to_string()),
    ...
    fn daemon_app_server_human_lines_include_path_and_version() {
        assert_eq!(
            ...
            vec![
                "Daemon used app-server:".to_string(),
                "  path: /opt/agent9527/bin/agent9527".to_string(),
        ...
        assert_eq!(
            ...
            json!({
                ...
                "daemon": {
                    "status": "started",
                    "pid": 42,
                    "managedAgent9527Path": "/opt/agent9527/bin/agent9527",
                    "managedAgent9527Version": "1.0.0",

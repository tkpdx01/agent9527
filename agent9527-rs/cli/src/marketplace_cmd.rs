...
use agent9527_core_plugins::PluginMarketplaceUpgradeOutcome;
...
use agent9527_core_plugins::marketplace::marketplace_root_dir;
use agent9527_core_plugins::marketplace_add::MarketplaceAddOutcome;
use agent9527_core_plugins::marketplace_add::MarketplaceAddRequest;
use agent9527_core_plugins::marketplace_add::add_marketplace;
use agent9527_core_plugins::marketplace_remove::MarketplaceRemoveOutcome;
use agent9527_core_plugins::marketplace_remove::MarketplaceRemoveRequest;
...
#[derive(Debug, Parser)]
#[command(bin_name = "agent9527 plugin marketplace")]
...
enum MarketplaceSubcommand {
    ...
    /// List plugin marketplaces Agent9527 is currently considering and their roots.
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "agent9527 plugin marketplace add",
    after_help = "Examples:\n  agent9527 plugin marketplace add ./path/to/marketplace\n  agent9527 plugin marketplace add owner/repo --ref main\n  agent9527 plugin marketplace add https://github.com/owner/repo --sparse plugins/foo"
)]
...
#[derive(Debug, Parser)]
#[command(bin_name = "agent9527 plugin marketplace list")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "agent9527 plugin marketplace upgrade",
    after_help = "Examples:\n  agent9527 plugin marketplace upgrade\n  agent9527 plugin marketplace upgrade debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "agent9527 plugin marketplace remove",
    after_help = "Example:\n  agent9527 plugin marketplace remove debug"
)]
...
async fn run_list(overrides: Vec<(String, toml::Value)>, args: ListMarketplaceArgs) -> Result<()> {
    let config = Config::load_with_cli_overrides(overrides)
        .await
        .context("failed to load configuration")?;
    let manager = PluginsManager::new(config.agent9527_home.to_path_buf());
    ...
    if args.json {
        let marketplace_sources =
            configured_marketplace_sources_by_root(config.agent9527_home.as_path(), &plugins_input);
        let output =
...
impl JsonMarketplaceListOutput {
    fn from_marketplaces(
        marketplaces: Vec<agent9527_core_plugins::marketplace::Marketplace>,
...
fn configured_marketplace_sources_by_root(
    ...
    let marketplace_sources = configured_marketplace_sources(plugins_input, agent9527_home);

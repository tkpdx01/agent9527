...
//! interactive-session layers. Direct `agent9527 login` intentionally does less: it preserves the
...
//! into a one-shot CLI command while still producing a durable `agent9527-login.log` artifact that
...
use agent9527_login::CLIENT_ID;
use agent9527_login::ServerOptions;
use agent9527_login::login_with_access_token;
use agent9527_login::login_with_api_key;
use agent9527_login::logout_with_revoke;
use agent9527_login::run_device_code_login;
...
/// Installs a small file-backed tracing layer for direct `agent9527 login` flows.
///
...
/// command produce a durable `agent9527-login.log` artifact without coupling it to the TUI's broader
/// telemetry and feedback initialization.
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match agent9527_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
    ...
    let log_path = log_dir.join("agent9527-login.log");
    ...
    let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        EnvFilter::new("agent9527_cli=info,agent9527_core=info,agent9527_login=info")
    });
    ...
    // Direct `agent9527 login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `agent9527 login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    read_stdin_secret(
        "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | agent9527 login --with-api-key`.",
...
pub fn read_access_token_from_stdin() -> String {
    read_stdin_secret(
        "--with-access-token expects the access token on stdin. Try piping it, e.g. `printenv AGENT9527_ACCESS_TOKEN | agent9527 login --with-access-token`.",
...
/// `agent9527 login` working in environments where device-code may be disabled/feature-gated.
...
pub async fn run_login_status(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match Agent9527Auth::from_auth_storage(
        &config.agent9527_home,
        config.cli_auth_credentials_store_mode,
        Some(&config.chatgpt_base_url),
        config.auth_keyring_backend_kind(),
...
mod tests {
    ...
    async fn clears_existing_auth_before_login() {
        let agent9527_home = tempdir().expect("create temporary Agent9527 home");
        login_with_api_key(
            agent9527_home.path(),
            "sk-existing",

...
const OTEL_SERVICE_NAME: &str = "agent9527-exec-server";

pub(crate) fn init(
    config: Option<&agent9527_core::config::Config>,
) -> (impl Send + Sync, agent9527_exec_server::ExecServerTelemetry) {
    let fmt_layer = tracing_subscriber::fmt::layer()
        .with_writer(std::io::stderr)
        .with_filter(stderr_env_filter());
    let otel = match config {
        Some(config) => agent9527_core::otel_init::build_provider(
            config,
    ...
    agent9527_core::otel_init::record_process_start(provider, OTEL_SERVICE_NAME);
    ...
    let telemetry = provider
        ...
        .cloned()
        .map(agent9527_exec_server::ExecServerTelemetry::new)
        .unwrap_or_default();

...
fn write_footer(out: &mut String, options: HumanOutputOptions) {
    if options.show_details {
        ...
        let _ = writeln!(
            ...
            "{}",
            dim(
                "Run agent9527 doctor without --summary for detailed diagnostics.",
                options
            )
        );
        let _ = writeln!(
...
fn header_suffix(report: &DoctorReport) -> String {
    let version = format!("v{}", report.agent9527_version);
    report
        .checks
        .iter()
...
fn update_note(check: &DoctorCheck, report: &DoctorReport) -> Option<DoctorNote> {
    ...
    let mut parenthetical = format!("current {}", report.agent9527_version);
...
mod tests {
    ...
    fn sample_report() -> DoctorReport {
        let checks = vec![
            ...
            .detail("terminal title project value: agent9527"),
            DoctorCheck::new(
                "state.paths",
                "state",
                CheckStatus::Ok,
            ...
            .remediation("Run `agent9527 login`."),
            DoctorCheck::new(
                "updates.status",
                "updates",
                CheckStatus::Ok,
    ...
    fn render_human_report_includes_details_by_default_without_color() {
        ...
        let expected = format!(
            "\
Agent9527 Doctor v0.0.0

Notes
   ⚠ terminal     narrow terminal
   ✗ auth         token expired - Run `agent9527 login`.
─────────────────────────────────────────────────────────────
...
Environment
  ...
  ✓ git          git version 2.54.0
      ...
      repo detected            true
  ⚠ terminal     narrow terminal
  ✓ title        default · project agent9527
      title source             default
      title items              activity, project-name
      project value            agent9527
...
Notes
   [!!] terminal     narrow terminal
   [XX] auth         token expired - Run `agent9527 login`.
-------------------------------------------------------------
...
Environment
  ...
  [!!] terminal     narrow terminal
  [ok] title        default | project agent9527
...
",
    ...
    fn detail_value_colors_inline_statuses_and_low_signal_values() {
        let rendered = detail_value(
            "npm: no · commit unknown · integrity ok · ~/code/agent9527/target/debug/agent9527 · <redacted>",
        ...
        assert!(rendered.contains("\u{1b}[38;5;117m~/code/agent9527/target/debug/agent9527"));
    ...
    fn redact_detail_preserves_env_var_names() {
        assert_eq!(
            redact_detail("auth env vars present: OPENAI_API_KEY, AGENT9527_API_KEY"),
            "auth env vars present: OPENAI_API_KEY, AGENT9527_API_KEY"
        );
    }

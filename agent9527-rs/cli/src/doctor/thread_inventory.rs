...
async fn thread_inventory_check_for_roots(
    ...
    let state_db_path = agent9527_state::state_db_path(sqlite_home);
    ...
    let rows = match agent9527_state::read_thread_state_audit_rows(&state_db_path).await {
        Ok(rows) => rows,
        Err(err) => {
...
mod tests {
    ...
    impl Fixture {
        ...
        async fn insert_thread_row(&self, id: &str, rollout_path: &Path, archived: bool) {
            let state_db_path = agent9527_state::state_db_path(self.sqlite_home.path());
            let pool = agent9527_state::SqliteConfig::new_for_testing(

...
fn install_details(parsed: &[ParsedDetail], options: HumanOutputOptions) -> Vec<HumanDetail> {
    ...
    let path_entries = numbered_values(parsed, "PATH agent9527 #");
...
fn state_details(parsed: &[ParsedDetail]) -> Vec<HumanDetail> {
    let mut out = Vec::new();
    push_row_if_present(&mut out, parsed, "AGENT9527_HOME", "AGENT9527_HOME");
...
fn display_label(label: &str) -> String {
    match label {
        "agent9527-linux-sandbox helper" => "linux helper",

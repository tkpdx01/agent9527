...
use agent9527_mcp::ResolvedMcpOAuthScopes;
use agent9527_mcp::compute_auth_statuses;
use agent9527_mcp::discover_supported_scopes;
...
use agent9527_rmcp_client::delete_oauth_tokens;
use agent9527_rmcp_client::perform_oauth_login;
use agent9527_utils_cli::CliConfigOverrides;
use agent9527_utils_cli::format_env_display;
use anyhow::Context;
use anyhow::Result;
use anyhow::anyhow;
use anyhow::bail;
...
/// - `add`    — add a server launcher entry to `~/.agent9527/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "agent9527 mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: agent9527_config::types::OAuthCredentialsStoreMode,
    keyring_backend_kind: agent9527_config::types::AuthKeyringBackendKind,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let mut servers = load_global_mcp_servers(&agent9527_home)
        .await
        .with_context(|| {
            format!(
    ...
    match oauth_login_support(&transport).await {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `agent9527 mcp login {name}` to login."
        ),
    }
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `agent9527 mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: agent9527 mcp remove {}", get_args.name);

...
async fn add_and_remove_server_updates_global_config() -> Result<()> {
    ...
    let mut add_cmd = agent9527_command(agent9527_home.path())?;
    add_cmd
    ...
    let mut remove_cmd = agent9527_command(agent9527_home.path())?;
    remove_cmd
    ...
    let mut remove_again_cmd = agent9527_command(agent9527_home.path())?;
    remove_again_cmd
        .args(["mcp", "remove", "docs"])
        .assert()
        .success()
...
async fn add_streamable_http_without_manual_token() -> Result<()> {
    ...
    assert!(!agent9527_home.path().join(".credentials.json").exists());
    assert!(!agent9527_home.path().join(".env").exists());
...
async fn add_streamable_http_with_oauth_options() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some("eci-prd-pub-agent9527-123")
    );
    assert_eq!(

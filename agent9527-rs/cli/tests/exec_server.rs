...
"#
    ...
    let agent9527_bin = agent9527_utils_cargo_bin::cargo_bin("agent9527")?;
    let agent9527_home = agent9527_home.path().to_path_buf();
    let subprocess = async move {
        let mut command = tokio::process::Command::new(agent9527_bin);
        command
...
fn local_exec_server_exits_successfully_on_sigterm() -> Result<()> {
    let agent9527_home = TempDir::new()?;
    let mut child = std::process::Command::new(agent9527_utils_cargo_bin::cargo_bin("agent9527")?)

...
async fn configure_http_oauth_server(agent9527_home: &Path, url: &str) -> Result<()> {
    let mut servers = load_global_mcp_servers(agent9527_home).await?;
    servers.insert(
        "oauth".to_string(),
...
async fn list_discovers_local_oauth_server_through_environment_proxy() -> Result<()> {
    let agent9527_home = TempDir::new()?;
    configure_http_oauth_server(agent9527_home.path(), "http://mcp-proxy.invalid/mcp").await?;
    ...
    let mut command = agent9527_command(agent9527_home.path())?;
    command
...
async fn list_with_macos_proxy_resolution_does_not_panic() -> Result<()> {
    ...
    Mock::given(method("GET"))
        ...
        .mount(&server)
        .await;
    configure_http_oauth_server(agent9527_home.path(), &format!("{}/mcp", server.uri())).await?;
...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = agent9527_command(agent9527_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(agent9527_home.path()).await?;
    ...
    let mut list_cmd = agent9527_command(agent9527_home.path())?;
    ...
    let mut list_json_cmd = agent9527_command(agent9527_home.path())?;
    ...
    let mut get_cmd = agent9527_command(agent9527_home.path())?;
    ...
    assert!(stdout.contains("remove: agent9527 mcp remove docs"));

    let mut get_json_cmd = agent9527_command(agent9527_home.path())?;
    get_json_cmd

...
fn write_file_auth_config(agent9527_home: &Path) -> Result<()> {
    std::fs::write(
...
fn read_auth_json(agent9527_home: &Path) -> Result<Value> {
    let auth_json = std::fs::read_to_string(agent9527_home.join("auth.json"))?;
...
fn login_with_api_key_reads_stdin_and_writes_auth_json() -> Result<()> {
    ...
    write_file_auth_config(agent9527_home.path())?;
    ...
    let auth = read_auth_json(agent9527_home.path())?;
...
async fn debug_prompt_input_follows_authenticated_attribution_setting() -> Result<()> {
    ...
    for enabled in [true, false] {
        let output = agent9527_command(agent9527_home.path())?
            .env("NO_PROXY", "127.0.0.1,localhost")
            .env("no_proxy", "127.0.0.1,localhost")
            .env_remove("AGENT9527_ACCESS_TOKEN")
        ...
        assert_eq!(
            prompt.contains("Co-authored-by: Agent9527 <noreply@openai.com>"),
            enabled
        );

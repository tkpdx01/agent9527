...
const PACKAGE_METADATA_FILENAME: &str = "agent9527-package.json";
const PATH_DIRNAME: &str = "agent9527-path";
const RELEASES_DIRNAME: &str = "releases";
const RESOURCES_DIRNAME: &str = "agent9527-resources";
...
pub struct InstallContext {
    ...
    pub package_layout: Option<Agent9527PackageLayout>,
}
...
pub enum InstallMethod {
    Standalone {
        ...
        /// such as
        /// `~/.agent9527/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        /// Package-layout installs use the package root that contains `bin/`,
        /// `agent9527-resources/`, and `agent9527-path/`.
    ...
    /// A Agent9527 binary launched through the npm-managed `agent9527.js` shim.
    Npm,
    /// A Agent9527 binary launched through the bun-managed `agent9527.js` shim.
    Bun,
    /// A Agent9527 binary launched through the pnpm-managed `agent9527.js` shim.
    Pnpm,
    /// A Agent9527 binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled Agent9527 binaries, custom
    /// internal launchers, and tests that execute Agent9527 from an arbitrary path.
    Other,
}
...
impl InstallContext {
    pub fn from_exe(
        ...
        let agent9527_home = agent9527_utils_home_dir::find_agent9527_home().ok();
        Self::from_exe_with_agent9527_home(
            is_macos,
            current_exe,
            method_override,
            agent9527_home.as_deref(),
        )
    }

    fn from_exe_with_agent9527_home(
        is_macos: bool,
        current_exe: Option<&Path>,
        method_override: Option<InstallMethod>,
        agent9527_home: Option<&Path>,
    ) -> Self {
        let package_layout = current_exe.and_then(Agent9527PackageLayout::from_exe);
        let method = if let Some(method) = method_override {
            method
        } else if let Some(exe_path) = current_exe {
            install_method_from_exe(exe_path, agent9527_home, package_layout.as_ref(), is_macos)
        } else {
            InstallMethod::Other
        };
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let method_override = if std::env::var_os("AGENT9527_MANAGED_BY_PNPM").is_some() {
                Some(InstallMethod::Pnpm)
            } else if std::env::var_os("AGENT9527_MANAGED_BY_NPM").is_some() {
                Some(InstallMethod::Npm)
            } else if std::env::var_os("AGENT9527_MANAGED_BY_BUN").is_some() {
...
fn install_method_from_exe(
    ...
    if let Some(standalone_method) =
        standalone_install_method(exe_path, agent9527_home, package_layout)
    {
...
fn standalone_install_method(
    ...
    let canonical_agent9527_home = canonical_absolute_path(agent9527_home?)?;
    ...
    let releases_root = canonical_agent9527_home
        .join("packages")
...
mod tests {
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        ...
        let context = InstallContext::from_exe_with_agent9527_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(&exe_path),
            /*method_override*/ None,
            /*agent9527_home*/ Some(agent9527_home.path()),
        );
        assert_eq!(
            context,
            InstallContext {
    ...
    fn detects_package_layout_independently_from_install_method() -> std::io::Result<()> {
        ...
        let package_layout = Agent9527PackageLayout {
    ...
    fn standalone_package_layout_keeps_standalone_install_method() -> std::io::Result<()> {
        let agent9527_home = tempfile::tempdir()?;
        let package_dir = agent9527_home
            .path()
            .join("packages/standalone/releases/1.2.3-x86_64-unknown-linux-musl");
    ...
    #[test]
    fn standalone_package_rg_falls_back_when_agent9527_path_is_missing() -> std::io::Result<()> {
        let package_dir = tempfile::tempdir()?;
        let bin_dir = package_dir.path().join(BIN_DIRNAME);
        fs::create_dir_all(&bin_dir)?;
        fs::write(package_dir.path().join(PACKAGE_METADATA_FILENAME), "{}")?;
    ...
    fn package_manager_method_overrides_take_precedence() {
        let pnpm_context = InstallContext::from_exe(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/agent9527")),
    ...
    fn brew_is_detected_on_macos_prefixes() {
        let context = InstallContext::from_exe_with_agent9527_home(
            /*is_macos*/ true,
            /*current_exe*/ Some(Path::new("/opt/homebrew/bin/agent9527")),
            /*method_override*/ None,
            /*agent9527_home*/ None,
        );
        assert_eq!(

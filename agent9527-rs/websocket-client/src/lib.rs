//! Proxy-aware WebSocket connection setup shared by Agent9527 API clients.
...
use agent9527_http_client::build_rustls_client_config_with_custom_ca;
use futures::Sink;
use futures::Stream;
...
/// Selects whether WebSocket TLS follows Agent9527 custom-CA policy or Tungstenite defaults.
#[derive(Clone, Copy, PartialEq, Eq)]
pub enum WebSocketTlsMode {
    /// Build an explicit TLS configuration from native roots and configured Agent9527 custom CAs.
    ExplicitAgent9527Tls,
...
impl WebSocketConnector {
    /// Creates a connector using native roots and any configured Agent9527 custom CA bundle.
    pub fn new(
        http_client_factory: &HttpClientFactory,
    ) -> Result<Self, BuildCustomCaTransportError> {
        Self::new_with_tls_mode(http_client_factory, WebSocketTlsMode::ExplicitAgent9527Tls)
    }

    /// Creates a connector with explicit Agent9527 TLS or the transport's existing TLS defaults.
    ///
    /// HTTPS proxy connections still build Agent9527 TLS configuration when they establish their
    ...
    pub fn new_with_tls_mode(
        ...
        let tls_config = match tls_mode {
            WebSocketTlsMode::ExplicitAgent9527Tls => {

...
use crate::events::Agent9527AcceptedLineFingerprintsEventParams;
use crate::events::Agent9527AcceptedLineFingerprintsEventRequest;
use crate::events::Agent9527AppMentionedEventRequest;
use crate::events::Agent9527AppServerClientMetadata;
use crate::events::Agent9527AppUsedEventRequest;
use crate::events::Agent9527CommandExecutionEventParams;
use crate::events::Agent9527CommandExecutionEventRequest;
use crate::events::Agent9527CompactionEventRequest;
use crate::events::Agent9527HookRunEventRequest;
use crate::events::Agent9527OnboardingExternalAgentImportFailureEventRequest;
use crate::events::Agent9527OnboardingExternalAgentImportFailureMetadata;
use crate::events::Agent9527PluginEventRequest;
use crate::events::Agent9527PluginInstallFailedEventRequest;
use crate::events::Agent9527PluginInstallFailedMetadata;
use crate::events::Agent9527PluginUsedEventRequest;
use crate::events::Agent9527ReviewEventParams;
use crate::events::Agent9527ReviewEventRequest;
use crate::events::Agent9527RuntimeMetadata;
use crate::events::Agent9527ToolItemEventBase;
use crate::events::Agent9527TurnEventRequest;
...
use crate::events::agent9527_app_metadata;
use crate::events::agent9527_hook_run_metadata;
use crate::events::agent9527_plugin_metadata;
use crate::events::agent9527_plugin_used_metadata;
...
use crate::facts::TurnAgent9527ErrorFact;
...
use agent9527_app_server_protocol::ApprovalsReviewer as AppServerApprovalsReviewer;
use agent9527_app_server_protocol::AskForApproval as AppServerAskForApproval;
...
use agent9527_app_server_protocol::CollabAgentTool;
use agent9527_app_server_protocol::CollabAgentToolCallStatus;
use agent9527_app_server_protocol::CommandAction;
use agent9527_app_server_protocol::CommandExecutionApprovalDecision;
use agent9527_app_server_protocol::CommandExecutionRequestApprovalParams;
use agent9527_app_server_protocol::CommandExecutionRequestApprovalResponse;
use agent9527_app_server_protocol::CommandExecutionSource;
use agent9527_app_server_protocol::CommandExecutionStatus;
use agent9527_app_server_protocol::DynamicToolCallStatus;
use agent9527_app_server_protocol::GuardianApprovalReview;
use agent9527_app_server_protocol::GuardianApprovalReviewAction;
use agent9527_app_server_protocol::GuardianApprovalReviewStatus;
use agent9527_app_server_protocol::GuardianCommandSource as AppServerGuardianCommandSource;
use agent9527_app_server_protocol::ImageGenerationItem;
use agent9527_app_server_protocol::InitializeCapabilities;
use agent9527_app_server_protocol::InitializeParams;
use agent9527_app_server_protocol::ItemCompletedNotification;
use agent9527_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification;
use agent9527_app_server_protocol::ItemStartedNotification;
use agent9527_app_server_protocol::JSONRPCErrorError;
use agent9527_app_server_protocol::McpToolCallAppContext;
use agent9527_app_server_protocol::McpToolCallStatus;
use agent9527_app_server_protocol::NonSteerableTurnKind;
use agent9527_app_server_protocol::PatchApplyStatus;
use agent9527_app_server_protocol::PermissionsRequestApprovalParams;
use agent9527_app_server_protocol::RequestId;
use agent9527_app_server_protocol::RequestPermissionProfile;
use agent9527_app_server_protocol::SandboxPolicy as AppServerSandboxPolicy;
...
use agent9527_app_server_protocol::SessionSource as AppServerSessionSource;
use agent9527_app_server_protocol::SubAgentActivityKind;
...
use agent9527_app_server_protocol::ThreadSource as AppServerThreadSource;
use agent9527_app_server_protocol::ThreadStartResponse;
use agent9527_app_server_protocol::ThreadStatus as AppServerThreadStatus;
use agent9527_app_server_protocol::Turn;
use agent9527_app_server_protocol::TurnCompletedNotification;
use agent9527_app_server_protocol::TurnDiffUpdatedNotification;
use agent9527_app_server_protocol::TurnError as AppServerTurnError;
use agent9527_app_server_protocol::TurnStartParams;
use agent9527_app_server_protocol::TurnStartedNotification;
use agent9527_app_server_protocol::TurnStatus as AppServerTurnStatus;
...
use agent9527_app_server_protocol::WebSearchItem;
use agent9527_login::default_client::DEFAULT_ORIGINATOR;
use agent9527_login::default_client::originator;
use agent9527_plugin::AppConnectorId;
use agent9527_plugin::PluginCapabilitySummary;
...
use agent9527_protocol::models::NetworkPermissions as CoreNetworkPermissions;
use agent9527_protocol::models::PermissionProfile as CorePermissionProfile;
...
use agent9527_protocol::request_permissions::PermissionGrantScope as CorePermissionGrantScope;
use agent9527_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use agent9527_protocol::request_permissions::RequestPermissionsResponse as CoreRequestPermissionsResponse;
use agent9527_utils_absolute_path::test_support::PathBufExt;
use agent9527_utils_absolute_path::test_support::test_path_buf;
...
const TEST_PRODUCT_CLIENT_ID: &str = "agent9527_work_desktop";
...
fn sample_app_server_client_metadata() -> Agent9527AppServerClientMetadata {
    Agent9527AppServerClientMetadata {
...
fn sample_runtime_metadata() -> Agent9527RuntimeMetadata {
    Agent9527RuntimeMetadata {
        agent9527_rs_version: "0.1.0".to_string(),
...
fn sample_turn_start_response(turn_id: &str) -> ClientResponsePayload {
    ClientResponsePayload::TurnStart(agent9527_app_server_protocol::TurnStartResponse {
        turn: Turn {
            id: turn_id.to_string(),
            items_view: agent9527_app_server_protocol::TurnItemsView::Full,
            items: vec![],
...
fn sample_turn_completed_notification(
    ...
    agent9527_error_info: Option<agent9527_app_server_protocol::Agent9527ErrorInfo>,
) -> ServerNotification {
    ServerNotification::TurnCompleted(TurnCompletedNotification {
        ...
        turn: Turn {
            ...
            items: vec![],
            status,
            error: agent9527_error_info.map(|agent9527_error_info| AppServerTurnError {
                message: "turn failed".to_string(),
                agent9527_error_info: Some(agent9527_error_info),
    ...
    JSONRPCErrorError {
        ...
        data: Some(
            serde_json::to_value(AppServerTurnError {
                message: "cannot steer a review turn".to_string(),
                agent9527_error_info: Some(Agent9527ErrorInfo::ActiveTurnNotSteerable {
            ...
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "agent9527-web".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "agent9527-web".to_string(),
    ...
    reducer
        .ingest(
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "agent9527-tui".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "agent9527-tui".to_string(),
                runtime: sample_runtime_metadata(),
                rpc_transport: AppServerRpcTransport::Stdio,
            },
            out,
...
fn sample_initialize_fact(connection_id: u64) -> AnalyticsFact {
    AnalyticsFact::Initialize {
        ...
        runtime: Agent9527RuntimeMetadata {
            agent9527_rs_version: "0.99.0".to_string(),
    ...
    ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: RequestPermissionProfile {
                network: Some(
                    agent9527_app_server_protocol::AdditionalNetworkPermissions {
                        enabled: Some(true),
                    },
                ),
                file_system: None,
...
fn sample_guardian_review_completed(
    ...
    ServerNotification::ItemGuardianApprovalReviewCompleted(
        ItemGuardianApprovalReviewCompletedNotification {
            ...
            decision_source: agent9527_app_server_protocol::AutoReviewDecisionSource::Agent,
...
fn normalize_path_for_skill_id_repo_scoped_uses_relative_path() {
    ...
    let skill_path = PathBuf::from("/repo/root/.agent9527/skills/doc/SKILL.md");
    ...
    assert_eq!(path, ".agent9527/skills/doc/SKILL.md");
}

#[test]
fn normalize_path_for_skill_id_user_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/Users/abc/.agent9527/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_admin_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/etc/agent9527/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_repo_root_not_in_skill_path_uses_absolute_path() {
    let repo_root = PathBuf::from("/repo/root");
    let skill_path = PathBuf::from("/other/path/.agent9527/skills/doc/SKILL.md");
...
fn app_mentioned_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppMentioned(Agent9527AppMentionedEventRequest {
        event_type: "agent9527_app_mentioned",
        event_params: agent9527_app_metadata(
            &tracking,
            AppInvocation {
...
fn app_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppUsed(Agent9527AppUsedEventRequest {
        event_type: "agent9527_app_used",
        event_params: agent9527_app_metadata(
            &tracking,
            AppInvocation {
...
fn accepted_line_fingerprints_event_serializes_expected_shape() {
    let event = TrackEventRequest::AcceptedLineFingerprints(Box::new(
        Agent9527AcceptedLineFingerprintsEventRequest {
            event_type: "agent9527_accepted_line_fingerprints",
            event_params: Agent9527AcceptedLineFingerprintsEventParams {
                event_type: "agent9527.accepted_line_fingerprints",
                turn_id: "turn-1".to_string(),
                thread_id: "thread-1".to_string(),
                product_surface: Some("agent9527".to_string()),
                model_slug: Some("gpt-5.1-agent9527".to_string()),
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": "turn-1",
                "thread_id": "thread-1",
                "product_surface": "agent9527",
                "model_slug": "gpt-5.1-agent9527",
                "completed_at": 1710000000,
...
async fn analytics_flush_delivers_completed_turn_with_file_diff() {
    ...
    let capture_path = std::env::temp_dir().join(format!(
        "agent9527-analytics-turn-flush-{}-{nonce}.jsonl",
        std::process::id()
    ));
    let auth_manager = agent9527_login::AuthManager::from_auth_for_testing(
        agent9527_login::Agent9527Auth::create_dummy_chatgpt_auth_for_testing(),
    );
...
"
    ...
    assert!(
        event_types
            .iter()
            .any(|event| event == "agent9527_turn_event")
    );
    assert!(
        event_types
            .iter()
            .any(|event| event == "agent9527_accepted_line_fingerprints")
    );
...
fn compaction_event_serializes_expected_shape() {
    let event = TrackEventRequest::Compaction(Box::new(Agent9527CompactionEventRequest {
        event_type: "agent9527_compaction_event",
        event_params: crate::events::agent9527_compaction_event_params(
            Agent9527CompactionEvent {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
                ...
                agent9527_error_kind: None,
                agent9527_error_http_status_code: None,
        ...
        json!({
            ...
            "event_params": {
                ...
                "app_server_client": {
                    ...
                    "client_name": "agent9527-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
                },
                "runtime": {
                    "agent9527_rs_version": "0.1.0",
                    "runtime_os": "macos",
                ...
                "strategy": "memento",
                "status": "completed",
                "agent9527_error_kind": null,
                "agent9527_error_http_status_code": null,
...
fn command_execution_event_serializes_expected_shape() {
    let event = TrackEventRequest::CommandExecution(Agent9527CommandExecutionEventRequest {
        event_type: "agent9527_command_execution_event",
        event_params: Agent9527CommandExecutionEventParams {
            base: Agent9527ToolItemEventBase {
                thread_id: "thread-1".to_string(),
                session_id: "session-thread-1".to_string(),
                turn_id: "turn-1".to_string(),
                item_id: "item-1".to_string(),
                app_server_client: Agent9527AppServerClientMetadata {
                    product_client_id: "agent9527_tui".to_string(),
                    client_name: Some("agent9527-tui".to_string()),
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "item_id": "item-1",
                "app_server_client": {
                    "product_client_id": "agent9527_tui",
                    "client_name": "agent9527-tui",
...
fn review_event_serializes_expected_shape() {
    let event = TrackEventRequest::ReviewEvent(Agent9527ReviewEventRequest {
        event_type: "agent9527_review_event",
        event_params: Agent9527ReviewEventParams {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            item_id: None,
...
async fn initialize_caches_client_and_thread_lifecycle_publishes_once_initialized() {
    ...
    assert_eq!(payload[0]["event_type"], "agent9527_thread_initialized");
    ...
    assert_eq!(
        payload[0]["event_params"]["runtime"]["agent9527_rs_version"],
        "0.99.0"
    );
...
async fn thread_originator_overrides_shared_connection_across_thread_events() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            json!({
                "event_type": "agent9527_turn_event",
                "product_client_id": TEST_PRODUCT_CLIENT_ID,
            }),
            json!({
                "event_type": "agent9527_compaction_event",
...
async fn compaction_event_ingests_custom_fact() {
    ...
    let parent_thread_id =
        agent9527_protocol::ThreadId::from_string("22222222-2222-2222-2222-222222222222")
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::Compaction(Box::new(
                Agent9527CompactionEvent {
                    ...
                    agent9527_error_kind: Some(Agent9527ErrKind::ContextWindowExceeded),
                    agent9527_error_http_status_code: None,
                    active_context_tokens_before: 131_000,
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "agent9527_compaction_event");
    assert_eq!(payload[0]["event_params"]["session_id"], "session-thread-1");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["turn_id"], "turn-compact");
    assert_eq!(
        payload[0]["event_params"]["agent9527_error_kind"],
        json!("context_window_exceeded")
    );
    assert_eq!(
        payload[0]["event_params"]["agent9527_error_http_status_code"],
        json!(null)
    );
    assert_eq!(
        payload[0]["event_params"]["app_server_client"]["product_client_id"],
...
async fn guardian_review_event_ingests_custom_fact_with_optional_target_item() {
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::GuardianReview(Box::new(
                GuardianReviewEventParams {
                    ...
                    guardian_default_review_model_id: Some("agent9527-auto-review".to_string()),
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "agent9527_guardian_review");
    assert_eq!(
...
async fn item_lifecycle_notifications_publish_command_execution_event() {
    ...
    assert_eq!(
        ...
        "agent9527_command_execution_event"
    );
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["session_id"], "session-thread-1");
...
async fn command_execution_approval_response_publishes_user_review_event() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "agent9527_review_event");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["turn_id"], "turn-1");
    assert_eq!(payload[0]["event_params"]["item_id"], "item-1");
...
async fn guardian_completed_notification_publishes_review_event_with_thread_metadata() {
    ...
    assert_eq!(payload["event_type"], "agent9527_review_event");
...
fn subagent_thread_started_review_serializes_expected_shape() {
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            ...
            forked_from_thread_id: None,
            product_client_id: "agent9527-tui".to_string(),
            client_name: "agent9527-tui".to_string(),
...
fn subagent_thread_started_thread_spawn_serializes_thread_lineage() {
    let parent_thread_id =
        agent9527_protocol::ThreadId::from_string("11111111-1111-1111-1111-111111111111")
            .expect("valid thread id");
    let forked_from_thread_id =
        agent9527_protocol::ThreadId::from_string("22222222-2222-4222-8222-222222222222")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
fn subagent_thread_started_other_serializes_explicit_parent_thread_id() {
    let parent_thread_id =
        agent9527_protocol::ThreadId::from_string("33333333-3333-4333-8333-333333333333")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
async fn subagent_events_keep_thread_originator_with_explicit_turn_connection() {
    ...
    let mut events = Vec::new();
    let parent_thread_id =
        agent9527_protocol::ThreadId::from_string("44444444-4444-4444-4444-444444444444")
            .expect("valid parent thread id");
    ...
    assert_eq!(params.runtime.agent9527_rs_version, "0.1.0");
...
fn plugin_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::PluginUsed(Agent9527PluginUsedEventRequest {
        event_type: "agent9527_plugin_used",
        event_params: agent9527_plugin_used_metadata(&tracking, sample_plugin_metadata()),
    });
...
fn plugin_management_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstalled(Agent9527PluginEventRequest {
        event_type: "agent9527_plugin_installed",
        event_params: agent9527_plugin_metadata(sample_plugin_metadata()),
    });
...
fn plugin_install_failed_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstallFailed(Agent9527PluginInstallFailedEventRequest {
        event_type: "agent9527_plugin_install_failed",
        event_params: Agent9527PluginInstallFailedMetadata {
            plugin: agent9527_plugin_metadata(sample_plugin_metadata()),
...
fn plugin_management_event_keeps_plugin_id_local_when_remote_id_exists() {
    ...
    let event = TrackEventRequest::PluginInstalled(Agent9527PluginEventRequest {
        event_type: "agent9527_plugin_installed",
        event_params: agent9527_plugin_metadata(plugin),
    });
...
fn hook_run_event_serializes_expected_shape() {
    let tracking = test_tracking_context("thread-3", "turn-3");
    let event = TrackEventRequest::HookRun(Agent9527HookRunEventRequest {
        event_type: "agent9527_hook_run",
        event_params: agent9527_hook_run_metadata(
            &tracking,
            HookRunFact {
...
fn hook_run_metadata_maps_sources_and_statuses() {
    ...
    let system = serde_json::to_value(agent9527_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let project = serde_json::to_value(agent9527_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let cloud_requirements = serde_json::to_value(agent9527_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
    ...
    let unknown = serde_json::to_value(agent9527_hook_run_metadata(
        &tracking,
        HookRunFact {
...
fn hook_run_metadata_maps_stopped_status() {
    ...
    let stopped = serde_json::to_value(agent9527_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
            hook_source: HookSource::User,
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::SkillInvoked(SkillInvokedInput {
                ...
                invocations: vec![SkillInvocation {
                    ...
                    skill_scope: agent9527_protocol::protocol::SkillScope::User,
                    skill_path,
                    plugin_id: None,
...
async fn reducer_includes_plugin_ids_for_plugin_skill_invocations() {
    ...
    let tracking = test_tracking_context("thread-1", "turn-1");
    let skill_path =
        PathBuf::from("/Users/abc/.agent9527/plugins/cache/test/sample/skills/doc/SKILL.md");
...
async fn reducer_ingests_hook_run_fact() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "agent9527_hook_run");
...
async fn reducer_ingests_app_and_plugin_facts() {
    ...
    assert_eq!(payload[0]["event_type"], "agent9527_app_mentioned");
    assert_eq!(payload[1]["event_type"], "agent9527_app_used");
    assert_eq!(payload[2]["event_type"], "agent9527_plugin_used");
    assert_eq!(
...
async fn reducer_ingests_plugin_state_changed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "agent9527_plugin_disabled",
            "event_params": {
                "plugin_id": "sample@test",
                "remote_plugin_id": null,
                "plugin_name": "sample",
...
async fn reducer_ingests_plugin_install_requested_fact() {
    ...
    assert_eq!(
        ...
        json!([{
            "event_type": "agent9527_plugin_install_requested",
            "event_params": {
...
async fn reducer_ingests_external_agent_config_import_completed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "agent9527_onboarding_external_agent_import_complete",
            "event_params": {
                "import_id": "import-1",
                "source": "app_server",
...
fn external_agent_config_import_failure_event_serializes_expected_shape() {
    let event = TrackEventRequest::ExternalAgentConfigImportFailure(
        Agent9527OnboardingExternalAgentImportFailureEventRequest {
            event_type: "agent9527_onboarding_external_agent_import_failure",
            event_params: Agent9527OnboardingExternalAgentImportFailureMetadata {
                import_id: "import-1".to_string(),
                source: "app_server".to_string(),
                provider_id: "test-provider-42".to_string(),
                item_type: "PLUGINS".to_string(),
...
fn turn_event_serializes_expected_shape() {
    let event = TrackEventRequest::TurnEvent(Box::new(Agent9527TurnEventRequest {
        event_type: "agent9527_turn_event",
        event_params: crate::events::Agent9527TurnEventParams {
            thread_id: "thread-2".to_string(),
    ...
    let expected = serde_json::from_str::<serde_json::Value>(
        r#"{
            ...
            "event_params": {
                ...
                "app_server_client": {
                    "product_client_id": "agent9527_cli_rs",
                    "client_name": "agent9527-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
...
async fn accepted_turn_steer_emits_expected_event() {
    ...
    assert_eq!(payload["event_type"], json!("agent9527_turn_steer_event"));
    ...
    assert_eq!(
        payload["event_params"]["runtime"]["agent9527_rs_version"],
        json!("0.1.0")
    );
...
async fn turn_lifecycle_emits_turn_event() {
    ...
    assert_eq!(payload["event_type"], json!("agent9527_turn_event"));
    assert_eq!(payload["event_params"]["thread_id"], json!("thread-2"));
    assert_eq!(
        payload["event_params"]["session_id"],
        json!("session-thread-2")
    ...
    assert_eq!(
        ...
        json!({
            "product_client_id": "agent9527-tui",
            "client_name": "agent9527-tui",
            "client_version": "1.0.0",
            "rpc_transport": "stdio",
...
async fn turn_event_counts_completed_tool_items() {
    ...
    assert_eq!(
        emitted_tool_events,
        vec![
            ("agent9527_command_execution_event", "session-thread-2"),
            ("agent9527_file_change_event", "session-thread-2"),
            ("agent9527_mcp_tool_call_event", "session-thread-2"),
            ("agent9527_dynamic_tool_call_event", "session-thread-2"),
            ("agent9527_collab_agent_tool_call_event", "session-thread-2"),
            ("agent9527_web_search_event", "session-thread-2"),
            ("agent9527_image_generation_event", "session-thread-2"),
        ]
    );
...
async fn turn_lifecycle_emits_failed_turn_event() {
    ...
    reducer
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::TurnAgent9527Error(Box::new(
                TurnAgent9527ErrorFact::from_agent9527_err(
                    "thread-2".to_string(),
                    "turn-2".to_string(),
                    &Agent9527Err::InvalidRequest(
    ...
    reducer
        .ingest(
            AnalyticsFact::Notification(Box::new(sample_turn_completed_notification(
                ...
                Some(agent9527_app_server_protocol::Agent9527ErrorInfo::BadRequest),
            ))),
            &mut out,
        )
        .await;
    ...
    assert_eq!(
        payload["event_params"]["agent9527_error_kind"],
        json!("invalid_request")
    );
    assert_eq!(
        payload["event_params"]["agent9527_error_http_status_code"],
        json!(null)
    );
}
...
async fn turn_lifecycle_emits_interrupted_turn_event_without_error() {
    ...
    assert_eq!(payload["event_params"]["agent9527_error_kind"], json!(null));
}

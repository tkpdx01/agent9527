...
#[cfg(debug_assertions)]
use crate::events::Agent9527PluginMetadata;
#[cfg(debug_assertions)]
use crate::events::Agent9527PluginUsedEventRequest;
#[cfg(debug_assertions)]
use crate::events::Agent9527PluginUsedMetadata;
#[cfg(debug_assertions)]
...
fn sample_mcp_tool_call_event(thread_id: &str, plugin_id: Option<&str>) -> TrackEventRequest {
    TrackEventRequest::McpToolCall(Agent9527McpToolCallEventRequest {
        ...
        event_params: Agent9527McpToolCallEventParams {
            base: Agent9527ToolItemEventBase {
                ...
                app_server_client: Agent9527AppServerClientMetadata {
                    product_client_id: "agent9527_desktop".to_string(),
                    client_name: None,
                    client_version: None,
                ...
                runtime: Agent9527RuntimeMetadata {
                    agent9527_rs_version: "0.0.0".to_string(),
...
fn sample_plugin_used_track_event(thread_id: &str, plugin_id: Option<&str>) -> TrackEventRequest {
    TrackEventRequest::PluginUsed(Agent9527PluginUsedEventRequest {
        event_type: "agent9527_plugin_used",
        event_params: Agent9527PluginUsedMetadata {
            plugin: Agent9527PluginMetadata {
                plugin_id: plugin_id.map(str::to_string),
                remote_plugin_id: None,
                ...
                product_client_id: Some("agent9527_desktop".to_string()),
            },
...
fn unique_capture_path(name: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "agent9527-analytics-{name}-{}-{nonce}.jsonl",
        std::process::id()
    ))
}
...
fn analytics_destination_uses_http_without_capture_file() {
    ...
    assert_eq!(
        ...
        AnalyticsEventsDestination::Http {
            url: "https://chatgpt.com/backend-api/agent9527/analytics-events/events".to_string()
        }
    );
}
...
async fn capture_file_writes_exact_serialized_request() {
    ...
    let auth = agent9527_login::Agent9527Auth::create_dummy_chatgpt_auth_for_testing();
...
async fn capture_file_writes_final_batches_as_separate_lines() {
    ...
    assert_eq!(
        ...
        "agent9527_accepted_line_fingerprints"
    );
...
#[tokio::test]
#[cfg(debug_assertions)]
async fn api_key_auth_sends_only_plugin_events_to_agent9527_backend() {
    ...
    let auth_manager = agent9527_login::AuthManager::from_auth_for_testing(
        agent9527_login::Agent9527Auth::from_api_key("sk-test"),
    );

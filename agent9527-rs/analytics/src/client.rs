...
impl AnalyticsEventsDestination {
    ...
    fn from_base_url_and_capture_file(base_url: String, capture_file: Option<PathBuf>) -> Self {
        ...
        Self::Http {
            url: format!("{base_url}/agent9527/analytics-events/events"),
        }
    }
}
...
impl AnalyticsEventsClient {
    ...
    pub fn track_compaction(&self, event: crate::facts::Agent9527CompactionEvent) {
        self.record_fact(AnalyticsFact::Custom(CustomAnalyticsFact::Compaction(
            Box::new(event),
        )));
    }

    pub fn track_goal_event(&self, event: Agent9527GoalEvent) {
    ...
    pub fn track_turn_agent9527_error(&self, fact: TurnAgent9527ErrorFact) {
        self.record_fact(AnalyticsFact::Custom(
            CustomAnalyticsFact::TurnAgent9527Error(Box::new(fact)),
        ));
    }
...
async fn send_track_events(
    ...
    } else if !auth.uses_agent9527_backend() {
        return;
    }
    if events.is_empty() {
        return;

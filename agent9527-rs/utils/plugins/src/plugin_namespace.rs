...
use agent9527_exec_server_protocol::DISCOVERABLE_PLUGIN_MANIFEST_PATHS;
...
/// plugin manifest (same `name` rules as full manifest loading in agent9527-core).
...
mod tests {
    ...
    async fn uses_manifest_name() {
        ...
        fs::create_dir_all(plugin_root.join(".agent9527-plugin")).expect("mkdir manifest");
        fs::write(
    ...
    fn ignores_unrelated_root_plugin_manifest_before_legacy_fallback() {
        let tmp = tempdir().expect("tempdir");
        let plugin_root = tmp.path().join("plugins/sample");
        let legacy_path = plugin_root.join(".agent9527-plugin/plugin.json");
    ...
    #[test]
    fn preserves_agent9527_claude_cursor_legacy_precedence() {
        let tmp = tempdir().expect("tempdir");
        let plugin_root = tmp.path().join("plugins/sample");
        let agent9527_path = plugin_root.join(".agent9527-plugin/plugin.json");
        let claude_path = plugin_root.join(".claude-plugin/plugin.json");
        let cursor_path = plugin_root.join(".cursor-plugin/plugin.json");
        for path in [&agent9527_path, &claude_path, &cursor_path] {
        ...
        assert_eq!(
            ...
            Some(agent9527_path.clone())
        );
        fs::remove_file(agent9527_path).expect("remove Agent9527 manifest");

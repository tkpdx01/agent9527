...
/// Filename used for the redacted `agent9527 doctor --json` feedback attachment.
pub const DOCTOR_REPORT_ATTACHMENT_FILENAME: &str = "agent9527-doctor-report.json";
/// Filename used for the raw Agent9527 Apps MCP tools cache feedback attachment.
pub const AGENT9527_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME: &str = "agent9527-apps-tools-cache.json";
/// Filename used for the raw connector directory cache feedback attachment.
pub const AGENT9527_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME: &str =
    "agent9527-app-directory-cache.json";
...
pub fn emit_feedback_request_tags_with_auth_env(
    ...
    tracing::info!(
        ...
        auth_env_agent9527_api_key_present = tracing::field::debug(auth_env.agent9527_api_key_env_present),
        auth_env_agent9527_api_key_enabled = tracing::field::debug(auth_env.agent9527_api_key_env_enabled),
...
#[derive(Clone)]
pub struct Agent9527Feedback {
    inner: Arc<FeedbackInner>,
}

impl Default for Agent9527Feedback {
    fn default() -> Self {
        Self::new()
    }
}

impl Agent9527Feedback {
    pub fn new() -> Self {
        ...
        S: tracing::Subscriber + for<'a> LookupSpan<'a>,
    {
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(Targets::new().with_default(Level::TRACE).with_target(
                "agent9527_api::responses_websocket_timing",
                LevelFilter::OFF,
            ))
    }
...
pub struct FeedbackUploadOptions<'a> {
    ...
    ///
    /// These are included after `agent9527-logs.log` and before path-backed rollout
...
impl FeedbackSnapshot {
    ...
    pub fn save_to_temp_file(&self) -> io::Result<PathBuf> {
        ...
        let filename = format!("agent9527-feedback-{}.log", self.thread_id);
    ...
    fn feedback_attachments(
        ...
        if include_logs {
            attachments.push(Attachment {
                ...
                filename: String::from("agent9527-logs.log"),
...
mod tests {
    ...
    fn ring_buffer_drops_front_when_full() {
        let fb = Agent9527Feedback::with_capacity(/*max_bytes*/ 8);
        {
    ...
    fn logger_layer_excludes_responses_websocket_timing_payloads() {
        let fb = Agent9527Feedback::new();
        let _guard = tracing_subscriber::registry()
            .with(fb.logger_layer())
            .set_default();

        tracing::trace!(target: "agent9527_api::responses_websocket_timing", payload = "secret");
        tracing::trace!(target: "agent9527_feedback_test", "retained");
    ...
    fn feedback_attachments_gate_connectivity_diagnostics() {
        let extra_filename = format!("agent9527-feedback-extra-{}.jsonl", ThreadId::new());
        ...
        let snapshot_with_diagnostics = Agent9527Feedback::new()
        ...
        let attachments_without_diagnostics = Agent9527Feedback::new()
            .snapshot(/*session_id*/ None)
    ...
    fn path_backed_attachments_use_binary_content_types() {
        ...
        let gzip_filename = format!("agent9527-desktop-app-logs-{suffix}.tar.gz");
        let unknown_filename = format!("agent9527-feedback-extra-{suffix}.binunknown");
        ...
        let attachments = Agent9527Feedback::new()
            .snapshot(/*session_id*/ None)

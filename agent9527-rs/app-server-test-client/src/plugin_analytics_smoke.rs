...
pub(super) const ANALYTICS_CAPTURE_ENV_VAR: &str = "AGENT9527_ANALYTICS_EVENTS_CAPTURE_FILE";
const TEST_USER_CONFIG_ENV_VAR: &str = "AGENT9527_APP_SERVER_TEST_USER_CONFIG_FILE";
...
pub(super) fn run(
    ...
    let capture_path = capture_file.unwrap_or_else(|| {
        std::env::temp_dir().join(format!(
            "agent9527-plugin-analytics-{}.jsonl",
            process::id()
        ))
    });
    ...
    let mut client =
        Agent9527Client::spawn_stdio_with_env(agent9527_bin, &overrides, &child_environment)?;
...
fn run_plugin_turn(client: &mut Agent9527Client, expected: &ExpectedPlugin) -> Result<String> {
...
fn wait_for_plugin_usage(
    ...
    loop {
        ...
        if events.iter().any(|event| {
            event["event_type"] == "agent9527_plugin_used"
...
fn plugin_installed(client: &mut Agent9527Client) -> Result<PluginInstalledResponse> {
...
pub(super) fn wait_until_capture_is_ready(path: &Path) -> Result<()> {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            bail!(
                "analytics capture did not become ready at {}; use a debug Agent9527 binary",
                path.display()
            );
        }
...
fn wait_for_turn_analytics(path: &Path, turn_id: &str) -> Result<Vec<Value>> {
    ...
    loop {
        ...
        if events.iter().any(|event| {
            event["event_type"] == "agent9527_turn_event"
...
fn validate_plugin_events(events: Vec<Value>, expected: &ExpectedPlugin) -> Result<Vec<Value>> {
    ...
    for event_type in required_event_types() {
        ...
        if event_type == "agent9527_plugin_used" {
...
fn required_event_types() -> [&'static str; 3] {
    [
        "agent9527_plugin_disabled",
        "agent9527_plugin_enabled",
        "agent9527_plugin_used",
    ]
}
...
fn validate_used_metadata(event: &Value) -> Result<()> {
    ...
    for field in [
        ...
        if params.get(field).is_none_or(Value::is_null) {
            bail!("agent9527_plugin_used event has null or missing `{field}`");
        }
    }
...
impl TemporaryConfigFile {
    fn create() -> Result<Self> {
        let path = std::env::temp_dir().join(format!(
            "agent9527-plugin-analytics-config-{}.toml",
            process::id()
        ));
        fs::write(&path, "")

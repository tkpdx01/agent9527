...
pub(super) fn run(
    ...
    let capture_path = capture_file.unwrap_or_else(|| {
        std::env::temp_dir().join(format!(
            "agent9527-plugin-analytics-mutation-{}.jsonl",
            process::id()
        ))
    });
    prepare_capture_file(&capture_path)?;
    let mut client = spawn_client(agent9527_bin, config_overrides, &capture_path)?;
    ...
    match (sequence_result, restoration) {
        ...
        (sequence_result, RestorationStatus::Unknown(cleanup_err)) => {
            ...
            print_recovery_command(agent9527_bin, config_overrides, remote_plugin_id);
            Err(cleanup_err)
        }
    }
}
...
pub(super) fn run_cleanup(
    ...
    let mut client = Agent9527Client::spawn_stdio(agent9527_bin, &overrides)?;
    client.initialize()?;
    ...
    match restore_uninstalled_state(&mut client, remote_plugin_id) {
        ...
        RestorationStatus::Dirty(err) => {
            print_dirty_recovery(agent9527_bin, config_overrides, remote_plugin_id, &err);
            Err(err)
        }
...
fn spawn_client(
    ...
    Agent9527Client::spawn_stdio_with_env(agent9527_bin, &overrides, &environment)
}
...
fn uninstall_remote_plugin(client: &mut Agent9527Client, remote_plugin_id: &str) -> Result<()> {
    let request_id = client.request_id();
...
fn print_recovery_command(
    ...
    let test_client = std::env::current_exe()
        ...
        .unwrap_or_else(|_| "agent9527-app-server-test-client".to_string());

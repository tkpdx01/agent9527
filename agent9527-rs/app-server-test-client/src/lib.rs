...
use agent9527_app_server_protocol::JSONRPCRequest;
...
use agent9527_app_server_protocol::SandboxPolicy;
...
use agent9527_protocol::dynamic_tools::normalize_dynamic_tool_specs;
...
const OTEL_SERVICE_NAME: &str = "agent9527-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $AGENT9527_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the Agent9527 app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Agent9527", version, about = "Bootstrap Agent9527 app-server", long_about = None)]
struct Cli {
    /// Path to the `agent9527` CLI binary. When set, requests use stdio by
    /// spawning `agent9527 app-server` as a child process.
    #[arg(long, env = "AGENT9527_BIN", global = true)]
    agent9527_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--agent9527-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "AGENT9527_APP_SERVER_URL", global = true)]
    url: Option<String>,

    /// Forwarded to the `agent9527` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `agent9527 app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/agent9527-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `agent9527 app-server --listen`.
    ...
    /// Send a user message through the Agent9527 app-server.
    SendMessage {
        /// User message to send to Agent9527.
        user_message: String,
    },
    ...
    TestLogin {
        ...
        device_code: bool,
        /// Use a Agent9527-managed Amazon Bedrock API key.
    ...
    TestLogout,
    /// Fetch the current account rate limits from the Agent9527 app-server.
    GetAccountRateLimits,
    /// List the available models from the Agent9527 app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the Agent9527 app-server.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "AGENT9527_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let agent9527_bin = agent9527_bin.unwrap_or_else(|| PathBuf::from("agent9527"));
            serve(&agent9527_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(agent9527_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url =
                resolve_shared_websocket_url(agent9527_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url =
                resolve_shared_websocket_url(agent9527_bin, url, "thread-decrement-elicitation")?;
        ...
        CliCommand::PluginAnalyticsSmoke {
            ...
            if url.is_some() {
                bail!("plugin-analytics-smoke requires --agent9527-bin and does not support --url");
            }
            let agent9527_bin =
                agent9527_bin.context("plugin-analytics-smoke requires --agent9527-bin")?;
            plugin_analytics_smoke::run(&agent9527_bin, &config_overrides, &plugin_id, capture_file)
        }
        CliCommand::PluginAnalyticsMutationSmoke {
            ...
            if url.is_some() {
                bail!(
                    "plugin-analytics-mutation-smoke requires --agent9527-bin and does not support --url"
                );
            }
            let agent9527_bin = agent9527_bin
                .context("plugin-analytics-mutation-smoke requires --agent9527-bin")?;
        ...
        CliCommand::PluginRemoteUninstall {
            ...
            if url.is_some() {
                bail!(
                    "plugin-remote-uninstall requires --agent9527-bin and does not support --url"
                );
            }
            let agent9527_bin =
                agent9527_bin.context("plugin-remote-uninstall requires --agent9527-bin")?;
...
fn resolve_endpoint(agent9527_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if agent9527_bin.is_some() && url.is_some() {
        bail!("--agent9527-bin and --url are mutually exclusive");
    }
    if let Some(agent9527_bin) = agent9527_bin {
        return Ok(Endpoint::SpawnAgent9527(agent9527_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if agent9527_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --agent9527-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(agent9527_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(agent9527_bin);
        if let Some(agent9527_bin_parent) = agent9527_bin.parent() {
            let mut path = OsString::from(agent9527_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", agent9527_bin.display()))?;
...
fn serve(
    ...
    let runtime_dir = PathBuf::from("/tmp/agent9527-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,agent9527_=trace {}",
        shell_quote(&agent9527_bin.display().to_string())
    );
    for override_kv in config_overrides {
    ...
    println!("started agent9527 app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnAgent9527(agent9527_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
            ...
            TestLoginMode::AmazonBedrock { api_key, region } => {
                ...
                let login_response: LoginAccountResponse = client.send_request(
                    ClientRequest::LoginAccount {
                        ...
                        params: agent9527_app_server_protocol::LoginAccountParams::AmazonBedrock {
                            api_key,
                            region,
                        },
                    },
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut Agent9527Client) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = Agent9527Client::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = Agent9527Client::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (agent9527_bin, url) {
        (Some(_), Some(_)) => bail!("--agent9527-bin and --url are mutually exclusive"),
        (Some(agent9527_bin), None) => {
            let server = BackgroundAppServer::spawn(&agent9527_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve agent9527-app-server-test-client binary path")?;
...
impl Agent9527Client {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnAgent9527(agent9527_bin) => {
                Self::spawn_stdio(agent9527_bin, config_overrides)
            }
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(agent9527_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        Self::spawn_stdio_with_env(agent9527_bin, config_overrides, &[])
    }
    ...
    fn spawn_stdio_with_env(
        ...
        let agent9527_bin_display = agent9527_bin.display();
        let mut cmd = Command::new(agent9527_bin);
        if let Some(agent9527_bin_parent) = agent9527_bin.parent() {
            let mut path = OsString::from(agent9527_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut agent9527_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{agent9527_bin_display}` app-server"))?;

        let stdin = agent9527_app_server
            .stdin
            .take()
            .context("agent9527 app-server stdin unavailable")?;
        let stdout = agent9527_app_server
            .stdout
            .take()
            .context("agent9527 app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: agent9527_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `agent9527-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "agent9527-toy-app-server".to_string(),
                    title: Some("Agent9527 Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: agent9527_app_server_protocol::LoginAccountParams::Chatgpt {
                app_brand: None,
                agent9527_streamlined_login: false,
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: agent9527_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to agent9527 app-server")?;
                    return Ok(());
                }
                bail!("agent9527 app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from agent9527 app-server")?;
                if bytes == 0 {
                    bail!("agent9527 app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = agent9527_core::otel_init::build_provider(
            &config,
...
impl Drop for Agent9527Client {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[agent9527 app-server exited: {status}]");
            return;
        }

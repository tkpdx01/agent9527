//! Custom CA handling shared by Agent9527 outbound HTTP and websocket clients.
//!
//! Agent9527 constructs outbound reqwest clients and secure websocket connections in a few crates, but
...
//!
//! - read CA material from `AGENT9527_CA_CERTIFICATE`, falling back to `SSL_CERT_FILE`
...
//!   because Agent9527 itself commonly runs spawned test processes under seatbelt, so this is not just
...
use agent9527_utils_rustls_provider::ensure_rustls_crypto_provider;
...
pub const AGENT9527_CA_CERT_ENV: &str = "AGENT9527_CA_CERTIFICATE";
pub const SSL_CERT_FILE_ENV: &str = "SSL_CERT_FILE";
const CA_CERT_HINT: &str = "If you set AGENT9527_CA_CERTIFICATE or SSL_CERT_FILE, ensure it points to a PEM file containing one or more CERTIFICATE blocks, or unset it to use system roots.";
...
/// Builds a reqwest client that honors Agent9527 custom CA environment variables.
///
/// Callers supply the baseline builder configuration they need, and this helper layers in custom
/// CA handling before finally constructing the client. `AGENT9527_CA_CERTIFICATE` takes precedence
...
/// easy mistake to make when adding a new outbound Agent9527 HTTP path, and the resulting bug only
...
/// Builds a rustls client config when a Agent9527 custom CA bundle is configured.
///
/// This is the websocket-facing sibling of [`build_reqwest_client_with_custom_ca`]. When
/// `AGENT9527_CA_CERTIFICATE` or `SSL_CERT_FILE` selects a CA bundle, the returned config starts from
...
/// Builds a rustls client config using native roots and any configured Agent9527 custom CA bundle.
///
...
fn build_rustls_client_config(
    ...
    // they would get from tungstenite's default rustls connector, then layer in the Agent9527 custom
...
fn build_reqwest_client_with_env(
    ...
    info!(
        agent9527_ca_certificate_configured = false,
...
trait EnvSource {
    ...
    ///
    /// `AGENT9527_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the Agent9527-specific override.
    ...
    fn configured_ca_bundle(&self) -> Option<ConfiguredCaBundle> {
        self.non_empty_path(AGENT9527_CA_CERT_ENV)
            .map(|path| ConfiguredCaBundle {
                source_env: AGENT9527_CA_CERT_ENV,
                path,
            })
            .or_else(|| {
...
impl ConfiguredCaBundle {
    ...
    /// Loads every certificate block from a PEM file intended for Agent9527 CA overrides.
    ///
    /// This accepts a few common real-world variants so Agent9527 behaves like other CA-aware tooling:
...
impl NormalizedPem {
    ...
    ///
    /// Agent9527 only needs certificate DER bytes to seed `reqwest`'s root store, but operators may
...
mod tests {
    ...
    use super::AGENT9527_CA_CERT_ENV;
    ...
    #[test]
    fn ca_path_prefers_agent9527_env() {
        let env = map_env(&[
            (AGENT9527_CA_CERT_ENV, "/tmp/agent9527.pem"),
            (SSL_CERT_FILE_ENV, "/tmp/fallback.pem"),
        ]);

        assert_eq!(
            env.configured_ca_bundle().map(|bundle| bundle.path),
            Some(PathBuf::from("/tmp/agent9527.pem"))
        );
    }
    ...
    fn rustls_config_uses_custom_ca_bundle_when_configured() {
        ...
        let env = map_env(&[(AGENT9527_CA_CERT_ENV, cert_path.to_string_lossy().as_ref())]);

# agent9527-http-client

`agent9527-http-client` is the low-level HTTP transport shared by Agent9527 crates. It is the intended
...
- custom CA handling through `AGENT9527_CA_CERTIFICATE` and `SSL_CERT_FILE`;
...
feature requires more than enabling reqwest's default proxy behavior: Agent9527 must resolve platform
...
Higher-level retry, SSE, and request-attempt telemetry policy remains in `agent9527-client`.
...
would skip Agent9527's route selection for the redirect target. In `RespectSystemProxy` mode the pool
...
```rust
use agent9527_http_client::ClientRouteClass;
use agent9527_http_client::ReqwestTransport;

let client = http_client_factory.build_client(api_url, ClientRouteClass::Api)?;
let transport = ReqwestTransport::from_http_client(client);
```

If the existing wrapper surface cannot support a use case, extend `agent9527-http-client` rather than

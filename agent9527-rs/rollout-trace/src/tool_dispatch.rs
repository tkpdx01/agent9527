...
use agent9527_protocol::models::SearchToolCallParams;
use serde::Serialize;
...
/// Core-facing request data for the canonical Agent9527 tool boundary.
pub struct ToolDispatchInvocation {
    pub thread_id: AgentThreadId,
    pub agent9527_turn_id: Agent9527TurnId,
...
/// Raw invocation payload for the canonical Agent9527 tool boundary.
#[derive(Serialize)]
...
impl ToolDispatchTraceContext {
    ...
    pub(crate) fn start(writer: Arc<TraceWriter>, invocation: ToolDispatchInvocation) -> Self {
        ...
        let context = EnabledToolDispatchTraceContext {
            ...
            agent9527_turn_id: invocation.agent9527_turn_id.clone(),
...
fn suppresses_tool_dispatch_trace(invocation: &ToolDispatchInvocation) -> bool {
    matches!(invocation.payload, ToolDispatchPayload::Custom { .. })
        ...
        && invocation.tool_name == agent9527_code_mode::PUBLIC_TOOL_NAME
}

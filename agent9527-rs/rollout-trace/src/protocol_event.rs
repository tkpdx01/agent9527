//! Mapping from Agent9527 protocol events into raw rollout-trace events.
//!
...
//! `agent9527-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `agent9527-rollout-trace`.
//!
...
use agent9527_protocol::protocol::PatchApplyStatus;
use agent9527_protocol::protocol::SubAgentActivityEvent;
use agent9527_protocol::protocol::TurnAbortReason;
use serde::Serialize;
use std::time::Duration;

use crate::Agent9527TurnId;
use crate::AgentThreadId;
use crate::ExecutionStatus;
use crate::RawTraceEventPayload;

pub(crate) struct Agent9527TurnTraceEvent {
    pub context_turn_id: Agent9527TurnId,
    pub payload: RawTraceEventPayload,
}

pub(crate) fn agent9527_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<Agent9527TurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let agent9527_turn_id = event.turn_id.clone();
            Some(Agent9527TurnTraceEvent {
                context_turn_id: agent9527_turn_id.clone(),
                payload: RawTraceEventPayload::Agent9527TurnStarted {
                    agent9527_turn_id,
                    thread_id,
                },
            })
        ...
        EventMsg::TurnComplete(event) => {
            ...
            Some(Agent9527TurnTraceEvent {
                context_turn_id: agent9527_turn_id.clone(),
                payload: RawTraceEventPayload::Agent9527TurnEnded {
                    agent9527_turn_id,
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a agent9527_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a agent9527_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a agent9527_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a agent9527_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a agent9527_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a agent9527_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a agent9527_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a agent9527_protocol::protocol::CollabCloseEndEvent),
...
struct ExecCommandBeginTracePayload<'a> {
    ...
    command: &'a [String],
    cwd: String,
    parsed_cmd: &'a [agent9527_protocol::parse_command::ParsedCommand],

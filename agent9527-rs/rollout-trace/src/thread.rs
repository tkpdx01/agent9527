...
use crate::protocol_event::agent9527_turn_trace_event;
...
pub const AGENT9527_ROLLOUT_TRACE_ROOT_ENV: &str = "AGENT9527_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `AGENT9527_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the Agent9527
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(AGENT9527_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed Agent9527 turn lifecycle events from protocol lifecycle events.
    pub fn record_agent9527_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            agent9527_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what Agent9527 did while executing that boundary.
    pub fn record_tool_call_event(
        &self,
        agent9527_turn_id: impl Into<Agent9527TurnId>,
        event: &EventMsg,
    ) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        ...
        context.append_with_context_best_effort(agent9527_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_agent9527_turn_id: impl Into<Agent9527TurnId>,
    ...
    pub fn record_agent9527_turn_started(&self, agent9527_turn_id: impl Into<Agent9527TurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let agent9527_turn_id = agent9527_turn_id.into();
        context.append_with_context_best_effort(
            agent9527_turn_id.clone(),
            RawTraceEventPayload::Agent9527TurnStarted {
                agent9527_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
    ...
    pub fn start_code_cell_trace(
        ...
        let context = self.code_cell_trace_context(agent9527_turn_id, runtime_cell_id);
    ...
    /// Builds reusable inference trace context for one Agent9527 turn.
    ///
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        &self,
        agent9527_turn_id: Agent9527TurnId,
        payload: RawTraceEventPayload,
    ) {
        let event_context = RawTraceEventContext {
            thread_id: Some(self.thread_id.clone()),
            agent9527_turn_id: Some(agent9527_turn_id),
        };

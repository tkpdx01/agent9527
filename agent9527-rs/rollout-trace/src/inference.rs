...
use crate::model::Agent9527TurnId;
...
const INFERENCE_CALL_ID_HEADER: &str = "x-agent9527-inference-call-id";
...
///
/// A Agent9527 turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one Agent9527 turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        agent9527_turn_id: Agent9527TurnId,
...
impl InferenceTraceAttempt {
    ...
    pub fn record_started(&self, request: &impl Serialize) {
        ...
        append_with_context_best_effort(
            ...
            RawTraceEventPayload::InferenceStarted {
                ...
                agent9527_turn_id: attempt.context.agent9527_turn_id.clone(),
    ...
    /// trace-specific serialization rules. That keeps agent9527-core focused on
    ...
    /// Records a provider stream that Agent9527 intentionally stopped consuming.
    ///
...
/// Agent9527 received it, so this helper restores that content in the raw payload.
...
fn append_with_context_best_effort(
    ...
    let event_context = RawTraceEventContext {
        ...
        agent9527_turn_id: Some(context.agent9527_turn_id.clone()),
    };
...
mod tests {
    ...
    fn enabled_context_records_replayable_inference_attempt() -> anyhow::Result<()> {
        ...
        assert_eq!(inference.agent9527_turn_id, "turn-1");

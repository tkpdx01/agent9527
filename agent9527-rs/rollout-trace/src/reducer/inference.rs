...
impl TraceReducer {
    ...
    pub(super) fn start_inference_call(
        ...
        let agent9527_turn_id = started.agent9527_turn_id.clone();
        let request_payload = started.request_payload.clone();
        let Some(turn) = self.rollout.agent9527_turns.get(&agent9527_turn_id) else {
            bail!(
                "inference start {inference_call_id} referenced unknown agent9527 turn {agent9527_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "inference start {inference_call_id} used thread {thread_id}, \
                 but agent9527 turn {agent9527_turn_id} belongs to {}",
                turn.thread_id
            );
        }
    ...
    /// If a call is still `Running`, Agent9527 stopped observing that provider stream
    ...
    pub(super) fn close_running_inference_calls_for_turn_end(
        ...
        for inference in self.rollout.inference_calls.values_mut() {
            if inference.agent9527_turn_id == agent9527_turn_id

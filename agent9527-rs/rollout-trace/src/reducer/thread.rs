...
impl TraceReducer {
    ...
    /// Starts a Agent9527 turn inside an existing thread.
    pub(super) fn start_agent9527_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if self
            .rollout
            .agent9527_turns
            .contains_key(&agent9527_turn_id)
        {
            bail!("duplicate agent9527 turn start for {agent9527_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.agent9527_turns.insert(
    ...
    /// Marks a Agent9527 turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_agent9527_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.agent9527_turns.get(&agent9527_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "agent9527 turn end for {agent9527_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.agent9527_turns.get_mut(&agent9527_turn_id) else {
            bail!("agent9527 turn end referenced unknown turn {agent9527_turn_id}");
        };

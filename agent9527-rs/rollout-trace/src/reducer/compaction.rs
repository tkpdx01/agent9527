...
impl TraceReducer {
    ...
    pub(super) fn start_compaction_request(
        ...
        let Some(turn) = self.rollout.agent9527_turns.get(&started.agent9527_turn_id) else {
            bail!(
                "compaction request {} referenced unknown agent9527 turn {}",
                started.compaction_request_id,
                started.agent9527_turn_id
            );
        };
        if turn.thread_id != started.thread_id {
            bail!(
                "compaction request {} used thread {}, but agent9527 turn {} belongs to {}",
                started.compaction_request_id,
                started.thread_id,
                started.agent9527_turn_id,
                turn.thread_id
        ...
        self.rollout.compaction_requests.insert(
            ...
            CompactionRequest {
                ...
                agent9527_turn_id: started.agent9527_turn_id,
    ...
    pub(super) fn reduce_compaction_installed_event(
        ...
        let Some(turn) = self.rollout.agent9527_turns.get(&agent9527_turn_id) else {
            bail!(
                "compaction install {compaction_id} referenced unknown agent9527 turn {agent9527_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "compaction install {compaction_id} used thread {thread_id}, but agent9527 turn {agent9527_turn_id} belongs to {}",
                turn.thread_id
            );
        }
...
pub(super) struct StartedCompactionRequest {
    ...
    pub(super) agent9527_turn_id: String,

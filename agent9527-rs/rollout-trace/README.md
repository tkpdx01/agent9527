...
> **Privacy:** Rollout tracing is not telemetry. Agent9527 does **not** upload or
> report these traces; it writes local bundles only when
> `AGENT9527_ROLLOUT_TRACE_ROOT` is set. Those local bundles can contain prompts,
...
during a Agent9527 session. It records raw runtime evidence into a local bundle on
...
Hot-path Agent9527 code does not try to build the final graph while the session is
...
model-visible conversation plus the runtime objects that explain how Agent9527 got
there.
...
```mermaid
flowchart TD
    subgraph Runtime["agent9527-core runtime"]
...
one from `AGENT9527_ROLLOUT_TRACE_ROOT`; fresh spawned child threads derive their
...
Agent9527 session fail just because diagnostic recording failed. Core emits raw
...
- `state.json`: optional reducer output written by `agent9527 debug trace-reduce`.

`trace_id` identifies this diagnostic artifact. `rollout_id` identifies the
Agent9527 rollout/session being observed. Keeping those separate lets us reason about
the stored trace without confusing it with the product-level session identity.

To reduce a bundle:

```bash
agent9527 debug trace-reduce <trace-bundle>
```

By default this writes `<trace-bundle>/state.json`. Rust callers can also call
`agent9527_rollout_trace::replay_bundle` directly.

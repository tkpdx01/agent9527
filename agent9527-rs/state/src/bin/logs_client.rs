...
use agent9527_state::LogQuery;
use agent9527_state::LogRow;
...
#[derive(Debug, Parser)]
#[command(name = "agent9527-state-logs")]
#[command(about = "Tail Agent9527 logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to AGENT9527_HOME. Defaults to $AGENT9527_HOME or ~/.agent9527.
    #[arg(long, env = "AGENT9527_HOME")]
    agent9527_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --agent9527-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let agent9527_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(
        SqliteConfig::from_sqlite_home(AbsolutePathBuf::relative_to_current_dir(agent9527_home)?),
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let agent9527_home = args
        .agent9527_home
        .clone()
        .unwrap_or_else(default_agent9527_home);
    let sqlite_home = AbsolutePathBuf::relative_to_current_dir(agent9527_home)?;
    Ok(SqliteConfig::from_sqlite_home(sqlite_home).logs_db_path())
}

fn default_agent9527_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".agent9527");
    }
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["agent9527-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["agent9527-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["agent9527-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["agent9527-state-logs", "--level", "WARN"])

...
async fn sqlite_sink_drops_low_level_opentelemetry_sdk_logs() {
    let agent9527_home =
        std::env::temp_dir().join(format!("agent9527-state-log-db-filter-{}", Uuid::new_v4()));
    ...
    tracing::trace!(target: "agent9527_state", "retained-trace");
    tracing::trace!(
        target: "agent9527_api::responses_websocket_timing",
    ...
    assert_eq!(
        ...
        vec![
            ...
            ("TRACE", "agent9527_state", Some("retained-trace")),
        ]
    );

...
//!
//! # async fn example(state_db: std::sync::Arc<agent9527_state::StateRuntime>) {
...
pub fn default_filter() -> Targets {
    Targets::new()
        ...
        .with_target("agent9527_otel.log_only", LevelFilter::OFF)
        .with_target("agent9527_otel.trace_safe", LevelFilter::OFF)
    ...
    S: tracing::Subscriber + for<'a> LookupSpan<'a>,
{
    ...
    fn on_event(&self, event: &Event<'_>, ctx: tracing_subscriber::layer::Context<'_, S>) {
        ...
        // were over 30% of retained logs in measured high-fanout Agent9527 environments.
...
mod tests {
    ...
    fn temp_agent9527_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("agent9527-state-log-db-{}", Uuid::new_v4()))
    }
    ...
    async fn sqlite_feedback_logs_match_feedback_formatter_shape() {
        let agent9527_home = temp_agent9527_home();

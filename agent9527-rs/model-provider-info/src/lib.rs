//! Registry of model providers supported by Agent9527.
//!
//! Providers can be defined in two places:
//!   1. Built-in defaults compiled into the binary so Agent9527 works out-of-the-box.
//!   2. User-defined entries inside `~/.agent9527/config.toml` under the `model_providers`
//!      key. These override or extend the defaults at runtime.

use agent9527_api::Provider as ApiProvider;
use agent9527_api::RetryConfig as ApiRetryConfig;
...
use agent9527_protocol::error::EnvVarError;
...
pub const CHATGPT_AGENT9527_BASE_URL: &str = "https://chatgpt.com/backend-api/agent9527";
...
const AMAZON_BEDROCK_MANTLE_CLIENT_AGENT_VALUE: &str = "agent9527";
const CHAT_WIRE_API_REMOVED_ERROR: &str = "`wire_api = \"chat\"` is no longer supported.\nHow to fix: set `wire_api = \"responses\"` in your provider config.\nMore info: https://github.com/openai/agent9527/discussions/7782";
...
impl ModelProviderInfo {
    ...
    fn build_header_map(&self) -> Agent9527Result<HeaderMap> {
    ...
    pub fn to_api_provider(&self, auth_mode: Option<AuthMode>) -> Agent9527Result<ApiProvider> {
    ...
    pub fn api_key(&self) -> Agent9527Result<Option<String>> {
        match &self.env_key {
            Some(env_key) => {
                let api_key = std::env::var(env_key)
                    ...
                    .ok_or_else(|| {
                        Agent9527Err::EnvVar(EnvVarError {
                            var: env_key.clone(),
...
pub fn built_in_model_providers(
    ...
    // providers are bundled with Agent9527 CLI, so we only include the OpenAI and
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    // These AGENT9527_OSS_ environment variables are experimental: we may
    // switch to reading values from config.toml instead.
    let default_agent9527_oss_base_url = format!(
        "http://localhost:{agent9527_oss_port}/v1",
        agent9527_oss_port = std::env::var("AGENT9527_OSS_PORT")
            .ok()
    ...
    let agent9527_oss_base_url = std::env::var("AGENT9527_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or(default_agent9527_oss_base_url);
    create_oss_provider_with_base_url(&agent9527_oss_base_url, wire_api)
}

...
use agent9527_code_mode_protocol::CodeModeSessionProvider;
use agent9527_code_mode_protocol::ExecuteRequest;
use agent9527_code_mode_protocol::FunctionCallOutputContentItem;
use agent9527_code_mode_protocol::RuntimeResponse;
...
use agent9527_code_mode_protocol::host::WireCellId;
...
fn host_program_override_takes_precedence() {
    assert_eq!(
        resolve_host_program(
            ...
            Ok(PathBuf::from("/opt/agent9527/bin/agent9527")),
        ),
...
fn host_program_is_next_to_the_main_executable_even_when_missing() {
    ...
    assert_eq!(
        ...
        PathBuf::from("/opt/agent9527/bin").join(executable_name)
    );
}
...
async fn provider_falls_back_to_in_process_session_when_host_is_missing() {
    ...
    assert_eq!(
        ...
        RuntimeResponse::Result {
            cell_id: agent9527_code_mode_protocol::CellId::new("1".to_string()),
...
async fn provider_returns_missing_host_error_when_in_process_fallback_is_disabled() {
    ...
    assert!(
        error.contains("failed to spawn code-mode host agent9527-code-mode-host-does-not-exist")
    );
...
async fn shutdown_before_open_does_not_spawn_the_host() {
    ...
    let error = session
        .execute(agent9527_code_mode_protocol::ExecuteRequest {
            tool_call_id: "call-1".to_string(),
            enabled_tools: Vec::new(),

...
use agent9527_file_system::find_nearest_native_ancestor_with_markers;
...
/// directory. If you need Agent9527 to work from such a checkout simply pass the
...
mod tests {
    ...
    async fn git_metadata_commands_do_not_inherit_stdin() {
        const CHILD_ENV: &str = "AGENT9527_GIT_UTILS_STDIN_CHILD";
    ...
    fn canonicalize_git_remote_url_normalizes_github_variants() {
        for remote in [
            "git@github.com:OpenAI/Agent9527.git",
            "ssh://git@github.com/openai/agent9527.git",
            "ssh://git@github.com:22/OpenAI/Agent9527.git",
            "https://github.com/openai/agent9527.git",
            "https://github.com:443/openai/agent9527.git",
            "https://token@github.com/openai/agent9527/",
            "github.com/OpenAI/Agent9527.git",
        ] {
            assert_eq!(
                canonicalize_git_remote_url(remote),
                Some("github.com/openai/agent9527".to_string())
            );
        }
    }
    ...
    async fn local_git_branches_excludes_detached_head_entry() {
        ...
        run_git(&[
            "-c",
            "user.name=Agent9527 Tests",
            "-c",
            "user.email=agent9527-tests@example.com",
            "commit",
            "--allow-empty",
            "-q",
            "-m",

...
fn managed_ca_dir() -> Result<PathBuf> {
    let agent9527_home =
        find_agent9527_home().context("failed to resolve AGENT9527_HOME for managed MITM CA")?;
    Ok(agent9527_home.join(MANAGED_MITM_CA_DIR).to_path_buf())
}
...
/// Returns whether `path` points at a current Agent9527-generated MITM CA bundle.

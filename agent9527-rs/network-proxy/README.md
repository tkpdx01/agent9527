# agent9527-network-proxy

`agent9527-network-proxy` is Agent9527's local network policy enforcement proxy. It runs:
...
`agent9527-network-proxy` reads from Agent9527's merged `config.toml` (via `agent9527-core` config loading).
...
# bundle env vars pointing at immutable public files under $AGENT9527_HOME/proxy/ so common HTTPS
...
```bash
cargo run -p agent9527-network-proxy --
```
...
`agent9527-network-proxy` can be embedded as a library with a thin API:

```rust
use agent9527_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `agent9527-network-proxy` is embedded in managed Agent9527 runtime, policy decisions emit structured
OTEL-compatible events with `target=agent9527_otel.network_proxy`.

Event name:

- `agent9527.network_proxy.policy_decision`
...
This section documents the protections implemented by `agent9527-network-proxy`, and the boundaries of

...
fn standalone_app_server_emits_json_info_events() -> Result<()> {
    ...
    let event = app_server_json_shutdown_event("agent9527-app-server", &[], agent9527_home.path())?;
...
async fn app_server_emits_structured_tool_call_timing_event() -> Result<()> {
    ...
    let mut app_server = TestAppServer::builder()
        ...
        .with_json_logging("warn,agent9527_core::tools::parallel=info")
        .build()
        .await?;
    ...
    let mut tool_call = app_server
        .wait_for_json_log_event("agent9527.tool_call")
        .await?;
    ...
    assert_eq!(
        ...
        json!({
            ...
            "target": "agent9527_core::tools::parallel",
        })
    );

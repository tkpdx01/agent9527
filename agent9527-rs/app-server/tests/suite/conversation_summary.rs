...
async fn get_conversation_summary_by_thread_id_reads_pathless_store_thread() -> Result<()> {
    ...
    create_config_toml_with_in_memory_thread_store(agent9527_home.path(), &store_id)?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn get_conversation_summary_by_relative_rollout_path_resolves_from_agent9527_home()
-> Result<()> {
    let agent9527_home = TempDir::new()?;
    let conversation_id = create_fake_rollout(
        agent9527_home.path(),
    ...
    let relative_path = rollout_path
        .strip_prefix(agent9527_home.path())?
        .to_path_buf();

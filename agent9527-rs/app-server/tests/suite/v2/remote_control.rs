...
use agent9527_app_server::AppServerRuntimeOptions;
use agent9527_app_server::AppServerTransport;
use agent9527_app_server::AppServerWebsocketAuthSettings;
use agent9527_app_server::PluginStartupTasks;
use agent9527_app_server::RemoteControlStartupMode;
use agent9527_app_server::run_main_with_transport_options;
...
use agent9527_app_server_protocol::RemoteControlClient;
use agent9527_app_server_protocol::RemoteControlClientsListOrder;
use agent9527_app_server_protocol::RemoteControlClientsListParams;
use agent9527_app_server_protocol::RemoteControlClientsListResponse;
use agent9527_app_server_protocol::RemoteControlClientsRevokeParams;
use agent9527_app_server_protocol::RemoteControlClientsRevokeResponse;
use agent9527_app_server_protocol::RemoteControlConnectionStatus;
use agent9527_app_server_protocol::RemoteControlDisableResponse;
use agent9527_app_server_protocol::RemoteControlEnableResponse;
...
use agent9527_app_server_protocol::RemoteControlPairingStatusResponse;
use agent9527_app_server_protocol::RemoteControlStatusChangedNotification;
use agent9527_app_server_protocol::RemoteControlStatusReadResponse;
...
use agent9527_state::RemoteControlEnrollmentRecord;
...
async fn explicit_remote_control_startup_fails_when_disabled_by_requirements() -> Result<()> {
    ...
    let managed_config_path = agent9527_home.path().join("managed_config.toml");
    let socket_path = agent9527_home.path().join("app-server.sock");
    let transport =
    ...
    let result = timeout(
        ...
        run_main_with_transport_options(
            Arg0DispatchPaths {
                agent9527_self_exe: Some(std::env::current_exe()?),
...
async fn listen_off_honors_persisted_remote_control_enable() -> Result<()> {
    let agent9527_home = TempDir::new()?;
    let listener = configured_remote_control_listener(agent9527_home.path()).await?;
...
async fn remote_control_disable_returns_disabled_status() -> Result<()> {
    let agent9527_home = TempDir::new()?;
    let _listener = configured_remote_control_listener(agent9527_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_agent9527_home(agent9527_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_enable_returns_connecting_status() -> Result<()> {
    let agent9527_home = TempDir::new()?;
    let mut backend = BlockingRemoteControlBackend::start(agent9527_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_agent9527_home(agent9527_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_pairing_start_returns_pairing_artifacts() -> Result<()> {
    let agent9527_home = TempDir::new()?;
    let mut backend = PairingRemoteControlBackend::start(agent9527_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_agent9527_home(agent9527_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_client_management_works_while_disabled() -> Result<()> {
    let agent9527_home = TempDir::new()?;
    let mut backend = ClientManagementRemoteControlBackend::start(agent9527_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_agent9527_home(agent9527_home.path())
        .without_auto_env()
        .build_initialized()
...
impl ClientManagementRemoteControlBackend {
    async fn start(agent9527_home: &std::path::Path) -> Result<Self> {
        let listener = configured_remote_control_listener(agent9527_home).await?;

...
async fn thread_archive_requires_materialized_rollout() -> Result<()> {
    ...
    // Verify file moved.
    let archived_directory = agent9527_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
...
async fn thread_archive_succeeds_when_descendant_archive_fails() -> Result<()> {
    ...
    let child_rollout_path =
        find_thread_path_by_id_str(agent9527_home.path(), &child_id, /*state_db_ctx*/ None)
            .await?
            .expect("child rollout path");
    let archived_child_path = agent9527_home
        .path()

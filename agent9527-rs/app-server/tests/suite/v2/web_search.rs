...
async fn standalone_web_search_round_trips_output() -> Result<()> {
    ...
    let turn_metadata_header = search_request
        .headers
        .get("x-agent9527-turn-metadata")
        .context("standalone search should include x-agent9527-turn-metadata")?
        .to_str()
        .context("x-agent9527-turn-metadata should be valid ASCII")?;
    let turn_metadata: Value = serde_json::from_str(turn_metadata_header)
        .context("x-agent9527-turn-metadata should be valid JSON")?;
...
async fn mount_search_response(server: &MockServer) {
    Mock::given(method("POST"))
        .and(path("/api/agent9527/alpha/search"))
...
async fn search_request(server: &MockServer) -> Result<wiremock::Request> {
    ...
    requests
        .into_iter()
        .find(|request| request.url.path() == "/api/agent9527/alpha/search")
...
name = "OpenAI"
base_url = "{server_uri}/api/agent9527"
wire_api = "responses"

...
async fn openai_model_header_mismatch_emits_model_rerouted_notification_v2() -> Result<()> {
    ...
    let (rerouted, warning) = collect_model_reroute_and_warning(&mut mcp).await?;
    assert_eq!(
        rerouted,
    ...
    assert_eq!(warning.thread_id, Some(thread.id));
...
async fn cyber_policy_response_emits_typed_error_notification_v2() -> Result<()> {
    ...
    assert_eq!(
        error,
        ErrorNotification {
            error: agent9527_app_server_protocol::TurnError {
...
async fn collect_model_reroute_and_warning(
    mcp: &mut TestAppServer,
) -> Result<(ModelReroutedNotification, WarningNotification)> {
    ...
    loop {
        ...
        match notification.method.as_str() {
            ...
            "turn/completed" => {
                let rerouted = rerouted.ok_or_else(|| {
    ...
    loop {
        ...
        match notification.method.as_str() {
            "error" => {
                ...
                if payload.error.agent9527_error_info == Some(Agent9527ErrorInfo::CyberPolicy) {
                    error = Some(payload);
                }
            }
            "model/rerouted" => {

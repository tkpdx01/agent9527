...
use agent9527_app_server_protocol::AppInfo;
use agent9527_app_server_protocol::AppMetadata;
...
use agent9527_app_server_protocol::HookEventName;
...
use agent9527_app_server_protocol::ScheduledTaskSchedule;
use agent9527_app_server_protocol::ScheduledTaskSummary;
use agent9527_app_server_protocol::ScheduledTaskWeekday;
use agent9527_app_server_protocol::SkillInterface;
    ...
    let request_id = mcp
        .send_plugin_read_request(PluginReadParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                agent9527_home.path().join("marketplace.json"),
            )?),
...
"#,
    ...
    write_installed_plugin(&agent9527_home, "openai-curated", "demo-plugin")?;
...
async fn plugin_read_returns_share_context_for_shared_local_plugin() -> Result<()> {
    ...
    write_plugin_share_local_path_mapping(agent9527_home.path(), "plugins_123", &plugin_path)?;
    Mock::given(method("GET"))
...
async fn plugin_read_falls_back_to_local_share_context_without_remote_auth() -> Result<()> {
    let agent9527_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    write_plugins_enabled_config(&agent9527_home)?;
    write_plugin_marketplace(
        repo_root.path(),
        "agent9527-curated",
        "demo-plugin",
...
async fn plugin_read_fails_on_malformed_share_mapping() -> Result<()> {
    ...
    write_plugin_source(repo_root.path(), "demo-plugin", &[])?;
    std::fs::create_dir_all(agent9527_home.path().join(".tmp"))?;
    std::fs::write(
        agent9527_home
            .path()
...
[plugins."demo-plugin@agent9527-curated"]
enabled = true

[hooks.state."demo-plugin@agent9527-curated:hooks/hooks.json:pre_tool_use:0:0"]
enabled = false
"#,
    )?;
    write_installed_plugin(&agent9527_home, "agent9527-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "agent9527-curated");
    assert_eq!(response.plugin.marketplace_path, Some(marketplace_path));
    assert_eq!(response.plugin.summary.id, "demo-plugin@agent9527-curated");
    assert_eq!(response.plugin.summary.name, "demo-plugin");
    assert_eq!(
        response.plugin.description.as_deref(),
    ...
    assert_eq!(
        ...
        vec![
            agent9527_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@agent9527-curated:hooks/hooks.json:pre_tool_use:0:0".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            agent9527_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@agent9527-curated:hooks/hooks.json:pre_tool_use:0:1".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            agent9527_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@agent9527-curated:hooks/hooks.json:session_start:0:0".to_string(),
...
async fn plugin_read_batches_large_app_metadata_requests() -> Result<()> {
    ...
    let agent9527_home = TempDir::new()?;
    write_connectors_config(agent9527_home.path(), &server_url)?;
    write_chatgpt_auth(
        agent9527_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn plugin_read_stops_batching_after_app_metadata_failure() -> Result<()> {
    ...
    let agent9527_home = TempDir::new()?;
    write_connectors_config(agent9527_home.path(), &server.uri())?;
    write_chatgpt_auth(
        agent9527_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn plugin_read_hides_apps_for_api_key_auth() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .without_auto_env()
        .with_env_overrides(&[
            ("AGENT9527_ACCESS_TOKEN", None),
            ("AGENT9527_API_KEY", None),
            ("OPENAI_API_KEY", None),
        ])
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
        .await?;
...
fn write_plugins_enabled_config(agent9527_home: &TempDir) -> Result<()> {
    std::fs::write(
        agent9527_home.path().join("config.toml"),
        r#"[features]
plugins = true
...
async fn batch_apps(
    ...
    let product_sku_ok = headers
        .get("oai-product-sku")
        .and_then(|value| value.to_str().ok())
        .is_some_and(|value| value == "agent9527");

...
async fn turn_start_shell_zsh_fork_executes_command_v2() -> Result<()> {
    ...
    let mut mcp = create_zsh_test_mcp_process(&agent9527_home, &workspace, &zsh_path).await?;
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            model: Some("mock-model".to_string()),
            effort: Some(agent9527_protocol::openai_models::ReasoningEffort::Medium),
            summary: Some(agent9527_protocol::config_types::ReasoningSummary::Auto),
            ..Default::default()
        })
        .await?;
    ...
    assert!(
        command.starts_with(
            &command_packaged_zsh_path(&agent9527_home)
                .display()
                .to_string()
        )
    );
...
async fn turn_start_shell_zsh_fork_subcommand_decline_marks_parent_declined_v2() -> Result<()> {
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            cwd: Some(workspace.clone()),
            approval_policy: Some(agent9527_app_server_protocol::AskForApproval::UnlessTrusted),
...
async fn create_zsh_test_mcp_process(
    ...
    let app_server = create_test_package_app_server(agent9527_home, zsh_path)?;
...
fn create_test_package_app_server(agent9527_home: &Path, zsh_path: &Path) -> Result<PathBuf> {
    let package_dir = agent9527_home.join("test-package");
    let bin_dir = package_dir.join("bin");
    let package_zsh_path = packaged_zsh_path(agent9527_home);
    ...
    std::fs::write(package_dir.join("agent9527-package.json"), "{}")?;

    let app_server = bin_dir.join("agent9527-app-server");
    copy_with_permissions(
        &agent9527_utils_cargo_bin::cargo_bin("agent9527-app-server")?,
        &app_server,
    )?;
...
fn packaged_zsh_path(agent9527_home: &Path) -> PathBuf {
    agent9527_home
        .join("test-package")
...
fn command_packaged_zsh_path(agent9527_home: &Path) -> PathBuf {
    let path = packaged_zsh_path(agent9527_home);
...
fn find_test_zsh_path() -> Result<Option<std::path::PathBuf>> {
    let repo_root = agent9527_utils_cargo_bin::repo_root()?;
    let dotslash_zsh = repo_root.join("agent9527-rs/app-server/tests/suite/zsh");

...
use agent9527_state::Stage1JobClaimOutcome;
...
async fn memory_reset_clears_memory_files_and_rows_preserves_threads() -> Result<()> {
    ...
    create_config_toml(agent9527_home.path())?;
    ...
    let thread_id = seed_stage1_output(&state_db, agent9527_home.path()).await?;
...
async fn seed_stage1_output(
    ...
    let mut builder = ThreadMetadataBuilder::new(
        thread_id,
        agent9527_home.join("sessions").join("test.jsonl"),
        now,
        SessionSource::Cli,
    );
...
fn create_config_toml(agent9527_home: &Path) -> std::io::Result<()> {

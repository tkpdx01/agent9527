...
use agent9527_app_server_protocol::DynamicToolCallResponse;
use agent9527_app_server_protocol::DynamicToolCallStatus;
use agent9527_app_server_protocol::DynamicToolFunctionSpec;
use agent9527_app_server_protocol::DynamicToolNamespaceSpec;
...
async fn dynamic_tool_call_round_trip_sends_text_content_items_to_model() -> Result<()> {
    ...
    let tool_namespace = "agent9527_app";
...
async fn start_function_dynamic_tool_call(call_id: &str) -> Result<PendingDynamicToolCall> {
    ...
    let model_info =
        agent9527_core::test_support::construct_model_info_offline("mock-model", &config);
    write_models_cache_with_models(agent9527_home.path(), vec![model_info])?;

use agent9527_app_server_protocol::AppConfig;
use agent9527_app_server_protocol::AppToolApproval;
use agent9527_app_server_protocol::ApprovalsReviewer;
use agent9527_app_server_protocol::AppsConfig;
use agent9527_app_server_protocol::AppsDefaultConfig;
...
use agent9527_app_server_protocol::ConfigLayerSource;
...
use agent9527_app_server_protocol::ForcedChatgptWorkspaceIds;
...
use agent9527_app_server_protocol::ToolsV2;
...
use agent9527_protocol::config_types::WebSearchContextSize;
use agent9527_protocol::config_types::WebSearchLocation;
...
fn write_config(agent9527_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
"#,
    ...
    let agent9527_home_path = agent9527_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(agent9527_home_path.join("config.toml"))?;
...
"#,
    ...
    assert_eq!(desktop.get("selected-avatar-id"), Some(&json!("agent9527")));
    assert_eq!(
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let agent9527_home = TempDir::new()?;
    write_config(&agent9527_home, r#"model = "gpt-user""#)?;
...
"#,
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_agent9527_folder: project_config
        }
    );
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let agent9527_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path =
        AbsolutePathBuf::resolve_path_against_base("config.toml", agent9527_home);
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let agent9527_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(agent9527_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
fn assert_layers_user_then_optional_system(
    layers: &[agent9527_app_server_protocol::ConfigLayer],

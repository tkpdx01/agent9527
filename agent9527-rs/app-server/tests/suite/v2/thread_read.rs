...
async fn thread_search_occurrences_reads_paginated_projection() -> Result<()> {
    ...
    let sqlite = agent9527_state::SqliteConfig::new_for_testing(agent9527_home.path().abs());
    let state_db =
        agent9527_state::StateRuntime::init(sqlite.clone(), "mock_provider".to_string()).await?;

...
async fn plugin_share_save_uploads_local_plugin() -> Result<()> {
    ...
    write_corrupt_plugin_share_local_path_mapping(agent9527_home.path())?;
...
async fn plugin_share_checkout_adds_personal_marketplace_entry() -> Result<()> {
    ...
    assert_eq!(
        ...
        PluginShareCheckoutResponse {
            remote_plugin_id: "plugins_123".to_string(),
            plugin_id: "demo-plugin@agent9527-curated".to_string(),
    ...
    let request_id = mcp
        .send_plugin_list_request(PluginListParams {
            ...
            marketplace_kinds: Some(vec![
                agent9527_app_server_protocol::PluginListMarketplaceKind::Local,
            ]),
            force_refetch: false,
        })
        .await?;
    ...
    assert_eq!(response.marketplaces[0].name, "agent9527-curated");
...
async fn plugin_share_update_targets_updates_share_targets() -> Result<()> {
    ...
    assert_eq!(
        ...
        PluginShareUpdateTargetsResponse {
            ...
            discoverability: agent9527_app_server_protocol::PluginShareDiscoverability::Unlisted,
        }
    );
    Ok(())
}
...
async fn plugin_share_update_targets_publishes_workspace_plugin() -> Result<()> {
    ...
    assert_eq!(
        ...
        PluginShareUpdateTargetsResponse {
            ...
            discoverability: agent9527_app_server_protocol::PluginShareDiscoverability::Listed,
        }
    );
    Ok(())
}
...
async fn plugin_share_delete_removes_created_workspace_plugin() -> Result<()> {
    ...
    let local_plugin_path = AbsolutePathBuf::try_from(agent9527_home.path().join("local-plugin"))?;
...
fn write_remote_plugin_config(agent9527_home: &Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
...
fn write_test_plugin(root: &Path, plugin_name: &str) -> std::io::Result<PathBuf> {
    ...
    write_file(
        &plugin_path.join(".agent9527-plugin/plugin.json"),
...
fn write_corrupt_plugin_share_local_path_mapping(agent9527_home: &Path) -> std::io::Result<()> {
    write_file(

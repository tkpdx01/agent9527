...
async fn selected_executor_plugin_exposes_its_mcps_only_to_that_thread() -> Result<()> {
    ...
    let agent9527_bin = toml::Value::String(
        agent9527_utils_cargo_bin::cargo_bin("agent9527")?
            .to_string_lossy()
            .into_owned(),
    );
...
"#
    ...
    std::fs::create_dir_all(plugin.path().join(".agent9527-plugin"))?;
    std::fs::write(
        plugin.path().join(".agent9527-plugin/plugin.json"),

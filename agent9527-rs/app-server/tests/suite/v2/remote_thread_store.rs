...
//! the temporary `agent9527_home` must not contain rollout session files or sqlite
...
use agent9527_app_server::in_process::InProcessClientHandle;
use agent9527_app_server::in_process::InProcessServerEvent;
...
use agent9527_thread_store::CreateThreadParams as StoreCreateThreadParams;
...
async fn thread_start_rejects_paginated_history_without_list_support() -> Result<()> {
    ...
    create_config_toml_with_thread_store(agent9527_home.path(), &server.uri(), &store_id)?;
...
async fn thread_delete_with_non_local_thread_store_does_not_create_local_persistence() -> Result<()>
{
    ...
    let agent9527_home = TempDir::new()?;
    let store_id = Uuid::new_v4().to_string();
    // Plugin startup warmups may create `.tmp` under agent9527_home. Disable them
    ...
    let mut client = start_in_process_server(agent9527_home.path()).await?;
    ...
    assert_no_local_persistence_artifacts(agent9527_home.path())?;
...
async fn start_in_process_server(agent9527_home: &Path) -> Result<InProcessClientHandle> {
    let loader_overrides = LoaderOverrides::without_managed_config_for_tests();
    let config = Arc::new(
        ConfigBuilder::default()
            .agent9527_home(agent9527_home.to_path_buf())
            .fallback_cwd(Some(agent9527_home.to_path_buf()))
            .loader_overrides(loader_overrides.clone())
            .build()
            .await?,
    );
...
fn assert_no_local_persistence_artifacts(agent9527_home: &Path) -> Result<()> {
    ...
    // the isolated test agent9527_home.
    assert!(
        !agent9527_home.join("sessions").exists(),
        "non-local thread persistence should not create local rollout sessions"
    );
    assert!(
        !agent9527_home.join("archived_sessions").exists(),
        "non-local thread persistence should not create archived rollout sessions"
    );
    assert!(
        !agent9527_state::state_db_path(agent9527_home).exists(),
        "non-local thread persistence should not create local thread sqlite"
    );

    let sqlite_artifacts = std::fs::read_dir(agent9527_home)?
    ...
    let mut entries = agent9527_home_entries(agent9527_home)?;
    // Bazel test runs may initialize shell snapshot storage under agent9527_home.
    ...
    assert_eq!(
        ...
        "non-local thread persistence should not create unexpected files in agent9527_home"
    );

    Ok(())
}

fn agent9527_home_entries(agent9527_home: &Path) -> Result<BTreeSet<String>> {
    Ok(std::fs::read_dir(agent9527_home)?
        .filter_map(|entry| {
            let entry = entry.ok()?;

...
async fn installed_apps_force_refresh_only_refreshes_tools_snapshot() -> Result<()> {
    ...
    let agent9527_home = configured_agent9527_home(fixture.base_url())?;
    let mut app_server = start_app_server(agent9527_home.path()).await?;
...
async fn start_app_server(agent9527_home: &Path) -> Result<TestAppServer> {
...
fn configured_agent9527_home(base_url: &str) -> Result<TempDir> {

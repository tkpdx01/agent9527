...
const AGENT9527_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are Agent9527, a coding agent using the configured model. You and the user share the same workspace and collaborate to achieve the user's goals.";

#[tokio::test]
async fn thread_resume_paginated_metadata_only_uses_model_context() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let agent9527_home = TempDir::new()?;
    mock_responses_config(&server.uri()).write(agent9527_home.path())?;
...
-> Result<()> {
    ...
    assert_eq!(model, agent9527_models_manager::MODEL_PRO);
...
async fn goal_first_live_thread_appears_in_state_db_thread_list() -> Result<()> {
    ...
    mock_responses_config(&server.uri()).write(&agent9527_home_path)?;
...
async fn thread_resume_tracks_thread_initialized_analytics() -> Result<()> {
    ...
    assert_basic_thread_initialized_event(
        ...
        &thread.session_id,
        "agent9527_work_desktop",
        agent9527_models_manager::MODEL_BALANCED,
        "resumed",
        "user",
    );
...
async fn thread_goal_lifecycle_emits_analytics_and_clear_deletes_goal() -> Result<()> {
    ...
    let get_id = mcp
        ...
        .await?;
    let get: agent9527_app_server_protocol::ThreadGoalGetResponse =
...
async fn thread_resume_rejoins_running_paginated_thread_with_initial_page() -> Result<()> {
    ...
    assert_eq!(model, agent9527_models_manager::MODEL_BALANCED);
...
"#;
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    mock_responses_config(&server.uri()).write(&agent9527_home)?;
...
async fn thread_resume_accepts_personality_override() -> Result<()> {
    ...
    let start_id = primary
        .send_thread_start_request_with_auto_env(ThreadStartParams {
            model: Some("exp-agent9527-personality".to_string()),
            ..Default::default()
        })
        .await?;
    let ThreadStartResponse { thread, .. } =
    ...
    assert!(
        instructions_text.contains(AGENT9527_INSTRUCTIONS_TEMPLATE_DEFAULT),
...
async fn setup_rollout_fixture(agent9527_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    mock_responses_config(server_uri).write(agent9527_home)?;

...
fn init_marketplace_repo(root: &Path, marketplace_name: &str, marker: &str) -> Result<String> {
    run_git(root, &["init"])?;
    run_git(root, &["config", "user.email", "agent9527@example.com"])?;
    run_git(root, &["config", "user.name", "Agent9527 Tests"])?;
...
fn disable_plugin_startup_tasks(agent9527_home: &Path) -> Result<()> {
...
fn marketplace_install_root(agent9527_home: &Path) -> std::path::PathBuf {
    agent9527_home.join(INSTALLED_MARKETPLACES_DIR)
}
...
fn expected_installed_root(
    ...
    AbsolutePathBuf::try_from(
        marketplace_install_root(&agent9527_home.canonicalize()?).join(marketplace_name),
    )
...
async fn marketplace_upgrade_all_configured_git_marketplaces() -> Result<()> {
    ...
    disable_plugin_startup_tasks(agent9527_home.path())?;
    ...
    let debug_root = expected_installed_root(agent9527_home.path(), "debug")?;
    let tools_root = expected_installed_root(agent9527_home.path(), "tools")?;

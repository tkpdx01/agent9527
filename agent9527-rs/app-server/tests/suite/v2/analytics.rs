...
use agent9527_config::types::OtelHttpProtocol;
...
fn set_metrics_exporter(config: &mut agent9527_core::config::Config) {
...
pub(crate) async fn wait_for_goal_event(
    ...
    wait_for_matching_analytics_event(server, read_timeout, |event| {
        event["event_type"] == "agent9527_goal_event"
...
pub(crate) fn thread_initialized_event(payload: &Value) -> Result<&Value> {
    ...
    events
        ...
        .ok_or_else(|| anyhow::anyhow!("agent9527_thread_initialized event should be present"))
}

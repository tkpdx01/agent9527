use agent9527_app_server::INPUT_TOO_LARGE_ERROR_CODE;
use agent9527_app_server::INVALID_PARAMS_ERROR_CODE;
use agent9527_app_server_protocol::AdditionalContextEntry;
use agent9527_app_server_protocol::AdditionalContextKind;
use agent9527_app_server_protocol::ByteRange;
...
use agent9527_app_server_protocol::FileChangePatchUpdatedNotification;
...
use agent9527_app_server_protocol::TextElement;
...
use agent9527_core::test_support::all_model_presets;
...
const TEST_ORIGINATOR: &str = "agent9527_vscode";
...
async fn run_local_image_turn(detail: Option<ImageDetail>) -> Result<Vec<Value>> {
    // Two Agent9527 turns hit the mock model (session start + turn/start).
    let responses = vec![
    ...
    let image_path = agent9527_home.path().join("image.png");
...
async fn turn_start_emits_thread_scoped_warning_notification_for_trimmed_skills() -> Result<()> {
    ...
    MockResponsesConfig::new(&server.uri())
        .enable_feature(Feature::Personality)
        .write(agent9527_home.path())?;
    write_models_cache(agent9527_home.path())?;
    let cache_path = agent9527_home.path().join("models_cache.json");
    ...
    write_test_skill(agent9527_home.path(), "alpha-skill")?;
    write_test_skill(agent9527_home.path(), "beta-skill")?;

    let isolated_home = agent9527_home.path().to_string_lossy();
    let mut mcp = TestAppServer::builder()
        .with_agent9527_home(agent9527_home.path())
        .with_env_overrides(&[
...
async fn turn_start_emits_raw_response_completed_with_upstream_usage() -> Result<()> {
    ...
    .await??;
    let notification: agent9527_app_server_protocol::ServerNotification =
        notification.try_into()?;
    let agent9527_app_server_protocol::ServerNotification::RawResponseCompleted(notification) =
        notification
    else {
...
async fn turn_start_tracks_thread_originator_in_analytics() -> Result<()> {
    ...
    let event =
        wait_for_analytics_event(&server, DEFAULT_READ_TIMEOUT, "agent9527_turn_event").await?;
...
async fn turn_start_accepts_managed_network_profile_from_requirements() -> Result<()> {
    ...
    MockResponsesConfig::new(&server.uri())
        ...
        .write(agent9527_home.path())?;
    std::fs::write(
        agent9527_home.path().join("requirements.toml"),
        r#"
    ...
    let turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            environments: Some(vec![TurnEnvironmentParams {
                ...
                cwd: agent9527_utils_absolute_path::AbsolutePathBuf::try_from(
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three Agent9527 turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ...
    assert_eq!(
        started.turn.status,
        agent9527_app_server_protocol::TurnStatus::InProgress
    );
    assert_eq!(started.turn.id, turn.id);
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let agent9527_home = tmp.path().to_path_buf();
    ...
    MockResponsesConfig::new(&server.uri())
        ...
        .write(agent9527_home.as_path())?;

    let mut mcp = TestAppServer::builder()
        .with_agent9527_home(agent9527_home.as_path())
        .build_initialized()
        .await?;
    ...
    let _: TurnStartResponse = mcp
        .request(|request_id| ClientRequest::TurnStart {
            ...
            params: TurnStartParams {
                ...
                approval_policy: Some(agent9527_app_server_protocol::AskForApproval::Never),
                sandbox_policy: Some(
...
async fn turn_start_explicit_local_environment_updates_legacy_cwd_between_turns() -> Result<()> {
    ...
    let first_writable_root =
        agent9527_utils_absolute_path::AbsolutePathBuf::try_from(first_cwd.clone())?;
    let _: TurnStartResponse = mcp
        .request(|request_id| ClientRequest::TurnStart {
            request_id,
            params: TurnStartParams {
                ...
                sandbox_policy: Some(
                    agent9527_app_server_protocol::SandboxPolicy::WorkspaceWrite {
...
async fn turn_start_permission_profile_rebinds_runtime_workspace_roots_between_turns() -> Result<()>
{
    ...
    let old_root = agent9527_utils_absolute_path::AbsolutePathBuf::from_absolute_path(old_root)?;
    let new_root = agent9527_utils_absolute_path::AbsolutePathBuf::from_absolute_path(new_root)?;
...
async fn turn_start_resolves_sticky_thread_local_environment_and_turn_overrides() -> Result<()> {
    ...
    let server = create_mock_responses_server_repeating_assistant("done").await;
    MockResponsesConfig::new(&server.uri()).write(&agent9527_home)?;
    std::fs::write(
        agent9527_home.join("environments.toml"),
        r#"
[[environments]]
id = "remote"
url = "ws://127.0.0.1:1"
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![agent9527_app_server_protocol::FileUpdateChange {
...
async fn turn_start_streams_apply_patch_change_updates_v2() -> Result<()> {
    ...
    MockResponsesConfig::new(&server.uri())
        ...
        .write(&agent9527_home)?;
    write_models_cache(&agent9527_home)?;
    let cache_path = agent9527_home.join("models_cache.json");
    let mut cache: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&cache_path)?)?;
    let models = cache["models"]
        .as_array_mut()
...
async fn turn_start_emits_spawn_agent_item_with_model_metadata_v2() -> Result<()> {
    ...
    const REQUESTED_MODEL: &str = agent9527_models_manager::MODEL_PRO;
...
async fn turn_start_emits_spawn_agent_item_with_effective_role_model_metadata_v2() -> Result<()> {
    ...
    const REQUESTED_MODEL: &str = agent9527_models_manager::MODEL_PRO;
    const REQUESTED_REASONING_EFFORT: ReasoningEffort = ReasoningEffort::Low;
    const ROLE_MODEL: &str = agent9527_models_manager::MODEL_BALANCED;
    ...
    MockResponsesConfig::new(&server.uri())
        ...
        .write(agent9527_home.path())?;
    std::fs::write(
        agent9527_home.path().join("custom-role.toml"),
...
async fn command_execution_notifications_include_trusted_plugin_id() -> Result<()> {
    ...
    let synced_root = agent9527_home.path().join(".tmp/plugins");
    for path in [
        plugin_root.join(".agent9527-plugin"),
        script_path
            .parent()
    ...
    std::fs::write(
        agent9527_home.path().join(".tmp/plugins.sha"),
        format!("{curated_sha}\n"),
    )?;
    std::fs::write(
...
async fn turn_start_with_elevated_override_does_not_persist_project_trust() -> Result<()> {
    ...
    let config_toml = std::fs::read_to_string(agent9527_home.path().join("config.toml"))?;
...
fn write_test_skill(agent9527_home: &Path, name: &str) -> std::io::Result<()> {
    let skill_dir = agent9527_home.join("skills").join(name);

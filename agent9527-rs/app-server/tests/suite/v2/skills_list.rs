...
use agent9527_app_server_protocol::SkillsChangedNotification;
...
fn write_cached_remote_plugin_with_skill(
    ...
    let plugin_root = agent9527_home.join("plugins/cache/openai-curated-remote/linear/local");
    std::fs::create_dir_all(plugin_root.join(".agent9527-plugin"))?;
    std::fs::write(
        plugin_root.join(".agent9527-plugin/plugin.json"),
        r#"{"name":"linear"}"#,
...
fn write_cached_local_curated_plugin_with_skill(agent9527_home: &std::path::Path) -> Result<()> {
    let plugin_root = agent9527_home.join("plugins/cache/openai-curated/google-calendar/local");
    std::fs::create_dir_all(plugin_root.join(".agent9527-plugin"))?;
    std::fs::write(
        plugin_root.join(".agent9527-plugin/plugin.json"),
...
async fn runtime_remote_plugin_toggle_updates_local_curated_plugin_skills() -> Result<()> {
    ...
    write_cached_local_curated_plugin_with_skill(agent9527_home.path())?;
    std::fs::write(
...
#[tokio::test]
async fn skills_list_excludes_plugin_skills_when_workspace_agent9527_plugins_disabled() -> Result<()>
{
    let agent9527_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    write_skill(&agent9527_home, "home-skill")?;
    ...
    assert!(
        ...
        "plugin skills should be hidden when workspace Agent9527 plugins are disabled"
    );
    Ok(())
}
...
async fn skills_list_skips_cwd_roots_when_environment_disabled() -> Result<()> {
    ...
    let cwd = TempDir::new()?;
    write_skill(&agent9527_home, "home-skill")?;
    let repo_skill_dir = cwd.path().join(".agent9527/skills/repo-skill");
...
async fn skills_list_accepts_relative_cwds() -> Result<()> {
    ...
    std::fs::create_dir_all(agent9527_home.path().join(&relative_cwd))?;
...
-> Result<()> {
    ...
    let skill_dir = cwd.path().join(".agent9527/skills/late-extra-skill");
    std::fs::create_dir_all(&skill_dir)?;
    std::fs::write(
        skill_dir.join("SKILL.md"),
...
async fn skills_changed_notification_is_emitted_after_skill_change() -> Result<()> {
    ...
    write_skill(&agent9527_home, "demo")?;

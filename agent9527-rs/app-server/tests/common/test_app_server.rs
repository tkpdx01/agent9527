...
use agent9527_app_server_protocol::ClientNotification;
...
use agent9527_app_server_protocol::ConfigReadParams;
use agent9527_app_server_protocol::ConfigValueWriteParams;
...
use agent9527_app_server_protocol::FsCopyParams;
use agent9527_app_server_protocol::FsCreateDirectoryParams;
use agent9527_app_server_protocol::FsGetMetadataParams;
use agent9527_app_server_protocol::FsReadDirectoryParams;
use agent9527_app_server_protocol::FsReadFileParams;
use agent9527_app_server_protocol::FsRemoveParams;
use agent9527_app_server_protocol::FsUnwatchParams;
use agent9527_app_server_protocol::FsWatchParams;
use agent9527_app_server_protocol::FsWriteFileParams;
...
use agent9527_app_server_protocol::ProcessKillParams;
use agent9527_app_server_protocol::ProcessResizePtyParams;
use agent9527_app_server_protocol::ProcessSpawnParams;
use agent9527_app_server_protocol::ProcessWriteStdinParams;
use agent9527_app_server_protocol::RemoteControlClientsListParams;
use agent9527_app_server_protocol::RemoteControlClientsRevokeParams;
use agent9527_app_server_protocol::RemoteControlPairingStartParams;
use agent9527_app_server_protocol::RemoteControlPairingStatusParams;
...
use agent9527_app_server_protocol::ThreadRealtimeListVoicesParams;
...
use agent9527_exec_server::AGENT9527_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR;
use agent9527_exec_server::AGENT9527_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR;
use agent9527_exec_server::AGENT9527_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR;
use agent9527_exec_server::AGENT9527_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR;
use agent9527_exec_server::AGENT9527_EXEC_SERVER_URL_ENV_VAR;
...
use core_test_support::test_agent9527::TestEnv;
use core_test_support::test_agent9527::test_env;
...
pub struct TestAppServer {
    ...
    // removing an owned AGENT9527_HOME that may still be its cwd on Windows.
    _delayed_exec_server: Option<(LocalWebsocketExecServer, WebsocketDelayInterposer)>,
    _owned_agent9527_home: Option<TempDir>,
}

pub const DEFAULT_CLIENT_NAME: &str = "agent9527-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "AGENT9527_APP_SERVER_DISABLE_MANAGED_CONFIG";
const CODE_MODE_HOST_PATH_ENV_VAR: &str = "AGENT9527_CODE_MODE_HOST_PATH";

impl TestAppServer {
    /// Starts building a server with a temporary AGENT9527_HOME and the standard
    ...
    /// Returns the effective AGENT9527_HOME used by the child app-server.
    pub fn agent9527_home(&self) -> &Path {
        &self.agent9527_home
    }
    ...
    async fn new_with_program_env_and_args(
        ...
        cmd.current_dir(agent9527_home);
        ...
        cmd.env(
            ...
            agent9527_home.join("managed_config.toml"),
        );
        cmd.env_remove(AGENT9527_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("agent9527-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: agent9527_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl TestAppServerBuilder {
    /// Uses this existing AGENT9527_HOME instead of a temporary one.
    pub fn with_agent9527_home(mut self, agent9527_home: &Path) -> Self {
        self.agent9527_home = Some(agent9527_home.to_path_buf());
        self
    }
    ...
    /// Builds a server with a temporary AGENT9527_HOME and automatic environment
    /// by default.
    pub async fn build(self) -> anyhow::Result<TestAppServer> {
        ...
        let (agent9527_home, owned_agent9527_home) = match agent9527_home {
            Some(agent9527_home) => (agent9527_home, None),
            None => {
                let owned_agent9527_home = TempDir::new()?;
                (
                    owned_agent9527_home.path().to_path_buf(),
                    Some(owned_agent9527_home),
                )
            }
        };
        let (auto_env, delayed_exec_server) = match environment {
            TestAppServerEnvironment::Auto => {
                let environments_toml = agent9527_home.join("environments.toml");
                ensure!(
                ...
                let (auto_env, delayed_exec_server) = match exec_server_delay {
                    Some(added_delay) => {
                        ...
                        let exec_server_program =
                            agent9527_utils_cargo_bin::cargo_bin("exec-server")
                        ...
                        let local_websocket_exec_server =
                            LocalWebsocketExecServer::start(&agent9527_home, &exec_server_program)
                                .await?;
                ...
                let mut auto_env_overrides = vec![
                    (
                        AGENT9527_EXEC_SERVER_URL_ENV_VAR.to_string(),
                        auto_env.exec_server_url().map(str::to_string),
                    ),
                    (
                        AGENT9527_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR.to_string(),
                        None,
                    ),
                    (
                        AGENT9527_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                    (
                        AGENT9527_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR.to_string(),
                        None,
                    ),
                    (
                        AGENT9527_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                ];
        ...
        if !env_overrides
            ...
            && let Ok(code_mode_host_program) =
                agent9527_utils_cargo_bin::cargo_bin("agent9527-code-mode-host")
        {
        ...
        let program = match program {
            ...
            None => agent9527_utils_cargo_bin::cargo_bin("agent9527-app-server")
                .context("should find binary for agent9527-app-server")?,
        };
        ...
        app_server._owned_agent9527_home = owned_agent9527_home;
...
impl Drop for TestAppServer {
    fn drop(&mut self) {
        // These tests spawn a `agent9527-app-server` child process.
        //

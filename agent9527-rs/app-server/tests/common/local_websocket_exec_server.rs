...
impl LocalWebsocketExecServer {
    pub(crate) async fn start(agent9527_home: &Path, exec_server_program: &Path) -> Result<Self> {
        let mut command = Command::new(exec_server_program);
        command.stdin(Stdio::null());
        command.stdout(Stdio::piped());
        command.stderr(Stdio::inherit());
        command.current_dir(agent9527_home);
        command.env("AGENT9527_HOME", agent9527_home);
        #[cfg(target_os = "linux")]
        command.env(
            AGENT9527_LINUX_SANDBOX_EXE_ENV_VAR,

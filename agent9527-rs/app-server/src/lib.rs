...
use agent9527_analytics::AppServerRpcTransport;
...
use agent9527_app_server_protocol::TextPosition as AppTextPosition;
use agent9527_app_server_protocol::TextRange as AppTextRange;
use agent9527_config::ConfigLayerSource;
use agent9527_config::ConfigLoadError;
use agent9527_config::TextRange as CoreTextRange;
use agent9527_core::ExecPolicyError;
...
use agent9527_rollout::state_db as rollout_state_db;
...
const SQLITE_RECOVERY_CONFIG_WARNING_SUMMARY: &str = "Agent9527 rebuilt its local database.";
...
const OTEL_SERVICE_NAME: &str = "agent9527-app-server";
#[cfg(debug_assertions)]
const TEST_USER_CONFIG_FILE_ENV_VAR: &str = "AGENT9527_APP_SERVER_TEST_USER_CONFIG_FILE";
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        disabled_folders.push((
            dot_agent9527_folder.as_path().display().to_string(),
...
pub async fn run_main_with_transport_options(
    ...
    let environment_manager = if loader_overrides.ignore_user_config {
        ...
        EnvironmentManager::from_agent9527_home(agent9527_home.clone(), Some(local_runtime_paths))
            .await
    }
    .map(Arc::new)
        ...
        .await
    {
        Ok(config) => {
            ...
            let auth_manager = AuthManager::shared_from_config(
                &config, /*enable_agent9527_api_key_env*/ false,
            )
            .await;
            config_manager
    ...
    let otel = agent9527_core::otel_init::build_provider(
        &config,
    ...
    agent9527_core::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    agent9527_core::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
    let unix_socket_startup_lock = match &transport {
        AppServerTransport::UnixSocket { socket_path } => {
            let startup_lock_path = app_server_startup_lock_path(&agent9527_home)?;
    ...
    let feedback = Agent9527Feedback::new();
...
async fn init_sqlite_state_db_with_fresh_start_on_corruption(
    ...
    loop {
        ...
        if !agent9527_state::is_sqlite_corruption_error(&err)
        ...
        emit_state_db_backup_warning(&format!(
            "Agent9527 local database at {} appears damaged. Moving it into a backup folder so the app server can rebuild it from saved data.",
            database_path.display()
        ));
        let backups = agent9527_state::backup_runtime_db_for_fresh_start(database_path.as_path())
            .await
            .map_err(|backup_err| {
                anyhow::anyhow!(
        ...
        for backup in &backups {
            emit_state_db_backup_warning(&format!(
                "Moved damaged Agent9527 local database file {} to {}",
...
mod tests {
    ...
    fn debug_test_user_config_file_overrides_loader_path() {
        let path = std::env::temp_dir().join("agent9527-app-server-test-config.toml");
        let loader_overrides = loader_overrides_with_test_user_config_file(

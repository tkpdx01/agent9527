...
use agent9527_cloud_config::cloud_config_bundle_loader;
...
/// Shared app-server entry point for loading effective Agent9527 configuration.
#[derive(Clone)]
...
impl ConfigManager {
    ...
    pub(crate) fn replace_cloud_config_bundle_loader(
        ...
        let loader =
            cloud_config_bundle_loader(auth_manager, chatgpt_base_url, self.agent9527_home.clone());
    ...
    fn current_thread_config_loader(&self) -> Arc<dyn ThreadConfigLoader> {
        self.thread_config_loader
            ...
            .unwrap_or_else(|_| Arc::new(agent9527_config::NoopThreadConfigLoader))
    }
    ...
    pub(crate) async fn load_with_cli_overrides(
        ...
        let mut config = agent9527_core::config::ConfigBuilder::default()
            .agent9527_home(self.agent9527_home.clone())
    ...
    fn apply_arg0_paths(&self, config: &mut Config) {
        config.agent9527_self_exe = self.arg0_paths.agent9527_self_exe.clone();
        config.agent9527_linux_sandbox_exe = self.arg0_paths.agent9527_linux_sandbox_exe.clone();
    ...
    pub(crate) fn new_for_tests(
        ...
        Self::new(
            ...
            Arc::new(agent9527_config::NoopThreadConfigLoader),
        )
    }

    #[cfg(test)]
    pub(crate) fn without_managed_config_for_tests(agent9527_home: PathBuf) -> Self {
        Self::new_for_tests(
            agent9527_home,
            Vec::new(),

...
use agent9527_app_server_protocol::experimental_required_message;
...
pub(crate) struct MessageProcessorArgs {
    ...
    pub(crate) feedback: Agent9527Feedback,
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            let manager = ThreadManager::new(
                config.as_ref(),
                auth_manager.clone(),
                agent9527_core::build_models_manager(config.as_ref(), auth_manager.clone()),
    ...
    pub(crate) async fn process_request(
        ...
        Self::run_request_with_context(
            ...
            async {
                let agent9527_request = deserialize_client_request(request);
                let result = match agent9527_request {
                    Ok(agent9527_request) => {
    ...
    async fn handle_client_request(
        ...
        agent9527_request: ClientRequest,
        session: Arc<ConnectionSessionState>,
        ...
        if let ClientRequest::Initialize { request_id, params } = agent9527_request {
    ...
    async fn dispatch_initialized_client_request(
        ...
        if let Some(reason) = agent9527_request.experimental_reason()
        ...
        let serialization_scope = agent9527_request.serialization_scope();

use agent9527_app_server_protocol::ConfigLayer as ApiConfigLayer;
use agent9527_app_server_protocol::ConfigLayerMetadata as ApiConfigLayerMetadata;
use agent9527_app_server_protocol::ConfigLayerSource as ApiConfigLayerSource;
use agent9527_config::ConfigLayer;
use agent9527_config::ConfigLayerMetadata;
use agent9527_config::ConfigLayerSource;

/// Converts a config-layer source owned by `agent9527-config` into the app-server wire type owned by
/// `agent9527-app-server-protocol`.
///
...
/// Converts config-layer metadata owned by `agent9527-config` into the app-server wire type owned by
/// `agent9527-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
...
/// Converts a config layer owned by `agent9527-config` into the app-server wire type owned by
/// `agent9527-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion

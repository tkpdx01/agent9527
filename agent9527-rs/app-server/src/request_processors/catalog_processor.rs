use super::*;
use agent9527_core::config::permission_profile_catalog;
...
) -> Vec<agent9527_app_server_protocol::SkillMetadata> {
    skills
        .iter()
        .map(|skill| {
            let enabled = !disabled_paths.contains(&skill.path_to_skills_md);
            agent9527_app_server_protocol::SkillMetadata {
                ...
                dependencies: skill.dependencies.clone().map(|dependencies| {
                    agent9527_app_server_protocol::SkillDependencies {
                        tools: dependencies
                            .tools
                            .into_iter()
                            .map(|tool| agent9527_app_server_protocol::SkillToolDependency {
                                r#type: tool.r#type,
                                value: tool.value,
...
fn hooks_to_info(hooks: &[agent9527_hooks::HookListEntry]) -> Vec<HookMetadata> {
    hooks
        .iter()
...
fn errors_to_info(
    errors: &[agent9527_core::skills::SkillError],
) -> Vec<agent9527_app_server_protocol::SkillErrorInfo> {
    errors
        .iter()
        .map(|err| agent9527_app_server_protocol::SkillErrorInfo {
...
impl CatalogRequestProcessor {
    ...
    async fn experimental_feature_list_response(
        ...
        let workspace_agent9527_plugins_enabled = self
            ...
            .map(|spec| {
                ...
                ApiExperimentalFeature {
                    ...
                    enabled: config.features.enabled(spec.id)
                        && (workspace_agent9527_plugins_enabled
                ...
                async move {
                    let (cwd_abs, config_layer_stack) = match self.resolve_cwd_config(&cwd).await {
                        ...
                        Err(message) => {
                            ...
                            return (
                                index,
                                agent9527_app_server_protocol::SkillsListEntry {
                                    cwd,
                                    skills: Vec::new(),
                                    errors: vec![agent9527_app_server_protocol::SkillErrorInfo {
                                        path: error_path,
                                        message,
                                    }],
                                },
                    ...
                    let effective_skill_roots = if workspace_agent9527_plugins_enabled {
                    ...
                    let skills_input = agent9527_core::skills::SkillsLoadInput::new(
                        cwd_abs.clone(),
    ...
    async fn hooks_list_response(
        ...
        for cwd in cwds {
                ...
                .await
            {
                ...
                Err(err) => {
                    let error_path = cwd.clone();
                    data.push(agent9527_app_server_protocol::HooksListEntry {
                        cwd,
                        hooks: Vec::new(),
                        warnings: Vec::new(),
                        errors: vec![agent9527_app_server_protocol::HookErrorInfo {
                            path: error_path,
            ...
            let workspace_agent9527_plugins_enabled = self
                ...
                .await;
            let plugins_enabled =
                config.features.enabled(Feature::Plugins) && workspace_agent9527_plugins_enabled;
            let plugin_hooks = if plugins_enabled {
                let plugins_input = config.plugins_config_input();
                let plugin_outcome = plugins_manager.plugins_for_config(&plugins_input).await;
                agent9527_core_plugins::PluginHookLoadOutcome {
                ...
                agent9527_core_plugins::PluginHookLoadOutcome::default()
            };
            let hooks = agent9527_hooks::list_hooks(agent9527_hooks::HooksConfig {
    ...
    async fn skills_config_write_response_inner(
        ...
        let edits = vec![edit];
        ConfigEditsBuilder::new(&self.config.agent9527_home)
            .with_edits(edits)
            .apply()
            .await
            .map(|()| {

...
//! the configured Agent9527 executable as a subprocess, accepts only valid JSON from
//! `agent9527 doctor --json`, derives a small set of Sentry tags, and otherwise
...
use agent9527_feedback::FeedbackAttachment;
...
pub(crate) struct DoctorFeedbackReport {
    /// JSON support report to upload as `agent9527-doctor-report.json`.
    pub(crate) attachment: FeedbackAttachment,
    /// Low-cardinality Sentry tags derived from the report status and check ids.
    pub(crate) tags: BTreeMap<String, String>,
}

/// Runs `agent9527 doctor --json` and returns a best-effort feedback attachment.
///
/// Failure to spawn Agent9527, finish before the timeout, or parse JSON means the

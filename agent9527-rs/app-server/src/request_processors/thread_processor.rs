...
use agent9527_app_server_protocol::SelectedCapabilityRoot;
...
const AGENT9527_TUI_CLIENT_NAME: &str = "agent9527-tui";
...
fn collect_resume_override_mismatches(
    ...
    if let Some(requested_review_policy) = request.approvals_reviewer.as_ref() {
        let active_review_policy: agent9527_app_server_protocol::ApprovalsReviewer =
    ...
    if let Some(requested_sandbox) = request.sandbox.as_ref() {
        ...
        let sandbox_matches = matches!(
            ...
            (
                ...
                agent9527_protocol::protocol::SandboxPolicy::DangerFullAccess
            ) | (
                SandboxMode::DangerFullAccess,
...
fn validate_dynamic_tools(tools: &[DynamicToolSpec]) -> Result<(), String> {
    ...
    fn validate_dynamic_tool<'a>(
        ...
        if let Err(err) = agent9527_tools::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
...
impl ThreadRequestProcessor {
    ...
    pub(crate) async fn thread_rollback(
        ...
        if app_server_client_name != Some(AGENT9527_TUI_CLIENT_NAME) {
    ...
    async fn load_thread(
        &self,
        thread_id: &str,
    ) -> Result<(ThreadId, Arc<Agent9527Thread>), JSONRPCErrorError> {
    ...
    fn listener_task_context(&self) -> ListenerTaskContext {
        ListenerTaskContext {
            ...
            agent9527_home: self.config.agent9527_home.to_path_buf(),
    ...
    async fn thread_start_inner(
        ...
        if matches!(
            history_mode,
            Some(agent9527_app_server_protocol::ThreadHistoryMode::Paginated)
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<agent9527_protocol::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<agent9527_app_server_protocol::ThreadStartSource>,
            ...
            && (requested_permissions_trust_project || effective_permissions_trust_project)
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                agent9527_core::config::set_project_trust_level(
                    &listener_task_context.agent9527_home,
        ...
        if let Ok(Some(err)) =
            agent9527_core::check_execpolicy_for_warnings(&config.config_layer_stack).await
        {
        ...
        let NewThread {
            ...
            .start_thread(StartThreadOptions {
                ...
                initial_history: match session_start_source
                    .unwrap_or(agent9527_app_server_protocol::ThreadStartSource::Startup)
                {
                    agent9527_app_server_protocol::ThreadStartSource::Startup => {
                        InitialHistory::New
                    }
                    agent9527_app_server_protocol::ThreadStartSource::Clear => {
            ...
            .map_err(|err| match err.details() {
                ...
                Agent9527ErrorDetails::UnsupportedOperation(message) => {
    ...
    fn build_thread_config_overrides(
        ...
        approval_policy: Option<agent9527_app_server_protocol::AskForApproval>,
        approvals_reviewer: Option<agent9527_app_server_protocol::ApprovalsReviewer>,
        ...
        ConfigOverrides {
            ...
            approval_policy: approval_policy
                .map(agent9527_app_server_protocol::AskForApproval::to_core),
            approvals_reviewer: approvals_reviewer
                .map(agent9527_app_server_protocol::ApprovalsReviewer::to_core),
            sandbox_mode: sandbox.map(SandboxMode::to_core),
            agent9527_linux_sandbox_exe: self.arg0_paths.agent9527_linux_sandbox_exe.clone(),
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = agent9527_core::util::normalize_thread_name(&name) else {
    ...
    async fn memory_reset_response_inner(&self) -> Result<MemoryResetResponse, JSONRPCErrorError> {
        ...
        clear_memory_roots_contents(&self.config.agent9527_home)
            .await
            .map_err(|err| {
                internal_error(format!(
                    "failed to clear memory directories under {}: {err}",
                    self.config.agent9527_home.display()
                ))
            })?;
    ...
    ) -> Result<agent9527_app_server_protocol::TurnsPage, JSONRPCErrorError> {
    ...
    async fn thread_resume_inner(
            ...
            .await
        {
            Ok(NewThread {
                thread_id,
                thread: agent9527_thread,
                session_configured,
                ..
            }) => {
                if let Err(err) = Self::set_app_server_client_info(
                    agent9527_thread.as_ref(),
                    app_server_client_name,
                    app_server_client_version,
                )
                .await
                ...
                let instruction_sources = agent9527_thread.legacy_instruction_sources().await;
                ...
                thread.thread_source = agent9527_thread
                    .config_snapshot()
                    .await
                    .thread_source
                    .map(Into::into);
                ...
                let config_snapshot = agent9527_thread.config_snapshot().await;
                ...
                self.thread_goal_processor
                    .emit_resume_goal_snapshot_and_continue(thread_id, agent9527_thread.as_ref())
                    .await;
            }
            Err(err) => {
                let error = match err.details() {
                    Agent9527ErrorDetails::InvalidRequest(message) => {
    ...
    async fn thread_fork_inner(
        ...
        let source_thread_name = source_thread
            .name
            .as_deref()
            .and_then(agent9527_core::util::normalize_thread_name);
        let prepared_fork = if paginated_source {
            let boundary = match (last_turn_id.as_deref(), before_turn_id.as_deref()) {
                (Some(turn_id), None) => {
                    agent9527_thread_store::ForkBoundary::ThroughTurn(turn_id.to_string())
                }
                (None, Some(turn_id)) => {
                    agent9527_thread_store::ForkBoundary::BeforeTurn(turn_id.to_string())
                }
                (None, None) => agent9527_thread_store::ForkBoundary::Latest,
                (Some(_), Some(_)) => unreachable!("fork boundaries are mutually exclusive"),
            };
            Some(
                self.thread_store
                    .prepare_fork(agent9527_thread_store::PrepareForkParams {
        ...
        let NewThread {
            ...
            Agent9527ErrorDetails::Io(_) | Agent9527ErrorDetails::Json(_) => {
...
fn deserialize_stored_thread_item(
    item: agent9527_thread_store::StoredThreadItem,
...
fn stored_turn_to_api_turn(
    ...
    let error = turn.error.map(|error| TurnError {
        ...
        agent9527_error_info: error.agent9527_error_info,
...
pub(super) fn core_thread_write_error(operation: &str, err: Agent9527Err) -> JSONRPCErrorError {
    match err.details() {
        Agent9527ErrorDetails::ThreadNotFound(thread_id) => {
            invalid_request(format!("thread not found: {thread_id}"))
        }
        Agent9527ErrorDetails::InvalidRequest(message) => invalid_request(message.clone()),
        Agent9527ErrorDetails::UnsupportedOperation(message) => method_not_found(message.clone()),
...
) -> (Thread, Option<agent9527_thread_store::StoredThreadHistory>) {
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<agent9527_protocol::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(agent9527_protocol::protocol::SessionSource::Unknown);
...
fn preview_from_rollout_items(items: &[RolloutItem]) -> String {
    items
        ...
        .find_map(|item| match item {
            RolloutItem::ResponseItem(item) => match agent9527_core::parse_turn_item(item) {
                Some(agent9527_protocol::items::TurnItem::UserMessage(user)) => {
                    Some(user.message())
                }
                _ => None,
            },
...
fn requested_permissions_trust_project(overrides: &ConfigOverrides, cwd: &Path) -> bool {
    if matches!(
        ...
        Some(
            agent9527_protocol::config_types::SandboxMode::WorkspaceWrite
                | agent9527_protocol::config_types::SandboxMode::DangerFullAccess
        )
    ) {
        return true;
    }
...
fn permission_profile_trusts_project(
    profile: &agent9527_protocol::models::PermissionProfile,
    cwd: &Path,
) -> bool {
    match profile {
        agent9527_protocol::models::PermissionProfile::Disabled
        | agent9527_protocol::models::PermissionProfile::External { .. } => true,
        agent9527_protocol::models::PermissionProfile::Managed { .. } => profile
...
fn build_thread_from_snapshot(
    thread_id: ThreadId,
    session_id: String,
    multi_agent_version: Option<agent9527_protocol::protocol::MultiAgentVersion>,
    config_snapshot: &ThreadConfigSnapshot,
    path: Option<PathBuf>,
) -> Thread {

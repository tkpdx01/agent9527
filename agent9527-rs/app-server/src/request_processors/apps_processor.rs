...
impl AppsRequestProcessor {
    ...
    async fn apps_list_task(
        ...
        let should_retry = result
            .as_ref()
            .is_ok_and(|(_, agent9527_apps_ready)| !agent9527_apps_ready);
        outgoing
        ...
        if should_retry && !retry_params.force_refetch {
            ...
            .await
            {
                warn!("failed to refresh app list after agent9527-apps readiness retry: {err:?}");
            }
        }
    }
    ...
    async fn apps_list_response(
        ...
        let mut agent9527_apps_ready = true;
        ...
        loop {
            ...
            match result {
                AppListLoadResult::Accessible(Ok(status)) => {
                    ...
                    agent9527_apps_ready = status.agent9527_apps_ready;
                }
            ...
            if accessible_loaded && all_loaded {
                ...
                return Ok((response, agent9527_apps_ready));
            }
        }
    }
...
const APPS_INSTALLED_DURATION_METRIC: &str = "agent9527.apps.installed.duration_ms";

...
fn extract_conversation_summary(
    ...
    let preview = head
        ...
        .find_map(|item| match agent9527_core::parse_turn_item(&item) {
...
pub(super) fn with_thread_spawn_agent_metadata(
    source: agent9527_protocol::protocol::SessionSource,
    agent_nickname: Option<String>,
    agent_role: Option<String>,
) -> agent9527_protocol::protocol::SessionSource {
    if agent_nickname.is_none() && agent_role.is_none() {
        return source;
    }

    match source {
        agent9527_protocol::protocol::SessionSource::SubAgent(
            agent9527_protocol::protocol::SubAgentSource::ThreadSpawn {
                parent_thread_id,
                depth,
                agent_path,
...
) -> Option<agent9527_app_server_protocol::ActivePermissionProfile> {
...
pub(crate) fn thread_settings_from_core_snapshot(
    snapshot: agent9527_protocol::protocol::ThreadSettingsSnapshot,
) -> ThreadSettings {
    let agent9527_protocol::protocol::ThreadSettingsSnapshot {
        model,
        model_provider_id,
        service_tier,
        approval_policy,
    ...
    let sandbox_policy = agent9527_sandboxing::compatibility_sandbox_policy_for_permission_profile(
        &permission_profile,
        cwd.as_path(),
    )
    .into();

...
impl PluginRequestProcessor {
    ...
    async fn plugin_list_response(
        ...
        let refresh_global_remote_catalog_cache = use_remote_global_catalog
            && agent9527_core_plugins::remote::has_cached_global_remote_plugin_catalog(
                config.agent9527_home.as_path(),
        ...
        if !remote_sources.is_empty() {
            match agent9527_core_plugins::remote::fetch_remote_marketplaces(
                ...
                auth.as_ref(),
                &remote_sources,
                /*global_catalog_cache_path*/ Some(config.agent9527_home.as_path()),
            )
            .await
            {
    ...
    async fn plugin_install_response(
        ...
        let apps_needing_auth = self
            .plugin_apps_needing_auth_for_install(
                &config,
                auth.as_ref().is_some_and(Agent9527Auth::is_chatgpt_auth),

...
use agent9527_model_provider::is_supported_amazon_bedrock_region;
use chrono::DateTime;
...
const LOGIN_ISSUER_OVERRIDE_ENV_VAR: &str = "AGENT9527_APP_SERVER_LOGIN_ISSUER";
#[cfg(debug_assertions)]
const LOGIN_OPEN_APP_URL_OVERRIDE_ENV_VAR: &str = "AGENT9527_APP_SERVER_DEV_OPEN_APP_URL";
...
impl AccountRequestProcessor {
    ...
    fn current_account_updated_notification(&self) -> AccountUpdatedNotification {
        ...
        AccountUpdatedNotification {
            ...
            plan_type: auth.as_ref().and_then(Agent9527Auth::account_plan_type),
        }
    }
            ...
            LoginAccountParams::Chatgpt {
                ...
                let login_success_page = if use_hosted_login_success_page {
                    let app_brand = match app_brand.unwrap_or_default() {
                        LoginAppBrand::Agent9527 => LoginSuccessPageBrand::Agent9527,
                        LoginAppBrand::Chatgpt => LoginSuccessPageBrand::Chatgpt,
                    };
                    LoginSuccessPage::Hosted {
                        url: AGENT9527_OPEN_APP_URL.parse().map_err(|err| {
                            internal_error(format!("invalid Agent9527 open app URL: {err}"))
                        })?,
                        app_brand,
                    }
                } else {
                ...
                self.login_chatgpt_v2(request_id, agent9527_streamlined_login, login_success_page)
                    .await;
            }
    ...
    async fn login_chatgpt_common(
        &self,
        agent9527_streamlined_login: bool,
    ...
    async fn login_chatgpt_v2(
        ...
        let result = self
            .login_chatgpt_response(agent9527_streamlined_login, login_success_page)
            .await;
        self.outgoing.send_result(request_id, result).await;
    }
    ...
    async fn login_chatgpt_response(
        ...
        let opts = self
            .login_chatgpt_common(agent9527_streamlined_login, login_success_page)
            .await?;
    ...
    async fn logout_common(&self) -> std::result::Result<Option<AuthMode>, JSONRPCErrorError> {
        let managed_bedrock_auth = matches!(
            self.auth_manager.auth_cached(),
            Some(Agent9527Auth::BedrockApiKey(_))
        );
    ...
    async fn get_account_rate_limits_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "agent9527 account authentication required to read rate limits",
            ));
        };
        ...
        let response = response.map_err(|err| {
            internal_error(format!("failed to fetch agent9527 rate limits: {err}"))
        })?;
        if response.rate_limits.is_empty() {
            return Err(internal_error(
                "failed to fetch agent9527 rate limits: no snapshots returned",
            ));
        }
        ...
        let rate_limits_by_limit_id: HashMap<_, _> = response
            ...
            .map(|snapshot| {
                let limit_id = snapshot
                    .limit_id
                    .clone()
                    .unwrap_or_else(|| "agent9527".to_string());
                (limit_id, snapshot)
            })
            .collect();
    ...
    async fn get_account_token_usage_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "agent9527 account authentication required to read token usage",
            ));
        };
    ...
    async fn get_workspace_messages_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "agent9527 account authentication required to read workspace messages",
            ));
        };
    ...
    async fn send_add_credits_nudge_email_inner(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "agent9527 account authentication required to notify workspace owner",
            ));
        };
...
mod tests {
    use super::*;
    use agent9527_backend_client::TokenUsageProfileDailyBucket;
    use agent9527_backend_client::TokenUsageProfileStats;
    ...
    fn workspace_messages_feature_disabled_only_for_not_found() {
        ...
        for (status, expected) in cases {
            let err = BackendRequestError::UnexpectedStatus {
                ...
                url: "https://example.test/api/agent9527/workspace-messages".to_string(),
                status,

...
use agent9527_connectors::ConnectorRuntimeTool;
use agent9527_connectors::connector_runtime_context_key;
use agent9527_connectors::connector_tool_is_synthetic;
use agent9527_connectors::installed_connector_runtime;
...
use agent9527_mcp::host_owned_agent9527_apps_enabled;
...
const APPS_INSTALLED_RESPONSE_BYTES_METRIC: &str = "agent9527.apps.installed.response_bytes";
const APPS_INSTALLED_CONNECTOR_COUNT_METRIC: &str = "agent9527.apps.installed.connector_count";
const APPS_INSTALLED_TOOL_COUNT_METRIC: &str = "agent9527.apps.installed.tool_count";
const APPS_SNAPSHOT_AGE_METRIC: &str = "agent9527.apps.snapshot.age_ms";
    ...
    pub(crate) async fn apps_installed(
        ...
        let result = async {
            ...
            let apps_enabled = config
                .features
                .apps_enabled_for_auth(auth.as_ref().is_some_and(Agent9527Auth::uses_agent9527_backend));
            ...
            let previous_snapshot = mcp_manager
                ...
                .current_snapshot(config.agent9527_home.to_path_buf(), cache_key.clone());
            let snapshot = if force_refresh && runtime_enabled {
                let refresh_result = async {
                    anyhow::ensure!(
                        !mcp_servers.is_empty(),
                        "host-owned MCP server '{AGENT9527_APPS_MCP_SERVER_NAME}' is not enabled"
                    );
                    ...
                    let agent9527_apps_auth_manager =
                        host_owned_agent9527_apps_enabled(&mcp_config, auth.as_ref())
                    ...
                    let result = if connection_manager
                        .wait_for_server_ready(AGENT9527_APPS_MCP_SERVER_NAME, startup_timeout)
                        .await
                    {
                        mcp_manager
                            .agent9527_apps_tools_cache()
                        ...
                        Err(anyhow::anyhow!(
                            "failed to refresh tools for MCP server '{AGENT9527_APPS_MCP_SERVER_NAME}'"
                        ))
                    };

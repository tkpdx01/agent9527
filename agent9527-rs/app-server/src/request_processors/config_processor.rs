...
use agent9527_app_server_protocol::BrowserUseRequirements;
use agent9527_app_server_protocol::ClientResponsePayload;
use agent9527_app_server_protocol::ComputerUseRequirements;
use agent9527_app_server_protocol::ConfigBatchWriteParams;
use agent9527_app_server_protocol::ConfigReadParams;
use agent9527_app_server_protocol::ConfigReadResponse;
use agent9527_app_server_protocol::ConfigRequirements;
use agent9527_app_server_protocol::ConfigRequirementsReadResponse;
use agent9527_app_server_protocol::ConfigValueWriteParams;
use agent9527_app_server_protocol::ConfigWriteErrorCode;
use agent9527_app_server_protocol::ConfigWriteResponse;
use agent9527_app_server_protocol::ConfiguredHookHandler;
use agent9527_app_server_protocol::ConfiguredHookMatcherGroup;
use agent9527_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use agent9527_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use agent9527_app_server_protocol::FeedbackRequirements;
use agent9527_app_server_protocol::JSONRPCErrorError;
use agent9527_app_server_protocol::ManagedHooksRequirements;
use agent9527_app_server_protocol::ModelProviderCapabilitiesReadResponse;
use agent9527_app_server_protocol::ModelsRequirements;
use agent9527_app_server_protocol::NetworkDomainPermission;
use agent9527_app_server_protocol::NetworkRequirements;
use agent9527_app_server_protocol::NetworkUnixSocketPermission;
...
use agent9527_config::HookEventsToml;
use agent9527_config::HookHandlerConfig as CoreHookHandlerConfig;
use agent9527_config::ManagedHooksRequirementsToml;
use agent9527_config::MatcherGroup as CoreMatcherGroup;
use agent9527_config::ResidencyRequirement as CoreResidencyRequirement;
use agent9527_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use agent9527_core::ThreadManager;
use agent9527_features::canonical_feature_for_key;
use agent9527_features::feature_for_key;
...
impl ConfigRequestProcessor {
    ...
    ) -> Result<agent9527_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes = agent9527_core_plugins::toggles::collect_plugin_enabled_candidates(
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ...
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(agent9527_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(agent9527_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
                ...
                .map(|implementations| {
                    implementations
                        ...
                        .map(|implementation| match implementation {
                            agent9527_config::types::WindowsSandboxModeToml::Elevated => {
                                WindowsSandboxSetupMode::Elevated
                            }
                            agent9527_config::types::WindowsSandboxModeToml::Unelevated => {
...
fn map_computer_use_requirements_to_api(
    computer_use: agent9527_config::ComputerUseRequirementsToml,
...
fn map_browser_use_requirements_to_api(
    browser_use: agent9527_config::BrowserUseRequirementsToml,
...
) -> agent9527_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => agent9527_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: agent9527_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(agent9527_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(agent9527_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(agent9527_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: agent9527_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        agent9527_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        agent9527_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}

fn map_network_unix_socket_permission_to_api(
    permission: agent9527_config::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        agent9527_config::NetworkUnixSocketPermissionToml::Allow => {
            NetworkUnixSocketPermission::Allow
        }
        agent9527_config::NetworkUnixSocketPermissionToml::Deny => {
...
mod tests {
    ...
    use agent9527_app_server_protocol::FeedbackRequirements;
    use agent9527_app_server_protocol::WindowsSandboxSetupMode;
    use agent9527_config::ComputerUseRequirementsToml;
    use agent9527_config::ConfigRequirementsToml;
    use agent9527_config::ModelsRequirementsToml;
    use agent9527_config::NewThreadModelDefaultsToml;
    use agent9527_config::WindowsRequirementsToml;

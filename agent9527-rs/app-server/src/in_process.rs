...
//!
//! # Relationship to `agent9527-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `agent9527-app-server-client`,
...
use agent9527_app_server_protocol::Result;
...
pub use agent9527_rollout::StateDbHandle;
pub use agent9527_state::log_db::LogDbLayer;
use tokio::sync::mpsc;
...
/// through `agent9527-app-server-client`, which adds worker-task buffering,
...
pub struct InProcessClientHandle {
    ...
    #[cfg(test)]
    _test_agent9527_home: Option<tempfile::TempDir>,
}
...
async fn start_uninitialized(args: InProcessStartArgs) -> IoResult<InProcessClientHandle> {
    ...
    let installation_id = resolve_installation_id(&args.config.agent9527_home).await?;
    ...
    let runtime_handle = tokio::spawn(async move {
        ...
        let auth_manager = AuthManager::shared_from_config(
            args.config.as_ref(),
            args.enable_agent9527_api_key_env,
        )
        .await;
        ...
        let config_manager = ConfigManager::new(
            args.config.agent9527_home.to_path_buf(),
            args.cli_overrides,
...
mod tests {
    ...
    async fn start_test_client_with_capacity(
        ...
        let config = Arc::new(build_test_config(agent9527_home.path()).await);
        let state_db = agent9527_rollout::state_db::try_init(config.as_ref())
            .await
        ...
        let args = InProcessStartArgs {
            ...
            initialize: InitializeParams {
                client_info: ClientInfo {
                    name: "agent9527-in-process-test".to_string(),
                    title: None,
        ...
        client._test_agent9527_home = Some(agent9527_home);
        client
    }

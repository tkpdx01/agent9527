...
use agent9527_utils_path_uri::PathUriParseError;
use anyhow::Context;
use anyhow::Result;
pub use parser::Hunk;
...
/// Special argv[1] flag used when the Agent9527 executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `agent9527-apply-patch` (to avoid forcing
/// `agent9527-arg0` to depend on `agent9527-core`), it remains part of the "agent9527 core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const AGENT9527_CORE_APPLY_PATCH_ARG1: &str = "--agent9527-run-as-apply-patch";

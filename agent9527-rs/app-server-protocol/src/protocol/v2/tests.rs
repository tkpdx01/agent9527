...
use agent9527_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use agent9527_protocol::config_types::MultiAgentMode;
use agent9527_protocol::items::AgentMessageContent;
use agent9527_protocol::items::AgentMessageItem;
use agent9527_protocol::items::CollabAgentTool as CoreCollabAgentTool;
use agent9527_protocol::items::CollabAgentToolCallItem;
use agent9527_protocol::items::CollabAgentToolCallStatus as CoreCollabAgentToolCallStatus;
use agent9527_protocol::items::CommandExecutionItem;
...
use agent9527_protocol::items::FileChangeItem;
use agent9527_protocol::items::ImageViewItem;
use agent9527_protocol::items::McpToolCallItem;
use agent9527_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
use agent9527_protocol::items::ReasoningItem;
...
use agent9527_protocol::items::WebSearchItem as CoreWebSearchItem;
use agent9527_protocol::mcp::CallToolResult;
use agent9527_protocol::mcp::McpServerInfo;
use agent9527_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use agent9527_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
...
use agent9527_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use agent9527_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use agent9527_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use agent9527_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use agent9527_protocol::protocol::AgentStatus as CoreAgentStatus;
use agent9527_protocol::protocol::AskForApproval as CoreAskForApproval;
use agent9527_protocol::protocol::ConversationTextRole;
use agent9527_protocol::protocol::ExecCommandSource as CoreExecCommandSource;
use agent9527_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use agent9527_protocol::protocol::NetworkAccess as CoreNetworkAccess;
use agent9527_protocol::protocol::SubAgentActivityKind as CoreSubAgentActivityKind;
use agent9527_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use agent9527_protocol::user_input::UserInput as CoreUserInput;
...
fn thread_sources_round_trip_as_scalar_labels() {
    for (source, label) in [
        ...
        let core_source: agent9527_protocol::protocol::ThreadSource = source.clone().into();
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        agent9527_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        agent9527_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
fn config_requirements_read_accepts_foreign_path_uris() {
    let response: ConfigRequirementsReadResponse = serde_json::from_value(json!({
        "requirements": {
            "sqliteHome": "file:///C:/Users/alice/.agent9527/state",
            "logDir": "file:///C:/Users/alice/.agent9527/logs",
            "modelCatalogJson": "file:///C:/Users/alice/.agent9527/models.json"
        }
    }))
    ...
    assert_eq!(
        ...
        Some(PathUri::parse("file:///C:/Users/alice/.agent9527/state").expect("valid URI"))
    );
    assert_eq!(
        requirements.log_dir,
        Some(PathUri::parse("file:///C:/Users/alice/.agent9527/logs").expect("valid URI"))
    );
    assert_eq!(
        requirements.model_catalog_json,
        Some(PathUri::parse("file:///C:/Users/alice/.agent9527/models.json").expect("valid URI"))
    );
}
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        agent9527_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    let user_item = TurnItem::UserMessage(UserMessageItem {
        ...
        content: vec![
            ...
            CoreUserInput::Skill {
                ...
                path: PathBuf::from("/repo/.agent9527/skills/skill-creator/SKILL.md"),
            },
    ...
    let command_item = TurnItem::CommandExecution(CommandExecutionItem {
        ...
        parsed_cmd: vec![agent9527_protocol::parse_command::ParsedCommand::Unknown {
    ...
    let sender_thread_id = agent9527_protocol::ThreadId::default();
    let receiver_thread_id = agent9527_protocol::ThreadId::default();
    ...
    let sub_agent_activity_item = TurnItem::SubAgentActivity(SubAgentActivityItem {
        ...
        agent_path: agent9527_protocol::AgentPath::root()
            .join("worker")
            .expect("worker path"),
    });
    ...
    assert_eq!(
        ThreadItem::from(TurnItem::Extension(
            agent9527_extension_items::ExtensionItem::WebSearch(expected_search_item.clone()),
        )),
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(agent9527_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
...
fn mcp_tool_call_app_context_serializes_connector_id() {
    let item = ThreadItem::McpToolCall {
        id: "mcp-1".to_string(),
        server: "agent9527_apps".to_string(),
...
fn plugin_share_params_and_response_serialization_use_camel_case_fields() {
    ...
    assert_eq!(
        serde_json::to_value(PluginShareCheckoutResponse {
            remote_plugin_id: "plugins~Plugin_00000000000000000000000000000000".to_string(),
            plugin_id: "gmail@agent9527-curated".to_string(),
            plugin_name: "gmail".to_string(),
            plugin_path,
            marketplace_name: "agent9527-curated".to_string(),
            marketplace_path,
        ...
        json!({
            "remotePluginId": "plugins~Plugin_00000000000000000000000000000000",
            "pluginId": "gmail@agent9527-curated",
            "pluginName": "gmail",
            "pluginPath": plugin_path_json,
            "marketplaceName": "agent9527-curated",
            "marketplacePath": marketplace_path_json,
            "remoteVersion": "1.2.3",
        }),
    );
...
#[test]
fn agent9527_error_info_serializes_http_status_code_in_camel_case() {
    let value = Agent9527ErrorInfo::ResponseTooManyFailedAttempts {
...
#[test]
fn agent9527_error_info_serializes_cyber_policy_in_camel_case() {
    assert_eq!(
        serde_json::to_value(Agent9527ErrorInfo::CyberPolicy).unwrap(),
        json!("cyberPolicy")
    );
}

#[test]
fn agent9527_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
    let value = Agent9527ErrorInfo::ActiveTurnNotSteerable {
...
fn turn_start_params_round_trip_multi_agent_mode() {
    ...
    assert_eq!(
        ...
        Some(agent9527_protocol::config_types::MultiAgentMode::Proactive)
    );
    assert_eq!(
...
fn thread_settings_update_params_preserve_field_level_experimental_gates() {
    ...
    let collaboration_mode = ThreadSettingsUpdateParams {
        ...
        collaboration_mode: Some(agent9527_protocol::config_types::CollaborationMode {
            mode: agent9527_protocol::config_types::ModeKind::Plan,
...
fn realtime_start_deserializes_client_handoff_channel_prefixes() {
    let params = serde_json::from_value::<ThreadRealtimeStartParams>(json!({
        "threadId": "thread_123",
        "outputModality": "audio",
        "agent9527ResponseHandoffChannelPrefixes": {
            "analysis": ["[THINKING]"],
    ...
    assert_eq!(
        params.agent9527_response_handoff_channel_prefixes,
        Some(BTreeMap::from([

...
use agent9527_protocol::protocol::RealtimeAudioFrame as CoreRealtimeAudioFrame;
...
pub struct ThreadRealtimeStartParams {
    ...
    /// Leaves Agent9527 response handoffs to the client's explicit append calls instead of forwarding
    /// them automatically. Defaults to false.
    #[ts(optional = nullable)]
    pub client_managed_handoffs: Option<bool>,
    /// Routes any transcript tail remaining at session end through Agent9527. Defaults to false.
    ...
    #[ts(optional = nullable)]
    pub agent9527_responses_as_items: Option<bool>,
    // TODO: Remove this experiment-only prefix with `agent9527_responses_as_items`.
    /// Optional prefix added to automatic Agent9527 response items when `agent9527ResponsesAsItems` is true.
    #[ts(optional = nullable)]
    pub agent9527_response_item_prefix: Option<String>,
    /// Selects how automatic Agent9527 responses are routed in Frameless Bidi sessions. Omitted values
    /// default to `thinking`. Realtime V1 and V2 ignore this setting.
    #[ts(optional = nullable)]
    pub agent9527_response_handoff_mode: Option<Agent9527ResponseHandoffMode>,
    ...
    /// Set to false to start without Agent9527's startup context. Omitted or null includes it.
    #[ts(optional = nullable)]

...
use agent9527_utils_template::Template;
use base64::Engine;
use chrono::Utc;
use rand::RngCore;
...
// Keep in sync with the Agent9527 CLI Hydra redirect URI allow-list.
...
pub struct ServerOptions {
    ...
    pub agent9527_streamlined_login: bool,
    ...
    match path.as_str() {
            ...
            .await
            {
                Ok(tokens) => {
                    ...
                    let redirect = compose_success_url(
                        ...
                        &tokens.id_token,
                        &tokens.access_token,
                        opts.agent9527_streamlined_login,
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to Agent9527 failed.",
                            io::ErrorKind::Other,
        ...
        "/success" => {
            let use_streamlined_success = parsed_url
                .query_pairs()
                .any(|(key, value)| key == "agent9527_streamlined_login" && value == "true");
...
fn build_authorize_url(
    ...
    let mut query = vec![
        ...
        (
            "agent9527_cli_simplified_flow".to_string(),
            "true".to_string(),
        ),
...
fn redact_sensitive_error_url(
    mut err: agent9527_http_client::HttpError,
) -> agent9527_http_client::HttpError {
...
pub(crate) async fn persist_tokens_async(
    ...
    let agent9527_home = agent9527_home.to_path_buf();
...
/// Returns true when the OAuth callback represents a missing Agent9527 entitlement.
fn is_missing_agent9527_entitlement_error(
    error_code: &str,
    error_description: Option<&str>,
) -> bool {
    error_code == "access_denied"
        && error_description.is_some_and(|description| {
            description
                .to_ascii_lowercase()
                .contains("missing_agent9527_entitlement")
        })
}
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_agent9527_entitlement_error(error_code, error_description) {
        return "Agent9527 is not enabled for your workspace. Contact your workspace administrator to request access to Agent9527.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_agent9527_entitlement_error(code, error_description) {
            (
                "You do not have access to Agent9527".to_string(),
                "This account is not currently authorized to use Agent9527 in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to Agent9527.".to_string(),
                "Contact your workspace administrator to get access to Agent9527, then return to Agent9527 and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to Agent9527 to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
...
mod tests {
    ...
    use super::is_missing_agent9527_entitlement_error;
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        let error_description = Some("missing_agent9527_entitlement");
        assert!(is_missing_agent9527_entitlement_error(
            "access_denied",
            error_description
        ));
        ...
        assert!(body.contains("You do not have access to Agent9527"));
        assert!(body.contains("Contact your workspace administrator"));
        assert!(!body.contains("missing_agent9527_entitlement"));
    }
}

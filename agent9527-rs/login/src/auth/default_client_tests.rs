...
#[test]
fn test_get_agent9527_user_agent() {
    let user_agent = get_agent9527_user_agent();
...
fn is_first_party_originator_matches_known_values() {
    ...
    assert_eq!(is_first_party_originator("agent9527-tui"), true);
    assert_eq!(is_first_party_originator("agent9527_vscode"), true);
    assert_eq!(is_first_party_originator("Agent9527 Something Else"), true);
    assert_eq!(is_first_party_originator("agent9527_cli"), false);
    assert_eq!(is_first_party_originator("Other"), false);
}

#[test]
fn is_first_party_chat_originator_matches_known_values() {
    assert_eq!(is_first_party_chat_originator("agent9527_atlas"), true);
    assert_eq!(
        is_first_party_chat_originator("agent9527_chatgpt_desktop"),
        true
    );
    assert_eq!(is_first_party_chat_originator(DEFAULT_ORIGINATOR), false);
    assert_eq!(is_first_party_chat_originator("agent9527_vscode"), false);
}
...
async fn test_create_client_sets_default_headers() {
    ...
    // User-Agent matches the computed Agent9527 UA for that originator
    let expected_ua = get_agent9527_user_agent();
...
fn test_invalid_suffix_is_sanitized() {
    let prefix = "agent9527_cli_rs/0.0.0";
    let suffix = "bad\rsuffix";

    assert_eq!(
        sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
        "agent9527_cli_rs/0.0.0 (bad_suffix)"
    );
}

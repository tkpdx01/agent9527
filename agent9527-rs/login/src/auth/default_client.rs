//! Default Agent9527 HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! reqwest/`HttpClient` construction.
//!
//! Use [`crate::default_client`] or [`agent9527_login::default_client`] from other crates in this
//! workspace.

use agent9527_http_client::BuildCustomCaTransportError;
use agent9527_http_client::BuildRouteAwareHttpClientError;
...
/// See https://github.com/openai/agent9527/pull/3388/files for an example of what that would look like.
...
/// Parenthesis will be added by Agent9527. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "agent9527_cli_rs";
pub const AGENT9527_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str =
    "AGENT9527_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-openai-internal-agent9527-residency";

pub use agent9527_config::ResidencyRequirement;
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(AGENT9527_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(AGENT9527_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "agent9527-tui"
        || originator_value == "agent9527_vscode"
        || originator_value.starts_with("Agent9527 ")
}

pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "agent9527_atlas" || originator_value == "agent9527_chatgpt_desktop"
}

pub fn get_agent9527_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Agent9527 user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Agent9527 user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Agent9527 originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
/// Agent9527's route-aware system/PAC resolution.
pub fn create_client() -> HttpClient {
    let inner = build_reqwest_client();
    HttpClient::new(inner)
}

/// Builds the default reqwest client used for ordinary Agent9527 HTTP traffic.
///
/// This starts from the standard Agent9527 user agent, default headers, and sandbox-specific proxy
/// policy, then layers in shared custom CA handling from `AGENT9527_CA_CERTIFICATE` /
...
///
/// This supported default path preserves reqwest's existing proxy behavior and does not opt into
/// Agent9527's route-aware system/PAC resolution. Auth callers with route settings must use
...
/// Tries to build the default reqwest client used for ordinary Agent9527 HTTP traffic.
///
...
/// Builds the default Agent9527 reqwest client for a concrete outbound route.
///
/// When route-aware proxy handling is disabled, or the client is running inside the Agent9527
...
/// Builds the default Agent9527 reqwest client for a concrete outbound route without blocking the
...
/// Builds an HTTP client for an auth endpoint without Agent9527 default headers.
...
/// Builds the default Agent9527 reqwest client for an auth endpoint.
...
/// Builds the default Agent9527 HTTP client wrapper for an auth endpoint.
...
pub fn default_headers() -> HeaderMap {
    ...
    if let Ok(user_agent) = HeaderValue::from_str(&get_agent9527_user_agent()) {
...
fn is_sandboxed() -> bool {
    std::env::var("AGENT9527_SANDBOX").as_deref() == Ok("seatbelt")
}

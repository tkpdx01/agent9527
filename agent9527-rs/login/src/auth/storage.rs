...
use agent9527_agent_identity::AgentIdentityJwtClaims;
use agent9527_agent_identity::decode_agent_identity_jwt;
use agent9527_config::types::AuthCredentialsStoreMode;
pub use agent9527_config::types::AuthKeyringBackendKind;
use agent9527_keyring_store::DefaultKeyringStore;
use agent9527_keyring_store::KeyringStore;
use agent9527_protocol::account::PlanType as AccountPlanType;
use agent9527_protocol::auth::AuthMode;
use agent9527_secrets::LocalSecretsNamespace;
use agent9527_secrets::SecretName;
use agent9527_secrets::SecretScope;
use agent9527_secrets::SecretsBackendKind;
use agent9527_secrets::SecretsManager;
use once_cell::sync::Lazy;

/// Expected structure for $AGENT9527_HOME/auth.json.
...
pub(super) fn get_auth_file(agent9527_home: &Path) -> PathBuf {
    agent9527_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(agent9527_home: &Path) -> std::io::Result<bool> {
    let auth_file = get_auth_file(agent9527_home);
...
impl FileAuthStorage {
    pub(super) fn new(agent9527_home: PathBuf) -> Self {
        Self { agent9527_home }
    }

    /// Attempt to read and parse the `auth.json` file in the given `AGENT9527_HOME` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.agent9527_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.agent9527_home)
    }
}

static AGENT9527_AUTH_SECRET_NAME: Lazy<SecretName> =
    Lazy::new(|| match SecretName::new("AGENT9527_AUTH") {
        Ok(name) => name,
        Err(err) => unreachable!("AGENT9527_AUTH should be a valid secret name: {err}"),
    });
const KEYRING_SERVICE: &str = "Agent9527 Auth";

// turns agent9527_home path into a stable, short key string
fn compute_store_key(agent9527_home: &Path) -> std::io::Result<String> {
    let canonical = agent9527_home
        .canonicalize()
        .unwrap_or_else(|_| agent9527_home.to_path_buf());
...
impl DirectKeyringAuthStorage {
    fn new(agent9527_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            agent9527_home,
            keyring_store,
        }
...
impl AuthStorageBackend for DirectKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.agent9527_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.agent9527_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.agent9527_home)?;
...
impl SecretsKeyringAuthStorage {
    fn new(agent9527_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        let direct_storage =
            DirectKeyringAuthStorage::new(agent9527_home.clone(), Arc::clone(&keyring_store));
        let secrets_manager = SecretsManager::new_with_keyring_store_and_namespace(
            agent9527_home.clone(),
            SecretsBackendKind::Local,
            keyring_store,
            LocalSecretsNamespace::Agent9527Auth,
        );
        Self {
            agent9527_home,
            direct_storage,
...
impl AuthStorageBackend for SecretsKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        match self
            .secrets_manager
            .get(&SecretScope::Global, &AGENT9527_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
    ...
    fn delete(&self) -> std::io::Result<bool> {
        let keyring_removed = self
            .secrets_manager
            .delete(&SecretScope::Global, &AGENT9527_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
...
impl AutoAuthStorage {
    fn new(
        ...
        Self {
            ...
            file_storage: Arc::new(FileAuthStorage::new(agent9527_home)),
        }
    }
}
...
// A global in-memory store for mapping agent9527_home -> AuthDotJson.
...
impl EphemeralAuthStorage {
    fn new(agent9527_home: PathBuf) -> Self {
        Self { agent9527_home }
    }
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_store(agent9527_home, mode, keyring_store, keyring_backend_kind)
}
...
fn create_auth_storage_with_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(agent9527_home)),
        AuthCredentialsStoreMode::Keyring => {
            create_keyring_auth_storage(agent9527_home, keyring_store, keyring_backend_kind)
        }
        ...
        AuthCredentialsStoreMode::Ephemeral => Arc::new(EphemeralAuthStorage::new(agent9527_home)),
    }
}
...
fn create_keyring_auth_storage(
    ...
    match keyring_backend_kind {
        AuthKeyringBackendKind::Direct => {
            Arc::new(DirectKeyringAuthStorage::new(agent9527_home, keyring_store))
        }

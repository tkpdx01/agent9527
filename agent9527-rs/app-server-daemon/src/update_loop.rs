...
#[cfg(unix)]
use crate::managed_install::resolved_managed_agent9527_bin;
...
#[cfg(unix)]
const INSTALL_URL: &str = "https://chatgpt.com/agent9527/install.sh";
...
async fn update_once(
    ...
    let managed_agent9527_bin =
        resolved_managed_agent9527_bin(&daemon.managed_agent9527_bin).await?;
    let managed_identity = executable_identity(&managed_agent9527_bin).await?;
    ...
    loop {
        ...
        match daemon
            .try_restart_if_running(restart_mode, updater_refresh_mode, &managed_agent9527_bin)
            .await?
        {
...
#[cfg(unix)]
pub(crate) fn reexec_managed_updater(managed_agent9527_bin: &std::path::Path) -> Result<()> {
    let err = StdCommand::new(managed_agent9527_bin)
        .args(["app-server", "daemon", "pid-update-loop"])
        .exec();
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed Agent9527 binary {}",
            managed_agent9527_bin.display()
        )
    })
}
...
async fn install_latest_standalone(http: &RouteAwareClientPool) -> Result<()> {
    ...
    let mut child = Command::new("/bin/sh")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone Agent9527 updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone Agent9527 updater stdin was unavailable")?;
    stdin
        .write_all(&script)
        .await
        .context("failed to pass standalone Agent9527 updater to shell")?;
    drop(stdin);
    let status = child
        .wait()
        .await
        .context("failed to wait for standalone Agent9527 updater")?;

    if status.success() {
        Ok(())
    } else {
        anyhow::bail!("standalone Agent9527 updater exited with status {status}")
    }
}
...
async fn fetch_installer_script(http: &impl InstallerHttp) -> Result<Vec<u8>> {
    match http.get(INSTALL_URL).await? {
        ...
        InstallerResponse::Unsuccessful { status } => {
            anyhow::bail!("standalone Agent9527 updater request failed with status {status}")
        }
    }
}
...
impl InstallerHttp for RouteAwareClientPool {
    async fn get(&self, url: &str) -> Result<InstallerResponse> {
        let response = RouteAwareClientPool::get(self, url)
            .send()
            .await
            .context("failed to fetch standalone Agent9527 updater")?;
        ...
        let body = response
            .bytes()
            .await
            .context("failed to read standalone Agent9527 updater")?
            .to_vec();

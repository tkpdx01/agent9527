...
use agent9527_app_server_protocol::RemoteControlConnectionStatus;
use agent9527_app_server_protocol::RemoteControlPairingStartResponse;
use agent9527_app_server_transport::app_server_control_socket_path;
...
use managed_install::managed_agent9527_bin;
#[cfg(unix)]
use managed_install::managed_agent9527_version;
use serde::Serialize;
...
pub struct LifecycleOutput {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub pid: Option<u32>,
    pub managed_agent9527_path: PathBuf,
    pub managed_agent9527_version: Option<String>,
...
fn ensure_supported_platform() -> Result<()> {
    Err(anyhow!(
        "agent9527 app-server daemon lifecycle is only supported on Unix platforms"
    ))
}
...
struct Daemon {
    ...
    managed_agent9527_bin: PathBuf,
}

impl Daemon {
    fn from_environment() -> Result<Self> {
        let agent9527_home = find_agent9527_home().context("failed to resolve AGENT9527_HOME")?;
        let socket_path = app_server_control_socket_path(agent9527_home.as_path())?
            .as_path()
            .to_path_buf();
        let state_dir = agent9527_home.as_path().join(STATE_DIR_NAME);
        Ok(Self {
            socket_path,
            ...
            managed_agent9527_bin: managed_agent9527_bin(agent9527_home.as_path()),
        })
    }
    ...
    async fn start(&self) -> Result<LifecycleOutput> {
        ...
        self.ensure_managed_agent9527_bin()?;
    ...
    async fn restart(&self) -> Result<LifecycleOutput> {
        ...
        {
            return Err(anyhow!(
                "app server is running but is not managed by agent9527 app-server daemon"
            ));
        }
    ...
    pub(crate) async fn try_restart_if_running(
        ...
        let outcome = if let Some(backend) = self.running_backend_instance(&settings).await? {
            ...
            let managed_version = if info.is_some() {
                Some(managed_agent9527_version(managed_agent9527_bin).await?)
            } else {
                None
            };
            match restart_decision(mode, info.as_ref(), managed_version.as_deref()) {
                ...
                RestartDecision::Restart => {
                    backend.stop().await?;
                    let _ = self
                        .start_managed_backend_with_bin(&settings, managed_agent9527_bin)
                        .await?;
        ...
        if should_reexec_updater(updater_refresh_mode, outcome) {
            crate::update_loop::reexec_managed_updater(managed_agent9527_bin)?;
        }
    ...
    async fn append_daemon_app_server_context(&self, context: &mut String) {
        let managed_agent9527_version = self
            .managed_agent9527_version_best_effort()
            .await
            .unwrap_or_else(|| "unknown".to_string());
        context.push_str(&format!(
            "\n\nDaemon used app-server:\n  path: {}\n  version: {managed_agent9527_version}",
            self.managed_agent9527_bin.display()
        ));
    }
    ...
    async fn bootstrap_locked(&self, options: BootstrapOptions) -> Result<BootstrapOutput> {
        ...
        let info = self.wait_until_ready().await?;
        let managed_agent9527_version = self.managed_agent9527_version_best_effort().await;
        Ok(BootstrapOutput {
            status: BootstrapStatus::Bootstrapped,
            backend: BackendKind::Pid,
            auto_update_enabled: true,
            remote_control_enabled: settings.remote_control_enabled,
            managed_agent9527_path: self.managed_agent9527_bin.clone(),
            managed_agent9527_version,
    ...
    async fn start_managed_backend(&self, settings: &DaemonSettings) -> Result<Option<u32>> {
        self.start_managed_backend_with_bin(settings, &self.managed_agent9527_bin)
            .await
    }
    ...
    async fn start_managed_backend_with_bin(
        ...
        let backend =
            backend::pid_backend(self.backend_paths_with_bin(settings, managed_agent9527_bin));
        backend.start().await
    }
    ...
    fn ensure_managed_agent9527_bin(&self) -> Result<()> {
        if self.managed_agent9527_bin.is_file() {
            return Ok(());
        }

        let managed_agent9527_path = self.managed_agent9527_bin.display();
        Err(anyhow!(
            "managed standalone Agent9527 install not found at {managed_agent9527_path}\n\n\
             This command requires the standalone install managed by the Agent9527 installer, because \
             the daemon starts and updates app-server from that fixed path.\n\n\
             Install it with:\n  curl -fsSL https://chatgpt.com/agent9527/install.sh | sh\n\n\
             Then rerun the command you just tried."
        ))
    }

    #[cfg(unix)]
    async fn managed_agent9527_version_best_effort(&self) -> Option<String> {
        managed_agent9527_version(&self.managed_agent9527_bin)
            .await
            .ok()
    }
    ...
    fn backend_paths(&self, settings: &DaemonSettings) -> BackendPaths {
        self.backend_paths_with_bin(settings, &self.managed_agent9527_bin)
    }
    ...
    fn backend_paths_with_bin(
        ...
        BackendPaths {
            agent9527_bin: managed_agent9527_bin.to_path_buf(),
...
mod tests {
    ...
    fn remote_control_start_output_serializes_inner_output_without_tag() {
        let lifecycle_output = LifecycleOutput {
            ...
            pid: None,
            managed_agent9527_path: "agent9527".into(),
            managed_agent9527_version: Some("1.2.3".to_string()),
            socket_path: "agent9527.sock".into(),
        ...
        assert_eq!(
            ...
            serde_json::json!({
                ...
                "backend": "pid",
                "managedAgent9527Path": "agent9527",
                "managedAgent9527Version": "1.2.3",
                "socketPath": "agent9527.sock",
                "cliVersion": "1.2.3",
    ...
    async fn not_ready_context_reports_daemon_app_server_before_stderr() {
        ...
        let daemon = Daemon {
            ...
            managed_agent9527_bin: temp_dir.path().join("missing-agent9527"),
        };
        ...
        assert_eq!(
            ...
            format!(
                ...
                daemon.managed_agent9527_bin.display(),
                stderr_log.display()
            )
        );
    }

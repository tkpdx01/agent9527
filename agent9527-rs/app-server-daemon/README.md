# agent9527-app-server-daemon

> `agent9527-app-server-daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`agent9527-app-server-daemon` backs the machine-readable `agent9527 app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for Agent9527 instances launched over SSH, including fresh developer
...
```sh
agent9527 app-server daemon start
agent9527 app-server daemon restart
agent9527 app-server daemon enable-remote-control
agent9527 app-server daemon disable-remote-control
agent9527 app-server daemon stop
agent9527 app-server daemon version
agent9527 app-server daemon bootstrap --remote-control
```
...
$HOME/.agent9527/packages/standalone/current/agent9527 app-server daemon bootstrap --remote-control
```

`bootstrap` requires the standalone managed install. It records the daemon
settings under `AGENT9527_HOME/app-server-daemon/`, starts app-server as a
pidfile-backed detached process, and launches a detached updater loop.

## Installation and update cases

The daemon assumes Agent9527 is installed through `install.sh` and always launches
the standalone managed binary under `AGENT9527_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `AGENT9527_HOME/packages/standalone/current/agent9527` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `AGENT9527_HOME/packages/standalone/current/agent9527` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Top-level `agent9527 remote-control` bootstraps with `--remote-control` when the
...
All mutating lifecycle commands are serialized per `AGENT9527_HOME`, so a concurrent
...
The daemon stores its local state under `AGENT9527_HOME/app-server-daemon/`:

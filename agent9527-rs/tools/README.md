...
`agent9527-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `agent9527-core`.
...
compatibility-sensitive orchestration in `agent9527-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `agent9527-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `agent9527-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `agent9527-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `agent9527-core`.
3. Leave compatibility-sensitive adapters in `agent9527-core` while downstream
...
`agent9527-core`, that is a sign to revisit the extraction boundary before adding
more here.

...
use agent9527_git_utils::diff_since_latest_init;
use agent9527_git_utils::reset_git_repository;
use agent9527_login::Agent9527Auth;
use agent9527_login::AuthManager;
use agent9527_model_provider::ModelProvider;
use agent9527_model_provider::ModelProviderFuture;
use agent9527_model_provider::ProviderAccountResult;
...
use agent9527_state::Phase2JobClaimOutcome;
...
async fn memories_startup_phase2_tracks_workspace_diff_across_runs() -> anyhow::Result<()> {
    ...
    let test = build_test_agent9527(&server, home.clone()).await?;
    trigger_memories_startup(&test).await;
...
async fn phase2_retries_when_clean_workspace_is_missing_artifacts() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        "# Memory Workspace Diff\n\nGenerated by Agent9527 before Phase 2 memory consolidation. Read this file first and do not edit it.\n\n## Status\n- none\n"
    );
...
async fn run_memory_phase_one_model_request_test(
    ...
    let test =
        build_test_agent9527_with_memories_config(server, Arc::clone(&home), memories).await?;
...
async fn run_memory_phase_two_model_request_test(
    ...
    let test = build_test_agent9527_with_memories_config(server, home.clone(), memories).await?;
    let provider = Arc::new(MockMemoryModelProvider::new(
        test.config.model_provider.clone(),
        Some(test.thread_manager.auth_manager()),
    ));
...
async fn build_test_agent9527(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
) -> anyhow::Result<TestAgent9527> {
    build_test_agent9527_with_memories_config(server, home, startup_test_memories_config()).await
}

async fn build_test_agent9527_with_memories_config(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
    memories: MemoriesConfig,
) -> anyhow::Result<TestAgent9527> {
...
async fn init_state_db(home: &Arc<TempDir>) -> anyhow::Result<Arc<agent9527_state::StateRuntime>> {
    let db = agent9527_state::StateRuntime::init(
        agent9527_state::SqliteConfig::new_for_testing(home.path().abs()),
        "test-provider".into(),
    )
    .await?;
...
async fn trigger_memories_startup(test: &TestAgent9527) {
    let config_snapshot = test.agent9527.config_snapshot().await;
...
async fn memory_startup_context_with_provider(
    ...
    Arc<agent9527_core::config::Config>,
) {
    let config_snapshot = test.agent9527.config_snapshot().await;
    let mut config = test.config.clone();
    config
...
impl ModelProvider for MockMemoryModelProvider {
    ...
    ) -> agent9527_models_manager::manager::SharedModelsManager {
        self.delegate
            .models_manager(agent9527_home, config_model_catalog)
    }
}

async fn seed_stage1_output(
    db: &agent9527_state::StateRuntime,
    agent9527_home: &Path,
    ...
    let mut metadata_builder = agent9527_state::ThreadMetadataBuilder::new(
        thread_id,
        agent9527_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = agent9527_home.join(format!("workspace-{rollout_slug}"));
...
async fn seed_stage1_candidate(
    ...
    let thread_id = ThreadId::new();
    let rollout_path = agent9527_home.join(format!("rollout-{thread_id}.jsonl"));
...
) -> anyhow::Result<agent9527_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.agent9527.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        agent9527_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
...
async fn shutdown_test_agent9527(test: &TestAgent9527) -> anyhow::Result<()> {

...
Runtime orchestration for Phase 1 and Phase 2 still lives in `agent9527-core` under
`agent9527-rs/core/src/memories/`.

## Crates

- `agent9527-rs/memories/read` (`agent9527-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `agent9527-rs/memories/write` (`agent9527-memories-write`) owns the write path:
...
- In `agent9527`, edit those undated template files in place.
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.agent9527/memories/.git` by `agent9527-git-utils`

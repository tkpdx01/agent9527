...
use agent9527_models_manager::MODEL_FLASH;
use agent9527_models_manager::manager::OpenAiModelsManager;
use agent9527_models_manager::manager::SharedModelsManager;
use agent9527_models_manager::manager::StaticModelsManager;
use agent9527_protocol::account::ProviderAccount;
...
/// Optional provider-backed features that Agent9527 may expose at runtime.
///
...
pub const DEFAULT_APPROVAL_REVIEW_PREFERRED_MODEL: &str = "agent9527-auto-review";

/// Default model used for memory extraction when a provider does not require a
/// backend-specific model ID.
pub const DEFAULT_MEMORY_EXTRACTION_PREFERRED_MODEL: &str = MODEL_FLASH;

/// Default model used for memory consolidation when a provider does not require
/// a backend-specific model ID.
pub const DEFAULT_MEMORY_CONSOLIDATION_PREFERRED_MODEL: &str = MODEL_BALANCED;
...
pub trait ModelProvider: fmt::Debug + Send + Sync {
    ...
    /// to think through whether Agent9527 should have a unified provider-specific auth
    /// manager throughout the codebase; that is a larger refactor than this change.
    fn auth_manager(&self) -> Option<Arc<AuthManager>>;

    /// Returns the current provider-scoped auth value, if one is configured.
    fn auth(&self) -> ModelProviderFuture<'_, Option<Agent9527Auth>>;

    /// Returns the current app-visible account state for this provider.
    fn account_state(&self) -> ProviderAccountResult;

    /// Maps an API client error into the provider's user-facing error representation.
    fn map_api_error(&self, error: ApiError) -> Agent9527Err {
        agent9527_api::map_api_error(error)
    }

    /// Returns provider configuration adapted for the API client.
    fn api_provider(&self) -> ModelProviderFuture<'_, agent9527_protocol::error::Result<Provider>> {
        Box::pin(async move {
            let auth = self.auth().await;
            self.info()
                .to_api_provider(auth.as_ref().map(Agent9527Auth::auth_mode))
        })
    }
    ...
    fn runtime_base_url(
        &self,
    ) -> ModelProviderFuture<'_, agent9527_protocol::error::Result<Option<String>>> {
    ...
    fn api_auth(
        &self,
    ) -> ModelProviderFuture<'_, agent9527_protocol::error::Result<SharedAuthProvider>> {
        Box::pin(async move {
            let auth = self.auth().await;
    ...
    /// Returns request credentials, optionally scoped to a Agent9527 session task.
    fn api_auth_for_scope(
        &self,
        scope: ProviderAuthScope,
    ) -> ModelProviderFuture<'_, agent9527_protocol::error::Result<ResolvedProviderAuth>> {
        Box::pin(async move {
    ...
    fn models_manager_without_cache(
        ...
        let model_catalog = config_model_catalog
            .or_else(|| agent9527_models_manager::bundled_models_response().ok())
            .unwrap_or_default();
        ...
        let account = if self.info.requires_openai_auth {
            self.auth_manager
                ...
                .and_then(|auth_manager| {
                    ...
                    if matches!(auth, Agent9527Auth::Headers(_)) {
                        return None;
                    }
                    Some(auth)
                })
                .map(|auth| match &auth {
                    Agent9527Auth::ApiKey(_) => Ok(ProviderAccount::ApiKey),
...
mod tests {
    ...
    use agent9527_model_provider_info::ModelProviderAwsAuthInfo;
    ...
    fn test_agent9527_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!(
            "agent9527-model-provider-test-{}",
            std::process::id()
        ))
    }
    ...
    fn bedrock_api_key_auth() -> Agent9527Auth {
        Agent9527Auth::BedrockApiKey(BedrockApiKeyAuth {
    ...
    fn create_model_provider_does_not_use_openai_auth_manager_for_amazon_bedrock_provider() {
        let provider = create_model_provider(
            ModelProviderInfo::create_amazon_bedrock_provider(Some(ModelProviderAwsAuthInfo {
                profile: Some("agent9527-bedrock".to_string()),
                region: None,
            })),
            Some(AuthManager::from_auth_for_testing(
                Agent9527Auth::from_api_key("openai-api-key"),
            )),
        );
    ...
    async fn amazon_bedrock_provider_creates_static_models_manager() {
        ...
        let manager =
            provider.models_manager(test_agent9527_home(), /*config_model_catalog*/ None);
    ...
    async fn configured_bedrock_catalog_only_allows_default_service_tier() {
        let mut configured_model = agent9527_models_manager::upstream_bundled_models_response()
            .expect("bundled models should parse")
            .models
            .into_iter()
            .find(|model| model.slug == "gpt-5.5")
            .expect("bundled models should include GPT-5.5");
        configured_model.slug = "openai.gpt-5.5".to_string();
        ...
        assert_eq!(catalog.models[0].slug, "openai.gpt-5.5");
        assert_eq!(

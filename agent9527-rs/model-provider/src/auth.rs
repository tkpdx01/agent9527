...
use agent9527_agent_identity::AgentIdentityKey;
use agent9527_agent_identity::authorization_header_for_agent_task;
...
use agent9527_login::AuthHeaders;
use agent9527_login::AuthManager;
use agent9527_login::auth::AgentIdentityAuth;
use agent9527_login::auth::AgentIdentityAuthError;
...
impl AuthProvider for AuthManagerAuthProvider {
    fn add_auth_headers(&self, headers: &mut HeaderMap) {
        let Some(auth) = self
            .auth_manager
            .auth_cached()
            .filter(Agent9527Auth::uses_agent9527_backend)
        else {
            return;
        };
...
pub(crate) fn resolve_provider_auth(
    ...
    provider: &ModelProviderInfo,
) -> agent9527_protocol::error::Result<SharedAuthProvider> {
    if matches!(auth, Some(Agent9527Auth::BedrockApiKey(_))) {
        return Err(Agent9527Err::UnsupportedOperation(
...
) -> agent9527_protocol::error::Result<ResolvedProviderAuth> {
    let ProviderAuthScope {
        agent_identity_policy,
        session_source,
        agent_identity_session_fallback,
    } = scope;
    if let Some(Agent9527Auth::AgentIdentity(agent_identity_auth)) = auth {
...
fn should_bootstrap_chatgpt_agent_identity(
    ...
    agent_identity_policy == AgentIdentityAuthPolicy::ChatGptAuth
        && matches!(auth, Some(Agent9527Auth::Chatgpt(_)))
}

fn bearer_auth_for_provider(
    provider: &ModelProviderInfo,
) -> agent9527_protocol::error::Result<Option<BearerAuthProvider>> {
...
/// Builds request-header auth for a first-party Agent9527 auth snapshot.
pub fn auth_provider_from_auth(auth: &Agent9527Auth) -> SharedAuthProvider {
    match auth {
        Agent9527Auth::AgentIdentity(auth) => {
            Arc::new(AgentIdentityAuthProvider { auth: auth.clone() })
        }
        Agent9527Auth::Headers(auth) => Arc::new(HeaderAuthProvider { auth: auth.clone() }),
        Agent9527Auth::BedrockApiKey(_) => unreachable!("{BEDROCK_API_KEY_UNSUPPORTED_MESSAGE}"),
        Agent9527Auth::ApiKey(_)
        | Agent9527Auth::Chatgpt(_)
        | Agent9527Auth::ChatgptAuthTokens(_)
        | Agent9527Auth::PersonalAccessToken(_) => Arc::new(BearerAuthProvider {
...
#[cfg(test)]
mod tests {
    use agent9527_agent_identity::generate_agent_key_material;
    use agent9527_login::AuthCredentialsStoreMode;
    use agent9527_login::AuthKeyringBackendKind;
    use agent9527_login::auth::AgentIdentityAuthRecord;
    use agent9527_login::auth::BedrockApiKeyAuth;
    use agent9527_login::auth::login_with_chatgpt_auth_tokens;
    use agent9527_model_provider_info::WireApi;
    use agent9527_model_provider_info::create_oss_provider_with_base_url;
    ...
    static NEXT_AGENT9527_HOME_ID: AtomicUsize = AtomicUsize::new(0);
    ...
    fn test_agent9527_home() -> PathBuf {
        let id = NEXT_AGENT9527_HOME_ID.fetch_add(1, Ordering::Relaxed);
        let path = std::env::temp_dir().join(format!(
            "agent9527-model-provider-agent-identity-{pid}-{id}",
            pid = std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&path);
        std::fs::create_dir_all(&path).expect("create temp agent9527 home");
        path
    }

    fn write_chatgpt_auth_json(agent9527_home: &Path) {
        let auth_json = json!({
            "tokens": {
    ...
    async fn chatgpt_auth_manager(
    ...
    ) -> (PathBuf, Arc<AuthManager>, Agent9527Auth) {
        let agent9527_home = test_agent9527_home();
        write_chatgpt_auth_json(&agent9527_home);
        ...
        (agent9527_home, auth_manager, auth)
    }
    ...
    fn header_auth_adds_predefined_headers() {
        ...
        let auth = Agent9527Auth::Headers(AuthHeaders::new(expected.clone()));
    ...
    fn openai_provider_rejects_bedrock_api_key_auth() {
        ...
        let auth = Agent9527Auth::BedrockApiKey(BedrockApiKeyAuth {
    ...
    async fn first_party_run_scope_uses_agent_assertion_and_exposes_telemetry() {
        let auth = Agent9527Auth::AgentIdentity(
    ...
    async fn agent_identity_auth_provider_preserves_account_routing_headers() {
        ...
        let provider = auth_provider_from_auth(&Agent9527Auth::AgentIdentity(auth));
    ...
    async fn chatgpt_bootstrap_unavailable_uses_session_bearer_fallback() {
        ...
        .await;
        let (_agent9527_home, auth_manager, auth) = chatgpt_auth_manager(server.uri()).await;
        let provider = ModelProviderInfo::create_openai_provider(/*base_url*/ None);

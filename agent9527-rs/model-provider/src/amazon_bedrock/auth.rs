...
use agent9527_aws_auth::AwsAuthContext;
use agent9527_aws_auth::AwsAuthError;
use agent9527_aws_auth::AwsRequestToSign;
use agent9527_http_client::Request;
use agent9527_http_client::RequestBody;
use agent9527_http_client::RequestCompression;
...
pub(super) async fn resolve_auth_method(
    ...
    let context = AwsAuthContext::load(config)
        .await
        .map_err(aws_auth_error_to_agent9527_error)?;
...
fn aws_auth_error_to_agent9527_error(error: AwsAuthError) -> Agent9527Err {
    Agent9527Err::Fatal(format!("failed to resolve Amazon Bedrock auth: {error}"))
}
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    ...
    // richer Agent9527 agent requests fail even though raw Responses requests work.

...
impl AmazonBedrockModelProvider {
    ...
    fn managed_auth(&self) -> Option<BedrockApiKeyAuth> {
        self.auth_manager
            ...
            .and_then(|auth| match auth {
                Agent9527Auth::BedrockApiKey(auth) => Some(auth),
                ...
                | Agent9527Auth::PersonalAccessToken(_) => None,
            })
    }
    ...
    async fn auth(&self) -> Option<Agent9527Auth> {
        if self.info.has_command_auth() {
            ...
            self.managed_auth().map(Agent9527Auth::BedrockApiKey)
        }
    }
    ...
    fn account_state(&self) -> ProviderAccountResult {
        Ok(ProviderAccountState {
            account: Some(ProviderAccount::AmazonBedrock {
                uses_agent9527_managed_credentials: self.managed_auth().is_some(),
            }),
...
mod tests {
    ...
    async fn managed_auth_takes_precedence_over_aws_auth() {
        ...
        let auth_manager =
            AuthManager::from_auth_for_testing(Agent9527Auth::BedrockApiKey(managed_auth.clone()));
        ...
        assert_eq!(
            provider.auth().await,
            Some(Agent9527Auth::BedrockApiKey(managed_auth))
        );
        assert_eq!(
            provider.account_state(),
            Ok(ProviderAccountState {

...
use agent9527_utils_absolute_path::canonicalize_preserving_symlinks;
...
const PROTECTED_METADATA_AGENT9527_PATH_NAME: &str = ".agent9527";
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_AGENT9527_PATH_NAME,
];
...
pub fn is_protected_metadata_directory_name(name: &OsStr) -> bool {
    name == OsStr::new(PROTECTED_METADATA_AGENTS_PATH_NAME)
        || name == OsStr::new(PROTECTED_METADATA_AGENT9527_PATH_NAME)
}
...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer Agent9527 degrades to warn-and-ignore
    /// instead of failing to load. Agent9527 0.112.0 rejected unknown values here,
...
const PROJECT_ROOTS_GLOB_PATTERN_PREFIX: &str = "agent9527-project-roots://";
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        for writable_root in writable_roots {
            for protected_path in default_read_only_subpaths_for_writable_root(
                writable_root,
                /*protect_missing_dot_agent9527*/ false,
            ) {
    ...
    pub fn from_legacy_sandbox_policy_for_cwd(sandbox_policy: &SandboxPolicy, cwd: &Path) -> Self {
        ...
        if let SandboxPolicy::WorkspaceWrite { writable_roots, .. } = sandbox_policy {
            if let Ok(cwd_root) = AbsolutePathBuf::from_absolute_path(cwd) {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    &cwd_root, /*protect_missing_dot_agent9527*/ true,
                ) {
    ...
    pub fn with_additional_legacy_workspace_writable_roots(
        ...
        for path in additional_writable_roots {
            ...
            for protected_path in default_read_only_subpaths_for_writable_root(
                path, /*protect_missing_dot_agent9527*/ false,
            ) {
                append_default_read_only_path_if_no_explicit_rule(
                    &mut self.entries,
                    protected_path,
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            let protect_missing_dot_agent9527 = AbsolutePathBuf::from_absolute_path(cwd)
                .ok()
                .is_some_and(|cwd| normalize_effective_absolute_path(cwd) == root);
            let mut read_only_subpaths: Vec<AbsolutePathBuf> =
                default_read_only_subpaths_for_writable_root(&root, protect_missing_dot_agent9527)
                    .into_iter()
            ...
            // Example: if `<root>/.agent9527 -> <root>/decoy`, bwrap must still see
            // `<root>/.agent9527`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.agent9527` so downstream sandboxes can still detect and mask
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    protect_missing_dot_agent9527: bool,
) -> Vec<AbsolutePathBuf> {
    ...
    // Keep top-level project metadata under .agent9527 read-only to the agent by
    ...
    let top_level_agent9527 = writable_root.join(PROTECTED_METADATA_AGENT9527_PATH_NAME);
    if protect_missing_dot_agent9527 || top_level_agent9527.as_path().is_dir() {
        subpaths.push(top_level_agent9527);
    }
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "AGENT9527_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_proactively_protect_missing_dot_agent9527() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let expected_dot_agent9527 = expected_root.join(".agent9527");
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .contains(&expected_dot_agent9527)
        );
    }
    ...
    fn legacy_workspace_write_projection_preserves_symbolic_project_root() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry {
                    path: FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(Some(".agent9527".into())),
                    },
                    access: FileSystemAccessMode::Read,
                },
            ])
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_dot_agent9527_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let explicit_dot_agent9527 = expected_root.join(".agent9527");
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Path {
                    path: explicit_dot_agent9527.clone(),
                },
                access: FileSystemAccessMode::Write,
            },
        ]);
        ...
        assert!(
            !workspace_root
                .protected_metadata_names
                .contains(&".agent9527".to_string()),
            "explicit .agent9527 rule should remove the metadata-name protection"
        );
        assert!(
            !workspace_root
                .read_only_subpaths
                .contains(&explicit_dot_agent9527),
            "explicit .agent9527 rule should win over the default protected carveout"
        );
        assert!(policy.can_write_path_with_cwd(
            explicit_dot_agent9527.join("config.toml").as_path(),
            cwd.path()
        ));
    }
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_agent9527_config = cwd.path().join(".agent9527").join("config.toml");
        ...
        assert!(!file_system_policy.can_write_path_with_cwd(&dot_agent9527_config, cwd.path()));
        ...
        assert!(!writable_roots[0].is_path_writable(&dot_agent9527_config));
    }
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        expected_entries.extend(
            default_read_only_subpaths_for_writable_root(
                &expected_root,
                /*protect_missing_dot_agent9527*/ true,
            )
            .into_iter()
            .map(|path| FileSystemSandboxEntry {
                path: FileSystemPath::Path { path },
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path::new(".agent9527/config.toml"), relative_cwd,)
        );
        assert!(
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let agent9527_dir = real_root.join(".agent9527");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&agent9527_dir).expect("create .agent9527");
        ...
        let expected_agent9527 = link_root.join(".agent9527");
    ...
    fn project_roots_special_path_preserves_symlinked_root() {
        ...
        let expected_agent9527 = expected_root.join(".agent9527");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-agent9527");
        let dot_agent9527 = root.join(".agent9527");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_agent9527).expect("create .agent9527 symlink");

        let root = AbsolutePathBuf::from_absolute_path(&root).expect("absolute root");
        let expected_dot_agent9527 = AbsolutePathBuf::from_absolute_path(
            root.as_path()
                .canonicalize()
                .expect("canonicalize root")
                .join(".agent9527"),
        )
        .expect("absolute .agent9527 symlink");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let agent9527_dir = real_tmpdir.join(".agent9527");

//! Defines the protocol for a Agent9527 session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Agent9527:";
...
pub struct ConversationStartParams {
    /// Whether Agent9527 response handoffs are managed through explicit client append calls.
    pub client_managed_handoffs: bool,
    /// Whether to route any remaining transcript tail through Agent9527 when the session ends.
    ...
    /// Sends automatic Agent9527 responses as realtime conversation items instead of handoff appends.
    pub agent9527_responses_as_items: bool,
    /// Optional prefix added to automatic Agent9527 response items when `agent9527_responses_as_items` is set.
    pub agent9527_response_item_prefix: Option<String>,
    /// Selects how automatic Agent9527 handoffs are routed in Frameless Bidi sessions.
    /// Realtime V1 and V2 ignore this setting.
    pub agent9527_response_handoff_mode: Agent9527ResponseHandoffMode,
    /// Optional client-selected BEM prefixes keyed by `analysis`, `commentary`, and `final`.
    pub agent9527_response_handoff_channel_prefixes: Option<BTreeMap<String, Vec<String>>>,
    /// Overrides the configured realtime model for this session only.
    pub model: Option<String>,
    /// Selects whether the realtime session should produce text or audio output.
    pub output_modality: RealtimeOutputModality,
    /// Whether to append Agent9527's startup context to the realtime backend prompt.
...
pub enum Op {
    ...
    /// Request Agent9527 to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down agent9527 instance.
    Shutdown,
...
/// running the command proposed by Agent9527.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.agent9527`, `.git`, notably `.git/hooks`) under
...
#[serde(rename_all = "camelCase")]
#[ts(rename_all = "camelCase")]
pub enum Agent9527ResponseHandoffMode {
    #[default]
    Thinking,
    Commentary,
    BemTags,
...
/// Agent9527 errors that we expose to clients.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "snake_case")]
#[ts(rename_all = "snake_case")]
pub enum Agent9527ErrorInfo {
...
pub struct ErrorEvent {
    pub message: String,
    #[serde(default)]
    pub agent9527_error_info: Option<Agent9527ErrorInfo>,
}

impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.agent9527_error_info
            .as_ref()
            .is_none_or(Agent9527ErrorInfo::affects_turn_status)
    }
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::Agent9527),
...
pub struct TurnContextItem {
    ...
    // Compatibility-only field written with a default value so older Agent9527
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => usize::try_from(
                agent9527_utils_string::approx_tokens_from_byte_count(*bytes),
            )
            .unwrap_or(usize::MAX),
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                agent9527_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_agent9527() {
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("agent9527".to_string()).restriction_product(),
            Some(Product::Agent9527)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::Agent9527])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::Agent9527]));
        assert!(
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd =
            agent9527_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".agent9527")).expect("create .agent9527");
        let canonical_cwd =
            agent9527_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_agent9527 =
            AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".agent9527"))
                .expect("canonical .agent9527");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_agent9527.as_path())
        );
    }
        ...
        assert_eq!(
            ...
            vec![
                (
                    canonical_cwd,
                    vec![
                        expected_dot_agent9527.to_path_buf(),
    ...
    fn generic_error_affects_turn_status() {
        let event = ErrorEvent {
            ...
            agent9527_error_info: Some(Agent9527ErrorInfo::Other),
        };
        assert!(event.affects_turn_status());
    }

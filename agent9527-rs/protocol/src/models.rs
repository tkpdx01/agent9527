...
use agent9527_utils_image::data_url_from_bytes;
...
use agent9527_utils_image::ImageProcessingError;
...
pub enum SandboxEnforcement {
    /// Agent9527 owns sandbox construction for this profile.
    #[default]
    Managed,
...
/// Filesystem permissions for profiles where Agent9527 owns sandbox construction.
...
fn local_media_error_placeholder(
    ...
    ContentItem::InputText {
        text: format!("Agent9527 could not read the local {media_name} at `{path}`: {error}"),
    }
}
...
fn unsupported_image_error_placeholder(path: &std::path::Path, mime: &str) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "Agent9527 cannot attach image at `{}`: unsupported image `{}`.",
            path.display(),
            mime
        ),
    }
...
fn unsupported_audio_error_placeholder(path: &std::path::Path) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "Agent9527 cannot attach audio at `{}`: unsupported audio format; use wav, mp3, m4a, webm, or ogg.",
            path.display()
        ),
    }
}
...
fn convert_mcp_content_to_items(
    ...
    const AGENT9527_ENCRYPTED_CONTENT_META_KEY: &str = "agent9527/encryptedContent";
    const AGENT9527_IMAGE_DETAIL_META_KEY: &str = "agent9527/imageDetail";
    ...
    for content in contents {
        let item = match serde_json::from_value::<McpContent>(content.clone()) {
            Ok(McpContent::Text { text, meta }) => {
                if meta
                    .as_ref()
                    .and_then(|meta| meta.get(AGENT9527_ENCRYPTED_CONTENT_META_KEY))
            ...
            Ok(McpContent::Image {
                ...
                FunctionCallOutputContentItem::InputImage {
                    ...
                    detail: meta
                        ...
                        .and_then(|meta| meta.get(AGENT9527_IMAGE_DETAIL_META_KEY))
...
// (Moved event mapping logic into agent9527-core to avoid coupling protocol to UI-facing events.)
...
mod tests {
    ...
    fn function_call_deserializes_optional_namespace() {
        let item: ResponseItem = serde_json::from_value(serde_json::json!({
            "type": "function_call",
            "name": "mcp__agent9527_apps__gmail_get_recent_emails",
            "namespace": "mcp__agent9527_apps__gmail",
            "arguments": "{\"top_k\":5}",
            "call_id": "call-1",
        }))
        ...
        assert_eq!(
            ...
            ResponseItem::FunctionCall {
                id: None,
                name: "mcp__agent9527_apps__gmail_get_recent_emails".to_string(),
                namespace: Some("mcp__agent9527_apps__gmail".to_string()),
...
- ["a", "b", "c"]"#
        ...
        for i in 0..200 {
            exec_policy
                .add_prefix_rule(
                    ...
                    agent9527_execpolicy::Decision::Allow,
                )
                .expect("add rule");
        }
    ...
    fn preserves_original_detail_metadata_on_mcp_images() -> Result<()> {
        let call_tool_result = CallToolResult {
            content: vec![serde_json::json!({
                ...
                "mimeType": "image/png",
                "_meta": {
                    "agent9527/imageDetail": "original",
                },
            })],
            structured_content: None,
            is_error: Some(false),
    ...
    fn replaces_unreadable_local_audio_with_placeholder() {
        ...
        assert!(
            text.starts_with("Agent9527 could not read the local audio at `missing.wav`: "),
    ...
    fn tool_search_output_roundtrips() -> Result<()> {
        let input = ResponseInputItem::ToolSearchOutput {
            ...
            tools: vec![serde_json::json!({
                "type": "function",
                "name": "mcp__agent9527_apps__calendar_create_event",
...
<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"></svg>"#,
        ...
        match item {
            ResponseInputItem::Message { content, .. } => {
                assert_eq!(content.len(), 1);
                let expected = format!(
                    "Agent9527 cannot attach image at `{}`: unsupported image `image/svg+xml`.",
                    svg_path.display()
                );
                match &content[0] {

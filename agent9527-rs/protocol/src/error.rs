...
impl Agent9527Err {
    pub fn is_retryable(&self) -> bool {
        match self {
            Agent9527Err::TurnAborted
            | Agent9527Err::SessionBudgetExceeded
            | Agent9527Err::Interrupted
            | Agent9527Err::EnvVar(_)
            | Agent9527Err::Fatal(_)
            | Agent9527Err::UsageNotIncluded
            | Agent9527Err::QuotaExceeded
            | Agent9527Err::InvalidImageRequest()
            | Agent9527Err::InvalidRequest(_)
            | Agent9527Err::RefreshTokenFailed(_)
            | Agent9527Err::UnsupportedOperation(_)
            | Agent9527Err::Sandbox(_)
            | Agent9527Err::LandlockSandboxExecutableNotProvided
            | Agent9527Err::RetryLimit(_)
            | Agent9527Err::ContextWindowExceeded
            | Agent9527Err::ThreadNotFound(_)
            | Agent9527Err::AgentLimitReached { .. }
            | Agent9527Err::Spawn
            | Agent9527Err::SessionConfiguredNotFirstEvent
            | Agent9527Err::UsageLimitReached(_)
            | Agent9527Err::ServerOverloaded
            | Agent9527Err::CyberPolicy { .. } => false,
            Agent9527Err::Stream(..)
            | Agent9527Err::Timeout
            | Agent9527Err::RequestTimeout
            | Agent9527Err::UnexpectedStatus(_)
            | Agent9527Err::ResponseStreamFailed(_)
            | Agent9527Err::ConnectionFailed(_)
            | Agent9527Err::InternalServerError
            | Agent9527Err::InternalAgentDied
            | Agent9527Err::Io(_)
            | Agent9527Err::Json(_)
            | Agent9527Err::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            Agent9527Err::LandlockRuleset(_) | Agent9527Err::LandlockPathFd(_) => false,
        }
    }
    ...
    pub fn to_agent9527_protocol_error(&self) -> Agent9527ErrorInfo {
        match self {
            Agent9527Err::ContextWindowExceeded => Agent9527ErrorInfo::ContextWindowExceeded,
            Agent9527Err::SessionBudgetExceeded => Agent9527ErrorInfo::SessionBudgetExceeded,
            Agent9527Err::UsageLimitReached(_)
            | Agent9527Err::QuotaExceeded
            | Agent9527Err::UsageNotIncluded => Agent9527ErrorInfo::UsageLimitExceeded,
            Agent9527Err::ServerOverloaded => Agent9527ErrorInfo::ServerOverloaded,
            Agent9527Err::CyberPolicy { .. } => Agent9527ErrorInfo::CyberPolicy,
            Agent9527Err::RetryLimit(_) => Agent9527ErrorInfo::ResponseTooManyFailedAttempts {
                http_status_code: self.http_status_code_value(),
            },
            Agent9527Err::ConnectionFailed(_) => Agent9527ErrorInfo::HttpConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            Agent9527Err::ResponseStreamFailed(_) => {
            ...
            Agent9527Err::RefreshTokenFailed(_) => Agent9527ErrorInfo::Unauthorized,
            Agent9527Err::SessionConfiguredNotFirstEvent
            | Agent9527Err::InternalServerError
            | Agent9527Err::InternalAgentDied => Agent9527ErrorInfo::InternalServerError,
            Agent9527Err::UnsupportedOperation(_)
            | Agent9527Err::ThreadNotFound(_)
            | Agent9527Err::AgentLimitReached { .. } => Agent9527ErrorInfo::BadRequest,
            Agent9527Err::Sandbox(_) => Agent9527ErrorInfo::SandboxError,
    ...
    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            Agent9527Err::RetryLimit(err) => Some(err.status),
            Agent9527Err::UnexpectedStatus(err) => Some(err.status),
            Agent9527Err::ConnectionFailed(err) => err.source.status(),
            Agent9527Err::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
pub fn get_error_message_ui(e: &Agent9527Err) -> String {
    let message = match e {
        Agent9527Err::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        Agent9527Err::Sandbox(SandboxErr::Timeout { output }) => {
            format!(

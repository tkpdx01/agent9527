...
use crate::protocol::Agent9527ErrorInfo;
...
use agent9527_utils_string::truncate_middle_chars;
use agent9527_utils_string::truncate_middle_with_token_budget;
use chrono::DateTime;
use chrono::Datelike;
use chrono::Local;
use chrono::Utc;
...
pub type Result<T> = std::result::Result<T, Agent9527Err>;
...
pub struct Agent9527Err {
    details: Agent9527ErrorDetails,
    retry_delay: Option<Duration>,
}

/// The semantic category and diagnostic payload for a [`Agent9527Err`].
#[derive(Error, Debug, EnumDiscriminants)]
#[strum_discriminants(name(Agent9527ErrKind))]
...
pub enum Agent9527ErrorDetails {
    ...
    Stream(String),
    #[error(
        "Agent9527 ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `Agent9527Error::Spawn` variant.
    ...
    QuotaExceeded,
    #[error(
        "To use Agent9527 with your ChatGPT plan, upgrade to Plus: https://chatgpt.com/explore/plus."
    )]
    UsageNotIncluded,
    ...
    #[error("agent9527-linux-sandbox was required but not provided")]
...
impl From<&Agent9527Err> for Agent9527ErrKind {
    fn from(error: &Agent9527Err) -> Self {
        error.details().into()
    }
}

impl fmt::Debug for Agent9527Err {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match &self.details {
            Agent9527ErrorDetails::Stream(message) => formatter
                .debug_tuple("Stream")
                .field(message)
...
impl fmt::Display for Agent9527Err {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::Display::fmt(&self.details, formatter)
    }
}

impl std::error::Error for Agent9527Err {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        self.details.source()
    }
}

impl From<Agent9527ErrorDetails> for Agent9527Err {
    fn from(details: Agent9527ErrorDetails) -> Self {
        Self {
            details,
            retry_delay: None,
        }
...
impl From<CancelErr> for Agent9527Err {
    fn from(error: CancelErr) -> Self {
        Agent9527ErrorDetails::from(error).into()
    }
}

impl From<SandboxErr> for Agent9527Err {
    fn from(error: SandboxErr) -> Self {
        Agent9527ErrorDetails::from(error).into()
    }
}

impl From<io::Error> for Agent9527Err {
    fn from(error: io::Error) -> Self {
        Agent9527ErrorDetails::from(error).into()
    }
}

impl From<serde_json::Error> for Agent9527Err {
    fn from(error: serde_json::Error) -> Self {
        Agent9527ErrorDetails::from(error).into()
    }
}

impl From<JoinError> for Agent9527Err {
...
#[cfg(target_os = "linux")]
impl From<landlock::RulesetError> for Agent9527Err {
...
#[cfg(target_os = "linux")]
impl From<landlock::PathFdError> for Agent9527Err {
    fn from(error: landlock::PathFdError) -> Self {
        Agent9527ErrorDetails::from(error).into()
    }
}

impl From<CancelErr> for Agent9527ErrorDetails {
    fn from(_: CancelErr) -> Self {
        Agent9527ErrorDetails::TurnAborted
    }
}

// TODO(anp): Remove this compatibility macro once callers construct
// `Agent9527ErrorDetails` directly.
macro_rules! agent9527_err_unit_constructors {
    ($($variant:ident),* $(,)?) => {
        $(
            #[doc(hidden)]
            #[allow(non_upper_case_globals)]
            pub const $variant: Self = Self {
                details: Agent9527ErrorDetails::$variant,
                retry_delay: None,
            };
        )*
    };
...
// TODO(anp): Remove this compatibility macro once callers construct
// `Agent9527ErrorDetails` directly.
macro_rules! agent9527_err_tuple_constructors {
    ($($(#[$attr:meta])* $variant:ident($value:ident: $value_type:ty)),* $(,)?) => {
        $(
            ...
            pub fn $variant($value: $value_type) -> Self {
                Agent9527ErrorDetails::$variant($value).into()
            }
        )*
    };
}
...
impl Agent9527Err {
    agent9527_err_unit_constructors!(
        TurnAborted,
        SessionBudgetExceeded,
        ContextWindowExceeded,
        SessionConfiguredNotFirstEvent,
    ...
    agent9527_err_tuple_constructors!(
    ...
    pub fn InvalidImageRequest() -> Self {
        Agent9527ErrorDetails::InvalidImageRequest().into()
    }

    /// Creates an error with no server-provided retry delay.
    pub fn new(details: Agent9527ErrorDetails) -> Self {
        details.into()
    }

    /// Returns the semantic failure and its diagnostic payload.
    pub fn details(&self) -> &Agent9527ErrorDetails {
        &self.details
    }
    ...
    pub fn is_retryable(&self) -> bool {
        match self.details() {
            Agent9527ErrorDetails::TurnAborted
            | Agent9527ErrorDetails::SessionBudgetExceeded
            | Agent9527ErrorDetails::Interrupted
            | Agent9527ErrorDetails::EnvVar(_)
            | Agent9527ErrorDetails::Fatal(_)
            | Agent9527ErrorDetails::UsageNotIncluded
            | Agent9527ErrorDetails::QuotaExceeded
            | Agent9527ErrorDetails::InvalidImageRequest()
            | Agent9527ErrorDetails::InvalidRequest(_)
            | Agent9527ErrorDetails::RefreshTokenFailed(_)
            | Agent9527ErrorDetails::UnsupportedOperation(_)
            | Agent9527ErrorDetails::Sandbox(_)
            | Agent9527ErrorDetails::LandlockSandboxExecutableNotProvided
            | Agent9527ErrorDetails::RetryLimit(_)
            | Agent9527ErrorDetails::ContextWindowExceeded
            | Agent9527ErrorDetails::ThreadNotFound(_)
            | Agent9527ErrorDetails::AgentLimitReached { .. }
            | Agent9527ErrorDetails::Spawn
            | Agent9527ErrorDetails::SessionConfiguredNotFirstEvent
            | Agent9527ErrorDetails::UsageLimitReached(_)
            | Agent9527ErrorDetails::ServerOverloaded
            | Agent9527ErrorDetails::CyberPolicy { .. } => false,
            Agent9527ErrorDetails::Stream(..)
            | Agent9527ErrorDetails::Timeout
            | Agent9527ErrorDetails::RequestTimeout
            | Agent9527ErrorDetails::UnexpectedStatus(_)
            | Agent9527ErrorDetails::ResponseStreamFailed(_)
            | Agent9527ErrorDetails::ConnectionFailed(_)
            | Agent9527ErrorDetails::InternalServerError
            | Agent9527ErrorDetails::InternalAgentDied
            | Agent9527ErrorDetails::Io(_)
            | Agent9527ErrorDetails::Json(_)
            | Agent9527ErrorDetails::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            Agent9527ErrorDetails::LandlockRuleset(_)
            | Agent9527ErrorDetails::LandlockPathFd(_) => false,
        }
    }
    ...
    /// Minimal shim so that existing `e.downcast_ref::<Agent9527Err>()` checks continue to compile
    ...
    pub fn to_agent9527_protocol_error(&self) -> Agent9527ErrorInfo {
        match &self.details {
            Agent9527ErrorDetails::ContextWindowExceeded => {
                Agent9527ErrorInfo::ContextWindowExceeded
            }
            Agent9527ErrorDetails::SessionBudgetExceeded => {
                Agent9527ErrorInfo::SessionBudgetExceeded
            }
            Agent9527ErrorDetails::UsageLimitReached(_)
            | Agent9527ErrorDetails::QuotaExceeded
            | Agent9527ErrorDetails::UsageNotIncluded => Agent9527ErrorInfo::UsageLimitExceeded,
            Agent9527ErrorDetails::ServerOverloaded => Agent9527ErrorInfo::ServerOverloaded,
            Agent9527ErrorDetails::CyberPolicy { .. } => Agent9527ErrorInfo::CyberPolicy,
            Agent9527ErrorDetails::RetryLimit(_) => {
                Agent9527ErrorInfo::ResponseTooManyFailedAttempts {
                    http_status_code: self.http_status_code_value(),
                }
            }
            Agent9527ErrorDetails::ConnectionFailed(_) => {
                Agent9527ErrorInfo::HttpConnectionFailed {
                    http_status_code: self.http_status_code_value(),
                }
            }
            Agent9527ErrorDetails::ResponseStreamFailed(_) => {
                Agent9527ErrorInfo::ResponseStreamConnectionFailed {
                    http_status_code: self.http_status_code_value(),
                }
            }
            Agent9527ErrorDetails::RefreshTokenFailed(_) => Agent9527ErrorInfo::Unauthorized,
            Agent9527ErrorDetails::SessionConfiguredNotFirstEvent
            | Agent9527ErrorDetails::InternalServerError
            | Agent9527ErrorDetails::InternalAgentDied => Agent9527ErrorInfo::InternalServerError,
            Agent9527ErrorDetails::UnsupportedOperation(_)
            | Agent9527ErrorDetails::ThreadNotFound(_)
            | Agent9527ErrorDetails::AgentLimitReached { .. } => Agent9527ErrorInfo::BadRequest,
            Agent9527ErrorDetails::Sandbox(_) => Agent9527ErrorInfo::SandboxError,
            _ => Agent9527ErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            agent9527_error_info: Some(self.to_agent9527_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match &self.details {
            Agent9527ErrorDetails::RetryLimit(err) => Some(err.status),
            Agent9527ErrorDetails::UnexpectedStatus(err) => Some(err.status),
            Agent9527ErrorDetails::ConnectionFailed(err) => err.source.status(),
            Agent9527ErrorDetails::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
impl std::fmt::Display for UsageLimitReachedError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(limit_name) = self
            ...
            && !limit_name.eq_ignore_ascii_case("agent9527")
        {
            return write!(
                f,
        ...
        let message = match self.plan_type.as_ref() {
            Some(PlanType::Known(KnownPlan::Plus)) => format!(
                "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/agent9527/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
            Some(PlanType::Known(
                KnownPlan::Team
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using Agent9527 (https://chatgpt.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
            Some(PlanType::Known(KnownPlan::Pro | KnownPlan::ProLite)) => format!(
                "You've hit your usage limit. Visit https://chatgpt.com/agent9527/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
...
pub fn get_error_message_ui(e: &Agent9527Err) -> String {
    let message = match e.details() {
        Agent9527ErrorDetails::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        Agent9527ErrorDetails::Sandbox(SandboxErr::Timeout { output }) => {
            format!(

...
pub use agent9527_state::LogEntry;
...
pub type StateDbHandle = Arc<agent9527_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    let runtime =
        agent9527_state::StateRuntime::init(sqlite_home.clone(), default_model_provider_id.clone())
            .await
            .with_context(|| {
                format!(
    ...
    .await;
    agent9527_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    ...
    loop {
        ...
        if backfill_state.status == agent9527_state::BackfillStatus::Complete {
            return Ok(());
        }
        ...
        if let Some(backfill_lease_seconds) = backfill_lease_seconds {
            ...
            metadata::backfill_sessions(runtime, agent9527_home, default_model_provider_id).await;
        }
        let backfill_state = runtime.get_backfill_state().await.map_err(|err| {
            anyhow::anyhow!(
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = agent9527_state::state_db_path(config.sqlite_home());
    ...
    let runtime = match agent9527_state::StateRuntime::init(
...
pub fn sqlite_telemetry_recorder(
    ...
    originator: &str,
) -> agent9527_state::DbTelemetryHandle {
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == agent9527_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<agent9527_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&agent9527_state::StateRuntime>,
    agent9527_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.agent9527_home() != agent9527_home {
        warn!(
            "state db agent9527_home mismatch: expected {}, got {}",
            ctx.agent9527_home().display(),
            agent9527_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => agent9527_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => agent9527_state::SortKey::UpdatedAt,
                ThreadSortKey::RecencyAt => agent9527_state::SortKey::RecencyAt,
            },
...
pub async fn list_threads_db(
    ...
    relation_filter: Option<agent9527_state::ThreadRelationFilter>,
    archived: bool,
    search_term: Option<&str>,
) -> Option<agent9527_state::ThreadsPage> {
    let ctx = context?;
    if ctx.agent9527_home() != agent9527_home {
        warn!(
            "state db agent9527_home mismatch: expected {}, got {}",
    ...
    let filters = agent9527_state::ThreadFilterOptions {
        ...
        sort_direction: match sort_direction {
            SortDirection::Asc => agent9527_state::SortDirection::Asc,
            SortDirection::Desc => agent9527_state::SortDirection::Desc,
        },
        search_term,
    };

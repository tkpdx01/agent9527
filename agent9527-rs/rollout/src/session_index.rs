...
pub async fn append_thread_name(
    ...
    append_session_index_entry(agent9527_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(agent9527_home);
...
fn session_index_path(agent9527_home: &Path) -> PathBuf {
    agent9527_home.join(SESSION_INDEX_FILE)
}

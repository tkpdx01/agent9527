...
use crate::state_db;
use agent9527_file_search as file_search;
...
impl From<agent9527_state::Anchor> for Cursor {
    fn from(anchor: agent9527_state::Anchor) -> Self {
        let ts = anchor
            .ts
            .timestamp_nanos_opt()
...
pub async fn get_threads(
    ...
    let root = agent9527_home.join(SESSIONS_SUBDIR);
    get_threads_in_root(
        root,
        page_size,
        cursor,
...
///
/// Directory layout: `~/.agent9527/sessions/YYYY/MM/DD/rollout-YYYY-MM-DDThh-mm-ss-<uuid>.jsonl`
...
async fn find_thread_path_by_id_str_in_subdir(
    ...
    subdir: &str,
    id_str: &str,
    state_db_ctx: Option<&agent9527_state::StateRuntime>,
    ...
    let mut root = agent9527_home.to_path_buf();
    root.push(subdir);
    if !root.exists() {
...
pub async fn find_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(agent9527_home, SESSIONS_SUBDIR, id_str, state_db_ctx)
        .await
}

...
fn test_config(agent9527_home: &Path) -> RolloutConfig {
    RolloutConfig {
        agent9527_home: agent9527_home.to_path_buf(),
        sqlite: agent9527_state::SqliteConfig::new_for_testing(agent9527_home.abs()),
...
async fn state_db_init_backfills_before_returning() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        agent9527_state::BackfillStatus::Complete
    );

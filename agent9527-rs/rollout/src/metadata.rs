...
use agent9527_state::BackfillState;
use agent9527_state::BackfillStats;
use agent9527_state::BackfillStatus;
use agent9527_state::DB_ERROR_METRIC;
use agent9527_state::DB_METRIC_BACKFILL;
use agent9527_state::DB_METRIC_BACKFILL_DURATION_MS;
use agent9527_state::ExtractionOutcome;
use agent9527_state::ThreadMetadataBuilder;
use agent9527_state::apply_rollout_item;
use chrono::DateTime;
...
pub(crate) async fn backfill_sessions_with_lease(
    ...
    let metric_client = agent9527_otel::global();
    ...
    let sessions_root = agent9527_home.join(SESSIONS_SUBDIR);
    let archived_root = agent9527_home.join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(agent9527_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(agent9527_home: &Path, path: &Path) -> String {
    path.strip_prefix(agent9527_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}

...
async fn import_repo_mcp_preserves_existing_same_named_server() {
    ...
    .expect("write mcp");
    fs::create_dir_all(repo_root.join(".agent9527")).expect("create agent9527 dir");
    let existing_config = r#"[mcp_servers.mixedTransport]
url = "https://example.com/mixed-transport"
"#;
    fs::write(
        repo_root.join(".agent9527").join("config.toml"),
        existing_config,
    )
    .expect("write config");
    ...
    assert_eq!(
        fs::read_to_string(repo_root.join(".agent9527").join("config.toml")).expect("read config"),
        existing_config
    );
}
    ...
    assert_eq!(
        ...
        vec![ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                repo_root.display(),
                repo_root.join(".agent9527").join("config.toml").display()
            ),
            cwd: Some(repo_root),
...
async fn import_home_migrates_supported_config_fields_skills_and_agents_md() {
    let (_root, external_agent_home, agent9527_home) = fixture_paths();
    let agents_skills = agent9527_home
        .parent()
    ...
    fs::write(
            ...
            format!(r#"{{"model":"{SOURCE_EXTERNAL_AGENT_NAME}","permissions":{{"ask":["git push"]}},"env":{{"FOO":"bar","CI":false,"MAX_RETRIES":3,"MY_TEAM":"agent9527","IGNORED":null,"LIST":["a","b"],"MAP":{{"x":1}}}},"sandbox":{{"enabled":true,"network":{{"allowLocalBinding":true}}}}}}"#),
        )
    ...
    service_for_paths(external_agent_home, agent9527_home.clone())
        .import(vec![
    ...
    assert_eq!(
        fs::read_to_string(agent9527_home.join("AGENTS.md")).expect("read agents"),
        "Agent9527 guidance"
    );

    let config: TomlValue = toml::from_str(
        &fs::read_to_string(agent9527_home.join("config.toml")).expect("read config"),
    )
    .expect("parse config");
...
"#,
    ...
    assert_eq!(
        ...
        "Use Agent9527 and Agent9527 utilities."
    );
}
...
async fn import_home_skips_empty_config_migration() {
    ...
    let outcome = service_for_paths(external_agent_home, agent9527_home.clone())
        .import(vec![ExternalAgentConfigMigrationItem {
            item_type: ExternalAgentConfigMigrationItemType::Config,
            description: String::new(),
            cwd: None,
    ...
    assert!(!agent9527_home.join("config.toml").exists());
}
...
async fn import_local_plugins_returns_completed_status() {
    ...
    fs::create_dir_all(plugin_root.join(".agent9527-plugin")).expect("create plugin manifest dir");
    ...
    fs::write(
        plugin_root.join(".agent9527-plugin").join("plugin.json"),
    ...
    let config = fs::read_to_string(agent9527_home.join("config.toml")).expect("read config");
...
async fn detect_home_skips_config_when_target_already_has_supported_fields() {
    ...
    let items = service_for_paths(external_agent_home, agent9527_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: true,
            include_memory: false,
            cwds: None,

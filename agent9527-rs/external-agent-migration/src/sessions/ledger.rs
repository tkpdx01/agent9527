...
pub fn has_current_session_been_imported(
    ...
    load_import_ledger(agent9527_home)?.contains_current_source(source_path)
}
...
pub fn record_completed_session_imports(
    ...
    let mut ledger = load_import_ledger(agent9527_home)?;
    ...
    save_import_ledger(agent9527_home, &ledger)
}

pub fn read_imported_connector_candidates(
    agent9527_home: &Path,
) -> io::Result<Vec<ImportedConnectorCandidate>> {
    let ledger = load_import_ledger(agent9527_home)?;
...
pub(crate) fn load_import_ledger(
    ...
    let path = import_ledger_path(agent9527_home);
...
fn import_ledger_path(agent9527_home: &Path) -> PathBuf {
    agent9527_home.join(SESSION_IMPORT_LEDGER_FILE)
}

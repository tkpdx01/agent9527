...
use agent9527_backend_client::ConfigBundleResponse;
use agent9527_backend_client::DeliveredTomlFragment;
use agent9527_config::AbsolutePathBuf;
use agent9527_config::CloudConfigFragment;
use agent9527_config::CloudConfigTomlBundle;
use agent9527_config::CloudRequirementsFragment;
use agent9527_config::CloudRequirementsTomlBundle;
...
fn write_auth_json(agent9527_home: &Path, value: serde_json::Value) -> std::io::Result<()> {
    std::fs::write(
...
fn create_test_cache(agent9527_home: &Path) -> CloudConfigBundleCache {
...
async fn auth_manager_with_agent_identity_business_plan() -> Arc<AuthManager> {
    ...
    AuthManager::from_auth_for_testing(Agent9527Auth::AgentIdentity(
...
struct TestExternalChatgptAuth {
    current: RwLock<Agent9527Auth>,
    refreshed: Agent9527Auth,
...
async fn get_bundle_ignores_invalid_cache_and_refetches() {
    let agent9527_home = tempdir().expect("tempdir");
    let cache = create_test_cache(agent9527_home.path());
    cache
        .save(
...
async fn get_bundle_refreshes_external_auth_after_unauthorized() {
    ...
    let initial_auth = Agent9527Auth::from_external_chatgpt_tokens(
    ...
    let refreshed_auth = Agent9527Auth::from_external_chatgpt_tokens(
        &refreshed_token,
        "account-12345",
        Some("enterprise"),
    )
...
fn bundle_response_conversion_preserves_fragment_order() {
    let response = ConfigBundleResponse {
        config_toml: Some(Some(Box::new(
            agent9527_backend_client::DeliveredConfigToml {
        ...
        requirements_toml: Some(Some(Box::new(
            agent9527_backend_client::DeliveredRequirementsToml {

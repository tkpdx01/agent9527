...
use agent9527_http_client::RouteAwareRequestError;
...
pub(crate) async fn fetch_and_cache_remote_plugin_catalog(
    ...
    catalog_cache::write_cached_directory_plugins(agent9527_home, config, auth, scope, &plugins);
    Ok(())
}
...
pub async fn fetch_and_cache_global_remote_plugin_catalog(
    ...
    fetch_and_cache_remote_plugin_catalog(agent9527_home, config, auth, RemotePluginScope::Global)
        .await
}
...
pub fn invalidate_cached_remote_plugin_catalog_scopes(
    ...
    for scope in scopes {
        catalog_cache::remove_cached_directory_plugins(agent9527_home, config, auth, *scope);
    }
}
...
pub(crate) fn has_fresh_cached_remote_plugin_catalog(
    ...
    catalog_cache::load_cached_directory_plugins(agent9527_home, config, auth, scope).is_some_and(
        |cached| {
            matches!(
                cached.freshness,
...
pub(crate) fn cached_remote_plugin_catalog_scopes(
    ...
    RemotePluginScope::CATALOG_CACHE_SCOPES
        .into_iter()
        .filter(|scope| {
            catalog_cache::load_cached_directory_plugins(agent9527_home, config, auth, *scope)
                .is_some()
        })
        .collect()
}
...
fn remove_remote_plugin_cache(
    ...
    let store = PluginStore::try_new(agent9527_home.clone())
    ...
    let legacy_remote_plugin_cache_root = agent9527_home
...
async fn fetch_directory_plugins_for_scope_with_cache(
    ...
    if cache_mode == RemotePluginCatalogCacheMode::PreferCache
        && let Some(agent9527_home) = agent9527_home
        && let Some(cached) =
            catalog_cache::load_cached_directory_plugins(agent9527_home, config, auth, scope)
    {
...
fn ensure_chatgpt_auth(
    auth: Option<&Agent9527Auth>,
) -> Result<&Agent9527Auth, RemotePluginCatalogError> {

use agent9527_config::HooksFile;
use agent9527_utils_absolute_path::AbsolutePathBuf;
use agent9527_utils_path_uri::PathConvention;
use agent9527_utils_path_uri::PathUri;
use agent9527_utils_plugins::AGENT_PLUGIN_MANIFEST_RELATIVE_PATH;
use agent9527_utils_plugins::find_plugin_manifest_path;
...
pub type PluginManifest = agent9527_plugin::manifest::PluginManifest<AbsolutePathBuf>;
pub type PluginManifestHooks = agent9527_plugin::manifest::PluginManifestHooks<AbsolutePathBuf>;
pub type PluginManifestInterface =
    agent9527_plugin::manifest::PluginManifestInterface<AbsolutePathBuf>;
pub type PluginManifestMcpServers =
    agent9527_plugin::manifest::PluginManifestMcpServers<AbsolutePathBuf>;
pub type PluginManifestPaths = agent9527_plugin::manifest::PluginManifestPaths<AbsolutePathBuf>;

pub type UriPluginManifest = agent9527_plugin::manifest::PluginManifest<PathUri>;
...
pub fn load_plugin_manifest(plugin_root: &Path) -> Option<PluginManifest> {
    ...
    let overlay = if is_agent_plugin {
        let overlay_path = plugin_root.join(".agent9527-plugin/plugin.json");
...
fn resolve_raw_plugin_manifest(
    ...
    let interface = interface.and_then(|interface| {
        ...
        let interface = agent9527_plugin::manifest::PluginManifestInterface {
            display_name,
            short_description,
            long_description,
            developer_name,
    ...
    Ok(agent9527_plugin::manifest::PluginManifest {
        name,
        version,
        description,
        keywords,
        paths: agent9527_plugin::manifest::PluginManifestPaths {
...
fn resolve_manifest_hooks(
    plugin_root: &PathUri,
    hooks: Option<RawPluginManifestHooks>,
) -> Option<agent9527_plugin::manifest::PluginManifestHooks<PathUri>> {
    match hooks? {
        RawPluginManifestHooks::Path(path) => {
            resolve_manifest_path(plugin_root, "hooks", Some(&path))
                .map(|path| agent9527_plugin::manifest::PluginManifestHooks::Paths(vec![path]))
        }
        RawPluginManifestHooks::Paths(paths) => {
            let hooks = paths
                ...
                .collect::<Vec<_>>();
            (!hooks.is_empty()).then_some(agent9527_plugin::manifest::PluginManifestHooks::Paths(
                hooks,
            ))
        }
        RawPluginManifestHooks::Inline(hooks) => Some(
            agent9527_plugin::manifest::PluginManifestHooks::Inline(vec![*hooks]),
        ),
        RawPluginManifestHooks::InlineList(hooks) => (!hooks.is_empty()).then_some(
            agent9527_plugin::manifest::PluginManifestHooks::Inline(hooks),
        ),
...
fn resolve_manifest_mcp_servers(
    plugin_root: &PathUri,
    mcp_servers: Option<RawPluginManifestMcpServers>,
) -> Option<agent9527_plugin::manifest::PluginManifestMcpServers<PathUri>> {
    match mcp_servers? {
        RawPluginManifestMcpServers::Path(path) => {
            resolve_manifest_path(plugin_root, "mcpServers", Some(&path))
                .map(agent9527_plugin::manifest::PluginManifestMcpServers::Path)
        }
        RawPluginManifestMcpServers::Object(servers) => match serde_json::to_string(&servers) {
            Ok(servers) => {
                Some(agent9527_plugin::manifest::PluginManifestMcpServers::Object(servers))
            }
            Err(err) => {
...
mod tests {
    ...
    use agent9527_plugin::PluginProvider;
    use agent9527_plugin::ResolvedPlugin;
    use agent9527_plugin::manifest::PluginManifest as GenericPluginManifest;
    use agent9527_plugin::manifest::PluginManifestHooks;
    use agent9527_plugin::manifest::PluginManifestInterface;
    use agent9527_plugin::manifest::PluginManifestMcpServers;
    use agent9527_plugin::manifest::PluginManifestPaths;
    ...
    fn write_manifest(plugin_root: &Path, version: Option<&str>, interface: &str) {
        fs::create_dir_all(plugin_root.join(".agent9527-plugin")).expect("create manifest dir");
        let version = version
  ...
  "interface": {{
    ...
    "composerIcon": "./assets/icon.svg"
        ...
        let manifest_contents =
            fs::read_to_string(plugin_root.join(".agent9527-plugin/plugin.json"))
                .expect("manifest");

...
use agent9527_config::types::PluginMcpServerConfig;
use agent9527_connectors::parse_plugin_app_config;
use agent9527_connectors::parse_plugin_app_config_value;
use agent9527_core_skills::PluginSkillSnapshots;
use agent9527_core_skills::config_rules::resolve_disabled_skill_paths;
...
use agent9527_mcp::parse_plugin_mcp_config;
use agent9527_plugin::AppDeclaration;
use agent9527_plugin::LoadedPlugin;
...
async fn load_plugins_from_layer_stack_with_scope(
    ...
    let configured_plugins = merge_configured_plugins_with_remote_installed(
        configured_plugins_from_stack(config_layer_stack, store.agent9527_home().as_path()),
        extra_plugins,
        store,
        remote_global_catalog_active,
    );
...
pub fn refresh_curated_plugin_cache(
    ...
    let store =
        PluginStore::try_new(agent9527_home.to_path_buf()).map_err(|err| err.to_string())?;
    let curated_marketplace_paths = curated_marketplace_paths_for_cache_refresh(agent9527_home)?;
...
fn curated_marketplace_paths_for_cache_refresh(
    ...
    let api_marketplace_path = agent9527_home
        .join(".tmp/plugins")
...
fn refresh_non_curated_plugin_cache_with_mode(
    ...
    for plugin_id in configured_non_curated_plugin_ids {
        ...
        let refresh_result = (|| -> Result<bool, String> {
            let materialized = materialize_marketplace_plugin_source(agent9527_home, &source)
                .map_err(|err| {
...
fn configured_plugins_from_agent9527_home(
    agent9527_home: &Path,
...
pub fn configured_curated_plugin_ids_from_agent9527_home(agent9527_home: &Path) -> Vec<PluginId> {
    curated_plugin_ids_from_config_keys(configured_plugins_from_agent9527_home(
        agent9527_home,
    ...
    match &manifest_paths.hooks {
        ...
        Some(PluginManifestHooks::Inline(hooks_files)) => {
            let manifest_path = find_plugin_manifest_path(plugin_root.as_path())
                ...
                .unwrap_or_else(|| plugin_root.join(".agent9527-plugin/plugin.json"));
...
pub fn materialize_marketplace_plugin_source(
    ...
    match source {
        ...
        MarketplacePluginSource::Git {
            ...
            let staging_root = agent9527_home.join("plugins/.marketplace-plugin-source-staging");

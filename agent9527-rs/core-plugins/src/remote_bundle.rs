...
pub enum RemotePluginBundleInstallError {
    ...
    DownloadBody {
        url: String,
        #[source]
        source: agent9527_http_client::HttpError,
    },
...
pub async fn download_and_install_remote_plugin_bundle(
    ...
    tokio::task::spawn_blocking(move || {
        install_remote_plugin_bundle(agent9527_home, bundle, bundle_bytes)
    })
    .await
    .map_err(|err| {
...
fn install_remote_plugin_bundle(
    ...
    let staging_root = agent9527_home.join(REMOTE_PLUGIN_INSTALL_STAGING_DIR);
    ...
    let store = PluginStore::try_new(agent9527_home)?;
...
mod tests {
    ...
    fn install_persists_remote_plugin_install_metadata() {
        ...
        let store = PluginStore::new(agent9527_home.path().to_path_buf());
        ...
        let metadata_path = store
            ...
            .join(".agent9527-remote-plugin-install.json");
        assert_eq!(
    ...
    fn find_extracted_plugin_root_uses_local_manifest_discovery() {
        ...
        std::fs::create_dir_all(extraction_root.path().join(".agent9527-plugin"))
            .expect("create manifest dir");
        std::fs::write(
            extraction_root.path().join(".agent9527-plugin/plugin.json"),
            r#"{"name":"linear"}"#,
        )

...
pub(crate) fn maybe_start_remote_installed_plugin_bundle_sync(
    ...
    let key = RemoteInstalledPluginBundleSyncKey {
        plugin_cache_root: remote_plugin_cache_root(&agent9527_home),
    };
    ...
    tokio::spawn(async move {
        let result =
            sync_remote_installed_plugin_bundles_once(agent9527_home, &config, Some(&auth)).await;
        match result {
            Ok(outcome) => {
                info!(
...
fn remote_plugin_cache_root(agent9527_home: &Path) -> PathBuf {
    agent9527_home.join(PLUGINS_CACHE_DIR)
}
...
mod tests {
    ...
    fn remote_installed_plugin_sync_in_flight_dedupes_by_cache_root() {
        ...
        let key = RemoteInstalledPluginBundleSyncKey {
            plugin_cache_root: remote_plugin_cache_root(agent9527_home.path()),
        };
    ...
    async fn sync_same_version_backfills_metadata_without_materialization() {
        ...
        let agent9527_home = tempfile::tempdir().expect("create agent9527 home");
        let cached_manifest = agent9527_home
            .path()
            .join(PLUGINS_CACHE_DIR)
        ...
        let metadata_path = PluginStore::new(agent9527_home.path().to_path_buf())
    ...
    fn stale_remote_plugin_cleanup_removes_stale_marketplace_caches_and_keeps_canonical_cache() {
        let agent9527_home = tempfile::tempdir().expect("create agent9527 home");
        let created_by_me_cached_manifest = agent9527_home
            .path()
            .join(PLUGINS_CACHE_DIR)
        ...
        std::fs::write(&cached_manifest, r#"{"name":"private-plugin"}"#)
            .expect("write cached plugin manifest");
        let canonical_cached_manifest = agent9527_home
            .path()
            .join(PLUGINS_CACHE_DIR)

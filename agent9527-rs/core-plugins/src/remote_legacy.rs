...
pub async fn fetch_remote_featured_plugin_ids(
    ...
    url.query_pairs_mut().append_pair(
        "platform",
        product.unwrap_or(Product::Agent9527).to_app_platform(),
    );
    ...
    if let Some(auth) = auth.filter(|auth| auth.uses_agent9527_backend()) {
        request = request
...
fn ensure_agent9527_backend_auth(
    auth: Option<&Agent9527Auth>,
) -> Result<&Agent9527Auth, RemotePluginMutationError> {
...
async fn post_remote_plugin_mutation(
    ...
    let auth = ensure_agent9527_backend_auth(auth)?;

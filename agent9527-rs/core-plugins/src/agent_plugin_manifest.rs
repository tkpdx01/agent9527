...
use agent9527_utils_plugins::AGENT_PLUGIN_SCHEMA_PREFIX;
use agent9527_utils_plugins::AGENT_PLUGIN_SCHEMA_URI;
use agent9527_utils_plugins::SUPPORTED_AGENT_PLUGIN_SCHEMA_URIS;
use serde::Deserialize;
use serde_json::Value as JsonValue;

const AGENT9527_AGENT_PLUGIN_EXTENSION_NAMESPACE: &str = "com.openai";
...
pub(super) fn parse_agent_plugin_manifest_uri(
    ...
    let agent9527_extension = object
        .get("extensions")
        .and_then(JsonValue::as_object)
        .and_then(|extensions| extensions.get(AGENT9527_AGENT_PLUGIN_EXTENSION_NAMESPACE))
        .and_then(|extension| {
            if extension.is_object() {
                ...
                tracing::warn!(
                    path = %manifest_path,
                    namespace = AGENT9527_AGENT_PLUGIN_EXTENSION_NAMESPACE,
    ...
    if let Some(extension_contents) = agent9527_extension.as_deref() {
        apply_agent9527_agent_plugin_extension(
            &mut resolved,
            plugin_root,
            manifest_path,
            extension_contents,
...
fn apply_agent9527_agent_plugin_extension(

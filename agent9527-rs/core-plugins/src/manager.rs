...
use crate::loader::configured_curated_plugin_ids_from_agent9527_home;
...
use agent9527_config::clear_user_plugin;
use agent9527_config::set_user_plugin_enabled;
use agent9527_config::types::PluginConfig;
...
use agent9527_core_skills::config_rules::skill_config_rules_from_stack;
use agent9527_core_skills::loader::MAX_CONCURRENT_ROOT_SCANS;
use agent9527_hooks::plugin_hook_declarations;
use agent9527_http_client::HttpClientFactory;
use agent9527_login::Agent9527Auth;
use agent9527_login::AuthManager;
use agent9527_model_provider::AMAZON_BEDROCK_PROVIDER_ID;
use agent9527_plugin::AppConnectorId;
use agent9527_plugin::PluginCapabilitySummary;
use agent9527_plugin::PluginId;
use agent9527_plugin::PluginIdError;
use agent9527_plugin::PluginTelemetryMetadata;
use agent9527_plugin::app_connector_ids_from_declarations;
use agent9527_plugin::prompt_safe_plugin_description;
...
use agent9527_skills::SkillMetadata;
use agent9527_tools::DiscoverablePluginInfo;
use agent9527_tools::DiscoverableTool;
...
use agent9527_utils_plugins::PluginIdentity;
...
pub struct RecommendedPluginCandidatesInput<'a> {
    ...
    pub auth: Option<&'a Agent9527Auth>,
...
impl RemoteCatalogCacheRefreshRequest {
    fn has_same_cache_identity(&self, other: &Self) -> bool {
        self.service_config == other.service_config
            && self.auth.as_ref().and_then(Agent9527Auth::get_account_id)
                == other.auth.as_ref().and_then(Agent9527Auth::get_account_id)
            && self
                .auth
                .as_ref()
                .and_then(Agent9527Auth::get_chatgpt_user_id)
                == other
                    .auth
                    .as_ref()
                    .and_then(Agent9527Auth::get_chatgpt_user_id)
            && self.auth.as_ref().map(Agent9527Auth::is_workspace_account)
                == other.auth.as_ref().map(Agent9527Auth::is_workspace_account)
    }
}
...
fn featured_plugin_ids_cache_key(
    ...
    FeaturedPluginIdsCacheKey {
        ...
        account_id: auth.and_then(Agent9527Auth::get_account_id),
        chatgpt_user_id: auth.and_then(Agent9527Auth::get_chatgpt_user_id),
        is_workspace_account: auth.is_some_and(Agent9527Auth::is_workspace_account),
    }
}
...
impl PluginsManager {
    pub fn new(agent9527_home: PathBuf) -> Self {
    ...
    pub fn new_with_options(
        ...
        // Product restrictions are enforced at marketplace admission time for a given AGENT9527_HOME:
        ...
        // contents of that AGENT9527_HOME and does not re-filter configured plugins by product, so
        // already-admitted plugins may continue exposing MCP servers/tools from shared local state.
        //
        // This assumes a single AGENT9527_HOME is only used by one product.
        Self {
            agent9527_home: agent9527_home.clone(),
            store: PluginStore::new(agent9527_home),
    ...
    pub fn cached_global_remote_discoverable_plugins_for_config(
        ...
        let Some(auth) = auth.filter(|auth| auth.uses_agent9527_backend()) else {
            return Vec::new();
        };
    ...
    async fn install_resolved_plugin(
        ...
        let plugin_version =
            if is_openai_curated_marketplace_name(&resolved.plugin_id.marketplace_name) {
                let curated_plugin_version =
                    read_curated_plugins_sha(self.agent9527_home.as_path()).ok_or_else(|| {
        ...
        let agent9527_home = self.agent9527_home.clone();
        ...
        let result: StorePluginInstallResult = tokio::task::spawn_blocking(move || {
            let materialized =
                materialize_marketplace_plugin_source(agent9527_home.as_path(), &resolved.source)
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        ...
        clear_user_plugin(&self.agent9527_home, plugin_id.as_key())
            .await
            .map_err(anyhow::Error::from)?;
    ...
    pub async fn read_plugin_detail_for_marketplace_plugin(
        ...
        let source_path = if plugin.source.is_install_materialized() && plugin.installed {
            ...
            let materialized = tokio::task::spawn_blocking(move || {
                materialize_marketplace_plugin_source(agent9527_home.as_path(), &source)
            })
            .await
            .map_err(|err| {
                MarketplaceError::InvalidPlugin(format!(
        ...
        let manifest = if agent9527_utils_plugins::find_plugin_manifest_path(source_path.as_path())
            .is_some()
        {
        ...
        let resolved_skills = load_plugin_skills_with_identity(
            ...
            &manifest,
            self.restriction_product,
            &agent9527_core_skills::config_rules::skill_config_rules_from_stack(
                &config.config_layer_stack,
            ),
            /*plugin_skill_snapshots*/ None,
            Arc::clone(&self.skill_root_scan_slots),
    ...
    pub fn maybe_start_plugin_startup_tasks_for_config(
        ...
        if config.plugins_enabled {
            let use_remote_global_catalog =
                config.remote_plugin_enabled && auth_manager.current_auth_uses_agent9527_backend();
            ...
            tokio::spawn(async move {
                ...
                let mut scopes = crate::remote::cached_remote_plugin_catalog_scopes(
                    manager.agent9527_home.as_path(),
    ...
    fn start_curated_repo_sync(self: &Arc<Self>, http_client_factory: HttpClientFactory) {
        ...
        if let Err(err) = std::thread::Builder::new()
            ...
            .spawn(move || {
                match sync_openai_plugins_repo(agent9527_home.as_path(), http_client_factory) {
                    Ok(curated_plugin_version) => {
                        let configured_curated_plugin_ids =
                            configured_curated_plugin_ids_from_agent9527_home(
        ...
        let curated_marketplace_path = if include_openai_curated {
                ...
                )
            {
                let api_marketplace_path =
                    curated_plugins_api_marketplace_path(self.agent9527_home.as_path());
                api_marketplace_path
                    .is_file()
                    .then_some(api_marketplace_path)
            } else {
                let curated_repo_root = curated_plugins_repo_path(self.agent9527_home.as_path());

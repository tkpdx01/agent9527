...
use agent9527_plugin::PluginLoadOutcome;
...
fn synced_plugin_root(
    ...
    let synced_root = curated_plugins_repo_path(agent9527_home);
    match marketplace {
    ...
    let root = PluginStore::new(agent9527_home.to_path_buf()).plugin_root(
        &plugin_id,
...
fn cached_remote_plugin_root(agent9527_home: &Path, plugin_name: &str) -> AbsolutePathBuf {
    let plugin_id = PluginId::new(
        plugin_name.to_string(),
        REMOTE_GLOBAL_MARKETPLACE_NAME.to_string(),
    )
    .expect("plugin id");
    let root = PluginStore::new(agent9527_home.to_path_buf()).plugin_root(&plugin_id, "1.2.3");
    fs::create_dir_all(root.as_path()).expect("create cached remote plugin root");
    root
}
fn installed_remote_plugin_root(agent9527_home: &Path, plugin_name: &str) -> AbsolutePathBuf {
    let root = cached_remote_plugin_root(agent9527_home, plugin_name);
    let plugin_id = PluginId::new(
        plugin_name.to_string(),
        REMOTE_GLOBAL_MARKETPLACE_NAME.to_string(),
    )
    .expect("plugin id");
    PluginStore::new(agent9527_home.to_path_buf())
...
fn roots_for(agent9527_home: &Path, plugins: Vec<LoadedPlugin>) -> TrustedPluginRoots {
    TrustedPluginRoots::from_plugin_load_outcome(
        &PluginLoadOutcome::from_plugins(plugins),
        agent9527_home,
    )
}
fn assert_untrusted(agent9527_home: &Path, config_name: &str, root: &Path) {
    assert!(
        roots_for(
            agent9527_home,
...
fn only_emits_safe_normalized_relative_script_paths() {
    ...
    assert_eq!(
        normalized_relative_script_path(Path::new(
            "/home/user/.agent9527/plugins/cache/openai-curated/sample/scripts/run.py"
        )),
        None
    );
}

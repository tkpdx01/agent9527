...
#[test]
fn legacy_agent9527_overlay_keeps_portable_components_fixed() {
    let tmp = tempdir().expect("tempdir");
    let plugin_root = tmp.path().join("demo-plugin");
    write_agent_plugin_manifest(
        &plugin_root,
  ...
  "description": "Portable description""#,
    ...
    fs::create_dir_all(plugin_root.join(".agent9527-plugin")).expect("create overlay dir");
    fs::write(
        plugin_root.join(".agent9527-plugin/plugin.json"),
        r#"{
  "name": "different-name",
  "version": "9.9.9",
  "description": "Agent9527 description",
  "skills": [],
  "mcpServers": null,
  "interface": {"displayName": "Agent9527 Demo"}
}"#,
    )
...
fn inline_openai_extension_precedes_legacy_overlay() {
  ...
  "extensions": {
    "com.openai": {
      "interface": {"displayName": "Inline Agent9527"}
    }
  }"#,
    );
    fs::create_dir_all(plugin_root.join(".agent9527-plugin")).expect("create overlay dir");
    fs::write(
        plugin_root.join(".agent9527-plugin/plugin.json"),
        r#"{"interface":{"displayName":"Legacy Agent9527"}}"#,
    )
    .expect("write overlay");
    ...
    assert_eq!(
        manifest
            .interface
            .and_then(|interface| interface.display_name),
        Some("Inline Agent9527".to_string())
    );
}

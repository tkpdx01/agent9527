...
const KEYRING_SERVICE: &str = "agent9527";
...
impl SecretsManager {
    pub fn new(agent9527_home: PathBuf, backend_kind: SecretsBackendKind) -> Self {
        let backend: Arc<dyn SecretsBackend> = match backend_kind {
            SecretsBackendKind::Local => {
                let keyring_store: Arc<dyn KeyringStore> = Arc::new(DefaultKeyringStore);
                Arc::new(LocalSecretsBackend::new(agent9527_home, keyring_store))
            }
        };
        Self { backend }
    }
...
pub fn compute_keyring_account(agent9527_home: &Path) -> String {

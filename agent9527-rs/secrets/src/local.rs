...
const AGENT9527_AUTH_SECRETS_FILENAME: &str = "agent9527_auth.age";
...
pub enum LocalSecretsNamespace {
    ...
    #[default]
    ManagedSecrets,
    /// Agent9527 authentication credentials used by the CLI, TUI, app server, and other clients.
    Agent9527Auth,
...
impl LocalSecretsBackend {
    pub fn new(agent9527_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
    ...
    fn secrets_dir(&self) -> PathBuf {
        self.agent9527_home.join("secrets")
    }

    fn secrets_path(&self) -> PathBuf {
        let filename = match self.namespace {
            LocalSecretsNamespace::ManagedSecrets => LOCAL_SECRETS_FILENAME,
            LocalSecretsNamespace::Agent9527Auth => AGENT9527_AUTH_SECRETS_FILENAME,
    ...
    fn load_or_create_passphrase(&self) -> Result<SecretString> {
        let account = compute_keyring_account(&self.agent9527_home);
        let loaded = self
            .keyring_store
...
mod tests {
    ...
    fn load_file_rejects_newer_schema_versions() -> Result<()> {
        ...
        let backend = LocalSecretsBackend::new(agent9527_home.path().to_path_buf(), keyring);
    ...
    fn set_fails_when_keyring_is_unavailable() -> Result<()> {
        let agent9527_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let account = compute_keyring_account(agent9527_home.path());
        keyring.set_error(
            &account,
    ...
    fn local_namespaces_write_separate_files() -> Result<()> {
        let agent9527_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let agent9527_auth_backend = LocalSecretsBackend::new_with_namespace(
        ...
        agent9527_auth_backend.set(&scope, &name, "agent9527-auth-value")?;
        mcp_backend.set(&scope, &name, "mcp-value")?;

        assert_eq!(
            agent9527_auth_backend.get(&scope, &name)?,
            Some("agent9527-auth-value".to_string())
        );
        assert_eq!(

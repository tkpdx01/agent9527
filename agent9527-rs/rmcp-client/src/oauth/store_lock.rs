...
const LOCK_CONTENTION_EVENT_TARGET: &str = "agent9527_rmcp_client::oauth::store_lock::contention";
...
impl OAuthStoreLock {
    pub(super) fn acquire(store: OAuthStore) -> Result<Self, OAuthStoreLockFailure> {
        ...
        // authority. Those stores are AGENT9527_HOME-backed today: if AGENT9527_HOME is unset they use
        // the default home (`~/.agent9527`), and if an embedder has no local home/filesystem authority
        ...
        let agent9527_home = find_agent9527_home()
            .map_err(|source| OAuthStoreLockFailure::Agent9527Home { store, source })?;
        Self::acquire_in(&agent9527_home, store, STORE_LOCK_ACQUIRE_TIMEOUT)
    }
    ...
    pub(super) fn acquire_in(
        ...
        let path = oauth_store_lock_path(agent9527_home, store);
...
pub(super) enum OAuthStoreLockFailure {
    #[error("failed to resolve AGENT9527_HOME for MCP OAuth {store} aggregate-store lock")]
    Agent9527Home {
        store: OAuthStore,
        #[source]
        source: io::Error,
...
fn oauth_store_lock_path(agent9527_home: &Path, store: OAuthStore) -> PathBuf {
    agent9527_home
        .join(OAUTH_LOCK_DIR)

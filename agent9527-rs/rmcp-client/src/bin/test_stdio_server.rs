...
const MEMO_URI: &str = "memo://agent9527/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const SANDBOX_STATE_META_CAPABILITY: &str = "agent9527/sandbox-state-meta";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of Agent9527 TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Agent9527 TUI):
    /// - Build this binary: `cargo build -p agent9527-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `agent9527 mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Agent9527 TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            uri_template: "memo://agent9527/{slug}".to_string(),
            name: "agent9527-memo".to_string(),
            title: Some("Agent9527 Memo".to_string()),
            description: Some(
                "Template for memo://agent9527/{slug} resources used in tests.".to_string(),
            ),
            mime_type: Some("text/plain".to_string()),
            icons: None,
        };
...
/// Scenarios for `image_scenario`, intended to exercise Agent9527 TUI handling of MCP image outputs.
///
    ...
    fn get_info(&self) -> ServerInfo {
        ...
        match dynamic_server_process_label() {
            Some(process_label) => server_info
                .with_server_info(
                    Implementation::new("agent9527-rmcp-test-server", env!("CARGO_PKG_VERSION"))
    ...
    async fn call_tool(
        ...
        match request.name.as_ref() {
            ...
            "encrypted_output" => {
                ...
                meta.insert("agent9527/encryptedContent".to_string(), json!(true));
    ...
    fn image_scenario_result(args: ImageScenarioArgs) -> Result<CallToolResult, McpError> {
        ...
        match args.scenario {
            ...
            ImageScenario::ImageOnlyOriginalDetail => {
                ...
                meta.insert(
                    "agent9527/imageDetail".to_string(),

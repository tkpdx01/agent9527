...
use agent9527_core::spawn::AGENT9527_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use agent9527_mcp_server::Agent9527ToolCallParam;
use agent9527_mcp_server::ExecApprovalElicitRequestParams;
use agent9527_mcp_server::ExecApprovalResponse;
use agent9527_mcp_server::PatchApprovalElicitRequestParams;
use agent9527_mcp_server::PatchApprovalResponse;
use agent9527_protocol::protocol::FileChange;
use agent9527_protocol::protocol::ReviewDecision;
use agent9527_shell_command::parse_command;
...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(AGENT9527_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Agent9527 sandbox."
        );
        return;
    }
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "agent9527" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let agent9527_request_id = mcp_process
        .send_agent9527_tool_call(Agent9527ToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            agent9527_request_id.to_string(),
            params.agent9527_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    // Verify the original `agent9527` tool call completes and that the file was created.
    let agent9527_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(agent9527_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(agent9527_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    agent9527_mcp_tool_call_id: String,
    agent9527_event_id: String,
    thread_id: agent9527_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow Agent9527 to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let agent9527_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        agent9527_elicitation: "exec-approval".to_string(),
        agent9527_mcp_tool_call_id,
        agent9527_event_id,
        agent9527_command: command,
        agent9527_cwd: workdir.to_path_buf(),
        agent9527_call_id: "call1234".to_string(),
        agent9527_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "agent9527" tool request that will trigger the apply_patch command
    let agent9527_request_id = mcp_process
        .send_agent9527_tool_call(Agent9527ToolCallParam {
    ...
    // Verify the original `agent9527` tool call completes
    let agent9527_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(agent9527_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_agent9527_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    agent9527_tool_passes_base_instructions()
        .await
        .expect("agent9527 tool should pass base instructions");
}

async fn agent9527_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `agent9527 mcp` with a specific config.toml.
    ...
    let mut mcp_process = McpProcess::new_with_env(
        agent9527_home.path(),
        &[("OPENAI_API_KEY", None), ("AGENT9527_ACCESS_TOKEN", None)],
    )
    .await?;
    ...
    .await??;
    assert_eq!(agent9527_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(
        agent9527_response.id,
        RequestId::Number(agent9527_request_id)
    );
    assert_eq!(
        agent9527_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
            ...
            "structuredContent": {
                "threadId": agent9527_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("agent9527 tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
    ...
    assert_eq!(
        developer_text
            .matches("Co-authored-by: Agent9527 <noreply@openai.com>")
            .count(),
        1
    );
    assert_eq!(
        developer_text.matches("Generated with Agent9527.").count(),
        1
    );
    assert!(
        developer_contents
...
async fn create_mcp_process(responses: Vec<String>) -> anyhow::Result<McpHandle> {
    ...
    let agent9527_home = TempDir::new()?;
    create_config_toml(agent9527_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(agent9527_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp_process.initialize()).await??;
    Ok(McpHandle {
        process: mcp_process,
        server,
...
/// Create a Agent9527 config that uses the mock server as the model provider.
/// It also uses `approval_policy = "untrusted"` so that we exercise the
/// elicitation code path for shell commands.
fn create_config_toml(agent9527_home: &Path, server_uri: &str) -> std::io::Result<()> {

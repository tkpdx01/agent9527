...
impl McpProcess {
    pub async fn new(agent9527_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(agent9527_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = agent9527_utils_cargo_bin::cargo_bin("agent9527-mcp-server")
            .context("should find binary for agent9527-mcp-server")?;
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = agent9527_login::default_client::originator().value;
    ...
    pub async fn send_agent9527_tool_call(
        &mut self,
        params: Agent9527ToolCallParam,
    ) -> anyhow::Result<i64> {
        let agent9527_tool_call_params = CallToolRequestParams::new("agent9527").with_arguments(
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(agent9527_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "agent9527/event" with params.msg.type == "task_complete".
        ...
        loop {
            ...
            match message {
                JsonRpcMessage::Notification(notification) => {
                    let is_match = if notification.notification.method == "agent9527/event" {
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `agent9527-mcp-server` child process.
        //

...
use agent9527_login::default_client::USER_AGENT_SUFFIX;
...
use crate::agent9527_tool_config::Agent9527ToolCallParam;
use crate::agent9527_tool_config::Agent9527ToolCallReplyParam;
use crate::agent9527_tool_config::create_tool_for_agent9527_tool_call_param;
use crate::agent9527_tool_config::create_tool_for_agent9527_tool_call_reply_param;
...
pub(crate) struct MessageProcessor {
    ...
    running_requests_id_to_agent9527_uuid: Arc<Mutex<HashMap<RequestId, ThreadId>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) async fn new(
        ...
        agent9527_image_generation_extension::install(
            &mut extensions,
            auth_manager.clone(),
            |config: &Config| Some(config.agent9527_home.clone()),
        );
        let thread_manager = Arc::new(ThreadManager::new(
            config.as_ref(),
            Arc::clone(&auth_manager),
            agent9527_core::build_models_manager(config.as_ref(), auth_manager),
            ...
            agent9527_core::thread_store_from_config(config.as_ref(), state_db.clone()),
        ...
        Self {
            ...
            arg0_paths,
            thread_manager,
            running_requests_id_to_agent9527_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_initialize(
        ...
        let server_info = Implementation::new("agent9527-mcp-server", env!("CARGO_PKG_VERSION"))
            .with_title("Agent9527");

        // Preserve Agent9527's existing non-spec `serverInfo.user_agent` field.
        ...
        if let serde_json::Value::Object(ref mut obj) = server_info_value {
            obj.insert("user_agent".to_string(), json!(get_agent9527_user_agent()));
        }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult {
            meta: None,
            tools: vec![
                create_tool_for_agent9527_tool_call_param(),
                create_tool_for_agent9527_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParams) {
        ...
        match name.as_ref() {
            "agent9527" => self.handle_tool_call_agent9527(id, arguments).await,
            "agent9527-reply" => {
                self.handle_tool_call_agent9527_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_agent9527(
        &self,
        id: RequestId,
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<Agent9527ToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg.into_config(self.arg0_paths.clone()).await {
                    ...
                    Err(e) => {
                        let result = CallToolResult::error(vec![rmcp::model::Content::text(
                            format!("Failed to load Agent9527 configuration from overrides: {e}"),
                        )]);
                        self.outgoing.send_response(id, result).await;
                        return;
                    }
                ...
                Err(e) => {
                    let result = CallToolResult::error(vec![rmcp::model::Content::text(format!(
                        "Failed to parse configuration for Agent9527 tool: {e}"
                    ))]);
                    self.outgoing.send_response(id, result).await;
                    return;
                }
            ...
            None => {
                let result = CallToolResult::error(vec![rmcp::model::Content::text(
                    "Missing arguments for agent9527 tool-call; the `prompt` field is required.",
                )]);
                self.outgoing.send_response(id, result).await;
                return;
            }
        ...
        let running_requests_id_to_agent9527_uuid =
            self.running_requests_id_to_agent9527_uuid.clone();

        // Spawn an async task to handle the Agent9527 session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Agent9527 session and stream events back to the client.
            crate::agent9527_tool_runner::run_agent9527_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
                thread_manager,
                running_requests_id_to_agent9527_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_agent9527_session_reply(
        &self,
        ...
        // parse arguments
        let agent9527_tool_call_reply_param: Agent9527ToolCallReplyParam = match arguments {
            Some(json_val) => match serde_json::from_value::<Agent9527ToolCallReplyParam>(json_val)
            {
                Ok(params) => params,
                Err(e) => {
                    tracing::error!("Failed to parse Agent9527 tool call reply parameters: {e}");
                    let result = CallToolResult::error(vec![rmcp::model::Content::text(format!(
                        "Failed to parse configuration for Agent9527 tool: {e}"
                    ))]);
            ...
            None => {
                tracing::error!(
                    "Missing arguments for agent9527-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult::error(vec![rmcp::model::Content::text(
                    "Missing arguments for agent9527-reply tool-call; the `thread_id` and `prompt` fields are required.",
                )]);
        ...
        let thread_id = match agent9527_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let agent9527 = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                let result = crate::agent9527_tool_runner::create_call_tool_result_with_thread_id(
                    thread_id,
        ...
        let prompt = agent9527_tool_call_reply_param.prompt.clone();
        tokio::spawn({
            let outgoing = outgoing.clone();
            let running_requests_id_to_agent9527_uuid =
                running_requests_id_to_agent9527_uuid.clone();

            async move {
                crate::agent9527_tool_runner::run_agent9527_tool_session_reply(
                    thread_id,
                    agent9527,
                    outgoing,
                    request_id,
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        let thread_id = {
            let map_guard = self.running_requests_id_to_agent9527_uuid.lock().await;
        ...
        // Obtain the Agent9527 thread from the server.
        let agent9527_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
        ...
        // Submit interrupt to Agent9527.
        if let Err(e) = agent9527_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: agent9527_protocol::protocol::Op::Interrupt,
                client_user_message_id: None,
                trace: None,
            })
            .await
        {
            tracing::error!("Failed to submit interrupt to Agent9527: {e}");
            return;
        }
        // unregister the id so we don't keep it in the map
        self.running_requests_id_to_agent9527_uuid
            .lock()
            .await
            .remove(&request_id);
    }

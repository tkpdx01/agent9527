...
pub struct ExecApprovalElicitRequestParams {
    ...
    // correlate the request with the agent9527 tool call.
    ...
    pub agent9527_command: Vec<String>,
    pub agent9527_cwd: PathBuf,
    pub agent9527_parsed_cmd: Vec<ParsedCommand>,
}
...
pub(crate) async fn handle_exec_approval_request(
    ...
    call_id: String,
    approval_id: String,
    agent9527_parsed_cmd: Vec<ParsedCommand>,
    thread_id: ThreadId,
) {
    let escaped_command =
        shlex::try_join(command.iter().map(String::as_str)).unwrap_or_else(|_| command.join(" "));
    let message = format!(
        "Allow Agent9527 to run `{escaped_command}` in `{cwd}`?",
    ...
    // Listen for the response on a separate task so we don't block the main agent loop.
    {
        ...
        tokio::spawn(async move {
            on_exec_approval_response(approval_id, event_id, on_response, agent9527).await;
        });
    }
}
...
async fn on_exec_approval_response(
    ...
    // Try to deserialize `value` and then make the appropriate call to `agent9527`.

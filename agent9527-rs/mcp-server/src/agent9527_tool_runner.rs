//! Asynchronous worker that executes a **Agent9527** tool-call inside a spawned
...
use agent9527_core::config::Config as Agent9527Config;
...
/// To adhere to MCP `tools/call` response format, include the Agent9527
...
/// Run a complete Agent9527 session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_agent9527_tool_session(
    id: RequestId,
    initial_prompt: String,
    config: Agent9527Config,
    ...
    let NewThread {
        ...
        Err(e) => {
            let result = CallToolResult::error(vec![Content::text(format!(
                "Failed to start Agent9527 session: {e}"
            ))]);
    ...
    // Use the original MCP request ID as the `sub_id` for the Agent9527 submission so that
    ...
    run_agent9527_tool_session_inner(
        thread_id,
        thread,
        outgoing,
        id,
...
pub async fn run_agent9527_tool_session_reply(
    thread_id: ThreadId,
...
async fn run_agent9527_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<Agent9527Thread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
    ...
    loop {
        match thread.next_event().await {
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(agent9527_event_to_notification(&event)) above has
            ...
            Err(e) => {
                let result = create_call_tool_result_with_thread_id(
                    thread_id,
                    format!("Agent9527 runtime error: {e}"),
                    Some(true),
                );
                outgoing.send_response(request_id.clone(), result).await;
                break;

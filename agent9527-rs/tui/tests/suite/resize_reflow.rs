...
#[ignore = "requires tmux and a locally built agent9527 binary; run with --ignored for manual resize smoke"]
async fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let agent9527 = agent9527_binary(&repo_root)?;
    ...
    write_config(agent9527_home.path(), &repo_root)?;
    write_auth(agent9527_home.path())?;

    let session_name = format!("agent9527-resize-reflow-smoke-{}", std::process::id());
    ...
    let agent9527_pane = stdout_text(&start_output).trim().to_string();
    anyhow::ensure!(!agent9527_pane.is_empty(), "tmux did not report a pane id");
    ...
    let baseline_capture =
        wait_for_capture_contains(&agent9527_pane, draft, Duration::from_secs(/*secs*/ 15))?;
    ...
    let final_capture = capture_pane(&agent9527_pane)?;
    let final_row =
...
async fn tmux_width_resize_restore_keeps_visible_content_anchored() -> Result<()> {
    ...
    let session_name = format!("agent9527-resize-width-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    sleep(Duration::from_millis(/*millis*/ 1_000));
    let restored_capture = capture_pane(&agent9527_pane)?;
    let restored_row =
...
async fn run_repeated_resize_smoke() -> Result<()> {
    ...
    let session_name = format!("agent9527-resize-repeat-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
...
fn agent9527_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = agent9527_utils_cargo_bin::cargo_bin("agent9527") {
        return Ok(path);
    }

    let fallback = repo_root.join("agent9527-rs/target/debug/agent9527");
    anyhow::ensure!(
        fallback.is_file(),
        "agent9527 binary is unavailable; run `cargo build -p agent9527-cli` first"
    );
    Ok(fallback)
}

fn write_config(agent9527_home: &Path, repo_root: &Path) -> Result<()> {
...
"#
    ...
    std::fs::write(agent9527_home.join("config.toml"), config)?;
    Ok(())
}

fn write_auth(agent9527_home: &Path) -> Result<()> {
    std::fs::write(

...
fn live_001_commit_on_overflow() {
    ...
    let mut term =
        agent9527_tui::Terminal::with_options(backend).expect("terminal should be constructed");
    let area = Rect::new(
    ...
    let mut rb = agent9527_tui::RowBuilder::new(/*target_width*/ 20);
    ...
    agent9527_tui::insert_history_lines(&mut term, lines)

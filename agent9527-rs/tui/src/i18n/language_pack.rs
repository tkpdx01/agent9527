...
pub(crate) const LANGUAGE_PACK_ROOT_ENV: &str = "AGENT9527_LANGUAGE_PACK_ROOT";
...
pub(crate) fn language_pack_root(agent9527_home: &Path) -> PathBuf {
    std::env::var_os(LANGUAGE_PACK_ROOT_ENV)
        .filter(|value| !value.is_empty())
        .map(PathBuf::from)
        .unwrap_or_else(|| agent9527_home.join(LANGUAGE_PACK_RELATIVE_ROOT))
}
...
fn inspect_language_pack(directory: &Path) -> LanguagePackCandidate {
    ...
    if manifest.i18n_api.min > I18N_API_VERSION || manifest.i18n_api.max < I18N_API_VERSION {
        ...
        return disabled_candidate(
            ...
            Some(manifest),
            format!(
                "Requires i18n API {min}..={max}; this Agent9527 build provides {I18N_API_VERSION}."
            ),
        );
    }
    ...
    if locale.language.as_str() == "en" {
        return disabled_candidate(
            directory_name,
            Some(manifest),
            "English is built into Agent9527 and cannot be replaced by an external pack."
                .to_string(),
        );
    }

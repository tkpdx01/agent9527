...
use agent9527_app_server_client::AppServerEvent;
use agent9527_app_server_client::AppServerPath;
use agent9527_app_server_client::AppServerRequestHandle;
use agent9527_app_server_client::TypedRequestError;
...
use agent9527_app_server_protocol::ConfigBatchWriteParams;
use agent9527_app_server_protocol::ConfigRequirementsReadResponse;
use agent9527_app_server_protocol::ConfigWriteResponse;
use agent9527_app_server_protocol::ExternalAgentConfigDetectParams;
use agent9527_app_server_protocol::ExternalAgentConfigDetectResponse;
use agent9527_app_server_protocol::ExternalAgentConfigImportParams;
use agent9527_app_server_protocol::ExternalAgentConfigImportResponse;
use agent9527_app_server_protocol::ExternalAgentConfigMigrationItem;
...
use agent9527_app_server_protocol::Model as ApiModel;
use agent9527_app_server_protocol::ModelListParams;
use agent9527_app_server_protocol::ModelListResponse;
use agent9527_app_server_protocol::NewThreadModelDefaults;
use agent9527_app_server_protocol::RateLimitSnapshot;
use agent9527_app_server_protocol::RequestId;
use agent9527_app_server_protocol::ReviewDelivery;
...
use agent9527_app_server_protocol::SessionSource;
...
use agent9527_app_server_protocol::ThreadStartSource;
...
use agent9527_protocol::openai_models::ModelAvailabilityNux;
use agent9527_protocol::openai_models::ModelPreset;
use agent9527_protocol::openai_models::ModelServiceTier;
use agent9527_protocol::openai_models::ModelUpgrade;
use agent9527_protocol::openai_models::ReasoningEffortPreset;
...
pub(crate) struct AppServerBootstrap {
    ...
    pub(crate) plan_type: Option<agent9527_protocol::account::PlanType>,
...
impl AppServerSession {
    ...
    pub(crate) fn agent9527_home_path(
        &self,
        local_agent9527_home: &AbsolutePathBuf,
    ) -> Option<AppServerPath> {
        self.client.agent9527_home(local_agent9527_home)
    }
    ...
    pub(crate) async fn turn_start(
        ...
        model: String,
        effort: Option<agent9527_protocol::openai_models::ReasoningEffort>,
        summary: Option<agent9527_protocol::config_types::ReasoningSummary>,
        service_tier: Option<Option<String>>,
        collaboration_mode: Option<agent9527_protocol::config_types::CollaborationMode>,
        personality: Option<agent9527_protocol::config_types::Personality>,
...
pub(crate) fn status_account_display_from_auth_mode(
    ...
    plan_type: Option<agent9527_protocol::account::PlanType>,
...
fn approvals_reviewer_override_from_config(
    config: &Config,
) -> Option<agent9527_app_server_protocol::ApprovalsReviewer> {
...
fn turn_permissions_overrides(
    ...
    Option<agent9527_app_server_protocol::SandboxPolicy>,
    Option<String>,
) {
...
fn display_permission_profile_from_thread_response(
    sandbox: &agent9527_app_server_protocol::SandboxPolicy,
    cwd: &std::path::Path,
    config: &Config,
    thread_params_mode: ThreadParamsMode,
) -> PermissionProfile {
...
async fn thread_session_state_from_thread_response(
    ...
    let history_config = agent9527_message_history::HistoryConfig::new(
        config.agent9527_home.clone(),
        &config.history,
    );
    let (log_id, entry_count) = agent9527_message_history::history_metadata(&history_config).await;
...
mod tests {
    ...
    fn rate_limit_snapshot(limit_id: &str) -> RateLimitSnapshot {
        RateLimitSnapshot {
            ...
            limit_name: None,
            primary: Some(agent9527_app_server_protocol::RateLimitWindow {
                used_percent: 0,
    ...
    fn app_server_rate_limit_snapshots_deduplicates_top_level_limit_from_map() {
        let response = GetAccountRateLimitsResponse {
            rate_limits: rate_limit_snapshot("agent9527"),
            rate_limits_by_limit_id: Some(HashMap::from([
                ("agent9527".to_string(), rate_limit_snapshot("agent9527")),
    ...
    fn legacy_turn_permissions_project_to_sandbox_when_explicitly_overridden() {
        ...
        assert_eq!(
            sandbox_policy,
            Some(agent9527_app_server_protocol::SandboxPolicy::ReadOnly {
    ...
    async fn persisted_resume_does_not_forward_implicit_service_tier() -> Result<()> {
        let agent9527_home = tempfile::tempdir().expect("tempdir");
        let mut config = build_config(&agent9527_home).await;
    ...
    {
        let agent9527_home = tempfile::tempdir().expect("tempdir");
        let config = build_config(&agent9527_home).await;
    ...
    async fn side_fork_excludes_turns_without_clearing_regular_ephemeral_fork() -> Result<()> {
        ...
        assert!(matches!(
            ...
            [agent9527_app_server_protocol::ThreadItem::UserMessage { content, .. }]
    ...
    async fn resume_response_restores_turns_from_thread_items() {
        ...
        let response = ThreadResumeResponse {
            thread: agent9527_app_server_protocol::Thread {
                ...
                source: agent9527_app_server_protocol::SessionSource::Cli,
                ...
                turns: vec![Turn {
                    ...
                    items: vec![
                        agent9527_app_server_protocol::ThreadItem::UserMessage {
                            id: "user-1".to_string(),
                            client_id: None,
                            content: vec![agent9527_app_server_protocol::UserInput::Text {
                                text: "hello from history".to_string(),
                                text_elements: Vec::new(),
                            }],
                        },
                        agent9527_app_server_protocol::ThreadItem::AgentMessage {
            ...
            approval_policy: agent9527_app_server_protocol::AskForApproval::Never,
            approvals_reviewer: agent9527_app_server_protocol::ApprovalsReviewer::User,
        ...
        let embedded_config = ConfigBuilder::default()
            .agent9527_home(temp_dir.path().join("embedded-agent9527-home"))
            .harness_overrides(ConfigOverrides {
                default_permissions: Some(BUILT_IN_PERMISSION_PROFILE_WORKSPACE.to_string()),
                ..ConfigOverrides::default()
            })
    ...
    async fn embedded_thread_response_uses_local_config_profile() {
        ...
        assert_eq!(
            display_permission_profile_from_thread_response(
                &agent9527_app_server_protocol::SandboxPolicy::DangerFullAccess,
                cwd.as_path(),
                &config,
                ThreadParamsMode::Embedded,
            ),
    ...
    async fn session_configured_populates_history_metadata() {
        ...
        agent9527_message_history::append_entry("older", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        agent9527_message_history::append_entry("newer", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        ...
        let session = thread_session_state_from_thread_response(
            ...
            agent9527_protocol::config_types::ApprovalsReviewer::User,
            PermissionProfile::read_only(),
            /*active_permission_profile*/ None,
            test_path_buf("/tmp/project").abs(),
            Vec::new(),
    ...
    fn status_account_display_from_auth_mode_uses_remapped_plan_labels() {
        let business = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(agent9527_protocol::account::PlanType::EnterpriseCbpUsageBased),
        );
        assert!(matches!(
            business,
            Some(StatusAccountDisplay::ChatGpt {
        ...
        let team = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(agent9527_protocol::account::PlanType::SelfServeBusinessUsageBased),
        );
        assert!(matches!(
            team,
            Some(StatusAccountDisplay::ChatGpt {

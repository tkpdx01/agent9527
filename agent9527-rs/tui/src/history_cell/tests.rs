...
use agent9527_otel::RuntimeMetricTotals;
...
async fn test_config() -> Config {
    let agent9527_home = std::env::temp_dir();
...
fn composite_cell_preserves_child_web_links() {
    let destination = "https://chatgpt.com/agent9527/settings/usage";
...
unset it, then restart Agent9527"
                .to_string(),
        ),
...
fn prefixed_wrapped_history_cell_indents_wrapped_lines() {
    ...
    assert_eq!(
        rendered,
        vec![
            "✔ You approved agent9527".to_string(),
...
fn multiline_command_wraps_with_extra_indent_on_subsequent_lines() {
    ...
    let cmd = "set -o pipefail\ncargo test -p agent9527-tui --quiet".to_string();
    let call_id = "c1".to_string();
    let mut cell = ExecCell::new(
        ExecCall {
            call_id: call_id.clone(),

...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<agent9527_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<agent9527_protocol::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(agent9527_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
///
/// Manual testing tip:
/// - Run the rmcp stdio test server (`agent9527-rs/rmcp-client/src/bin/test_stdio_server.rs`) and
///   register it as an MCP server via `agent9527 mcp add`.
    ...
    let lines: Vec<Line<'static>> = vec![
        ...
        Line::from(vec![
            ...
            crate::terminal_hyperlinks::osc8_hyperlink(
                "https://developers.openai.com/agent9527/mcp",
                "MCP docs",
            )
            .underlined(),
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, agent9527_protocol::mcp::Tool>,
...
pub(crate) fn new_mcp_tools_output_from_statuses(
    ...
    for status in statuses {
        ...
        let auth_status = match status.auth_status {
            agent9527_app_server_protocol::McpAuthStatus::Unsupported => McpAuthStatus::Unsupported,
            agent9527_app_server_protocol::McpAuthStatus::NotLoggedIn => McpAuthStatus::NotLoggedIn,
            agent9527_app_server_protocol::McpAuthStatus::BearerToken => McpAuthStatus::BearerToken,

...
const ANNOUNCEMENT_TIP_URL: &str =
    "https://raw.githubusercontent.com/openai/agent9527/main/announcement_tip.toml";

const IS_MACOS: bool = cfg!(target_os = "macos");
const IS_WINDOWS: bool = cfg!(target_os = "windows");

const APP_TOOLTIP: &str = "Try the **Desktop app**. Run 'agent9527 app' or visit https://chatgpt.com/agent9527?app-landing-page=true";
const FAST_TOOLTIP: &str =
    "*New* Use **/fast** to enable our fastest inference with increased plan usage.";
const OTHER_TOOLTIP: &str = "*New* Build faster with the **Desktop app**. Run 'agent9527 app' or visit https://chatgpt.com/agent9527?app-landing-page=true";
const OTHER_TOOLTIP_NON_MAC: &str = "*New* Build faster with Agent9527.";
const FREE_GO_TOOLTIP: &str =
    "*New* For a limited time, Agent9527 is included in your plan for free – let’s build together.";
...
lazy_static! {
    static ref TOOLTIPS: Vec<&'static str> = RAW_TOOLTIPS
        ...
        .filter(|line| {
            ...
            if !IS_MACOS && !IS_WINDOWS && line.contains("agent9527 app") {
                return false;
            }
            true
        })
...
/// Pick a random tooltip to show to the user when starting Agent9527.
...
fn localize_tooltip(tip: &str) -> String {
    let key = match tip {
        APP_TOOLTIP => "tooltip-agent9527-app",
        FAST_TOOLTIP => "tooltip-fast-mode",
        OTHER_TOOLTIP => "tooltip-build-faster-app",
        OTHER_TOOLTIP_NON_MAC => "tooltip-build-faster-agent9527",
        ...
        "Use /permissions to control when Agent9527 asks for confirmation." => {
            "tooltip-permissions"
        }
        "Run /review to get a code review of your current changes." => "tooltip-review",
        "Use /skills to list available skills or ask Agent9527 to use one." => "tooltip-skills",
        ...
        "Run `agent9527 app` to open the Desktop app (it installs on macOS if needed)." => {
            "tooltip-open-desktop"
        }
        "Use /personality to customize how Agent9527 communicates." => "tooltip-personality",
        "Use /rename to rename your threads for easier thread resuming." => {
            "tooltip-rename-threads"
        }
        "Use the OpenAI docs MCP for API questions; enable it with `agent9527 mcp add openaiDeveloperDocs --url https://developers.openai.com/mcp`." => {
            "tooltip-openai-docs-mcp"
        }
        "Join the OpenAI community Discord: http://discord.gg/openai" => {
            "tooltip-community-discord"
        }
        "Visit the Agent9527 community forum: https://community.openai.com/c/agent9527/37" => {
            "tooltip-community-forum"
        }
        "You can run any shell command from Agent9527 using `!` (e.g. `!ls`)" => {
        ...
        "[tui.keymap] in ~/.agent9527/config.toml lets you rebind supported shortcuts." => {
        ...
        "You can resume a previous conversation by running `agent9527 resume`" => "tooltip-resume",
...
pub(crate) mod announcement {
    ...
    impl TargetOs {
        const fn current() -> Self {
            if cfg!(target_os = "macos") {
                ...
                // Agent9527 currently publishes CLI builds for macOS, Windows, and Linux.
                Self::Linux
            }
        }
    }
    ...
    pub(crate) fn parse_announcement_tip_toml(
        ...
        for raw in announcements {
            ...
            if tip.version_matches(AGENT9527_CLI_VERSION)
...
from_date = "2000-01-01"
    ...
    fn announcement_tip_toml_parse_comments() {
        let toml = r#"
# Example announcement tips for Agent9527 TUI.
...
[[announcements]]
content = "Welcome to Agent9527! Check out the new onboarding flow."

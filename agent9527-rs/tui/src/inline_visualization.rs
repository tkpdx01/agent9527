...
pub(crate) const DIRECTIVE_PREFIX: &str = "::agent9527-inline-vis{";
...
impl InlineVisualizationContext {
    pub(crate) fn new(agent9527_home: &Path, thread_id: ThreadId) -> Option<Self> {
        Self::new_with_writable_roots(agent9527_home, thread_id, std::iter::empty())
    }
    ...
    fn new_with_writable_roots<'a>(
        ...
        let visualizations_dir = agent9527_home.join("visualizations");
...
fn link_placeholder() -> String {
    ...
    format!("https://agent9527.invalid/inline-visualization/{token}")
}

    ...
    app.chat_widget.handle_server_notification(
        ServerNotification::Error(ErrorNotification {
            error: AppServerTurnError {
                ...
                agent9527_error_info: Some(Agent9527ErrorInfo::UsageLimitExceeded),
...
async fn stale_rate_limit_reads_preserve_newer_workspace_hard_stop_for_every_origin() -> Result<()>
{
    for origin_name in [
        ...
        // External-API-only distributions intentionally hide `/usage`, so the composer cannot
        // produce this UI refresh origin.
        if origin_name == "usage"
            && !agent9527_features::product_policy::account_authentication_enabled()
        {
            continue;
        }
        ...
        if origin_name == "reset-picker" {
            rolling_snapshot.limit_id = Some("agent9527_other".to_string());
        }

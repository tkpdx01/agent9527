...
const CURRENT_MODEL: &str = agent9527_models_manager::MODEL_PRO;
const FASTER_MODEL: &str = agent9527_models_manager::MODEL_BALANCED;
...
fn next_user_turn_event(
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        if let AppEvent::Agent9527Op(turn @ AppCommand::UserTurn { .. }) = event {
            return turn;
        }
    }
...
"#,
    ...
    app.config.sqlite = agent9527_state::SqliteConfig::new_for_testing(agent9527_home.path().abs());
...
"#,
    ...
    state_db
        ...
        .account_thread_goal_usage(
            ...
            /*token_delta*/ 50,
            agent9527_state::GoalAccountingMode::ActiveOrStopped,

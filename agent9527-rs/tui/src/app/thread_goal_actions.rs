...
const EPHEMERAL_THREAD_GOAL_ERROR_MESSAGE: &str = concat!(
    ...
    "Run `agent9527` to start a saved session, or `agent9527 resume` / `/resume` to reopen one.",
);
...
impl App {
    ...
    pub(super) async fn open_thread_goal_editor(
        ...
        let agent9527_home = app_server.agent9527_home_path(&self.config.agent9527_home);
    ...
    pub(super) async fn set_thread_goal_draft(
        ...
        let (objective, output_dir) =
            match goal_files::materialize_goal_draft(app_server, agent9527_home.as_ref(), draft)
                .await
            {

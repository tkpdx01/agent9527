...
//! managed AGENT9527_HOME cache, while custom pets remain entirely user-owned data
//! under `$AGENT9527_HOME/pets/<pet-id>/pet.json` or legacy avatar directories.
//!
...
pub(crate) const DEFAULT_PET_ID: &str = "agent9527";
...
pub(crate) fn ensure_builtin_pack_for_pet(
    ...
    if let Some(pet) = catalog::builtin_pet(pet_id) {
        asset_pack::ensure_builtin_pet(agent9527_home, pet)?;
    }
    Ok(())
}

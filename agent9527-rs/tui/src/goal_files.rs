...
const GOAL_FILE_PREFIX: &str = "Read the Agent9527 goal objective file at ";
...
pub(crate) async fn materialize_goal_draft(
    ...
    agent9527_home: Option<&GoalFilePath>,
    draft: GoalDraft,
    ...
    for (placeholder, text) in draft.pending_pastes.iter() {
        ...
        let path = ensure_goal_output_dir(app_server, agent9527_home, &mut output_dir)
            .await?
...
pub(crate) async fn objective_text_for_edit(
    ...
    let Some(path) = objective_file_path(objective, agent9527_home) else {
...
async fn ensure_goal_output_dir(
    ...
    let agent9527_home = agent9527_home
        .context("App server did not report $AGENT9527_HOME; cannot materialize goal files")?;

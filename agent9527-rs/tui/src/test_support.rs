...
pub(crate) use agent9527_utils_absolute_path::test_support::PathBufExt;
pub(crate) use agent9527_utils_absolute_path::test_support::test_path_buf;
use serde::Serialize;
use serde::de::DeserializeOwned;

pub(crate) static TEST_MODEL_PRESETS: LazyLock<Vec<ModelPreset>> = LazyLock::new(|| {
    let mut response = upstream_bundled_models_response()
...
where
    T: DeserializeOwned,
{
    from_app_server_wire(agent9527_app_server_protocol::SessionSource::Cli)
}
...
where
    T: DeserializeOwned,
{
    from_app_server_wire(agent9527_app_server_protocol::SkillScope::User)
}
...
where
    T: DeserializeOwned,
{
    from_app_server_wire(agent9527_app_server_protocol::SkillScope::Repo)
}

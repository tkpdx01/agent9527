use agent9527_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `agent9527 resume` subcommand.
    // These are not exposed as user flags on the base `agent9527` command.
    #[clap(skip)]
    ...
    /// top-level `agent9527 resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `agent9527 fork` subcommand.
    // These are not exposed as user flags on the base `agent9527` command.
    #[clap(skip)]
    ...
    /// top-level `agent9527 fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]

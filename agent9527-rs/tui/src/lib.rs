...
// The standalone `agent9527-tui` binary prints a short help message before the
...
use agent9527_app_server_client::AppServerClient;
use agent9527_app_server_client::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
use agent9527_app_server_client::InProcessAppServerClient;
use agent9527_app_server_client::InProcessClientStartArgs;
use agent9527_app_server_client::RemoteAppServerClient;
use agent9527_app_server_client::RemoteAppServerConnectArgs;
pub use agent9527_app_server_client::RemoteAppServerEndpoint;
use agent9527_app_server_protocol::Account as AppServerAccount;
use agent9527_app_server_protocol::AskForApproval;
use agent9527_app_server_protocol::ConfigWarningNotification;
use agent9527_app_server_protocol::Thread as AppServerThread;
use agent9527_app_server_protocol::ThreadListCwdFilter;
use agent9527_app_server_protocol::ThreadListParams;
use agent9527_app_server_protocol::ThreadSortKey as AppServerThreadSortKey;
use agent9527_app_server_protocol::ThreadSourceKind;
use agent9527_cloud_config::cloud_config_bundle_loader_for_storage;
use agent9527_config::CloudConfigBundleLoader;
use agent9527_config::ConfigLoadError;
use agent9527_config::LoaderOverrides;
use agent9527_config::format_config_error_with_source;
...
use agent9527_login::AuthConfig;
use agent9527_login::AuthRouteConfig;
use agent9527_login::default_client::originator;
use agent9527_login::default_client::set_default_client_residency_requirement;
use agent9527_login::enforce_login_restrictions;
...
use agent9527_rollout::StateDbHandle;
use agent9527_rollout::state_db;
use agent9527_state::log_db;
...
use agent9527_utils_oss::ensure_oss_provider_ready;
use agent9527_utils_oss::get_default_model_for_oss_provider;
use app::App;
...
pub(crate) use agent9527_app_server_client::legacy_core;
...
const TUI_LOG_FILE_NAME: &str = "agent9527-tui.log";
...
async fn start_embedded_app_server(
    ...
    feedback: agent9527_feedback::Agent9527Feedback,
...
async fn init_state_db_for_app_server_target(
    ...
    match app_server_target {
        AppServerTarget::Embedded => state_db::try_init(config).await.map(Some).map_err(|err| {
            let database_path = agent9527_state::runtime_db_path_for_corruption_error(&err)
...
fn remove_legacy_tui_log_file(agent9527_home: &Path) {
    ...
    let _ = std::fs::remove_file(agent9527_home.join("log").join(TUI_LOG_FILE_NAME));
}
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let agent9527_home =
                find_agent9527_home().wrap_err("failed to resolve AGENT9527_HOME")?;
            agent9527_app_server_client::app_server_control_socket_path(&agent9527_home)
...
#[cfg(unix)]
async fn maybe_probe_default_daemon_socket(agent9527_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path =
        agent9527_app_server_client::app_server_control_socket_path(agent9527_home).ok()?;
...
#[cfg(not(unix))]
async fn maybe_probe_default_daemon_socket(_agent9527_home: &Path) -> Option<AbsolutePathBuf> {
    None
}
...
pub(crate) async fn start_app_server_for_picker(
    ...
    let app_server = start_app_server(
        ...
        agent9527_feedback::Agent9527Feedback::new(),
        /*log_db*/ None,
        state_db,
        environment_manager,
    )
...
async fn resolve_startup_resume_or_fork_cwd(
    tui: &mut Tui,
    config: &Config,
    state_db: Option<&agent9527_state::StateRuntime>,
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = agent9527_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    #[allow(clippy::print_stderr)]
    let agent9527_home = match find_agent9527_home() {
        Ok(agent9527_home) => agent9527_home.to_path_buf(),
        Err(err) => {
            eprintln!("Error finding agent9527 home: {err}");
            std::process::exit(1);
        }
    };
    ...
    if let Some(profile_v2) = cli.config_profile_v2.as_ref() {
        let user_config_path = resolve_profile_v2_config_path(&agent9527_home, profile_v2);
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        maybe_probe_default_daemon_socket(&agent9527_home).await
    } else {
        None
    };
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        arg0_paths.agent9527_self_exe.clone(),
    ...
    let prepared_environment_manager =
        if should_load_configured_environments(&loader_overrides, &app_server_target) {
            EnvironmentManager::prepare_from_agent9527_home(&agent9527_home).await
        } else {
    ...
    remove_legacy_tui_log_file(config.agent9527_home.as_path());

    let otel_originator = originator().value;
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        agent9527_app_server_client::build_otel_provider(
            &config,
    ...
    if let Some(metrics) = otel
        .as_ref()
        .and_then(agent9527_otel::OtelProvider::metrics)
    {
        let _ = agent9527_otel::record_process_start_once(metrics, otel_originator.as_str());
        let telemetry =
            agent9527_rollout::sqlite_telemetry_recorder(metrics.clone(), otel_originator.as_str());
        let _ = agent9527_state::install_process_db_telemetry(telemetry);
    }
    ...
    let (tui_file_layer, _tui_file_log_guard) = if config_toml_log_dir_configured {
        ...
        let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("agent9527_core=info,agent9527_tui=info,agent9527_rmcp_client=info")
        });
    ...
    let feedback = agent9527_feedback::Agent9527Feedback::new();
...
async fn run_ratatui_app(
    ...
    let login_status = if product_policy::account_authentication_enabled()
    ...
    let config = if should_show_onboarding {
        ...
        if show_login_screen && !uses_remote_workspace {
            cloud_config_bundle = cloud_config_bundle_loader_for_storage(
                initial_config.agent9527_home.to_path_buf(),
                /*enable_agent9527_api_key_env*/ false,
    ...
    let mut missing_session_exit = |id_str: &str, action: &str| {
        ...
        Ok(AppExitInfo {
            ...
            exit_reason: ExitReason::Fatal(format!(
                "No saved session found with ID {id_str}. Run `agent9527 {action}` without an ID to choose from existing sessions."
            )),
        })
    };
    ...
    let required_elevated_sandbox_needs_setup = windows_sandbox_level
        ...
        && config
            ...
            .source
            .is_some()
        && !crate::windows_sandbox::sandbox_setup_is_complete(config.agent9527_home.as_path());
    #[cfg(target_os = "windows")]
...
async fn load_bootstrap_config_or_exit(
    ...
    cli_kv_overrides: Vec<(String, agent9527_config::TomlValue)>,
    loader_overrides: LoaderOverrides,
    strict_config: bool,
    cloud_config_bundle: CloudConfigBundleLoader,
    ...
    match load_config_toml_with_layer_stack(
        ...
        cwd,
        cli_kv_overrides,
        agent9527_config::ConfigLoadOptions {
            loader_overrides,
            strict_config,
            cloud_config_bundle,
        },
...
fn should_show_login_screen(login_status: LoginStatus, config: &Config) -> bool {
    if !product_policy::account_authentication_enabled() {
        return false;
    }
...
mod tests {
    ...
    async fn build_config(temp_dir: &TempDir) -> std::io::Result<Config> {
        ConfigBuilder::default()
            .agent9527_home(temp_dir.path().to_path_buf())
            .build()
            .await
    }
    ...
    fn write_session_rollout(
        ...
        let rollout_path = agent9527_home
            .join("sessions")
            .join(year)
            .join(month)
            .join(day)
        ...
        let session_meta = agent9527_protocol::protocol::SessionMeta {
            session_id: thread_id.into(),
            id: thread_id,
            timestamp: meta_rfc3339.to_string(),
            cwd: cwd.to_path_buf(),
            originator: "agent9527".to_string(),
            cli_version: "0.0.0".to_string(),
            source: agent9527_protocol::protocol::SessionSource::Cli,
            model_provider: Some(model_provider.to_string()),
            ..Default::default()
        };
        let session_meta = serde_json::to_value(agent9527_protocol::protocol::SessionMetaLine {
            meta: session_meta,
            git: None,
        })?;
        let lines = [
    ...
    async fn startup_services_use_final_cloud_managed_http_policy() -> color_eyre::Result<()> {
            ...
            [(true, false), (false, true)]
        {
            ...
            let prepared_environment_manager =
                EnvironmentManager::prepare_from_agent9527_home(agent9527_home.path()).await?;
            ...
            let cloud_config_bundle =
                agent9527_config::test_support::CloudConfigBundleFixture::loader_with_enterprise_requirement(
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        let agent9527_home = find_agent9527_home().wrap_err("failed to resolve AGENT9527_HOME")?;
        assert_eq!(
            resolve_remote_addr("unix://")?,
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: agent9527_app_server_client::app_server_control_socket_path(
                    &agent9527_home
                )?,
            }
        );
    ...
    fn resolve_remote_addr_accepts_relative_socket_path() -> color_eyre::Result<()> {
        assert_eq!(
            resolve_remote_addr("unix://agent9527.sock")?,
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: AbsolutePathBuf::relative_to_current_dir("agent9527.sock")?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_absolute_socket_path() -> color_eyre::Result<()> {
        let temp_dir = TempDir::new()?;
        let socket_path = temp_dir.path().join("agent9527.sock");
        assert_eq!(
    ...
    async fn default_daemon_auto_connect_skips_missing_socket() -> color_eyre::Result<()> {
        let agent9527_home = TempDir::new()?;
        assert!(
            maybe_probe_default_daemon_socket(agent9527_home.path())
                .await
                .is_none()
        );
        Ok(())
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        let agent9527_home = TempDir::new()?;
        let socket_path =
            agent9527_app_server_client::app_server_control_socket_path(agent9527_home.path())?;
        ...
        assert_eq!(
            maybe_probe_default_daemon_socket(agent9527_home.path()).await,
            Some(socket_path)
        );
        Ok(())
    }
    ...
    {
        let socket_path = AbsolutePathBuf::relative_to_current_dir("agent9527.sock")?;
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        ...
        let mut app_server = AppServerSession::new(
            agent9527_app_server_client::AppServerClient::InProcess(
    ...
    async fn lookup_session_target_by_name_uses_backend_title_search() -> color_eyre::Result<()> {
        Box::pin(async {
            ...
            let state_runtime = agent9527_state::StateRuntime::init(
                agent9527_state::SqliteConfig::new_for_testing(
                    config.agent9527_home.as_path().abs(),
                ),
            ...
            let mut builder = agent9527_state::ThreadMetadataBuilder::new(
                thread_id,
                rollout_path.clone(),
                created_at,
    ...
    async fn embedded_state_db_failure_is_typed_for_cli_recovery() -> color_eyre::Result<()> {
        ...
        let sqlite =
            agent9527_state::SqliteConfig::new_for_testing(occupied_sqlite_home.as_path().abs());
    ...
    -> color_eyre::Result<()> {
        ...
        let sqlite = agent9527_state::SqliteConfig::new_for_testing(sqlite_home.as_path().abs());
        ...
        assert!(
            agent9527_state::sqlite_error_detail_is_corruption(startup_error.detail()),
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let agent9527_home = temp_dir.path().to_path_buf();

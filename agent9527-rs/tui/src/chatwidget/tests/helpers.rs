...
pub(super) async fn test_config() -> Config {
    ...
    let agent9527_home = tempfile::Builder::new()
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config = Config::load_default_with_cli_overrides_for_agent9527_home(
        agent9527_home.clone(),
        Vec::new(),
    )
    .await
    .expect("config");
    config.agent9527_home = agent9527_home.abs();
    config.sqlite = agent9527_state::SqliteConfig::new_for_testing(agent9527_home.as_path().abs());
    config.log_dir = agent9527_home.join("log");
...
pub(super) async fn make_chatwidget_manual_with_auth(
    ...
    let mut widget =
        ChatWidget::new_with_op_target(common, super::Agent9527OpTarget::Direct(op_tx));
    widget.transcript.active_cell = None;
    widget.transcript.active_cell_revision = 0;
    widget.normal_placeholder_text = "Ask Agent9527 to do anything".to_string();
...
pub(crate) fn set_chatgpt_auth(chat: &mut ChatWidget) {
    ...
    chat.has_agent9527_backend_auth = true;
...
fn token_usage_breakdown(usage: TokenUsage) -> agent9527_app_server_protocol::TokenUsageBreakdown {
    agent9527_app_server_protocol::TokenUsageBreakdown {
            ...
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    agent9527_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: agent9527_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn handle_error(
    ...
    agent9527_error_info: Option<Agent9527ErrorInfo>,
) {
    chat.handle_server_notification(
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = agent9527_shell_command::parse_command::parse_command(&command)
        .into_iter()
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            agent9527_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_absolute_path(path: &str) -> AbsolutePathBuf {
    std::env::temp_dir()
        .join("agent9527-plugin-menu-tests")
        .join(path)
        .abs()
}
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(agent9527_app_server_protocol::HookEventName, usize)],
    apps: &[&str],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
        ...
        hooks: hooks
            ...
            .flat_map(|(event_index, (event_name, handler_count))| {
                (0..*handler_count).map(move |handler_index| {
                    agent9527_app_server_protocol::PluginHookSummary {
        ...
        hook_run(
            ...
            vec![
                agent9527_app_server_protocol::HookOutputEntry {
                    kind: agent9527_app_server_protocol::HookOutputEntryKind::Warning,
...
fn hook_event_label(event_name: agent9527_app_server_protocol::HookEventName) -> &'static str {
    match event_name {
        agent9527_app_server_protocol::HookEventName::PreToolUse => "PreToolUse",
        agent9527_app_server_protocol::HookEventName::PermissionRequest => "PermissionRequest",
        agent9527_app_server_protocol::HookEventName::PostToolUse => "PostToolUse",
        agent9527_app_server_protocol::HookEventName::PreCompact => "PreCompact",
        agent9527_app_server_protocol::HookEventName::PostCompact => "PostCompact",
        agent9527_app_server_protocol::HookEventName::SessionStart => "SessionStart",
        agent9527_app_server_protocol::HookEventName::SessionEnd => "SessionEnd",
        agent9527_app_server_protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        agent9527_app_server_protocol::HookEventName::SubagentStart => "SubagentStart",
        agent9527_app_server_protocol::HookEventName::SubagentStop => "SubagentStop",
        agent9527_app_server_protocol::HookEventName::Stop => "Stop",
    }
}

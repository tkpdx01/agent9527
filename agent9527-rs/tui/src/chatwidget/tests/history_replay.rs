...
use agent9527_app_server_protocol::NetworkAccess;
...
async fn forked_thread_history_line_without_name_shows_id_once_snapshot() {
    ...
    chat.config.agent9527_home =
        agent9527_utils_absolute_path::AbsolutePathBuf::from_absolute_path(temp.path())
...
async fn app_server_forked_thread_history_line_uses_app_server_title_snapshot() {
    ...
    assert!(
        ...
        "app-server fork title lookup should not read local AGENT9527_HOME"
    );
...
async fn replayed_in_progress_mcp_tool_call_stays_active() {
    ...
    chat.replay_thread_item(
        AppServerThreadItem::McpToolCall {
            ...
            status: agent9527_app_server_protocol::McpToolCallStatus::InProgress,
...
async fn deferred_mcp_lifecycle_events_keep_fifo_after_stream_finishes() {
    ...
    chat.on_mcp_tool_call_completed(AppServerThreadItem::McpToolCall {
        ...
        server: "copilot-bridge".to_string(),
        tool: "copilot".to_string(),
        status: agent9527_app_server_protocol::McpToolCallStatus::Completed,
        arguments: json!({"action": "wait"}),
        app_context: None,
        mcp_app_resource_uri: None,
        plugin_id: None,
        result: Some(Box::new(agent9527_app_server_protocol::McpToolCallResult {
    ...
    for (summary_index, delta) in [
        ...
        chat.handle_server_notification(
            ServerNotification::ReasoningSummaryPartAdded(
                agent9527_app_server_protocol::ReasoningSummaryPartAddedNotification {
                    thread_id: "thread-1".to_string(),
                    turn_id: "turn-1".to_string(),
                    item_id: "reasoning-1".to_string(),
                    summary_index,

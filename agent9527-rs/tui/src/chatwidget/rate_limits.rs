...
use agent9527_models_manager::canonical_model_id;

pub(super) const NUDGE_MODEL_SLUG: &str = MODEL_FLASH;
...
pub(super) fn app_server_rate_limit_error_kind(
    info: &AppServerAgent9527ErrorInfo,
) -> Option<RateLimitErrorKind> {
    match info {
        AppServerAgent9527ErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        AppServerAgent9527ErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        AppServerAgent9527ErrorInfo::ResponseTooManyFailedAttempts {
...
pub(super) fn is_app_server_cyber_policy_error(info: &AppServerAgent9527ErrorInfo) -> bool {
    matches!(info, AppServerAgent9527ErrorInfo::CyberPolicy)
}
...
impl ChatWidget {
    ...
    fn on_rate_limit_snapshot_from(
        ...
        if let Some(mut snapshot) = snapshot {
            ...
            let is_agent9527_limit = limit_id.eq_ignore_ascii_case("agent9527");
            if is_agent9527_limit
                && (matches!(source, RateLimitSnapshotSource::AccountUsage)
                    || snapshot.spend_control_reached.is_some())
            {
                self.agent9527_spend_control_reached = snapshot.spend_control_reached;
            }
            if (is_agent9527_limit && matches!(source, RateLimitSnapshotSource::AccountUsage))
                || snapshot.rate_limit_reached_type.is_some()
            {
                self.agent9527_rate_limit_reached_type = snapshot.rate_limit_reached_type;
            }
            let workspace_limit_reached = self.agent9527_spend_control_reached == Some(true)
                || matches!(
                    self.agent9527_rate_limit_reached_type,
                    Some(
            ...
            if is_agent9527_limit && has_workspace_credits {
            ...
            let should_warn_about_rate_limit_usage = is_agent9527_limit && !has_workspace_credits;
            ...
            let high_usage = is_agent9527_limit
                && (snapshot
                    .secondary
                    .as_ref()
            ...
            self.agent9527_rate_limit_reached_type = None;
            self.agent9527_spend_control_reached = None;
        }
        ...
        models
            ...
            .find(|preset| {
                preset.show_in_picker
                    && canonical_model_id(&preset.model).as_ref() == NUDGE_MODEL_SLUG
            })
            .cloned()
            .map(|mut preset| {
                preset.id = NUDGE_MODEL_SLUG.to_string();
                preset.model = NUDGE_MODEL_SLUG.to_string();
                preset.display_name = "DeepSeek V4 Flash".to_string();
                preset
            })
    }
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::Agent9527Op(AppCommand::override_turn_context(
                /*cwd*/ None,
    ...
    pub(super) fn open_workspace_owner_nudge_prompt(
        ...
        let (title, prompt) = match credit_type {
            ...
            AddCreditsNudgeCreditType::UsageLimit => (
                "Usage limit reached",
                "Request a limit increase from your owner to continue using agent9527. Request increase?",
            ),
        };

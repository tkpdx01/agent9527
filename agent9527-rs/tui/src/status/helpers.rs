...
mod tests {
    ...
    async fn test_config(agent9527_home: &TempDir, cwd: &TempDir) -> Config {
    ...
    async fn compose_agents_summary_includes_global_agents_path() {
        let agent9527_home = TempDir::new().expect("temp agent9527 home");
        let cwd = TempDir::new().expect("temp cwd");
        let global_agents_path = agent9527_home.path().join("global.md");
        let config = test_config(&agent9527_home, &cwd).await;
    ...
    async fn compose_agents_summary_names_global_agents_override() {
        let agent9527_home = TempDir::new().expect("temp agent9527 home");
        let cwd = TempDir::new().expect("temp cwd");
        let override_path = agent9527_home.path().join("override.md");
        let config = test_config(&agent9527_home, &cwd).await;

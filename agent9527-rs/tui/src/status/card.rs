...
impl HistoryCell for StatusHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        lines.push(Line::from(vec![
            Span::from(format!("{}{COMPACT_LOGO} ", FieldFormatter::INDENT)).dim(),
            Span::from(PRODUCT_NAME).bold(),
            Span::from(" ").dim(),
            Span::from(format!("(v{AGENT9527_CLI_VERSION})")).dim(),
        ]));
        ...
        let account_value = self.account.as_ref().map(|account| match account {
            ...
            StatusAccountDisplay::ApiKey => status_text(
                ...
                "API key configured (run agent9527 login to use ChatGPT)",
            ),
        });

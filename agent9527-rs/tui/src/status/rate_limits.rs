...
use agent9527_app_server_protocol::CreditsSnapshot as CoreCreditsSnapshot;
use agent9527_app_server_protocol::RateLimitSnapshot;
use agent9527_app_server_protocol::RateLimitWindow;
use agent9527_app_server_protocol::SpendControlLimitSnapshot as CoreSpendControlLimitSnapshot;
...
pub(crate) struct RateLimitSnapshotDisplay {
    /// Canonical limit identifier (for example: `agent9527` or `agent9527_other`).
...
pub(crate) fn rate_limit_snapshot_display(
    ...
    rate_limit_snapshot_display_for_limit(snapshot, "agent9527".to_string(), captured_at)
}
...
pub(crate) fn compose_rate_limit_data_many(
    ...
    for snapshot in snapshots {
        ...
        let show_limit_prefix = !limit_bucket_label.eq_ignore_ascii_case("agent9527");
        ...
        let combine_non_agent9527_single_limit = show_limit_prefix && window_count == 1;

        if show_limit_prefix && !combine_non_agent9527_single_limit {
        ...
        if let Some(primary) = snapshot.primary.as_ref() {
            let label = if combine_non_agent9527_single_limit {
                format!(
                    "{} {} limit",
                    limit_bucket_label,
...
mod tests {
    ...
    #[test]
    fn non_agent9527_single_limit_renders_combined_row() {
        let now = Local::now();
        let agent9527 = RateLimitSnapshotDisplay {
            limit_name: "agent9527".to_string(),
            captured_at: now,
        ...
        let other = RateLimitSnapshotDisplay {
            limit_name: "agent9527-other".to_string(),
            captured_at: now,
        ...
        let rows = match compose_rate_limit_data_many(&[agent9527, other], now) {
        ...
        assert_eq!(
            ...
            vec![
                "5h limit".to_string(),
                "Credits".to_string(),
                "agent9527-other 5h limit".to_string(),
                "Credits".to_string(),
            ]
        );
    ...
    #[test]
    fn non_agent9527_multi_limit_keeps_group_row() {
        let now = Local::now();
        let other = RateLimitSnapshotDisplay {
            limit_name: "agent9527-other".to_string(),
            captured_at: now,
        ...
        assert_eq!(
            labels,
            vec![
                "agent9527-other limit".to_string(),

...
use agent9527_config::NetworkUnixSocketPermissionToml;
...
use agent9527_config::WebSearchModeRequirement;
use agent9527_config::format_config_layer_source;
...
mod tests {
    ...
    use agent9527_config::ConstraintError;
    use agent9527_config::FeatureRequirementsToml;
    use agent9527_config::FilesystemConstraints;
    use agent9527_config::HookEventsToml;
    use agent9527_config::HookHandlerConfig;
    use agent9527_config::LoaderOverrides;
    use agent9527_config::ManagedHooksRequirementsToml;
    use agent9527_config::MatcherGroup;
    ...
    use agent9527_config::NetworkUnixSocketPermissionToml;
    use agent9527_config::NetworkUnixSocketPermissionsToml;
    use agent9527_config::RequirementSource;
    use agent9527_config::ResidencyRequirement;
    use agent9527_config::SandboxModeRequirement;
...
"#,
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\agent9527\\config.toml")
        } else {
            absolute_path("/etc/agent9527/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.agent9527")
        } else {
            absolute_path("/repo/.agent9527")
        };
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_agent9527_folder: project_folder,
                },
                empty_toml_table(),
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\OpenAI\\Agent9527\\requirements.toml")
        } else {
            absolute_path("/etc/agent9527/requirements.toml")
        };
        ...
        let sqlite_home = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.agent9527\\state")
        } else {
            absolute_path("/home/alice/.agent9527/state")
        };
        let log_dir = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.agent9527\\logs")
        } else {
            absolute_path("/home/alice/.agent9527/logs")
        };
        let model_catalog_json = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.agent9527\\models.json")
        } else {
            absolute_path("/home/alice/.agent9527/models.json")
        };
                ...
                BTreeMap::from([(
                    ...
                    McpServerRequirement::Identity {
                        identity: McpServerIdentity::Command {
                            command: "agent9527-mcp".to_string(),
                        },
                    },
                )]),
                RequirementSource::LegacyManagedConfigTomlFromMdm,
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.agent9527\\config.toml")
        } else {
            absolute_path("/home/alice/.agent9527/config.toml")
        };
        let stack = ConfigLayerStack::new(
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
        ...
        let requirements_source = (RequirementSource::LegacyManagedConfigTomlFromMdm).to_string();
        assert!(rendered.contains(&format!(
            "experimental_network: unix_sockets={{/tmp/agent9527.sock=allow, /tmp/blocked.sock=deny}} (source: {requirements_source})"
        )));
    }
...
"#;
        ...
        let mdm_base_dir = if cfg!(windows) {
            absolute_path("C:\\agent9527")
        } else {
            absolute_path("/var/lib/agent9527")
        };

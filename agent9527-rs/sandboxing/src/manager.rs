...
use crate::landlock::AGENT9527_LINUX_SANDBOX_ARG0;
...
pub struct SandboxTransformRequest<'a> {
    ...
    pub agent9527_linux_sandbox_exe: Option<&'a Path>,
...
pub struct SandboxDirectSpawnTransformRequest<'a> {
    ...
    pub windows_sandbox_proxy_settings_mode:
        agent9527_windows_sandbox::WindowsSandboxProxySettingsMode,
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingLinuxSandboxExecutable => {
                write!(f, "missing agent9527-linux-sandbox executable path")
            }
...
impl SandboxManager {
    ...
    pub fn transform(
        ...
        let (argv, arg0_override, pending_sandboxed_request) = match sandbox {
            ...
            SandboxType::LinuxSeccomp => {
                let pending = pending_sandboxed_request?;
                let exe = agent9527_linux_sandbox_exe
    ...
    pub fn transform_for_direct_spawn(
        ...
        #[cfg(target_os = "windows")]
        {
            let agent9527_home = agent9527_utils_home_dir::find_agent9527_home()
                .map_err(|err| SandboxTransformError::WindowsSandboxPreparation(err.to_string()))?;
            self.transform_for_direct_spawn_with_agent9527_home(request, agent9527_home.as_path())
        }
    ...
    #[cfg(target_os = "windows")]
    fn transform_for_direct_spawn_with_agent9527_home(
        &self,
        request: SandboxDirectSpawnTransformRequest<'_>,
        agent9527_home: &Path,
    ) -> Result<SandboxExecRequest, SandboxTransformError> {
...
fn wrap_windows_sandbox_exec_request_for_direct_spawn(
    ...
    agent9527_home: &Path,
    proxy_settings_mode: agent9527_windows_sandbox::WindowsSandboxProxySettingsMode,
    ...
    let helper =
        agent9527_windows_sandbox::resolve_exe_for_launch(source.as_path(), agent9527_home);
    *program = helper.to_string_lossy().into_owned();
    ...
    let mut wrapper_args =
        agent9527_windows_sandbox::create_windows_sandbox_command_args_for_permission_profile(
            inner_command,
            &native_cwd,
            workspace_roots,
            &request.env,
...
fn linux_sandbox_arg0_override(exe: &Path) -> String {
    if exe.file_name().and_then(|name| name.to_str()) == Some(AGENT9527_LINUX_SANDBOX_ARG0) {

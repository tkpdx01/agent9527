...
use agent9527_network_proxy::ManagedNetworkSandboxContext;
...
use agent9527_network_proxy::build_config_state;
...
use agent9527_protocol::permissions::PROTECTED_METADATA_PATH_NAMES;
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/agent9527-unreadable");
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.agent9527".to_string(),
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/agent9527-readable");
    let unreadable = absolute_path("/tmp/agent9527-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
...
fn create_seatbelt_args_allowlists_explicit_unix_socket_paths_without_proxy() {
    ...
    let extra_allow_unix_sockets = vec![absolute_path("/tmp/agent9527-browser-use")];
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
    ...
    let expected_socket_root = normalize_path_for_sandbox(Path::new("/tmp/agent9527-browser-use"))
...
async fn create_seatbelt_args_merges_proxy_and_explicit_unix_socket_paths() -> anyhow::Result<()> {
    ...
    let network_socket = "/tmp/agent9527-proxy-use";
    let explicit_socket = "/tmp/agent9527-browser-use";
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_agent9527_subpaths() {
    ...
    // write to .agent9527/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected explicit writable root .git/.agent9527 carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_agent9527_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that .agent9527/config.toml cannot be modified under the generated
    // Seatbelt policy.
    let config_toml = dot_agent9527_canonical.join("config.toml");
    ...
    // Verify that writing a file to the folder containing .git and .agent9527 is allowed.
...
#[test]
fn create_seatbelt_args_block_first_time_dot_agent9527_creation_with_metadata_name_regex() {
    let tmp = TempDir::new().expect("tempdir");
    ...
    let dot_agent9527 = repo_root.join(".agent9527");
    let config_toml = dot_agent9527.join("config.toml");
    let policy = SandboxPolicy::WorkspaceWrite {
    ...
    let shell_command: Vec<String> = [
        ...
        "bash",
        dot_agent9527.to_string_lossy().as_ref(),
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    let expected_dot_agent9527 = format!(
        "-DWRITABLE_ROOT_0_EXCLUDED_1={}",
        dot_agent9527_canonical.to_string_lossy()
    );
    assert!(
        args.contains(&expected_dot_agent9527),
        "missing {expected_dot_agent9527}: {args:#?}"
    );
...
struct PopulatedTmp {
    ...
    /// ran `git commit` themselves, or modified .agent9527/config.toml to
    ...
    dot_agent9527_canonical: PathBuf,
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".agent9527")).expect("create .agent9527");
    fs::write(
        vulnerable_root.join(".agent9527").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .agent9527/config.toml");
    ...
    let dot_agent9527_canonical = vulnerable_root_canonical.join(".agent9527");

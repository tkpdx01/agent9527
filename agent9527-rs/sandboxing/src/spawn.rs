...
pub async fn spawn_process(request: SpawnRequest<'_>) -> Result<SpawnedProcess> {
    if request.sandbox == SandboxType::WindowsRestrictedToken {
        #[cfg(target_os = "windows")]
        {
            ...
            let agent9527_home = agent9527_utils_home_dir::find_agent9527_home()
                .context("windows sandbox: failed to resolve agent9527_home")?;
            let empty_paths = &[];
            let overrides = windows.filesystem_overrides;

            return agent9527_windows_sandbox::spawn_windows_sandbox_session_for_level(
                agent9527_windows_sandbox::WindowsSandboxSessionRequest {
                    permission_profile: windows.permission_profile,
                    workspace_roots: windows.workspace_roots,
                    agent9527_home: agent9527_home.as_path(),
    ...
    if request.tty {
        agent9527_utils_pty::pty::spawn_process(
            program,
            args,
            request.cwd,
            request.env,
        ...
        agent9527_utils_pty::pipe::spawn_process(
            program,
            args,
            request.cwd,
            request.env,
        ...
        agent9527_utils_pty::pipe::spawn_process_no_stdin(
            program,
            args,
            request.cwd,
            request.env,

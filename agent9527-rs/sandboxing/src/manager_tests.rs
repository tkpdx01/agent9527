...
fn transform_linux_seccomp_request(
    agent9527_linux_sandbox_exe: &std::path::Path,
    ...
    manager
        .transform(SandboxTransformRequest {
            ...
            network: None,
            sandbox_policy_cwd: &cwd_uri,
            agent9527_linux_sandbox_exe: Some(agent9527_linux_sandbox_exe),
            use_legacy_landlock: false,
            windows_sandbox_level: WindowsSandboxLevel::Disabled,
            windows_sandbox_private_desktop: false,
        })
...
fn transform_linux_seccomp_preserves_helper_path_in_arg0_when_available() {
    let agent9527_linux_sandbox_exe = std::path::PathBuf::from("/tmp/agent9527-linux-sandbox");
    let exec_request = transform_linux_seccomp_request(&agent9527_linux_sandbox_exe);

    assert_eq!(
        exec_request.arg0,
        Some(agent9527_linux_sandbox_exe.to_string_lossy().into_owned())
    );
}
...
fn transform_linux_seccomp_uses_helper_alias_when_launcher_is_not_helper_path() {
    let agent9527_linux_sandbox_exe = std::path::PathBuf::from("/tmp/agent9527");
    let exec_request = transform_linux_seccomp_request(&agent9527_linux_sandbox_exe);

    assert_eq!(
        exec_request.arg0,
        Some("agent9527-linux-sandbox".to_string())
    );
}
...
fn transform_for_direct_spawn_windows_materializes_inner_helper() {
    let agent9527_home = tempfile::TempDir::new().expect("agent9527 home");
    let helper_dir = tempfile::TempDir::new().expect("helper dir");
    let configured_helper = helper_dir.path().join("configured-agent9527-helper.exe");
    ...
    let manager = SandboxManager::new();
    let exec_request = manager
        .transform_for_direct_spawn_with_agent9527_home(
            SandboxDirectSpawnTransformRequest {
                workspace_roots: workspace_roots.as_slice(),
                windows_sandbox_proxy_settings_mode:
                    agent9527_windows_sandbox::WindowsSandboxProxySettingsMode::Preserve,
                transform: SandboxTransformRequest {
                    command: SandboxCommand {
                        program: configured_helper.as_os_str().to_owned(),
                        args: vec!["--agent9527-run-as-fs-helper".to_string()],
                        cwd: cwd_uri.clone(),
                        env: HashMap::from([(
                            "Path".to_string(),

...
use agent9527_file_system::ExecutorFileSystem;
...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/agent9527/config.toml";
...
///
/// - system    `/etc/agent9527/requirements.toml` (Unix) or
///   `%ProgramData%\OpenAI\Agent9527\requirements.toml` (Windows)
...
///
/// - admin:    managed preferences (*)
/// - system    `/etc/agent9527/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\Agent9527\config.toml` (Windows)
/// - cloud     enterprise-managed cloud config bundle fragments
/// - user      `${AGENT9527_HOME}/config.toml`
/// - profile   `${AGENT9527_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.agent9527/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.agent9527/config.toml` (loaded but disabled when untrusted)
/// - runtime   e.g., --config flags, model selector in UI
///
/// (*) Only available on macOS via managed device profiles.
///
/// See https://developers.openai.com/agent9527/security for details.
///
...
pub async fn load_config_layers_state(
    ...
    if !ignore_managed_requirements {
        if let Some(bundle) = cloud_config_bundle.get().await.map_err(io::Error::other)? {
            let cloud_config_base_dir = AbsolutePathBuf::from_absolute_path(agent9527_home)?;
        ...
        #[cfg(target_os = "macos")]
        {
            let managed_preferences_base_dir = AbsolutePathBuf::from_absolute_path(agent9527_home)?;
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, agent9527_home, overrides.clone(), strict_config)
            .await?;
    if !ignore_managed_requirements {
    ...
    // contain overrides.
    let active_user_file = overrides.user_config_path(agent9527_home)?;
    let base_user_file =
        AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, agent9527_home);
        ...
        && let Some(base_user_config) = base_user_layer.config.as_table()
    {
        ...
        if legacy_profile_is_selected || legacy_profile_table_exists {
            return Err(io::Error::new(
                ...
                format!(
                    "--profile `{active_user_profile}` cannot be used while {} contains legacy `profile = \"{active_user_profile}\"` or `[profiles.{active_user_profile}]` config; move those settings into {} and remove the legacy profile selector/table. See https://developers.openai.com/agent9527/config-advanced#profiles for more information.",
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        let raw_toml_base_dir = AbsolutePathBuf::from_absolute_path(agent9527_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/agent9527/requirements.toml"))
}
...
#[cfg(windows)]
fn windows_agent9527_system_dir() -> PathBuf {
    ...
    program_data.join("OpenAI").join("Agent9527")
}

#[cfg(windows)]
fn windows_system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    let requirements_toml_file = windows_agent9527_system_dir().join("requirements.toml");
    AbsolutePathBuf::try_from(requirements_toml_file)
}

#[cfg(windows)]
fn windows_system_config_toml_file() -> io::Result<AbsolutePathBuf> {
    let config_toml_file = windows_agent9527_system_dir().join("config.toml");
...
fn legacy_requirements_to_toml_value(legacy: LegacyManagedConfigToml) -> io::Result<TomlValue> {
    ...
    if let Some(sandbox_mode) = sandbox_mode {
        ...
        // Allowing read-only is a requirement for Agent9527 to function correctly.
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".agent9527"))
    }
}

fn project_layer_entry(
    dot_agent9527_folder: &AbsolutePathBuf,
    config: TomlValue,
    disabled_reason: Option<String>,
    hooks_config_folder_override: Option<AbsolutePathBuf>,
) -> ConfigLayerEntry {
    let source = ConfigLayerSource::Project {
        dot_agent9527_folder: dot_agent9527_folder.clone(),
    };
...
fn project_ignored_config_keys_warning(
    ...
    let config_path = dot_agent9527_folder.join(CONFIG_TOML_FILE);
...
async fn load_project_layers(
    ...
    let agent9527_home_abs = AbsolutePathBuf::from_absolute_path(agent9527_home)?;
    let agent9527_home_normalized = normalize_path(agent9527_home_abs.as_path())
        .unwrap_or_else(|_| agent9527_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        let dot_agent9527_abs = dir.join(".agent9527");
        let dot_agent9527_uri = PathUri::from_abs_path(&dot_agent9527_abs);
        if !fs
            .get_metadata(&dot_agent9527_uri, /*sandbox*/ None)
            .await
        ...
        let dot_agent9527_normalized = normalize_path(dot_agent9527_abs.as_path())
            .unwrap_or_else(|_| dot_agent9527_abs.to_path_buf());
        if dot_agent9527_abs == agent9527_home_abs
            || dot_agent9527_normalized == agent9527_home_normalized
        {
            continue;
        }
        let config_file = dot_agent9527_abs.join(CONFIG_TOML_FILE);
            ...
            Ok(contents) => {
                ...
                if disabled_reason.is_none() && strict_config {
                    validate_config_toml_strictly(
                        ...
                        &contents,
                        &config,
                        dot_agent9527_abs.as_path(),
                    )?;
                }
                let ignored_project_config_keys = sanitize_project_config(&mut config);
                let config =
                    resolve_relative_paths_in_config_toml(config, dot_agent9527_abs.as_path())?;
...
/// from a file like `/etc/agent9527/managed_config.toml` that has the same
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Agent9527").join("config.toml"))
        );
    }
}

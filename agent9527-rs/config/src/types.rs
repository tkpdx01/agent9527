//! Types used to define loaded and effective Agent9527 configuration values.
...
pub use agent9527_protocol::config_types::ModeKind;
pub use agent9527_protocol::config_types::Personality;
pub use agent9527_protocol::config_types::ServiceTier;
use agent9527_protocol::config_types::ShellEnvironmentPolicy;
use agent9527_protocol::config_types::ShellEnvironmentPolicyInherit;
...
pub enum ResumeCwdMode {
    /// Use the directory where Agent9527 was launched.
    Current,
...
/// Determine where Agent9527 should store CLI auth credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in AGENT9527_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in AGENT9527_HOME.
    Auto,
    /// Store credentials in memory only for the current process.
    Ephemeral,
}

/// Determine where Agent9527 should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum OAuthCredentialsStoreMode {
    ...
    #[default]
    Auto,
    /// AGENT9527_HOME/.credentials.json
...
pub struct AnalyticsConfigToml {
    /// When `false`, disables analytics across Agent9527 product surfaces in this profile.
...
pub struct FeedbackConfigToml {
    /// When `false`, disables the feedback flow across Agent9527 product surfaces.
    pub enabled: Option<bool>,
}
...
pub struct MemoriesToml {
    ...
    /// Minimum remaining percentage required in Agent9527 rate-limit windows before memory startup runs.
...
pub struct AppConfig {
    /// When `false`, Agent9527 does not surface this app.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
/// Fallback resize-reflow row cap when Agent9527 cannot identify a terminal-specific scrollback size.
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$AGENT9527_HOME/themes` for custom themes.
    #[serde(default)]
    pub theme: Option<String>,

    /// Pet id to preselect in the terminal pet picker.
    ///
    /// Custom pet ids resolve against AGENT9527_HOME/pets/<pet-id>/pet.json.
    #[serde(default)]
    ...
    /// transcript exceeds this cap. Omit to use Agent9527's terminal-specific default. Set to `0` to
...
/// (primarily the Agent9527 IDE extension). NOTE: these are different from
...
pub struct Notice {
    ...
    /// Tracks whether the user opted out of Agent9527-managed fast defaults.
    pub fast_default_opt_out: Option<bool>,
    /// Tracks whether the user opted out of the rate limit model switch reminder.
    pub hide_rate_limit_model_nudge: Option<bool>,
    /// Tracks whether the user has seen the model migration prompt
    pub hide_gpt5_1_migration_prompt: Option<bool>,
    /// Tracks whether the user has seen the gpt-5.1-agent9527-max migration prompt
    #[serde(rename = "hide_gpt-5.1-agent9527-max_migration_prompt")]
    pub hide_gpt_5_1_agent9527_max_migration_prompt: Option<bool>,
...
pub struct PluginMcpServerConfig {
    /// When `false`, Agent9527 skips initializing this plugin MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct MarketplaceConfig {
    /// Last time Agent9527 successfully added or refreshed this marketplace.
    #[serde(default)]
    pub last_updated: Option<String>,
    /// Git revision Agent9527 last successfully activated for this marketplace.
    #[serde(default)]

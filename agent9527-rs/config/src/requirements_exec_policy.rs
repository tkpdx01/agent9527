...
use agent9527_execpolicy::rule::PatternToken;
use agent9527_execpolicy::rule::PrefixPattern;
use agent9527_execpolicy::rule::PrefixRule;
...
pub enum RequirementsExecPolicyParseError {
    ...
    #[error(
        "rules prefix_rule at index {rule_index} has decision 'allow', which is not permitted in requirements.toml: Agent9527 merges these rules with other config and uses the most restrictive result (use 'prompt' or 'forbidden')"
    )]
...
impl RequirementsExecPolicyToml {
    ...
    /// representation used by `agent9527-execpolicy`.

...
use agent9527_rollout::parse_cursor;
...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => agent9527_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => agent9527_rollout::ThreadSortKey::UpdatedAt,
        ThreadSortKey::RecencyAt => agent9527_rollout::ThreadSortKey::RecencyAt,
    };
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => agent9527_rollout::SortDirection::Asc,
        SortDirection::Desc => agent9527_rollout::SortDirection::Desc,
    };
    let state_db = store.state_db().await;
    let rollout_config = RolloutConfig {
        agent9527_home: store.config.agent9527_home.clone(),
        sqlite: store.config.sqlite.clone(),
        cwd: store.config.agent9527_home.clone(),
...
pub(super) async fn list_rollout_threads(
    state_db: Option<agent9527_rollout::StateDbHandle>,
    config: &RolloutConfig,
    default_model_provider_id: &str,
    params: &ListThreadsParams,
    cursor: Option<&agent9527_rollout::Cursor>,
    sort_key: agent9527_rollout::ThreadSortKey,
    sort_direction: agent9527_rollout::SortDirection,
) -> ThreadStoreResult<agent9527_rollout::ThreadsPage> {
        ...
        let relation_filter = params
            ...
            .map(|relation_filter| match relation_filter {
                ThreadRelationFilter::DirectChildrenOf(parent_thread_id) => {
                    agent9527_state::ThreadRelationFilter::DirectChildrenOf(parent_thread_id)
                }
                ThreadRelationFilter::DescendantsOf(ancestor_thread_id) => {
                    agent9527_state::ThreadRelationFilter::DescendantsOf(ancestor_thread_id)
                }
            });
        let page = agent9527_rollout::state_db::list_threads_db(
            state_db.as_deref(),
            &config.sqlite,
            params.page_size,
            cursor,

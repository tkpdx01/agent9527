...
use agent9527_rollout::ThreadItem;
use agent9527_rollout::find_thread_names_by_ids;
...
pub(super) fn rollout_path_is_archived(agent9527_home: &Path, path: &Path) -> bool {
    path.starts_with(agent9527_home.join(ARCHIVED_SESSIONS_SUBDIR))
        || path
            .components()
...
pub(super) fn stored_thread_from_rollout_item(
    ...
    let preview = item
        ...
        .unwrap_or_default();
    let rollout_path = agent9527_rollout::plain_rollout_path(item.path.as_path());
...
pub(super) async fn resolve_thread_names(
    ...
    if let Ok(legacy_names) =
        find_thread_names_by_ids(store.config.agent9527_home.as_path(), &legacy_thread_ids).await
    {

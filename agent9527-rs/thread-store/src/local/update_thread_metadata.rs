...
use agent9527_rollout::ARCHIVED_SESSIONS_SUBDIR;
...
pub(super) async fn update_thread_metadata(
    ...
    agent9527_rollout::state_db::reconcile_rollout(
...
async fn refresh_resolved_rollout_path(resolved: &mut ResolvedRolloutPath) {
    if let Some(path) = agent9527_rollout::existing_rollout_path(resolved.path.as_path()).await {
        resolved.path = path;
    }
}
        ...
        async {
            ...
            if existing.is_none()
                ...
                && let Some(existing_rollout_path) =
                    agent9527_rollout::existing_rollout_path(path).await
            {
                agent9527_rollout::state_db::reconcile_rollout(
...
async fn metadata_for_missing_sqlite_row(
    ...
    patch: &ThreadMetadataPatch,
) -> ThreadStoreResult<agent9527_state::ThreadMetadata> {
...
async fn canonical_history_mode(
    ...
    let session_meta = match read_session_meta_line(rollout_path).await {
        ...
        Err(err) => {
            if agent9527_rollout::existing_rollout_path(rollout_path)
                .await
                .is_none()
...
fn normalize_cwd(cwd: PathBuf) -> PathBuf {
    agent9527_utils_path::normalize_for_path_comparison(cwd.as_path()).unwrap_or(cwd)
}
...
async fn apply_thread_git_info_to_rollout(
    ...
    session_meta.git = Some(GitInfo {
        commit_hash: sha.as_deref().map(agent9527_git_utils::GitSha::new),
        branch: branch.clone(),
...
fn rollout_path_is_archived(store: &LocalThreadStore, path: &Path) -> bool {
    path.starts_with(store.config.agent9527_home.join(ARCHIVED_SESSIONS_SUBDIR))
}
...
mod tests {
    ...
    async fn update_thread_metadata_sets_name_on_active_rollout_and_indexes_name() {
        ...
        let latest_name = agent9527_rollout::find_thread_name_by_id(home.path(), &thread_id)
            .await
    ...
    async fn paginated_name_updates_use_sqlite_without_rollout_writes() {
        ...
        assert_eq!(
            agent9527_rollout::find_thread_name_by_id(home.path(), &thread_id)
                .await
                .expect("find thread name")
                .as_deref(),
    ...
    async fn update_thread_metadata_sets_git_info() {
            ...
            .update_thread_metadata(UpdateThreadMetadataParams {
                ...
                patch: ThreadMetadataPatch {
                    git_info: Some(GitInfoPatch {
                        ...
                        origin_url: Some(Some("https://github.com/openai/agent9527".to_string())),
                    }),
                    ..Default::default()
                },
                include_archived: false,
        ...
        assert_eq!(
            git_info.repository_url.as_deref(),
            Some("https://github.com/openai/agent9527")
        );
    }
    ...
    async fn observed_metadata_normalizes_cwd_for_list_filters() {
        ...
        let normalized_cwd =
            agent9527_utils_path::normalize_for_path_comparison(workspace.as_path())

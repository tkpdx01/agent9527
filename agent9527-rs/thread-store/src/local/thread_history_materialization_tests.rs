...
use agent9527_rollout::RolloutConfig;
use agent9527_rollout::RolloutRecorder;
use agent9527_rollout::RolloutRecorderParams;
...
/// Separate Agent9527 and SQLite homes must work together across startup backfill,
...
async fn split_homes_support_backfill_listing_and_paginated_history() {
    ...
    let runtime = agent9527_rollout::state_db::try_init(&rollout_config)
        .await
        .expect("backfill state from Agent9527 home");
    assert!(
        runtime
    ...
    for sqlite_path in [&state_db_path, &thread_history_db_path] {
        ...
        assert!(
            !agent9527_home.join(filename).exists(),
            "SQLite database should not be created under Agent9527 home"
        );
    }
}
...
async fn paginated_live_append_materializes_turn_items_and_state() {
    ...
    let pool = agent9527_state::open_thread_history_db(
...
async fn sqlite_failure_does_not_fail_durable_jsonl_write() {
    ...
    config.sqlite = agent9527_state::SqliteConfig::new_for_testing(sqlite_home.as_path().abs());

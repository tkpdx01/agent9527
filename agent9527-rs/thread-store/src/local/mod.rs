...
impl LocalThreadStoreConfig {
    pub fn from_config(config: &impl agent9527_rollout::RolloutConfigView) -> Self {
        Self {
...
impl LocalThreadStore {
    ...
    pub fn new(config: LocalThreadStoreConfig, state_db: Option<StateDbHandle>) -> Self {
        let writer_lock_coordinator = Arc::new(WriterLockCoordinator::new(&config.agent9527_home));
        Self {
            config,
    ...
    async fn thread_history_db(&self) -> ThreadStoreResult<&sqlx::SqlitePool> {
        self.thread_history_db
            .get_or_try_init(|| async {
                agent9527_state::open_thread_history_db(&self.config.sqlite).await
            })
            .await
    ...
    async fn acquire_paginated_writer_locks(
        ...
        for &thread_id in thread_ids {
            ...
            {
                Some(rollout_path) => {
                    agent9527_rollout::read_session_meta_line(rollout_path.as_path())
                        .await
                        .ok()
...
mod tests {
    ...
    async fn read_thread_uses_live_writer_rollout_path_for_external_resume() {
        ...
        assert!(error.to_string().contains("must be in Agent9527 home"));
    }

...
use agent9527_rollout::RolloutReferenceIndex;
use agent9527_rollout::SESSIONS_SUBDIR;
use agent9527_rollout::find_archived_thread_path_by_id_str;
use agent9527_rollout::find_thread_path_by_id_str;
use agent9527_rollout::remove_thread_name_entries;
...
async fn scan_reference_index(
    ...
    RolloutReferenceIndex::scan(store.config.agent9527_home.as_path())
        .await
...
fn referenced_thread_error(thread_id: agent9527_protocol::ThreadId) -> ThreadStoreError {
...
async fn delete_thread_after_reference_check(
    ...
    remove_thread_name_entries(store.config.agent9527_home.as_path(), thread_id)
        .await
        .map_err(|err| ThreadStoreError::Internal {
...
fn delete_rollout_file(
    ...
    let plain_path = agent9527_rollout::plain_rollout_path(rollout_path);
...
fn delete_rollout_path(
    ...
    let canonical_rollout_path = scoped_rollout_path(
        store.config.agent9527_home.join(SESSIONS_SUBDIR),
        rollout_path,
        "sessions",
    )
    .or_else(|_| {
        scoped_rollout_path(
            store.config.agent9527_home.join(ARCHIVED_SESSIONS_SUBDIR),
            rollout_path,
            "archived sessions",
        )
    })

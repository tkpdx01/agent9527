...
use super::AGENT9527_HOME_FLAG;
use super::AGENT9527_WINDOWS_SANDBOX_ARG1;
...
fn windows_wrapper_args_round_trip() {
    ...
    let args = create_windows_sandbox_command_args_for_permission_profile(
        vec![
            "agent9527.exe".to_string(),
            "--agent9527-run-as-fs-helper".to_string(),
        ],
        &command_cwd,
        ...
        Path::new(r"C:\Users\me\.agent9527"),
    );

    assert_eq!(args[0], AGENT9527_WINDOWS_SANDBOX_ARG1);
    assert!(args.contains(&AGENT9527_HOME_FLAG.to_string()));
    ...
    assert_eq!(
        parsed.command,
        vec!["agent9527.exe", "--agent9527-run-as-fs-helper"]
    );

...
pub fn current_log_file_path_for_agent9527_home(agent9527_home: &Path) -> PathBuf {
    current_log_file_path(&crate::sandbox_dir(agent9527_home))
}
...
mod tests {
    ...
    #[test]
    fn current_log_file_path_for_agent9527_home_uses_sandbox_dir() {
        let agent9527_home = Path::new("agent9527-home");

        assert_eq!(
            current_log_file_path_for_agent9527_home(agent9527_home),
            current_log_file_path(&agent9527_home.join(".sandbox"))
        );
    }
}

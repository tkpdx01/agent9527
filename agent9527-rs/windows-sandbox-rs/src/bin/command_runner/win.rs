...
use agent9527_windows_sandbox::ExitPayload;
use agent9527_windows_sandbox::FramedMessage;
use agent9527_windows_sandbox::IPC_PROTOCOL_VERSION;
use agent9527_windows_sandbox::LocalSid;
use agent9527_windows_sandbox::Message;
use agent9527_windows_sandbox::OutputPayload;
use agent9527_windows_sandbox::OutputStream;
use agent9527_windows_sandbox::PipeSpawnHandles;
use agent9527_windows_sandbox::ResizePayload;
use agent9527_windows_sandbox::SpawnReady;
use agent9527_windows_sandbox::SpawnRequest;
use agent9527_windows_sandbox::StderrMode;
use agent9527_windows_sandbox::StdinMode;
use agent9527_windows_sandbox::WindowsSandboxTokenMode;
use agent9527_windows_sandbox::allow_null_device;
use agent9527_windows_sandbox::create_readonly_token_with_caps_and_user_from;
use agent9527_windows_sandbox::create_workspace_write_token_with_caps_and_user_from;
use agent9527_windows_sandbox::decode_bytes;
use agent9527_windows_sandbox::encode_bytes;
use agent9527_windows_sandbox::get_current_token_for_restriction;
use agent9527_windows_sandbox::hide_current_user_profile_dir;
use agent9527_windows_sandbox::log_note;
use agent9527_windows_sandbox::read_frame;
use agent9527_windows_sandbox::read_handle_loop;
use agent9527_windows_sandbox::spawn_process_with_pipes;
use agent9527_windows_sandbox::to_wide;
use agent9527_windows_sandbox::token_mode_for_permission_profile;
use agent9527_windows_sandbox::write_frame;
use anyhow::Context;
use anyhow::Result;
use std::ffi::OsStr;
use std::fs::File;
...
// Kept in sync with agent9527_exec_server::AGENT9527_FS_HELPER_ARG1 without introducing
// a dependency cycle.
const FS_HELPER_ARG: &str = "--agent9527-run-as-fs-helper";
const READ_ACL_MUTEX_NAME: &str = "Local\\Agent9527SandboxReadAcl";
...
struct IpcSpawnedProcess {
    ...
    conpty_owner: Option<agent9527_windows_sandbox::ConptyInstance>,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.agent9527_home.clone();
    hide_current_user_profile_dir(req.agent9527_home.as_path());
    ...
    let (pi, job, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, mut conpty) = agent9527_windows_sandbox::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,

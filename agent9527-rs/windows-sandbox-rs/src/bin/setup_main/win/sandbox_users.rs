...
use agent9527_windows_sandbox::dpapi_protect;
...
pub const SANDBOX_USERS_GROUP: &str = "Agent9527SandboxUsers";
const SANDBOX_USERS_GROUP_COMMENT: &str = "Agent9527 sandbox internal group (managed)";
...
fn write_secrets(
    ...
    let secrets_dir = sandbox_secrets_dir(agent9527_home);
...
pub(super) fn prepare_setup_marker(agent9527_home: &Path, real_user: &str) -> Result<()> {
    let marker_path = sandbox_dir(agent9527_home).join("setup_marker.json");

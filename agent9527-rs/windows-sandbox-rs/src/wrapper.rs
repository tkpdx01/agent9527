//! Internal `agent9527.exe --run-as-windows-sandbox` wrapper.
//!
...
pub const AGENT9527_WINDOWS_SANDBOX_ARG1: &str = "--run-as-windows-sandbox";

const COMMAND_CWD_FLAG: &str = "--command-cwd";
const AGENT9527_HOME_FLAG: &str = "--agent9527-home";
...
pub fn create_windows_sandbox_command_args_for_permission_profile(
    ...
    let mut args = vec![
        AGENT9527_WINDOWS_SANDBOX_ARG1.to_string(),
        AGENT9527_HOME_FLAG.to_string(),
        agent9527_home.to_string_lossy().into_owned(),
...
async fn run_windows_sandbox_wrapper_request(request: WindowsSandboxWrapperRequest) -> Result<i32> {
    ...
    let spawned =
        crate::spawn_windows_sandbox_session_for_level(crate::WindowsSandboxSessionRequest {
            ...
            agent9527_home: request.agent9527_home.as_path(),
...
fn parse_windows_sandbox_wrapper_args(args: Vec<String>) -> Result<WindowsSandboxWrapperRequest> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            AGENT9527_HOME_FLAG => {
                agent9527_home = Some(PathBuf::from(next_flag_value(&mut args, &arg)?))
            }
            COMMAND_CWD_FLAG => {
    ...
    let agent9527_home =
        agent9527_home.ok_or_else(|| anyhow!("missing required {AGENT9527_HOME_FLAG}"))?;
    if !agent9527_home.is_absolute() {
        bail!(
            "{AGENT9527_HOME_FLAG} must be absolute: {}",

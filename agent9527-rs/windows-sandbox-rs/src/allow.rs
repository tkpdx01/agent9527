...
mod tests {
    ...
    #[test]
    fn denies_agent9527_and_agents_inside_writable_root() {
        let tmp = TempDir::new().expect("tempdir");
        let command_cwd = tmp.path().join("workspace");
        let agent9527_dir = command_cwd.join(".agent9527");
        let agents_dir = command_cwd.join(".agents");
        let _ = fs::create_dir_all(&agent9527_dir);
        ...
        let expected_deny: HashSet<PathBuf> = [
            dunce::canonicalize(&agent9527_dir).unwrap(),

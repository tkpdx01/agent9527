...
# Run a suite of smoke tests against the Windows sandbox via the Agent9527 CLI
...
def _resolve_agent9527_cmd() -> List[str]:
    """Resolve the Agent9527 CLI to invoke `agent9527 sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run Agent9527.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "agent9527.exe",
        ws_root / "target" / "release" / "agent9527.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "agent9527.exe",
            cargo_base / "release" / "agent9527.exe",
        ])
    ...
    if shutil.which("agent9527"):
        return ["agent9527"]

    raise FileNotFoundError(
        "Agent9527 CLI not found. Build it first, e.g.\n"
        "  cargo build -p agent9527-cli --release\n"
        "or for debug:\n"
        "  cargo build -p agent9527-cli\n"
    )

AGENT9527_CMD = _resolve_agent9527_cmd()
print(AGENT9527_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to agent9527 CLI overrides.
    ...
    argv = [*AGENT9527_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"AGENT9527_HOME": str(proxy_home)},
            )
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\agent9527_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.agent9527 artifacts) denied
    agent9527_home = Path(os.environ["USERPROFILE"]) / ".agent9527"
    cap_sid_target = agent9527_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .agent9527\\policy.json"], WS_ROOT)
    add("WS: .agent9527 cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .agent9527 policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > agent9527_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://agent9527-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(

#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $AGENT9527_HOME/skills."""
...
def _agent9527_home() -> str:
    return os.environ.get("AGENT9527_HOME", os.path.expanduser("~/.agent9527"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "agent9527")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "agent9527-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_agent9527_home(), "skills")

...
- the user wants Agent9527 to see the updated plugin without manually editing marketplace files
...
1. Update the plugin manifest to a single Agent9527 cachebuster suffix:
...
outside Agent9527 depends on a specific token:
...
```bash
agent9527 plugin add <plugin-name>@<marketplace-name-from-marketplace-json>
```

The default personal marketplace is discovered implicitly from
`~/.agents/plugins/marketplace.json`. You do not need `agent9527 plugin marketplace add` for that
path, and `agent9527 plugin marketplace list` is not the right check for whether that default
marketplace exists.
...
```bash
agent9527 plugin add <plugin-name>@<local-marketplace>
```

6. Prompt the user to use a new thread to try the updated plugin, so that Agent9527 picks up new skills
   and tools.
...
```text
<base-version>+agent9527.<cachebuster>
```

Examples:

- `0.1.0` → `0.1.0+agent9527.local-20260519-184516`
- `0.1.0+agent9527.old-token` → `0.1.0+agent9527.local-20260519-184516`
- `1.2.3-beta.1+agent9527.prev` → `1.2.3-beta.1+agent9527.local-20260519-184516`
- `dev-build+other-tag` → `dev-build+agent9527.local-20260519-184516`

Replace the existing Agent9527 cachebuster instead of appending another one. Do not keep incrementing
...
- Read the personal marketplace name with
  ...
  `agent9527 plugin add <plugin-name>@<marketplace-name>`.
...
- Do not tell the user to run `agent9527 plugin marketplace add` for the default personal-marketplace
  flow. That marketplace is discovered implicitly by Agent9527.
...
- Use `agent9527 plugin list` when the plugin lives in a different configured marketplace and you need
...
- If a non-default local marketplace has not been configured yet, install it with
  `agent9527 plugin marketplace add <path-to-marketplace-root>` before telling the user to run
  `agent9527 plugin add <plugin-name>@<marketplace-name>`.

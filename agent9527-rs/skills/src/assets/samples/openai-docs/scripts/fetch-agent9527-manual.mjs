...
const DEFAULT_MANUAL_URL = "https://developers.openai.com/agent9527/agent9527-manual.md";
const DEFAULT_CACHE_DIR_NAME = "openai-docs-cache";
const CACHE_FILE_NAME = "agent9527-manual.md";
const OUTLINE_FILE_NAME = "agent9527-manual.outline.md";
const HASH_HEADER = "x-content-sha256";
const USER_AGENT = "agent9527-openai-docs";
...
const tempFilePath = (cacheDir, suffix) =>
  path.join(
    cacheDir,
    `.fetch-agent9527-manual-${process.pid}-${Date.now()}-${Math.random()
      .toString(16)
      .slice(2)}${suffix}`
  );
...
const outlineMarkdown = (outline) => `# Agent9527 Manual Outline\n\n${outline.text}\n`;
...
const fetchAgent9527Manual = async ({
...
const main = async () => {
  ...
  const { outlineText, status } = await fetchAgent9527Manual(args);
...
export { DEFAULT_MANUAL_URL, fetchAgent9527Manual };

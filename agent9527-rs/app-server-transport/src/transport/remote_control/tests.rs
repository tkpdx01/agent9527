...
use agent9527_app_server_protocol::ServerNotificationEnvelope;
use agent9527_config::types::AuthCredentialsStoreMode;
use agent9527_core::test_support::auth_manager_from_auth;
use agent9527_core::test_support::auth_manager_from_auth_with_home;
...
use agent9527_login::save_auth;
use agent9527_login::token_data::TokenData;
use agent9527_login::token_data::parse_chatgpt_jwt_claims;
...
fn remote_control_auth_manager() -> Arc<AuthManager> {
    auth_manager_from_auth(Agent9527Auth::create_dummy_chatgpt_auth_for_testing())
}

fn remote_control_auth_manager_with_home(agent9527_home: &TempDir) -> Arc<AuthManager> {
...
async fn remote_control_state_runtime(agent9527_home: &TempDir) -> Arc<StateRuntime> {
    StateRuntime::init(
...
async fn plain_start_resolves_persisted_remote_control_preference() {
    ...
    let agent9527_home = TempDir::new().expect("temp dir should create");
    let state_db = remote_control_state_runtime(&agent9527_home).await;
...
async fn ephemeral_enable_preserves_durable_preference() {
    ...
    remote_handle.state_db = Some(remote_control_state_runtime(&agent9527_home).await);
    remote_handle
        .desired_state_tx
...
async fn remote_control_transport_manages_virtual_clients_and_routes_messages() {
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Notification(
                    agent9527_app_server_protocol::JSONRPCNotification {
    ...
    let initialize_message =
        JSONRPCMessage::Request(agent9527_app_server_protocol::JSONRPCRequest {
            id: agent9527_app_server_protocol::RequestId::Integer(1),
    ...
    let followup_message =
        JSONRPCMessage::Notification(agent9527_app_server_protocol::JSONRPCNotification {
            method: "initialized".to_string(),
            params: None,
        });
    send_client_event(
...
async fn remote_control_transport_reconnects_after_disconnect() {
    ...
    let (remote_task, remote_handle) = start_remote_control(
        ...
        Some(remote_control_state_runtime(&agent9527_home).await),
        remote_control_auth_manager(),
        transport_event_tx,
        shutdown_token.clone(),
        /*app_server_client_name_rx*/ None,
    ...
    send_client_event(
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Request(agent9527_app_server_protocol::JSONRPCRequest {
                    id: agent9527_app_server_protocol::RequestId::Integer(2),
                    method: "initialize".to_string(),
                    params: Some(json!({
                        "clientInfo": {
                            "name": "remote-test-client",
...
async fn remote_control_http_mode_enrolls_before_connecting() {
    ...
    assert_eq!(
        handshake_request.headers.get("x-agent9527-server-id"),
        Some(&"srv_e_test".to_string())
    );
    assert_eq!(
        handshake_request.headers.get("x-agent9527-name"),
        Some(&base64::engine::general_purpose::STANDARD.encode(&expected_server_name))
    );
    assert_eq!(
        handshake_request
            .headers
            .get("x-agent9527-protocol-version"),
    ...
    let writer = {
        let initialize_message =
            JSONRPCMessage::Request(agent9527_app_server_protocol::JSONRPCRequest {
                id: agent9527_app_server_protocol::RequestId::Integer(11),
                method: "initialize".to_string(),
                params: Some(json!({
                    "clientInfo": {
    ...
    writer
        .send(QueuedOutgoingMessage::new(OutgoingMessage::Response(
            crate::outgoing_message::OutgoingResponse {
                id: agent9527_app_server_protocol::RequestId::Integer(11),
                result: json!({
                    "userAgent": "agent9527-test-agent"
                }),
            },
        )))
        .await
...
async fn remote_control_http_mode_refreshes_persisted_enrollment_before_connecting() {
    ...
    let (remote_task, _remote_handle) = start_remote_control(
        ...
        Some(state_db.clone()),
        remote_control_auth_manager_with_home(&agent9527_home),
        transport_event_tx,
        shutdown_token.clone(),
        /*app_server_client_name_rx*/ None,
        RemoteControlStartupMode::EnabledEphemeral,
...
async fn remote_control_http_mode_reenrolls_after_explicit_missing_server_404() {
    ...
    assert_eq!(
        websocket_request.headers.get("x-agent9527-server-id"),

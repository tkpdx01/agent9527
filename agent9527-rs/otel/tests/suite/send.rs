...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "agent9527-cli"), ("env", "prod")])?;

    metrics.counter_with_description(
        "agent9527.turns",
        "Total number of Agent9527 turns.",
        /*inc*/ 1,
        &[("model", "gpt-5.1"), ("env", "dev")],
    )?;
    metrics.histogram(
        "agent9527.tool_latency",
        /*value*/ 25,
        &[("tool", "shell")],
    )?;
    metrics.gauge_with_description(
        "agent9527.active",
        "Number of active Agent9527 operations.",
        /*value*/ 2,
        &[("component", "test")],
    )?;
    metrics.shutdown()?;
    ...
    let counter =
        find_metric(&resource_metrics, "agent9527.turns").expect("counter metric missing");
    assert_eq!(counter.description(), "Total number of Agent9527 turns.");
    ...
    let expected_counter_attributes = BTreeMap::from([
        ("service".to_string(), "agent9527-cli".to_string()),
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "agent9527.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        find_metric(&resource_metrics, "agent9527.tool_latency")
            ...
            .expect("agent9527.tool_latency histogram attributes should exist"),
    );
    ...
    let gauge = find_metric(&resource_metrics, "agent9527.active").expect("gauge metric missing");
    assert_eq!(
        gauge.description(),
        "Number of active Agent9527 operations."
    );
...
fn send_merges_default_tags_per_line() -> Result<()> {
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "agent9527.alpha").expect("agent9527.alpha metric missing");
    ...
    let beta_metric =
        find_metric(&resource_metrics, "agent9527.beta").expect("agent9527.beta metric missing");
...
fn client_sends_enqueued_metric() -> Result<()> {
    ...
    metrics.counter("agent9527.turns", /*inc*/ 1, &[("model", "gpt-5.1")])?;
    metrics.shutdown()?;
...
fn shutdown_flushes_in_memory_exporter() -> Result<()> {
    ...
    metrics.counter("agent9527.turns", /*inc*/ 1, &[])?;
    metrics.shutdown()?;

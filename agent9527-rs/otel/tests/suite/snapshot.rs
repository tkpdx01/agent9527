...
fn snapshot_collects_metrics_without_shutdown() -> Result<()> {
    ...
    .with_tag("service", "agent9527-cli")?
    .with_runtime_reader();
    ...
    let metric = find_metric(&snapshot, "agent9527.tool.call").expect("counter metric missing");
...
fn observable_gauge_is_collected_on_every_delta_snapshot() -> Result<()> {
    let exporter = InMemoryMetricExporter::default();
    let config =
        MetricsConfig::in_memory("test", "agent9527-cli", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader();
    let metrics = MetricsClient::new(config)?;
    ...
    for snapshot in [metrics.snapshot()?, metrics.snapshot()?] {
        let gauge = find_metric(&snapshot, "agent9527.active").expect("gauge metric missing");

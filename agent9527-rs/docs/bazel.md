# Bazel in agent9527-rs

This repository uses Bazel to build the Rust workspace under `agent9527-rs`.
...
- `rules_rs` imports third-party crates from `agent9527-rs/Cargo.toml` and
  `agent9527-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `agent9527_rust_crate`, which wraps `rust_library`,
...
- Each crate in `agent9527-rs/*/BUILD.bazel` typically uses `agent9527_rust_crate` and
...
Agent9527 uses BuildBuddy for a shared Bazel cache and remoted builds and tests. To use it
...
| --- | --- | --- |
| Push to `main` in `openai/agent9527` | Yes | Yes |
| `workflow_dispatch` in `openai/agent9527` | Yes | Yes |
| Same-repository pull request in `openai/agent9527` | Yes | Yes |
| Fork pull request into `openai/agent9527` | No | No; local |
...
```bash
BUILDBUDDY_API_KEY=... GITHUB_REPOSITORY=my-fork/agent9527 \
...
run in `openai/agent9527`. A missing or malformed pull request event
...
2. Create a `BUILD.bazel` that calls `agent9527_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `agent9527_rust_crate` to configure it.

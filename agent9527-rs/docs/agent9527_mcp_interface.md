# Agent9527 MCP Server Interface [experimental]

This document describes Agent9527's experimental MCP server interface: a JSON-RPC API that runs over the Model Context Protocol (MCP) transport to control a local Agent9527 engine.

- Status: experimental and subject to change without notice
- Server binary: `agent9527 mcp-server` (or `agent9527-mcp-server`)
- Transport: standard MCP over stdio (JSON-RPC 2.0, line-delimited)

## Overview

Agent9527 exposes MCP-compatible methods to manage threads, turns, accounts, config, and approvals. The types live in `app-server-protocol/src/protocol/{common,v1,v2}.rs` and are consumed by the app server implementation in `app-server/`.
...
- Notifications
  ...
  - `agent9527/event/*` stream notifications for live agent events
...
Run Agent9527 as an MCP server and connect an MCP client:

```bash
agent9527 mcp-server | your_mcp_client
```

For a simple inspection UI, you can also try:

```bash
npx @modelcontextprotocol/inspector agent9527 mcp-server
```

Use the separate `agent9527 mcp` subcommand to manage configured MCP server launchers in `config.toml`.
...
Fetch the catalog of models available in the current Agent9527 build with `model/list`. The request accepts optional pagination inputs:
...
- `agent9527/event` with the serialized Agent9527 event payload. The shape matches `core/src/protocol.rs`'s `Event` and `EventMsg` types. Some notifications include a `_meta.requestId` to correlate with the originating request.
...
The `agent9527` and `agent9527-reply` tools return standard MCP `CallToolResult` payloads. For compatibility with MCP clients that prefer `structuredContent`, Agent9527 mirrors the content blocks inside `structuredContent` alongside the `threadId`.

Example:

```json
{
  "content": [{ "type": "text", "text": "Hello from Agent9527" }],
  "structuredContent": {
    "threadId": "019bbed6-1e9e-7f31-984c-a05b65045719",
    "content": "Hello from Agent9527"
  }
}
```
...
When Agent9527 needs approval to apply changes or run commands, the server issues JSON-RPC requests to the client:

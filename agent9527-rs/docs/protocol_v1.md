...
These are entities exit on the agent9527 backend. The intent of this section is to establish vocabulary and construct a shared mental model for the `Agent9527` core system.

0. `Model`
   - In our case, this is the Responses REST API
1. `Agent9527`
   - The core engine of agent9527
...
2. `Session`
   - The `Agent9527`'s current configuration and state
   - `Agent9527` starts with no `Session`, and it is initialized by `Op::ConfigureSession`, which should be the first message sent by the UI.
...
3. `Task`
   - A `Task` is `Agent9527` executing work in response to user input.
...
4. `Turn`
   - One cycle of iteration in a `Task`, consists of:
     ...
     - `Agent9527` then executes command(s), applies patch(es), and outputs message(s) returned by the `Model`
...
The term "UI" is used to refer to the application driving `Agent9527`. This may be the CLI / TUI chat-like interface that users operate, or it may be a GUI interface like a VSCode extension. The UI is external to `Agent9527`, as `Agent9527` is intended to be operated by arbitrary UI implementations.
...
Since only 1 `Task` can be run at a time, for parallel tasks it is recommended that a single `Agent9527` be run for each thread of work.
...
- `Submission`
  - These are messages sent on the `SQ` (UI -> `Agent9527`)
...
- `Event`
  - These are messages sent on the `EQ` (`Agent9527` -> UI)
...
sequenceDiagram
    ...
    box Daemon
    participant agent9527 as Agent9527
    ...
    user->>agent9527: Op::ConfigureSession
    agent9527-->>session: create session
    agent9527->>user: Event::SessionConfigured

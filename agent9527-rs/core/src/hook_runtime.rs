...
use agent9527_analytics::HookRunFact;
use agent9527_analytics::build_track_events_context;
use agent9527_hooks::PermissionRequestDecision;
use agent9527_hooks::PermissionRequestOutcome;
use agent9527_hooks::PermissionRequestRequest;
use agent9527_hooks::PostToolUseOutcome;
use agent9527_hooks::PostToolUseRequest;
use agent9527_hooks::PreToolUseOutcome;
use agent9527_hooks::PreToolUseRequest;
use agent9527_hooks::SessionStartOutcome;
use agent9527_hooks::StartHookTarget;
use agent9527_hooks::StopHookTarget;
use agent9527_hooks::StopOutcome;
use agent9527_hooks::SubagentHookContext;
use agent9527_hooks::UserPromptSubmitOutcome;
use agent9527_hooks::UserPromptSubmitRequest;
use agent9527_otel::HOOK_RUN_DURATION_METRIC;
use agent9527_otel::HOOK_RUN_METRIC;
...
use agent9527_protocol::protocol::HookCompletedEvent;
use agent9527_protocol::protocol::HookEventName;
use agent9527_protocol::protocol::HookRunStatus;
use agent9527_protocol::protocol::HookRunSummary;
use agent9527_protocol::protocol::HookSource;
use agent9527_protocol::protocol::HookStartedEvent;
        ...
        let target = match &turn_context.session_source {
            SessionSource::SubAgent(SubAgentSource::ThreadSpawn { agent_role, .. })
                if matches!(
                    ...
                    agent9527_hooks::SessionStartSource::Startup
                ) =>
            {
...
pub(crate) async fn run_turn_stop_hooks(
    ...
    let request = agent9527_hooks::StopRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_session_end_hooks(sess: &Arc<Session>) {
    ...
    let request = agent9527_hooks::SessionEndRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = agent9527_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = agent9527_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(agent9527_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: agent9527_hooks::HookEvent::AfterAgent {
                event: agent9527_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            agent9527_hooks::HookResult::Success => continue,
            agent9527_hooks::HookResult::FailedContinue(error) => (error, false),
            agent9527_hooks::HookResult::FailedAbort(error) => (error, true),
        };
    ...
    let event = EventMsg::Error(agent9527_protocol::protocol::ErrorEvent {
        message,
...
fn hook_run_analytics_payload(
    ...
    completed: &HookCompletedEvent,
) -> (agent9527_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use agent9527_protocol::protocol::HookEventName;
    use agent9527_protocol::protocol::HookExecutionMode;
    use agent9527_protocol::protocol::HookHandlerType;
    use agent9527_protocol::protocol::HookRunStatus;
    use agent9527_protocol::protocol::HookScope;
    use agent9527_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    agent9527_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {

...
use agent9527_analytics::GuardianReviewDecision;
use agent9527_analytics::GuardianReviewFailureReason;
use agent9527_analytics::GuardianReviewTerminalStatus;
use agent9527_analytics::GuardianReviewTrackContext;
use agent9527_analytics::GuardianReviewedAction;
...
use agent9527_protocol::protocol::GuardianAssessmentDecisionSource;
...
impl GuardianReviewError {
    ...
    fn session_with_error_info(err: anyhow::Error, error_info: Agent9527ErrorInfo) -> Self {
        Self::Session {
            message: err.to_string(),
...
pub(crate) fn is_guardian_reviewer_source(
    session_source: &agent9527_protocol::protocol::SessionSource,
) -> bool {
    matches!(
        session_source,
        agent9527_protocol::protocol::SessionSource::SubAgent(SubAgentSource::Other(label))
...
pub(crate) fn spawn_approval_request_review(
    ...
    let spawn_result = std::thread::Builder::new()
        .name("agent9527-approval-review".to_string())
        .spawn(move || {
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = review_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset.supported_reasoning_efforts.iter().any(|effort| {
                effort.effort == agent9527_protocol::openai_models::ReasoningEffort::Low
            }),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| {
                    preset.effort == agent9527_protocol::openai_models::ReasoningEffort::Low
                }),
            ...
            GuardianReviewError::Session {
                error_info: Some(
                    Agent9527ErrorInfo::ServerOverloaded
                        ...
                        | Agent9527ErrorInfo::InternalServerError
                        | Agent9527ErrorInfo::ResponseStreamDisconnected { .. }
                ),
                ..
            } | GuardianReviewError::Parse { .. }
        )

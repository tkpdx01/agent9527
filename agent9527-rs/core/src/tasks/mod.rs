...
use agent9527_analytics::TurnProfileFact;
use agent9527_analytics::TurnTokenUsageFact;
...
use agent9527_otel::TURN_E2E_DURATION_METRIC;
use agent9527_otel::TURN_MEMORY_METRIC;
use agent9527_otel::TURN_NETWORK_PROXY_METRIC;
use agent9527_otel::TURN_TOKEN_USAGE_METRIC;
use agent9527_otel::TURN_TOOL_CALL_METRIC;
...
const TASK_COMPACT_METRIC: &str = "agent9527.task.compact";

pub(crate) type SessionTaskResult = Agent9527Result<Option<String>>;
...
///
/// Implementations encapsulate a specific Agent9527 workflow (regular chat,
...
pub(crate) trait SessionTask: Send + Sync + 'static {
    ...
    /// [`Agent9527Err::TurnAborted`] completes the task through the aborted-turn
    /// lifecycle instead.
    fn run(
        self: Arc<Self>,
...
impl Session {
    ...
    pub(crate) async fn start_task<T: SessionTask>(
        ...
        let task_span = info_span!(
            ...
            agent9527.turn.reasoning_effort = %reasoning_effort,
            agent9527.turn.token_usage.input_tokens = field::Empty,
            agent9527.turn.token_usage.cached_input_tokens = field::Empty,
            agent9527.turn.token_usage.cache_write_input_tokens = field::Empty,
            agent9527.turn.token_usage.non_cached_input_tokens = field::Empty,
            agent9527.turn.token_usage.output_tokens = field::Empty,
            agent9527.turn.token_usage.reasoning_output_tokens = field::Empty,
            agent9527.turn.token_usage.total_tokens = field::Empty,
        );
                ...
                if let Err(err) = sess.flush_rollout().await {
                    ...
                    sess.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                            message: format!(
                                "Failed to save the conversation transcript; Agent9527 will continue retrying. Error: {err}"
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) fn has_outstanding_durable_sleep(&self) -> bool {
        self.services
            ...
            .get::<agent9527_extension_items::sleep::SleepItem>()
            .is_some()
    }
    ...
    pub async fn on_task_finished(
        ...
        // Emit token usage metrics.
        {
            ...
            current_span.record(
                "agent9527.turn.token_usage.input_tokens",
                turn_token_usage.input_tokens,
            );
            current_span.record(
                "agent9527.turn.token_usage.cached_input_tokens",
                turn_token_usage.cached_input(),
            );
            current_span.record(
                "agent9527.turn.token_usage.cache_write_input_tokens",
                turn_token_usage.cache_write_input_tokens,
            );
            current_span.record(
                "agent9527.turn.token_usage.non_cached_input_tokens",
                turn_token_usage.non_cached_input(),
            );
            current_span.record(
                "agent9527.turn.token_usage.output_tokens",
                turn_token_usage.output_tokens,
            );
            current_span.record(
                "agent9527.turn.token_usage.reasoning_output_tokens",
                turn_token_usage.reasoning_output_tokens,
            );
            current_span.record(
                "agent9527.turn.token_usage.total_tokens",

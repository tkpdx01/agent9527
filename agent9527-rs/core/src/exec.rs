...
use agent9527_protocol::error::SandboxErr;
...
use agent9527_protocol::protocol::ExecCommandOutputDeltaEvent;
use agent9527_protocol::protocol::ExecOutputStream;
use agent9527_sandboxing::SandboxCommand;
use agent9527_sandboxing::SandboxManager;
use agent9527_sandboxing::SandboxTransformRequest;
use agent9527_sandboxing::SandboxType;
use agent9527_sandboxing::SandboxablePreference;
use agent9527_sandboxing::WindowsSandboxFilesystemOverrides;
pub(crate) use agent9527_sandboxing::is_likely_sandbox_denied;
#[cfg(test)]
use agent9527_sandboxing::permission_profile_supports_windows_restricted_token_sandbox;
use agent9527_sandboxing::resolve_windows_elevated_filesystem_overrides;
use agent9527_sandboxing::resolve_windows_restricted_token_filesystem_overrides;
#[cfg(test)]
use agent9527_sandboxing::unsupported_windows_restricted_token_sandbox_reason;
use agent9527_sandboxing::windows_sandbox_uses_elevated_backend;
...
use agent9527_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: agent9527_protocol::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: agent9527_protocol::config_types::WindowsSandboxLevel,
...
fn network_proxy_environment_error(
    ...
    Agent9527Err::Io(io::Error::other(format!(
...
pub fn build_exec_request(
    ...
    let mut exec_req = manager
        .transform(SandboxTransformRequest {
            ...
            agent9527_linux_sandbox_exe: agent9527_linux_sandbox_exe.as_deref(),
            use_legacy_landlock,
            windows_sandbox_level,
            windows_sandbox_private_desktop,
        })
    ...
    .map_err(Agent9527Err::UnsupportedOperation)?;
    Ok(exec_req)
}
...
pub(crate) async fn execute_exec_request(
    ...
    let cwd = cwd
        .to_abs_path()
        .map_err(|err| Agent9527Err::InvalidRequest(format!("invalid exec cwd: {err}")))?;
    ...
    let windows_sandbox_policy_cwd = windows_sandbox_policy_cwd
        .to_abs_path()
        .map_err(|err| Agent9527Err::InvalidRequest(format!("invalid sandbox cwd: {err}")))?;
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        agent9527_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    };
    if let Some(metrics) = agent9527_otel::global() {
        let _ = metrics.counter(
            "agent9527.windows_sandbox.createprocessasuserw_failed",
            /*inc*/ 1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_agent9527_home;
    use agent9527_windows_sandbox::run_windows_sandbox_capture_for_permission_profile_elevated;
    use agent9527_windows_sandbox::run_windows_sandbox_capture_with_filesystem_overrides;
    ...
    let (cancellation, timeout_ms) = if capture_policy.uses_expiration() {
        let cancellation = expiration.cancellation_token().map(|token| {
            agent9527_windows_sandbox::WindowsSandboxCancellationToken::new(move || {
                token.is_cancelled()
            })
        });
    ...
    let agent9527_home = find_agent9527_home().map_err(|err| {
        Agent9527Err::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve agent9527_home: {err}"
        )))
    })?;
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_for_permission_profile_elevated(
                agent9527_windows_sandbox::ElevatedSandboxProfileCaptureRequest {
                    permission_profile: &permission_profile,
                    workspace_roots: workspace_roots.as_slice(),
                    agent9527_home: agent9527_home.as_ref(),
                    command,
                    cwd: &cwd,
                    env_map: env,
                    timeout_ms,
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(Agent9527Err::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, Agent9527Err>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(Agent9527Err::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(Agent9527Err::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(Agent9527Err::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
...
/// AGENT9527_SANDBOX_NETWORK_DISABLED=1 is added to the environment of the spawned
/// process.
///
...
async fn consume_output(
    ...
    let stdout_reader = child.stdout.take().ok_or_else(|| {
        Agent9527Err::Io(io::Error::other(
            ...
            match outcome {
                ...
                Some(ExecExpirationOutcome::Cancelled) => {
                    ...
                    let should_escalate = if let Some(process_group_id) = process_group_id {
                        agent9527_utils_pty::process_group::terminate_process_group(process_group_id)?
                    } else {
                        false
                    };
                    ...
                    .await
                    {
                        Ok(status) => {
                            ...
                            {
                                agent9527_utils_pty::process_group::kill_process_group(
                                    process_group_id,
                                )?;
                            }
                        }

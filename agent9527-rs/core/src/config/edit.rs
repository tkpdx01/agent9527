...
pub fn apply_blocking(agent9527_home: &Path, edits: &[ConfigEdit]) -> anyhow::Result<()> {
...
///
pub async fn apply(agent9527_home: &Path, edits: Vec<ConfigEdit>) -> anyhow::Result<()> {
...
impl ConfigEditsBuilder {
    pub fn new(agent9527_home: &Path) -> Self {
        Self::for_config_path(&agent9527_home.join(CONFIG_TOML_FILE))
    }
    ...
    pub fn for_config(config: &crate::config::Config) -> Self {
        let config_path = config
            ...
            .map(agent9527_utils_absolute_path::AbsolutePathBuf::to_path_buf)
            .unwrap_or_else(|| config.agent9527_home.join(CONFIG_TOML_FILE).to_path_buf());

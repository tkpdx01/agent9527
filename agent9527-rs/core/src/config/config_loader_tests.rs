...
use agent9527_config::ConfigError;
use agent9527_config::ConfigLayerEntry;
use agent9527_config::ConfigLayerSource;
use agent9527_config::ConfigLayerStackOrdering;
use agent9527_config::ConfigLoadError;
use agent9527_config::ConfigLoadOptions;
use agent9527_config::ConfigRequirements;
use agent9527_config::ConfigRequirementsToml;
use agent9527_config::ConfigRequirementsWithSources;
use agent9527_config::FilesystemDenyReadPattern;
use agent9527_config::LoaderOverrides;
use agent9527_config::RequirementSource;
use agent9527_config::RequirementsLayerEntry;
use agent9527_config::SessionThreadConfig;
use agent9527_config::StaticThreadConfigLoader;
use agent9527_config::ThreadConfigSource;
use agent9527_config::compose_requirements;
use agent9527_config::config_error_from_ignored_toml_fields;
use agent9527_config::config_error_from_toml;
use agent9527_config::config_error_from_typed_toml;
...
use agent9527_config::loader::load_requirements_toml;
...
use agent9527_protocol::config_types::EnvironmentVariablePattern;
...
async fn write_project_hook_config(
    dot_agent9527_folder: &Path,
    foo: Option<&str>,
    command: &str,
) -> std::io::Result<()> {
    tokio::fs::create_dir_all(dot_agent9527_folder).await?;
    let foo = foo
        .map(|value| format!("foo = \"{value}\"\n\n"))
        .unwrap_or_default();
    tokio::fs::write(
        dot_agent9527_folder.join(CONFIG_TOML_FILE),
        format!(
            r#"{foo}[hooks]
...
async fn cli_overrides_resolve_relative_paths_against_cwd() -> std::io::Result<()> {
    let agent9527_home = tempdir().expect("tempdir");
...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when AGENT9527_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .agent9527_home(tmp.path().to_path_buf())
    ...
    let config_error = config_error_from_io(&err);
    let _guard = agent9527_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        agent9527_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when AGENT9527_HOME/config.toml does not exist");
...
#[cfg(target_os = "macos")]
#[tokio::test]
async fn managed_preferences_requirements_resolve_paths_against_agent9527_home()
-> anyhow::Result<()> {
    use base64::Engine;

    let tmp = tempdir()?;
    let agent9527_home = tmp.path().join("agent9527-home");
...
"#
    ...
    let expected_log_dir =
        AbsolutePathBuf::resolve_path_against_base("~/.agent9527/logs", &agent9527_home);
    let requirements = layers.requirements_toml();

    assert_eq!(
        requirements.sqlite_home.as_deref(),
        Some(agent9527_home.join("state").as_path())
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(agent9527_home.join("models.json").as_path())
    );
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![agent9527_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(agent9527_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(agent9527_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
...
async fn system_requirements_control_in_app_updates() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let agent9527_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&agent9527_home).await?;
    ...
    assert_eq!(
        ...
        Err(ConstraintError::InvalidValue {
            ...
            requirement_source: RequirementSource::MdmManagedPreferences {
                domain: "com.openai.agent9527".to_string(),
...
"#,
    ...
    assert_eq!(
        ...
        vec![
            ...
            ConfigLayerSource::User {
                file: AbsolutePathBuf::from_absolute_path(agent9527_home.join(CONFIG_TOML_FILE))?,
                profile: None,
            },
        ]
    );
...
#[tokio::test]
async fn load_config_layers_resolves_relative_bundle_requirements_paths_against_agent9527_home()
-> anyhow::Result<()> {
    let tmp = tempdir()?;
    let agent9527_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&agent9527_home).await?;
...
"#;
    ...
    assert_eq!(
        ...
        Some(vec![
            ...
            .expect("bundle requirements path should resolve against agent9527_home")
        ])
    );
...
async fn strict_config_rejects_unknown_cloud_config_key() {
    let tmp = tempdir().expect("tempdir");
    let agent9527_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&agent9527_home)
        .await
        .expect("create agent9527 home");
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            agent9527_config::SandboxModeRequirement::ReadOnly,
            agent9527_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
        ...
        ConfigLoadOptions {
            cloud_config_bundle: CloudConfigBundleLoader::new(async {
                Err(CloudConfigBundleLoadError::new(
                    agent9527_config::CloudConfigBundleLoadErrorCode::RequestFailed,
                    /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".agent9527")).await?;
    tokio::fs::create_dir_all(project_root.join(".agent9527")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".agent9527").join(CONFIG_TOML_FILE),
        r#"foo = "root"
"#,
    )
    .await?;
    tokio::fs::write(
        nested.join(".agent9527").join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            ConfigLayerSource::Project {
                dot_agent9527_folder,
            } => Some(dot_agent9527_folder),
            _ => None,
        })
        .collect();
    ...
    assert_eq!(
        ...
        nested.join(".agent9527").as_path()
    );
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".agent9527").as_path()
    );
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".agent9527")).await?;
    tokio::fs::create_dir_all(worktree_child.join(".agent9527")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    write_project_hook_config(
        &repo_root.join(".agent9527"),
        Some("repo-root"),
        "echo repo root hook",
    )
    .await?;
    write_project_hook_config(
        &repo_child.join(".agent9527"),
        Some("repo-child"),
        "echo repo child hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_root.join(".agent9527"),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(".agent9527"),
        Some("worktree-child"),
...
async fn nested_project_root_markers_do_not_redirect_regular_repo_hooks() -> std::io::Result<()> {
    ...
    .await?;
    write_project_hook_config(
        &project_root.join(".agent9527"),
        /*foo*/ None,
        "echo project root hook",
    )
    .await?;
    write_project_hook_config(
        &nested.join(".agent9527"),
        /*foo*/ None,
        "echo nested hook",
    )
    .await?;
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_agent9527_and_override_in_order()
-> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    ...
    .await?;
    tokio::fs::write(nested.join(".agent9527").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".agent9527").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".agent9527").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let agent9527_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&agent9527_home).await?;
    tokio::fs::write(agent9527_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_agent9527_exists_without_config_toml()
-> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_agent9527_folder: AbsolutePathBuf::from_absolute_path(
                project_root.join(".agent9527"),
            )?,
        },
        TomlValue::Table(toml::map::Map::new()),
...
#[tokio::test]
async fn agent9527_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let agent9527_home = home_dir.join(".agent9527");
    tokio::fs::create_dir_all(&agent9527_home).await?;
    tokio::fs::write(
        agent9527_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
#[tokio::test]
async fn agent9527_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_agent9527 = project_root.join(".agent9527");
    let nested_dot_agent9527 = nested.join(".agent9527");

    tokio::fs::create_dir_all(&nested_dot_agent9527).await?;
    tokio::fs::create_dir_all(project_root.join(".git")).await?;
    tokio::fs::write(
        nested_dot_agent9527.join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;

    tokio::fs::create_dir_all(&project_dot_agent9527).await?;
    make_config_for_test(
        &project_dot_agent9527,
        &project_root,
        TrustLevel::Trusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let user_config_path = project_dot_agent9527.join(CONFIG_TOML_FILE);
...
"#,
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_agent9527_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_agent9527)?,
        },
        child_config,
    );
    assert_eq!(vec![&expected_project_layer], project_layers);
...
"#,
    ...
    let agent9527_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&agent9527_home_untrusted).await?;
    make_config_for_test(
        &agent9527_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = agent9527_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let agent9527_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&agent9527_home_unknown).await?;
    tokio::fs::write(
        agent9527_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
async fn project_layer_ignores_unsupported_config_keys() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(&dot_agent9527).await?;
    ...
    // `.agent9527` folder. The malformed profile value below would fail typed path
    // resolution if `profiles` were not stripped before that pass runs.
    tokio::fs::write(
        dot_agent9527.join(CONFIG_TOML_FILE),
        r#"
model = "project-model"
...
"#,
    ...
    let expected_startup_warnings = vec![format!(
        ...
        dot_agent9527.join(CONFIG_TOML_FILE).display(),
    ...
    // still use the project-local `.agent9527` folder as its relative-path base.
    assert_eq!(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".agent9527")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".agent9527").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let agent9527_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&agent9527_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let agent9527_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&agent9527_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &agent9527_home,
...
mod requirements_exec_policy_tests {
    ...
    use agent9527_config::ConfigRequirementsWithSources;
    use agent9527_config::RequirementSource;
    use agent9527_config::RequirementsExecPolicyDecisionToml;
    use agent9527_config::RequirementsExecPolicyParseError;
    use agent9527_config::RequirementsExecPolicyPatternTokenToml;
    use agent9527_config::RequirementsExecPolicyPrefixRuleToml;
    use agent9527_config::RequirementsExecPolicyToml;
    use agent9527_execpolicy::Decision;
    use agent9527_execpolicy::Evaluation;
    use agent9527_execpolicy::RuleMatch;
    use agent9527_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
    ...
    fn config_stack_for_dot_agent9527_folder_with_requirements(
        dot_agent9527_folder: &Path,
        requirements: ConfigRequirements,
    ) -> ConfigLayerStack {
        let dot_agent9527_folder = AbsolutePathBuf::from_absolute_path(dot_agent9527_folder)
            .expect("absolute dot_agent9527_folder");
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_agent9527_folder_with_requirements(temp_dir.path(), requirements);

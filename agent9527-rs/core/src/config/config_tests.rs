...
use agent9527_config::ConfigLayerEntry;
use agent9527_config::ConfigLayerSource;
use agent9527_config::ConfigLayerStack;
use agent9527_config::McpServerCommandMatcher;
use agent9527_config::McpServerIdentity;
use agent9527_config::McpServerRequirement;
use agent9527_config::McpServerValueMatcher;
use agent9527_config::ProfileV2Name;
use agent9527_config::RequirementSource;
use agent9527_config::Sourced;
use agent9527_config::config_toml::AgentRoleToml;
use agent9527_config::config_toml::AgentsToml;
use agent9527_config::config_toml::AutoReviewToml;
use agent9527_config::config_toml::ConfigToml;
use agent9527_config::config_toml::ExperimentalRequestUserInput;
use agent9527_config::config_toml::ProjectConfig;
use agent9527_config::config_toml::RealtimeConfig;
use agent9527_config::config_toml::RealtimeToml;
use agent9527_config::config_toml::RealtimeTransport;
use agent9527_config::config_toml::RealtimeWsMode;
use agent9527_config::config_toml::RealtimeWsVersion;
use agent9527_config::config_toml::ToolsToml;
...
use agent9527_config::permissions_toml::NetworkDomainPermissionToml;
use agent9527_config::permissions_toml::NetworkDomainPermissionsToml;
use agent9527_config::permissions_toml::NetworkMitmActionToml;
use agent9527_config::permissions_toml::NetworkMitmHookToml;
use agent9527_config::permissions_toml::NetworkMitmToml;
...
use agent9527_config::permissions_toml::WorkspaceRootsToml;
use agent9527_config::types::AppToolApproval;
use agent9527_config::types::ApprovalsReviewer;
use agent9527_config::types::BundledSkillsConfig;
use agent9527_config::types::FeedbackConfigToml;
use agent9527_config::types::HistoryPersistence;
use agent9527_config::types::McpServerEnvVar;
use agent9527_config::types::McpServerOAuthConfig;
use agent9527_config::types::McpServerToolConfig;
use agent9527_config::types::McpServerTransportConfig;
use agent9527_config::types::MemoriesConfig;
use agent9527_config::types::MemoriesToml;
use agent9527_config::types::ModelAvailabilityNuxConfig;
use agent9527_config::types::Notice;
use agent9527_config::types::NotificationCondition;
use agent9527_config::types::NotificationMethod;
use agent9527_config::types::Notifications;
use agent9527_config::types::OtelConfigToml;
use agent9527_config::types::OtelExporterKind;
use agent9527_config::types::ResumeCwdMode;
use agent9527_config::types::SandboxWorkspaceWrite;
use agent9527_config::types::SessionPickerViewMode;
use agent9527_config::types::SkillsConfig;
use agent9527_config::types::ToolSuggestDisabledTool;
use agent9527_config::types::ToolSuggestDiscoverableType;
use agent9527_config::types::Tui;
use agent9527_config::types::TuiKeymap;
use agent9527_config::types::TuiNotificationSettings;
use agent9527_config::types::TuiPetAnchor;
use agent9527_config::types::WindowsSandboxModeToml;
use agent9527_config::types::WindowsToml;
...
use agent9527_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use agent9527_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use agent9527_model_provider_info::WireApi;
use agent9527_models_manager::upstream_bundled_models_response;
use agent9527_network_proxy::NetworkMode;
...
use agent9527_protocol::models::BUILT_IN_PERMISSION_PROFILE_DANGER_FULL_ACCESS;
use agent9527_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY;
use agent9527_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE;
...
use agent9527_protocol::protocol::NetworkAccess;
use agent9527_protocol::protocol::RealtimeVoice;
use agent9527_protocol::protocol::SandboxPolicy;
use agent9527_utils_path_uri::LegacyAppPathString;
...
use agent9527_config::test_support::CloudConfigBundleFixture;
...
fn stdio_mcp_with_args(command: &str, args: &[&str]) -> McpServerConfig {
    McpServerConfig {
        ...
        environment_id: agent9527_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string(),
        enabled: true,
        required: false,
...
async fn load_config_resolves_non_prefixed_mcp_tool_servers() -> std::io::Result<()> {
    ...
    for (config_contents, expected_servers, expected_prefix) in cases {
        ...
        let plugins_manager = PluginsManager::new(agent9527_home.path().to_path_buf());
...
[features.code_mode]
enabled = true
excluded_tool_namespaces = ["mcp__agent9527_apps", "multi_agent_v1"]
...
"#,
    ...
    assert_eq!(
        ...
        agent9527_core_plugins::remote::RemotePluginServiceConfig::new(
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        agent9527_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
-> std::io::Result<()> {
    ...
    let memories_root = agent9527_home.path().join("memories").abs();
    assert!(
        !config
            .permissions
            .file_system_sandbox_policy()
...
-> std::io::Result<()> {
    ...
    let agent9527_home = temp_dir.path().join("agent9527-home");
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".agent9527"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".agent9527".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&root.join(".agent9527"), cwd.as_path()),
            "expected .agent9527 carveout under {root:?}, policy: {policy:?}"
        );
    }
    assert_eq!(
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".agent9527"), cwd.path()),
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".agent9527"] {
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of Agent9527 and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of Agent9527 and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `dev` does not define any recognized filesystem entries for this version of Agent9527."
        )),
        "{:?}",
        config.startup_warnings
    );
                        ...
                        file_system_policy
                            ...
                            .contains(&FileSystemSandboxEntry {
                                ...
                                missing_path_behavior: Some(
                                    agent9527_protocol::permissions::FileSystemSandboxEntryMissingPathBehavior::Skip,
                                ),
                            }),
...
fn filter_plugin_mcp_servers_without_allowlists_does_not_filter_any_plugin() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            agent9527_config::PluginRequirementsToml { mcp_servers: None },
        )]),
        RequirementSource::LegacyManagedConfigTomlFromMdm,
    );
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    let agent9527_home = TempDir::new()?;
    let user_file =
        AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, agent9527_home.path());
    let project_dot_agent9527 =
        AbsolutePathBuf::resolve_path_against_base("project/.agent9527", agent9527_home.path());
    ...
    let requirements_toml = agent9527_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = agent9527_config::ConfigRequirements {
        ...
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_agent9527_folder: project_dot_agent9527.clone(),
                },
                toml::toml! {
    ...
    let refreshed_config = Config::load_config_with_layer_stack(
        ...
        refreshed_toml,
        ConfigOverrides {
            cwd: Some(agent9527_home.path().to_path_buf()),
            ..Default::default()
        },
        agent9527_home.abs(),
    ...
    let thread_layer_stack = ConfigLayerStack::new(
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_agent9527_folder: project_dot_agent9527,
                },
                toml::toml! {
                    [mcp_servers.fresh_project]
...
-> anyhow::Result<()> {
    let agent9527_home = TempDir::new()?;
    let plugin_root = agent9527_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    std::fs::create_dir_all(plugin_root.join(".agent9527-plugin"))?;
    std::fs::write(
        plugin_root.join(".agent9527-plugin/plugin.json"),
        r#"{"name":"sample"}"#,
    )?;
    std::fs::write(
...
"#,
    ...
    assert_eq!(
        ...
        Some((
            agent9527_mcp::McpServerSource::SelectedPlugin(McpPluginAttribution::new(
                "selected-root".to_string(),
                "Selected Plugin".to_string(),
            )),
            selected,
...
async fn default_zsh_path_sets_runtime_zsh_path() -> std::io::Result<()> {
    let agent9527_home = TempDir::new()?;
    let default_zsh_path = agent9527_home.path().join("packaged-zsh");
...
#[tokio::test]
async fn sqlite_home_defaults_to_agent9527_home_for_workspace_write() -> std::io::Result<()> {
    let agent9527_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite.home(), agent9527_home.path());
...
-> std::io::Result<()> {
    let agent9527_home = TempDir::new()?;
    let memories_root = agent9527_home.path().join("memories");
    let writable_root = agent9527_home.path().join("writable").abs();
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
"#,
    ...
    let project_config_dir = workspace.path().join(".agent9527");
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let agent9527_home = TempDir::new()?;
    let managed_path = agent9527_home.path().join("managed_config.toml");
    let config_path = agent9527_home.path().join(CONFIG_TOML_FILE);
    ...
    let cwd = agent9527_home.path().abs();
    let config_layer_stack = load_config_layers_state(
        ...
        &Vec::new(),
        overrides,
        &agent9527_config::NoopThreadConfigLoader,
    )
    .await?;
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(agent9527_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    let agent9527_home = TempDir::new()?;
    let expected_cwd = LegacyAppPathString::from_path(agent9527_home.path());
    ...
    let loaded = load_global_mcp_servers(agent9527_home.path()).await?;
...
"#,
    ...
    assert_eq!(
        ...
        Some(&serde_json::json!("agent9527"))
    );
    assert_eq!(
...
"#,
    ...
    let err = load_global_mcp_servers(agent9527_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/agent9527-mcp");
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/agent9527-mcp""#),
...
async fn replace_mcp_servers_streamable_http_serializes_oauth_resource() -> anyhow::Result<()> {
    ...
    let servers = BTreeMap::from([(
        ...
        McpServerConfig {
            ...
            oauth: Some(McpServerOAuthConfig {
                client_id: Some("eci-prd-pub-agent9527-123".to_string()),
            }),
    ...
    assert!(serialized.contains(r#"client_id = "eci-prd-pub-agent9527-123""#));
    ...
    assert_eq!(docs.oauth_client_id(), Some("eci-prd-pub-agent9527-123"));
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(agent9527_home.path())
        .set_model(Some("gpt-5.4"), Some(ReasoningEffort::High))
        .apply()
        .await?;

    let serialized =
        tokio::fs::read_to_string(agent9527_home.path().join(CONFIG_TOML_FILE)).await?;
...
async fn for_config_writes_selected_user_config_file() -> anyhow::Result<()> {
    let agent9527_home = TempDir::new()?;
    let base_config = agent9527_home.path().join(CONFIG_TOML_FILE);
    let selected_config = agent9527_home.path().join("work.config.toml");
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(agent9527_home.path(), &profile_name),
        agent9527_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
impl PrecedenceTestFixture {
    ...
    fn agent9527_home(&self) -> AbsolutePathBuf {
        self.agent9527_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let agent9527_home = TempDir::new()?;
    let workspace = agent9527_home.path().join("workspace");
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, agent9527_home.abs()).await?;
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        agent9527_config::ConfigRequirementsToml {
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let agent9527_home = TempDir::new()?;
    let missing_path = agent9527_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let agent9527_home = TempDir::new()?;
    let role_config_path = agent9527_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config_layer_stack = agent9527_config::ConfigLayerStack::new(
        vec![agent9527_config::ConfigLayerEntry::new(
            ConfigLayerSource::User {
                file: agent9527_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
        Default::default(),
        agent9527_config::ConfigRequirementsToml::default(),
    )
    .map_err(std::io::Error::other)?;
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".agent9527").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let agent9527_home = TempDir::new()?;
    let researcher_path = agent9527_home.path().join("agents").join("researcher.toml");
    let reviewer_path = agent9527_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = agent9527_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let agent9527_home = TempDir::new()?;
    let catalog_path = agent9527_home.path().join("catalog.json");
    let mut catalog = upstream_bundled_models_response()
...
"#;
    ...
    let agent9527_home_temp_dir = TempDir::new().unwrap();

    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        agent9527_home: agent9527_home_temp_dir,
        cfg,
    })
}
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: agent9527_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn config_toml_priority_service_tier_uses_priority_request_value() -> std::io::Result<()> {
    ...
    let agent9527_home = fixture.agent9527_home();
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = agent9527_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![agent9527_config::WebSearchModeRequirement::Cached]),
    ...
    let requirement_source = agent9527_config::RequirementSource::Unknown;
    ...
    let requirements = agent9527_config::ConfigRequirements {
        web_search_mode: agent9527_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        agent9527_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
projects = { "/Users/mbolin/code/agent95274" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/agent95273" = { trust_level = "trusted" } }
model = "foo""#;
    let mut doc = initial.parse::<DocumentMut>()?;

    // Approve a new directory
    let new_project = Path::new("/Users/mbolin/code/agent95272");
...
[projects."/Users/mbolin/code/agent95274"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/agent95273"]
trust_level = "trusted"
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(agent9527_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(agent9527_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
...
"#,
    ...
    assert_eq!(
        ...
        Some(agent9527_config::types::WindowsSandboxModeToml::Elevated)
    );
    assert!(
...
"#,
    ...
    assert_eq!(
        err.to_string(),
        "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; Agent9527 would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode."
    );
    Ok(())
}
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let agent9527_home = TempDir::new()?;
    let cwd = agent9527_home.path().join("workspace");
    let outside_root = agent9527_home.path().join("outside-write");
...
export_dir = "locks"
allow_agent9527_version_mismatch = true
...
"#,
    ...
    assert!(config.config_lock_allow_agent9527_version_mismatch);
...
async fn debug_config_lockfile_load_path_loads_lock_from_nested_table() -> std::io::Result<()> {
    let agent9527_home = TempDir::new()?;
    let lock_path = agent9527_home.path().join("session.config.lock.toml");
    std::fs::write(
        &lock_path,
        format!(
            r#"version = {}
agent9527_version = "older-version"
...
-> std::io::Result<()> {
    let agent9527_home = TempDir::new()?;
    let selected_config = agent9527_home.path().join("default.config.toml");
    std::fs::write(
        &selected_config,
        r#"approvals_reviewer = "user"
"#,
...
async fn exact_requirements_apply_to_runtime_config() -> std::io::Result<()> {
    let agent9527_home = TempDir::new()?;
    let catalog_path = agent9527_home.path().join("required-models.json");
    let mut catalog = upstream_bundled_models_response()
        .unwrap_or_else(|err| panic!("bundled models.json should parse: {err}"));
    catalog.models = catalog.models.into_iter().take(1).collect();
    std::fs::write(
...
"#,
    ...
    let required_sqlite_home = agent9527_home.path().join("required-state");
    let required_log_dir = agent9527_home.path().join("required-logs");
...
"#,
    ...
    let config = load_with_enterprise_requirement(&agent9527_home, requirements).await?;
...
async fn absent_allow_login_shell_does_not_report_an_override() -> std::io::Result<()> {
    let agent9527_home = TempDir::new()?;
    let config =
        load_with_enterprise_requirement(&agent9527_home, "allow_login_shell = false").await?;
...
fn sqlite_home_env_conflict_reports_an_override() -> std::io::Result<()> {
    let agent9527_home = TempDir::new()?;
    let required = AbsolutePathBuf::try_from(agent9527_home.path().join("required-state"))?;
    let environment = agent9527_home.path().join("environment-state");
    ...
    assert_eq!(
        warnings,
        vec![format!(
            "Environment value for `$AGENT9527_SQLITE_HOME` is overridden by the required `sqlite_home` value {required:?} from {}.",
            RequirementSource::Unknown
        )]
    );

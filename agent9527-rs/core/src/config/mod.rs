...
use agent9527_config::ConfigRequirements;
use agent9527_config::ConfigRequirementsToml;
use agent9527_config::ConstrainedWithSource;
use agent9527_config::FeatureRequirementsToml;
use agent9527_config::McpServerRequirement;
use agent9527_config::PluginRequirementsToml;
use agent9527_config::ProfileV2Name;
use agent9527_config::ResidencyRequirement;
use agent9527_config::SandboxModeRequirement;
use agent9527_config::Sourced;
use agent9527_config::ThreadConfigLoader;
use agent9527_config::config_toml::ConfigLockfileToml;
use agent9527_config::config_toml::ConfigToml;
use agent9527_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
use agent9527_config::config_toml::ProjectConfig;
use agent9527_config::config_toml::RealtimeAudioConfig;
use agent9527_config::config_toml::RealtimeConfig;
use agent9527_config::config_toml::ThreadStoreToml;
use agent9527_config::config_toml::validate_model_providers;
use agent9527_config::loader::load_config_layers_state;
use agent9527_config::loader::project_trust_key;
use agent9527_config::permissions_toml::PermissionsToml;
use agent9527_config::sandbox_mode_requirement_for_permission_profile;
...
use agent9527_config::types::History;
use agent9527_config::types::McpServerConfig;
use agent9527_config::types::McpServerDisabledReason;
...
use agent9527_config::types::ToolSuggestConfig;
use agent9527_config::types::ToolSuggestDisabledTool;
use agent9527_config::types::ToolSuggestDiscoverable;
...
use agent9527_config::types::UriBasedFileOpener;
use agent9527_config::types::WindowsSandboxModeToml;
use agent9527_core_plugins::PluginLoadOutcome;
use agent9527_core_plugins::PluginsConfigInput;
use agent9527_exec_server::ExecutorFileSystem;
use agent9527_exec_server::LOCAL_FS;
use agent9527_features::CodeModeConfigToml;
use agent9527_features::CurrentTimeReminderConfigToml;
use agent9527_features::CurrentTimeReminderDeliveryMode;
use agent9527_features::CurrentTimeSource;
use agent9527_features::Feature;
use agent9527_features::FeatureConfigSource;
use agent9527_features::FeatureOverrides;
use agent9527_features::FeatureToml;
use agent9527_features::Features;
use agent9527_features::FeaturesToml;
use agent9527_features::MultiAgentV2ConfigToml;
use agent9527_features::NetworkProxyConfigToml;
use agent9527_features::TokenBudgetConfigToml;
...
use agent9527_install_context::InstallContext;
use agent9527_login::AuthManagerConfig;
use agent9527_login::AuthRouteConfig;
use agent9527_mcp::McpConfig;
use agent9527_mcp::McpPluginAttribution;
use agent9527_mcp::McpServerRegistration;
use agent9527_mcp::ResolvedMcpCatalog;
use agent9527_memories_read::memory_root;
use agent9527_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use agent9527_model_provider_info::ModelProviderInfo;
use agent9527_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use agent9527_model_provider_info::built_in_model_providers;
use agent9527_model_provider_info::merge_configured_model_providers;
use agent9527_models_manager::ModelsManagerConfig;
use agent9527_protocol::config_types::AltScreenMode;
use agent9527_protocol::config_types::AutoCompactTokenLimitScope;
...
use agent9527_protocol::config_types::ShellEnvironmentPolicy;
use agent9527_protocol::config_types::TrustLevel;
use agent9527_protocol::config_types::Verbosity;
use agent9527_protocol::config_types::WebSearchConfig;
use agent9527_protocol::config_types::WebSearchMode;
...
pub use agent9527_thread_store::ExtraConfig;
use agent9527_utils_absolute_path::AbsolutePathBuf;
use agent9527_utils_absolute_path::AbsolutePathBufGuard;
...
use crate::config::permissions::get_readable_roots_required_for_agent9527_runtime;
...
#[cfg(test)]
mod schema;
pub use agent9527_config::ConfigLoadOptions;
pub use agent9527_config::Constrained;
pub use agent9527_config::ConstraintError;
pub use agent9527_config::ConstraintResult;
pub use agent9527_config::LoaderOverrides;
pub use agent9527_network_proxy::NetworkProxyAuditMetadata;
use agent9527_sandboxing::compatibility_sandbox_policy_for_permission_profile;
pub use agent9527_sandboxing::system_bwrap_warning;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<AbsolutePathBuf> {
    let raw = std::env::var(agent9527_state::SQLITE_HOME_ENV).ok()?;
...
pub(crate) async fn test_config() -> Config {
    ...
    Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(agent9527_home.path())
...
impl Permissions {
    ...
    /// selection. Internal Agent9527-only writable roots are intentionally excluded.
...
// when Agent9527 is still responsible for the network policy. `Disabled` means no
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<agent9527_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, Agent9527 will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Agent9527
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.agent9527/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Agent9527"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Agent9527 '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Agent9527 home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Agent9527 can reach out to for tool calls.
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Agent9527 unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/agent9527/blob/main/agent9527-rs/rmcp-client/src/oauth.rs#L2
    /// file: AGENT9527_HOME/.credentials.json
    ///       This file will be readable to Agent9527 and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, Agent9527 will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all Agent9527 state (defaults to `~/.agent9527` but can be
    /// overridden by the `AGENT9527_HOME` environment variable).
    pub agent9527_home: AbsolutePathBuf,

    /// Resolved configuration shared by all Agent9527 SQLite databases.
    pub sqlite: agent9527_state::SqliteConfig,

    /// Directory where Agent9527 writes log files (defaults to `$AGENT9527_HOME/log`).
    pub log_dir: PathBuf,

    /// Directory where Agent9527 writes effective session config lock files.
    pub config_lock_export_dir: Option<AbsolutePathBuf>,

    /// Whether config lock replay ignores Agent9527 version drift between the
    /// lock metadata and the regenerated lock.
    pub config_lock_allow_agent9527_version_mismatch: bool,
    ...
    /// Settings that govern if and what will be written to `~/.agent9527/history.jsonl`.
    pub history: History,
    ...
    /// Path to the current Agent9527 executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub agent9527_self_exe: Option<PathBuf>,

    /// Path to the `agent9527-linux-sandbox` executable. This must be set if
    /// [`agent9527_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `agent9527-linux-sandbox`.
    pub agent9527_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `agent9527-execve-wrapper` executable used for shell
    ...
    /// Whether Agent9527-owned clients should respect host system proxy settings.
    ...
    /// When `true`, checks for Agent9527 updates on startup and surfaces update prompts.
    /// Set to `false` only if your Agent9527 updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across Agent9527 product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across Agent9527 product surfaces.
    /// Defaults to `true`.
    ...
    pub otel: agent9527_config::types::OtelConfig,
}
...
impl AuthManagerConfig for Config {
    fn agent9527_home(&self) -> PathBuf {
        self.agent9527_home.to_path_buf()
    }
...
impl ConfigBuilder {
    pub fn agent9527_home(mut self, agent9527_home: PathBuf) -> Self {
        self.agent9527_home = Some(agent9527_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let agent9527_home = match agent9527_home {
            Some(agent9527_home) => AbsolutePathBuf::from_absolute_path(agent9527_home)?,
            None => find_agent9527_home()?,
        };
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&agent9527_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) =
                    agent9527_config::first_layer_config_error::<ConfigToml>(
                        &config_layer_stack,
                        agent9527_config::CONFIG_TOML_FILE,
                    )
                    .await
                {
                    return Err(agent9527_config::io_error_from_config_error(
        ...
        {
            let allow_agent9527_version_mismatch = config_lock_settings
                .and_then(|config_lock| config_lock.allow_agent9527_version_mismatch)
                .unwrap_or(false);
            ...
            config.config_lock_allow_agent9527_version_mismatch = allow_agent9527_version_mismatch;
...
impl Config {
    pub fn sqlite_config(&self) -> &agent9527_state::SqliteConfig {
        &self.sqlite
    }
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &agent9527_core_plugins::PluginsManager,
    ) -> McpConfig {
    ...
    pub(crate) fn to_mcp_config_with_loaded_plugins(
        ...
        McpConfig {
            ...
            agent9527_home: self.agent9527_home.to_path_buf(),
            ...
            agent9527_linux_sandbox_exe: self.agent9527_linux_sandbox_exe.clone(),
            ...
            connector_snapshot:
                agent9527_connectors::ConnectorSnapshot::from_plugin_capability_summaries(
    ...
    pub async fn rebuild_preserving_session_layers(
        ...
        Self::load_config_with_layer_stack(
            ...
            refreshed_config.agent9527_home.clone(),
            config_layer_stack,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        Self::load_default_with_cli_overrides_for_agent9527_home(
            agent9527_home.to_path_buf(),
            cli_overrides,
        )
        .await
    }

    /// Load a default configuration for a specific Agent9527 home without reading
    /// user, project, or system config layers.
    pub async fn load_default_with_cli_overrides_for_agent9527_home(
        ...
        let cli_layer = agent9527_config::build_cli_overrides_layer(&cli_overrides);
        agent9527_config::merge_toml_values(&mut merged, &cli_layer);
        let agent9527_home = AbsolutePathBuf::from_absolute_path_checked(agent9527_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &agent9527_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
            ConfigOverrides::default(),
    ...
    /// ignores user settings. For example, the `agent9527 exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `agent9527_self_exe`, `agent9527_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    load_config_toml_with_layer_stack(agent9527_home, cwd, cli_overrides, options)
        .await
...
pub async fn load_config_toml_with_layer_stack(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, agent9527_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .agent9527/ folders.
...
/// Patch `AGENT9527_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(agent9527_home: &Path, provider: &str) -> std::io::Result<()> {
    agent9527_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &agent9527_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            agent9527_protocol::permissions::FileSystemSandboxEntry {
                path: agent9527_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: agent9527_protocol::permissions::FileSystemAccessMode::Deny,
            ...
            agent9527_protocol::permissions::FileSystemSandboxEntry {
                path: agent9527_protocol::permissions::FileSystemPath::Path { path },
                access: agent9527_protocol::permissions::FileSystemAccessMode::Deny,
                missing_path_behavior: None,
            }
        };
...
fn resolve_orchestrator_feature_enabled(
    feature: Option<&agent9527_config::config_toml::OrchestratorFeatureToml>,
) -> bool {
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let memories_root = memory_root(&agent9527_home);
        ...
        let model_provider = model_providers
            ...
            .clone();
        product_policy::validate_model_provider(model_provider.requires_openai_auth)
            .map_err(|message| std::io::Error::new(std::io::ErrorKind::InvalidInput, message))?;
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(agent9527_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| agent9527_home.join("log").to_path_buf());
        ...
        let sqlite_home = cfg
            ...
            .cloned()
            .or(sqlite_home_env)
            .unwrap_or_else(|| agent9527_home.clone());
        ...
        let mut helper_readable_roots = get_readable_roots_required_for_agent9527_runtime(
            &agent9527_home,
            zsh_path.as_ref(),
        ...
        let config = Self {
            ...
            agent9527_home,
            sqlite: agent9527_state::SqliteConfig::from_sqlite_home(sqlite_home),
            log_dir,
            ...
            config_lock_allow_agent9527_version_mismatch: cfg
                .debug
                .as_ref()
                .and_then(|debug| debug.config_lockfile.as_ref())
                .and_then(|config_lock| config_lock.allow_agent9527_version_mismatch)
...
/// Returns the path to the Agent9527 configuration directory, which can be
/// specified by the `AGENT9527_HOME` environment variable. If not set, defaults to
/// `~/.agent9527`.
///
/// - If `AGENT9527_HOME` is set, the value must exist and be a directory. The
///   value will be canonicalized and this function will Err otherwise.
/// - If `AGENT9527_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_agent9527_home() -> std::io::Result<AbsolutePathBuf> {
    agent9527_utils_home_dir::find_agent9527_home()
}

/// Returns the path to the folder where Agent9527 logs are stored. Does not verify

...
use agent9527_config::permissions_toml::NetworkUnixSocketPermissionToml;
use agent9527_config::permissions_toml::NetworkUnixSocketPermissionsToml;
...
use agent9527_features::NetworkProxyDomainPermissionToml;
use agent9527_features::NetworkProxyModeToml;
use agent9527_features::NetworkProxyUnixSocketPermissionToml;
use agent9527_network_proxy::NetworkMode;
use agent9527_network_proxy::NetworkProxyConfig;
#[cfg(test)]
use agent9527_network_proxy::NetworkUnixSocketPermission as ProxyNetworkUnixSocketPermission;
...
use agent9527_protocol::permissions::project_roots_glob_pattern;
...
/// for Agent9527 to function. These should always be added to the
/// `FileSystemSandboxPolicy` for a thread.
pub(crate) fn get_readable_roots_required_for_agent9527_runtime(
    agent9527_home: &Path,
    zsh_path: Option<&PathBuf>,
    main_execve_wrapper_exe: Option<&PathBuf>,
) -> Vec<AbsolutePathBuf> {
    let arg0_root =
        AbsolutePathBuf::from_absolute_path(agent9527_home.join("tmp").join("arg0")).ok();
...
// Adding a new `:special_path` must not make older Agent9527 versions reject the
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of Agent9527. Filesystem access will remain restricted. Upgrade Agent9527 if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{subpath}` is not recognized by this version of Agent9527 and will be ignored. Upgrade Agent9527 if this path is required."
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of Agent9527 and will be ignored. Upgrade Agent9527 if this path is required."
            ),
        },
    );
}

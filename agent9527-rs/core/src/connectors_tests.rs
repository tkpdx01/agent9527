...
use agent9527_config::AppRequirementToml;
use agent9527_config::AppsRequirementsToml;
...
use agent9527_connectors::merge::plugin_connector_to_app_info;
use agent9527_connectors::metadata::connector_install_url;
use agent9527_connectors::metadata::sanitize_name;
...
fn agent9527_app_tool(
    ...
    let tool_namespace = connector_name
        .map(sanitize_name)
        .map(|connector_name| format!("mcp__{AGENT9527_APPS_MCP_SERVER_NAME}__{connector_name}"))
        .unwrap_or_else(|| AGENT9527_APPS_MCP_SERVER_NAME.to_string());
...
fn synthetic_links_are_exposed_to_the_agent_but_not_accessible_in_app_list() {
    let mut synthetic_tool =
        agent9527_app_tool("gmail_batch_read_email", "gmail", Some("Gmail"), &[]);
    ...
    let tools = vec![
        synthetic_tool,
        agent9527_app_tool("calendar_list_events", "calendar", Some("Calendar"), &[]),
    ];
...
fn accessible_connectors_from_mcp_tools_preserves_description() {
    let mcp_tools = vec![ToolInfo {
        ...
        callable_namespace: "mcp__agent9527_apps__calendar".to_string(),
...
"#
        ...
        assert_eq!(
            mcp_approvals_reviewer(&config, AGENT9527_APPS_MCP_SERVER_NAME, Some("calendar")),
            expected_app
        );
        assert_eq!(
            mcp_approvals_reviewer(&config, AGENT9527_APPS_MCP_SERVER_NAME, Some("drive")),
            expected_default
        );
        assert_eq!(
...
"#,
    ...
    let plugins_manager = PluginsManager::new(config.agent9527_home.to_path_buf());

...
use agent9527_mcp::PreparedMcpCall;
...
async fn handle_approved_mcp_tool_call(
    ...
    .await;
    maybe_track_agent9527_app_used(sess, turn_context, &server, &metadata).await;
...
async fn maybe_request_agent9527_apps_auth_elicitation(
    ...
    refresh_agent9527_apps_after_connector_auth(sess, turn_context).await;
...
async fn refresh_agent9527_apps_after_connector_auth(
    ...
    let mcp_tools_result = sess.hard_refresh_latest_agent9527_apps_tools().await;
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let sandbox_state = serde_json::to_value(SandboxState {
        permission_profile,
        agent9527_linux_sandbox_exe: prepared_call.config().agent9527_linux_sandbox_exe.clone(),
        sandbox_cwd,
...
async fn maybe_request_mcp_tool_approval(
    ...
    hook_tool_name: &HookToolName,
    metadata: &McpToolApprovalMetadata,
    config: &agent9527_mcp::McpConfig,
    policy: McpToolApprovalPolicy,
...
fn mcp_tool_metadata(prepared_call: &PreparedMcpCall) -> McpToolApprovalMetadata {
    ...
    let connector_description = (server == AGENT9527_APPS_MCP_SERVER_NAME)

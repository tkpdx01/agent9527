...
use agent9527_protocol::capabilities::CapabilityRootLocation;
...
impl agent9527_agent_graph_store::AgentGraphStore for FakeAgentGraphStore {
    fn upsert_thread_spawn_edge(
        &self,
        _parent_thread_id: ThreadId,
        _child_thread_id: ThreadId,
        _status: agent9527_agent_graph_store::ThreadSpawnEdgeStatus,
    ) -> agent9527_agent_graph_store::AgentGraphStoreFuture<'_, ()> {
    ...
    fn list_thread_spawn_children(
        &self,
        _parent_thread_id: ThreadId,
        _status_filter: Option<agent9527_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> agent9527_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
    ...
    fn list_thread_spawn_descendants(
        &self,
        root_thread_id: ThreadId,
        status_filter: Option<agent9527_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> agent9527_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
...
fn effective_originator_prefers_thread_scoped_sources_before_env_originator() {
    for (metrics_service_name, persisted_originator, inherited_originator, expected_originator) in [
        (
            Some("agent9527_work_desktop"),
            Some("persisted_originator"),
            Some("inherited_originator"),
            "agent9527_work_desktop",
        ),
        (
            Some("agent9527_work_web"),
            Some("persisted_originator"),
            Some("inherited_originator"),
            "agent9527_work_web",
        ),
        (
            Some("agent9527_work_mobile"),
            Some("persisted_originator"),
            Some("inherited_originator"),
            "agent9527_work_mobile",
        ),
        (
            Some("agent9527_work_cca"),
            Some("persisted_originator"),
            Some("inherited_originator"),
            "agent9527_work_cca",
        ),
...
async fn shutdown_all_threads_bounded_submits_shutdown_to_every_thread() {
    ...
    config.agent9527_home = temp_dir.path().join("agent9527-home").abs();
    config.cwd = config.agent9527_home.abs();
    std::fs::create_dir_all(&config.agent9527_home).expect("create agent9527 home");
...
async fn mcp_invalidation_refreshes_threads_that_are_still_starting() {
    ...
    impl agent9527_extension_api::ThreadLifecycleContributor<Config> for BlockingThreadStartup {
        fn on_thread_start<'a>(
            &'a self,
            _input: agent9527_extension_api::ThreadStartInput<'a, Config>,
    ...
    impl agent9527_extension_api::McpServerContributor<Config> for BlockingThreadStartup {
        ...
        fn contribute<'a>(
            &'a self,
            _context: agent9527_extension_api::McpServerContributionContext<'a, Config>,
...
async fn start_thread_seeds_extension_data_for_mcp_and_lifecycle_contributors() {
    ...
    impl agent9527_extension_api::ThreadLifecycleContributor<Config> for InitialDataRecorder {
        fn on_thread_start<'a>(
            &'a self,
            input: agent9527_extension_api::ThreadStartInput<'a, Config>,
        ) -> agent9527_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
    ...
    impl agent9527_extension_api::McpServerContributor<Config> for InitialDataRecorder {
        fn id(&self) -> &'static str {
            "selected_root_test"
        }
        ...
        > {
            Box::pin(async move {
                ...
                let mut server = agent9527_mcp::agent9527_apps_mcp_server_config(
                ...
                vec![
                    agent9527_extension_api::McpServerContribution::SelectedPlugin {
    ...
    let selected_root_init = |id: &str, environment_id: &str| {
        let mut init = agent9527_extension_api::ExtensionDataInit::new();
    ...
    let first_thread = manager
        .start_thread(StartThreadOptions {
            metrics_service_name: Some("agent9527_work_desktop".to_string()),
            environments: Some(Vec::new()),
    ...
    let selected_servers = |config: &agent9527_mcp::McpConfig| {
        agent9527_mcp::configured_mcp_servers(config)
            .into_iter()
    ...
    let agent9527_apps_server = agent9527_mcp::configured_mcp_servers(&first_resolved.config)
        .remove(agent9527_mcp::AGENT9527_APPS_MCP_SERVER_NAME)
        .expect("Agent9527 Apps server should be configured");
    let agent9527_apps_headers = match agent9527_apps_server.transport {
        agent9527_config::McpServerTransportConfig::StreamableHttp { http_headers, .. } => {
            http_headers
        }
        agent9527_config::McpServerTransportConfig::Stdio { .. } => {
            panic!("Agent9527 Apps server should use streamable HTTP")
        }
    };
    assert_eq!(
        agent9527_apps_headers
            .expect("Agent9527 Apps headers should be configured")
            .get("originator"),
...
#[tokio::test]
async fn explicit_installation_id_skips_agent9527_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.agent9527_home = temp_dir.path().join("agent9527-home").abs();
    config.cwd = config.agent9527_home.abs();
...
async fn injected_models_manager_controls_refresh_policy() {
    ...
    assert!(!config.agent9527_home.join("models_cache.json").exists());
}

...
use crate::agent9527_thread::BackgroundTerminalInfo;
...
const NETWORK_ACCESS_DENIED_MESSAGE: &str =
    "Network access was denied by the Agent9527 sandbox network proxy.";
...
) -> agent9527_exec_server::ExecEnvPolicy {
    let mut exclude = policy
        .exclude
        .iter()
        .map(std::string::ToString::to_string)
        .collect::<Vec<_>>();
    exclude.push(AGENT9527_PERMISSION_PROFILE_ENV_VAR.to_string());
    let mut r#set = policy.r#set.clone();
    r#set.retain(|key, _| !key.eq_ignore_ascii_case(AGENT9527_PERMISSION_PROFILE_ENV_VAR));
    agent9527_exec_server::ExecEnvPolicy {
...
fn env_overlay_for_exec_server(
    ...
    request_env
        .iter()
        .filter(|(key, value)| {
            key.as_str() == AGENT9527_PERMISSION_PROFILE_ENV_VAR
...
fn exec_server_env_for_request(
    ...
    Option<agent9527_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
fn exec_server_params_for_request(
    ...
    request: &ExecRequest,
    tty: bool,
) -> agent9527_exec_server::ExecParams {
...
fn network_approval_error_message(err: ToolError) -> String {
    match err {
        ...
        ToolError::Agent9527(err) => err.to_string(),
    }
}
...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        network_proxy_launch: Option<agent9527_network_proxy::RemoteNetworkProxyLaunchConfig>,
        ...
        .map_err(|err| match err {
            UnifiedExecError::SandboxDenied { output, .. } => {
                ToolError::Agent9527(Agent9527Err::Sandbox(SandboxErr::Denied {
    ...
    pub(crate) async fn open_session_with_prepared_exec_env(
        ...
        if request.sandbox == agent9527_sandboxing::SandboxType::WindowsRestrictedToken {
            ...
            let agent9527_home = crate::config::find_agent9527_home().map_err(|err| {
            ...
            let spawned = match request.windows_sandbox_level {
                agent9527_protocol::config_types::WindowsSandboxLevel::Elevated => {
                    agent9527_windows_sandbox::spawn_windows_sandbox_session_elevated_for_permission_profile(
                ...
                agent9527_protocol::config_types::WindowsSandboxLevel::RestrictedToken
                | agent9527_protocol::config_types::WindowsSandboxLevel::Disabled => {
                    agent9527_windows_sandbox::spawn_windows_sandbox_session_legacy(
                        &request.permission_profile,
                        request.windows_sandbox_workspace_roots.as_slice(),
                        agent9527_home.as_ref(),
                        request.command.clone(),
        ...
        let spawn_result = if tty {
            agent9527_utils_pty::pty::spawn_process_with_inherited_fds(
                program,
                args,
                native_cwd.as_path(),
                &request.env,
                &request.arg0,
                agent9527_utils_pty::TerminalSize::default(),
                &inherited_fds,
            )
            .await
        } else {
            agent9527_utils_pty::pipe::spawn_process_no_stdin_with_inherited_fds(
                program,
                args,
                native_cwd.as_path(),
                &request.env,
    ...
    pub(super) async fn open_session_with_sandbox(
        ...
        env.insert(
            AGENT9527_THREAD_ID_ENV_VAR.to_string(),

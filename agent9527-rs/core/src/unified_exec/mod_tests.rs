...
use agent9527_exec_server::ExecProcess;
use agent9527_exec_server::ExecProcessEventReceiver;
use agent9527_exec_server::ExecProcessFuture;
use agent9527_exec_server::ProcessId;
use agent9527_exec_server::ProcessSignal;
use agent9527_exec_server::ReadResponse;
use agent9527_exec_server::StartedExecProcess;
use agent9527_exec_server::WriteResponse;
use agent9527_exec_server::WriteStatus;
...
use core_test_support::test_agent9527::test_env as remote_test_env;
...
impl BlockingTerminateExecProcess {
    async fn read(&self) -> Result<ReadResponse, agent9527_exec_server::ExecServerError> {
        Ok(ReadResponse {
            chunks: Vec::new(),
            next_seq: 1,
            exited: false,
    ...
    async fn write(&self) -> Result<WriteResponse, agent9527_exec_server::ExecServerError> {
        Ok(WriteResponse {
            status: WriteStatus::Accepted,
        })
    }

    async fn terminate(&self) -> Result<(), agent9527_exec_server::ExecServerError> {
...
async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        "export AGENT9527_INTERACTIVE_SHELL_VAR=agent9527\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "echo $AGENT9527_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    assert!(
...
async fn unified_exec_timeouts() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        format!("export AGENT9527_INTERACTIVE_SHELL_VAR={TEST_VAR_VALUE}\n").as_str(),
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "sleep 5 && echo $AGENT9527_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 10,
    )
    .await?;
    assert!(
...
async fn completed_pipe_commands_preserve_exit_code() -> anyhow::Result<()> {
    ...
    let environment = agent9527_exec_server::Environment::default_for_tests();

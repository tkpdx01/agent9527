...
fn unified_exec_env_injects_defaults() {
    ...
    let expected = HashMap::from([
        ...
        ("AGENT9527_CI".to_string(), "1".to_string()),
    ]);
...
fn env_overlay_for_exec_server_keeps_runtime_changes_only() {
    ...
    let request_env = HashMap::from([
        ...
        ("SHELL_SET".to_string(), "policy".to_string()),
        ("AGENT9527_THREAD_ID".to_string(), "thread-1".to_string()),
        (
            AGENT9527_PERMISSION_PROFILE_ENV_VAR.to_string(),
            "current-profile".to_string(),
        ),
        (
            "AGENT9527_SANDBOX_NETWORK_DISABLED".to_string(),
            "1".to_string(),
        ),
    ]);
...
fn exec_env_policy_excludes_runtime_permission_profile() {
    let policy = ShellEnvironmentPolicy {
        r#set: HashMap::from([
            (
                "agent9527_permission_profile".to_string(),
    ...
    assert_eq!(
        ...
        agent9527_exec_server::ExecEnvPolicy {
            ...
            exclude: vec![AGENT9527_PERMISSION_PROFILE_ENV_VAR.to_string()],
...
fn exec_server_params_use_path_uri_and_env_policy_overlay_contract() {
    let cwd: agent9527_utils_absolute_path::AbsolutePathBuf = std::env::current_dir()
        .expect("current dir")
        .try_into()
        .expect("absolute path");
    let file_system_sandbox_policy =
        agent9527_protocol::permissions::FileSystemSandboxPolicy::unrestricted();
    let network_sandbox_policy = agent9527_protocol::permissions::NetworkSandboxPolicy::Restricted;
    let permission_profile = agent9527_protocol::models::PermissionProfile::Disabled;
    ...
    let mut request = ExecRequest {
        ...
        env: HashMap::from([
            ...
            (
                "AGENT9527_NETWORK_PROXY_ACTIVE".to_string(),
                "1".to_string(),
            ),
            (
        ...
        exec_server_env_config: Some(ExecServerEnvConfig {
            policy: agent9527_exec_server::ExecEnvPolicy {
                inherit: agent9527_protocol::config_types::ShellEnvironmentPolicyInherit::Core,
        ...
        sandbox: agent9527_sandboxing::SandboxType::None,
    ...
    let proxy_settings_mode = agent9527_sandboxing::WindowsSandboxProxySettingsMode::Preserve;
    ...
    request.exec_server_sandbox = Some(
        agent9527_exec_server::FileSystemSandboxContext::from_permission_profile(
            permission_profile,
        ),
    );
    ...
    assert_eq!(
        ...
        Some(agent9527_sandboxing::WindowsSandboxProxySettingsMode::Preserve)
    );
...
async fn failed_initial_end_for_unstored_process_uses_fallback_output() {
    ...
    let request = ExecCommandRequest {
        ...
        shell_mode: agent9527_tools::UnifiedExecShellMode::Direct,
        network: None,
        tty: true,
    ...
    let agent9527_protocol::protocol::EventMsg::ItemCompleted(completed_event) = event.msg else {
        panic!("expected ItemCompleted event");
    };
    let agent9527_protocol::items::TurnItem::CommandExecution(item) = completed_event.item else {
        panic!("expected CommandExecution item");
    };
    assert_eq!(item.id, "call-unified-denied");
    assert_eq!(
        item.status,
        agent9527_protocol::items::CommandExecutionStatus::Failed
    );
...
async fn pruning_does_not_evict_live_process_while_exited_process_is_finalizing() {
    let exited_process = Arc::new(
        crate::unified_exec::process_tests::remote_process(
            agent9527_exec_server::WriteStatus::Accepted,

...
use agent9527_config::types::AppConfig;
use agent9527_config::types::AppToolConfig;
use agent9527_config::types::AppToolsConfig;
use agent9527_config::types::ApprovalsReviewer;
use agent9527_config::types::AppsConfigToml;
...
use agent9527_rollout_trace::ToolDispatchInvocation;
use agent9527_rollout_trace::ToolDispatchPayload;
use agent9527_rollout_trace::ToolDispatchRequester;
...
fn write_sample_plugin_mcp(agent9527_home: &std::path::Path) {
    let plugin_root = agent9527_home.join("plugins/cache/test/sample/local");
    ...
    let dispatch_trace = session
        ...
        .start_tool_dispatch_trace(|| {
            Some(ToolDispatchInvocation {
                ...
                agent9527_turn_id: turn_context.sub_id.clone(),
...
fn install_mcp_permission_request_hook(
    ...
    std::fs::create_dir_all(&turn_context.config.agent9527_home)
        .expect("create agent9527 home for MCP permission hook");
    let script = format!(
        r#"import json
...
"#,
    ...
    std::fs::write(
        turn_context.config.agent9527_home.join("hooks.json"),
        serde_json::json!({
            "hooks": {
                "PermissionRequest": [{
                    "matcher": matcher,
    ...
    let hook_list = agent9527_hooks::list_hooks(HooksConfig {
    ...
    let trusted_config_layer_stack = trusted_config_layer_stack(
        ...
        &turn_context.config.agent9527_home,
        hook_list.hooks,
    );
...
fn attach_trace_bundle(
    ...
    let rollout_thread_trace =
        agent9527_rollout_trace::ThreadTraceContext::start_root_in_root_for_test(
            root,
    ...
    rollout_thread_trace.record_agent9527_turn_started(turn_context.sub_id.as_str());
...
#[test]
fn openai_file_params_are_only_honored_for_agent9527_apps() {
    let params = HashMap::from([("file".to_string(), Vec::new())]);

    assert_eq!(
        openai_file_input_optional_fields_for_server(AGENT9527_APPS_MCP_SERVER_NAME, &params),
        Some(params.clone())
    );
    assert_eq!(
...
async fn mcp_tool_call_span_records_error_type_and_error_code() {
    ...
    let span = mcp_tool_call_span(
        ...
        &turn_context,
        McpToolCallSpanFields {
            server_name: AGENT9527_APPS_MCP_SERVER_NAME,
            tool_name: "calendar_search",
            call_id: "call-123",
            server_origin: Some("https://chatgpt.com/api/agent9527/ps/mcp"),
            connector_id: Some("calendar"),
            connector_name: Some("Calendar"),
        },
    );
    ...
    assert!(
        logs.contains("error.type=\"tool_result\"")
            && logs.contains("agent9527.mcp.error.code=\"RATE_LIMITED\""),
...
async fn mcp_result_telemetry_records_allowlisted_span_fields() {
    ...
    assert!(
        logs.contains("agent9527.mcp.target.id=\"com.apple.reminders\"")
            && logs.contains("agent9527.mcp.server_user_flow.triggered=false"),
...
async fn mcp_result_telemetry_ignores_invalid_and_missing_values() {
    ...
    .await;
    assert!(
        !invalid_logs.contains("agent9527.mcp.target.id=")
            && !invalid_logs.contains("agent9527.mcp.server_user_flow.triggered="),
    ...
    .await;
    assert!(
        !missing_logs.contains("agent9527.mcp.target.id=")
            && !missing_logs.contains("agent9527.mcp.server_user_flow.triggered="),
    ...
    assert!(
        !no_meta_logs.contains("agent9527.mcp.target.id=")
            && !no_meta_logs.contains("agent9527.mcp.server_user_flow.triggered="),
...
async fn mcp_result_telemetry_truncates_long_target_id() {
    ...
    assert!(
        logs.contains(&format!("agent9527.mcp.target.id=\"{truncated}\""))
            && !logs.contains("tail"),
...
#[test]
fn agent9527_apps_tool_question_uses_fallback_app_label() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        AGENT9527_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn trusted_agent9527_apps_tool_question_offers_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        AGENT9527_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn agent9527_apps_tool_question_without_elicitation_omits_always_allow() {
    let session_key = McpToolApprovalKey {
        server: AGENT9527_APPS_MCP_SERVER_NAME.to_string(),
...
#[test]
fn agent9527_apps_connectors_support_persistent_approval() {
    let invocation = McpInvocation {
        server: AGENT9527_APPS_MCP_SERVER_NAME.to_string(),
...
async fn mcp_tool_call_request_meta_includes_turn_metadata_for_custom_server() {
    ...
    let turn_metadata = meta
        .get(crate::X_AGENT9527_TURN_METADATA_HEADER)
    ...
    assert_eq!(
        meta,
        serde_json::json!({
            crate::X_AGENT9527_TURN_METADATA_HEADER: expected_turn_metadata,
        })
    );
}
...
#[test]
fn mcp_tool_call_item_metadata_only_trusts_agent9527_apps_identity() {
    let mut metadata = approval_metadata(
...
#[tokio::test]
async fn agent9527_apps_tool_call_request_meta_includes_turn_metadata_and_agent9527_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
        .current_meta_value_for_mcp_request(mcp_turn_metadata_context(&turn_context))
    ...
    assert_eq!(
        ...
        Some(serde_json::json!({
            crate::X_AGENT9527_TURN_METADATA_HEADER: expected_turn_metadata,
            MCP_TOOL_AGENT9527_APPS_META_KEY: {
...
#[tokio::test]
async fn agent9527_apps_tool_call_request_meta_includes_call_id_without_existing_agent9527_apps_meta()
 {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
...
fn agent9527_apps_auth_failure_result() -> CallToolResult {
    CallToolResult {
        content: vec![serde_json::json!({
            "type": "text",
...
fn agent9527_apps_auth_failure_metadata() -> McpToolApprovalMetadata {
    approval_metadata(
...
async fn host_owned_agent9527_apps_manager(
    session: &Session,
    turn_context: &TurnContext,
) -> Arc<agent9527_mcp::McpConnectionManager> {
    ...
    let mcp_servers = HashMap::from([(
        ...
        agent9527_mcp::EffectiveMcpServer::configured(
            agent9527_mcp::agent9527_apps_mcp_server_config(
                "https://chatgpt.com",
    ...
    let manager = agent9527_mcp::McpConnectionManager::new(
        &mcp_servers,
        ...
        agent9527_mcp::McpRuntimeContext::new(
        ...
        turn_context.config.agent9527_home.to_path_buf(),
        session.services.mcp_manager.agent9527_apps_tools_cache(),
        session.services.mcp_manager.tool_catalog_cache(),
        agent9527_connectors::connector_runtime_context_key(auth.as_ref()),
        turn_context.config.prefix_mcp_tool_names(),
        rmcp::model::ElicitationCapability::default(),
        /*supports_openai_form_elicitation*/ false,
        agent9527_mcp::ToolPluginProvenance::default(),
        auth.as_ref(),
        /*agent9527_apps_auth_manager*/ None,
        /*elicitation_reviewer*/ None,
        /*elicitation_lifecycle*/ None,
        agent9527_mcp::ElicitationRequestRouter::default(),
    )
    .await;
    Arc::new(manager)
}
...
#[tokio::test]
async fn agent9527_apps_auth_elicitation_feature_disabled_returns_original_result() {
    ...
    let manager = host_owned_agent9527_apps_manager(&session, &turn_context).await;
    let result = agent9527_apps_auth_failure_result();
    let metadata = agent9527_apps_auth_failure_metadata();

    let returned = maybe_request_agent9527_apps_auth_elicitation(
        &session,
        &turn_context,
        manager.as_ref(),
        "call_123",
...
#[tokio::test]
async fn agent9527_apps_auth_elicitation_non_host_owned_server_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let mut features = Features::with_defaults();
...
#[tokio::test]
async fn agent9527_apps_auth_elicitation_disallowed_by_policy_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let manager = host_owned_agent9527_apps_manager(&session, &turn_context).await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
...
#[tokio::test]
async fn agent9527_apps_auth_elicitation_granular_mcp_disabled_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let manager = host_owned_agent9527_apps_manager(&session, &turn_context).await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
...
#[tokio::test]
async fn agent9527_apps_auth_elicitation_enabled_by_default_requests_elicitation() {
    let (session, turn_context, rx_event) = make_session_and_context_with_rx().await;
    let manager = host_owned_agent9527_apps_manager(&session, &turn_context).await;
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let request_task = tokio::spawn({
        ...
        async move {
            maybe_request_agent9527_apps_auth_elicitation(
                &session,
                &turn_context,
                manager.as_ref(),
                "call_123",
    ...
    assert_eq!(request.server_name, AGENT9527_APPS_MCP_SERVER_NAME);
    assert_eq!(
        request.id,
        agent9527_protocol::mcp::RequestId::String("agent9527_apps_auth_call_123".to_string())
    );
    assert!(matches!(
        request.request,
        agent9527_protocol::approvals::ElicitationRequest::Url { .. }
    ));

    session
        .resolve_elicitation(
            AGENT9527_APPS_MCP_SERVER_NAME.to_string(),
            rmcp::model::RequestId::String("agent9527_apps_auth_call_123".into()),
            ElicitationResponse {
...
async fn guardian_review_decision_maps_to_mcp_tool_decision() {
    ...
    session.services.guardian_rejections.lock().await.insert(
        ...
        crate::guardian::GuardianRejection {
            ...
            source: agent9527_protocol::protocol::GuardianAssessmentDecisionSource::Agent,
        },
    );
...
#[test]
fn approval_elicitation_meta_includes_connector_source_for_agent9527_apps() {
    assert_eq!(
        build_mcp_tool_approval_elicitation_meta(
            AGENT9527_APPS_MCP_SERVER_NAME,
            Some(&approval_metadata(
...
#[tokio::test]
async fn persist_agent9527_app_tool_approval_writes_tool_override() {
    ...
    persist_agent9527_app_tool_approval(&config, "calendar", "calendar/list_events")
        .await
...
async fn custom_mcp_tool_approval_mode_uses_plugin_mcp_policy() {
    ...
    write_sample_plugin_mcp(agent9527_home.as_path());
    std::fs::write(
...
async fn maybe_persist_mcp_tool_approval_reloads_session_config_for_custom_server() {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .agent9527_home(agent9527_home.clone().to_path_buf())
        .build()
        .await
        .expect("load config");
    turn_context.config = Arc::new(config);
...
"#,
    ...
    let contents =
        std::fs::read_to_string(agent9527_home.join(CONFIG_TOML_FILE)).expect("read config");
    let parsed: ConfigToml = toml::from_str(&contents).expect("parse config");
    let tool = parsed
        .plugins
        .get("sample@test")
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_agent9527_dir = project_dir.path().join(".agent9527");
    std::fs::create_dir_all(&project_agent9527_dir).expect("create project .agent9527 dir");
    std::fs::write(
        project_agent9527_dir.join(CONFIG_TOML_FILE),
        "[mcp_servers.docs]\ncommand = \"docs-server\"\n",
    )
    ...
    ConfigEditsBuilder::new(&agent9527_home)
        .set_project_trust_level(
            project_dir.path(),
            agent9527_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
    ...
    let contents = std::fs::read_to_string(project_agent9527_dir.join(CONFIG_TOML_FILE))

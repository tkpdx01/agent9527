...
use agent9527_protocol::protocol::McpServerRefreshConfig;
...
impl SessionIo {
    ...
    pub(crate) async fn shutdown_and_wait(&self) -> Agent9527Result<()> {
        ...
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(Agent9527Err::InternalAgentDied) => {}
...
impl Session {
    ...
    async fn maybe_warn_on_server_model_mismatch(
        ...
        let warning_message = format!(
            "Your account was flagged for potentially high-risk cyber activity and this request was routed to gpt-5.2 as a fallback. To regain access to gpt-5.3-agent9527, apply for trusted access: {CYBER_VERIFY_URL} or learn more: {CYBER_SAFETY_URL}"
        );

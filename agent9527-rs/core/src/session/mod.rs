...
use agent9527_analytics::SubAgentThreadStartedInput;
use agent9527_analytics::TurnAgent9527ErrorFact;
use agent9527_async_utils::OrCancelExt;
use agent9527_connectors::connector_runtime_context_key;
use agent9527_core_skills::injection::HostSkillsCatalogInWorldState;
use agent9527_exec_server::Environment;
use agent9527_exec_server::EnvironmentManager;
use agent9527_execpolicy::prefix_rule_migration;
...
use agent9527_extension_api::PromptSlot;
use agent9527_extension_api::TurnContextContributionInput;
use agent9527_features::FEATURES;
use agent9527_features::Feature;
use agent9527_features::unstable_features_warning_event;
use agent9527_hooks::Hooks;
use agent9527_hooks::HooksConfig;
use agent9527_login::Agent9527Auth;
use agent9527_login::AuthManager;
use agent9527_login::auth_env_telemetry::collect_auth_env_telemetry;
use agent9527_mcp::McpResourceClient;
...
use agent9527_network_proxy::NetworkProxyAuditMetadata;
use agent9527_network_proxy::normalize_host;
use agent9527_otel::current_span_trace_id;
...
use agent9527_protocol::approvals::ElicitationRequest;
use agent9527_protocol::approvals::ElicitationRequestEvent;
use agent9527_protocol::approvals::ExecPolicyAmendment;
use agent9527_protocol::approvals::NetworkPolicyAmendment;
use agent9527_protocol::approvals::NetworkPolicyRuleAction;
...
use agent9527_protocol::dynamic_tools::DynamicToolResponse;
use agent9527_protocol::dynamic_tools::DynamicToolSpec;
use agent9527_protocol::items::EnteredReviewModeItem;
use agent9527_protocol::items::TurnItem;
use agent9527_protocol::items::UserMessageItem;
...
use agent9527_protocol::models::format_allow_prefixes;
...
use agent9527_protocol::protocol::HasLegacyEvent;
use agent9527_protocol::protocol::HistoryPosition;
...
use agent9527_protocol::protocol::MULTI_AGENT_MODE_OPEN_TAG;
use agent9527_protocol::protocol::MultiAgentVersion;
use agent9527_protocol::protocol::RawResponseItemEvent;
...
use agent9527_protocol::protocol::TurnContextItem;
use agent9527_protocol::protocol::TurnContextNetworkItem;
...
use agent9527_protocol::protocol::WorldStateItem;
...
use agent9527_protocol::request_permissions::RequestPermissionsArgs;
use agent9527_protocol::request_permissions::RequestPermissionsEvent;
use agent9527_protocol::request_permissions::RequestPermissionsResponse;
use agent9527_protocol::request_user_input::RequestUserInputArgs;
use agent9527_protocol::request_user_input::RequestUserInputResponse;
use agent9527_rmcp_client::ElicitationResponse;
use agent9527_rollout::state_db;
use agent9527_rollout_trace::AgentResultTracePayload;
use agent9527_rollout_trace::ThreadStartedTraceMetadata;
use agent9527_rollout_trace::ThreadTraceContext;
use agent9527_sandboxing::policy_transforms::intersect_permission_profiles;
use agent9527_shell_command::parse_command::parse_command;
use agent9527_terminal_detection::user_agent;
use agent9527_thread_store::CreateThreadParams;
use agent9527_thread_store::LiveThread;
use agent9527_thread_store::LiveThreadInitGuard;
use agent9527_thread_store::LocalThreadStore;
use agent9527_thread_store::ReadThreadParams;
use agent9527_thread_store::ResumeThreadParams;
...
use crate::agent9527_thread::ThreadConfigSnapshot;
...
#[cfg(test)]
use agent9527_protocol::exec_output::StreamOutput;
    ...
    fn to_error_event(&self) -> ErrorEvent {
        match self {
            Self::NoActiveTurn(_) => ErrorEvent {
                ...
                agent9527_error_info: Some(Agent9527ErrorInfo::BadRequest),
            },
...
use agent9527_core_plugins::PluginCommandAttribution;
use agent9527_core_plugins::PluginsManager;
use agent9527_core_plugins::RecommendedPluginCandidatesInput;
use agent9527_git_utils::get_git_repo_root;
use agent9527_mcp::McpConfig;
use agent9527_mcp::effective_mcp_servers;
use agent9527_otel::SessionTelemetry;
use agent9527_otel::THREAD_STARTED_METRIC;
...
use agent9527_protocol::models::LocalImagePreparation;
...
use agent9527_protocol::protocol::ApplyPatchApprovalRequestEvent;
use agent9527_protocol::protocol::AskForApproval;
use agent9527_protocol::protocol::CompactedItem;
use agent9527_protocol::protocol::DeprecationNoticeEvent;
use agent9527_protocol::protocol::ErrorEvent;
...
use agent9527_protocol::protocol::ModelRerouteEvent;
use agent9527_protocol::protocol::ModelRerouteReason;
use agent9527_protocol::protocol::ModelVerification;
use agent9527_protocol::protocol::ModelVerificationEvent;
use agent9527_protocol::protocol::NetworkApprovalContext;
...
use agent9527_protocol::protocol::RequestUserInputEvent;
...
use agent9527_protocol::protocol::SessionNetworkProxyRuntime;
use agent9527_protocol::protocol::StreamErrorEvent;
...
use agent9527_protocol::protocol::TurnModerationMetadataEvent;
use agent9527_protocol::protocol::WarningEvent;
use agent9527_protocol::user_input::UserInput;
use agent9527_tools::UnifiedExecShellMode;
use agent9527_utils_absolute_path::AbsolutePathBuf;
#[cfg(test)]
use agent9527_utils_stream_parser::ProposedPlanSegment;
...
pub(crate) struct SessionSpawnArgs {
    ...
    pub(crate) code_mode_session_provider: Arc<dyn agent9527_code_mode::CodeModeSessionProvider>,
    pub(crate) extensions: Arc<agent9527_extension_api::ExtensionRegistry<crate::config::Config>>,
    ...
    pub(crate) windows_sandbox_proxy_settings_mode:
        agent9527_sandboxing::WindowsSandboxProxySettingsMode,
}
...
const CYBER_SAFETY_URL: &str = "https://developers.openai.com/agent9527/concepts/cyber-safety";

impl Session {
    /// Spawn and initialize a new session.
    pub(crate) async fn spawn(args: SessionSpawnArgs) -> Agent9527Result<(Arc<Self>, SessionIo)> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if agent9527_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
    ...
    async fn spawn_internal(args: SessionSpawnArgs) -> Agent9527Result<(Arc<Self>, SessionIo)> {
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            {
                let agent9527_home = config.agent9527_home.clone();
                let policy_path = default_policy_path(agent9527_home.as_path());
                if let Err(err) = prefix_rule_migration(
                    agent9527_home.as_path(),
                    policy_path.as_path(),
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| Agent9527Err::Fatal(format!("failed to load rules: {err}")))?,
            )
        };

        let config = Arc::new(config);
        let refresh_strategy = if session_source.is_non_root_agent() {
            agent9527_models_manager::manager::RefreshStrategy::Offline
        } else {
            agent9527_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.agent9527_home)
        })?;
...
impl SessionIo {
    ...
    pub(crate) async fn submit(&self, op: Op) -> Agent9527Result<String> {
    ...
    pub(crate) async fn submit_with_id(&self, mut sub: Submission) -> Agent9527Result<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| Agent9527Err::InternalAgentDied)?;
        Ok(())
    }

    pub(crate) async fn shutdown_and_wait(&self) -> Agent9527Result<()> {
        let session_loop_termination = self.session_loop_termination.clone();
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(err) if matches!(err.details(), Agent9527ErrorDetails::InternalAgentDied) => {}
    ...
    pub(crate) async fn next_event(&self) -> Agent9527Result<Event> {
        let event = self
            .rx_event
            .recv()
            .await
...
fn session_permission_profile_state_from_config(
    config: &Config,
) -> Agent9527Result<PermissionProfileState> {
...
impl Session {
    ...
    /// Builds the `x-agent9527-beta-features` header value for this session.
    ///
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &agent9527_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn agent9527_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn agent9527_network_proxy::BlockedRequestObserver>>,
    ...
    #[cfg(test)]
    pub(crate) async fn agent9527_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.agent9527_home().clone()
    }
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect Agent9527 performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn user_instructions(
        &self,
    ) -> Option<agent9527_extension_api::UserInstructions> {
    ...
    /// Record a terminal Agent9527Err before the app-server completion notification is reduced.
    pub(crate) fn track_turn_agent9527_error(
        &self,
        turn_context: &TurnContext,
        error: &Agent9527Err,
    ) {
        self.services
            .analytics_events_client
            .track_turn_agent9527_error(TurnAgent9527ErrorFact::from_agent9527_err(
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        if let EventMsg::Error(error) = &legacy_source
            && error
                ...
                .as_ref()
                .is_some_and(Agent9527ErrorInfo::affects_turn_status)
        {
            turn_context
                .terminal_error
                .lock()
        ...
        self.services
            ...
            .record_agent9527_turn_event(&turn_context.sub_id, &legacy_source);
        self.services
            .rollout_thread_trace
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &agent9527_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| agent9527_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn send_event_raw_without_materializing_rollout(&self, event: Event) {
        let persist = match self.current_rollout_path().await {
            Ok(Some(path)) => agent9527_rollout::existing_rollout_path(&path)
                .await
                .is_some(),
            Ok(None) => true,
            Err(err) => {
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&agent9527_home, amendment)
            .await?;
    ...
    pub(crate) async fn request_permissions_for_environment(
        ...
        let Ok(native_environment_cwd) = environment.cwd.to_abs_path() else {
            warn!(
                cwd = %environment.cwd,
                "request_permissions requires a cwd native to the Agent9527 host"
            );
            return Some(RequestPermissionsResponse {
                permissions: RequestPermissionProfile::default(),
                scope: PermissionGrantScope::Turn,
            ...
            let review_rx = crate::guardian::spawn_approval_request_review(
                ...
                GuardianReviewOptions {
                    ...
                    approval_request_source:
                        agent9527_analytics::GuardianApprovalRequestSource::MainTurn,
    ...
    pub(crate) async fn capture_step_context(
        ...
        turn_context: Arc<TurnContext>,
        cancellation_token: &CancellationToken,
    ) -> Agent9527Result<Arc<StepContext>> {
        ...
        let executor_capability_discovery = self
            ...
            .await;
        let extension_data =
            agent9527_extension_api::ExtensionData::new(turn_context.sub_id.clone());
    ...
    async fn maybe_warn_on_server_model_mismatch(
        ...
        let warning_message = format!(
            "Your account was flagged for potentially high-risk cyber activity and this request was routed to {server_model} as a fallback. To regain access to {requested_model}, apply for trusted access: {CYBER_VERIFY_URL} or learn more: {CYBER_SAFETY_URL}"
        );
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        {
            let mut state = self.state.lock().await;
            state.queue_pending_session_start_source(agent9527_hooks::SessionStartSource::Compact);
        }
    }
    ...
    pub(crate) async fn notify_stream_error(
        ...
        agent9527_error: Agent9527Err,
    ) {
        let additional_details = agent9527_error.to_string();
        let agent9527_error_info = Agent9527ErrorInfo::ResponseStreamDisconnected {
            http_status_code: agent9527_error.http_status_code_value(),
        };
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<agent9527_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
...
async fn build_hooks_for_config(
    ...
    Hooks::new(HooksConfig {
        ...
        feature_enabled: config.features.enabled(Feature::Agent9527Hooks),

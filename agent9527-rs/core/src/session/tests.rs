...
use super::*;
use crate::agent9527_thread::TryStartTurnIfIdleRejectionReason;
...
use agent9527_config::ConfigLayerStackOrdering;
use agent9527_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID;
use agent9527_config::LoaderOverrides;
use agent9527_config::NetworkConstraints;
use agent9527_config::NetworkDomainPermissionToml;
use agent9527_config::NetworkDomainPermissionsToml;
use agent9527_config::RequirementSource;
use agent9527_config::Sourced;
...
use agent9527_config::types::ToolSuggestDisabledTool;
use agent9527_core_skills::HostSkillsSnapshot;
...
use agent9527_login::auth::AgentIdentityAuthPolicy;
use agent9527_model_provider_info::ModelProviderInfo;
use agent9527_models_manager::MODEL_BALANCED;
use agent9527_models_manager::model_info;
...
use agent9527_models_manager::upstream_bundled_models_response;
use agent9527_protocol::AgentPath;
use agent9527_protocol::ResponseItemId;
use agent9527_protocol::SessionId;
use agent9527_protocol::ThreadId;
use agent9527_protocol::config_types::SERVICE_TIER_DEFAULT_REQUEST_VALUE;
...
use agent9527_protocol::models::AgentMessageInputContent;
use agent9527_protocol::models::FileSystemPermissions;
use agent9527_protocol::models::FunctionCallOutputBody;
use agent9527_protocol::models::FunctionCallOutputContentItem;
use agent9527_protocol::models::FunctionCallOutputPayload;
use agent9527_protocol::models::ImageDetail;
use agent9527_protocol::models::PermissionProfile;
use agent9527_protocol::models::SandboxEnforcement;
use agent9527_protocol::openai_models::ModelServiceTier;
...
use agent9527_protocol::protocol::NonSteerableTurnKind;
use agent9527_protocol::protocol::SandboxPolicy;
use agent9527_protocol::protocol::TurnEnvironmentSelections;
use agent9527_protocol::request_permissions::PermissionGrantScope;
use agent9527_protocol::request_permissions::RequestPermissionProfile;
use agent9527_utils_path_uri::PathUri;
use tracing::Span;
...
use agent9527_config::config_toml::ProjectConfig;
use agent9527_config::permissions_toml::FilesystemPermissionToml;
use agent9527_config::permissions_toml::FilesystemPermissionsToml;
use agent9527_config::permissions_toml::NetworkToml;
use agent9527_config::permissions_toml::PermissionProfileToml;
use agent9527_config::permissions_toml::PermissionsToml;
use agent9527_execpolicy::Decision;
use agent9527_execpolicy::NetworkRuleProtocol;
use agent9527_execpolicy::Policy;
use agent9527_network_proxy::NetworkProxyConfig;
use agent9527_otel::MetricsClient;
use agent9527_otel::MetricsConfig;
use agent9527_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use agent9527_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use agent9527_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use agent9527_otel::THREAD_SKILLS_TRUNCATED_METRIC;
...
use agent9527_protocol::items::HookPromptFragment;
use agent9527_protocol::items::build_hook_prompt_message;
use agent9527_protocol::models::BaseInstructions;
use agent9527_protocol::models::ContentItem;
use agent9527_protocol::models::InternalChatMessageMetadataPassthrough;
...
use agent9527_protocol::protocol::CompactedItem;
use agent9527_protocol::protocol::ConversationAudioParams;
use agent9527_protocol::protocol::CreditsSnapshot;
use agent9527_protocol::protocol::GranularApprovalConfig;
use agent9527_protocol::protocol::InitialHistory;
use agent9527_protocol::protocol::InterAgentCommunication;
use agent9527_protocol::protocol::MultiAgentVersion;
use agent9527_protocol::protocol::NetworkApprovalProtocol;
use agent9527_protocol::protocol::RateLimitSnapshot;
use agent9527_protocol::protocol::RateLimitWindow;
use agent9527_protocol::protocol::RealtimeAudioFrame;
use agent9527_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
use agent9527_protocol::protocol::RealtimeVoice;
...
use agent9527_protocol::protocol::SessionMeta;
use agent9527_protocol::protocol::SessionMetaLine;
use agent9527_protocol::protocol::SkillScope;
use agent9527_protocol::protocol::Submission;
use agent9527_protocol::protocol::ThreadRolledBackEvent;
use agent9527_protocol::protocol::ThreadSettingsOverrides;
use agent9527_protocol::protocol::TokenCountEvent;
use agent9527_protocol::protocol::TokenUsage;
use agent9527_protocol::protocol::TokenUsageInfo;
use agent9527_protocol::protocol::TurnAbortedEvent;
use agent9527_protocol::protocol::TurnCompleteEvent;
use agent9527_protocol::protocol::TurnStartedEvent;
use agent9527_protocol::protocol::UserMessageEvent;
use agent9527_protocol::protocol::W3cTraceContext;
use agent9527_rmcp_client::ElicitationAction;
...
use core_test_support::test_agent9527::local;
...
use agent9527_protocol::mcp::CallToolResult as McpCallToolResult;
...
pub(crate) fn mcp_config_for_test(config: &crate::config::Config) -> Arc<agent9527_mcp::McpConfig> {
    Arc::new(config.to_mcp_config_with_loaded_plugins(
        &agent9527_core_plugins::PluginLoadOutcome::default(),
        std::iter::empty(),
    ))
}
...
impl StepContext {
    pub(crate) fn for_test(turn: Arc<TurnContext>) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            mcp: Arc::new(agent9527_mcp::McpBinding::empty(mcp_config_for_test(
                &turn.config,
            ))),
            mcp_tools: Vec::new(),
...
async fn regular_turn_emits_turn_started_with_trace_id_without_waiting_for_startup_prewarm() {
    let _trace_test_context = install_test_tracing("agent9527-core-tests");
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        agent9527_protocol::protocol::SessionSource::Exec,
        "test_originator".to_string(),
...
fn write_project_hooks(dot_agent9527: &Path) -> std::io::Result<()> {
    std::fs::create_dir_all(dot_agent9527)?;
    std::fs::write(
        dot_agent9527.join("hooks.json"),
        r#"{
  "hooks": {
    "SessionStart": [
      {
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        agent9527_home.join(agent9527_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<agent9527_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&agent9527_hooks::SessionStartRequest {
            session_id: ThreadId::new(),
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            target: agent9527_hooks::StartHookTarget::SessionStart {
                source: agent9527_hooks::SessionStartSource::Startup,
            },
        }),
    )
}
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn agent9527_network_proxy::NetworkPolicyDecider> =
        Arc::new({
            let decider_calls = Arc::clone(&decider_calls);
            move |_request| {
                decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
                async { agent9527_network_proxy::NetworkDecision::ask("not_allowed") }
            }
        });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Approvable<()> for ProbeToolRuntime {
        ...
        fn approval_action(
            ...
            Ok(crate::tools::sandboxing::ApprovalAction::Shell {
                ...
                environment_id: agent9527_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
                command: Vec::new(),
                #[allow(deprecated)]
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> agent9527_sandboxing::SandboxablePreference {
            agent9527_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(agent9527_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: agent9527_tools::ToolName::plain("probe"),
    };
...
async fn user_shell_commands_do_not_inherit_managed_network_proxy() -> anyhow::Result<()> {
    let expected_http_proxy = std::env::var("HTTP_PROXY").unwrap_or_else(|_| "not-set".to_string());
    let permission_profile = PermissionProfile::workspace_write();
    let network_spec = crate::config::NetworkProxySpec::from_config_and_constraints(
        NetworkProxyConfig::default(),
        Some(NetworkConstraints {
    ...
    loop {
        ...
        if let EventMsg::ExecCommandEnd(event) = event.msg {
            ...
            assert_eq!(event.stdout.trim(), expected_http_proxy);
            break;
        }
    }
...
async fn get_base_instructions_no_user_content() {
    ...
    let models_response = upstream_bundled_models_response()
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let agent9527_home = session.agent9527_home().await;
    std::fs::create_dir_all(&agent9527_home).expect("create agent9527 home");
    let config_toml_path = agent9527_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = agent9527_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let agent9527_home = session.agent9527_home().await;
    std::fs::create_dir_all(&agent9527_home).expect("create agent9527 home");
    let base_config_path = agent9527_home.join(CONFIG_TOML_FILE);
    let profile_config_path = agent9527_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .agent9527_home(agent9527_home.to_path_buf())
    ...
    assert_eq!(
        config
            ...
            .map(agent9527_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    let session = make_session_with_config(|config| {
        config
            ...
            .expect("enable Agent9527 hooks");
    })
    .await?;
    let agent9527_home = session.agent9527_home().await;
    std::fs::create_dir_all(&agent9527_home)?;
    let config_toml_path = agent9527_home.join(CONFIG_TOML_FILE);
    let user_config: agent9527_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let request = agent9527_hooks::SessionStartRequest {
    ...
    let config = session.get_config().await;
    let hook_list = agent9527_hooks::list_hooks(agent9527_hooks::HooksConfig {
        feature_enabled: true,
    ...
    assert_eq!(
        ...
        agent9527_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: agent9527_config::TomlValue =
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: agent9527_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: agent9527_config::MatcherGroup {
                matcher: None,
                hooks: vec![agent9527_config::HookHandlerConfig::Command {
        ...
        let identity = agent9527_config::TomlValue::try_from(identity)?;
        agent9527_config::version_for_toml(&identity)
    };
...
async fn refresh_mcp_config_replaces_managed_server_and_plugin_requirements() {
    ...
    let requirement = serde_json::from_value::<agent9527_config::McpServerRequirement>(json!({
        "identity": { "url": "https://example.com/mcp" }
    }))
    .expect("valid managed MCP requirement");
    let plugin_requirements = std::collections::BTreeMap::from([(
        "example-plugin".to_string(),
        agent9527_config::PluginRequirementsToml {
    ...
    next_config.mcp_servers = agent9527_config::Constrained::normalized(
...
async fn subagent_activity_emits_matching_start_and_completion() {
    let (session, turn_context, rx) = make_session_and_context_with_rx().await;
    let item = agent9527_protocol::items::SubAgentActivityItem {
        id: "activity-1".to_string(),
        kind: agent9527_protocol::protocol::SubAgentActivityKind::Started,
...
async fn record_inter_agent_communication_preserves_item_id_in_rollout_and_resume() {
    let (mut session, turn_context, _rx) = make_session_and_context_with_auth_and_config_and_rx(
        Agent9527Auth::from_api_key("Test API Key"),
        Vec::new(),
        |_| {},
    )
    .await;
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl agent9527_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage<'a>(
            &'a self,
            session_store: &'a agent9527_extension_api::ExtensionData,
            thread_store: &'a agent9527_extension_api::ExtensionData,
            turn_store: &'a agent9527_extension_api::ExtensionData,
            token_usage: &'a TokenUsageInfo,
        ) -> agent9527_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
    ...
    let mut builder =
        agent9527_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
...
async fn turn_start_lifecycle_exposes_turn_metadata_and_token_baseline() {
    ...
    impl agent9527_extension_api::TurnLifecycleContributor for TurnStartRecorder {
        fn on_turn_start<'a>(
            &'a self,
            input: agent9527_extension_api::TurnStartInput<'a>,
        ) -> agent9527_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    impl agent9527_extension_api::TurnLifecycleContributor for TurnErrorRecorder {
        fn on_turn_error<'a>(
            &'a self,
            input: agent9527_extension_api::TurnErrorInput<'a>,
        ) -> agent9527_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
    ...
    let expected = RecordedTurnError {
        ...
        turn_level_id: turn_context.sub_id.clone(),
        turn_id: turn_context.sub_id.clone(),
        error: Agent9527ErrorInfo::UsageLimitExceeded,
        saw_session_store: true,
        saw_thread_store: true,
    };

    session
        .emit_turn_error_lifecycle(&turn_context, Agent9527ErrorInfo::UsageLimitExceeded)
        .await;
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl agent9527_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
        fn on_config_changed(
            &self,
            session_store: &agent9527_extension_api::ExtensionData,
            thread_store: &agent9527_extension_api::ExtensionData,
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: agent9527_protocol::protocol::NetworkAccess::Restricted,
    };
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_agent9527().with_config(|config| {
        config.permissions.approval_policy =
            agent9527_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    fork_config.permissions.approval_policy =
        agent9527_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "agent9527_core__agent9527_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: agent9527_protocol::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            agent9527_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            agent9527_protocol::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            agent9527_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_thread_history() {
    ...
    assert_eq!(
        error_event.agent9527_error_info,
        Some(Agent9527ErrorInfo::ThreadRollbackFailed)
    );
    assert_eq!(
...
async fn set_rate_limits_retains_previous_credits() {
    let agent9527_home = tempfile::tempdir().expect("create temp dir");
    let config = build_test_config(agent9527_home.path()).await;
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(agent9527_protocol::account::PlanType::Plus),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(initial.clone());

    let update = RateLimitSnapshot {
        limit_id: Some("agent9527_other".to_string()),
        limit_name: Some("agent9527_other".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 40.0,
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        individual_limit: None,
        spend_control_reached: None,
        plan_type: Some(agent9527_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
    ...
    assert_eq!(
        state.latest_rate_limits,
        Some(RateLimitSnapshot {
            limit_id: Some("agent9527".to_string()),
            limit_name: None,
...
async fn turn_context_with_model_updates_model_fields() {
    ...
    assert_eq!(updated.collaboration_mode().model(), MODEL_BALANCED);
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.agent9527_error_info
                    == Some(Agent9527ErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
...
async fn build_test_config(agent9527_home: &Path) -> Config {
...
async fn emit_subagent_session_started_includes_fork_lineage_and_originator() {
    ...
    Mock::given(method("POST"))
        .and(path("/agent9527/analytics-events/events"))
        .respond_with(ResponseTemplate::new(200))
        .mount(&server)
        .await;

    let auth_manager =
        AuthManager::from_auth_for_testing(Agent9527Auth::create_dummy_chatgpt_auth_for_testing());
    ...
    emit_subagent_session_started(
        ...
        AppServerClientMetadata {
            client_name: Some("agent9527-tui".to_string()),
            ...
            if let Some(requests) = server.received_requests().await {
                for request in requests {
                    ...
                    if let Some(event) = payload["events"].as_array().and_then(|events| {
                        events
                            .iter()
                            .find(|event| event["event_type"] == "agent9527_thread_initialized")
                    }) {
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile =
        agent9527_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        agent9527_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn active_profile_update_rebuilds_network_proxy_config() -> std::io::Result<()> {
    let agent9527_home = tempfile::tempdir().expect("create agent9527 home");
    let cwd = tempfile::tempdir().expect("create cwd");
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let agent9527_home = parent_config.agent9527_home.clone();
    let skill_dir = agent9527_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&agent9527_exec_server::LOCAL_FS));
    ...
    let role_path = agent9527_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    let agent9527_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(agent9527_home.path()).await;
    config
        .features
    ...
    let auth_manager =
        AuthManager::from_auth_for_testing(Agent9527Auth::from_api_key("Test API Key"));
    ...
    let plugins_manager = Arc::new(PluginsManager::new(config.agent9527_home.to_path_buf()));
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(agent9527_code_mode::InProcessCodeModeSessionProvider),
        Arc::new(agent9527_extension_api::ExtensionRegistryBuilder::new().build()),
        agent9527_extension_api::ExtensionDataInit::default(),
        /*supports_openai_form_elicitation*/ false,
        AgentControl::default(),
        environment_manager,
        /*inherited_environments*/ None,
        /*analytics_events_client*/ None,
        Arc::new(agent9527_thread_store::LocalThreadStore::new(
            agent9527_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        agent9527_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
        /*external_time_provider*/ None,
        Some(config.multi_agent_version_from_features()),
        GitEnrichmentPolicy::Fresh,
        agent9527_sandboxing::WindowsSandboxProxySettingsMode::Reconcile,
    )
    .await;
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let mcp_runtime = Arc::new(agent9527_mcp::McpRuntime::empty(
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: agent9527_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(agent9527_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: agent9527_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: agent9527_extension_api::ExtensionData::new(thread_id.to_string()),
        selected_capability_roots: Vec::new(),
        mcp_thread_init: agent9527_extension_api::ExtensionDataInit::default(),
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(agent9527_thread_store::LocalThreadStore::new(
            agent9527_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
...
async fn load_latest_config_for_session(session: &Session) -> Config {
    let config = session.get_config().await;
    ConfigBuilder::default()
        .agent9527_home(config.agent9527_home.to_path_buf())
...
async fn make_session_with_history_source_and_agent_control_and_rx(
    ...
    let session = Session::new(
        ...
        Arc::new(agent9527_thread_store::LocalThreadStore::new(
            agent9527_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            Some(
                agent9527_state::StateRuntime::init(
                    config.sqlite.clone(),
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            agent9527_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(agent9527_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
    ...
    assert_eq!(
        session
            .granted_turn_permissions(agent9527_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
        None
    );
}
    ...
    *session.active_turn.lock().await = Some(current_active_turn);
    ...
    session
        .record_granted_request_permissions_for_turn(
            ...
            agent9527_exec_server::LOCAL_ENVIRONMENT_ID,
    ...
    assert_eq!(
        originating_turn_state
            .lock()
            .await
            .granted_permissions(agent9527_exec_server::LOCAL_ENVIRONMENT_ID),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = agent9527_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(agent9527_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            ...
            session
                .request_permissions_for_environment(
                    &turn_context,
                    call_id,
                    agent9527_protocol::request_permissions::RequestPermissionsArgs {
                        environment_id: None,
    ...
    assert_eq!(
        ...
        Some(agent9527_exec_server::LOCAL_ENVIRONMENT_ID)
    );
    #[allow(deprecated)]
    let turn_cwd = turn_context.cwd.clone();
        ...
        async move {
            handler
                .handle(ToolInvocation {
                    ...
                    tracker,
                    call_id,
                    tool_name: agent9527_tools::ToolName::plain("request_permissions"),
...
async fn request_permissions_response_materializes_session_cwd_grants_before_recording() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_session_permissions(agent9527_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
...
async fn user_turn_updates_approvals_reviewer() {
    ...
    handlers::user_input_or_turn(
        ...
        Op::UserInput {
            ...
            responsesapi_client_metadata: None,
            additional_context: Default::default(),
            thread_settings: agent9527_protocol::protocol::ThreadSettingsOverrides {
                environments: Some(local_selections(config.cwd.clone())),
                approval_policy: Some(config.permissions.approval_policy.value()),
                approvals_reviewer: Some(agent9527_config::types::ApprovalsReviewer::AutoReview),
                sandbox_policy: Some(config.legacy_sandbox_policy()),
                summary: config.model_reasoning_summary,
                personality: config.personality,
                collaboration_mode: Some(agent9527_protocol::config_types::CollaborationMode {
                    mode: agent9527_protocol::config_types::ModeKind::Default,
                    settings: agent9527_protocol::config_types::Settings {
    ...
    assert_eq!(
        ...
        agent9527_config::types::ApprovalsReviewer::AutoReview
    );
    assert!(
...
async fn primary_environment_uses_first_turn_environment() {
    ...
    let second_cwd_uri = agent9527_utils_path_uri::PathUri::from_abs_path(&second_cwd);
    turn_context
        .environments
        .environments
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        agent9527_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context =
        agent9527_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(agent9527_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn agent9527_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    assert_eq!(
        agent9527_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_runs_full_thread_teardown() {
    ...
    impl agent9527_extension_api::ThreadLifecycleContributor<crate::config::Config>
        for ThreadStopRecorder
    {
        fn on_thread_stop<'a>(
            &'a self,
            input: agent9527_extension_api::ThreadStopInput<'a>,
        ) -> agent9527_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl agent9527_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort<'a>(
            &'a self,
            input: agent9527_extension_api::TurnAbortInput<'a>,
        ) -> agent9527_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
{
    let (tx_event, rx_event) = async_channel::unbounded();
    let mut config = build_test_config(agent9527_home).await;
...
async fn refresh_mcp_servers_uses_latest_state_for_existing_turns() {
    ...
    .expect("parse refreshed MCP servers");
    {
        ...
        config.mcp_oauth_credentials_store_mode =
            agent9527_config::types::OAuthCredentialsStoreMode::Auto;
        config
            .features
    ...
    let new_step = session
        ...
        .await
        .expect("a fresh cancellation token cannot be cancelled");
    let old_environments = turn_context.environments.refresh_readiness();
    ...
    let configured_servers = agent9527_mcp::configured_mcp_servers(new_step.mcp.config());
    assert_eq!(
        configured_servers.get("refreshed"),
        refreshed_mcp_servers.get("refreshed")
    );
    assert!(
        !agent9527_mcp::configured_mcp_servers(old_step.mcp.config()).contains_key("refreshed"),
        "an already-bound step must keep its captured config"
    );
    assert!(
        agent9527_mcp::configured_mcp_servers(rematerialized_old.config())
    ...
    assert!(
        agent9527_mcp::configured_mcp_servers(current.config()).contains_key("refreshed"),
...
async fn mcp_refresh_updates_plugin_auth_mode_before_checking_pending_state() {
    let agent9527_home = tempfile::tempdir().expect("create auth test directory");
    let (mut session, _turn_context) = make_session_and_context().await;
    session.services.auth_manager = AuthManager::from_auth_for_testing_with_home(
        Agent9527Auth::from_api_key("old-api-key"),
        agent9527_home.path().to_path_buf(),
    );
    let session = Arc::new(session);
    ...
    agent9527_login::login_with_api_key(
        agent9527_home.path(),
        "new-api-key",
        agent9527_login::AuthCredentialsStoreMode::File,
        agent9527_login::AuthKeyringBackendKind::default(),
    )
...
impl agent9527_exec_server::NoiseRendezvousConnectProvider for PendingNoiseConnectProvider {
    fn connect_bundle(
        &self,
        _: agent9527_exec_server::NoiseChannelPublicKey,
    ) -> futures::future::BoxFuture<
        '_,
        Result<
            agent9527_exec_server::NoiseRendezvousConnectBundle,
            agent9527_exec_server::ExecServerError,
        >,
    > {
...
async fn deferred_environment_roots_refresh_plugin_availability() {
    ...
    impl agent9527_extension_api::McpServerContributor<Config> for ReadyPluginContributor {
        ...
        fn contribute<'a>(
            &'a self,
            context: agent9527_extension_api::McpServerContributionContext<'a, Config>,
        ) -> agent9527_extension_api::ExtensionFuture<
            'a,
            Vec<agent9527_extension_api::McpServerContribution>,
        > {
            Box::pin(async move {
                ...
                available
                    .then(|| {
                        agent9527_extension_api::McpServerContribution::SelectedPluginPackage {
    ...
    let (mut session, turn_context) = make_session_and_context().await;
    let mut registry = agent9527_extension_api::ExtensionRegistryBuilder::new();
    ...
    session.services.mcp_manager = Arc::new(McpManager::new_with_extensions(
        ...
        registry,
        crate::Agent9527AppsToolsCache::default(),
    ));
    let session = Arc::new(session);
    session
    ...
    let old_binding = mcp_runtime
        .current_binding()
        .await
        .expect("initial MCP binding");

    let selected_root = agent9527_protocol::capabilities::SelectedCapabilityRoot {
        id: "skill-only".to_string(),
        location: agent9527_protocol::capabilities::CapabilityRootLocation::Environment {
    ...
    registration
        .complete(Ok(agent9527_exec_server::EnvironmentReadyInfo {
    ...
    assert!(Arc::ptr_eq(&mcp_runtime, &session.services.mcp_runtime));
}
...
async fn conflicting_ready_environment_root_ids_keep_first_location() {
    ...
    let selected_root = |environment_id: &str, path: &str| {
        agent9527_protocol::capabilities::SelectedCapabilityRoot {
    ...
    for selected_root in &selected_roots {
        let agent9527_protocol::capabilities::CapabilityRootLocation::Environment {
            environment_id,
            ..
...
async fn step_context_keeps_its_mcp_runtime_for_tools() -> anyhow::Result<()> {
    ...
    let next_step = session
        ...
        .await
        .expect("a fresh cancellation token cannot be cancelled");
    assert!(agent9527_mcp::configured_mcp_servers(next_step.mcp.config()).contains_key("newer"));
    ...
    let current = session
        ...
        .await
        .expect("refreshed runtime should be available");
    assert!(agent9527_mcp::configured_mcp_servers(current.config()).contains_key("newer"));
...
impl agent9527_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a agent9527_extension_api::ExtensionData,
        thread_store: &'a agent9527_extension_api::ExtensionData,
    ) -> std::pin::Pin<
        Box<
            dyn std::future::Future<Output = Vec<agent9527_extension_api::PromptFragment>>
                + Send
                + 'a,
        >,
    > {
        Box::pin(async move {
            thread_store
                ...
                .is_some()
                .then(|| {
                    agent9527_extension_api::PromptFragment::developer_policy(
...
-> Arc<agent9527_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = agent9527_extension_api::ExtensionRegistryBuilder::new();
    builder.prompt_contributor(Arc::new(PromptExtensionTestContributor));
    Arc::new(builder.build())
}

impl agent9527_extension_api::ContextContributor for TurnContextExtensionTestContributor {
    fn contribute_turn_context<'a>(
        &'a self,
        input: agent9527_extension_api::TurnContextContributionInput<'a>,
    ) -> std::pin::Pin<
        Box<
            dyn std::future::Future<Output = Vec<agent9527_extension_api::PromptFragment>>
                + Send
...
async fn attach_in_memory_thread_store(
    session: &mut Session,
) -> Arc<agent9527_thread_store::InMemoryThreadStore> {
    let store = Arc::new(agent9527_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn agent9527_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
...
async fn wait_for_flush_count(
    store: &agent9527_thread_store::InMemoryThreadStore,
...
async fn guardian_auto_review_emits_thread_idle_after_interrupt() {
        ...
        for ThreadIdleRecorder
    {
        fn on_thread_idle<'a>(
            &'a self,
            _input: agent9527_extension_api::ThreadIdleInput<'a>,
        ) -> agent9527_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = agent9527_protocol::user_input::TextElement::new(
        agent9527_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn task_finish_emits_thread_idle_lifecycle_after_active_turn_clears() {
        ...
        for ThreadIdleRecorder
    {
        fn on_thread_idle<'a>(
            &'a self,
            input: agent9527_extension_api::ThreadIdleInput<'a>,
        ) -> agent9527_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(agent9527_extension_api::ExtensionData::new(
            tc.sub_id.clone(),
        )),
...
async fn rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    use crate::turn_diff_tracker::TurnDiffTracker;
    use agent9527_protocol::protocol::AskForApproval;
    use agent9527_tools::ShellCommandBackendConfig;
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            call_id,
            tool_name: agent9527_tools::ToolName::plain(tool_name),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: agent9527_tools::ToolName::plain("exec_command"),
            source: crate::tools::context::ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let agent9527_home = temp.path().join("home");
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let root_dot_agent9527 = project_root.join(".agent9527");
    let nested_dot_agent9527 = nested.join(".agent9527");

    std::fs::create_dir_all(&agent9527_home)?;
    std::fs::create_dir_all(&nested_dot_agent9527)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&root_dot_agent9527)?;
    write_project_hooks(&nested_dot_agent9527)?;
    write_project_trust_config(&agent9527_home, &[(&nested, TrustLevel::Trusted)]).await?;

    let config = ConfigBuilder::default()
        .agent9527_home(agent9527_home)
    ...
    let expected_source_path = agent9527_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        nested_dot_agent9527.join("hooks.json"),
    )?;
    assert_eq!(
        hook_list
            .hooks
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_agent9527 = project_root.join(".agent9527");
    std::fs::create_dir_all(&nested)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&dot_agent9527)?;
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let agent9527_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&agent9527_home)?;
        write_project_trust_config(&agent9527_home, &trust_entries).await?;

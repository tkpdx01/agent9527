...
use agent9527_analytics::CompactionPhase;
use agent9527_analytics::CompactionReason;
use agent9527_analytics::InvocationType;
use agent9527_analytics::TurnResolvedConfigFact;
...
use agent9527_core_skills::injection::InjectedHostSkillPrompts;
use agent9527_extension_api::ExtensionData;
use agent9527_extension_api::TurnInputContext;
use agent9527_extension_api::TurnInputEnvironment;
use agent9527_features::Feature;
use agent9527_file_system::FindUpErrorPolicy;
use agent9527_file_system::find_nearest_ancestor_with_markers;
use agent9527_mcp::ToolInfo;
...
use agent9527_protocol::items::PlanItem;
...
use agent9527_protocol::protocol::AgentMessageContentDeltaEvent;
use agent9527_protocol::protocol::AgentReasoningSectionBreakEvent;
use agent9527_protocol::protocol::ErrorEvent;
use agent9527_protocol::protocol::EventMsg;
use agent9527_protocol::protocol::PlanDeltaEvent;
use agent9527_protocol::protocol::RawResponseCompletedEvent;
use agent9527_protocol::protocol::ReasoningContentDeltaEvent;
use agent9527_protocol::protocol::ReasoningRawContentDeltaEvent;
use agent9527_protocol::protocol::SafetyBufferingEvent;
...
use agent9527_tools::ToolName;
use agent9527_tools::filter_request_plugin_install_discoverable_tools_for_client;
use agent9527_utils_path_uri::PathUri;
use agent9527_utils_stream_parser::AssistantTextChunk;
use agent9527_utils_stream_parser::AssistantTextStreamParser;
use agent9527_utils_stream_parser::ProposedPlanSegment;
use agent9527_utils_stream_parser::extract_proposed_plan_text;
use agent9527_utils_stream_parser::strip_citations;
...
const POST_SAMPLING_TOKEN_ESTIMATE_TARGET: &str = "agent9527_core::post_sampling_token_estimate";
...
pub(crate) async fn run_turn(
    ...
    turn_extension_data: Arc<agent9527_extension_api::ExtensionData>,
    input: Vec<TurnInput>,
    prewarmed_client_session: Option<ModelClientSession>,
    cancellation_token: CancellationToken,
) -> Agent9527Result<Option<String>> {
    ...
    .await
    {
        if matches!(err.details(), Agent9527ErrorDetails::TurnAborted) {
            run_hooks_and_record_inputs(&sess, &turn_context, &input).await;
            return Err(err);
        }
        let error = err.to_agent9527_protocol_error();
        ...
        .await
    {
        ...
        Err(err) if matches!(err.details(), Agent9527ErrorDetails::TurnAborted) => {
            run_hooks_and_record_inputs(&sess, &turn_context, &input).await;
            return Err(err);
        }
    ...
    // Although from the perspective of agent9527.rs, TurnDiffTracker has the lifecycle of a Task which contains
    ...
    loop {
        ...
        let sampling_request_result: Agent9527Result<_> = async {
            ...
            let responses_metadata = turn_context.turn_metadata_state.to_responses_metadata(
                ...
                window_id,
                Agent9527ResponsesRequestKind::Turn,
            );
        ...
        match sampling_request_result {
            ...
            Err(agent9527_error)
                if matches!(
                    agent9527_error.details(),
                    Agent9527ErrorDetails::InvalidImageRequest()
                ) =>
            {
                sess.track_turn_agent9527_error(turn_context.as_ref(), &agent9527_error);
                let error = Agent9527ErrorInfo::BadRequest;
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                let event = EventMsg::Error(ErrorEvent {
                    message: "Invalid image in your last message. Please remove it and try again."
                        .to_string(),
                    agent9527_error_info: Some(error),
                });
            ...
            Err(e) => {
                info!("Turn error: {e:#}");
                let error = e.to_agent9527_protocol_error();
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                sess.track_turn_agent9527_error(turn_context.as_ref(), &e);
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = agent9527_connectors::merge::merge_plugin_connectors_with_accessible(
            connector_snapshot
                .connector_ids()
                .iter()
...
///
/// Returns `None` when the active authentication does not use the Agent9527 backend, the provider is
...
async fn capture_current_model_fallback_step_context(
    ...
    previous_model: &str,
    cancellation_token: &CancellationToken,
) -> Agent9527Result<Option<Arc<StepContext>>> {
    let uses_agent9527_backend = turn_context
        .auth_manager
        .as_deref()
        .is_some_and(agent9527_login::AuthManager::current_auth_uses_agent9527_backend);
    if !uses_agent9527_backend
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = agent9527_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    step_context: Arc<StepContext>,
    turn_store: Arc<agent9527_extension_api::ExtensionData>,
    turn_diff_tracker: SharedTurnDiffTracker,
    client_session: &mut ModelClientSession,
    responses_metadata: &Agent9527ResponsesMetadata,
    input: Vec<ResponseItem>,
    cancellation_token: CancellationToken,
) -> Agent9527Result<(SamplingRequestResult, Vec<ResponseItem>)> {
    ...
    loop {
        ...
        .await
        {
            ...
            Err(err) => match err.details() {
                ...
                Agent9527ErrorDetails::UsageLimitReached(e) => {
...
pub(super) fn agent_message_text(item: &agent9527_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            agent9527_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: agent9527_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(agent9527_protocol::items::AgentMessageItem {
...
async fn handle_assistant_item_done_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    turn_store: &agent9527_extension_api::ExtensionData,
    item: &ResponseItem,
    state: &mut PlanModeStreamState,
    previously_active_item: Option<&TurnItem>,
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, Agent9527Result<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> Agent9527Result<()> {
...
async fn try_run_sampling_request(
    ...
    prompt: &Prompt,
    cancellation_token: CancellationToken,
) -> Agent9527Result<SamplingRequestResult> {
    feedback_tags!(
        model = turn_context.model_info.slug.clone(),
        approval_policy = turn_context.approval_policy.value(),
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let mut in_flight: FuturesOrdered<BoxFuture<'static, Agent9527Result<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: Agent9527Result<SamplingRequestResult> = loop {
        let handle_responses = trace_span!(
            ...
            from = field::Empty,
            agent9527.request.reasoning_effort = %reasoning_effort,
            gen_ai.usage.input_tokens = field::Empty,
            gen_ai.usage.cache_read.input_tokens = field::Empty,
            gen_ai.usage.cache_write.input_tokens = field::Empty,
            gen_ai.usage.output_tokens = field::Empty,
            agent9527.usage.reasoning_output_tokens = field::Empty,
            agent9527.usage.total_tokens = field::Empty,
        );
        ...
        {
            Ok(event) => event,
            Err(agent9527_async_utils::CancelErr::Cancelled) => {
                break Err(Agent9527Err::TurnAborted);
            }
        };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![agent9527_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
    ...
    if cancellation_token.is_cancelled() {
        return Err(Agent9527Err::TurnAborted);
    }

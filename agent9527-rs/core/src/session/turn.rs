...
use agent9527_git_utils::get_git_repo_root_with_fs;
...
pub(crate) async fn run_turn(
    ...
    if let Err(err) = run_pre_sampling_compact(&sess, &turn_context, &mut client_session).await {
        if matches!(err, Agent9527Err::TurnAborted) {
            return Err(err);
        }
    ...
    loop {
        ...
        match sampling_request_result {
            ...
            Err(err @ Agent9527Err::TurnAborted) => {
                return Err(err);
            }
            Err(agent9527_error @ Agent9527Err::InvalidImageRequest()) => {
                {
...
async fn run_sampling_request(
    ...
    loop {
        ...
        .await
        {
            ...
            Err(Agent9527Err::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(Agent9527Err::ContextWindowExceeded);
            }
            Err(Agent9527Err::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, *rate_limits).await;
                }
                return Err(Agent9527Err::UsageLimitReached(e));
            }
            Err(err) => err,
        };
...
pub(crate) async fn built_tools(
    ...
    step_context: &StepContext,
    cancellation_token: &CancellationToken,
) -> Agent9527Result<Arc<ToolRouter>> {
    let turn_context = step_context.turn.as_ref();

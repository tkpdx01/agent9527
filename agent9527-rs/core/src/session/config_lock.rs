...
use agent9527_config::config_toml::OrchestratorFeatureToml;
use agent9527_config::config_toml::OrchestratorToml;
...
use agent9527_features::RolloutBudgetConfigToml;
...
pub(crate) async fn validate_config_lock_if_configured(
    ...
    let options = ConfigLockReplayOptions {
        allow_agent9527_version_mismatch: config.config_lock_allow_agent9527_version_mismatch,
    };
...
/// Agent9527 actually ran with, not only the user-authored TOML inputs.
...
mod tests {
    ...
    async fn lock_contains_prompts_and_materializes_features() {
        ...
        for spec in agent9527_features::FEATURES {
            assert_eq!(
        ...
        assert_eq!(
            ...
            Some(FeatureToml::Config(CurrentTimeReminderConfigToml {
                ...
                clock_source: Some(agent9527_features::CurrentTimeSource::System),
                delivery_mode: Some(
                    agent9527_features::CurrentTimeReminderDeliveryMode::AnyInference
                ),
    ...
    async fn lock_contains_exact_managed_requirements() {
        ...
        let sqlite_home = agent9527_home.path().join("managed-state");
        let log_dir = agent9527_home.path().join("managed-logs");
        let catalog_path = agent9527_home.path().join("managed-models.json");
...
"#,
    ...
    async fn lock_drops_unmanaged_model_catalog_input() {
        let agent9527_home = tempfile::tempdir().expect("create temp dir");
        let catalog_path = agent9527_home.path().join("user-models.json");
        write_model_catalog(&catalog_path);
    ...
    #[tokio::test]
    async fn lock_validation_rejects_agent9527_version_mismatch_by_default() {
        let sc = crate::session::tests::make_session_configuration_for_tests().await;
        let mut expected = sc.to_config_lockfile_toml().expect("lock should serialize");
        expected.agent9527_version = "older-version".to_string();
        let actual = sc.to_config_lockfile_toml().expect("lock should serialize");
        ...
        let message = error.to_string();
        assert!(
            message.contains("config lock Agent9527 version mismatch"),
            "{message}"
        );
        assert!(
            message.contains("debug.config_lockfile.allow_agent9527_version_mismatch=true"),
            "{message}"
        );
    }

    #[tokio::test]
    async fn lock_validation_can_ignore_agent9527_version_mismatch() {
        let sc = crate::session::tests::make_session_configuration_for_tests().await;
        let mut expected = sc.to_config_lockfile_toml().expect("lock should serialize");
        expected.agent9527_version = "older-version".to_string();
        let actual = sc.to_config_lockfile_toml().expect("lock should serialize");

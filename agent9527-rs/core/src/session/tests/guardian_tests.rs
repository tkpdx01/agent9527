...
use agent9527_protocol::models::AdditionalPermissionProfile as PermissionProfile;
...
use core_test_support::agent9527_linux_sandbox_exe_or_skip;
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                agent9527_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_shell_command_additional_permissions_requests_past_policy_validation() {
    ...
    turn_context_raw.permission_profile = agent9527_protocol::models::PermissionProfile::Disabled;
    let mut config = (*turn_context_raw.config).clone();
    config
        .permissions
        .set_permission_profile(agent9527_protocol::models::PermissionProfile::Disabled)
        .expect("test setup should allow disabling the permission profile");
    config.agent9527_linux_sandbox_exe = agent9527_linux_sandbox_exe_or_skip!();
    config
        .features
        .enable(Feature::GuardianApproval)
        .expect("test setup should allow enabling guardian approvals");
    ...
    let handler = crate::tools::handlers::ShellCommandHandler::from(
        agent9527_tools::ShellCommandBackendConfig::Classic,
    );
    #[allow(deprecated)]
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: agent9527_tools::ToolName::plain("shell_command"),
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    let agent9527_home = tempdir().expect("create agent9527 home");
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_agent9527_folder: project_dir.path().abs(),
            },
    ...
    let thread_store = Arc::new(agent9527_thread_store::LocalThreadStore::new(
        agent9527_thread_store::LocalThreadStoreConfig::from_config(&config),
        /*state_db*/ None,
    ));
    ...
    let (session, io) = Session::spawn(SessionSpawnArgs {
        ...
        plugins_manager,
        mcp_manager,
        code_mode_session_provider: Arc::new(agent9527_code_mode::InProcessCodeModeSessionProvider),
        extensions: agent9527_extension_api::empty_extension_registry(),
        ...
        parent_rollout_thread_trace: agent9527_rollout_trace::ThreadTraceContext::disabled(),
        user_shell_override: None,
        parent_trace: None,
        environment_selections: Vec::new(),
        thread_extension_init: agent9527_extension_api::ExtensionDataInit::default(),
        ...
        windows_sandbox_proxy_settings_mode:
            agent9527_sandboxing::WindowsSandboxProxySettingsMode::Preserve,
    })
    .await

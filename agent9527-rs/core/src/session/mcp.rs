...
use super::*;
use agent9527_exec_server::ExecutorCapabilityDiscoveryCache;
use agent9527_exec_server::ExecutorCapabilityDiscoverySnapshot;
use agent9527_exec_server::MAX_SELECTED_CAPABILITY_ROOTS;
use agent9527_exec_server::ResolvedSelectedCapabilityRoot;
use agent9527_mcp::AGENT9527_APPS_MCP_SERVER_NAME;
use agent9527_mcp::ElicitationReviewRequest;
use agent9527_mcp::ElicitationReviewer;
use agent9527_mcp::ElicitationReviewerHandle;
use agent9527_protocol::capabilities::SelectedCapabilityRoot;
use agent9527_protocol::config_types::ApprovalsReviewer;
use agent9527_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_ELICITATION_APPROVAL_KIND_KEY;
use agent9527_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_ELICITATION_APPROVAL_KIND_MCP_TOOL_CALL;
use agent9527_protocol::mcp_approval_meta::APPROVAL_KIND_TOOL_SUGGESTION as MCP_ELICITATION_APPROVAL_KIND_TOOL_SUGGESTION;
use agent9527_protocol::mcp_approval_meta::APPROVALS_REVIEWER_KEY as MCP_ELICITATION_APPROVALS_REVIEWER_KEY;
use agent9527_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_ELICITATION_CONNECTOR_DESCRIPTION_KEY;
use agent9527_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_ELICITATION_CONNECTOR_ID_KEY;
use agent9527_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_ELICITATION_CONNECTOR_NAME_KEY;
use agent9527_protocol::mcp_approval_meta::REQUEST_TYPE_APPROVAL_REQUEST as MCP_ELICITATION_REQUEST_TYPE_APPROVAL_REQUEST;
use agent9527_protocol::mcp_approval_meta::REQUEST_TYPE_KEY as MCP_ELICITATION_REQUEST_TYPE_KEY;
use agent9527_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_ELICITATION_TOOL_DESCRIPTION_KEY;
use agent9527_protocol::mcp_approval_meta::TOOL_NAME_KEY as MCP_ELICITATION_TOOL_NAME_KEY;
use agent9527_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_ELICITATION_TOOL_PARAMS_KEY;
use agent9527_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_ELICITATION_TOOL_TITLE_KEY;
use agent9527_rmcp_client::Elicitation;
...
impl Session {
    ...
    pub(crate) async fn runtime_mcp_servers(
        ...
        agent9527_mcp::configured_mcp_servers(&self.runtime_mcp_config(config).await)
    }
    ...
    pub(crate) async fn refresh_mcp_if_dirty(self: &Arc<Self>) {
        ...
        loop {
            ...
            if self
                .services
                .plugins_manager
                .set_auth_mode(auth.as_ref().map(Agent9527Auth::api_auth_mode))
                || !self
                    .services
                    .mcp_runtime
    ...
    pub(crate) async fn hard_refresh_latest_agent9527_apps_tools(
        self: &Arc<Self>,
    ) -> anyhow::Result<Vec<agent9527_mcp::ToolInfo>> {
        ...
        anyhow::ensure!(
            input
                .mcp_servers
                .contains_key(AGENT9527_APPS_MCP_SERVER_NAME),
            "unknown MCP server '{AGENT9527_APPS_MCP_SERVER_NAME}'"
        );
    ...
    pub(crate) async fn mcp_runtime_for_step(
        ...
        {
            self.mark_mcp_runtime_dirty();
            if let Ok(_refresh) = self.mcp_refresh.acquire().await
                && self
                    .services
                    .mcp_runtime
                    .current_ready_selected_capability_roots()
        ...
        Arc::new(agent9527_mcp::McpBinding::empty(config))
    }
    ...
    pub(crate) fn mcp_elicitation_lifecycle(&self) -> agent9527_mcp::ElicitationLifecycle {
        self.mcp_elicitation_lifecycle_handle
            .get_or_init(|| {
                let elicitations = self.services.elicitations.clone();
                agent9527_mcp::ElicitationLifecycle::new(move || elicitations.register())
            })
            .clone()
    }
    ...
    pub async fn request_mcp_server_elicitation(
        ...
        if self.services.mcp_runtime.elicitations_auto_deny() {
            return McpServerElicitationOutcome {
                response: Some(ElicitationResponse {
                    action: agent9527_rmcp_client::ElicitationAction::Accept,
        ...
        let id = match request_id {
            rmcp::model::NumberOrString::String(value) => {
                agent9527_protocol::mcp::RequestId::String(value.to_string())
            }
            rmcp::model::NumberOrString::Number(value) => {
                agent9527_protocol::mcp::RequestId::Integer(value)
            }
        };
...
async fn review_guardian_mcp_elicitation(
    ...
    match approval_policy {
        AskForApproval::Never => {
            if agent9527_mcp::mcp_permission_prompt_is_auto_approved(
                approval_policy,
                &mcp_config.permission_profile,
                agent9527_mcp::McpPermissionPromptAutoApproveContext::default(),
            ) && matches!(
                &request.elicitation,
                Elicitation::Mcp(
...
fn plugin_install_elicitation_telemetry_metadata(
    ...
    let agent9527_protocol::approvals::ElicitationRequest::Form {

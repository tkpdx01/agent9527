...
pub(crate) struct Session {
    ...
    pub(super) mcp_elicitation_reviewer_handle: OnceLock<agent9527_mcp::ElicitationReviewerHandle>,
    pub(super) mcp_elicitation_lifecycle_handle: OnceLock<agent9527_mcp::ElicitationLifecycle>,
...
pub(crate) struct SessionConfiguration {
    ...
    /// Directory containing all Agent9527 state for this session.
    pub(super) agent9527_home: AbsolutePathBuf,
...
impl SessionConfiguration {
    ...
    pub(crate) fn agent9527_home(&self) -> &AbsolutePathBuf {
        &self.agent9527_home
    }
    ...
    pub(super) fn sandbox_policy(&self) -> SandboxPolicy {
        ...
        agent9527_sandboxing::compatibility_sandbox_policy_for_permission_profile(
            &permission_profile,
            self.cwd(),
        )
    }
            ...
            if let Some(active_permission_profile) = next_configuration.active_permission_profile()
            {
                ...
                config.permissions.network = config
                    ...
                    .map_err(|err| ConstraintError::InvalidValue {
                        ...
                        requirement_source: agent9527_config::RequirementSource::Unknown,
                    })?;
                config
                    .permissions
...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        config: Arc<Config>,
        user_instructions: Option<agent9527_extension_api::UserInstructions>,
        ...
        code_mode_session_provider: Arc<dyn agent9527_code_mode::CodeModeSessionProvider>,
        extensions: Arc<agent9527_extension_api::ExtensionRegistry<crate::config::Config>>,
        ...
        windows_sandbox_proxy_settings_mode: agent9527_sandboxing::WindowsSandboxProxySettingsMode,
        ...
        thread_extension_init.insert(agent9527_extension_api::ThreadOriginator(
            session_configuration.originator.clone(),
        ));
        let mcp_thread_init = thread_extension_init.clone();
        let thread_extension_data = agent9527_extension_api::ExtensionData::new_with_init(
            thread_id.to_string(),
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            let trace_agent_path = session_configuration
                .session_source
                .get_agent_path()
                .unwrap_or_else(agent9527_protocol::AgentPath::root);
            let trace_task_name =
            ...
            let config_path = config.agent9527_home.join(CONFIG_TOML_FILE);
            ...
            let auth_mode = telemetry_auth
                .map(Agent9527Auth::auth_mode)
                .map(TelemetryAuthMode::from);
            let account_id = telemetry_auth.and_then(Agent9527Auth::get_account_id);
            let account_email = telemetry_auth.and_then(Agent9527Auth::get_account_email);
            let originator = session_configuration.originator.clone();
            let terminal_type = user_agent();
            let session_model = session_configuration.collaboration_mode.model().to_string();
            let auth_env_telemetry = collect_auth_env_telemetry(
                &session_configuration.provider,
                auth_manager.agent9527_api_key_env_enabled(),
            );
            ...
            let mcp_server_names =
                agent9527_mcp::effective_mcp_servers(
            ...
            let session_extension_data =
                agent9527_extension_api::ExtensionData::new(session_id.to_string());
            let mcp_resource_client = Arc::new(McpResourceClient::new(Arc::clone(&mcp_runtime)));
            for contributor in extensions.thread_lifecycle_contributors() {
                contributor.on_thread_start(agent9527_extension_api::ThreadStartInput {
                ...
                || mcp_auth_changes.has_changed().unwrap_or(false)
            {
                sess.services
                    .plugins_manager
                    .set_auth_mode(latest_auth.as_ref().map(Agent9527Auth::api_auth_mode));
                sess.services
                    .mcp_manager
                    .runtime_config_for_step(
                        config.as_ref(),
            ...
            let session_start_source = match &initial_history {
                InitialHistory::Resumed(_) => agent9527_hooks::SessionStartSource::Resume,
                InitialHistory::New | InitialHistory::Forked(_) => {
                    agent9527_hooks::SessionStartSource::Startup
                }
                InitialHistory::Cleared => agent9527_hooks::SessionStartSource::Clear,
            };

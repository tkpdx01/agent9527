...
use agent9527_config::ConfigEditsBuilder;
use agent9527_config::McpServerConfig;
use agent9527_config::McpServerTransportConfig;
use agent9527_config::load_global_mcp_servers;
use agent9527_login::default_client::is_first_party_originator;
...
pub(crate) async fn maybe_prompt_and_install_mcp_dependencies(
    ...
    if mentioned_skills.is_empty()
        || !config
            .features
            .enabled(agent9527_features::Feature::SkillMcpDependencyInstall)
    {
        return;
    }
...
pub(crate) async fn maybe_install_mcp_dependencies(
    ...
    let mut servers = match load_global_mcp_servers(&agent9527_home).await {
        Ok(servers) => servers,
        Err(err) => {
    ...
    if let Err(err) = ConfigEditsBuilder::new(&agent9527_home)

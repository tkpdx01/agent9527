...
use agent9527_config::types::OtelExporterKind as Kind;
use agent9527_config::types::OtelHttpProtocol as Protocol;
use agent9527_features::Feature;
use agent9527_login::default_client::originator;
use agent9527_otel::OtelExporter;
use agent9527_otel::OtelHttpProtocol;
use agent9527_otel::OtelProvider;
use agent9527_otel::OtelSettings;
use agent9527_otel::OtelTlsConfig as OtelTlsSettings;
use std::error::Error;
...
/// Filter predicate for exporting only Agent9527-owned events via OTEL.
/// Keeps events that originated from agent9527_otel module
pub fn agent9527_export_filter(meta: &tracing::Metadata<'_>) -> bool {
    meta.target().starts_with("agent9527_otel")
}
...
pub fn record_process_start(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let _ = agent9527_otel::record_process_start_once(metrics, originator);
}
...
pub fn install_sqlite_telemetry(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let telemetry = agent9527_rollout::sqlite_telemetry_recorder(metrics.clone(), originator);

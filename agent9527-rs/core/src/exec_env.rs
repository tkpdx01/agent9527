...
pub use agent9527_protocol::shell_environment::AGENT9527_THREAD_ID_ENV_VAR;
...
pub const AGENT9527_PERMISSION_PROFILE_ENV_VAR: &str = "AGENT9527_PERMISSION_PROFILE";
...
///
/// `AGENT9527_THREAD_ID` is injected when a thread id is provided, even when
/// `include_only` is set.
pub fn create_env(
...
pub(crate) fn inject_permission_profile_env(
    ...
    if cfg!(windows) {
        env.retain(|key, _| !key.eq_ignore_ascii_case(AGENT9527_PERMISSION_PROFILE_ENV_VAR));
    } else {
        env.remove(AGENT9527_PERMISSION_PROFILE_ENV_VAR);
    }

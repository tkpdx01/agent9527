...
use agent9527_analytics::CompactionStrategy;
use agent9527_analytics::CompactionTrigger;
...
use agent9527_utils_string::to_ascii_json_string;
use http::HeaderMap as ApiHeaderMap;
use http::HeaderValue;
use serde::Serialize;
use serde_json::Value;

use crate::client::X_AGENT9527_INSTALLATION_ID_HEADER;
use crate::client::X_AGENT9527_PARENT_THREAD_ID_HEADER;
use crate::client::X_AGENT9527_TURN_METADATA_HEADER;
use crate::client::X_AGENT9527_WINDOW_ID_HEADER;
...
const RESERVED_METADATA_KEYS: &[&str] = &[
    ...
    X_AGENT9527_TURN_METADATA_HEADER,
...
pub(crate) enum Agent9527ResponsesRequestKind {
    Turn,
    Prewarm,
...
impl Agent9527ResponsesRequestKind {
    fn metadata(self) -> (&'static str, Option<CompactionTurnMetadata>) {
        match self {
            Agent9527ResponsesRequestKind::Turn => ("turn", None),
            Agent9527ResponsesRequestKind::Prewarm => ("prewarm", None),
            Agent9527ResponsesRequestKind::Compaction(metadata) => ("compaction", Some(metadata)),
            Agent9527ResponsesRequestKind::Memory => ("memory", None),
        }
    }

    fn has_turn_identity(self) -> bool {
        !matches!(self, Agent9527ResponsesRequestKind::Memory)
    }
}
...
/// Caller-owned snapshot of Agent9527 metadata sent to ResponsesAPI.
///
/// The full Agent9527 turn metadata blob is transported canonically as
/// `client_metadata["x-agent9527-turn-metadata"]`. Flat `client_metadata` keys and direct HTTP/ws
/// headers are generated compatibility projections of this snapshot, not separate sources of
/// truth.
#[derive(Clone, Debug)]
pub struct Agent9527ResponsesMetadata {
    pub(crate) installation_id: String,
    pub(crate) session_id: String,
    pub(crate) thread_id: String,
    pub(crate) turn_id: Option<String>,
    pub(crate) window_id: String,
    pub(crate) request_kind: Option<Agent9527ResponsesRequestKind>,
...
impl Agent9527ResponsesMetadata {
    pub(crate) fn new(
    ...
    pub(crate) fn client_metadata(&self) -> HashMap<String, String> {
        ...
        if let Some(parent_thread_id) = self.parent_thread_id {
            client_metadata.insert(
                X_AGENT9527_PARENT_THREAD_ID_HEADER.to_string(),
        ...
        {
            client_metadata.insert(
                X_AGENT9527_TURN_METADATA_HEADER.to_string(),
                turn_metadata_json,
            );
        }
        client_metadata
    ...
    pub(crate) fn compatibility_headers(&self) -> ApiHeaderMap {
        ...
        insert_header(&mut headers, X_AGENT9527_WINDOW_ID_HEADER, &self.window_id);
        // Direct x-agent9527-turn-metadata is compatibility output. Keep the unbounded Code Mode
        // mapping in client_metadata only so HTTP and WebSocket headers remain bounded.
        if self.has_turn_metadata()
            && let Ok(turn_metadata_json) = to_ascii_json_string(&Agent9527TurnMetadataPayload {
                code_mode_tool_names: None,
    ...
    fn turn_metadata_payload(&self) -> Agent9527TurnMetadataPayload<'_> {
        ...
        let has_turn_identity =
            request_kind.is_none_or(Agent9527ResponsesRequestKind::has_turn_identity);
        let has_request_identity =
            request_kind.is_some_and(Agent9527ResponsesRequestKind::has_turn_identity);
        Agent9527TurnMetadataPayload {
            ...
            compaction,
            // responsesapi_client_metadata enriches the Agent9527 turn metadata blob, not literal
            // top-level Responses client_metadata. Reserved Agent9527-owned keys are filtered when
...
#[derive(Serialize)]
struct Agent9527TurnMetadataPayload<'a> {
    #[serde(default, skip_serializing_if = "Option::is_none")]

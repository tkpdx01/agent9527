...
impl Agent9527ResponsesMetadata {
    ...
    pub(crate) fn compatibility_headers(&self) -> ApiHeaderMap {
        ...
        // Direct x-agent9527-turn-metadata is compatibility output. New per-request consumers should
        // prefer client_metadata["x-agent9527-turn-metadata"], which is rendered from this same object.
        if self.has_turn_metadata()
            && let Some(turn_metadata_json) = self.turn_metadata_json()
        {
            insert_header(

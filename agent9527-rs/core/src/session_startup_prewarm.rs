...
use agent9527_otel::STARTUP_PREWARM_AGE_AT_FIRST_TURN_METRIC;
use agent9527_otel::STARTUP_PREWARM_DURATION_METRIC;
...
pub(crate) struct SessionStartupPrewarmHandle {
    task: AbortOnDropHandle<Agent9527Result<ModelClientSession>>,
    started_at: Instant,
    timeout: Duration,
}
...
impl SessionStartupPrewarmHandle {
    pub(crate) fn new(
        task: JoinHandle<Agent9527Result<ModelClientSession>>,
        started_at: Instant,
        timeout: Duration,
    ) -> Self {
        Self {
    ...
    fn resolution_from_join_result(
        result: std::result::Result<Agent9527Result<ModelClientSession>, tokio::task::JoinError>,
        started_at: Instant,
    ) -> SessionStartupPrewarmResolution {
        match result {
...
async fn schedule_startup_prewarm_inner(
...
) -> Agent9527Result<ModelClientSession> {
    ...
    let responses_metadata = startup_turn_context
        ...
        .to_responses_metadata(
            ...
            window_id,
            Agent9527ResponsesRequestKind::Prewarm,
        );

...
use agent9527_api::RealtimeAudioFrame;
use agent9527_api::RealtimeContextAppendChannel;
use agent9527_api::RealtimeEvent;
use agent9527_api::RealtimeEventParser;
use agent9527_api::RealtimeSessionConfig;
use agent9527_api::RealtimeSessionMode;
use agent9527_api::RealtimeWebsocketClient;
use agent9527_api::RealtimeWebsocketEvents;
use agent9527_api::RealtimeWebsocketWriter;
...
use agent9527_login::read_openai_api_key_from_env;
...
use agent9527_protocol::protocol::Agent9527ResponseHandoffMode;
...
use agent9527_protocol::protocol::RealtimeConversationClosedEvent;
use agent9527_protocol::protocol::RealtimeConversationRealtimeEvent;
use agent9527_protocol::protocol::RealtimeConversationSdpEvent;
use agent9527_protocol::protocol::RealtimeConversationStartedEvent;
use agent9527_protocol::protocol::RealtimeHandoffRequested;
use agent9527_protocol::protocol::RealtimeOutputModality;
use agent9527_protocol::protocol::RealtimeTranscriptEntry;
use agent9527_protocol::protocol::RealtimeVoice;
use agent9527_protocol::protocol::RealtimeVoicesList;
use agent9527_utils_output_truncation::approx_bytes_for_tokens;
use agent9527_utils_string::approx_token_count;
use agent9527_utils_string::take_bytes_at_char_boundary;
use anyhow::Context;
...
const STANDALONE_HANDOFF_ID: &str = "agent9527";
...
struct RealtimeHandoffState {
    ...
    agent9527_responses_as_items: bool,
    agent9527_response_item_prefix: Option<String>,
    agent9527_response_handoff_mode: Agent9527ResponseHandoffMode,
    agent9527_response_handoff_channel_prefixes: Arc<BTreeMap<String, Vec<String>>>,
...
impl RealtimeHandoffState {
    fn streams_handoff_append(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            ...
            && !self.agent9527_responses_as_items
    }

    fn routes_handoff_by_bem(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            && self.agent9527_response_handoff_mode == Agent9527ResponseHandoffMode::BemTags
    }
}
...
struct RealtimeStart {
    ...
    agent9527_response_item_prefix: Option<String>,
    agent9527_response_handoff_mode: Agent9527ResponseHandoffMode,
    agent9527_response_handoff_channel_prefixes: Option<BTreeMap<String, Vec<String>>>,
...
impl RealtimeConversationManager {
    ...
    async fn start(&self, start: RealtimeStart) -> Agent9527Result<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> Agent9527Result<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
            extra_headers,
            client_managed_handoffs,
            flush_transcript_tail_on_session_end,
            agent9527_responses_as_items,
            agent9527_response_item_prefix,
            agent9527_response_handoff_mode,
            agent9527_response_handoff_channel_prefixes,
        ...
        let handoff = RealtimeHandoffState {
            ...
            agent9527_response_item_prefix,
            agent9527_response_handoff_mode,
            agent9527_response_handoff_channel_prefixes: Arc::new(
                agent9527_response_handoff_channel_prefixes.unwrap_or_default(),
            ),
            session_kind,
            event_parser,
        };
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> Agent9527Result<()> {
        let sender = {
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(Agent9527Err::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, mut params: ConversationTextParams) -> Agent9527Result<()> {
        let sender = {
            let guard = self.state.lock().await;
            guard
                .as_ref()
        ...
        sender
            .send(params)
            .await
            .map_err(|_| Agent9527Err::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }
    ...
    pub(crate) async fn handoff_out(
        ...
        let phase = if handoff.routes_handoff_by_bem() {
            match bem_message_phase(
                &output_text,
                &handoff.agent9527_response_handoff_channel_prefixes,
            ) {
        ...
        let output = match active_handoff {
            Some(handoff_id) => {
                ...
                *handoff.last_output.lock().await = Some(RealtimeHandoffOutput {
                ...
                if handoff.agent9527_responses_as_items {
                    RealtimeOutbound::ConversationItem {
                        text: realtime_backend_item(
                            output_text,
                            handoff.agent9527_response_item_prefix.as_deref(),
                        ),
                        phase,
                    }
    ...
    pub(crate) async fn append_speech(&self, text: String) -> Agent9527Result<()> {
    ...
    pub(crate) async fn handoff_complete(&self) -> Agent9527Result<()> {
        let handoff = {
            let guard = self.state.lock().await;
            guard.as_ref().map(|state| state.handoff.clone())
        };
        ...
        let output = if handoff.agent9527_responses_as_items {
    ...
    pub(crate) async fn shutdown(&self) -> Agent9527Result<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> Agent9527Result<PreparedRealtimeConversationStart> {
    ...
    Ok(PreparedRealtimeConversationStart {
        ...
        agent9527_responses_as_items: params.agent9527_responses_as_items,
        agent9527_response_item_prefix: params.agent9527_response_item_prefix,
        agent9527_response_handoff_mode: params.agent9527_response_handoff_mode,
        agent9527_response_handoff_channel_prefixes: params
            .agent9527_response_handoff_channel_prefixes,
        realtime_call_api_provider,
...
) -> Agent9527Result<RealtimeSessionConfig> {
    let config = sess.get_config().await;
...
fn realtime_api_key(
    ...
    if let Some(api_key) = auth.and_then(Agent9527Auth::api_key) {
    ...
    Err(Agent9527Err::InvalidRequest(
...
fn realtime_request_headers(
    ...
    event_parser: RealtimeEventParser,
    originator: &str,
) -> Agent9527Result<Option<HeaderMap>> {
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            Agent9527Err::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;
...
fn v3_output_writer(
    writer: &RealtimeWebsocketWriter,
    phase: Option<&MessagePhase>,
    handoff_mode: Agent9527ResponseHandoffMode,
) -> RealtimeWebsocketWriter {
    let channel = match handoff_mode {
        Agent9527ResponseHandoffMode::Thinking => None,
        Agent9527ResponseHandoffMode::Commentary => Some(RealtimeContextAppendChannel::Commentary),
        Agent9527ResponseHandoffMode::BemTags => match phase {
        ...
        RealtimeEventParser::FramelessBidi => match handoff_output {
            RealtimeOutbound::StandaloneHandoff { text, phase } => {
                v3_output_writer(
                    writer,
                    phase.as_ref(),
                    handoff_state.agent9527_response_handoff_mode,
                )
                .send_standalone_handoff(STANDALONE_HANDOFF_ID.to_string(), text)
                .await
            }
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    agent9527_error_info: Agent9527ErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,

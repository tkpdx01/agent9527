...
fn foreign_agents_md_uses_environment_native_paths() {
    let (cwd, rendered_cwd) = if cfg!(windows) {
        (
            PathUri::parse("file:///agent9527%20runtime").expect("POSIX cwd URI"),
            "/agent9527 runtime",
        )
    } else {
        (
            PathUri::parse("file:///C:/agent9527%20runtime").expect("Windows cwd URI"),
            r"C:\agent9527 runtime",
        )
    };
...
async fn make_config(root: &TempDir, limit: usize, instructions: Option<&str>) -> TestConfig {
    ...
    let user_instructions = instructions.map(|text| UserInstructions {
        text: text.to_owned(),
        source: config.agent9527_home.join(DEFAULT_AGENTS_MD_FILENAME),
    });
    TestConfig {
        config,
        user_instructions,
...
async fn project_root_markers_are_honored_for_agents_discovery() {
    ...
    fs::write(root.path().join(".agent9527-root"), "").unwrap();
...
async fn project_layers_do_not_override_project_root_markers() {
    ...
    config.cwd = nested.abs();
    let project_layer = |dot_agent9527_folder: AbsolutePathBuf, marker: &str| {
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![
            project_layer(root.path().join(".agent9527").abs(), ".ignored-root-marker"),
            project_layer(config.cwd.join(".agent9527"), ".ignored-nested-marker"),
        ],
...
async fn instruction_sources_include_global_before_agents_md_docs() {
    ...
    let global_agents = cfg.agent9527_home.join(DEFAULT_AGENTS_MD_FILENAME);
    fs::create_dir_all(&cfg.agent9527_home).unwrap();
...
async fn skills_are_not_appended_to_agents_md() {
    ...
    let cfg = make_config(&tmp, /*limit*/ 4096, /*instructions*/ None).await;
    create_skill(
        cfg.agent9527_home.to_path_buf(),
        "pdf-processing",
        "extract from pdfs",
    );
...
fn create_skill(agent9527_home: PathBuf, name: &str, description: &str) {
    let skill_dir = agent9527_home.join(format!("skills/{name}"));

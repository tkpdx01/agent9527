...
pub fn elevated_setup_failure_details(err: &anyhow::Error) -> Option<(String, String)> {
    ...
    let message = agent9527_windows_sandbox::sanitize_setup_metric_tag_value(&failure.message);
    Some((code, message))
}
...
pub fn run_elevated_provisioning_setup(
    ...
    agent9527_windows_sandbox::run_elevated_provisioning_setup(agent9527_home, real_user)
}

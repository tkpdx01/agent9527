...
//!
//! `ModelClient` is intended to live for the lifetime of a Agent9527 session and holds the stable
...
//! per-turn state such as the `x-agent9527-turn-state` token used for sticky routing.
//!
...
use agent9527_api::AuthProvider;
use agent9527_api::CompactClient as ApiCompactClient;
use agent9527_api::CompactionInput as ApiCompactionInput;
use agent9527_api::Compression;
use agent9527_api::MemoriesClient as ApiMemoriesClient;
use agent9527_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use agent9527_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use agent9527_api::Provider as ApiProvider;
use agent9527_api::RawMemory as ApiRawMemory;
use agent9527_api::RealtimeCallClient as ApiRealtimeCallClient;
use agent9527_api::RealtimeSessionConfig as ApiRealtimeSessionConfig;
use agent9527_api::Reasoning;
use agent9527_api::ReasoningContext;
use agent9527_api::RequestTelemetry;
use agent9527_api::ReqwestTransport;
use agent9527_api::ResponseCreateWsRequest;
use agent9527_api::ResponsesApiRequest;
use agent9527_api::ResponsesClient as ApiResponsesClient;
use agent9527_api::ResponsesOptions as ApiResponsesOptions;
use agent9527_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use agent9527_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use agent9527_api::ResponsesWsRequest;
use agent9527_api::SharedAuthProvider;
use agent9527_api::SseTelemetry;
use agent9527_api::StreamOptions;
use agent9527_api::TransportError;
use agent9527_api::WebsocketTelemetry;
use agent9527_api::auth_header_telemetry;
use agent9527_api::build_session_headers;
use agent9527_api::create_text_param_for_request;
use agent9527_api::response_create_client_metadata;
...
use agent9527_login::RefreshTokenError;
use agent9527_login::UnauthorizedRecovery;
use agent9527_login::default_client::add_originator_header;
use agent9527_login::default_client::create_client_for_route;
...
use agent9527_protocol::config_types::Verbosity as VerbosityConfig;
...
use agent9527_protocol::protocol::InternalSessionSource;
use agent9527_protocol::protocol::SessionSource;
use agent9527_protocol::protocol::W3cTraceContext;
use agent9527_rollout_trace::CompactionTraceContext;
use agent9527_rollout_trace::InferenceTraceAttempt;
use agent9527_rollout_trace::InferenceTraceContext;
use agent9527_tools::create_tools_json_for_responses_api;
use agent9527_tools::create_tools_raw_json_for_responses_api;
...
use agent9527_login::auth_env_telemetry::AuthEnvTelemetry;
use agent9527_login::auth_env_telemetry::collect_auth_env_telemetry;
use agent9527_model_provider::AgentIdentitySessionFallback;
use agent9527_model_provider::ProviderAuthScope;
use agent9527_model_provider::SharedModelProvider;
use agent9527_model_provider::create_model_provider;
#[cfg(test)]
use agent9527_model_provider_info::DEFAULT_WEBSOCKET_CONNECT_TIMEOUT_MS;
...
use agent9527_protocol::error::Result;
use agent9527_response_debug_context::extract_response_debug_context;
use agent9527_response_debug_context::extract_response_debug_context_from_api_error;
use agent9527_response_debug_context::telemetry_api_error_message;
use agent9527_response_debug_context::telemetry_transport_error_message;

pub const OPENAI_BETA_HEADER: &str = "OpenAI-Beta";
pub const X_AGENT9527_INSTALLATION_ID_HEADER: &str = "x-agent9527-installation-id";
pub const X_AGENT9527_TURN_STATE_HEADER: &str = "x-agent9527-turn-state";
pub const X_AGENT9527_TURN_METADATA_HEADER: &str = "x-agent9527-turn-metadata";
pub const X_AGENT9527_PARENT_THREAD_ID_HEADER: &str = "x-agent9527-parent-thread-id";
pub const X_AGENT9527_WINDOW_ID_HEADER: &str = "x-agent9527-window-id";
...
const X_AGENT9527_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str =
    "x-agent9527-ws-stream-request-start-ms";
const WS_REQUEST_HEADER_RESPONSES_LITE_CLIENT_METADATA_KEY: &str =
    "ws_request_header_x_openai_internal_agent9527_responses_lite";
const RESPONSES_WEBSOCKETS_V2_BETA_HEADER_VALUE: &str = "responses_websockets=2026-02-06";
const X_OPENAI_INTERNAL_AGENT9527_RESPONSES_LITE_HEADER: &str =
    "x-openai-internal-agent9527-responses-lite";
...
///
/// This holds configuration and state that should be shared across turns within a Agent9527 session
...
/// - The `x-agent9527-turn-state` sticky-routing token, which must be replayed for all requests within
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each Agent9527 turn. Reusing it across turns would replay
...
pub struct ModelClientSession {
    ...
    /// on turn start via the `x-agent9527-turn-state` response header. Once set, this value
    /// should be sent back to the server in the `x-agent9527-turn-state` request header for
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a Agent9527 session. Per-turn values
    ...
    pub fn new(
        ...
        let agent9527_api_key_env_enabled = model_provider
            .auth_manager()
            .as_ref()
            .is_some_and(|manager| manager.agent9527_api_key_env_enabled());
        let auth_env_telemetry =
            collect_auth_env_telemetry(model_provider.info(), agent9527_api_key_env_enabled);
    ...
    fn prompt_cache_key(&self, responses_metadata: &Agent9527ResponsesMetadata) -> String {
    ...
    pub(crate) fn force_http_fallback(
        ...
        if activated {
            ...
            session_telemetry.counter(
                "agent9527.transport.fallback_to_http",
                /*inc*/ 1,
    ...
    pub(crate) async fn compact_conversation_history(
        ...
        responses_metadata: &Agent9527ResponsesMetadata,
        ...
        let request_telemetry = Self::build_request_telemetry(
            ...
            AuthRequestTelemetryContext::new(
                client_setup.auth.as_ref().map(Agent9527Auth::auth_mode),
        ...
        if let Ok(header_value) = HeaderValue::from_str(&responses_metadata.installation_id) {
            extra_headers.insert(X_AGENT9527_INSTALLATION_ID_HEADER, header_value);
        }
    ...
    fn build_responses_request(
        &self,
        provider: &agent9527_api::Provider,
        prompt: &Prompt,
        model_info: &ModelInfo,
        effort: Option<ReasoningEffortConfig>,
        summary: ReasoningSummaryConfig,
        ...
        .then_some(StreamOptions {
            reasoning_summary_delivery: agent9527_api::ReasoningSummaryDelivery::SequentialCutoff,
        });
    ...
    async fn connect_websocket(
        &self,
        session_telemetry: &SessionTelemetry,
        api_provider: agent9527_api::Provider,
        api_auth: SharedAuthProvider,
        responses_metadata: &Agent9527ResponsesMetadata,
        auth_context: AuthRequestTelemetryContext,
        request_route_telemetry: RequestRouteTelemetry,
        ...
        let result = match tokio::time::timeout(
            ...
            ApiWebSocketResponsesClient::new(api_provider, api_auth).connect(
                &self.http_client_factory,
                headers,
                agent9527_login::default_client::default_headers(),
                /*turn_state*/ None,
...
impl ModelClientSession {
    ...
    fn responses_request_compression(&self, auth: Option<&Agent9527Auth>) -> Compression {
        if self.client.state.enable_request_compression
            && auth.is_some_and(Agent9527Auth::uses_agent9527_backend)
    ...
    async fn stream_responses_websocket(
        ...
        loop {
            ...
            if let Some(turn_state) = self.turn_state.get() {
                client_metadata.insert(
                    X_AGENT9527_TURN_STATE_HEADER.to_string(),
                    turn_state.clone(),
                );
            }
            match self
    ...
    /// Permanently disables WebSockets for this Agent9527 session and resets WebSocket state.
    ///
...
fn stamp_ws_stream_request_start_ms(request: &mut ResponsesWsRequest<'_>) {
    ...
    payload
        ...
        .insert(
            X_AGENT9527_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY.to_string(),
...
///
/// These headers implement Agent9527-specific conventions:
///
/// - `x-agent9527-beta-features`: comma-separated beta feature keys enabled for the session.
/// - `x-agent9527-turn-state`: sticky routing token captured earlier in the turn.
fn build_responses_headers(
    ...
    {
        headers.insert("x-agent9527-beta-features", header_value);
    }
    if let Some(turn_state) = turn_state
        && let Some(state) = turn_state.get()
        && let Ok(header_value) = HeaderValue::from_str(state)
    {
        headers.insert(X_AGENT9527_TURN_STATE_HEADER, header_value);
    }
    headers
}
...
fn add_responses_lite_header(headers: &mut ApiHeaderMap, use_responses_lite: bool) {
    if use_responses_lite {
        headers.insert(
            X_OPENAI_INTERNAL_AGENT9527_RESPONSES_LITE_HEADER,
            HeaderValue::from_static("true"),
        );
    }
}
...
fn map_response_stream(
    api_stream: agent9527_api::ResponseStream,
    session_telemetry: SessionTelemetry,
    inference_trace_attempt: InferenceTraceAttempt,
    provider: SharedModelProvider,
) -> (ResponseStream, oneshot::Receiver<LastResponse>) {
    let agent9527_api::ResponseStream {
        rx_event,
        upstream_request_id,
    } = api_stream;
    let api_stream = agent9527_api::ResponseStream {
        rx_event,
...
/// the mapped `Agent9527Err` is returned to the caller.
...
struct WebsocketConnectParams<'a> {
    ...
    api_provider: agent9527_api::Provider,
    api_auth: SharedAuthProvider,
    responses_metadata: &'a Agent9527ResponsesMetadata,
    auth_context: AuthRequestTelemetryContext,
    request_route_telemetry: RequestRouteTelemetry,
}
        ...
        && recovery.has_next()
    {
        ...
        return match recovery.next().await {
            ...
            Err(RefreshTokenError::Permanent(failed)) => {
                ...
                Err(Agent9527Err::RefreshTokenFailed(failed))
            }

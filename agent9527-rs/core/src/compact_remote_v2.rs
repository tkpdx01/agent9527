...
use agent9527_rollout_trace::CompactionCheckpointTracePayload;
...
async fn run_remote_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            ...
            attempt
                .track(
                    sess.as_ref(),
                    agent9527_analytics::CompactionStatus::Interrupted,
                    Some(&error),
                    analytics_details,
                )
                .await;
    ...
    if result.is_ok() {
        ...
        if let PostCompactHookOutcome::Stopped = post_compact_outcome {
            attempt
                .track(sess.as_ref(), status, agent9527_error, analytics_details)
                .await;
    ...
    match result {
        Ok(()) => Ok(()),
        Err(err @ Agent9527Err::TurnAborted) => Err(err),
        Err(err) => {
            sess.track_turn_agent9527_error(turn_context, &err);
...
) -> Agent9527Result<RemoteCompactionV2Output> {
...
mod tests {
    ...
    fn response_stream(events: Vec<Agent9527Result<ResponseEvent>>) -> ResponseStream {

...
async fn clears_active_handoff_explicitly() {
    ...
    let state = RealtimeHandoffState {
        ...
        agent9527_response_handoff_channel_prefixes: Arc::new(BTreeMap::new()),
...
fn realtime_headers_include_only_non_default_originator() {
    let default_originator = agent9527_login::default_client::originator();
    for (originator, expected_header) in [
        ("agent9527_work_desktop", Some("agent9527_work_desktop")),

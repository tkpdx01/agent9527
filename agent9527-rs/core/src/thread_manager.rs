use crate::Agent9527AppsToolsCache;
use crate::SkillsService;
use crate::agent::AgentControl;
use crate::agent9527_thread::Agent9527Thread;
...
use agent9527_agent_graph_store::AgentGraphStore;
use agent9527_agent_graph_store::LocalAgentGraphStore;
...
use agent9527_code_mode::CodeModeSessionProvider;
use agent9527_code_mode::InProcessCodeModeSessionProvider;
use agent9527_code_mode::ProcessOwnedCodeModeSessionProvider;
...
use agent9527_model_provider_info::OPENAI_PROVIDER_ID;
...
use agent9527_thread_store::LoadThreadHistoryParams;
use agent9527_thread_store::LocalThreadStore;
use agent9527_thread_store::LocalThreadStoreConfig;
use agent9527_thread_store::PreparedFork;
use agent9527_thread_store::ReadThreadByRolloutPathParams;
use agent9527_thread_store::ReadThreadParams;
use agent9527_thread_store::StoredModelContext;
...
struct TempAgent9527HomeGuard {
    path: PathBuf,
}

impl Drop for TempAgent9527HomeGuard {
    fn drop(&mut self) {
        let _ = std::fs::remove_dir_all(&self.path);
    }
}

/// Represents a newly created Agent9527 thread (formerly called a conversation), including the first event
...
pub struct ThreadManager {
    ...
    _test_agent9527_home_guard: Option<TempAgent9527HomeGuard>,
}
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<agent9527_protocol::dynamic_tools::DynamicToolSpec>,
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<Agent9527Thread>>>>,
...
pub fn thread_store_from_config(
    ...
    match &config.experimental_thread_store {
        ThreadStoreConfig::Local => {
            ...
            {
                agent9527_rollout::spawn_rollout_compression_worker(
                    config.agent9527_home.to_path_buf(),
                );
            }
...
impl ThreadManager {
    ...
    pub fn new(
        ...
        auth_manager: Arc<AuthManager>,
        models_manager: SharedModelsManager,
        agent9527_apps_tools_cache: Agent9527AppsToolsCache,
        session_source: SessionSource,
        environment_manager: Arc<EnvironmentManager>,
        extensions: Arc<ExtensionRegistry<Config>>,
        user_instructions_provider: Arc<dyn UserInstructionsProvider>,
        ...
        Self {
            ...
            _test_agent9527_home_guard: None,
        }
    }
    ...
    /// Construct with a dummy AuthManager containing the provided Agent9527Auth.
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let agent9527_home = std::env::temp_dir().join(format!(
            "agent9527-thread-manager-test-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&agent9527_home)
            .unwrap_or_else(|err| panic!("temp agent9527 home dir create failed: {err}"));
        ...
        manager._test_agent9527_home_guard = Some(TempAgent9527HomeGuard {
            path: agent9527_home,
        });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided Agent9527Auth and agent9527 home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        let absolute_agent9527_home =
            match AbsolutePathBuf::from_absolute_path_checked(&agent9527_home) {
                Ok(agent9527_home) => agent9527_home,
                Err(err) => panic!("test agent9527_home should be absolute: {err}"),
            };
        let (thread_created_tx, _) = broadcast::channel(THREAD_CREATED_CHANNEL_CAPACITY);
        ...
        let thread_store: Arc<dyn ThreadStore> = Arc::new(LocalThreadStore::new(
            LocalThreadStoreConfig {
                ...
                sqlite: agent9527_state::SqliteConfig::new_for_testing(absolute_agent9527_home),
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                models_manager: create_model_provider(provider, Some(auth_manager.clone()))
                    .models_manager(agent9527_home, /*config_model_catalog*/ None),
                environment_manager,
                starting_mcp_runtimes: std::sync::Mutex::new(Vec::new()),
                skills_service,
                plugins_manager,
    ...
    pub fn validate_environment_selections(
        ...
        for environment in environments {
            if !environment_ids.insert(environment.environment_id.as_str()) {
                return Err(Agent9527Err::InvalidRequest(format!(
            ...
            self.state
                ...
                .ok_or_else(|| {
                    Agent9527Err::InvalidRequest(format!(
    ...
    pub async fn list_models(
        ...
        http_client_factory: agent9527_http_client::HttpClientFactory,
    ) -> Vec<ModelPreset> {
        self.state
            .models_manager
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> Agent9527Result<Arc<Agent9527Thread>> {
        self.state.get_thread(thread_id).await
    }

    /// Updates metadata for loaded and cold threads through one entrypoint.
    ///
    /// Loaded threads route through `Agent9527Thread`/`LiveThread`, so metadata changes stay ordered
        ...
        self.state
            ...
            .map_err(|err| match err {
                ThreadStoreError::ThreadNotFound { thread_id } => {
                    Agent9527Err::ThreadNotFound(thread_id)
                }
    ...
    pub async fn list_agent_subtree_thread_ids(
        ...
        if let Some(agent_graph_store) = self.state.agent_graph_store() {
            for descendant_id in agent_graph_store
                ...
                .await
                .map_err(|err| {
                    Agent9527Err::Fatal(format!("failed to load thread-spawn descendants: {err}"))
                })?
            {
    ...
    pub async fn start_thread(&self, options: StartThreadOptions) -> Agent9527Result<NewThread> {
    ...
    /// as `Arc<Agent9527Thread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<Agent9527Thread>> {
    ...
    async fn initial_history_from_rollout_path(
        &self,
        rollout_path: PathBuf,
    ) -> Agent9527Result<InitialHistory> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(
        &self,
        thread_id: ThreadId,
    ) -> Agent9527Result<Arc<Agent9527Thread>> {
        let threads = self.threads.read().await;
        match threads.get(&thread_id) {
            Some(thread) if !thread.session_source.is_internal() => Ok(thread.clone()),
            Some(_) | None => Err(Agent9527Err::ThreadNotFound(thread_id)),
        }
    }
    ...
    pub(crate) async fn read_stored_thread(
        ...
        self.thread_store
            ...
            .map_err(|err| match err {
                ...
                err => Agent9527Err::Fatal(format!("failed to read stored thread {thread_id}: {err}")),
            })
    }
    ...
    ) -> Agent9527Result<StoredModelContext> {
        let thread_id = params.thread_id;
        self.thread_store
    ...
    pub(crate) async fn send_op(&self, thread_id: ThreadId, op: Op) -> Agent9527Result<String> {
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<Agent9527Thread>> {
    ...
    async fn effective_originator(
        ...
        let env_originator = std::env::var(AGENT9527_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
            .is_ok()
    ...
    pub(crate) async fn spawn_thread(
        ...
        thread_source: Option<ThreadSource>,
        dynamic_tools: Vec<agent9527_protocol::dynamic_tools::DynamicToolSpec>,
        metrics_service_name: Option<String>,
        parent_trace: Option<W3cTraceContext>,
    ...
    async fn finalize_thread_spawn(
        ...
        let session_configured = match event {
            ...
            _ => {
                return Err(Agent9527Err::SessionConfiguredNotFirstEvent);
            }
        };

        {
            ...
            if let std::collections::hash_map::Entry::Vacant(e) = threads.entry(thread_id) {
                let thread = Arc::new(Agent9527Thread::new(
                    session,
                    io,
        ...
        Err(Agent9527Err::InvalidRequest(format!(
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> agent9527_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return agent9527_rollout_trace::ThreadTraceContext::disabled();
        };
        ...
        self.get_thread(*parent_thread_id)
            ...
            .unwrap_or_else(agent9527_rollout_trace::ThreadTraceContext::disabled)
    }
}
...
fn thread_store_rollout_read_error(err: ThreadStoreError) -> Agent9527Err {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => Agent9527Err::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => Agent9527Err::InvalidRequest(message),
        err => Agent9527Err::Fatal(format!("failed to read thread by rollout path: {err}")),
    }
}

fn thread_store_metadata_update_error(thread_id: ThreadId, err: ThreadStoreError) -> Agent9527Err {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => Agent9527Err::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => Agent9527Err::InvalidRequest(message),
        ThreadStoreError::Unsupported { operation } => Agent9527Err::UnsupportedOperation(format!(

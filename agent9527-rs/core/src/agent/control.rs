...
pub(crate) struct AgentControl {
    ...
    /// `ThreadManagerState -> Agent9527Thread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    pub(crate) async fn interrupt_agent(&self, agent_id: ThreadId) -> Agent9527Result<String> {
        let state = self.upgrade()?;
        self.handle_thread_request_result(
            agent_id,
            &state,
    ...
    async fn handle_thread_request_result(
        ...
        agent_id: ThreadId,
        state: &Arc<ThreadManagerState>,
        result: Agent9527Result<String>,
    ) -> Agent9527Result<String> {
        if result
            .as_ref()
            .is_err_and(|err| matches!(err.details(), Agent9527ErrorDetails::InternalAgentDied))
        {
    ...
    pub(crate) fn ensure_agent_known(&self, agent_id: ThreadId) -> Agent9527Result<AgentMetadata> {
        self.state
            .agent_metadata_for_thread(agent_id)
            .ok_or_else(|| Agent9527Err::ThreadNotFound(agent_id))
    }
    ...
    pub(crate) async fn resolve_agent_reference(
        ...
        Err(Agent9527Err::UnsupportedOperation(format!(
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> Agent9527Result<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
    ...
    ) -> Agent9527Result<Vec<ListedAgent>> {
        let state = self.upgrade()?;
    ...
    fn prepare_thread_spawn(
        ...
        agent_role: Option<String>,
        preferred_agent_nickname: Option<String>,
    ) -> Agent9527Result<(SessionSource, AgentMetadata)> {
        if depth == 1 {
    ...
    fn upgrade(&self) -> Agent9527Result<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
            .ok_or_else(|| Agent9527Err::UnsupportedOperation("thread manager dropped".to_string()))
    }
    ...
    async fn open_thread_spawn_children(
        &self,
        parent_thread_id: ThreadId,
    ) -> Agent9527Result<Vec<(ThreadId, AgentMetadata)>> {
    ...
    async fn live_thread_spawn_children(
        &self,
    ) -> Agent9527Result<HashMap<ThreadId, Vec<(ThreadId, AgentMetadata)>>> {
        let state = self.upgrade()?;
    ...
    async fn persist_thread_spawn_edge_for_source(
        &self,
        child_thread: &crate::Agent9527Thread,
        child_thread_id: ThreadId,
        session_source: Option<&SessionSource>,
    ) {
        ...
        if let Err(err) = agent_graph_store
            .upsert_thread_spawn_edge(
                parent_thread_id,
                child_thread_id,
                agent9527_agent_graph_store::ThreadSpawnEdgeStatus::Open,
            )
            .await
        {

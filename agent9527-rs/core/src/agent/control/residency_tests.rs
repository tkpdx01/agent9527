...
async fn residency_slot_reservation_unloads_oldest_idle_v2_agent() {
    ...
    config.agent9527_home = temp_home.path().to_path_buf().try_into().unwrap();
    ...
    match manager.get_thread(first.thread_id).await {
        Err(Agent9527Err::ThreadNotFound(thread_id)) => assert_eq!(thread_id, first.thread_id),
...
async fn interrupted_v2_agent_is_lost_after_residency_eviction() {
    ...
    match err {
        Agent9527Err::ThreadNotFound(thread_id) => assert_eq!(thread_id, first.thread_id),
...
async fn mark_thread_completed(thread: &Agent9527Thread) {
...
async fn mark_thread_interrupted(thread: &Agent9527Thread) {
    let turn = thread.session.new_default_turn().await;
    thread
        .session
        .send_event(
...
async fn clear_active_turn(thread: &Agent9527Thread) {

...
async fn test_config_with_cli_overrides(
    ...
    let config = ConfigBuilder::default()
        .agent9527_home(home_path.clone())
...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.agent9527_linux_sandbox_exe = Some(PathBuf::from("/tmp/agent9527-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/agent9527-execve-wrapper"));
    let role_path = write_role_config(
        &home,
    ...
    assert_eq!(
        config.agent9527_linux_sandbox_exe,
        Some(PathBuf::from("/tmp/agent9527-linux-sandbox"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/agent9527-execve-wrapper"))
    );
}
...
"#,
    ...
    let snapshot = skills_service
        .snapshot_for_config(
            &skills_input,
            Some(Arc::clone(&agent9527_exec_server::LOCAL_FS)),
        )
        .await;

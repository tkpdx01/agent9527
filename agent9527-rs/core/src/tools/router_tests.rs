...
use agent9527_extension_api::ResponsesApiTool;
use agent9527_extension_api::ToolCall as ExtensionToolCall;
...
impl agent9527_extension_api::ToolContributor for ExtensionEchoContributor {
    fn tools(
        &self,
...
impl ToolExecutor<ExtensionToolCall> for ExtensionEchoExecutor {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Namespace(ResponsesApiNamespace {
            ...
            tools: vec![ResponsesApiNamespaceTool::Function(ResponsesApiTool {
                ...
                strict: true,
                parameters: agent9527_extension_api::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
    ...
    fn handle(&self, call: ExtensionToolCall) -> agent9527_tools::ToolExecutorFuture<'_> {
...
impl ExtensionEchoExecutor {
    async fn handle_call(
        ...
        Ok(Box::new(agent9527_tools::JsonToolOutput::new(json!({
            "arguments": arguments,
            "callId": call.call_id,
...
async fn build_tool_call_uses_namespace_for_registry_name() -> anyhow::Result<()> {
    ...
    let call = ToolRouter::build_tool_call(ResponseItem::FunctionCall {
        ...
        namespace: Some("mcp__agent9527_apps__calendar".to_string()),
        arguments: "{}".to_string(),
    ...
    assert_eq!(
        call.tool_name,
        ToolName::namespaced("mcp__agent9527_apps__calendar", tool_name)
    );
...
async fn specs_filter_deferred_dynamic_tools() -> anyhow::Result<()> {
    ...
    assert_eq!(
        namespace_function_names(&router.model_visible_specs(), "agent9527_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        router.deferred_tool_namespaces(),
        BTreeMap::from([(
            "agent9527_app".to_string(),
            "Agent9527 app tools.".to_string()
        )])
    );
...
fn mcp_runtime(tool_info: agent9527_mcp::ToolInfo) -> Arc<dyn CoreToolRuntime> {
    ...
    let router = ToolRouter::from_context(
        ...
        ToolRouterParams {
            ...
            extension_tool_executors: extension_tool_executors(
                &session,
                &agent9527_extension_api::ExtensionData::new(turn.sub_id.clone()),
            ),
            wait_for_environment_tool_config: None,
            dynamic_tools: turn.dynamic_tools.as_slice(),
        },

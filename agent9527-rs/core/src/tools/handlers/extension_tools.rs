...
use agent9527_tools::ConversationHistory;
use agent9527_tools::ExtensionTurnItem;
use agent9527_tools::ToolCall as ExtensionToolCall;
use agent9527_tools::ToolEnvironment;
use agent9527_tools::ToolName;
use agent9527_tools::ToolSearchInfo;
use agent9527_tools::ToolSpec;
use agent9527_tools::TurnItemEmissionFuture;
use agent9527_tools::TurnItemEmitter;
...
pub(crate) struct ExtensionToolAdapter(Arc<dyn agent9527_tools::ToolExecutor<ExtensionToolCall>>);

impl ExtensionToolAdapter {
    pub(crate) fn new(executor: Arc<dyn agent9527_tools::ToolExecutor<ExtensionToolCall>>) -> Self {
        Self(executor)
    }
}
...
async fn to_extension_call(invocation: &ToolInvocation) -> ExtensionToolCall {
    ...
    let agent9527_turn_metadata = invocation
        .turn
        .turn_metadata_state
...
mod tests {
    ...
    use agent9527_extension_items::web_search::WebSearchItem;
    use agent9527_protocol::items::TurnItem;
    ...
    impl agent9527_extension_api::ToolExecutor<agent9527_tools::ToolCall> for StubExtensionExecutor {
        fn tool_name(&self) -> agent9527_tools::ToolName {
            agent9527_tools::ToolName::plain("extension_echo")
        }

        fn spec(&self) -> agent9527_tools::ToolSpec {
            agent9527_tools::ToolSpec::Function(agent9527_tools::ResponsesApiTool {
                name: "extension_echo".to_string(),
                description: "Echoes arguments.".to_string(),
                strict: true,
                parameters: agent9527_tools::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
        ...
        fn handle(
            &self,
            _call: agent9527_tools::ToolCall,
        ) -> agent9527_tools::ToolExecutorFuture<'_> {
            Box::pin(async {
                Ok(
                    Box::new(agent9527_tools::JsonToolOutput::new(json!({ "ok": true })))
                        as Box<dyn agent9527_tools::ToolOutput>,
                )
            })
        }
    }
    ...
    struct CapturingExtensionExecutor {
        captured_call: Arc<Mutex<Option<agent9527_tools::ToolCall>>>,
    }

    impl agent9527_extension_api::ToolExecutor<agent9527_tools::ToolCall>
    ...
    impl CapturingExtensionExecutor {
        async fn handle_call(
            &self,
            call: agent9527_tools::ToolCall,
        ) -> Result<Box<dyn agent9527_tools::ToolOutput>, agent9527_tools::FunctionCallError>
        {
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: agent9527_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = agent9527_tools::JsonToolOutput::new(json!({ "ok": true }));
    ...
    async fn image_generation_publication_preserves_extension_saved_path() {
        ...
        agent9527_tools::TurnItemEmitter::emit_started(
            &emitter,
            ExtensionTurnItem {
        ...
        .await;
        agent9527_tools::TurnItemEmitter::emit_completed(
            &emitter,
            ExtensionTurnItem {

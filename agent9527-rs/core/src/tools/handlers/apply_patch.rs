...
use agent9527_apply_patch::ApplyPatchAction;
use agent9527_apply_patch::ApplyPatchFileChange;
use agent9527_apply_patch::Hunk;
use agent9527_apply_patch::StreamingPatchParser;
...
use agent9527_protocol::protocol::PatchApplyUpdatedEvent;
use agent9527_sandboxing::policy_transforms::effective_file_system_sandbox_policy;
use agent9527_sandboxing::policy_transforms::merge_permission_profiles;
use agent9527_sandboxing::policy_transforms::normalize_additional_permissions;
...
fn format_update_chunks_for_progress(chunks: &[agent9527_apply_patch::UpdateFileChunk]) -> String {
...
fn write_permissions_for_paths(
    ...
    file_system_sandbox_policy: &agent9527_protocol::permissions::FileSystemSandboxPolicy,
    cwd: &AbsolutePathBuf,
...
async fn effective_patch_permissions(
    ...
    agent9527_protocol::permissions::FileSystemSandboxPolicy,
)> {
...
impl ApplyPatchHandler {
    async fn handle_call(
        ...
        let args = match agent9527_apply_patch::parse_patch(&patch_input) {
            Ok(args) => args,
            Err(parse_error) => {
        ...
        match agent9527_apply_patch::verify_apply_patch_args(
            args,
        ...
        .await
        {
            agent9527_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            ...
            agent9527_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            agent9527_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            agent9527_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    match agent9527_apply_patch::maybe_parse_apply_patch_verified(command, cwd, fs, Some(&sandbox))
        .await
    {
        agent9527_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
        ...
        agent9527_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}

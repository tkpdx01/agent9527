...
use agent9527_analytics::PluginInstallRequestSource;
use agent9527_analytics::PluginInstallRequested;
use agent9527_analytics::PluginInstallRequestedPlugin;
...
use agent9527_tools::DiscoverableToolAction;
use agent9527_tools::DiscoverableToolType;
use agent9527_tools::LIST_AVAILABLE_PLUGINS_TO_INSTALL_TOOL_NAME;
use agent9527_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use agent9527_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use agent9527_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use agent9527_tools::RequestPluginInstallArgs;
use agent9527_tools::RequestPluginInstallResult;
use agent9527_tools::ToolName;
use agent9527_tools::ToolSpec;
use agent9527_tools::all_requested_connectors_picked_up;
use agent9527_tools::build_request_plugin_install_elicitation_request;
use agent9527_tools::filter_request_plugin_install_discoverable_tools_for_client;
use agent9527_tools::verified_connector_install_completed;
...
impl RequestPluginInstallHandler {
    async fn handle_call(
        ...
        if (requested_tool_type == Some(DiscoverableToolType::Plugin)
            ...
            && turn.app_server_client_name.as_deref() == Some("agent9527-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in agent9527-tui yet".to_string(),
            ));
        }
...
async fn maybe_persist_disabled_install_request(
    ...
    if let Err(err) = persist_disabled_install_request(&turn.config.agent9527_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn persist_disabled_install_request(
    agent9527_home: &agent9527_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn verify_request_plugin_install_completed(
    session: &crate::session::session::Session,
    turn: &crate::session::turn_context::TurnContext,
    manager: &agent9527_mcp::McpConnectionManager,
    tool: &DiscoverableTool,
    auth: Option<&agent9527_login::Agent9527Auth>,
) -> bool {
    match tool {
...
async fn refresh_missing_requested_connectors(
    ...
    match manager.hard_refresh_agent9527_apps_tools_cache().await {
        Ok(mcp_tools) => {
            let accessible_connectors = connectors::with_app_enabled_state(
                connectors::accessible_connectors_from_mcp_tools(&mcp_tools),
                &turn.config,
        ...
        Err(err) => {
            warn!(
                "failed to refresh agent9527 apps tools cache after plugin install request for {tool_id}: {err:#}"
            );
            None
        }
    }

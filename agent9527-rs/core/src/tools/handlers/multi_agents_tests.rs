...
use agent9527_models_manager::MODEL_PRO;
...
use agent9527_protocol::protocol::FileSystemAccessMode;
use agent9527_protocol::protocol::FileSystemPath;
use agent9527_protocol::protocol::FileSystemSandboxEntry;
use agent9527_protocol::protocol::FileSystemSandboxPolicy;
use agent9527_protocol::protocol::InitialHistory;
use agent9527_protocol::protocol::InterAgentCommunication;
use agent9527_protocol::protocol::NetworkSandboxPolicy;
...
fn service_tier_models_manager() -> SharedModelsManager {
    let bundled_models = bundled_models_response()
        .expect("bundled models should parse")
        .models;
    let supported_model = bundled_models
        .iter()
        .find(|model| model.slug == MODEL_BALANCED)
        .cloned()
        .expect("balanced model should exist in bundled catalog");
    let mut unsupported_model = bundled_models
        .iter()
        .find(|model| model.slug == MODEL_FLASH)
        .cloned()
        .expect("flash model should exist in bundled catalog");
    unsupported_model.service_tiers.clear();

    Arc::new(StaticModelsManager::new(
        /*auth_manager*/ None,
        ModelsResponse {
            models: vec![supported_model, unsupported_model],
        },
    ))
}
...
async fn install_role_with_model_override(turn: &mut TurnContext) -> String {
    ...
    tokio::fs::create_dir_all(&turn.config.agent9527_home)
        .await
        .expect("agent9527 home should be created");
        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    agent9527_protocol::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
async fn multi_agent_v2_spawn_fork_turns_all_rejects_agent_type_override() {
    ...
    let turn = TurnContext {
        ...
        multi_agent_version: agent9527_protocol::protocol::MultiAgentVersion::V2,
        ..turn
    };
...
async fn multi_agent_v2_spawn_rejects_child_model_from_different_backend() {
    ...
    assert_eq!(
        ...
        FunctionCallError::RespondToModel(format!(
            "Unknown model `gpt-5.4` for spawn_agent. Available models: {MODEL_PRO}, {MODEL_BALANCED}"
        ))
    );
}
...
async fn spawn_agent_service_tier_override_validates_the_effective_child_model() {
...

    {
        ...
        session.services.models_manager = service_tier_models_manager();
        let manager = thread_manager();
        let root = manager
            .start_thread(StartThreadOptions::new((*turn.config).clone()))
            .await
...

    {
        ...
        assert_eq!(
            err,
            FunctionCallError::RespondToModel(format!(
                "Service tier `turbo` is not supported for model `{MODEL_BALANCED}`. Supported service tiers: priority"
            ))
        );
    }

    {
        ...
        assert_eq!(
            err,
            FunctionCallError::RespondToModel(format!(
                "Service tier `priority` is not supported for model `{MODEL_FLASH}`. Supported service tiers: none"
            ))
        );
    }
}
...
async fn spawn_agent_service_tier_inheritance_preserves_supported_or_configured_tiers() {
...

    {
        ...
        session.services.models_manager = service_tier_models_manager();
        let mut turn = turn
            .with_model(MODEL_BALANCED.to_string(), &session.services.models_manager)
            .await;
        let mut config = (*turn.config).clone();
...
async fn multi_agent_v2_spawn_partial_fork_turns_allows_agent_type_override() {
    ...
    assert_eq!(snapshot.model, "ollama-role-override");
    assert_eq!(snapshot.model_provider_id, "ollama");
...
async fn resume_agent_restores_closed_agent_and_accepts_send_input() {
    ...
    let thread = manager
        .resume_thread_with_history(
            ...
            AuthManager::from_auth_for_testing(Agent9527Auth::from_api_key("dummy")),
            /*parent_trace*/ None,
...
async fn tool_handlers_cascade_close_and_resume_and_keep_explicitly_closed_subtrees_closed() {
    ...
    config.model = Some(MODEL_FLASH.to_string());
    config
        .features
        .enable(Feature::Sqlite)
        .expect("test config should allow sqlite");
...
async fn build_agent_spawn_config_uses_turn_context_values() {
    ...
    config.agent9527_linux_sandbox_exe = Some(PathBuf::from("/bin/echo"));
    turn.config = Arc::new(config);

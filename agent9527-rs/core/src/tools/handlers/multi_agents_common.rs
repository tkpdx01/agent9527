...
pub(crate) fn collab_spawn_error(err: Agent9527Err) -> FunctionCallError {
    match err {
        Agent9527Err::UnsupportedOperation(message) if message == "thread manager dropped" => {
            FunctionCallError::RespondToModel("collab manager unavailable".to_string())
        }
        Agent9527Err::UnsupportedOperation(message) => FunctionCallError::RespondToModel(message),
        err => FunctionCallError::RespondToModel(format!("collab spawn failed: {err}")),
    }
}

pub(crate) fn collab_agent_error(agent_id: ThreadId, err: Agent9527Err) -> FunctionCallError {
    match err {
        Agent9527Err::ThreadNotFound(id) => {

...
use agent9527_protocol::openai_models::ApplyPatchToolType;
...
use agent9527_protocol::openai_models::WebSearchToolType;
...
fn zsh_fork_config_for_spec_plan_tests() -> agent9527_tools::ZshForkConfig {
    let placeholder_exe = agent9527_utils_absolute_path::AbsolutePathBuf::try_from(
...
impl ToolExecutor<ExtensionToolCall> for TestNamespaceExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Namespace(agent9527_tools::ResponsesApiNamespace {
            ...
            tools: vec![ResponsesApiNamespaceTool::Function(ResponsesApiTool {
                ...
                strict: false,
                defer_loading: None,
                parameters: agent9527_tools::JsonSchema::default(),
                output_schema: None,
            })],
        })
    }

    fn handle(&self, _call: ExtensionToolCall) -> agent9527_tools::ToolExecutorFuture<'_> {
        Box::pin(async {
            Ok(Box::new(agent9527_tools::JsonToolOutput::new(json!({}))) as Box<dyn ToolOutput>)
        })
    }
}
...
impl ToolExecutor<ExtensionToolCall> for DeferredExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Function(ResponsesApiTool {
            ...
            strict: true,
            defer_loading: None,
            parameters: agent9527_tools::JsonSchema::object(
                BTreeMap::from([(
                    "message".to_string(),
                    agent9527_tools::JsonSchema::string(/*description*/ None),
                )]),
...
fn dynamic_tool(namespace: Option<&str>, name: &str, defer_loading: bool) -> DynamicToolSpec {
    let function = agent9527_protocol::dynamic_tools::DynamicToolFunctionSpec {
        name: name.to_string(),
    ...
    match namespace {
        Some(namespace) => DynamicToolSpec::Namespace(
            agent9527_protocol::dynamic_tools::DynamicToolNamespaceSpec {
                name: namespace.to_string(),
                description: format!("{namespace} dynamic tools"),
                tools: vec![
                    agent9527_protocol::dynamic_tools::DynamicToolNamespaceTool::Function(function),
                ],
            },
        ),
...
async fn request_user_input_stays_direct_in_code_mode_only() {
    ...
    plan.assert_visible_contains(&[
        "request_user_input",
        agent9527_code_mode::PUBLIC_TOOL_NAME,
        agent9527_code_mode::WAIT_TOOL_NAME,
    ]);
    ...
    let ToolSpec::Freeform(exec) = plan.visible_spec(agent9527_code_mode::PUBLIC_TOOL_NAME) else {
...
async fn shell_zsh_fork_stays_standalone_until_unified_exec_composition_is_enabled() {
    ...
    if agent9527_utils_pty::conpty_supported() {
...
async fn zsh_fork_unified_exec_hides_shell_parameter() {
    ...
    let plan = probe(|turn| {
        ...
        turn.unified_exec_shell_mode =
            agent9527_tools::UnifiedExecShellMode::ZshFork(zsh_fork_config_for_spec_plan_tests());
    })
    .await;
        ...
        turn.environments
            ...
            .push(TurnEnvironmentState::Ready(
                crate::session::turn_context::TurnEnvironment::new(
                    "remote".to_string(),
                    Arc::new(
                        agent9527_exec_server::Environment::create_for_tests(Some(
...
async fn environment_tools_follow_the_step_context() {
    ...
    turn.environments.environments.clear();
    let turn = Arc::new(turn);
    let mcp = Arc::new(agent9527_mcp::McpBinding::empty(mcp_config_for_test(
        &turn.config,
    )));
...
async fn sleep_tool_stays_direct_and_outside_code_mode() {
    for code_mode_only in [false, true] {
        ...
        let ToolSpec::Freeform(exec) = plan.visible_spec(agent9527_code_mode::PUBLIC_TOOL_NAME)
        else {
            panic!("expected code mode exec tool");
        };
        if code_mode_only {
...
async fn code_mode_only_exposes_code_executor_and_hides_nested_tools() {
    ...
    assert_eq!(
        plain.namespace_function_names("agent9527_app"),
        &["lookup".to_string()]
    );
    ...
    assert_eq!(
        code_mode_only.namespace_function_names("agent9527_app"),

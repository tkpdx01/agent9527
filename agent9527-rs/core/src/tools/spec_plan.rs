...
use agent9527_protocol::openai_models::ToolMode;
use agent9527_protocol::protocol::MultiAgentVersion;
use agent9527_tools::ResponsesApiNamespace;
use agent9527_tools::ResponsesApiNamespaceTool;
use agent9527_tools::TOOL_SEARCH_TOOL_NAME;
use agent9527_tools::ToolCall as ExtensionToolCall;
use agent9527_tools::ToolEnvironmentMode;
use agent9527_tools::ToolExecutor;
use agent9527_tools::ToolName;
use agent9527_tools::ToolSearchInfo;
use agent9527_tools::ToolSpec;
use agent9527_tools::UnifiedExecShellMode;
use agent9527_tools::can_request_original_image_detail;
use agent9527_tools::collect_code_mode_exec_prompt_tool_definitions;
use agent9527_tools::collect_request_plugin_install_entries;
use agent9527_tools::default_namespace_description;
use agent9527_tools::request_user_input_available_modes;
use agent9527_tools::shell_command_backend_for_features;
use agent9527_tools::shell_type_for_model_and_features;
...
struct CoreToolPlanContext<'a> {
    ...
    mcp: &'a agent9527_mcp::McpBinding,
...
fn spec_for_model_request(
    ...
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly)
        ...
        && agent9527_code_mode::is_code_mode_nested_tool(spec.name())
    {
        agent9527_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
    ...
    provider.uses_openai_actor_authorization()
        || (provider.requires_openai_auth
            && turn_context
                .auth_manager
                .as_deref()
                .is_some_and(AuthManager::current_auth_uses_agent9527_backend))
}
...
fn is_hidden_by_code_mode_only(
    ...
    tool_mode == ToolMode::CodeModeOnly
        && exposure != ToolExposure::DirectModelOnly
        && agent9527_code_mode::is_code_mode_nested_tool(
            &agent9527_tools::code_mode_name_for_tool_name(tool_name),
        )
}
...
fn build_code_mode_executors(
    ...
    if turn_context.model_info.use_responses_lite {
        let code_mode_tool_names =
            collect_code_mode_exec_prompt_tool_definitions(code_mode_nested_tool_specs.iter())
                ...
                .map(|tool| {
                    (
                        agent9527_code_mode::normalize_code_mode_identifier(&tool.name),
                        tool.tool_name,
                    )
                })
                .collect();
    ...
    let default_exec_yield_time_ms =
        default_exec_yield_time_override_ms(&turn_context.config.features)
            .unwrap_or(agent9527_code_mode::DEFAULT_EXEC_YIELD_TIME_MS);
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, agent9527_code_mode::ToolNamespaceDescription> {
    ...
    for spec in specs {
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| agent9527_code_mode::ToolNamespaceDescription {
...
fn append_extension_tool_executors(
    ...
    let tool_mode = effective_tool_mode(turn_context);
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly) {
        reserved_tool_names.insert(ToolName::plain(agent9527_code_mode::PUBLIC_TOOL_NAME));
        reserved_tool_names.insert(ToolName::plain(agent9527_code_mode::WAIT_TOOL_NAME));
    }
    if search_tool_enabled(turn_context)
        && planned_tools
            .runtimes()
...
impl ToolExecutor<ToolInvocation> for MultiAgentV2NamespaceOverride {
    ...
    fn handle(&self, invocation: ToolInvocation) -> agent9527_tools::ToolExecutorFuture<'_> {
...
fn compare_code_mode_tools(
    left: &agent9527_code_mode::ToolDefinition,
    right: &agent9527_code_mode::ToolDefinition,
    namespace_descriptions: &BTreeMap<String, agent9527_code_mode::ToolNamespaceDescription>,
) -> std::cmp::Ordering {
...
fn code_mode_namespace_name<'a>(
    tool: &agent9527_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, agent9527_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()

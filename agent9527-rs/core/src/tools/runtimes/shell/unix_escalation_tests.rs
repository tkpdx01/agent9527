...
use agent9527_execpolicy::PolicyParser;
...
use agent9527_sandboxing::policy_transforms::effective_permission_profile;
...
fn extract_shell_script_supports_wrapped_command_prefixes() {
    assert_eq!(
        extract_shell_script(&[
            "/usr/bin/env".into(),
            "AGENT9527_EXECVE_WRAPPER=1".into(),
            "/bin/zsh".into(),
            "-lc".into(),
            "echo hello".into()
        ])
...
async fn preapproved_additional_permissions_escalate_intercepted_exec() -> anyhow::Result<()> {
    ...
    let provider = CoreShellActionProvider {
        policy: Arc::new(RwLock::new(agent9527_execpolicy::Policy::empty())),
        ...
        stopwatch: agent9527_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = agent9527_shell_escalation::EscalationPolicy::determine_action(
        &provider,
    ...
    let expected = agent9527_shell_escalation::EscalationDecision::Escalate(
        EscalationExecution::Permissions(EscalationPermissions::ResolvedPermissionProfile(
            ResolvedPermissionProfile { permission_profile },
        )),
    );
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    std::fs::create_dir_all(&turn_context.config.agent9527_home)
        .context("recreate agent9527 home for hook fixtures")?;
    ...
    let expected_hook_command = agent9527_shell_command::parse_command::shlex_join(&[
        "/usr/bin/touch".to_string(),
        target_str,
    ]);
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(agent9527_execpolicy::Policy::empty())),
    ...
    let action = tokio::time::timeout(
        ...
        agent9527_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        agent9527_shell_escalation::EscalationDecision::Escalate(
            agent9527_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
...
"#
    ...
    assert_eq!(action, agent9527_shell_escalation::EscalationDecision::Run);
    Ok(())
}
...
async fn denied_reads_keep_granular_sandbox_rejection_for_escalation() -> anyhow::Result<()> {
    ...
    assert_eq!(
        action,
        agent9527_shell_escalation::EscalationDecision::Deny {

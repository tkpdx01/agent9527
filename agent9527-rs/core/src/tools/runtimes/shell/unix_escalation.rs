...
use agent9527_execpolicy::MatchOptions;
...
use agent9527_protocol::protocol::NetworkPolicyRuleAction;
...
use agent9527_shell_command::bash::parse_shell_lc_plain_commands;
use agent9527_shell_command::bash::parse_shell_lc_single_command_prefix;
use agent9527_shell_escalation::EscalateServer;
use agent9527_shell_escalation::EscalationDecision;
use agent9527_shell_escalation::EscalationExecution;
use agent9527_shell_escalation::EscalationPermissions;
use agent9527_shell_escalation::EscalationPolicy;
use agent9527_shell_escalation::EscalationPolicyFuture;
use agent9527_shell_escalation::EscalationSession;
use agent9527_shell_escalation::ExecParams;
use agent9527_shell_escalation::ExecResult;
use agent9527_shell_escalation::PreparedExec;
use agent9527_shell_escalation::ResolvedPermissionProfile;
use agent9527_shell_escalation::ShellCommandExecutor;
use agent9527_shell_escalation::ShellCommandExecutorFuture;
use agent9527_shell_escalation::Stopwatch;
...
pub(super) async fn try_run_zsh_fork(
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        sandbox_policy_cwd,
        windows_sandbox_workspace_roots,
        agent9527_linux_sandbox_exe: ctx.turn.config.agent9527_linux_sandbox_exe.clone(),
        ...
        Ok(stopwatch
            .pause_for(async move {
                ...
                let permission_request = PermissionRequestPayload::bash(
                    agent9527_shell_command::parse_command::shlex_join(&command),
                    /*description*/ None,
                );
...
struct CoreShellCommandExecutor {
    ...
    network: Option<agent9527_network_proxy::NetworkProxy>,
...
impl CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["AGENT9527_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            agent9527_linux_sandbox_exe: self.agent9527_linux_sandbox_exe.as_deref(),
...
fn map_exec_result(
    ...
    if result.timed_out {
        return Err(ToolError::Agent9527(Agent9527Err::Sandbox(
            SandboxErr::Timeout {

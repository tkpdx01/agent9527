...
*/
use crate::exec_env::AGENT9527_PERMISSION_PROFILE_ENV_VAR;
use crate::exec_env::AGENT9527_THREAD_ID_ENV_VAR;
...
pub(crate) use agent9527_network_proxy::is_managed_proxy_env_var;
pub(crate) use agent9527_network_proxy::strip_managed_proxy_env;
...
/// PATH entries owned by Agent9527 runtime setup.
///
...
/// runtime-only vars like `AGENT9527_THREAD_ID` without pretending they came from
/// the explicit override policy.
///
/// `runtime_path_prepends` contains Agent9527-owned PATH entries already applied to
...
pub(crate) fn maybe_wrap_shell_lc_with_snapshot(
    ...
    for key in [
        AGENT9527_THREAD_ID_ENV_VAR,
        AGENT9527_PERMISSION_PROFILE_ENV_VAR,
    ] {
    ...
    let (override_captures, override_exports) =
        build_override_exports(&override_env, &[AGENT9527_PERMISSION_PROFILE_ENV_VAR]);
...
fn build_override_exports(
    ...
    build_override_exports_for_keys("__AGENT9527_SNAPSHOT_OVERRIDE", &keys)
}
...
fn build_proxy_env_exports() -> (String, String) {
    ...
    let (captures, restores) =
        build_override_exports_for_keys("__AGENT9527_SNAPSHOT_PROXY_OVERRIDE", &keys);
    let key = PROXY_ACTIVE_ENV_KEY;
    let proxy_blocks = (
        format!("{captures}\n__AGENT9527_SNAPSHOT_PROXY_ENV_SET=\"${{{key}+x}}\""),
        format!(
            "if [ -n \"$__AGENT9527_SNAPSHOT_PROXY_ENV_SET\" ] || [ -n \"${{{key}+x}}\" ]; then\n{restores}\nfi"
        ),
    );
    let git_blocks = build_agent9527_proxy_git_ssh_command_exports();
    (
...
#[cfg(target_os = "macos")]
fn build_agent9527_proxy_git_ssh_command_exports() -> (String, String) {
    let key = PROXY_GIT_SSH_COMMAND_ENV_KEY;
    let marker_pattern = format!("{}\\ *", AGENT9527_PROXY_GIT_SSH_COMMAND_MARKER.trim_end());
    (
        format!(
            "__AGENT9527_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET=\"${{{key}+x}}\"\n__AGENT9527_SNAPSHOT_PROXY_GIT_SSH_COMMAND=\"${{{key}-}}\"\ncase \"$__AGENT9527_SNAPSHOT_PROXY_GIT_SSH_COMMAND\" in\n  {marker_pattern}) __AGENT9527_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED=1 ;;\n  *) __AGENT9527_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED= ;;\nesac"
        ),
        format!(
            "case \"${{{key}-}}\" in\n  {marker_pattern}) __AGENT9527_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED=1 ;;\n  *) __AGENT9527_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED= ;;\nesac\nif [ -n \"$__AGENT9527_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED\" ]; then\n  if [ -z \"${{{key}+x}}\" ] || [ -n \"$__AGENT9527_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n    export {key}=\"$__AGENT9527_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  fi\nelif [ -n \"$__AGENT9527_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n  if [ -n \"$__AGENT9527_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET\" ]; then\n    export {key}=\"$__AGENT9527_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  else\n    unset {key}\n  fi\nfi"
        ),
    )
}

...
use agent9527_exec_server::SelectedCapabilityRootsStatus;
...
use agent9527_thread_store::ThreadStoreResult;
...
/// Explains why `Agent9527Thread::try_start_turn_if_idle` rejected an automatic
/// idle turn.
...
pub struct Agent9527ThreadSettingsOverrides {
...
/// (formerly called a conversation) in Agent9527.
impl Agent9527Thread {
    pub(crate) fn new(
        session: Arc<Session>,
        io: SessionIo,
        session_configured: SessionConfiguredEvent,
    ...
    pub async fn submit(&self, op: Op) -> Agent9527Result<String> {
    ...
    pub async fn shutdown_and_wait(&self) -> Agent9527Result<()> {
    ...
    pub(crate) async fn emit_thread_resume_lifecycle(&self) {
        ...
        {
            contributor
                .on_thread_resume(agent9527_extension_api::ThreadResumeInput {
    ...
    /// callers that only hold a `Agent9527Thread`.
    ...
    pub async fn preview_thread_settings_overrides(
        &self,
        overrides: Agent9527ThreadSettingsOverrides,
    ...
    async fn thread_settings_update(
        ...
        let Agent9527ThreadSettingsOverrides {
            environments,
    ...
    pub async fn submit_with_id(&self, sub: Submission) -> Agent9527Result<()> {
        self.io.submit_with_id(sub).await
    }

    pub async fn next_event(&self) -> Agent9527Result<Event> {
    ...
    pub async fn inject_response_items(&self, items: Vec<ResponseItem>) -> Agent9527Result<()> {
        if items.is_empty() {
    ...
    ) -> (agent9527_mcp::McpConfig, agent9527_mcp::McpRuntimeContext) {
    ...
    pub async fn current_mcp_config_and_runtime_context(
        ...
        Arc<agent9527_mcp::McpConfig>,
        agent9527_mcp::McpRuntimeContext,
    ) {
    ...
    pub async fn increment_out_of_band_elicitation_count(&self) -> Agent9527Result<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        let incremented = elicitations.count.checked_add(1).ok_or_else(|| {
            Agent9527Err::Fatal("out-of-band elicitation count overflowed".to_string())
        })?;
    ...
    pub async fn decrement_out_of_band_elicitation_count(&self) -> Agent9527Result<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        if elicitations.count == 0 {
            return Err(Agent9527Err::InvalidRequest(

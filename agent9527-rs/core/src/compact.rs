    ...
    loop {
        ...
        match attempt_result {
            ...
            Err(err)
                if matches!(
                    err.details(),
                    Agent9527ErrorDetails::Interrupted | Agent9527ErrorDetails::TurnAborted
                ) =>
            {
                return Err(err);
            }
            Err(e) if matches!(e.details(), Agent9527ErrorDetails::SessionBudgetExceeded) => {
                sess.track_turn_agent9527_error(turn_context.as_ref(), &e);
                let event = EventMsg::Error(e.to_error_event(/*message_prefix*/ None));
                sess.send_event(&turn_context, event).await;
                return Err(e);
            }
            Err(e) if matches!(e.details(), Agent9527ErrorDetails::ContextWindowExceeded) => {
                if turn_input_len > 1 {

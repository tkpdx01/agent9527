...
fn test_responses_metadata_json(
    ...
    window_id: &str,
    request_kind: Agent9527ResponsesRequestKind,
) -> String {
    state
...
fn test_turn_responses_metadata_json(state: &TurnMetadataState, window_id: &str) -> String {
    test_responses_metadata_json(state, window_id, Agent9527ResponsesRequestKind::Turn)
}
...
fn test_compaction_responses_metadata_json(
    ...
    test_responses_metadata_json(
        state,
        window_id,
        Agent9527ResponsesRequestKind::Compaction(compaction),
    )
}
...
fn turn_metadata_state_merges_client_metadata_without_replacing_reserved_fields() {
    ...
    state.set_responsesapi_client_metadata(HashMap::from([
        ...
        (
            "x-agent9527-installation-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
            "x-agent9527-parent-thread-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
    ...
    assert!(json.get("x-agent9527-installation-id").is_none());
    assert!(json.get("x-agent9527-parent-thread-id").is_none());

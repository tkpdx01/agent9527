...
This crate implements the business logic for Agent9527. It is designed to be used by the various Agent9527 UIs written in Rust.

## Wine-exec integration tests

On x86-64 Linux, run the shared suite against the Windows exec server with
`bazel test //agent9527-rs/core:core-all-wine-exec-test`.
...
Note that `agent9527-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.agent9527` read-only.
...
Expects the binary containing `agent9527-core` to run the equivalent of `agent9527 sandbox` when `arg0` is `agent9527-linux-sandbox`. See the `agent9527-arg0` crate for details.
...
`bwrap` is missing, it falls back to the bundled `agent9527-resources/bwrap`
binary shipped with Agent9527 and Agent9527 surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
Agent9527 also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so Agent9527 rejects sandboxed shell commands that would enter the
...
Expects the binary containing `agent9527-core` to simulate the virtual
`apply_patch` CLI when `arg1` is `--agent9527-run-as-apply-patch`. See the
`agent9527-arg0` crate for details.

...
use crate::test_agent9527::TestAgent9527;
...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) =
        agent9527_utils_cargo_bin::cargo_bin("agent9527-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `agent9527-execve-wrapper` binary");
        return Ok(None);
    };
...
pub fn zsh_fork_test_builder(
    ...
    test_agent9527().with_config(move |config| {

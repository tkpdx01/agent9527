...
use agent9527_utils_cargo_bin::CargoBinError;
...
pub use agent9527_core::test_support::TestAgent9527ResponsesRequestKind;
pub use agent9527_core::test_support::responses_metadata;
use agent9527_utils_absolute_path::AbsolutePathBuf;
pub use agent9527_utils_absolute_path::test_support::PathBufExt;
pub use agent9527_utils_absolute_path::test_support::PathExt;
...
pub mod test_agent9527;
pub mod test_agent9527_exec;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    agent9527_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    agent9527_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(agent9527_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = agent9527_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("agent9527-rs"));
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\agent9527\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.agent9527`.
pub async fn load_default_config_for_test(agent9527_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        agent9527_linux_sandbox_exe: Some(
            find_agent9527_linux_sandbox_exe()
                .expect("should find binary for agent9527-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_agent9527_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    agent9527_utils_cargo_bin::cargo_bin("agent9527-linux-sandbox")
}

pub async fn wait_for_event<F>(
    agent9527: &Agent9527Thread,
    predicate: F,
) -> agent9527_protocol::protocol::EventMsg
where
    F: FnMut(&agent9527_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(agent9527, predicate, Duration::from_secs(1)).await
}
...
pub async fn submit_thread_settings(
    ...
    let submission_id = agent9527
        ...
        .await?;
    loop {
        let ev = timeout(Duration::from_secs(10), agent9527.next_event())
            .await
...
pub async fn wait_for_event_match<T, F>(agent9527: &Agent9527Thread, matcher: F) -> T
where
    F: Fn(&agent9527_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(agent9527, |ev| matcher(ev).is_some()).await;
...
pub fn sandbox_env_var() -> &'static str {
    agent9527_core::spawn::AGENT9527_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    agent9527_core::spawn::AGENT9527_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

pub fn format_with_current_shell(command: &str) -> Vec<String> {
    agent9527_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    agent9527_utils_cargo_bin::cargo_bin("test_stdio_server")
...
#[macro_export]
macro_rules! agent9527_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_agent9527_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("agent9527-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }

...
use agent9527_core::TimeProvider;
use agent9527_core::config::Config;
use agent9527_core::resolve_installation_id;
use agent9527_core::shell::Shell;
use agent9527_core::shell::get_shell_by_model_provided_path;
use agent9527_core::thread_store_from_config;
use agent9527_exec_server::CreateDirectoryOptions;
use agent9527_exec_server::ExecutorFileSystem;
use agent9527_exec_server::RemoveOptions;
...
use agent9527_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
...
type WorkspaceSetup = dyn FnOnce(AbsolutePathBuf, Arc<dyn ExecutorFileSystem>) -> BoxFuture<'static, Result<()>>
    + Send;
const TEST_MODEL_WITH_EXPERIMENTAL_TOOLS: &str = "test-gpt-5.1-agent9527";
const REMOTE_EXEC_SERVER_URL_ENV_VAR: &str = "AGENT9527_TEST_REMOTE_EXEC_SERVER_URL";
...
#[derive(Debug)]
pub struct TestEnv {
    environment: agent9527_exec_server::Environment,
...
impl TestEnv {
    ...
    pub async fn local_with_exec_server_url(exec_server_url: Option<String>) -> Result<Self> {
        ...
        let selection = match exec_server_url {
            Some(_) => TurnEnvironmentSelection {
                environment_id: agent9527_exec_server::REMOTE_ENVIRONMENT_ID.to_string(),
                cwd: PathUri::from_abs_path(&cwd),
                workspace_roots: vec![PathUri::from_abs_path(&cwd)],
            },
        ...
        let environment =
            agent9527_exec_server::Environment::create_for_tests(exec_server_url.clone())?;
        Ok(Self {
            environment,
            exec_server_url,
            cwd,
    ...
    pub fn environment(&self) -> &agent9527_exec_server::Environment {
        &self.environment
    }
...
pub async fn test_env() -> Result<TestEnv> {
    match test_environment() {
        remote_env @ (TestEnvironment::Docker { .. } | TestEnvironment::WineExec) => {
            ...
            let environment =
                agent9527_exec_server::Environment::create_for_tests(Some(websocket_url.clone()))?;
            let cwd = remote_env
...
pub struct TestAgent9527Builder {
    ...
    use_bundled_model_catalog: bool,
}

impl TestAgent9527Builder {
    ...
    pub fn with_auth(mut self, auth: Agent9527Auth) -> Self {
        self.auth = auth;
        self
    }
    ...
    pub fn with_dynamic_model_catalog(mut self) -> Self {
        self.use_bundled_model_catalog = false;
        self
    }
    ...
    pub async fn build(&mut self, server: &wiremock::MockServer) -> anyhow::Result<TestAgent9527> {
    ...
    ///
    /// With no remote test configuration, or with `AGENT9527_TEST_ENVIRONMENT=local`, this uses a
    /// temporary local environment just like [`Self::build`]. `AGENT9527_TEST_ENVIRONMENT=docker` or
    /// `AGENT9527_TEST_ENVIRONMENT=wine-exec` selects the remote exec server configured by
    /// `AGENT9527_TEST_REMOTE_EXEC_SERVER_URL`; the legacy `AGENT9527_TEST_REMOTE_ENV` Docker-container
    ...
    async fn build_with_home_and_base_url(
        ...
        #[cfg(target_os = "linux")]
        let agent9527_linux_sandbox_exe = Some(
            crate::find_agent9527_linux_sandbox_exe()
                .context("should find binary for agent9527-linux-sandbox")?,
        );
        #[cfg(not(target_os = "linux"))]
        let agent9527_linux_sandbox_exe = None;
        let local_runtime_paths = agent9527_exec_server::ExecServerRuntimePaths::new(
            std::env::current_exe()?,
            agent9527_linux_sandbox_exe,
        )?;
        let environment_manager = Arc::new(if include_local_environment {
            agent9527_exec_server::EnvironmentManager::create_for_tests_with_local(
                exec_server_url,
                local_runtime_paths,
            )
            .await
        } else {
            agent9527_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        resume_from: Option<PathBuf>,
        test_env: TestEnv,
        environment_manager: Arc<agent9527_exec_server::EnvironmentManager>,
    ) -> anyhow::Result<TestAgent9527> {
        let auth = self.auth.clone();
        let state_db = agent9527_core::init_state_db(&config).await;
        let thread_store = thread_store_from_config(&config, state_db.clone());
        let installation_id = resolve_installation_id(&config.agent9527_home).await?;
        ...
        let auth_manager = agent9527_core::test_support::auth_manager_from_auth(auth.clone());
        let thread_manager = ThreadManager::new(
            ...
            thread_store,
            agent9527_core::local_agent_graph_store_from_state_db(state_db.as_ref()),
            installation_id,
        ...
        let code_mode_host_program = self
            ...
            .take()
            .or_else(|| agent9527_utils_cargo_bin::cargo_bin("agent9527-code-mode-host").ok());
        let thread_manager = if config.features.enabled(Feature::CodeModeHost)
            && let Some(code_mode_host_program) = code_mode_host_program
        {
            agent9527_core::test_support::with_code_mode_host_program(
                thread_manager,
                code_mode_host_program,
                &config,
            )
        ...
        let new_conversation = match (resume_from, user_shell_override) {
            (Some(path), Some(user_shell_override)) => {
                ...
                Box::pin(
                    agent9527_core::test_support::resume_thread_from_rollout_with_user_shell_override(
            ...
            (None, Some(user_shell_override)) => {
                Box::pin(
                    agent9527_core::test_support::start_thread_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        user_shell_override,
                        self.supports_openai_form_elicitation,
        ...
        Ok(TestAgent9527 {
            ...
            cwd,
            config,
            agent9527: new_conversation.thread,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = agent9527_utils_cargo_bin::cargo_bin("agent9527") {
            config.agent9527_self_exe = Some(path);
        } else if let Ok(path) = agent9527_utils_cargo_bin::cargo_bin("agent9527-exec") {
            // `agent9527-exec` also supports `--agent9527-run-as-apply-patch`, so use it
        ...
        {
            let agent9527 = bin_dir.join("agent9527");
            let agent9527_exec = bin_dir.join("agent9527-exec");
            if agent9527.is_file() {
                config.agent9527_self_exe = Some(agent9527);
            } else if agent9527_exec.is_file() {
                config.agent9527_self_exe = Some(agent9527_exec);
            }
        }
        ...
        if self.use_bundled_model_catalog && config.model_catalog.is_none() {
            config.model_catalog = Some(
                upstream_bundled_models_response().context("bundled models.json should parse")?,
            );
        }
...
pub struct TestAgent9527 {
    pub home: Arc<TempDir>,
    pub cwd: Arc<TempDir>,
    pub agent9527: Arc<Agent9527Thread>,
...
impl TestAgent9527 {
    ...
    pub fn agent9527_home_path(&self) -> &Path {
        self.config.agent9527_home.as_path()
    }
    ...
    async fn submit_turn_with_context(
        ...
        let turn_id = wait_for_event_match(&self.agent9527, |event| match event {
...
pub struct TestAgent9527Harness {
    server: MockServer,
    test: TestAgent9527,
}

impl TestAgent9527Harness {
    pub async fn with_builder(mut builder: TestAgent9527Builder) -> Result<Self> {
        let server = start_mock_server().await;
        let test = builder.build(&server).await?;
        Ok(Self { server, test })
    }

    pub async fn with_auto_env_builder(mut builder: TestAgent9527Builder) -> Result<Self> {
        let server = start_mock_server().await;
    ...
    pub fn test(&self) -> &TestAgent9527 {
        &self.test
    }
...
pub fn test_agent9527() -> TestAgent9527Builder {
    TestAgent9527Builder {
        ...
        auth: Agent9527Auth::from_api_key("dummy"),
        pre_build_hooks: vec![],
        ...
        history_mode: None,
        use_bundled_model_catalog: true,
    }
}

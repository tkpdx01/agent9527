...
use agent9527_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use agent9527_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use agent9527_core::Agent9527ResponsesMetadata;
use agent9527_core::ModelClient;
use agent9527_core::ModelClientSession;
use agent9527_core::Prompt;
use agent9527_core::ResponseEvent;
use agent9527_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
...
use agent9527_rollout_trace::ConversationPart;
use agent9527_rollout_trace::InferenceTraceContext;
use agent9527_rollout_trace::RawTraceEventPayload;
use agent9527_rollout_trace::TraceWriter;
...
const MODEL: &str = MODEL_BALANCED;
...
fn turn_metadata(
    ...
    responses_metadata(harness, turn_id, TestAgent9527ResponsesRequestKind::Turn)
}
...
fn prewarm_metadata(
    ...
    responses_metadata(harness, turn_id, TestAgent9527ResponsesRequestKind::Prewarm)
}

fn websocket_connection_metadata(harness: &WebsocketTestHarness) -> Agent9527ResponsesMetadata {
    responses_metadata(
        harness,
        /*turn_id*/ None,
        TestAgent9527ResponsesRequestKind::WebsocketConnection,
    )
}
...
async fn responses_websocket_streams_request() {
    ...
    assert_eq!(
        ...
        Some(agent9527_login::default_client::get_agent9527_user_agent())
    );
    assert_eq!(
        body["client_metadata"]["x-agent9527-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    let stream_request_start_ms = body["client_metadata"]
        [X_AGENT9527_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY]
        .as_str()
...
async fn responses_websocket_preconnect_reuses_connection() {
    ...
    assert_eq!(
        server.single_handshake().header("x-agent9527-window-id"),
...
async fn responses_websocket_request_prewarm_reuses_connection() {
    ...
    let warmup_turn_metadata: serde_json::Value = serde_json::from_str(
        warmup["client_metadata"]["x-agent9527-turn-metadata"]
            .as_str()
...
async fn responses_websocket_request_prewarm_traces_logical_request() {
    ...
    writer
        .append(RawTraceEventPayload::Agent9527TurnStarted {
            agent9527_turn_id: "turn-1".to_string(),
            thread_id: harness.thread_id.to_string(),
        })
        .expect("turn started");
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let usage_limit_error = json!({
        ...
        "headers": {
            "x-agent9527-primary-used-percent": "100.0",
            "x-agent9527-secondary-used-percent": "87.5",
            "x-agent9527-primary-over-secondary-limit-percent": "95.0",
            "x-agent9527-primary-window-minutes": "15",
            "x-agent9527-secondary-window-minutes": "60",
            "x-agent9527-credits-has-credits": "true",
            "x-agent9527-credits-unlimited": "false",
            "x-agent9527-credits-balance": "",
            "x-agent9527-rate-limit-reached-type": "workspace_member_usage_limit_reached"
        }
    });
    ...
    let test = builder
        ...
        .await
        .expect("build websocket agent9527");
    ...
    let token_event = wait_for_event(&test.agent9527, |msg| {
    ...
    let error_event =
        wait_for_event(&test.agent9527, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    let first_metadata: serde_json::Value = serde_json::from_str(
        first["client_metadata"]["x-agent9527-turn-metadata"]
            .as_str()
    ...
    let second_metadata: serde_json::Value = serde_json::from_str(
        second["client_metadata"]["x-agent9527-turn-metadata"]
            .as_str()
...
async fn responses_websocket_sends_canonical_turn_metadata() {
    ...
    let turn_metadata: serde_json::Value = serde_json::from_str(
        body["client_metadata"]["x-agent9527-turn-metadata"]
            .as_str()
...
async fn websocket_harness_with_provider_options(
    ...
    let model_info = agent9527_core::test_support::construct_model_info_offline(MODEL, &config);

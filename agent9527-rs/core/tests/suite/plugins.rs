...
use agent9527_core_plugins::store::PluginStore;
...
const SAMPLE_PLUGIN_APP_NAMESPACE: &str = "mcp__agent9527_apps__google_calendar";
...
async fn build_analytics_plugin_test_agent9527(
    server: &MockServer,
    agent9527_home: Arc<TempDir>,
) -> Result<TestAgent9527> {
    let chatgpt_base_url = server.uri();
    let mut builder = test_agent9527()
        .with_home(agent9527_home)
...
async fn build_apps_enabled_plugin_test_agent9527(
    server: &MockServer,
    agent9527_home: Arc<TempDir>,
...
async fn persisted_remote_plugin_command_attribution_flows_through_turn_context() -> Result<()> {
    ...
    skip_if_remote!(
        Ok(()),
        "remote plugin attribution fixture uses a local Agent9527 home cache"
    );

    let server = start_mock_server().await;
    let agent9527_home = Arc::new(TempDir::new()?);
    let script_path = write_remote_plugin_script_and_config(agent9527_home.as_ref());
    ...
    let mut builder = test_agent9527()
        .with_home(Arc::clone(&agent9527_home))
        .with_model("gpt-5.2");
    let test_agent9527 = builder.build_with_auto_env(&server).await?;
    let agent9527 = Arc::clone(&test_agent9527.agent9527);
    let cwd = test_agent9527.config.cwd.clone();
    let session_model = test_agent9527.session_configured.model.clone();
    let (sandbox_policy, permission_profile) =
        turn_permission_fields(PermissionProfile::Disabled, cwd.as_path());
    agent9527
        .submit(Op::UserInput {
            items: vec![agent9527_protocol::user_input::UserInput::Text {
    ...
    let begin = wait_for_event_match(&agent9527, |event| match event {
        EventMsg::ExecCommandBegin(event) if event.call_id == call_id => Some(event.clone()),
        _ => None,
    })
    .await;
    let end = wait_for_event_match(&agent9527, |event| match event {
...
async fn capability_sections_render_in_developer_message_in_order() -> Result<()> {
    ...
    let agent9527_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(agent9527_home.as_ref());
    write_plugin_app_plugin(agent9527_home.as_ref());
    let test_agent9527 = build_apps_enabled_plugin_test_agent9527(
        &server,
        Arc::clone(&agent9527_home),
...
async fn explicit_plugin_mentions_use_apps_for_chatgpt_dual_surface_plugins() -> Result<()> {
    ...
    write_plugin_skill_plugin(agent9527_home.as_ref());
    write_plugin_mcp_plugin(agent9527_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(agent9527_home.as_ref());
    ...
    .await?;
    let agent9527 = Arc::clone(&test_agent9527.agent9527);
    wait_for_mcp_server(&agent9527, AGENT9527_APPS_MCP_SERVER_NAME).await?;

    agent9527
        .submit(Op::UserInput {
            items: vec![agent9527_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
...
async fn explicit_plugin_mentions_keep_non_conflicting_mcp_for_chatgpt_auth() -> Result<()> {
    ...
    write_plugin_skill_plugin(agent9527_home.as_ref());
    write_plugin_mcp_plugin(agent9527_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin_with_name(agent9527_home.as_ref(), "sample_app");
    ...
    .await?;
    let agent9527 = Arc::clone(&test_agent9527.agent9527);
    wait_for_mcp_server(&agent9527, "sample").await?;
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let agent9527_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(agent9527_home.as_ref());
    let test_agent9527 = build_analytics_plugin_test_agent9527(&server, agent9527_home).await?;
    let agent9527 = Arc::clone(&test_agent9527.agent9527);
    ...
    let event = wait_for_analytics_event(&server, "agent9527_plugin_used").await;
    ...
    assert_eq!(
        ...
        serde_json::json!(agent9527_login::default_client::originator().value)
    );
...
async fn explicit_plugin_skill_invocation_tracks_remote_plugin_id() -> Result<()> {
    ...
    let agent9527_home = Arc::new(TempDir::new()?);
    let skill_path =
        std::fs::canonicalize(write_remote_plugin_skill_plugin(agent9527_home.as_ref()))?;
    persist_sample_remote_plugin_id(agent9527_home.as_ref());
    let test_agent9527 = build_analytics_plugin_test_agent9527(&server, agent9527_home).await?;
    let agent9527 = Arc::clone(&test_agent9527.agent9527);
...
async fn implicit_plugin_skill_invocation_tracks_remote_plugin_id() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let agent9527_home = Arc::new(TempDir::new()?);
    let skill_path = write_remote_plugin_skill_plugin(agent9527_home.as_ref());
    persist_sample_remote_plugin_id(agent9527_home.as_ref());
...
async fn wait_for_analytics_event(server: &MockServer, event_type: &str) -> serde_json::Value {
    ...
    loop {
        ...
        if let Some(event) = requests
            .into_iter()
            .filter(|request| request.url.path() == "/agent9527/analytics-events/events")
            .find_map(|request| {

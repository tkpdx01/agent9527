...
use agent9527_core::config::CurrentTimeReminderConfig;
use agent9527_extension_items::ExtensionItem;
use agent9527_extension_items::sleep::SleepItem;
...
async fn build_agent9527(server: &StreamingSseServer) -> Arc<Agent9527Thread> {
    test_agent9527()
        .with_model("gpt-5.4")
        .build_with_streaming_server(server)
        .await
        .expect("build streaming Agent9527 test session")
        .agent9527
}

async fn submit_user_input(agent9527: &Agent9527Thread, text: &str) {
    agent9527
        .submit(Op::UserInput {
...
async fn submit_danger_full_access_user_turn(test: &TestAgent9527, text: &str) {
...
async fn steer_user_input(agent9527: &Agent9527Thread, text: &str) {
    agent9527
        .steer_input(
            vec![UserInput::Text {
                text: text.to_string(),
...
async fn enqueue_queue_only_agent_mail(agent9527: &Agent9527Thread, text: &str) {
    agent9527
...
async fn submit_queue_only_agent_mail(agent9527: &Agent9527Thread, text: &str) {
    enqueue_queue_only_agent_mail(agent9527, text).await;
    agent9527
...
async fn wait_for_reasoning_item_started(agent9527: &Agent9527Thread) {
    wait_for_event(agent9527, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(agent9527: &Agent9527Thread, text: &str) {
...
async fn wait_for_turn_complete(agent9527: &Agent9527Thread) {
    wait_for_event(agent9527, |event| {
...
async fn wait_for_sleep_item_started(agent9527: &Agent9527Thread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(agent9527, |event| {
        matches!(
            event,
...
impl agent9527_extension_api::ThreadLifecycleContributor<agent9527_core::config::Config>
    for SleepingRootExtension
{
    fn on_thread_start<'a>(
        &'a self,
        input: agent9527_extension_api::ThreadStartInput<'a, agent9527_core::config::Config>,
...
async fn queue_only_agent_mail_wakes_sleeping_root_and_persists_message() {
    ...
    let mut extensions =
        agent9527_extension_api::ExtensionRegistryBuilder::<agent9527_core::config::Config>::new();
    ...
    let agent9527 = test_agent9527()
        ...
        .build_with_streaming_server(&server)
        .await
        .expect("build Agent9527 test session")
        .agent9527;

    enqueue_queue_only_agent_mail(&agent9527, CHILD_MESSAGE).await;
    wait_for_turn_complete(&agent9527).await;

    assert_eq!(server.requests().await.len(), 1);
    let history = agent9527
    ...
    assert!(history.items.iter().any(|item| {
        matches!(
            item,
            RolloutItem::ResponseItem(agent9527_protocol::models::ResponseItem::AgentMessage {
                content,
                ..
            }) if content.iter().any(|content| matches!(
                content,
                agent9527_protocol::models::AgentMessageInputContent::InputText { text }
...
async fn steer_interrupts_wait_agent_and_is_sent_in_follow_up_request() {
    ...
    submit_user_input(&agent9527, INITIAL_PROMPT).await;
    wait_for_event(&agent9527, |event| {
        matches!(event, EventMsg::CollabWaitingBegin(_))
    })
    .await;

    steer_user_input(&agent9527, STEER_PROMPT).await;
    wait_for_turn_complete(&agent9527).await;
...
async fn any_new_input_interrupts_sleep() {
    ...
    submit_user_input(&agent9527, INITIAL_PROMPT).await;
    wait_for_sleep_item_started(&agent9527, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    steer_user_input(&agent9527, STEER_PROMPT).await;
    wait_for_sleep_item_completed(&agent9527, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_sleep_item_started(&agent9527, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    submit_queue_only_agent_mail(&agent9527, "new mailbox input").await;
    wait_for_sleep_item_completed(&agent9527, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_turn_complete(&agent9527).await;
    ...
    let rollout_path = agent9527.rollout_path().expect("rollout path");
...
async fn queued_inter_agent_mail_triggers_follow_up_after_reasoning_item() {
    ...
    let agent9527 = build_agent9527(&server).await;

    submit_user_input(&agent9527, "first prompt").await;

    wait_for_reasoning_item_started(&agent9527).await;

    submit_queue_only_agent_mail(&agent9527, "queued child update").await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_commentary_message_item() {
    ...
    wait_for_agent_message(&agent9527, "first answer").await;
...
async fn queued_inter_agent_mail_does_not_restart_after_final_answer() {
    ...
    submit_user_input(&agent9527, "second prompt").await;
    wait_for_turn_complete(&agent9527).await;
...
async fn user_input_does_not_preempt_after_reasoning_item() {
    ...
    steer_user_input(&agent9527, "second prompt").await;
...
async fn steered_user_input_waits_for_model_continuation_after_mid_turn_compact() {
    ...
    wait_for_agent_message(&agent9527, "resumed old task").await;
    wait_for_turn_complete(&agent9527).await;
...
async fn steered_user_input_follows_compact_when_only_the_steer_needs_follow_up() {
    ...
    wait_for_agent_message(&agent9527, "processed steered prompt").await;
    wait_for_turn_complete(&agent9527).await;

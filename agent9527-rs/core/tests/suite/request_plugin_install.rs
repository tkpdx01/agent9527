...
use agent9527_protocol::approvals::ElicitationAction;
...
const CALENDAR_NAMESPACE: &str = "mcp__agent9527_apps__calendar";
...
fn configure_apps_without_search_tool(config: &mut Config, apps_base_url: &str) {
    ...
    let mut model_catalog = upstream_bundled_models_response()
...
async fn start_install_turn(test: &TestAgent9527, prompt: &str) -> Result<ElicitationRequestEvent> {
    ...
    Ok(wait_for_event_match(&test.agent9527, |event| match event {
        ...
        if let Some(event) = requests
            ...
            .find_map(|request| {
                ...
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "agent9527_plugin_install_requested")
                        .cloned()
                })
            })
        {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": turn_id,
                "model_slug": "gpt-5.4",
                "product_client_id": agent9527_login::default_client::originator().value,
            }
        })
    );

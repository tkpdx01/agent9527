...
use core_test_support::test_agent9527::ApplyPatchModelOutput;
...
pub async fn apply_patch_harness() -> Result<TestAgent9527Harness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestAgent9527Builder) -> TestAgent9527Builder,
) -> Result<TestAgent9527Harness> {
    let builder = configure(test_agent9527());
    // Box harness construction so apply_patch_cli tests do not inline the
    // full test-thread startup path into each test future.
    Box::pin(TestAgent9527Harness::with_auto_env_builder(builder)).await
}

async fn submit_without_wait(harness: &TestAgent9527Harness, prompt: &str) -> Result<()> {
...
async fn apply_patch_cli_uses_agent9527_self_exe_with_linux_sandbox_helper_alias() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let harness = apply_patch_harness().await?;
    let agent9527_linux_sandbox_exe = harness
        .test()
        .config
        .agent9527_linux_sandbox_exe
        .as_ref()
        .expect("linux test config should include agent9527-linux-sandbox helper");
    assert_eq!(
        agent9527_linux_sandbox_exe
            .file_name()
...
async fn apply_patch_custom_tool_streaming_emits_updated_changes() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&agent9527_protocol::protocol::FileChange::Add {
            content: String::new(),
        })
    );
    assert_eq!(
...
async fn apply_patch_turn_diff_tracks_local_and_remote_environment_paths() -> Result<()> {
    ...
    let shared_cwd = PathBuf::from(format!(
        "/tmp/agent9527-remote-turn-diff-{}",

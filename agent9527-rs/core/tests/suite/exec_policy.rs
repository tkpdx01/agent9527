...
async fn submit_user_turn(
    test: &core_test_support::test_agent9527::TestAgent9527,
    prompt: &str,
        ...
        .submit(Op::UserInput {
            ...
            thread_settings: agent9527_protocol::protocol::ThreadSettingsOverrides {
                ...
                collaboration_mode: collaboration_mode.or({
                    Some(agent9527_protocol::config_types::CollaborationMode {
...
"#;
    ...
    let mut migrated_builder = test_agent9527().with_config(|config| {
        let policy_path = config.agent9527_home.join("rules/default.rules");
    ...
    let migrated_policy_path = migrated.agent9527_home_path().join("rules/default.rules");
    ...
    let mut ignored_builder = test_agent9527().with_config(|config| {
        let policy_path = config.agent9527_home.join("rules/default.rules");
        fs::create_dir_all(policy_path.parent().expect("rules directory"))
            .expect("create rules directory");
        fs::write(policy_path, LEGACY_POLICY).expect("write legacy policy");
    ...
    let ignored_policy_path = ignored.agent9527_home_path().join("rules/default.rules");
...
async fn granular_complex_forced_rm_requests_approval_when_allowed() -> Result<()> {
    ...
    let approval_event = wait_for_event(&test.agent9527, |event| {
        matches!(
            event,
...
async fn execpolicy_blocks_shell_invocation() -> Result<()> {
    let mut builder = test_agent9527().with_config(|config| {
        let policy_path = config.agent9527_home.join("rules").join("policy.rules");
        fs::create_dir_all(
            policy_path
                .parent()
    ...
    let EventMsg::ExecCommandEnd(end) = wait_for_event(&test.agent9527, |event| {
...
"#,
        ...
        .with_config(|config| {
            config
                ...
                .enable(Feature::UnifiedExec)
                .expect("test config should allow feature update");
            let policy_path = config.agent9527_home.join("rules").join("broken.rules");
            fs::create_dir_all(
                policy_path
                    .parent()
                    .expect("policy directory must have a parent"),

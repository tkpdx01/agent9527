...
async fn user_turn_includes_skill_instructions() -> Result<()> {
    ...
    let mut builder = test_agent9527().with_workspace_setup(move |cwd, fs| async move {
    ...
    core_test_support::wait_for_event(test.agent9527.as_ref(), |event| {
        matches!(
            event,
            agent9527_protocol::protocol::EventMsg::TurnComplete(_)
        )
    })
    .await;

...
//! `just test -p agent9527-core --test all --run-ignored only live_cli` provided they set a valid
//! `OPENAI_API_KEY`.
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let agent9527_home = home.path().join(".agent9527");
    ...
    let mut cmd = Command::new(agent9527_utils_cargo_bin::cargo_bin("agent9527-rs").unwrap());
    ...
    let mut child = cmd.spawn().expect("failed to spawn agent9527-rs");

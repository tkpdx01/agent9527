...
use agent9527_protocol::protocol::GitInfo;
...
const EXTERNAL_API_TOKEN: &str = "at-cli-test";
const EXTERNAL_API_TOKEN_AUTHORIZATION: &str = "Bearer at-cli-test";
const TEST_API_KEY_ENV_VAR: &str = "AGENT9527_TEST_API_KEY";
const CLI_TIMEOUT: Duration = Duration::from_secs(30);

fn repo_root() -> std::path::PathBuf {
    agent9527_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
fn add_external_provider_config(command: &mut Command, base_url: &str, env_key: &str) {
    command
        .arg("-c")
        .arg(format!(
            "model_providers.mock={{ name = \"mock\", base_url = \"{base_url}\", env_key = \"{env_key}\", wire_api = \"responses\", requires_openai_auth = false }}"
        ))
        .arg("-c")
        .arg("model_provider=\"mock\"");
}

fn request_contains_instructions(body: &serde_json::Value, marker: &str) -> bool {
    body.get("instructions")
        .and_then(serde_json::Value::as_str)
        .is_some_and(|instructions| instructions.contains(marker))
        || body
            .get("input")
...
fn external_provider_exec_command(server: &MockServer, home: &TempDir) -> Command {
    let bin = agent9527_utils_cargo_bin::cargo_bin("agent9527").unwrap();
    let mut cmd = Command::new(bin);
    cmd.arg("exec").arg("--skip-git-repo-check");
    add_external_provider_config(
        &mut cmd,
        &format!("{}/api/agent9527", server.uri()),
        TEST_API_KEY_ENV_VAR,
    );
    cmd.arg("-C").arg(repo_root()).arg("hello?");
    cmd.env("AGENT9527_HOME", home.path())
        .env(TEST_API_KEY_ENV_VAR, EXTERNAL_API_TOKEN)
        .env_remove(AGENT9527_ACCESS_TOKEN_ENV_VAR)
        .env_remove(AGENT9527_API_KEY_ENV_VAR)
...
impl Drop for ChildProcessCleanupGuard {
    fn drop(&mut self) {
        #[cfg(unix)]
        {
            let _ = agent9527_utils_pty::process_group::kill_process_group(self.0);
        }
...
// Use this for new `agent9527 exec` subprocess tests in this file. These commands
...
async fn responses_mode_stream_cli_supports_external_bearer_tokens() {
    skip_if_no_network!();
    ...
    let mut cmd = external_provider_exec_command(&server, &home);
    let output = run_cli_command(&mut cmd).unwrap();

    assert!(
        output.status.success(),
        "agent9527-cli exec failed: {}",
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn responses_mode_stream_cli_does_not_attempt_oauth_refresh_for_external_tokens_after_401() {
    skip_if_no_network!();

    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/api/agent9527/responses"))
        .and(header("authorization", EXTERNAL_API_TOKEN_AUTHORIZATION))
...
async fn responses_mode_stream_cli() {
    ...
    cmd.arg("-C").arg(&repo_root).arg("hello?");
    cmd.env("AGENT9527_HOME", home.path())
        .env(TEST_API_KEY_ENV_VAR, "dummy");
...
/// Ensures explicit external provider configuration routes Responses API requests.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn responses_mode_stream_cli_supports_external_provider_config() {
    skip_if_no_network!();
...
async fn exec_cli_applies_model_instructions_file() {
    ...
    // included in either the classic field or the Responses Lite developer input.
    let request = resp_mock.single_request();
    let body = request.body_json();
    assert!(
        request_contains_instructions(&body, marker),
        "request did not contain custom instructions marker; got: {body}"
    );
}

/// Verify that `agent9527 exec --profile ...` preserves the active user config
...
async fn exec_cli_profile_applies_model_instructions_file() {
    ...
    assert!(
        request_contains_instructions(&body, marker),
        "request did not contain profile instructions marker; got: {body}"
    );
}
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the agent9527 CLI and invoke `exec`, which is what records a session.
    let bin = agent9527_utils_cargo_bin::cargo_bin("agent9527").unwrap();
    let mut cmd = Command::new(bin);
    cmd.arg("exec").arg("--skip-git-repo-check");
    add_external_provider_config(
    ...
    let bin2 = agent9527_utils_cargo_bin::cargo_bin("agent9527").unwrap();
    let mut cmd2 = Command::new(bin2);
    cmd2.arg("exec").arg("--skip-git-repo-check");
    add_external_provider_config(
        &mut cmd2,
        &format!("{}/v1", server.uri()),
        TEST_API_KEY_ENV_VAR,
    ...
    cmd2.arg("-C")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("AGENT9527_HOME", home.path())
        .env(TEST_API_KEY_ENV_VAR, "dummy");

    let output2 = run_cli_command(&mut cmd2).unwrap();
    assert!(output2.status.success(), "resume agent9527-cli run failed");

...
use agent9527_image_generation_extension::install as install_image_generation_extension;
...
const RESPONSES_LITE_HEADER: &str = "x-openai-internal-agent9527-responses-lite";

fn responses_extensions(auth: &Agent9527Auth) -> Arc<ExtensionRegistry<Config>> {
...
fn configure_image_capable_model(model_info: &mut agent9527_protocol::openai_models::ModelInfo) {
...
async fn responses_lite_uses_input_items_for_instructions_and_tools() -> Result<()> {
    ...
    let turn_metadata: Value = serde_json::from_str(
        client_metadata["x-agent9527-turn-metadata"]
            .as_str()
    ...
    assert!(!client_metadata.contains_key("x-agent9527-code-mode-tool-names"));
...
async fn responses_lite_prepares_images() -> Result<()> {
    ...
    let mut builder = test_agent9527().with_model_info_override("gpt-5.4", |model_info| {
        model_info.use_responses_lite = true;

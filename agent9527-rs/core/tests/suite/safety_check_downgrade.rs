...
const SERVER_MODEL: &str = agent9527_models_manager::MODEL_BALANCED;
const REQUESTED_MODEL: &str = agent9527_models_manager::MODEL_PRO;
const MODEL_WITH_BUNDLED_METADATA: &str = "gpt-5.4";
const TRUSTED_ACCESS_FOR_CYBER_VERIFICATION: &str = "trusted_access_for_cyber";

const CYBER_POLICY_MESSAGE: &str =
    "This request has been flagged for potentially high-risk cyber activity.";

fn disabled_text_turn(test: &TestAgent9527, text: &str) -> Op {
...
async fn openai_model_header_mismatch_emits_warning_event() -> Result<()> {
    ...
    let mut builder = test_agent9527().with_model(REQUESTED_MODEL);
    ...
    let reroute = wait_for_event(&test.agent9527, |event| {
    ...
    let _ = wait_for_event(&test.agent9527, |event| {
...
async fn cyber_policy_response_emits_typed_error_without_retry() -> Result<()> {
    ...
    let error = wait_for_event(&test.agent9527, |event| matches!(event, EventMsg::Error(_))).await;
    let EventMsg::Error(error) = error else {
        panic!("expected error event");
    };
    assert_eq!(error.message, CYBER_POLICY_MESSAGE);
    assert_eq!(
        error.agent9527_error_info,
...
async fn model_verification_emits_structured_event_without_reroute_or_warning() -> Result<()> {
    ...
    let mut builder = test_agent9527().with_model(MODEL_WITH_BUNDLED_METADATA);
    let test = builder.build(&server).await?;
    ...
    assert_eq!(reroute_count, 0);
    assert_eq!(warning_messages, Vec::<String>::new());

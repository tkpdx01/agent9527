...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let initial_thread = Arc::clone(&initial.agent9527);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.agent9527, "after resume").await?;
    shutdown_thread(&resumed.agent9527).await?;
...
async fn submit_user_turn(agent9527: &Arc<Agent9527Thread>, text: &str) -> Result<()> {
    agent9527
        .submit(Op::UserInput {
...
async fn submit_compact_turn(agent9527: &Arc<Agent9527Thread>) -> Result<()> {
    agent9527.submit(Op::Compact).await?;
    let warning_event =
        wait_for_event(agent9527, |event| matches!(event, EventMsg::Warning(_))).await;
...
async fn shutdown_thread(agent9527: &Arc<Agent9527Thread>) -> Result<()> {
...
fn window_id_parts(request: &ResponsesRequest) -> (String, u64) {
    let window_id = request
        ...
        .expect("missing x-agent9527-window-id header");

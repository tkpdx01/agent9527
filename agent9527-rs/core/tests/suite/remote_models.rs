...
use agent9527_protocol::openai_models::default_input_modalities;
...
const REMOTE_MODEL_SLUG: &str = "agent9527-test";
...
async fn remote_models_get_model_info_uses_longest_matching_prefix() -> Result<()> {
    ...
    let specific = test_remote_model_with_policy(
        "remote-5.3-agent9527",
        ModelVisibility::List,
        /*priority*/ 1_000,
        TruncationPolicyConfig::bytes(/*limit*/ 10_000),
    );
    let specific = ModelInfo {
        display_name: "GPT 5.3 Agent9527".to_string(),
    ...
    let manager = agent9527_core::test_support::models_manager_with_provider(
        agent9527_home.path().to_path_buf(),
        agent9527_core::test_support::auth_manager_from_auth(auth),
        provider,
    );
    ...
    let model_info = manager
        .get_model_info(
            "remote-5.3-agent9527-test",
            &config.to_models_manager_config(),
        )
        .await;

    assert_eq!(model_info.slug, "remote-5.3-agent9527-test");
...
async fn remote_models_config_context_window_override_clamps_to_max_context_window() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let requested_model = "remote-context-window-test";
    ...
    let turn_started_event = wait_for_event(&agent9527, |event| {
        matches!(
            event,
...
async fn remote_models_long_model_slug_is_sent_with_custom_reasoning() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let requested_model = "remote-reasoning-agent9527-test";
    let prefix_model = "remote-reasoning-agent9527";
...
async fn namespaced_model_slug_uses_catalog_metadata_without_fallback_warning() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let requested_model = "custom/gpt-5.2-agent9527";
    let response_mock = mount_sse_once(
        &server,
        sse(vec![ev_response_created("resp-1"), ev_completed("resp-1")]),
    )
    ...
    loop {
        let event = wait_for_event(&agent9527, |_| true).await;
        match event {
...
async fn remote_models_truncation_policy_without_override_preserves_remote() -> Result<()> {
    ...
    let slug = "agent9527-test-truncation-policy";
...
async fn remote_models_truncation_policy_with_tool_output_override() -> Result<()> {
    ...
    let slug = "agent9527-test-truncation-override";
    let remote_model = test_remote_model_with_policy(
        slug,
        ModelVisibility::List,
        /*priority*/ 1,
...
async fn remote_models_hide_picker_only_models() -> Result<()> {
    ...
    let hidden = available
        .iter()
        .find(|model| model.model == "agent9527-auto-balanced")
...
fn bundled_default_model_slug() -> String {
    let mut response = bundled_models_response().expect("bundled models.json should parse");

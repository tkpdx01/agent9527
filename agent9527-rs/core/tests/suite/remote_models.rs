...
async fn remote_models_get_model_info_uses_longest_matching_prefix() -> Result<()> {
    ...
    let model_info = manager
        .get_model_info("gpt-5.3-agent9527-test", &config.to_models_manager_config())
        .await;

    assert_eq!(model_info.slug, "gpt-5.3-agent9527-test");
...
async fn remote_models_long_model_slug_is_sent_with_custom_reasoning() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let requested_model = "gpt-5.3-agent9527-test";
    let prefix_model = "gpt-5.3-agent9527";
...
fn bundled_default_model_slug() -> String {
    agent9527_core::test_support::all_model_presets()
        .iter()

...
const FIXED_CWD: &str = "/tmp/agent9527_remote_compaction_parity_workspace";
...
impl AuthCase {
    fn build(self) -> Agent9527Auth {
        match self {
            AuthCase::ChatGpt => Agent9527Auth::create_dummy_chatgpt_auth_for_testing(),
            AuthCase::ApiKey => Agent9527Auth::from_api_key("dummy"),
        }
    }
}
...
async fn run_manual_session(
    ...
    for (idx, step) in scenario.steps.iter().enumerate() {
        submit_user_input(&agent9527, user_input_for_step(scenario.name, idx, *step)).await?;
    }
...
async fn run_manual_hook_session(mode: Mode) -> Result<Value> {
    ...
    let home = harness.test().agent9527_home_path();
...
async fn build_auto_harness(mode: Mode) -> Result<TestAgent9527Harness> {
    build_harness_inner(
        mode,
        RunSettings::default(),
        /*hooks*/ false,
...
async fn build_harness_inner(
    ...
    TestAgent9527Harness::with_builder(builder.with_config(move |config| {
        config.cwd = agent9527_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
fn rollout_path(harness: &TestAgent9527Harness) -> PathBuf {
    harness
        .test()
        .session_configured
        .rollout_path
...
async fn capture_from_requests(
    mode: Mode,
    agent9527: &agent9527_core::Agent9527Thread,
    rollout_path: &Path,
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        if let Some(start_index) = start {
            text.replace_range(start_index..marker_index, "<AGENT9527_HOME>");
            search_start = start_index + "<AGENT9527_HOME>".len() + marker.len();
        } else {
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    ...
    assert_eq!(
        text,
        "file: <AGENT9527_HOME>/skills/.system/imagegen/SKILL.md and \
         <AGENT9527_HOME>/skills/custom/SKILL.md"
    );
}

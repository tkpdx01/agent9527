...
use agent9527_web_search_extension::install as install_web_search_extension;
use anyhow::Result;
use base64::Engine;
...
async fn run_code_mode_turn(
    ...
    prompt: &str,
    code: &str,
) -> Result<(TestAgent9527, ResponseMock)> {
...
async fn run_code_mode_turn_with_builder(
    ...
    prompt: &str,
    code: &str,
    mut builder: TestAgent9527Builder,
) -> Result<(TestAgent9527, ResponseMock)> {
...
async fn missing_process_host_falls_back_to_in_process_code_mode() -> Result<()> {
    ...
    let builder = test_agent9527()
        .with_model("test-gpt-5.1-agent9527")
        .with_code_mode_host_program("agent9527-code-mode-host-does-not-exist".into())
        .with_config(|config| {
            config
                .features
...
async fn missing_process_host_fails_when_in_process_fallback_is_disabled() -> Result<()> {
    ...
    assert!(
        output.contains("failed to spawn code-mode host agent9527-code-mode-host-does-not-exist")
    );
...
async fn missing_process_host_error_is_bounded_when_in_process_fallback_is_disabled() -> Result<()>
{
    ...
    let server = responses::start_mock_server().await;
    let executable = "agent9527-code-mode-host-does-not-exist";
...
"#,
    ...
    let auth = Agent9527Auth::from_api_key("dummy");
    ...
    assert_eq!(
        search_body["model"],
        serde_json::json!("test-gpt-5.1-agent9527")
    );
    assert_eq!(
...
async fn run_code_mode_turn_with_rmcp(
    ...
    run_code_mode_turn_with_rmcp_model(server, prompt, code, "test-gpt-5.1-agent9527").await
}
...
async fn run_code_mode_turn_with_rmcp_config(
    ...
    let test = builder.build(server).await?;
    wait_for_mcp_server(&test.agent9527, "rmcp").await?;
...
const tool = ALL_TOOLS.find(
  ({ name }) => name === "mcp__agent9527_apps__calendar_timezone_option_99"
);
if (!tool) {
...
"#,
        visible_tool_name = "mcp__agent9527_apps__calendar_timezone_option_99",
...
async fn code_mode_can_call_hidden_dynamic_tools() -> Result<()> {
    ...
    let code = r#"
const tool = ALL_TOOLS.find(({ name }) => name === "agent9527_app__hidden_dynamic_tool");
const out = await tools.agent9527_app__hidden_dynamic_tool({ city: "Paris" });
text(
  JSON.stringify({
    name: tool?.name ?? null,
...
"#;
    ...
    test.agent9527
        .submit(Op::UserInput {
            ...
            thread_settings: agent9527_protocol::protocol::ThreadSettingsOverrides {
                environments: Some(
                    agent9527_protocol::protocol::TurnEnvironmentSelections::new(cwd, Vec::new()),
                ),
    ...
    let turn_id = wait_for_event_match(&test.agent9527, |event| match event {
    ...
    .await;
    assert_eq!(request.namespace.as_deref(), Some("agent9527_app"));
    ...
    test.agent9527
        ...
        .await?;
    wait_for_event(&test.agent9527, |event| match event {
    ...
    assert_eq!(
        parsed.get("name"),
        Some(&Value::String(
            "agent9527_app__hidden_dynamic_tool".to_string()
        ))
    );
    assert_eq!(
        parsed.get("out"),
    ...
    assert!(
        parsed
            ...
            .is_some_and(|description| {
                description.contains("Agent9527 app tools.")
                    && description.contains("A hidden dynamic tool.")
                    && description.contains("declare const tools:")
                    && description.contains("agent9527_app__hidden_dynamic_tool(args:")
            })
    );

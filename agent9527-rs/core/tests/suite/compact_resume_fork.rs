...
//! request payload that Agent9527 would send to the model and assert that the
...
fn network_disabled() -> bool {
    std::env::var(AGENT9527_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn start_test_conversation(
    ...
    (test.home, test.config, test.thread_manager, test.agent9527)
}

async fn user_turn(conversation: &Arc<Agent9527Thread>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<Agent9527Thread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<Agent9527Thread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<Agent9527Thread>) {
    conversation
        .shutdown_and_wait()
        .await

...
async fn web_search_mode_cached_sets_external_web_access_false() {
    ...
    let test = builder
        .build(&server)
        .await
        .expect("create test Agent9527 conversation");
...
async fn web_search_tool_config_from_config_toml_is_forwarded_to_request() {
    ...
    let home = Arc::new(tempfile::TempDir::new().expect("create agent9527 home"));
    std::fs::write(
...
"#,
    ...
    let mut builder = test_agent9527().with_model("gpt-5.2").with_home(home);
    let test = builder
        .build(&server)
        .await
        .expect("create test Agent9527 conversation");

...
use agent9527_core::RolloutRecorder;
use agent9527_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(
    ...
    let sessions = agent9527_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(agent9527_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(agent9527_home, "sessions", id)
}
...
async fn upsert_thread_metadata(
    ...
    let runtime = StateRuntime::init(agent9527_home.to_path_buf(), "test-provider".to_string())
        .await
        .unwrap();
    runtime
    ...
    builder.cwd = agent9527_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_agent9527_home_directory() {
    let repo = TempDir::new().unwrap();
    let agent9527_home = repo.path().join(".agent9527");
    std::fs::create_dir_all(&agent9527_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".agent9527/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&agent9527_home, id);

    let found =
        find_thread_path_by_id_str(&agent9527_home, &id.to_string(), /*state_db_ctx*/ None)
            .await
            .unwrap();

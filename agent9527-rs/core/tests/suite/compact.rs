use agent9527_core::compact::SUMMARIZATION_PROMPT;
...
use core_test_support::test_agent9527::turn_permission_fields;
...
fn model_info_with_context_window(slug: &str, context_window: i64) -> ModelInfo {
    ...
    let mut model_info = models_response
        ...
        .cloned()
        .expect("model template missing from models.json");
...
async fn assert_compaction_uses_turn_lifecycle_id(
    agent9527: &std::sync::Arc<agent9527_core::Agent9527Thread>,
) {
    ...
    while turn_completed_id.is_none() {
        let event = agent9527.next_event().await.expect("next event");
        match event.msg {
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn Agent9527.
    ...
    agent9527.submit(Op::Compact).await.unwrap();
    let warning_event = wait_for_event(&agent9527, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    // Shut down Agent9527 to flush rollout entries before inspecting the file.
    agent9527.submit(Op::Shutdown).await.unwrap();
...
async fn manual_pre_compact_block_decision_does_not_block_compaction() {
    ...
    let completed = wait_for_event_match(&agent9527, |ev| match ev {
    ...
    wait_for_event(&agent9527, |ev| matches!(ev, EventMsg::Warning(_))).await;
    wait_for_event(&agent9527, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let agent9527 = builder.build(&server).await.unwrap().agent9527;
    ...
    let first = wait_for_event_match(&agent9527, |ev| match ev {
    ...
    let last = wait_for_event_match(&agent9527, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        agent9527_protocol::models::ResponseItem::Message {
            id: None,
        ...
        agent9527_protocol::models::ResponseItem::Compaction {
            id: None,
    ...
    let mut resume_builder = test_agent9527().with_config(move |config| {
        set_test_compact_prompt(config);
        config.model_auto_compact_token_limit = Some(limit);
        let _ = config.features.disable(Feature::RemoteCompactionV2);
    });
    ...
    wait_for_event(&resumed.agent9527, |event| {
...
async fn pre_sampling_compact_runs_on_switch_to_smaller_context_model() {
    ...
    let previous_model = "compact-large-model";
    let next_model = "compact-small-model";
    ...
    let test = builder.build(&server).await.expect("build test agent9527");
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.agent9527).await;
...
async fn pre_sampling_compact_falls_back_from_retired_previous_model_after_rename() {
    ...
    let server = MockServer::start().await;
    let retired_model = "retired-compact-model";
    let previous_model_family = "retired-compact-model";
    let renamed_model = "renamed-compact-model";
    ...
    let request_log = mount_response_sequence(
        ...
        vec![
            ...
            invalid_request_response(format!(
                "The '{retired_model}' model is not supported when using Agent9527 with a ChatGPT account."
            )),
            sse_response(sse(vec![
                json!({
                    "type": "response.output_item.done",
    ...
    let mut initial_builder = test_agent9527()
        .with_dynamic_model_catalog()
        .with_auth(Agent9527Auth::create_dummy_chatgpt_auth_for_testing())
    ...
    let initial = initial_builder
        .build(&server)
        .await
        .expect("build initial test agent9527");
    let home = initial.home.clone();
    let rollout_path = initial
        .session_configured
        .rollout_path
    ...
    let model_provider = openai_model_provider(&server);
    let mut resumed_builder = test_agent9527()
        .with_dynamic_model_catalog()
        .with_auth(Agent9527Auth::create_dummy_chatgpt_auth_for_testing())
        .with_model(retired_model)
        .with_config(move |config| {
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.agent9527).await;
...
async fn pre_sampling_compact_falls_back_after_previous_model_invalid_request_on_downshift() {
    ...
    let server = MockServer::start().await;
    let retired_model = "retired-large-compact-model";
    let previous_model_family = "retired-large-compact-model";
    let next_model = "replacement-small-compact-model";
...
async fn pre_sampling_compact_skips_when_either_comp_hash_is_missing() {
    ...
    let server = MockServer::start().await;
    let model_without_hash = "compact-model-without-hash";
    let model_with_hash = "compact-model-with-hash";
    let next_model_without_hash = "next-compact-model-without-hash";
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let reconnect_message = wait_for_event_match(&agent9527, |event| match event {
    ...
    let task_error_message = wait_for_event_match(&agent9527, |event| match event {
...
async fn manual_compact_twice_preserves_latest_user_messages() {
    ...
    assert_eq!(
        ...
        requests[1].header("x-agent9527-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        requests[2].header("x-agent9527-window-id").as_deref()
    );
    assert_ne!(
...
async fn auto_compact_clamps_config_limit_to_context_window() {
    ...
    let agent9527 = builder.build(&server).await.unwrap();

    agent9527.submit_turn("OVER_LIMIT_TURN").await.unwrap();
    agent9527
...
async fn snapshot_request_shape_manual_compact_without_previous_user_messages() {
    ...
    agent9527.submit(Op::Compact).await.expect("run /compact");
    wait_for_event(&agent9527, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn manual_compaction_keeps_the_creation_time_global_instructions() -> Result<()> {
    ...
    assert_eq!(
        test.agent9527.instruction_sources().await,
...
-> Result<()> {
    ...
    test.agent9527.flush_rollout().await?;
    ...
    let mut resume_builder = test_agent9527()
        .with_home(Arc::clone(&home))
        .with_auth(Agent9527Auth::create_dummy_chatgpt_auth_for_testing())
        .with_config(move |config| {
    ...
    assert_eq!(
        resumed.agent9527.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],

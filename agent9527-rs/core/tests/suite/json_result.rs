...
async fn agent9527_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    agent9527_returns_json_result("gpt-5.4".to_string()).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn agent9527_returns_json_result_for_gpt5_agent9527() -> anyhow::Result<()> {
    agent9527_returns_json_result("gpt-5.4".to_string()).await
}

async fn agent9527_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("agent9527_output_schema".into()))
    ...
    let message = wait_for_event(&agent9527, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;

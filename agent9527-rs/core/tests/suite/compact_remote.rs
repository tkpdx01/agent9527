...
use agent9527_core::compact::SUMMARY_PREFIX;
use agent9527_features::Feature;
use agent9527_login::Agent9527Auth;
use agent9527_login::auth::AgentIdentityAuth;
use agent9527_login::auth::AgentIdentityAuthRecord;
use agent9527_protocol::account::PlanType as AccountPlanType;
use agent9527_protocol::config_types::ServiceTier;
use agent9527_protocol::dynamic_tools::DynamicToolCallOutputContentItem;
use agent9527_protocol::dynamic_tools::DynamicToolFunctionSpec;
use agent9527_protocol::dynamic_tools::DynamicToolNamespaceSpec;
use agent9527_protocol::dynamic_tools::DynamicToolNamespaceTool;
...
use agent9527_protocol::protocol::RealtimeConversationRealtimeEvent;
use agent9527_protocol::protocol::RealtimeEvent;
use agent9527_protocol::protocol::RealtimeOutputModality;
...
use core_test_support::test_agent9527::TestAgent9527Builder;
use core_test_support::test_agent9527::TestAgent9527Harness;
use core_test_support::test_agent9527::test_agent9527 as base_test_agent9527;
...
const TURN_STATE_HEADER: &str = "x-agent9527-turn-state";
...
fn test_agent9527() -> TestAgent9527Builder {
    base_test_agent9527().with_config(|config| {
        let _ = config.features.disable(Feature::RemoteCompactionV2);
    })
}

fn remote_realtime_test_agent9527_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestAgent9527Builder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_agent9527()
        .with_auth(Agent9527Auth::from_api_key("dummy"))
...
async fn start_realtime_conversation(agent9527: &agent9527_core::Agent9527Thread) -> Result<()> {
    agent9527
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            client_managed_handoffs: false,
            flush_transcript_tail_on_session_end: false,
            agent9527_responses_as_items: false,
            agent9527_response_item_prefix: None,
            agent9527_response_handoff_mode:
                agent9527_protocol::protocol::Agent9527ResponseHandoffMode::Thinking,
            agent9527_response_handoff_channel_prefixes: None,
            model: None,
    ...
    wait_for_event_match(agent9527, |msg| match msg {
...
async fn close_realtime_conversation(agent9527: &agent9527_core::Agent9527Thread) -> Result<()> {
    agent9527.submit(Op::RealtimeConversationClose).await?;
    wait_for_event_match(agent9527, |msg| match msg {
...
async fn wait_for_turn_complete(agent9527: &agent9527_core::Agent9527Thread) {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestAgent9527Harness::with_builder(
        test_agent9527().with_auth(Agent9527Auth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let agent9527 = harness.test().agent9527.clone();
    ...
    agent9527
        ...
        .await?;
    wait_for_turn_complete(&agent9527).await;

    agent9527.submit(Op::Compact).await?;
    wait_for_turn_complete(&agent9527).await;
    ...
    assert_eq!(
        ...
        compact_request.header("x-agent9527-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        follow_up_request.header("x-agent9527-window-id").as_deref()
    );
    assert_ne!(
...
async fn remote_compact_uses_agent_identity_assertion() -> Result<()> {
    ...
    let harness = TestAgent9527Harness::with_builder(
        test_agent9527().with_auth(Agent9527Auth::AgentIdentity(
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let mut builder = test_agent9527().with_auth(auth);
    ...
    let harness = TestAgent9527Harness::with_builder(builder).await?;
    let agent9527 = harness.test().agent9527.clone();
    let image_url =
...
-> Result<()> {
    ...
    assert_remote_manual_compact_request_parity(
        Agent9527Auth::from_api_key("dummy"),
...
async fn remote_compact_v2_reuses_compaction_trigger_for_followups() -> Result<()> {
    ...
    assert!(
        compact_request
            .header("x-agent9527-beta-features")
            .as_deref()
    ...
    assert_eq!(
        compact_request.body_json()["client_metadata"]["x-agent9527-window-id"].as_str(),
        compact_metadata["window_id"].as_str()
    );
    assert_eq!(
        compact_metadata["compaction"],
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let dynamic_tools = vec![DynamicToolSpec::Namespace(DynamicToolNamespaceSpec {
        name: "agent9527_app".to_string(),
        description: "Agent9527 app tools.".to_string(),
        tools: vec![
    ...
    let new_thread = test
        ...
        .await?;
    test.agent9527 = new_thread.thread;
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "agent9527_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "agent9527_app"),
        vec![visible_tool.to_string()]
    );
...
async fn remote_compact_does_not_charge_inline_audio_payload_as_text() -> Result<()> {
    ...
    test.agent9527
        ...
        .await?;
    let EventMsg::DynamicToolCallRequest(request) = wait_for_event(&test.agent9527, |event| {
    ...
    test.agent9527
        ...
        .await?;
    wait_for_turn_complete(&test.agent9527).await;
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&agent9527, |event| match event {
...
async fn auto_remote_compact_failure_stops_agent_loop() -> Result<()> {
    ...
    let error_message = wait_for_event_match(&agent9527, |event| match event {
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestAgent9527Harness::with_builder(
        test_agent9527()
            .with_auth(Agent9527Auth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    .await?;
    let baseline_agent9527 = baseline_harness.test().agent9527.clone();
    ...
    baseline_agent9527
        ...
        .await?;
    wait_for_event(&baseline_agent9527, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    baseline_agent9527.submit(Op::Compact).await?;
    wait_for_event(&baseline_agent9527, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let override_harness = TestAgent9527Harness::with_builder(
        test_agent9527()
            .with_auth(Agent9527Auth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
    ...
    .await?;
    let override_agent9527 = override_harness.test().agent9527.clone();
    ...
    override_agent9527
        ...
        .await?;
    wait_for_event(&override_agent9527, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    override_agent9527.submit(Op::Compact).await?;
    wait_for_event(&override_agent9527, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_manual_compact_emits_context_compaction_items() -> Result<()> {
    ...
    {
        let event = agent9527.next_event().await.unwrap();
        match event.msg {
...
async fn remote_compact_persists_replacement_history_in_rollout() -> Result<()> {
    ...
    agent9527.submit(Op::Shutdown).await?;
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    initial.agent9527.submit(Op::Compact).await?;
    wait_for_event(&initial.agent9527, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.agent9527, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    let mut builder =
        remote_realtime_test_agent9527_builder(&realtime_server).with_config(|config| {
    ...
    start_realtime_conversation(test.agent9527.as_ref()).await?;
    ...
    close_realtime_conversation(test.agent9527.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_agent9527_builder(&realtime_server).with_config({
...
async fn active_realtime_does_not_diff_changed_start_instructions_after_resume() -> Result<()> {
    ...
    let mut initial_builder = remote_realtime_test_agent9527_builder(&initial_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(initial.agent9527.as_ref()).await?;
    initial.submit_turn("USER_ONE").await?;
    close_realtime_conversation(initial.agent9527.as_ref()).await?;
    initial.agent9527.submit(Op::Shutdown).await?;
    wait_for_event(&initial.agent9527, |ev| {
        matches!(ev, EventMsg::ShutdownComplete)
    })
    ...
    let mut resume_builder = remote_realtime_test_agent9527_builder(&resumed_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(resumed.agent9527.as_ref()).await?;
    ...
    close_realtime_conversation(resumed.agent9527.as_ref()).await?;
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_agent9527_builder(&realtime_server);
    let test = builder.build(&server).await?;

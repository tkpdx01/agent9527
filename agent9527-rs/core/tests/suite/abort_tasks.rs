...
async fn interrupt_long_running_tool_emits_turn_aborted() {
    ...
    wait_for_event(&agent9527, |ev| matches!(ev, EventMsg::ExecCommandBegin(_))).await;

    agent9527.submit(Op::Interrupt).await.unwrap();

    // Expect TurnAborted soon after.
    wait_for_event(&agent9527, |ev| matches!(ev, EventMsg::TurnAborted(_))).await;
}
...
async fn interrupt_tool_records_history_entries() {
    ...
    let fixture = test_agent9527()
        ...
        .await
        .unwrap();
    let agent9527 = Arc::clone(&fixture.agent9527);

...
async fn apps_guidance_appears_after_background_recovery_within_a_turn() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    let mcp_runtime = test.agent9527.current_mcp_runtime().await;
...
-> Result<()> {
    ...
    let runtime_mcp_config = test.agent9527.runtime_mcp_config(&test.config).await;
    let refresh_config = McpServerRefreshConfig {
        mcp_servers: serde_json::to_value(agent9527_mcp::configured_mcp_servers(
            &runtime_mcp_config,
        ))?,
        mcp_oauth_credentials_store_mode: serde_json::to_value(

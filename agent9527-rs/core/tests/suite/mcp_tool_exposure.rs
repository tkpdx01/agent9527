...
async fn rapid_mcp_refreshes_coalesce_to_the_latest_config() -> Result<()> {
    ...
    let test = core_test_support::test_agent9527::test_agent9527()
...
async fn out_of_band_resource_read_reconciles_the_published_mcp_runtime() -> Result<()> {
    ...
    let user_config_path = refresh_config.agent9527_home.join("config.toml");
...
"#,
    ...
    let refreshed_servers = user_config
        .get("mcp_servers")
        .cloned()
        .map(HashMap::<String, agent9527_config::types::McpServerConfig>::deserialize)
        .transpose()?
    ...
    test.agent9527.refresh_runtime_config(refresh_config).await;
    test.agent9527.submit(Op::RefreshMcpServers).await?;
...
async fn code_mode_only_exposes_direct_model_only_mcp_namespaces() -> Result<()> {
    ...
    assert!(
        exec_description.is_some_and(|description| {
            !description.contains("mcp__agent9527_apps__calendar_create_event(args:")
        }),
...
async fn deferred_tool_world_state_tracks_initial_unchanged_and_removed_namespaces() -> Result<()> {
    ...
    wait_for_mcp_server(&test.agent9527, AGENT9527_APPS_MCP_SERVER_NAME).await?;
...
async fn initially_empty_deferred_tool_world_state_is_not_rendered_or_persisted() -> Result<()> {
    ...
    let mut builder = core_test_support::test_agent9527::test_agent9527()
        .with_config(enable_deferred_tool_world_state_without_agents);
    let test = builder.build_with_auto_env(&server).await?;
...
async fn deferred_tool_world_state_survives_resume_without_duplicate_updates() -> Result<()> {
    ...
    wait_for_mcp_server(&initial.agent9527, AGENT9527_APPS_MCP_SERVER_NAME).await?;
    initial
        .submit_turn("inspect deferred tools before resume")
        .await?;

    initial.agent9527.ensure_rollout_materialized().await;
    initial.agent9527.flush_rollout().await?;
    ...
    assert_eq!(
        persisted_tools,
        json!({
            "mcp__agent9527_apps__calendar": "Plan events and manage your calendar."
        })
    );
    drop(initial);
    ...
    wait_for_mcp_server(&resumed.agent9527, AGENT9527_APPS_MCP_SERVER_NAME).await?;
    resumed
...
async fn apps_guidance_and_deferred_namespace_appear_after_recovery_within_a_turn() -> Result<()> {
    ...
    test.agent9527
        ...
        .await?;
    let EventMsg::RequestUserInput(request) = wait_for_event(&test.agent9527, |event| {
    ...
    wait_for_event(&test.agent9527, |event| {
        matches!(
            ...
            EventMsg::McpStartupUpdate(update)
                if update.server == AGENT9527_APPS_MCP_SERVER_NAME
                    && matches!(update.status, agent9527_protocol::protocol::McpStartupStatus::Ready)
        )
    })
    .await;
...
-> Result<()> {
    ...
    tokio::fs::remove_dir_all(
        test.agent9527_home_path()
            .join("cache/agent9527_apps_tools"),
    )
    .await?;
    startup_control.fail_next_initialize_attempts(/*attempts*/ 1);
    test.agent9527.submit(Op::RefreshMcpServers).await?;

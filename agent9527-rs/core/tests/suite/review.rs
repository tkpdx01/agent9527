...
use agent9527_core::REVIEW_PROMPT;
...
use agent9527_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
...
use agent9527_protocol::protocol::ReviewCodeLocation;
use agent9527_protocol::protocol::ReviewFinding;
use agent9527_protocol::protocol::ReviewLineRange;
use agent9527_protocol::protocol::ReviewOutputEvent;
...
use agent9527_protocol::review_format::render_review_output_text;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under Agent9527 sandbox network restrictions.
    skip_if_no_network!();
    ...
    let agent9527 = new_conversation_for_server(&server, agent9527_home.clone(), |_| {}).await;
    ...
    let entered_started = wait_for_event(&agent9527, |ev| {
        matches!(
            ev,
    ...
    let entered_completed = wait_for_event(&agent9527, |ev| {
        matches!(
            ev,
    ...
    let entered = wait_for_event(&agent9527, |ev| {
    ...
    let exited_started = wait_for_event(&agent9527, |ev| {
        matches!(
            ev,
            EventMsg::ItemStarted(event)
    ...
    let exited_completed = wait_for_event(&agent9527, |ev| {
        matches!(
            ev,
            EventMsg::ItemCompleted(event)
                if matches!(event.item, TurnItem::ExitedReviewMode(_))
    ...
    let closed = wait_for_event(&agent9527, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    ...
    let _complete = wait_for_event(&agent9527, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    let path = agent9527.rollout_path().expect("rollout path");
...
async fn cancelled_review_does_not_forward_delegate_mcp_startup() {
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            match agent9527.next_event().await.expect("review event").msg {
    ...
    agent9527.submit(Op::Interrupt).await.unwrap();
...
async fn review_op_with_plain_text_emits_review_fallback() {
    ...
    let _entered = wait_for_event(&agent9527, |ev| {
        matches!(ev, EventMsg::EnteredReviewMode(_))
    })
    .await;
    let closed = wait_for_event(&agent9527, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let agent9527 = new_conversation_for_server(&server, agent9527_home.clone(), |cfg| {
    ...
    .await;
    let _closed = wait_for_event(&agent9527, |ev| {
        matches!(
            ev,
...
async fn review_uses_session_model_when_review_model_unset() {
    ...
    let session_model = "review-session-model";
    let (server, request_log) =
        start_responses_server_with_sse(completed_sse(), /*expected_requests*/ 1).await;
    let agent9527_home = Arc::new(TempDir::new().unwrap());
    let agent9527 = new_conversation_for_server(&server, agent9527_home.clone(), |cfg| {
        cfg.model = Some(session_model.to_string());
    ...
    assert_eq!(request.path(), "/v1/responses");
    let body = request.body_json();
    assert_eq!(body["model"].as_str().unwrap(), session_model);
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = agent9527_home.path().join("resume.jsonl");
    {
        ...
        let user = agent9527_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = agent9527_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let agent9527 = resume_conversation_for_server(
        &server,
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let agent9527 = new_conversation_for_server(&server, agent9527_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, agent9527_home, resume_path)
        .await

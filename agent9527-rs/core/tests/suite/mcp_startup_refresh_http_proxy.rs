...
const PROXY_TEST_SUBPROCESS_ENV_VAR: &str = "AGENT9527_MCP_HTTP_PROXY_TEST_SUBPROCESS";
...
const SERVER_URL: &str = "http://mcp-proxy.invalid/api/agent9527/ps/mcp";
...
async fn local_mcp_startup_and_refresh_use_configured_http_client() -> Result<()> {
    ...
    if std::env::var_os(PROXY_TEST_SUBPROCESS_ENV_VAR).is_none() {
        ...
        command
            ...
            .env("http_proxy", proxy.uri())
            .env("NO_PROXY", agent9527_network_proxy::DEFAULT_NO_PROXY_VALUE)
            .env("no_proxy", agent9527_network_proxy::DEFAULT_NO_PROXY_VALUE);

use agent9527_config::types::Personality;
...
fn read_only_user_turn(test: &TestAgent9527, items: Vec<UserInput>, model: String) -> Op {
...
async fn model_change_appends_model_instructions_developer_message() -> Result<()> {
    ...
    let mut builder = test_agent9527().with_model("gpt-5.2");
...
async fn model_and_personality_change_only_appends_model_instructions() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    let next_model = "exp-agent9527-personality";
...
async fn unsupported_configured_service_tier_warns_at_session_start() -> Result<()> {
    ...
    let warning = wait_for_event(&test.agent9527, |event| {
        matches!(
            event,
...
async fn model_switch_to_smaller_model_updates_token_context_window() -> Result<()> {
    ...
    let large_window_event = wait_for_event(&test.agent9527, |event| {
        matches!(
            event,
    ...
    let smaller_turn_started_event = wait_for_event(&test.agent9527, |event| {
        matches!(
            event,
    ...
    let smaller_window_event = wait_for_event(&test.agent9527, |event| {
        matches!(
            event,
            EventMsg::TokenCount(token_count)
                if token_count

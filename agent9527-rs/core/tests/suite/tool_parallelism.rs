...
async fn run_turn(test: &TestAgent9527, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestAgent9527, prompt: &str) -> anyhow::Result<Duration> {
    let start = Instant::now();
    run_turn(test, prompt).await?;
    Ok(start.elapsed())
}

async fn build_agent9527_with_test_tool(
    server: &wiremock::MockServer,
) -> anyhow::Result<TestAgent9527> {
    let mut builder = test_agent9527().with_model("test-gpt-5.1-agent9527");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_agent9527_with_test_tool(&server).await?;
...
async fn shell_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    run_turn(&test, "warm up shell tool").await?;

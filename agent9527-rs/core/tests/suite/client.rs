...
use agent9527_core::ModelClient;
use agent9527_core::NewThread;
use agent9527_core::Prompt;
use agent9527_core::ResponseEvent;
use agent9527_core::StartThreadOptions;
use agent9527_core::ThreadManager;
use agent9527_core::resolve_installation_id;
use agent9527_core::thread_store_from_config;
use agent9527_extension_api::empty_extension_registry;
...
use agent9527_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID;
...
use agent9527_protocol::models::DEFAULT_IMAGE_DETAIL;
...
use agent9527_protocol::models::LocalShellAction;
use agent9527_protocol::models::LocalShellExecAction;
use agent9527_protocol::models::LocalShellStatus;
use agent9527_protocol::models::MessagePhase;
use agent9527_protocol::models::ReasoningItemContent;
use agent9527_protocol::models::ReasoningItemReasoningSummary;
use agent9527_protocol::models::ResponseItem;
use agent9527_protocol::models::WebSearchAction;
use agent9527_protocol::openai_models::InputModality;
...
use agent9527_protocol::protocol::RolloutLine;
...
use core_test_support::TestAgent9527ResponsesRequestKind;
...
use core_test_support::test_agent9527::TestAgent9527;
...
fn test_turn_responses_metadata(
    _client: &ModelClient,
    thread_id: ThreadId,
) -> agent9527_core::Agent9527ResponsesMetadata {
    ...
    test_responses_metadata(
        ...
        TestAgent9527ResponsesRequestKind::Turn,
    )
}
...
fn assert_agent9527_client_metadata(
    ...
    assert_eq!(
        client_metadata["x-agent9527-installation-id"].as_str(),
        Some(installation_id)
    );
    assert_eq!(client_metadata["session_id"].as_str(), Some(session_id));
    assert_eq!(client_metadata["thread_id"].as_str(), Some(thread_id));
    let turn_metadata_str = client_metadata["x-agent9527-turn-metadata"]
        .as_str()
        .expect("missing x-agent9527-turn-metadata client metadata");
    let turn_metadata = serde_json::from_str::<serde_json::Value>(turn_metadata_str)
        .expect("invalid x-agent9527-turn-metadata json");
    assert_eq!(
    ...
    assert_eq!(
        client_metadata["x-agent9527-window-id"].as_str(),
...
async fn openai_stateless_responses_requests_preserve_item_turn_metadata_across_turns() {
    ...
    .await;
    let test = test_agent9527().build(&server).await.unwrap();
...
async fn non_openai_responses_requests_include_item_ids_without_passthrough_metadata() {
    ...
    agent9527
        ...
        .await
        .unwrap();
    wait_for_event(&agent9527, |event| {
...
async fn response_item_ids_persist_across_resume_and_preserve_server_ids() -> anyhow::Result<()> {
    ...
    initial.agent9527.submit(Op::Shutdown).await?;
    wait_for_event(&initial.agent9527, |event| {
...
async fn synthetic_call_output_id_is_stable_across_resumes() -> anyhow::Result<()> {
    ...
    .await;
    let agent9527_home = Arc::new(TempDir::new()?);
    let mut builder = test_agent9527();
    let first = builder
        .resume(&server, Arc::clone(&agent9527_home), session_path.clone())
        .await?;

    first.submit_turn("first resume").await?;
    first.agent9527.submit(Op::Shutdown).await?;
    wait_for_event(&first.agent9527, |event| {
        matches!(event, EventMsg::ShutdownComplete)
    })
    .await;
    assert!(
    ...
    let second = builder
        .resume(&server, agent9527_home, session_path)
        .await?;
...
async fn response_item_ids_are_sent_for_all_remote_v2_compaction_requests() -> anyhow::Result<()> {
    ...
    test.agent9527.submit(Op::Compact).await?;
    wait_for_event(&test.agent9527, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
/// Writes an `auth.json` into the provided `agent9527_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        agent9527_home.path().join("auth.json"),
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: agent9527_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = agent9527_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = agent9527_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![agent9527_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = agent9527_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Agent9527 to resume from our file
    let agent9527_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_agent9527()
        .with_home(agent9527_home.clone())
        .with_pre_build_hook(|home| {
            std::fs::write(home.join("AGENTS.md"), "be nice").expect("write global instructions");
        });
    let test = builder
        .resume(&server, agent9527_home, session_path.clone())
        .await
...
async fn resume_replays_legacy_js_repl_image_rollout_shapes() {
    ...
    let agent9527_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_agent9527().with_model("gpt-5.4");
    let test = builder
        .resume(&server, agent9527_home, session_path.clone())
        .await
        .expect("resume conversation");
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let mut builder = test_agent9527().with_auth(Agent9527Auth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.agent9527_home_path().join(INSTALLATION_ID_FILENAME))
    ...
    assert_agent9527_client_metadata(
        &request_body,
...
async fn send_request_with_provider(provider: ModelProviderInfo) {
    ...
    let mut config = load_default_config_for_test(&agent9527_home).await;
    config.model_provider_id = provider.name.clone();
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort.clone();
    let summary = config.model_reasoning_summary;
    let model = agent9527_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        agent9527_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let client = ModelClient::new(
        Some(AuthManager::from_auth_for_testing(
            Agent9527Auth::from_api_key("unused-api-key"),
        )),
    ...
    let mut stream = client_session
        .stream(
            ...
            /*service_tier*/ None,
            &responses_metadata,
            &agent9527_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_agent9527()
        .with_auth(Agent9527Auth::from_api_key("Test API Key"))
        .with_config(|config| {
            config.base_instructions = Some("test instructions".to_string());
        });
    let agent9527 = builder
        .build(&server)
        .await
        .expect("create new conversation")
        .agent9527;
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut model_provider =
        built_in_model_providers(/* openai_base_url */ /*openai_base_url*/ None)["openai"].clone();
    model_provider.base_url = Some(format!("{}/api/agent9527", server.uri()));
    model_provider.supports_websockets = false;
    let mut builder = test_agent9527()
        .with_auth(create_dummy_agent9527_auth())
        .with_config(move |config| {
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/agent9527/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth = Agent9527Auth::from_auth_storage(
        agent9527_home.path(),
        AuthCredentialsStoreMode::File,
        /*chatgpt_base_url*/ None,
        AuthKeyringBackendKind::default(),
        &agent9527_login::test_support::transport_default_auth_route_config(),
    )
    .await
    .expect("Failed to load Agent9527Auth")
    .expect("No Agent9527Auth found in agent9527_home");
    let auth_manager = agent9527_core::test_support::auth_manager_from_auth(auth);
    let installation_id = resolve_installation_id(&config.agent9527_home)
        .await
        .expect("resolve installation id");
    let thread_manager = ThreadManager::new(
        &config,
        auth_manager.clone(),
        agent9527_core::build_models_manager(&config, auth_manager),
        agent9527_core::Agent9527AppsToolsCache::default(),
        SessionSource::Exec,
        Arc::new(agent9527_exec_server::EnvironmentManager::default_for_tests()),
        empty_extension_registry(),
        Arc::new(agent9527_core::test_support::EmptyUserInstructionsProvider),
...
async fn omits_apps_guidance_when_orchestrator_mcp_is_disabled() {
    ...
    assert!(
        !request.body_contains_text("mcp__agent9527_apps"),
        "did not expect agent9527_apps MCP tools when orchestrator MCP is disabled, got {:?}",
    ...
    assert!(
        ...
        "did not expect agent9527_apps resource calls: {resource_methods:?}"
    );
}
...
async fn skills_append_to_developer_message() {
    ...
    let agent9527_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = agent9527_home.path().join("skills/demo");
    ...
    let agent9527_home_path = agent9527_home.path().to_path_buf();
    let mut builder = test_agent9527()
        .with_home(agent9527_home.clone())
        .with_auth(Agent9527Auth::from_api_key("Test API Key"))
        .with_config(move |config| {
            config.cwd = agent9527_home_path.abs();
        });
    let agent9527 = builder
        .build(&server)
        .await
    ...
    let _agent9527_home_guard = agent9527_home;
}
...
async fn skills_use_aliases_in_developer_message_under_budget_pressure() {
    ...
    let agent9527_home_parent = TempDir::new().unwrap();
    let long_home_parent = agent9527_home_parent
        .path()
        .join("agent9527-home-with-long-shared-prefix-for-skill-alias-budget-test");
    std::fs::create_dir_all(&long_home_parent).expect("create long home parent");
    let agent9527_home = Arc::new(TempDir::new_in(long_home_parent).unwrap());
    let skill_root = agent9527_home.path().join("skills");
    for index in 0..12 {
    ...
    let mut builder = test_agent9527()
        ...
        .with_config(move |config| {
            config.cwd = agent9527_home_path.abs();
            let user_config_path = agent9527_home_path.join("config.toml").abs();
    ...
    let _agent9527_home_guard = agent9527_home;
    let _agent9527_home_parent_guard = agent9527_home_parent;
}
...
async fn includes_configured_max_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestAgent9527 { agent9527, .. } = test_agent9527()
        .with_model("gpt-5.4")
        .with_config(|config| {
...
async fn model_without_summary_parameter_support_omits_configured_summary() -> anyhow::Result<()> {
    ...
    let mut model_catalog =
        upstream_bundled_models_response().expect("bundled models.json should parse");
...
async fn azure_responses_request_includes_store_and_prefixed_item_ids() {
    ...
    let model_info =
        agent9527_core::test_support::construct_model_info_offline(model.as_str(), &config);
    let thread_id = ThreadId::new();
    let auth_manager = agent9527_core::test_support::auth_manager_from_auth(
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-agent9527-primary-used-percent", "12.5")
        .insert_header("x-agent9527-secondary-used-percent", "40.0")
        .insert_header("x-agent9527-primary-window-minutes", "10")
        .insert_header("x-agent9527-secondary-window-minutes", "60")
        .insert_header("x-agent9527-primary-reset-at", "1704069000")
        .insert_header("x-agent9527-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_agent9527()
        .with_auth(Agent9527Auth::from_api_key("test"))
        .with_config(move |config| {
            config.model_provider = provider;
        });
    let agent9527 = builder
    ...
    wait_for_event(&agent9527, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-agent9527-primary-used-percent", "100.0")
        .insert_header("x-agent9527-secondary-used-percent", "87.5")
        .insert_header("x-agent9527-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-agent9527-primary-window-minutes", "15")
        .insert_header("x-agent9527-secondary-window-minutes", "60")
        .insert_header("x-agent9527-credits-has-credits", "true")
        .insert_header("x-agent9527-credits-unlimited", "false")
        .insert_header("x-agent9527-credits-balance", "")
        .insert_header(
    ...
    let mut builder = test_agent9527();
    let agent9527_fixture = builder.build(&server).await?;
    let agent9527 = agent9527_fixture.agent9527.clone();
    ...
    let token_event =
        wait_for_event(&agent9527, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&agent9527, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&agent9527, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&agent9527, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = Agent9527Err::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_agent9527_auth() -> Agent9527Auth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Agent9527 sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();

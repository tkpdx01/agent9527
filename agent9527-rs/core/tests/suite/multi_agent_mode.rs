...
async fn ultra_reasoning_uses_max_and_proactive_mode() -> Result<()> {
    ...
    submit_turn(&test.agent9527, "hello", /*effort*/ None).await?;
...
async fn configured_mode_hint_uses_custom_mode_across_reasoning_efforts() -> Result<()> {
    ...
    let test = test_agent9527()
        ...
        .build(&server)
        .await?;
    submit_turn(&test.agent9527, "explicit", Some(ReasoningEffort::High)).await?;
    submit_turn(&test.agent9527, "proactive", Some(ReasoningEffort::Ultra)).await?;
...
async fn empty_configured_mode_hint_emits_no_mode_message() -> Result<()> {
    ...
    submit_turn(&test.agent9527, "hello", Some(ReasoningEffort::High)).await?;
...
async fn changing_configured_mode_hint_to_empty_emits_no_update() -> Result<()> {
    ...
    submit_turn(&initial.agent9527, "before resume", /*effort*/ None).await?;
    drop(initial);
    ...
    submit_turn(&resumed.agent9527, "after resume", /*effort*/ None).await?;
...
async fn live_mode_change_appends_mode_without_reappending_usage_hint() -> Result<()> {
    ...
    submit_turn(&test.agent9527, "proactive", /*effort*/ None).await?;
    submit_turn(&test.agent9527, "explicit", Some(ReasoningEffort::High)).await?;

...
//! - sensitive subpaths such as `.git`, `.agents`, and `.agent9527` remain
...
use agent9527_protocol::permissions::is_protected_metadata_name;
...
use agent9527_protocol::protocol::FileSystemSpecialPath;
use agent9527_protocol::protocol::WritableRoot;
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.agent9527`.
                ...
                if project_subpath != Path::new(".git")
                    ...
                    && project_subpath != Path::new(".agent9527")
                {
                    return None;
                }
    ...
    for (search_root, globs) in patterns_by_search_root {
        for path in ripgrep_files(search_root.as_path(), &globs, max_depth)? {
            ...
            if expanded_paths.len() > MAX_UNREADABLE_GLOB_MATCHES {
                return Err(Agent9527Err::Fatal(format!(
...
mod tests {
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let real_agent9527 = temp_dir.path().join("real-agent9527");
        let logical_agent9527 = logical_home.join(".agent9527");
        let real_memories = real_agent9527.join("memories");
        let logical_memories = logical_agent9527.join("memories");
        ...
        std::os::unix::fs::symlink(&real_agent9527, &logical_agent9527)
            .expect("create symlinked agent9527 home");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".agent9527"));
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_agent9527 = path_to_string(&temp_dir.path().join(".agent9527"));
        ...
        assert_empty_directory_mounted_read_only(&args.args, Path::new(&dot_agent9527));
        ...
        assert!(synthetic_targets.contains(&PathBuf::from(&dot_agent9527)));
        assert_eq!(
            protected_create_target_paths(&args),
            Vec::<PathBuf>::new(),
    ...
    fn mounts_dev_before_writable_dev_binds() {
        ...
        assert_eq!(
            ...
            vec![
                PathBuf::from("/.git"),
                PathBuf::from("/.agents"),
                PathBuf::from("/.agent9527"),
                PathBuf::from("/dev/.git"),
                PathBuf::from("/dev/.agents"),
                PathBuf::from("/dev/.agent9527"),
            ]
        );
        assert_eq!(
            args.args,

# agent9527-linux-sandbox

This crate is responsible for producing:

- a `agent9527-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Agent9527 CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `agent9527-exec` CLI can check if its arg0 is `agent9527-linux-sandbox` and, if so, execute as if it were `agent9527-linux-sandbox`
  - this should also be true of the `agent9527` multitool CLI

On Linux, Agent9527 prefers the first `bwrap` found on `PATH`
...
the helper falls back to the bundled `agent9527-resources/bwrap` binary shipped
with Agent9527.
Agent9527 also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. Agent9527 surfaces the same startup warning
...
it cannot create the required user namespaces, so Agent9527 rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the bundled
  `agent9527-resources/bwrap` path.
- If `bwrap` is missing, Agent9527 also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, Agent9527 surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; Agent9527 rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.agent9527`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface is `agent9527 sandbox`; the host OS selects the sandbox backend.

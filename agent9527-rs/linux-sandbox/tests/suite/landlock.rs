...
fn agent9527_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_agent9527-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> agent9527_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<agent9527_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let agent9527_linux_sandbox_exe = Some(agent9527_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(
    output: &agent9527_protocol::exec_output::ExecToolCallOutput,
) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(err) => match err.details() {
            ...
            Agent9527ErrorDetails::Sandbox(SandboxErr::Timeout { .. }) => true,
...
fn expect_denied(
    result: Result<agent9527_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> agent9527_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let agent9527_linux_sandbox_exe: Option<PathBuf> = Some(agent9527_linux_sandbox_exe());
...
#[tokio::test]
async fn sandbox_blocks_git_and_agent9527_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_agent9527 = tmpdir.path().join(".agent9527");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_agent9527).expect("create .agent9527");

    let git_target = dot_git.join("config");
    let agent9527_target = dot_agent9527.join("config.toml");
    ...
    let agent9527_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", agent9527_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".agent9527 write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(agent9527_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_agent9527_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-agent9527");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_agent9527 = tmpdir.path().join(".agent9527");
    symlink(&decoy, &dot_agent9527).expect("create .agent9527 symlink");
    ...
    let agent9527_output = expect_denied(
        ...
        .await,
        ".agent9527 symlink replacement should be denied",
    );
    assert_ne!(agent9527_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_reports_agent9527_symlink_build_failure_without_panicking() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    .await
    {
        Err(err) => match err.details() {
            ...
            details => panic!(".agent9527 symlink build failure should deny: {details:?}"),
        },
        Ok(output) => panic!(".agent9527 symlink build failure should deny: {output:?}"),
    };
...
if grep -E '(^|[[:space:]])\.(git|agent9527|agents)(/|$)' status.before; then
  cat status.before
  exit 21
fi
"#,
    ...
    let mkdir_agent9527_output = expect_denied(
        run_cmd_result_with_cwd_and_writable_roots(
            &["mkdir", ".agent9527"],
            &subdir,
            std::slice::from_ref(&subdir),
        ...
        .await,
        "child .agent9527 directory creation should be denied",
    );
    assert_ne!(mkdir_agent9527_output.exit_code, 0);
    assert!(!subdir.join(".agent9527").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = agent9527_linux_sandbox_exe()
        .parent()

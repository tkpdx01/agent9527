...
use agent9527_http_client::HttpError;
use agent9527_protocol::auth::PlanType as AuthPlanType;
...
const AGENT_IDENTITY_JWT_AUDIENCE: &str = "agent9527-app-server";
const AGENT_IDENTITY_JWT_ISSUER: &str = "https://chatgpt.com/agent9527-backend/agent-identity";
...
const AGENT_IDENTITY_KEY_DERIVATION_CONTEXT: &[u8] = b"agent9527-agent-identity-ed25519-v1";
...
impl ChatGptEnvironment {
    pub fn from_chatgpt_base_url(chatgpt_base_url: &str) -> Result<Self> {
        match chatgpt_base_url.trim_end_matches('/') {
            ...
            | "https://chatgpt.com/agent9527"
            | "https://chatgpt.com/backend-api/agent9527"
            | "https://chat.openai.com"
            | "https://chat.openai.com/backend-api"
            | "https://chat.openai.com/agent9527"
            | "https://chat.openai.com/backend-api/agent9527" => Ok(Self::Production),
            "https://chatgpt-staging.com"
            | "https://chatgpt-staging.com/backend-api"
            | "https://chatgpt-staging.com/agent9527"
            | "https://chatgpt-staging.com/backend-api/agent9527" => Ok(Self::Staging),
            _ => anyhow::bail!(
...
/// single Agent9527 run, while the agent runtime id and private key are the
...
pub fn build_abom(session_source: SessionSource) -> AgentBillOfMaterials {
    AgentBillOfMaterials {
        ...
        agent_harness_id: match &session_source {
            SessionSource::VSCode => "agent9527-app".to_string(),
            SessionSource::Cli
            | SessionSource::Exec
            | SessionSource::Mcp
            ...
            | SessionSource::Unknown => "agent9527-cli".to_string(),
        },
...
mod tests {
    ...
    use agent9527_protocol::auth::KnownPlan;
...
-----END PRIVATE KEY-----"#,
    ...
    fn chatgpt_environment_maps_known_urls_to_authapi() -> anyhow::Result<()> {
        assert_eq!(
            ChatGptEnvironment::from_chatgpt_base_url("https://chatgpt.com/backend-api/agent9527")?,
    ...
    fn agent_identity_jwks_url_uses_jwt_issuer_base_url() {
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/agent9527"),
            "http://localhost:8080/api/agent9527/agent-identities/jwks"
        );
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/agent9527/"),
            "http://localhost:8080/api/agent9527/agent-identities/jwks"

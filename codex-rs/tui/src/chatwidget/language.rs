...
impl ChatWidget {
    pub(crate) fn open_language_picker(&mut self) {
            ...
            Ok(candidates) if candidates.is_empty() => {
                items.push(SelectionItem {
                    ...
                    disabled_reason: Some(localizer.text(
                        "language-picker-install-with-codex",
                        None,
                        || "Install language packs under CODEX_HOME/languages.".to_string(),
                    )),
                    ..Default::default()
                });
            }
        ...
        let subtitle = localizer.text("language-picker-subtitle", None, || {
            "Language packs are loaded independently. Restart Codex after selection."
                .to_string()
        });

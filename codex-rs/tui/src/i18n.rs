...
const UI_LANGUAGE_ENV: &str = "CODEX_UI_LANGUAGE";
const SYSTEM_LOCALE_ENV: &str = "CODEX_SYSTEM_LOCALE";
...
impl Localizer {
    ...
    pub(crate) fn from_runtime() -> Self {
        ...
        let requested_locale = explicit_locale()
            .or_else(|| read_language_preference(&codex_home))
...
fn read_language_preference(codex_home: &Path) -> Option<String> {
...
pub(crate) fn save_language_preference(
    ...
    fs::write(
        language_preference_path(codex_home),
        format!("{locale}\n"),
    )

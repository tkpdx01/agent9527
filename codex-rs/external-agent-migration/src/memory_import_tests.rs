...
fn copies_only_selected_projects_and_recopies_changed_content() {
    ...
    assert_eq!(
        projects_needing_import(&codex_home, &all_files)
    ...
    assert_eq!(
        projects_needing_import(&codex_home, &updated_files)
    ...
    let outcome = copy_resources(&codex_home, &changed_files, &selected_memory)

...
async fn run_memory_phase_one_model_request_test(
    ...
    let test =
        build_test_codex_with_memories_config(server, Arc::clone(&home), memories).await?;
...
async fn init_state_db(home: &Arc<TempDir>) -> anyhow::Result<Arc<codex_state::StateRuntime>> {
    let db = codex_state::StateRuntime::init(home.path().to_path_buf(), "test-provider".into())
        .await?;

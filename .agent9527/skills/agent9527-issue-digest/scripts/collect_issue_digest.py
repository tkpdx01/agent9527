#!/usr/bin/env python3
"""Collect recent openai/agent9527 issue activity for owner-focused digests."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Collect recent GitHub issue activity for a Agent9527 owner digest."
    )
    parser.add_argument(
        "--repo", default="openai/agent9527", help="OWNER/REPO, default openai/agent9527"
    )
    parser.add_argument(
        "--labels",
        nargs="+",
...
def clean_title_for_description(title):
    ...
    cleaned = re.sub(
        r"^(agent9527(?: desktop| app|\.app| cli)?|desktop|windows agent9527 app)\s*[:,-]\s*",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )

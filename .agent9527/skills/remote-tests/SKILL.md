...
Use `TestAgent9527Builder::build_with_auto_env()` to opt-in to remote execution in core integration
...
`$AGENT9527_HOME/environments.toml` or will define custom environments at runtime.
...
in bash also provides the `agent9527_remote_env_cleanup` function to use after testing.
...
bash -c '
  ...
  trap agent9527_remote_env_cleanup EXIT

  cd agent9527-rs
  just test -p agent9527-core --test all
'
```
...
bash -c '
  ...
  cd agent9527-rs
  just test -p agent9527-app-server --test all
'
```
...
```sh
bazel test //agent9527-rs/app-server:app-server-all-wine-exec-test
```
...
You can list devboxes via `applied_devbox ls`, pick the one with `agent9527` in the name.
Connect to devbox via `ssh <devbox_name>`.
Reuse the same checkout of agent9527 in `~/code/agent9527`. Reset files if needed. Multiple checkouts take longer to build and take up more space.

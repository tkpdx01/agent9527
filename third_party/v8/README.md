...
`RUSTY_V8_ARCHIVE`/`RUSTY_V8_SRC_BINDING_PATH` with Agent9527 release assets. Bazel
...
1. Bump the `v8` crate version and refresh `agent9527-rs/Cargo.lock`.
...
by Agent9527's Bazel Windows GNU toolchain.
...
downloaded `RUSTY_V8_SRC_BINDING_PATH` to point at those `openai/agent9527` release
...
the exact resolved `v8` crate version in `agent9527-rs/Cargo.lock`.

...
if ((Test-Path "D:\") -and (Test-DevDrive "D:")) {
    ...
    try {
        $VhdPath = Join-Path $env:RUNNER_TEMP "agent9527-dev-drive.vhdx"
        $SizeBytes = 64GB
        ...
        $Volume = $Partition | Format-Volume -FileSystem ReFS -NewFileSystemLabel "Agent9527DevDrive" -DevDrive -Confirm:$false -Force -ErrorAction Stop

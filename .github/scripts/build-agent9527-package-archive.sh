...
usage() {
  cat <<'EOF'
Usage: build-agent9527-package-archive.sh \
  --target <rust-target> \
  ...
  [--zsh-bin <path>] \
  [--zsh-manifest <path>] \
  [--agent9527-command-runner-bin <path>] \
  [--agent9527-windows-sandbox-setup-bin <path>] \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --agent9527-command-runner-bin)
      resource_args+=(
        --agent9527-command-runner-bin
        "${2:?--agent9527-command-runner-bin requires a value}"
      )
    ...
    --agent9527-windows-sandbox-setup-bin)
      resource_args+=(
        --agent9527-windows-sandbox-setup-bin
        "${2:?--agent9527-windows-sandbox-setup-bin requires a value}"
      )
...
case "$bundle" in
  primary)
    variant="agent9527"
    entrypoint="agent9527"
    archive_stem="agent9527-package"
    ;;
  app-server)
    variant="agent9527-app-server"
    entrypoint="agent9527-app-server"
    archive_stem="agent9527-app-server-package"
    ;;
  *)
    echo "No Agent9527 package variant for bundle: $bundle" >&2
    exit 1
    ;;
esac
...
code_mode_host_bin="${entrypoint_dir%/}/agent9527-code-mode-host${exe_suffix}"
...
case "$target" in
  *linux*)
    ...
    ;;
  *windows*)
    command_runner_bin="${entrypoint_dir%/}/agent9527-command-runner.exe"
    sandbox_setup_bin="${entrypoint_dir%/}/agent9527-windows-sandbox-setup.exe"
    if [[ "$command_runner_bin_provided" == "false" && -f "$command_runner_bin" ]]; then
      resource_args+=(--agent9527-command-runner-bin "$command_runner_bin")
    fi
    if [[ "$sandbox_setup_bin_provided" == "false" && -f "$sandbox_setup_bin" ]]; then
      resource_args+=(--agent9527-windows-sandbox-setup-bin "$sandbox_setup_bin")
    fi
    ;;
esac
...
python_args=(
  "${repo_root}/scripts/build_agent9527_package.py"
  --target "$target"
  --variant "$variant"

...
publishes the result via `GITHUB_ENV` as `AGENT9527_BAZEL_WINDOWS_PATH` so later
...
Write-Host 'Derived AGENT9527_BAZEL_WINDOWS_PATH entries:'
foreach ($pathEntry in $stablePathEntries) {
  Write-Host "  $pathEntry"
}
"AGENT9527_BAZEL_WINDOWS_PATH=$stablePath" | Out-File -FilePath $env:GITHUB_ENV -Encoding utf8 -Append

...
work_root="$(mktemp -d "${temp_root%/}/agent9527-zsh-release.XXXXXX")"
trap 'rm -rf "$work_root"' EXIT

source_root="${work_root}/zsh"
package_root="${work_root}/agent9527-zsh"
...
#!/usr/bin/env bash
set -euo pipefail
: "${AGENT9527_WRAPPER_LOG:?missing AGENT9527_WRAPPER_LOG}"
printf '%s\n' "$@" > "$AGENT9527_WRAPPER_LOG"
file="$1"
shift
if [[ "$#" -eq 0 ]]; then
  exec "$file"
...
AGENT9527_WRAPPER_LOG="$wrapper_log_path" \
...
(cd "$work_root" && tar -czf "${workspace}/${archive_path}" agent9527-zsh)
